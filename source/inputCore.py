...
class GlobalGestureMap:
	...
	def add(
		self, gesture: str, module: str, className: str, script: Optional[ScriptNameT], replace: bool = False
	):
...
class InputManager(baseObject.AutoPropertyObject):
	...
	def executeGesture(self, gesture):
		...
		if config.conf["keyboard"]["speakCommandKeys"] and gesture.shouldReportAsCommand:
			queueHandler.queueFunction(
				queueHandler.eventQueue, speech.speakMessage, gesture.displayName, _immediate=True
			)
