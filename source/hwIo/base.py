	...
	def write(self, data: bytes):
		...
		if not ctypes.windll.kernel32.WriteFile(self._writeFile, data, size, None, byref(self._writeOl)):
			...
			ctypes.windll.kernel32.GetOverlappedResult(
				self._writeFile, byref(self._writeOl), byref(byteData), True
			)
...
class Serial(IoBase):
	...
	def __init__(
		...
		**kwargs,
		...
		super().__init__(self._ser._port_handle, onReceive, onReadError=onReadError, ioThread=ioThread)
...
class Bulk(IoBase):
	...
	def __init__(
		...
		readHandle = CreateFile(
			readPath, winKernel.GENERIC_READ, 0, None, winKernel.OPEN_EXISTING, FILE_FLAG_OVERLAPPED, None
		)
		...
		writeHandle = CreateFile(
			writePath, winKernel.GENERIC_WRITE, 0, None, winKernel.OPEN_EXISTING, FILE_FLAG_OVERLAPPED, None
		)
