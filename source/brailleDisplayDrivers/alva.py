...
DOUBLED_KEY_COUNTS = {ALVA_THUMB_GROUP: 5, ALVA_SP_GROUP: 9}
...
class BrailleDisplayDriver(braille.BrailleDisplayDriver, ScriptableObject):
			...
			try:
				if self.isHid:
					...
					self._dev = hwIo.Serial(
						port, timeout=self.timeout, writeTimeout=self.timeout, onReceive=self._ser6OnReceive
					)
					# Get the device ID
	...
	def _hidOnReceive(self, data: bytes):
		...
		if reportID == ALVA_KEY_REPORT:
			self._handleInput(data[ALVA_KEY_REPORT_KEY_GROUP_POS], data[ALVA_KEY_REPORT_KEY_POS])
	...
	def _hidDisplay(self, cellBytes: bytes) -> None:
		for offset in range(0, len(cellBytes), ALVA_BRAILLE_OUTPUT_MAX_SIZE):
			...
			data = b"".join(
				[ALVA_BRAILLE_OUTPUT_REPORT, intToByte(offset), intToByte(len(cellsToWrite)), cellsToWrite]
			)
	...
	def _handleTime(self, time: bytes):
		...
		try:
			displayDateTime = datetime.datetime(
				year=year, month=time[2], day=time[3], hour=time[4], minute=time[5], second=time[6]
			)
		except ValueError:
	...
	def _set_hidKeyboardInput(self, state):
		...
		if self.isHid:
			...
			keySettings: int = self._dev.getFeature(ALVA_KEY_SETTINGS_REPORT)[ALVA_KEY_SETTINGS_POS]
