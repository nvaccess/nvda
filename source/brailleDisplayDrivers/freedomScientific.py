...
FOCUS_1_DOTS_TABLE = [0x01, 0x02, 0x04, 0x10, 0x20, 0x40, 0x08, 0x80]
...
class BrailleDisplayDriver(braille.BrailleDisplayDriver, ScriptableObject):
	...
	def __init__(self, port="auto"):
		...
		self.gestureMap.add(
			"br(freedomScientific):topRouting1", "globalCommands", "GlobalCommands", "braille_scrollBack"
		)
		self.gestureMap.add(
	...
	def _handlePacket(self, packetType: bytes, arg1: bytes, arg2: bytes, arg3: bytes, payload: bytes):
		...
		if packetType == FS_PKT_ACK:
			...
			manuBytes = payload[INFO_MANU_START:INFO_MANU_END].replace(FS_BYTE_NULL, b"")
			self._manufacturer = manuBytes.decode()
			modelBytes = payload[INFO_MODEL_START:INFO_MODEL_END].replace(FS_BYTE_NULL, b"")
			self._model = modelBytes.decode()
			firmwareBytes = payload[INFO_VERSION_START:INFO_VERSION_END].replace(FS_BYTE_NULL, b"")
			...
			try:
				...
				isDown, isRight = [(False, False), (True, False), (True, True), (False, True)][wheelNumber]
			except IndexError:
	...
	def _handleExtendedKeys(self, keyBits: int):
		...
		keyBits, isRelease, keyBitsBeforeRelease, newKeysPressed = self._updateKeyBits(
			keyBits, self._extendedKeyBits, 24
		)
		if newKeysPressed:
			self._ignoreKeyReleases = False
	...
	def display(self, cells: List[int]):
		...
		if not self._awaitingAck:
			self._sendPacket(FS_PKT_WRITE, intToByte(self.numCells), FS_BYTE_NULL, FS_BYTE_NULL, bytes(cells))
			self._pendingCells = []
		else:
