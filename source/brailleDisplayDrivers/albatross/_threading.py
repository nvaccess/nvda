		...
		while not self._event.is_set():
			...
			if not self._dev.is_open:
				...
				if not self._portPresent():
					log.debug(f"Sleepin {KC_INTERVAL} seconds, port {self._dev.name} not present")
				...
				self._readFunction()
				if not self._dev.is_open:
					log.debug(f"Sleepin {KC_INTERVAL} seconds, port {self._dev.name} not open")
					self._event.wait(KC_INTERVAL)
					continue
			...
			try:
				...
				result = ctypes.windll.kernel32.WaitCommEvent(
					self._dev._port_handle, byref(dwEvtMask), byref(self._dev._overlapped_read)
				)
				...
				result = ctypes.windll.kernel32.GetOverlappedResult(
					self._dev._port_handle, byref(self._dev._overlapped_read), byref(data), True
				)
				if result:
...
class RepeatedTimer:
	...
	def __init__(self, interval: float, feedFunction: Callable[[], None]):
		"""Constructor.
