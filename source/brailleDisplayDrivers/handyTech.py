...
class InvisibleDriverWindow(windowUtils.CustomWindow):
	...
	def windowProc(self, hwnd: int, msg: int, wParam: int, lParam: int):
		if msg == self.window_message:
			...
			if instanceCount == 0:
				log.error("Received Handy_Tech_Server window message while no driver instances are alive")
...
class Model(AutoPropertyObject):
	...
	def display(self, cells: List[int]):
		...
		self._display.sendExtendedPacket(HT_EXTPKT_BRAILLE, cellBytes)
...
class TimeSyncFirmnessMixin(object):
	...
	def syncTime(self, dt: datetime.datetime):
		...
		timeList: List[int] = [dt.year & 0xFF, dt.year >> 8, dt.month, dt.day, dt.hour, dt.minute, dt.second]
...
class Activator(
	ActiveSplitMixin, TimeSyncFirmnessMixin, AtcMixin, JoystickMixin, TripleActionKeysMixin, Model
):
...
class ActivatorPro(ActiveSplitMixin, TimeSyncFirmnessMixin, AtcMixin, TripleActionKeysMixin, Model):
...
class BrailleDisplayDriver(braille.BrailleDisplayDriver, ScriptableObject):
	...
	def sendExtendedPacket(self, packetType: bytes, data: bytes = b""):
		if BrailleDisplayDriver._sleepcounter > 0:
			...
			return
		packetBytes: bytes = b"".join([intToByte(len(data) + len(packetType)), packetType, data, b"\x16"])
		if self._model:
	...
	def _handleKeyRelease(self):
		...
		try:
			inputCore.manager.executeGesture(InputGesture(self._model, self._keysDown, self.brailleInput))
	...
	def _handleInputStream(self, htPacketType: bytes, stream):
		if htPacketType in (HT_PKT_OK, HT_PKT_EXTENDED, HT_PKT_OK_WITH_LENGTH):
			...
			if not self._model:
				if modelId not in MODELS:
					...
					raise RuntimeError("The model with ID %r is not supported by this driver" % modelId)
		...
		if htPacketType == HT_PKT_OK:
			...
			if extPacketType == HT_EXTPKT_CONFIRMATION:
				...
				log.debugWarning("Unhandled extended packet of type %r: %r" % (extPacketType, packet))
		else:
