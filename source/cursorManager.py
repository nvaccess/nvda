...
class FindDialog(
	...
	def onOk(self, evt):
		...
		core.callLater(
			100, self.activeCursorManager.doFindText, text, caseSensitive=caseSensitive, reverse=self.reverse
		)
		self.Destroy()
...
class CursorManager(documentBase.TextContainerObject, baseObject.ScriptableObject):
	...
	def script_moveByPage_back(self, gesture):
		self._caretMovementScriptHelper(
			gesture, textInfos.UNIT_LINE, -config.conf["virtualBuffers"]["linesPerPage"], extraDetail=False
		)
	...
	def script_moveByCharacter_back(self, gesture):
		self._caretMovementScriptHelper(
			gesture, textInfos.UNIT_CHARACTER, -1, extraDetail=True, handleSymbols=True
		)
	...
	def script_moveByWord_back(self, gesture):
		self._caretMovementScriptHelper(
			gesture, textInfos.UNIT_WORD, -1, extraDetail=True, handleSymbols=True
		)
	...
	def script_selectPage_forward(self, gesture):
		self._selectionMovementScriptHelper(
			unit=textInfos.UNIT_LINE, direction=config.conf["virtualBuffers"]["linesPerPage"]
		)
	...
	def script_selectToBottomOfDocument(self, gesture):
		self._selectionMovementScriptHelper(
			toPosition=textInfos.POSITION_LAST, unit=textInfos.UNIT_CHARACTER, direction=1
		)
...
class ReviewCursorManager(CursorManager):
	...
	def initCursorManager(self):
		...
		self.TextInfo = type(
			"ReviewCursorManager_%s" % realTI.__name__, (_ReviewCursorManagerTextInfo, realTI), {}
		)
