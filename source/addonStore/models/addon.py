...
class _AddonStoreModel(_AddonGUIModel):
	...
	def tempDownloadPath(self) -> str:
		...
		"""
		return os.path.join(WritePaths.addonStoreDownloadDir, f"{self.name}.download")
	...
	def cachedDownloadPath(self) -> str:
		...
		"""
		return os.path.join(
			WritePaths.addonStoreDownloadDir, f"{self.name}-{self.addonVersionName}.nvda-addon"
		)
