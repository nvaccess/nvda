...
def registerArm64Server(fileName: str) -> None:
	"""Registers the COM proxy dll with the given file name, using the ARM64 version of regsvr32.

	:param fileName: The ARM64 path to the DLL
	"""
	# NVDA is 64 bit; on ARM64 Windows, System32 contains the ARM64 binaries.
	...
	try:
		...
		log.error(f"Error registering {fileName} in an ARM64 context: {e}")
	else:
		log.debug(f"Registered {fileName} in an ARM64 context.")
...
def applyArm64RegistryPatch(fileName: str) -> None:
	"""Applies the registry patch with the given file name, using ARM64 reg.exe.

	:param fileName: The ARM64 path to the .reg file
	"""
	if not os.path.isfile(fileName):
		raise FileNotFoundError(f"Cannot apply ARM64 registry patch: {fileName} not found.")
	# NVDA is 64 bit; on ARM64 Windows, System32 contains the ARM64 binaries.
	regExe = os.path.join(SYSTEM32, "reg.exe")
	# Make sure a console window doesn't show when running reg.exe
	startupInfo = subprocess.STARTUPINFO()
	startupInfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
	startupInfo.wShowWindow = subprocess.SW_HIDE
	try:
		...
		log.error(f"Error applying ARM64 registry patch from {fileName} with {regExe}: {e}")
	else:
		log.debug(f"Applied ARM64 registry patch from {fileName}")
...
def fixCOMRegistrations() -> None:
	...
	isArm64 = winVer.processorArchitecture == "ARM64"
	...
	if isArm64:
		applyArm64RegistryPatch(OLEACC_REG_FILE_PATH)
	...
	if isArm64:
		registerArm64Server(os.path.join(SYSTEM32, "oleaut32.dll"))
		registerArm64Server(os.path.join(SYSTEM32, "actxprxy.dll"))
