...
try:
    ...
    raise ImportError(
        "Mos Eisley requires pywin32. Install with: pip install pywin32"
    ) from e

__version__ = "1.0.0"
__all__ = ["SandboxConfig", "SandboxPopen", ]
...
UpdateProcThreadAttribute.argtypes = [
    LPPROC_THREAD_ATTRIBUTE_LIST, DWORD, DWORD, PVOID, SIZE_T, PVOID, PSIZE_T
]
...
CreateProcess.argtypes = [
    ...
    BOOL, DWORD, LPVOID, LPCWSTR, POINTER(STARTUPINFOEX),
...
def create_well_known_sid(well_known_sid_type, domain_sid=None):
    ...
    advapi32.CreateWellKnownSid(well_known_sid_type, domain_sid,
                                None, ctypes.byref(cbSid))
    if cbSid.value:
        Sid = (ctypes.c_char * cbSid.value)()
        if advapi32.CreateWellKnownSid(well_known_sid_type, domain_sid,
                                       Sid, ctypes.byref(cbSid)):
            return Sid
...
class SandboxConfig:
    ...
    def log_config(self):
        ...
        logger.info(
            f"  Restricted Token: {'ENABLED' if self.enable_restricted_token else 'DISABLED'}")
        logger.info(
            f"  Privilege Removal: {'ENABLED' if self.enable_privilege_removal else 'DISABLED'}")
        logger.info(
            f"  Low Integrity: {'ENABLED' if self.enable_low_integrity else 'DISABLED'}")
        logger.info(
            f"  SID Restrictions: {'ENABLED' if self.enable_sid_restrictions else 'DISABLED'}")
        logger.info(
            f"  Restrict User SID: {'ENABLED' if self.restrict_user_sid else 'DISABLED'}")
        logger.info("Job Object Restrictions:")
        logger.info(
            f"  Job Object: {'ENABLED' if self.enable_job_object else 'DISABLED'}")
        logger.info(
            f"  Memory Limits: {'ENABLED' if self.enable_memory_limits else 'DISABLED'}")
        logger.info(
            f"  Process Limits: {'ENABLED' if self.enable_process_limits else 'DISABLED'}")
        logger.info(
...
def create_restricted_token(config: SandboxConfig):
    ...
    try:
        ...
        token_handle = win32security.OpenProcessToken(
            current_process, win32security.TOKEN_ALL_ACCESS
        )
        ...
        if config.enable_privilege_removal:
            ...
            for priv_const in privilege_constants:
                try:
                    ...
                    logger.debug(
                        f"Info: Could not lookup privilege {priv_const}: {e}")
        ...
        if config.enable_sid_restrictions:
            logger.debug(
                "Preparing SID restrictions for user directory access control...")
            try:
                ...
                if config.restrict_user_sid:
                    logger.debug(
                        f"Disabling user SID for access control: {user_sid_string}")
                    sids_to_disable.append((user_sid, 0))
                else:
                    logger.debug(
                        f"Keeping user SID (restrict_user_sid=False): {user_sid_string}")
                ...
                if "-" in user_sid_string:
                    ...
                    user_domain_groups.add(
                        f"{domain_part}-513")  # Domain Users
                    user_domain_groups.add(
                        f"{domain_part}-515")  # Domain Computers

                for group_sid, attributes in groups:
                    group_sid_string = win32security.ConvertSidToStringSid(
                        group_sid)

                    # Skip essential system groups
                    if group_sid_string in essential_group_sids:
                        logger.debug(
                            f"Keeping essential group: {group_sid_string}")
                        continue

                    # Skip integrity level SIDs
                    if group_sid_string.startswith("S-1-16-"):
                        logger.debug(
                            f"Keeping integrity level SID: {group_sid_string}")
                        continue

                    # Skip user's domain groups
                    if group_sid_string in user_domain_groups:
                        logger.debug(
                            f"Keeping user domain group: {group_sid_string}")
                        continue

                    # Keep logon session SIDs - REQUIRED for ctypes!
                    if group_sid_string.startswith("S-1-5-5-"):
                        logger.debug(
                            f"Keeping logon session SID (REQUIRED): {group_sid_string}")
                        continue
                    ...
                    should_disable = any(group_sid_string.startswith(
                        pattern) for pattern in non_essential_patterns)

                    if should_disable:
                        sids_to_disable.append((group_sid, 0))
                        logger.debug(
                            f"Will disable non-essential group: {group_sid_string}")
                    else:
                        logger.debug(
                            f"Keeping group for functionality: {group_sid_string}")

                logger.info(
                    f"SID restrictions: {len(sids_to_disable)} SIDs to disable")
        ...
        if config.enable_low_integrity:
            try:
                logger.debug(
                    "Attempting to set Low Integrity Level on token...")
                ...
                if not advapi32.SetTokenInformation(
                    ...
                    raise ctypes.WinError(
                        error_code, "SetTokenInformation for Low Integrity Level failed")
...
class SandboxPopen(subprocess.Popen):
    ...
    def __init__(self, *args, config=None, restricted_token=None, job_object=None,
                 low_integrity_dirs=None, sandbox_temp_dir=None,
                 allowed_directory=None, **kwargs):
        ...
        logger.debug(
            f"Initializing SandboxPopen with restricted token: {self.restricted_token is not None}")
        logger.debug(f"Job object: {self.job_object is not None}")
        logger.debug(
            f"Low integrity directories: {len(self.low_integrity_dirs)}")
    ...
    def _execute_child_windows(self, *args, **kwargs):
        ...
        param_names = ['args', 'executable', 'preexec_fn', 'close_fds', 'pass_fds',
        ...
        if c2pwrite is not None and c2pwrite != -1:
            ...
            startupinfo.hStdOutput = win32api.GetStdHandle(
        ...
        if errwrite is not None and errwrite != -1:
            ...
            startupinfo.hStdError = win32api.GetStdHandle(
                win32api.STD_ERROR_HANDLE)
            logger.debug(f"Set stderr to inherited handle: {startupinfo.hStdError}")

        if startupinfo.hStdInput != win32api.GetStdHandle(win32api.STD_INPUT_HANDLE) or \
           startupinfo.hStdOutput != win32api.GetStdHandle(win32api.STD_OUTPUT_HANDLE) or \
