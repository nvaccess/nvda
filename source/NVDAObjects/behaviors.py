	...
	def _reportErrorInPreviousWord(self):
		...
		def _delayedDetection():
			try:
				...
				log.debugWarning(
					"Error fetching formatting for last character of previous word", exc_info=True
				)
				return
...
class EditableTextWithoutAutoSelectDetection(
	editableText.EditableTextWithoutAutoSelectDetection, EditableText
):
		...
		while self._keepMonitoring:
			...
			try:
				...
				if config.conf["presentation"]["reportDynamicContentChanges"]:
					...
					if outLines:
						queueHandler.queueFunction(
							queueHandler.eventQueue, self._reportNewLines, outLines, _immediate=True
						)
				oldText = newText
			except:  # noqa: E722
...
class EnhancedTermTypedCharSupport(Terminal):
	...
	@script(
		gestures=["kb:enter", "kb:numpadEnter", "kb:tab", "kb:control+c", "kb:control+d", "kb:control+pause"]
	)
