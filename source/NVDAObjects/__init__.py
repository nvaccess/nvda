...
class NVDAObject(
	documentBase.TextContainerObject, baseObject.ScriptableObject, metaclass=DynamicNVDAObjectType
):
	...
	def _get_annotations(self) -> typing.Optional[AnnotationOrigin]:
		if config.conf["debugLog"]["annotations"]:
			log.debugWarning(f"Fetching annotations not supported on: {self.__class__.__qualname__}")
		return None
	...
	def _get_detailsSummary(self) -> typing.Optional[str]:
		log.warning(
			"NVDAObject.detailsSummary is deprecated. Use NVDAObject.annotations instead.", stack_info=True
		)
		return None
	...
	def hasDetails(self) -> bool:
		...
		"""
		log.warning(
			"NVDAObject.hasDetails is deprecated. Use NVDAObject.annotations instead.", stack_info=True
		)
	...
	def _get_detailsRole(self) -> typing.Optional[controlTypes.Role]:
		log.warning(
			"NVDAObject.detailsRole is deprecated. Use NVDAObject.annotations instead.", stack_info=True
		)
		if config.conf["debugLog"]["annotations"]:
	...
	def _get_simpleLastChild(self):
		...
		if presType != self.presType_content:
			return child._findSimpleNext(
				useChild=(presType != self.presType_unavailable), useParent=False, goPrevious=True
			)  # noqa: E701
		return child
