	...
	if not eventHandler.isPendingEvents("gainFocus") and (
		...
		if config.conf["inputComposition"]["autoReportAllCandidates"] and (
			...
			queueHandler.queueFunction(
				queueHandler.eventQueue, ui.message, candidateObject.visibleCandidateItemsText
			)
...
class MSCandUI21_candidateMenuItem(BaseCandidateItem):
	def _get_previous(self):
		...
		return MSCandUI21_candidateMenuItem(
			IAccessibleObject=item.IAccessibleObject, IAccessibleChildID=item.IAccessibleChildID
		)
...
class MSCandUIWindow(IAccessible):
	...
	def event_show(self):
		item = MSCandUIWindow_candidateListItem(
			IAccessibleObject=self.IAccessibleObject, IAccessibleChildID=3
		)
		reportSelectedCandidate(item)
