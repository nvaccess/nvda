...
class MozAnnotationTarget(AnnotationTarget):
	...
	def role(self) -> Optional[controlTypes.Role]:
		...
		detailsRole = IAccessibleHandler.IAccessibleRolesToNVDARoles.get(self._target.IAccessibleRole)
...
class MozAnnotation(AnnotationOrigin):
	...
	def __bool__(self) -> bool:
		...
		return bool(self._originObj.detailsRelations)
	...
	def _get_parent(self):
		...
		if not res:
			...
			res = IAccessibleHandler.accNavigate(
				self.IAccessibleObject, self.IAccessibleChildID, IAccessibleHandler.NAVRELATION_NODE_CHILD_OF
			)
		if not res:
			return None
...
class GeckoPluginWindowRoot(WindowRoot):
	...
	def _get_parent(self):
		...
		res = IAccessibleHandler.accNavigate(
			parent.IAccessibleObject, 0, IAccessibleHandler.NAVRELATION_EMBEDS
		)
		if res:
...
def findExtraOverlayClasses(obj, clsList):
	...
	ia2Web.findExtraOverlayClasses(obj, clsList, baseClass=Mozilla, documentClass=Document)
