...
class IA2TextTextInfo(textInfos.offsets.OffsetsTextInfo):
	...
	def _getOffsetFromPoint(self, x, y):
		if self.obj.IAccessibleTextObject.nCharacters > 0:
			offset = self.obj.IAccessibleTextObject.OffsetAtPoint(x, y, IA2.IA2_COORDTYPE_SCREEN_RELATIVE)
	...
	def _getBoundingRectFromOffsetInObject(cls, obj, offset):
		try:
			res = RectLTWH(
				*obj.IAccessibleTextObject.characterExtents(offset, IA2.IA2_COORDTYPE_SCREEN_RELATIVE)
			)
		except COMError:
	...
	def _getSentenceOffsets(self, offset):
		...
		try:
			start, end, text = self.obj.IAccessibleTextObject.TextAtOffset(
				offset, IA2.IA2_TEXT_BOUNDARY_SENTENCE
			)
			if start == end:
				raise NotImplementedError
			return start, end
	...
	def _getParagraphOffsets(self, offset):
		...
		try:
			start, end, text = self.obj.IAccessibleTextObject.TextAtOffset(
				offset, IA2.IA2_TEXT_BOUNDARY_PARAGRAPH
			)
			if start >= end:
	...
	def _iterTextWithEmbeddedObjects(
		self, withFields, formatConfig=None
...
class IAccessible(Window):
		...
		for key in keys:
			...
			if classString and classString.find(".") > 0:
				...
				mod = importlib.import_module(
					"NVDAObjects.IAccessible.%s" % modString, package="NVDAObjects.IAccessible"
				)
		...
		if not hasattr(IAccessible, "IID_ITextServices"):
			try:
				IAccessible.IID_ITextServices = ctypes.cast(
					ctypes.windll.msftedit.IID_ITextServices, ctypes.POINTER(GUID)
				).contents
			except WindowsError:
		...
		if IAccessible.IID_ITextServices:
			try:
				pDoc = self.IAccessibleObject.QueryInterface(IServiceProvider).QueryService(
					IAccessible.IID_ITextServices, ITextDocument
				)
			except COMError:
				pDoc = None
			if pDoc:
	...
	def _isEqual(self, other):
		...
		if isinstance(self.IAccessibleObject, IA2.IAccessible2) and isinstance(
			other.IAccessibleObject, IA2.IAccessible2
		):
	...
	def _get_states(self) -> typing.Set[controlTypes.State]:  # noqa: C901
		...
		try:
			...
			states.update(IAccessibleHandler.calculateNvdaStates(self.IAccessibleRole, IAccessibleStates))
	...
	def _get_labeledBy(self):
		try:
			(pacc, accChild) = IAccessibleHandler.accNavigate(
				self.IAccessibleObject, self.IAccessibleChildID, IAccessibleHandler.NAVRELATION_LABELLED_BY
			)
	...
	def _get_next(self):
		res = IAccessibleHandler.accNavigate(
			self.IAccessibleObject, self.IAccessibleChildID, oleacc.NAVDIR_NEXT
		)
		if not res:
			return None
	...
	def _get_previous(self):
		res = IAccessibleHandler.accNavigate(
			self.IAccessibleObject, self.IAccessibleChildID, oleacc.NAVDIR_PREVIOUS
		)
		if not res:
			return None
		if res[0] == self.IAccessibleObject:
	...
	def _get_firstChild(self):
		child = IAccessibleHandler.accNavigate(
			self.IAccessibleObject, self.IAccessibleChildID, oleacc.NAVDIR_FIRSTCHILD
		)
	...
	def _get_lastChild(self):
		child = IAccessibleHandler.accNavigate(
			self.IAccessibleObject, self.IAccessibleChildID, oleacc.NAVDIR_LASTCHILD
		)
		if not child and self.IAccessibleChildID == 0:
			try:
	...
	def _get_children(self):
		...
		for IAccessibleObject, IAccessibleChildID in IAccessibleHandler.accessibleChildren(
			self.IAccessibleObject, 0, childCount
		):
			...
			for i in range(nHeaders):
				...
				if self.IA2UniqueID is not None:  # No point checking if we don't have this cell's UID
					try:
						...
						log.debugWarning(
							"could not get IAccessible2::uniqueID to use as headerUniqueID", exc_info=True
						)
						headerUniqueID = None
	...
	def _getIA2RelationFirstTarget(
		self, relationType: typing.Union[str, "IAccessibleHandler.RelationType"]
		...
		try:
			...
			for target in targetsGen:
				...
				# Just take the first.
				return IAccessible(IAccessibleObject=ia2Object, IAccessibleChildID=0)
	...
	def _getIA2RelationTargetsOfType(
		self, relationType: Union[str, IAccessibleHandler.RelationType]
		...
		try:
			...
			for target in targetsGen:
				ia2Object = IAccessibleHandler.normalizeIAccessible(target)
				ia = IAccessible(IAccessibleObject=ia2Object, IAccessibleChildID=0)
				yield ia
			...
			for relation in self._IA2Relations:
				if relation.relationType == relationType:
					...
					for i in range(0, relation.nTargets):
						...
						ia2Object = IAccessibleHandler.normalizeIAccessible(target)
						yield IAccessible(IAccessibleObject=ia2Object, IAccessibleChildID=0)
			return
		except (NotImplementedError, COMError):
			log.debug("Unable to fetch _IA2Relations", exc_info=True)
			pass
