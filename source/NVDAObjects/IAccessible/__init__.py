...
"""  
		...
		if not windowHandle and isinstance(IAccessibleObject,IAccessibleHandler.IAccessible2):
			try:
				...
				log.warn("IAccessible2::windowHandle failed",exc_info=True)
...
"""
		...
		if event_childID is None and isinstance(IAccessibleObject,IAccessibleHandler.IAccessible2):
			try:
				...
				log.warning("could not get IAccessible2::uniqueID to use as event_childID",exc_info=True)
	...
	def _get_states(self):
		try:
			...
			log.warning("could not get IAccessible states",exc_info=True)
			states=set()
		else:
		...
		try:
			...
			log.warning("could not get IAccessible2 states",exc_info=True)
	...
	def _get_parent(self):
		...
		if res:
			try:
				...
				log.warning("parent has bad role",exc_info=True)
				return None
