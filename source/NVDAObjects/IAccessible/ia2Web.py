...
class IA2WebAnnotation(AnnotationOrigin):
	...
	def __bool__(self) -> bool:
		return bool(self._originObj.IA2Attributes.get("details-roles"))
...
class Ia2Web(IAccessible):
	...
	def _get_states(self):
		...
		if controlTypes.State.HASPOPUP in states:
			popupState = aria.ariaHaspopupValuesToNVDAStates.get(self.IA2Attributes.get("haspopup"))
			if popupState:
	...
	def event_IA2AttributeChange(self):
		...
		if self is api.getFocusObject():
			...
			speech.speakObjectProperties(self, current=True, reason=controlTypes.OutputReason.CHANGE)
