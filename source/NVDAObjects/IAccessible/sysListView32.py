...
class List(List):
			...
			if groupIndex >= 0:
				...
				if info is not None:
					...
					groupingObj = GroupingItem(
						windowHandle=self.windowHandle, parentNVDAObject=self, groupInfo=info
					)
	...
	def _getColumnOrderArrayRawOutProc(self, columnCount: int) -> Optional[ctypes.Array]:
		...
		internalCoa = winKernel.virtualAllocEx(
			processHandle, None, sizeof(coa), winKernel.MEM_COMMIT, winKernel.PAGE_READWRITE
		)  # noqa: F405
		try:
			...
			res = watchdog.cancellableSendMessage(
				self.windowHandle, LVM_GETCOLUMNORDERARRAY, columnCount, internalCoa
			)
			if res:
...
class ListItem(RowWithFakeNavigation, RowWithoutCellObjects, ListItemWithoutColumnSupport):
	...
	def _getColumnLocationRawOutProc(self, index: int) -> ctypes.wintypes.RECT:
		...
		internalRect = winKernel.virtualAllocEx(
			processHandle, None, sizeof(localRect), winKernel.MEM_COMMIT, winKernel.PAGE_READWRITE
		)  # noqa: F405
		try:
			winKernel.writeProcessMemory(
				processHandle, internalRect, byref(localRect), sizeof(localRect), None
			)  # noqa: F405
			res = watchdog.cancellableSendMessage(
				self.windowHandle, LVM_GETSUBITEMRECT, self.IAccessibleChildID - 1, internalRect
			)
			if res:
				winKernel.readProcessMemory(
					processHandle, internalRect, ctypes.byref(localRect), ctypes.sizeof(localRect), None
				)
		finally:
	...
	def _getColumnContentRawOutProc(self, index: int) -> Optional[str]:
		...
		internalItem = winKernel.virtualAllocEx(
			processHandle, None, sizeof(self.LVITEM), winKernel.MEM_COMMIT, winKernel.PAGE_READWRITE
		)  # noqa: F405
		try:
			internalText = winKernel.virtualAllocEx(
				processHandle, None, CBEMAXSTRLEN * 2, winKernel.MEM_COMMIT, winKernel.PAGE_READWRITE
			)
			try:
				item = self.LVITEM(
				...
				winKernel.writeProcessMemory(
					processHandle, internalItem, byref(item), sizeof(self.LVITEM), None
				)  # noqa: F405
				len = watchdog.cancellableSendMessage(
					self.windowHandle, LVM_GETITEMTEXTW, (self.IAccessibleChildID - 1), internalItem
				)
				if len:
					winKernel.readProcessMemory(
						processHandle, internalItem, byref(item), sizeof(self.LVITEM), None
	...
	def _getColumnImageIDRaw(self, index):
		...
		try:
			item = self.LVITEM(
				iItem=self.IAccessibleChildID - 1, mask=LVIF_IMAGE | LVIF_COLUMNS, iSubItem=index
			)
	...
	def _getColumnHeaderRawOutProc(self, index: int) -> Optional[str]:
		...
		internalColumn = winKernel.virtualAllocEx(
			processHandle, None, sizeof(self.LVCOLUMN), winKernel.MEM_COMMIT, winKernel.PAGE_READWRITE
		)  # noqa: F405
		try:
			internalText = winKernel.virtualAllocEx(
				processHandle, None, CBEMAXSTRLEN * 2, winKernel.MEM_COMMIT, winKernel.PAGE_READWRITE
			...
			try:
				column = self.LVCOLUMN(
					mask=LVCF_TEXT, iSubItem=index, pszText=internalText, cchTextMax=CBEMAXSTRLEN
				)
				winKernel.writeProcessMemory(
					processHandle, internalColumn, byref(column), sizeof(self.LVCOLUMN), None
				)  # noqa: F405
				res = watchdog.cancellableSendMessage(
					self.windowHandle, LVM_GETCOLUMNW, index, internalColumn
				)
				if res:
					winKernel.readProcessMemory(
						processHandle, internalColumn, byref(column), sizeof(self.LVCOLUMN), None
