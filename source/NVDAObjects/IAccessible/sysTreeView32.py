...
class TreeViewItem(IAccessible):
	...
	def _get_treeview_hItem(self):
		if not hasattr(self, "_treeview_hItem"):
			self._treeview_hItem = watchdog.cancellableSendMessage(
				self.windowHandle, TVM_MAPACCIDTOHTREEITEM, self.IAccessibleChildID, 0
			)
	...
	def _get_states(self):
		...
		itemStates = watchdog.cancellableSendMessage(
			self.windowHandle, TVM_GETITEMSTATE, hItem, TVIS_STATEIMAGEMASK
		)
		ch = (itemStates >> 12) & 3
		if ch > 0:
	...
	def _get_parent(self):
		...
		if not newID:
			...
			newID = parentItem
		return IAccessible(
			windowHandle=self.windowHandle, IAccessibleObject=self.IAccessibleObject, IAccessibleChildID=newID
		)
	...
	def _get_childCount(self):
		...
		while childItem > 0:
			...
			childItem = watchdog.cancellableSendMessage(
				self.windowHandle, TVM_GETNEXTITEM, TVGN_NEXT, childItem
			)
		return numItems
	...
	def _get_positionInfo(self):
		...
		while newItem > 0:
			...
			newItem = watchdog.cancellableSendMessage(
				self.windowHandle, TVM_GETNEXTITEM, TVGN_PREVIOUS, newItem
			)
		newItem = hItem
		numItems = index - 1
		while newItem > 0:
