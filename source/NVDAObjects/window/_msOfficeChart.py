...
class OfficeChart(OfficeChartElementList):
	...
	def __init__(self, windowHandle, officeApplicationObject, officeChartObject, initialDocument, keyIndex=0):
			...
			for i in range(seriesCount):
				self.addElement(
					OfficeChartElementSeries(
						...
						officeChartObject=self.officeChartObject,
						elementID=xlSeries,
						arg1=i + 1,
					),
					self,
		...
		self.addElement(
			OfficeChartElementCollection(
				windowHandle=self.windowHandle, officeChartObject=self.officeChartObject
			),
			self,
		)
		try:
	...
	def _get_name(self):
		...
		text = _("Chart title: {chartTitle}, type: {chartType}").format(
			chartTitle=name, chartType=chartTypeText
		)
		return text
		...
		if count > 0:
			...
			if count > 1:
				for i in range(1, count + 1):
					...
					seriesValueString += ", " + _("series {number} {name}").format(
						number=i, name=self.officeChartObject.SeriesCollection(i).Name
					)
...
class OfficeChartElementCollection(OfficeChartElementList):
	...
	def __init__(self, windowHandle=None, officeChartObject=None, elementID=None, arg1=None, arg2=None):
		...
		if officeChartObject.HasTitle:
			self.addElement(
				OfficeChartElementChartTitle(windowHandle=windowHandle, officeChartObject=officeChartObject),
				self,
			)

		axisAndAxisTitles = OfficeChartElementAxis.getAvailableAxisAndAxisTitle(
			windowHandle, officeChartObject
		)
		for item in axisAndAxisTitles:
			self.addElement(item, self)

		chartAreaObject = OfficeChartElementChartArea(
			windowHandle=self.windowHandle, officeChartObject=officeChartObject
		)
		chartAreaObject.reportExtraInfo = True
		self.addElement(chartAreaObject, self)
		plotAreaObject = OfficeChartElementPlotArea(
			windowHandle=self.windowHandle, officeChartObject=officeChartObject
		)
		...
		if officeChartObject.HasLegend:
			self.addElement(
				OfficeChartElementLegend(windowHandle=self.windowHandle, officeChartObject=officeChartObject),
				self,
			)
			...
			for legendEntryIndex in range(1, self.legendEntryCount + 1):
				legendEntry = OfficeChartElementLegendEntry(
					windowHandle=self.windowHandle,
					officeChartObject=self.officeChartObject,
					elementID=xlLegendEntry,
					arg1=legendEntryIndex,
					arg2=self.legendEntryCount,
				)
...
class OfficeChartElementSeries(OfficeChartElementList):
	...
	def __init__(self, windowHandle, officeChartObject, elementID, arg1=None, arg2=None):
		super(OfficeChartElementSeries, self).__init__(
			windowHandle=windowHandle, officeChartObject=officeChartObject
		)
		self.elementID = elementID
		...
		if self.officeChartObject.ChartType in (xlPie, xlPieExploded, xlPieOfPie):
			...
			ui.message(
				_("Slice color: {colorName} ").format(
					colorName=colors.RGB.fromCOLORREF(
						int(
							self.officeChartObject.SeriesCollection(self.seriesIndex)
							.Points(self.currentPointIndex)
	...
	def _getChartElementText(self, ElementID, arg1, arg2, reportExtraInfo=False):
		if ElementID == xlSeries:
			if arg2 == -1:
				...
				return _("{seriesName} series {seriesIndex} of {seriesCount}").format(
					seriesName=self.officeChartObject.SeriesCollection(arg1).Name,
					seriesIndex=arg1,
					seriesCount=self.officeChartObject.SeriesCollection().Count,
				)
			else:
				if self.officeChartObject.Application.name == "Microsoft Excel":
					...
					if len(formulas) == 4:
						chartSeriesXValue = (
							self.officeChartObject.Application.Range(formulas[1]).Rows[arg2].Text
						)
					else:
				...
				if self.officeChartObject.ChartType in (
					...
					if arg2 > 1:
						...
						if rightDataPoint == leftDataPoint:
							...
							output += _("no change from point {previousIndex}, ").format(
				...
				if (
					self.officeChartObject.HasAxis(xlCategory)
					and self.officeChartObject.Axes(xlCategory).HasTitle
				):
					...
					output += _("{categoryAxisTitle} {categoryAxisData}: ").format(
						categoryAxisTitle=self.officeChartObject.Axes(xlCategory).AxisTitle.Text,
						categoryAxisData=chartSeriesXValue,
					)
				else:
					# Translators: Specifies the category of a data point.
					# {categoryAxisData} will be replaced with the category itself; e.g. "January".
				...
				if self.officeChartObject.HasAxis(xlValue) and self.officeChartObject.Axes(xlValue).HasTitle:
					...
					output += _("{valueAxisTitle} {valueAxisData}").format(
						valueAxisTitle=self.officeChartObject.Axes(xlValue).AxisTitle.Text,
						valueAxisData=self.officeChartObject.SeriesCollection(arg1).Values[arg2 - 1],
					)
				else:
					# Translators: Specifies the value of a data point.
					# {valueAxisData} will be replaced with the value itself; e.g. "1000".
					output += _("value {valueAxisData}").format(
						valueAxisData=self.officeChartObject.SeriesCollection(arg1).Values[arg2 - 1]
					)
				...
				if self.officeChartObject.ChartType in (xlPie, xlPieExploded, xlPieOfPie):
					...
					output += _(
						...
						* 100.00,
						pointIndex=arg2,
						pointCount=len(self.officeChartObject.SeriesCollection(arg1).Values),
					)
				else:
					# Translators: Details about a segment of a chart.
					# For example, this might report "column 1 of 5"
					output += _(" {segmentType} {pointIndex} of {pointCount}").format(
						segmentType=self.GetChartSegment(),
						pointIndex=arg2,
						pointCount=len(self.officeChartObject.SeriesCollection(arg1).Values),
					)
		...
		for axisType in [xlCategory, xlValue, xlSeriesAxis]:
			for axisGroup in [xlPrimary, xlSecondary]:
				if tempChartObject.HasAxis(axisType, axisGroup):
					listOfChartAxis.append(
						OfficeChartElementAxis(
							windowHandle=windowHandle,
							officeChartObject=tempChartObject,
							elementID=xlAxis,
							arg1=axisType,
							arg2=axisGroup,
						)
	...
	def _getChartElementText(self, ElementID, arg1, arg2, reportExtraInfo=False):
		...
		if self.officeChartObject.HasAxis(arg2, arg1) and self.officeChartObject.Axes(arg2, arg1).HasTitle:
			...
			axisTitle += _(" title: {axisTitle}").format(
				axisTitle=self.officeChartObject.Axes(self.axisType, self.axisGroup).AxisTitle.Text
			)
		return axisTitle
	...
	def _getChartElementText(self, ElementID, arg1, arg2, reportExtraInfo=False):
		if self.currentTrendline.DisplayEquation or self.currentTrendline.DisplayRSquared:
			...
			output = _(
				...
				seriesName=self.officeChartObject.SeriesCollection(self.seriesIndex).Name,
				trendlineType=self._trendlineTypeMap[self.currentTrendline.Type],
				trendlineName=self.currentTrendline.Name,
				trendlineLabel=label,
			)
		else:
			# Translators: This message gives trendline type and name for selected series
			output = _("{seriesName} trendline type: {trendlineType}, name: {trendlineName} ").format(
				seriesName=self.officeChartObject.SeriesCollection(self.seriesIndex).Name,
				trendlineType=self._trendlineTypeMap[self.currentTrendline.Type],
				trendlineName=self.currentTrendline.Name,
			)
	...
	def _getChartElementText(self, ElementID, arg1, arg2, reportExtraInfo=False):
		if reportExtraInfo:
			...
			return _(
				...
				chartAreaTop=self.officeChartObject.ChartArea.Top,
				chartAreaLeft=self.officeChartObject.ChartArea.Left,
			)
		else:
	...
	def _getChartElementText(self, ElementID, arg1, arg2, reportExtraInfo=False):
		if reportExtraInfo:
			...
			return _(
				...
				plotAreaInsideTop=self.officeChartObject.PlotArea.InsideTop,
				plotAreaInsideLeft=self.officeChartObject.PlotArea.InsideLeft,
			)
		else:
...
class OfficeChartElementLegendEntry(OfficeChartElementBase):
	...
	def _getChartElementText(self, ElementID, arg1, arg2, reportExtraInfo=False):
		...
		if self.eventDriven:
			...
			return _("Legend entry for series {seriesName} {seriesIndex} of {seriesCount}").format(
				seriesName=self.officeChartObject.SeriesCollection(arg1).Name,
				seriesIndex=arg1,
				seriesCount=self.officeChartObject.SeriesCollection().Count,
			)
			...
			return _("Legend entry {legendEntryIndex} of {legendEntryCount}").format(
				legendEntryIndex=arg1, legendEntryCount=arg2
			)
...
class OfficeChartElementLegendKey(OfficeChartElementBase):
	...
	def _getChartElementText(self, ElementID, arg1, arg2, reportExtraInfo=False):
		if ElementID == xlLegendKey:
			...
			return _("Legend key for Series {seriesName} {seriesIndex} of {seriesCount}").format(
				seriesName=self.officeChartObject.SeriesCollection(arg1).Name,
				seriesIndex=arg1,
				seriesCount=self.officeChartObject.SeriesCollection().Count,
			)
