...
class EditTextInfo(textInfos.offsets.OffsetsTextInfo):
		...
		if self.obj.editAPIVersion == 1 or self.obj.editAPIVersion >= 3:
			...
			try:
				...
				winKernel.writeProcessMemory(
					processHandle, internalP, ctypes.byref(p), ctypes.sizeof(p), None
				)
				watchdog.cancellableSendMessage(
					self.obj.windowHandle, winUser.EM_POSFROMCHAR, internalP, offset
				)
		...
		if self.obj.editAPIVersion >= 1:
			...
			try:
				...
				offset = watchdog.cancellableSendMessage(
					self.obj.windowHandle, winUser.EM_CHARFROMPOS, 0, internalP
				)
			finally:
				winKernel.virtualFreeEx(processHandle, internalP, 0, winKernel.MEM_RELEASE)
		else:
			...
			if self._getStoryLength() > 0xFFFF:
				...
				lineStart = watchdog.cancellableSendMessage(
					self.obj.windowHandle, winUser.EM_LINEINDEX, lineNum, 0
				)
	...
	def _getCharFormat(self, offset):
		...
		internalCharFormat = winKernel.virtualAllocEx(
			processHandle, None, ctypes.sizeof(charFormat), winKernel.MEM_COMMIT, winKernel.PAGE_READWRITE
		)
		try:
			winKernel.writeProcessMemory(
				processHandle, internalCharFormat, ctypes.byref(charFormat), ctypes.sizeof(charFormat), None
			)
			watchdog.cancellableSendMessage(
				self.obj.windowHandle, EM_GETCHARFORMAT, SCF_SELECTION, internalCharFormat
			)
	...
	def _getFormatFieldAndOffsets(self, offset, formatConfig, calculateOffsets=True):  # noqa: C901
		...
		if self.obj.editAPIVersion < 1:
			return super(EditTextInfo, self)._getFormatFieldAndOffsets(
				offset, formatConfig, calculateOffsets=calculateOffsets
			)
		if calculateOffsets:
	...
	def _setFormatFieldColor(
		self, charFormat: Union[CharFormat2AStruct, CharFormat2WStruct], formatField: textInfos.FormatField
	) -> None:
	...
	def _getSelectionOffsets(self):
		if self.obj.editAPIVersion >= 1:
			...
			internalCharRange = winKernel.virtualAllocEx(
				processHandle, None, ctypes.sizeof(charRange), winKernel.MEM_COMMIT, winKernel.PAGE_READWRITE
			)
			try:
				watchdog.cancellableSendMessage(self.obj.windowHandle, EM_EXGETSEL, 0, internalCharRange)
				winKernel.readProcessMemory(
					processHandle, internalCharRange, ctypes.byref(charRange), ctypes.sizeof(charRange), None
				)
			finally:
			...
			res = watchdog.cancellableSendMessage(
				self.obj.windowHandle, winUser.EM_GETSEL, ctypes.byref(start), ctypes.byref(end)
			)  # noqa: F841
	...
	def _getStoryLength(self):
		if self.obj.editAPIVersion >= 2:
			...
			internalInfo = winKernel.virtualAllocEx(
				processHandle, None, ctypes.sizeof(info), winKernel.MEM_COMMIT, winKernel.PAGE_READWRITE
			)
			try:
				winKernel.writeProcessMemory(
					processHandle, internalInfo, ctypes.byref(info), ctypes.sizeof(info), None
				)
			...
			try:
				...
				try:
					...
					numChars = watchdog.cancellableSendMessage(
						self.obj.windowHandle, EM_GETTEXTRANGE, 0, internalTextRange
					)
				finally:
	...
	def _getWordOffsets(self, offset):
		if self.obj.editAPIVersion >= 2:
			start = watchdog.cancellableSendMessage(
				self.obj.windowHandle, EM_FINDWORDBREAK, WB_MOVEWORDLEFT, offset
			)
			end = watchdog.cancellableSendMessage(
				self.obj.windowHandle, EM_FINDWORDBREAK, WB_MOVEWORDRIGHT, start
			)
			if end <= offset:
				start = end
				end = watchdog.cancellableSendMessage(
					self.obj.windowHandle, EM_FINDWORDBREAK, WB_MOVEWORDRIGHT, offset
				)
			return (start, end)
		else:
...
class ITextDocumentTextInfo(textInfos.TextInfo):
	...
	def _setFormatFieldColor(self, fontObj, formatField: textInfos.FormatField) -> None:
	...
	def _getEmbeddedObjectLabel(self, embedRangeObj):
		...
		label = displayModel.DisplayModelTextInfo(
			self.obj, locationHelper.RectLTRB(left, top, right, bottom)
		).text
		...
		if not hasattr(self, "_ITextDocumentObject"):
			try:
				...
				ctypes.windll.oleacc.AccessibleObjectFromWindow(
					self.windowHandle, -16, ctypes.byref(ptr._iid_), ctypes.byref(ptr)
				)
