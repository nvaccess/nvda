...
class WordDocumentRevisionQuickNavItem(WordDocumentCollectionQuickNavItem):
	...
	def label(self):
		...
		return _("{revisionType} {description}: {text} by {author} on {date}").format(
			revisionType=revisionType, author=author, text=text, date=date, description=description
		)
		...
		for index, item in enumerate(commandList):
			if isinstance(item, textInfos.FieldCommand):
				...
				if isinstance(lastItem, textInfos.FieldCommand) and isinstance(
					lastItem.field, textInfos.FormatField
				):
					try:
			...
			if not isFirst or includeCurrent:
				...
				if level and 0 < level < 10 and (not neededLevel or neededLevel == level):
					...
					yield WordDocumentHeadingQuickNavItem(
						nodeType, self, BrowseModeWordDocumentTextInfo(self, None, _rangeObj=rangeObj), level
					)
			isFirst = False
			if direction == "next":
	...
	def _iterNodesByType(self, nodeType, direction="next", pos=None):
		...
		if nodeType == "link":
			return LinkWinWordCollectionQuicknavIterator(
				nodeType, self, direction, rangeObj, includeCurrent
			).iterate()
...
class WordDocument(Window):
		...
		if not getattr(self, "_WinwordWindowObject", None):
			try:
				pDispatch = oleacc.AccessibleObjectFromWindow(
					self.documentWindowHandle, winUser.OBJID_NATIVEOM, interface=comtypes.automation.IDispatch
				)
	...
	def script_toggleBold(self, gesture):
		...
		val = self._WaitForValueChangeForAction(
			lambda: gesture.send(), lambda: self.WinwordSelectionObject.font.bold
		)
		if val:
	...
	def script_toggleItalic(self, gesture):
		if not self.WinwordSelectionObject:
			...
			return gesture.send()
		val = self._WaitForValueChangeForAction(
			lambda: gesture.send(), lambda: self.WinwordSelectionObject.font.italic
		)
		if val:
			# Translators: a message when toggling formatting in Microsoft word
	...
	def script_toggleUnderline(self, gesture):
		if not self.WinwordSelectionObject:
			...
			return gesture.send()
		val = self._WaitForValueChangeForAction(
			lambda: gesture.send(), lambda: self.WinwordSelectionObject.font.underline
		)
		if val:
			# Translators: a message when toggling formatting in Microsoft word
	...
	def script_toggleAlignment(self, gesture):
		if not self.WinwordSelectionObject:
			...
			return gesture.send()
		val = self._WaitForValueChangeForAction(
			lambda: gesture.send(), lambda: self.WinwordSelectionObject.paragraphFormat.alignment
		)
		alignmentMessages = {
	...
	def script_changeParagraphLeftIndent(self, gesture):
		...
		val = self._WaitForValueChangeForAction(
			lambda: gesture.send(), lambda: self.WinwordSelectionObject.paragraphFormat.LeftIndent
		)
	...
	def script_increaseDecreaseOutlineLevel(self, gesture):
		if not self.WinwordSelectionObject:
			...
			return gesture.send()
		val = self._WaitForValueChangeForAction(
			lambda: gesture.send(), lambda: self.WinwordSelectionObject.paragraphFormat.outlineLevel
		)
	...
	def script_increaseDecreaseFontSize(self, gesture):
		if not self.WinwordSelectionObject:
			...
			return gesture.send()
		val = self._WaitForValueChangeForAction(
			lambda: gesture.send(), lambda: self.WinwordSelectionObject.font.size
		)
	...
	def script_toggleDisplayNonprintingCharacters(self, gesture):
		if not self.WinwordWindowObject:
			...
			return gesture.send()
		val = self._WaitForValueChangeForAction(
			lambda: gesture.send(), lambda: self.WinwordWindowObject.ActivePane.View.ShowAll
		)
		if val:
	...
	def script_changeLineSpacing(self, gesture):
		if not self.WinwordSelectionObject:
			...
			return gesture.send()
		val = self._WaitForValueChangeForAction(
			lambda: gesture.send(), lambda: self.WinwordSelectionObject.ParagraphFormat.LineSpacingRule
		)
