...
class ScintillaTextInfo(textInfos.offsets.OffsetsTextInfo):
		...
		if calculateOffsets:
			...
			while startOffset > lineStart:
				curStyle = watchdog.cancellableSendMessage(
					self.obj.windowHandle, SCI_GETSTYLEAT, startOffset - 1, 0
				)
				if curStyle == style:
					startOffset -= 1
				else:
			...
			while endOffset < lineEnd:
				curStyle = watchdog.cancellableSendMessage(
					self.obj.windowHandle, SCI_GETSTYLEAT, endOffset, 0
				)
				if curStyle == style:
					endOffset += 1
				else:
		...
		if formatConfig["reportFontName"]:
			...
			try:
				...
				winKernel.readProcessMemory(
					self.obj.processHandle, internalBuf, fontNameBuf, len(fontNameBuf), None
				)
			finally:
	...
	def _getStoryLength(self):
		if not hasattr(self, "_storyLength"):
			self._storyLength = watchdog.cancellableSendMessage(
				self.obj.windowHandle, SCI_GETTEXTLENGTH, 0, 0
			)
	...
	def _getTextRange(self, start, end):
		...
		try:
			...
			try:
				...
				numBytes = watchdog.cancellableSendMessage(
					self.obj.windowHandle, SCI_GETTEXTRANGE, 0, internalTextRange
				)
			finally:
		...
		return textUtils.getTextFromRawBytes(
			buf.raw, numChars=numBytes, encoding=self.encoding, errorsFallback="surrogateescape"
		)
