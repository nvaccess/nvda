...
def getCellBorderStyleDescription(bordersObj, reportBorderColor=False):
		...
		if border.lineStyle != xlLineStyleNone:
			...
			if not desc:
				...
				desc = _("{weight} {style}").format(
					style=borderStyleLabels.get(style), weight=borderWeightLabels.get(weight)
				)
			if reportBorderColor:
				# Translators: border styles in Microsoft Excel.
				d[pos] = _("{color} {desc}").format(
					color=colors.RGB.fromCOLORREF(int(border.color)).name, desc=desc
				)
			else:
				d[pos] = desc
	s = []
	if (
		d.get(xlEdgeTop) == d.get(xlEdgeBottom) == d.get(xlEdgeLeft) == d.get(xlEdgeRight)
		and d.get(xlEdgeTop) is not None
	):
		# Translators: border styles in Microsoft Excel.
	...
	for pos, desc in d.items():
		# Translators: border styles in Microsoft Excel.
		s.append(_("{desc} {position}").format(desc=desc, position=bordersIndexLabels.get(pos)))
	return ", ".join(s)
