...
class UIAWebTextInfo(UIATextInfo):
	...
	def _moveToEdgeOfReplacedContent(self, back=False):
		...
		if not back:
			textRange.MoveEndpointByRange(
				UIAHandler.TextPatternRangeEndpoint_Start, textRange, UIAHandler.TextPatternRangeEndpoint_End
			)
			textRange.move(UIAHandler.TextUnit_Character, -1)
		else:
			textRange.MoveEndpointByRange(
				UIAHandler.TextPatternRangeEndpoint_End, textRange, UIAHandler.TextPatternRangeEndpoint_Start
			)
	...
	def _getControlFieldForUIAObject(self, obj, isEmbedded=False, startOfNode=False, endOfNode=False):
		field = super()._getControlFieldForUIAObject(
			obj, isEmbedded=isEmbedded, startOfNode=startOfNode, endOfNode=endOfNode
		)
	...
	def getTextWithFields(  # noqa: C901
		self, formatConfig: Optional[Dict] = None
	...
	def level(self):
		if not hasattr(self, "_level"):
			styleVal = getUIATextAttributeValueFromRange(
				self.textInfo._rangeObj, UIAHandler.UIA_StyleIdAttributeId
			)
