...
class EdgeTextInfo_preGapRemoval(EdgeTextInfo):
		...
		if children.length:
			...
			if not child.getCurrentPropertyValue(UIAHandler.UIA_IsTextPatternAvailablePropertyId):
				...
				if childChildren.length == 1 and UIAHandler.handler.clientObject.compareElements(
					child, childChildren.getElement(0)
				):
					return True
		return False
	...
	def _getTextWithFieldsForUIARange(  # noqa: C901
		...
		startRange.MoveEndpointByRange(
			UIAHandler.TextPatternRangeEndpoint_End, startRange, UIAHandler.TextPatternRangeEndpoint_Start
		)
		enclosingElement = getEnclosingElementWithCacheFromUIATextRange(
			startRange, self._controlFieldUIACacheRequest
		)
		...
		startRange.MoveEndpointByRange(
			UIAHandler.TextPatternRangeEndpoint_End, enclosingRange, UIAHandler.TextPatternRangeEndpoint_End
		)
		if 0 < startRange.CompareEndpoints(
			UIAHandler.TextPatternRangeEndpoint_End, textRange, UIAHandler.TextPatternRangeEndpoint_End
		):
			startRange.MoveEndpointByRange(
				UIAHandler.TextPatternRangeEndpoint_End, textRange, UIAHandler.TextPatternRangeEndpoint_End
			)
		...
		if childElements.length == 1 and UIAHandler.handler.clientObject.compareElements(
			rootElement, childElements.getElement(0)
		):
		...
		clippedStart = 0 > enclosingRange.CompareEndpoints(
			UIAHandler.TextPatternRangeEndpoint_Start, startRange, UIAHandler.TextPatternRangeEndpoint_Start
		)
		clippedEnd = 0 < enclosingRange.CompareEndpoints(
			UIAHandler.TextPatternRangeEndpoint_End, startRange, UIAHandler.TextPatternRangeEndpoint_End
		)
		for field in super(EdgeTextInfo, self)._getTextWithFieldsForUIARange(
			enclosingElement,
			startRange,
		...
		for parentElement, field in parents:
			...
			tempRange.MoveEndpointByRange(
				UIAHandler.TextPatternRangeEndpoint_Start, tempRange, UIAHandler.TextPatternRangeEndpoint_End
			)
...
class EdgeNode(web.UIAWeb):
	...
	def getNormalizedUIATextRangeFromElement(self, UIAElement):
		...
		lastCharInfo = EdgeTextInfo_preGapRemoval(obj=self, position=None, _rangeObj=textRange)
		...
		while 0 != super(EdgeTextInfo, charInfo).move(textInfos.UNIT_CHARACTER, 1):
