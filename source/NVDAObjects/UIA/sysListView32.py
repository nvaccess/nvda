...
class SysListViewItem(ListItem):
			...
			if (
				...
				try:
					columnHeaderItems = e.getCachedPropertyValueEx(
						UIAHandler.UIA.UIA_TableItemColumnHeaderItemsPropertyId, False
					)
				except COMError:
	...
	def _get_indexInParent(self) -> Optional[int]:
		...
		element = UIAHandler.handler.baseTreeWalker.GetFirstChildElementBuildCache(
			self.UIAElement, childCacheRequest
		)
		val = element.getCachedPropertyValueEx(UIAHandler.UIA.UIA_GridItemRowPropertyId, True)
