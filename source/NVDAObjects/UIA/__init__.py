...
from UIAHandler.types import IUIAutomationTextRangeT
...
class UIATextInfo(textInfos.TextInfo):
	...
	def find(self, text, caseSensitive=False, reverse=False):
		...
		if r:
			r.MoveEndpointByRange(
				UIAHandler.TextPatternRangeEndpoint_End, r, UIAHandler.TextPatternRangeEndpoint_Start
			)
			self._rangeObj = r
			return True
		return False
	...
	def _getFormatFieldAtRange(  # noqa: C901
		...
		try:
			...
			if formatConfig["reportSuperscriptsAndSubscripts"]:
				IDs.update({UIAHandler.UIA_IsSuperscriptAttributeId, UIAHandler.UIA_IsSubscriptAttributeId})
		...
		if formatConfig["reportFontAttributes"]:
			...
			val = fetcher.getValue(
				UIAHandler.UIA_UnderlineStyleAttributeId, ignoreMixedValues=ignoreMixedValues
			)
			if val != UIAHandler.handler.reservedNotSupportedValue:
				formatField["underline"] = bool(val)
			val = fetcher.getValue(
				UIAHandler.UIA_StrikethroughStyleAttributeId, ignoreMixedValues=ignoreMixedValues
			)
			if val != UIAHandler.handler.reservedNotSupportedValue:
		...
		if formatConfig["reportSuperscriptsAndSubscripts"]:
			textPosition = None
			val = fetcher.getValue(
				UIAHandler.UIA_IsSuperscriptAttributeId, ignoreMixedValues=ignoreMixedValues
			)
			if val != UIAHandler.handler.reservedNotSupportedValue and val:
				textPosition = TextPosition.SUPERSCRIPT
			else:
				val = fetcher.getValue(
					UIAHandler.UIA_IsSubscriptAttributeId, ignoreMixedValues=ignoreMixedValues
				)
				if val != UIAHandler.handler.reservedNotSupportedValue and val:
		...
		if formatConfig["reportColor"]:
			val = fetcher.getValue(
				UIAHandler.UIA_BackgroundColorAttributeId, ignoreMixedValues=ignoreMixedValues
			)
			if isinstance(val, int):
				formatField["background-color"] = colors.RGB.fromCOLORREF(val)
			val = fetcher.getValue(
				UIAHandler.UIA_ForegroundColorAttributeId, ignoreMixedValues=ignoreMixedValues
			)
			if isinstance(val, int):
		...
		if formatConfig["reportHeadings"]:
			styleIDValue = fetcher.getValue(
				UIAHandler.UIA_StyleIdAttributeId, ignoreMixedValues=ignoreMixedValues
			)
		...
		if fetchAnnotationTypes:
			annotationTypes = fetcher.getValue(
				UIAHandler.UIA_AnnotationTypesAttributeId, ignoreMixedValues=ignoreMixedValues
			)
	...
	def _getFormatFieldIndent(
		...
		formatField = textInfos.FormatField()
		val = fetcher.getValue(
			UIAHandler.UIA_IndentationFirstLineAttributeId, ignoreMixedValues=ignoreMixedValues
		)
		uiaIndentFirstLine = val if isinstance(val, float) else None
		val = fetcher.getValue(
			UIAHandler.UIA_IndentationLeadingAttributeId, ignoreMixedValues=ignoreMixedValues
		)
		uiaIndentLeading = val if isinstance(val, float) else None
		val = fetcher.getValue(
			UIAHandler.UIA_IndentationTrailingAttributeId, ignoreMixedValues=ignoreMixedValues
		)
	...
	def __init__(  # noqa: C901
		self, obj: NVDAObject, position: str, _rangeObj: Optional[IUIAutomationTextRangeT] = None
	):
	...
	def _get_UIAElementAtStart(self):
		...
		if children and children.length == 1:
			...
			child = getEnclosingElementWithCacheFromUIATextRange(
				tempRange, UIAHandler.handler.baseCacheRequest
			)
		return child
	...
	def _getControlFieldForUIAObject(
		self, obj: "UIA", isEmbedded=False, startOfNode=False, endOfNode=False
	...
	def _getTextWithFields_text(
		...
		if debug:
			log.debug(f"Walking by unit {unit}, " f"with further units of: {furtherUIAFormatUnits}")
			...
			if text is not None:
				...
				try:
					field = self._getFormatFieldAtRange(
						tempRange, formatConfig, ignoreMixedValues=len(furtherUIAFormatUnits) == 0
					)
					...
					for subfield in self._getTextWithFields_text(
						tempRange, formatConfig, UIAFormatUnits=furtherUIAFormatUnits
					):
						yield subfield
					if debug:
	...
	def _getTextWithFieldsForUIARange(  # noqa: C901
		...
		if alwaysWalkAncestors:
			...
			try:
				parentElement = getEnclosingElementWithCacheFromUIATextRange(
					textRange, self._controlFieldUIACacheRequest
				)
			except COMError:
				parentElement = None
			while parentElement:
				...
				parentElement = UIAHandler.handler.baseTreeWalker.getParentElementBuildCache(
					parentElement, self._controlFieldUIACacheRequest
				)
		else:
		...
		for index, (parentElement, parentClipped) in enumerate(parentElements):
			...
			try:
				...
				field = self._getControlFieldForUIAObject(
					obj, isEmbedded=objIsEmbedded, startOfNode=startOfNode, endOfNode=endOfNode
				)
			except LookupError:
				if debug:
		...
		if childCount > 0:
			...
			tempRange.MoveEndpointByRange(
				UIAHandler.TextPatternRangeEndpoint_End, tempRange, UIAHandler.TextPatternRangeEndpoint_Start
			)
			if debug:
	...
	def move(
		...
		if endPoint == "start":
			res = self._rangeObj.MoveEndpointByUnit(
				UIAHandler.TextPatternRangeEndpoint_Start, UIAUnit, direction
			)
		elif endPoint == "end":
			res = self._rangeObj.MoveEndpointByUnit(
				UIAHandler.TextPatternRangeEndpoint_End, UIAUnit, direction
			)
		else:
...
class UIA(Window):
	...
	def findOverlayClasses(self, clsList):  # NOQA: C901
		...
		# Windows 8.x toast, although a form of tool tip, is covered separately.
			...
			if (
				...
				and self.role in (controlTypes.Role.PANE, controlTypes.Role.DOCUMENT)
				and self.parent
				and (
		...
		try:
			...
			try:
				...
				parentElement = UIAHandler.handler.baseTreeWalker.GetParentElementBuildCache(
					self.UIAElement, UIAHandler.handler.baseCacheRequest
				)
	...
	def kwargsFromSuper(cls, kwargs, relation=None, ignoreNonNativeElementsWithFocus=True):
		...
		if isinstance(relation, tuple):
			UIAElement = UIAHandler.handler.clientObject.ElementFromPointBuildCache(
				POINT(relation[0], relation[1]), UIAHandler.handler.baseCacheRequest
			)
			...
			if not UIAHandler.handler.isNativeUIAElement(UIAElement):
				if UIAHandler._isDebug():
					log.debug(f"kwargsFromSuper: ignoring non native element at coordinates {relation}")
				return False
			...
			if ignoreNonNativeElementsWithFocus and not UIAHandler.handler.isNativeUIAElement(UIAElement):
				if UIAHandler._isDebug():
					log.debug("kwargsFromSuper: ignoring non native element with focus")
				return False
			# This object may be in a different window, so we need to recalculate the window handle.
			kwargs["windowHandle"] = None
		else:
			UIAElement = UIAHandler.handler.clientObject.ElementFromHandleBuildCache(
				windowHandle, UIAHandler.handler.baseCacheRequest
			)
		if not UIAElement:
			return False
	...
	def _get_UIASelectionPattern2(self):
		try:
			self.UIASelectionPattern2 = self._getUIAPattern(
				UIAHandler.UIA_SelectionPattern2Id, UIAHandler.IUIAutomationSelectionPattern2
			)
		except COMError:
	...
	def _get_UIATextPattern(self):
		self.UIATextPattern = self._getUIAPattern(
			UIAHandler.UIA_TextPatternId, UIAHandler.IUIAutomationTextPattern, cache=False
		)
		return self.UIATextPattern

	def _get_UIATableItemPattern(self):
		self.UIATableItemPattern = self._getUIAPattern(
			UIAHandler.UIA_TableItemPatternId, UIAHandler.IUIAutomationTableItemPattern, cache=False
		)
	...
	def _get_UIATextEditPattern(self):
		...
		self.UIATextEditPattern = self._getUIAPattern(
			UIAHandler.UIA_TextEditPatternId, UIAHandler.IUIAutomationTextEditPattern, cache=False
		)
	...
	def _get_UIALegacyIAccessiblePattern(self):
		self.UIALegacyIAccessiblePattern = self._getUIAPattern(
			UIAHandler.UIA_LegacyIAccessiblePatternId, UIAHandler.IUIAutomationLegacyIAccessiblePattern
		)
	...
	def _get_role(self):
		role = UIAHandler.UIAControlTypesToNVDARoles.get(
			self.UIAElement.cachedControlType, controlTypes.Role.UNKNOWN
		)
	...
	def _get_states(self):
		...
		try:
			isDataValid = self._getUIACacheablePropertyValue(
				UIAHandler.UIA_IsDataValidForFormPropertyId, True
			)
		except COMError:
		...
		try:
			s = self._getUIACacheablePropertyValue(
				UIAHandler.UIA_ExpandCollapseExpandCollapseStatePropertyId, True
			)
		except COMError:
			s = UIAHandler.handler.reservedNotSupportedValue
		if s != UIAHandler.handler.reservedNotSupportedValue:
	...
	def isDescendantOf(self, obj: "NVDAObjects.NVDAObject") -> bool:
		if isinstance(obj, UIA):
			...
			UIACondition = UIAHandler.handler.clientObject.createPropertyCondition(
				UIAHandler.UIA_RuntimeIdPropertyId, objIDArray
			)
