...
def ReadConsoleOutputCharacter(handle,length,x,y):
	...
	if windll.kernel32.ReadConsoleOutputCharacterW(handle,buf,length,COORD(x,y),byref(numCharsRead))==0:  # noqa: F405
		raise WinError()  # noqa: F405
...
def ReadConsoleOutput(handle, length, rect):
	...
	if windll.kernel32.ReadConsoleOutputW(handle, buf, COORD(rect.Right-rect.Left+1, rect.Bottom-rect.Top+1), COORD(0,0), byref(rect))==0:  # noqa: F405
		raise WinError()  # noqa: F405
	return buf
