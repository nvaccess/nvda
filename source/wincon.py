...
def ReadConsoleOutputCharacter(handle, length, x, y):
	...
	if (
		windll.kernel32.ReadConsoleOutputCharacterW(handle, buf, length, COORD(x, y), byref(numCharsRead))
		== 0
	):  # noqa: F405
		raise WinError()  # noqa: F405
	return textUtils.getTextFromRawBytes(
		buf.raw, numChars=numCharsRead.value, encoding=textUtils.WCHAR_ENCODING
	)
...
def ReadConsoleOutput(handle, length, rect):
	...
	if (
		windll.kernel32.ReadConsoleOutputW(
			handle,
			buf,
