...
def waitForFreezeRecovery(waitedSince: float):
	...
	log.info(f"Recovered from freeze after {_timer() - waitedSince} seconds.")
...
def _shouldRecoverAfterMinTimeout():
	...
	if (
		...
		and api.getFocusObject().windowClassName != "#32768"
	):
		return True
	if winUser.getClassName(info.hwndFocus) in safeWindowClassSet:
		return False
	focus = api.getFocusObject()
	if (not isinstance(focus, Window)) or (
		not winUser.isDescendantWindow(info.hwndActive, focus.windowHandle)
	):
	...
	try:
		...
		with open(dumpPath, "wb") as mdf:
			mdExc = MINIDUMP_EXCEPTION_INFORMATION(
				ThreadId=threadId, ExceptionPointers=exceptionInfo, ClientPointers=False
			)
...
def initialize():
	...
	NVDAHelper._setDllFuncPointer(
		NVDAHelper.localLib, "_notifySendMessageCancelled", _notifySendMessageCancelled
	)
...
def terminate():
	...
	windll.kernel32.SetWaitableTimer(
		_coreDeadTimer, ctypes.byref(ctypes.wintypes.LARGE_INTEGER(0)), 0, None, None, False
	)
	_watcherThread.join()
...
class CancellableCallThread(threading.Thread):
	...
	def execute(self, func, *args, pumpMessages=True, **kwargs):
		...
		waitHandles = (ctypes.wintypes.HANDLE * 2)(self._executionDoneEvent, _cancelCallEvent)
		waitIndex = ctypes.wintypes.DWORD()
		if pumpMessages:
			oledll.ole32.CoWaitForMultipleHandles(
				0, winKernel.INFINITE, 2, waitHandles, ctypes.byref(waitIndex)
			)
		else:
			waitIndex.value = windll.kernel32.WaitForMultipleObjects(
				2, waitHandles, False, winKernel.INFINITE
			)
...
def cancellableSendMessage(hwnd, msg, wParam, lParam, flags=0, timeout=60000):
	...
	NVDAHelper.localLib.cancellableSendMessageTimeout(
		hwnd, msg, wParam, lParam, flags, timeout, ctypes.byref(result)
	)
	return result.value
