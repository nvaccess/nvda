		...
		if os.path.isdir(os.path.join(source, dirName)) and not dirName.startswith("."):
			rulesList.extend(
				getRecursiveDataFiles(
					os.path.join(dest, dirName), os.path.join(source, dirName), excludes=excludes
				)
			)
	return rulesList
...
def _genManifestTemplate(shouldHaveUIAccess: bool) -> tuple[int, int, bytes]:
	return (RT_MANIFEST, 1, (_manifestTemplate % {"uiAccess": shouldHaveUIAccess}).encode("utf-8"))
