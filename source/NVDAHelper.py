...
def _lookupKeyboardLayoutNameWithHexString(layoutString):
	...
	if (
		...
		try:
			if (
				windll.advapi32.RegQueryValueExW(key, "Layout Display Name", 0, None, buf, byref(bufSize))
				== 0
			):  # noqa: F405
...
def nvdaControllerInternal_reportLiveRegion(text: str, politeness: str):
	...
	try:
		...
		log.error(
			f"nvdaControllerInternal_reportLiveRegion got unknown politeness of {politeness}", exc_info=True
		)
		return -1
...
def handleInputCompositionStart(compositionString, selectionStart, selectionEnd, isReading):
	...
	if focus.parent and isinstance(focus.parent, InputComposition):
		...
		focus.parent.compositionUpdate(
			compositionString, selectionStart, selectionEnd, isReading, announce=announce
		)
		return 0
...
def nvdaControllerInternal_inputCandidateListUpdate(candidatesString, selectionIndex, inputMethod):
	queueHandler.queueFunction(
		queueHandler.eventQueue, handleInputCandidateListUpdate, candidatesString, selectionIndex, inputMethod
	)
	return 0
...
def nvdaControllerInternal_inputConversionModeUpdate(oldFlags, newFlags, lcid):
	queueHandler.queueFunction(
		queueHandler.eventQueue, handleInputConversionModeUpdate, oldFlags, newFlags, lcid
	)
	return 0
...
def initialize() -> None:
	...
	VBuf_getTextInRange = CFUNCTYPE(c_int, c_int, c_int, c_int, POINTER(BSTR), c_int)(  # noqa: F405
		("VBuf_getTextInRange", localLib), ((1,), (1,), (1,), (2,), (1,))
	)
	if config.isAppX:
