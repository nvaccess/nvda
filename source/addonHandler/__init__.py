...
class AddonsState(collections.UserDict[AddonStateCategory, CaseInsensitiveSet[str]]):
	...
	def fromPickledDict(
		self, pickledState: Dict[str, Union[Set[str], addonAPIVersion.AddonApiVersionT, MajorMinorPatch]]
	) -> None:
	...
	def _cleanupCompatibleAddonsFromDowngrade(self) -> None:
		...
		for blockedAddon in CaseInsensitiveSet(
			self[AddonStateCategory.BLOCKED].union(self[AddonStateCategory.OVERRIDE_COMPATIBILITY])
		):
			# Iterate over copy of set to prevent updating the set while iterating over it.
			if (
...
def getIncompatibleAddons(
	currentAPIVersion=addonAPIVersion.CURRENT, backCompatToAPIVersion=addonAPIVersion.BACK_COMPAT_TO
) -> "AddonHandlerModelGeneratorT":
	...
	return getAvailableAddons(
		filterFunc=lambda addon: (
			not isAddonCompatible(
				addon, currentAPIVersion=currentAPIVersion, backwardsCompatToVersion=backCompatToAPIVersion
			)
			and (
...
def _getAvailableAddonsFromPath(path: str, isFirstLoad: bool = False) -> "AddonHandlerModelGeneratorT":
...
def getAvailableAddons(
	refresh: bool = False, filterFunc: Optional[Callable[["Addon"], bool]] = None, isFirstLoad: bool = False
) -> "AddonHandlerModelGeneratorT":
...
class Addon(AddonBase):
	...
	def completeRemove(self, runUninstallTask: bool = True) -> None:
		...
		tempPath = _createEmptyTempFileForDeletingFile(
			suffix=DELETEDIR_SUFFIX, dir=os.path.dirname(self.path)
		)
		try:
	...
	def runInstallTask(self, taskName: Literal["onInstall", "onUninstall"], *args, **kwargs) -> None:
		"""
	...
	def __init__(self, input, translatedInput=None):
		...
		if result != True:  # noqa: E712
			...
			self._errors = "Constraint not met: minimumNVDAVersion ({}) <= lastTestedNVDAVersion ({})".format(
				self.get("minimumNVDAVersion"), self.get("lastTestedNVDAVersion")
			)
