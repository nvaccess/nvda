...
_ScriptFilterT = Callable[
	[Optional[_ScriptFunctionT], "NVDAObjects.NVDAObject", "inputCore.InputGesture"],
...
def script(
	...
	def script_decorator(decoratedScript):
		...
		if not isinstance(decoratedScript, types.FunctionType):
			log.warning("Using the script decorator is unsupported for %r" % decoratedScript, stack_info=True)
