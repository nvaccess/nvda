...
def isRegistered() -> bool:
	try:
		winreg.OpenKey(
			winreg.HKEY_LOCAL_MACHINE, RegistryKey.APP.value, 0, winreg.KEY_READ | winreg.KEY_WOW64_64KEY
		)
		return True
...
def _getAutoStartConfiguration(autoStartContext: AutoStartContext) -> List[str]:
	...
	try:
		k = winreg.OpenKey(
			autoStartContext.value, RegistryKey.ROOT.value, 0, winreg.KEY_READ | winreg.KEY_WOW64_64KEY
		)
	except FileNotFoundError:
...
def setAutoStart(autoStartContext: AutoStartContext, enable: bool) -> None:
	...
	if changed:
		...
		winreg.SetValueEx(k, "Configuration", None, winreg.REG_SZ, ",".join(conf))
