...
def _getDisplayDriver(moduleName: str, caseSensitive: bool = True) -> Type["BrailleDisplayDriver"]:
	try:
		return importlib.import_module(
			"brailleDisplayDrivers.%s" % moduleName, package="brailleDisplayDrivers"
		...
		for loader, name, isPkg in pkgutil.iter_modules(brailleDisplayDrivers.__path__):
			if name.startswith("_") or name.lower() != moduleName.lower():
				continue
			return importlib.import_module(
				"brailleDisplayDrivers.%s" % name, package="brailleDisplayDrivers"
			).BrailleDisplayDriver
		else:
			raise initialException
...
def _getAnnotationProperty(propertyValues: Dict[str, Any]) -> str:
	...
	def update(self):
		...
		if role == controlTypes.Role.MATH:
			...
			if mathPres.brailleProvider:
				try:
					text += TEXT_SEPARATOR + mathPres.brailleProvider.getBrailleForMathMl(obj.mathMl)
...
def getControlFieldBraille(
	...
	if presCat == field.PRESCAT_LAYOUT:
		...
		return _("%s end") % getPropertiesBraille(role=role, roleText=roleText)
	...
	if content:
		...
		if mathPres.brailleProvider:
			try:
				if text:
					text += TEXT_SEPARATOR
				text += mathPres.brailleProvider.getBrailleForMathMl(info.getMathMl(field))
			except (NotImplementedError, LookupError):
				pass
	return text
...
class BrailleBuffer(baseObject.AutoPropertyObject):
	...
	def _get_windowEndPos(self):
		...
		try:
			...
			return min(rindex(self.brailleCells, 0, self.windowStartPos, endPos) + 1, endPos)
		except ValueError:
			pass
		return endPos
...
def getFocusRegions(
	...
	region = (ReviewNVDAObjectRegion if review else NVDAObjectRegion)(
		obj, appendText=TEXT_SEPARATOR if region2 else ""
	)
	region.update()
	yield region
	if region2:
...
class BrailleHandler(baseObject.AutoPropertyObject):
	...
	def handleGainFocus(self, obj: "NVDAObject", shouldAutoTether: bool = True) -> None:
		...
		self._doNewObject(
			itertools.chain(
				getFocusContextRegions(obj, oldFocusRegions=self.mainBuffer.regions), getFocusRegions(obj)
			)
		)
	...
	def handleCaretMove(self, obj: "NVDAObject", shouldAutoTether: bool = True) -> None:
		if not self.enabled or config.conf["braille"]["mode"] == BrailleMode.SPEECH_OUTPUT.value:
			return
		if objectBelowLockScreenAndWindowsIsLocked(obj):
			return
		...
		try:
			...
			for region in self._regionsPendingUpdate:
				...
				try:
					...
					log.debugWarning(
						f"Region update failed for {region}, object probably died", exc_info=True
					)
					continue
	...
	def _enableDetection(
		self, usb: bool = True, bluetooth: bool = True, limitToDevices: Optional[List[str]] = None
	):
...
class BrailleDisplayDriver(driverHandler.Driver):
	...
	@classmethod
	def _getTryPorts(cls, port: Union[str, bdDetect.DeviceMatch]) -> typing.Iterator[bdDetect.DeviceMatch]:
	...
	def _getModifierGestures(cls, model=None):
		...
		prefixes = [
			"br({source})".format(source=cls.name),
		]
		if model:
...
class BrailleDisplayGesture(inputCore.InputGesture):
	...
	def _get_identifiers(self):
		...
		if self.model:
			...
			ids.insert(
				0, "br({source}.{model}):{id}".format(source=self.source, model=self.model, id=self.id)
			)
		import brailleInput
	...
	def _get_script(self):
		...
		if gestureKeys != set(self.keyNames):
			...
			id = "+".join(gestureKeys)
			fakeGestureIds = [
				"br({source}):{id}".format(source=self.source, id=id),
			]
			if self.model:
				fakeGestureIds.insert(
					0, "br({source}.{model}):{id}".format(source=self.source, model=self.model, id=id)
				)
			scriptNames = []
			...
			combinedScriptName = "kb:{modifiers}+{keys}".format(
				modifiers="+".join(gestureModifiers), keys=scriptNames[0].split(":")[1]
			)
		elif script and scriptName:
			combinedScriptName = "kb:{modifiers}+{keys}".format(
				modifiers="+".join(gestureModifiers), keys=scriptName.split(":")[1]
			)
		else:
			return None
	...
	def getDisplayTextForIdentifier(cls, identifier):
		...
		if modelName:  # The identifier contains a model name
			return description, "{modelName}: {key}".format(modelName=modelName, key=key)
		else:
		...
		if "bluetoothName" in info:
			yield (
				...
				_("Bluetooth Serial: {port} ({deviceName})").format(
					port=info["port"], deviceName=info["bluetoothName"]
				),
			)
		else:
			yield (
...
def getDisplayDrivers(
	...
	for loader, name, isPkg in pkgutil.iter_modules(brailleDisplayDrivers.__path__):
		...
		try:
			...
			log.error(f"Error while importing braille display driver {name}", exc_info=True)
			continue
