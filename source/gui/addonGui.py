...
def promptUserForRestart():
	...
	result = gui.messageBox(
		message=restartMessage, caption=restartTitle, style=wx.YES | wx.NO | wx.ICON_WARNING
	)
	if wx.YES == result:
...
class ErrorAddonInstallDialog(nvdaControls.MessageDialog):
	def __init__(self, parent, title, message, showAddonInfoFunction):
		super().__init__(parent, title, message, dialogType=nvdaControls.MessageDialog.DIALOG_TYPE_ERROR)
		self._showAddonInfoFunction = showAddonInfoFunction
...
def installAddon(parentWindow: wx.Window, addonPath: str) -> bool:  # noqa: C901
	...
	if not bundle._hasGotRequiredSupport:
		...
		shouldInstall, rememberChoice = _shouldInstallWhenAddonTooOldDialog(
			parentWindow, bundle._addonGuiModel
		)
		if shouldInstall:
	...
	def __init__(
		...
		self.unknownCompatibilityAddonsList = list(
			addonHandler.getIncompatibleAddons(
				currentAPIVersion=APIVersion, backCompatToAPIVersion=APIBackwardsCompatToVersion
			)
		)
		...
		mainSizer.Add(
			settingsSizer, border=guiHelper.BORDER_FOR_DIALOGS, flag=wx.ALL | wx.EXPAND, proportion=1
		)
		mainSizer.Fit(self)
