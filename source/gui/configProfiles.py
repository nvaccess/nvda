...
class ProfilesDialog(
	...
	def __init__(self, parent):
		...
		profilesListGroupSizer.Add(
			profilesListGroupContents, border=guiHelper.BORDER_FOR_DIALOGS, flag=wx.ALL
		)
	...
	def onChangeState(self, evt):
		...
		try:
			...
			gui.messageBox(_("Error activating profile."), _("Error"), wx.OK | wx.ICON_ERROR, self)
			return
		self.Close()
	...
	def onDelete(self, evt):
		...
		try:
			...
			gui.messageBox(_("Error deleting profile."), _("Error"), wx.OK | wx.ICON_ERROR, self)
			return
	...
	def onRename(self, evt):
		...
		try:
			...
			log.debugWarning("", exc_info=True)
			gui.messageBox(_("Error renaming profile."), _("Error"), wx.OK | wx.ICON_ERROR, self)
			return
...
class TriggersDialog(
	...
	def onTriggerListChoice(self, evt):
		...
		try:
			...
			log.error("Trigger %s: invalid profile %s" % (trig.spec, trig.profile))
			self.profileList.Selection = 0
			trig.profile = None
...
class NewProfileDialog(
	...
	def __init__(self, parent):
		...
		self.triggerChoice = sHelper.addItem(
			wx.RadioBox(self, label=_("Use this profile for:"), choices=[trig[1] for trig in triggers])
		)
