...
class CustomCheckListBox(wx.CheckListBox):
	...
	def notifyIAccessible(self, evt):
		...
		winUser.NotifyWinEvent(
			winUser.EVENT_OBJECT_STATECHANGE, self.Handle, winUser.OBJID_CLIENT, evt.Selection + 1
		)
...
class AutoWidthColumnCheckListCtrl(AutoWidthColumnListCtrl, listmix.CheckListCtrlMixin):
	...
	def __init__(
		...
		AutoWidthColumnListCtrl.__init__(
			self, parent, id=id, pos=pos, size=size, style=style, autoSizeColumn=autoSizeColumn
		)
...
class MessageDialog(DPIScaledDialog):
	...
	def __init__(self, parent, title, message, dialogType=DIALOG_TYPE_STANDARD):
		...
		mainSizer.Add(contentsSizer.sizer, border=guiHelper.BORDER_FOR_DIALOGS, flag=wx.ALL)
		mainSizer.Fit(self)
...
class FeatureFlagCombo(wx.Choice):
	...
	def __init__(
		...
		if onChoiceEventHandler is not None:
			self.Bind(wx.EVT_CHOICE, onChoiceEventHandler)
	...
	def _createOptionsDict(
		self, translatedOptions: OrderedDict[FeatureFlagEnumT, str]
		...
		defaultOptionLabel: str = _("Default ({})").format(translatedStringForBehaviorOfDefault)
