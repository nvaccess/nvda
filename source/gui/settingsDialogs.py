...
class AdvancedPanelControls(wx.Panel):
	...
	def __init__(self, parent):
		...
		# Translators: This is the label for a checkbox in the
		#  Advanced settings panel.
		label = _("Detect changes by c&haracter when supported")
	...
	def haveConfigDefaultsBeenRestored(self):
		return (
			...
			and self.useDMPWhenSupportedCheckBox.IsChecked() == self.useDMPWhenSupportedCheckBox.defaultValue
	...
	def restoreToDefaults(self):
		...
		self.useDMPWhenSupportedCheckBox.SetValue(self.useDMPWhenSupportedCheckBox.defaultValue)
	...
	def onSave(self):
		...
		config.conf["terminals"]["useDMPWhenSupported"] = self.useDMPWhenSupportedCheckBox.IsChecked()
