...
class AddonDetails(
	...
	def __init__(
		...
		self.addonNameCtrl = wx.StaticText(self, style=wx.ALIGN_CENTRE_HORIZONTAL | wx.ST_NO_AUTORESIZE)
		...
		self.descriptionLabel = wx.StaticText(self.contentsPanel, label=AddonDetails._descriptionLabelText)
		...
		self.actionsButton.Bind(
			...
			handler=lambda e: self._actionsContextMenu.popupContextMenuFromPosition(
				self, self.actionsButton.Position
			),
		)
		...
		with guiHelper.autoThaw(self):
			...
			if numSelectedAddons > 1:
				...
				self.descriptionTextCtrl.SetStyle(
					0, self.descriptionTextCtrl.GetLastPosition(), self.defaultStyle
				)
