...
class AddonStoreDialog(SettingsDialog):
	...
	def makeSettings(self, settingsSizer: wx.BoxSizer):
		...
		tabPageHelper.addItem(self.listLabel, flag=wx.EXPAND)
		self._setListLabels()
	...
	def _createFilterControls(self, filterCtrlHelper: guiHelper.BoxSizerHelper) -> None:
		...
		self.includeIncompatibleCtrl = cast(
			wx.CheckBox, filterCtrlsLine0.addItem(wx.CheckBox(self, label=incompatibleAddonsLabel))
		)
		self.includeIncompatibleCtrl.SetValue(0)
		self.includeIncompatibleCtrl.Bind(
			wx.EVT_CHECKBOX, self.onIncompatibleFilterChange, self.includeIncompatibleCtrl
		)
	...
	def openExternalInstall(self, evt: wx.EVT_BUTTON):
		...
		try:
			...
			callLater(
				delay=0, callable=self._storeVM.onDisplayableError.notify, displayableError=displayableError
			)
			return
		self._storeVM.refresh()
