...
def _showAddonRequiresNVDAUpdateDialog(parent: wx.Window, addon: _AddonGUIModel) -> None:
...
def _showConfirmAddonInstallDialog(parent: wx.Window, addon: _AddonGUIModel) -> int:
...
class UpdatableAddonsDialog(
	...
	def _setupButtons(self, sHelper: BoxSizerHelper):
		...
		self.updateAllButton = bHelper.addButton(
			self, wx.ID_CLOSE, label=pgettext("addonStore", "&Update all")
		)
