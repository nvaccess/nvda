...
def processText(
	locale: str, text: str, symbolLevel: characterProcessing.SymbolLevel, normalize: bool = False
) -> str:
	"""
...
def speakMessage(text: str, priority: Optional[Spri] = None) -> None:
...
def spellTextInfo(
	info: textInfos.TextInfo, useCharacterDescriptions: bool = False, priority: Optional[Spri] = None
) -> None:
	...
	for field in info.getTextWithFields({}):
		if isinstance(field, str):
			speakSpelling(
				field, curLanguage, useCharacterDescriptions=useCharacterDescriptions, priority=priority
			)
...
def getSpellingSpeech(
	text: str, locale: Optional[str] = None, useCharacterDescriptions: bool = False
) -> Generator[SequenceItemT, None, None]:
	synth = getSynth()
	synthConfig = config.conf["speech"][synth.name]
...
def getObjectSpeech(
	...
	sequence = getObjectPropertiesSpeech(
		obj, reason=reason, _prefixSpeechCommand=_prefixSpeechCommand, **allowProperties
	)
	if reason == OutputReason.ONLYCACHE:
		return sequence
	if shouldReportTextContent:
		...
		if mathPres.speechProvider:
			try:
				sequence.extend(mathPres.speechProvider.getSpeechForMathMl(obj.mathMl))
...
def getIndentationSpeech(indentation: str, formatConfig: Dict[str, bool]) -> SpeechSequence:
	...
	toneIndentConfig = (
		...
		in (ReportLineIndentation.TONES, ReportLineIndentation.SPEECH_AND_TONES)
	...
	for index in range(len(speechSequence)):
		...
		if isinstance(item, str):
			speechSequence[index] = processText(
				curLanguage, item, symbolLevel, normalize=unicodeNormalization
			)
...
def speakPreselectedText(text: str, priority: Optional[Spri] = None):
...
def getPreselectedTextSpeech(text: str) -> SpeechSequence:
...
def speakTextSelected(text: str, priority: Optional[Spri] = None):
...
def speakSelectionMessage(message: str, text: str, priority: Optional[Spri] = None):
...
def _getTextInfoSpeech_considerSpelling(
	...
	if not onlyInitialFields:
		spellingSequence = list(getSpellingSpeech(textWithFields[0], locale=language))
...
def getPropertiesSpeech(  # noqa: C901
	reason: OutputReason = OutputReason.QUERY, **propertyValues
) -> SpeechSequence:
	...
	if (
		...
		and (
			roleText
			or reason
			not in (OutputReason.SAYALL, OutputReason.CARET, OutputReason.FOCUS, OutputReason.QUICKNAV)
			or not (name or value or cellCoordsText or rowNumber or columnNumber)
			or role not in controlTypes.silentRolesOnFocus
		)
		and (role != controlTypes.Role.MATH or reason not in (OutputReason.CARET, OutputReason.SAYALL))
	):
	...
	if cellCoordsText or rowNumber or columnNumber:
		...
		if includeTableCellCoords and not cellCoordsText and rowSpan > 1 and columnSpan > 1:
			...
			rowColSpanTranslation: str = _("through row {row} column {column}").format(
				row=rowNumber + rowSpan - 1, column=columnNumber + columnSpan - 1
			)
...
def getControlFieldSpeech(  # noqa: C901
	...
	presCat = attrs.getPresentationCategory(
		ancestorAttrs, formatConfig, reason=reason, extraDetail=extraDetail
	)
	...
	if config.conf["presentation"]["reportKeyboardShortcuts"]:
		keyboardShortcutSequence = getPropertiesSpeech(reason=reason, keyboardShortcut=keyboardShortcut)
	...
	if description is not None:
		descriptionSequence = getPropertiesSpeech(reason=reason, description=description)
	...
	if presCat == attrs.PRESCAT_SINGLELINE:
		...
		speakEntry = True
		speakExitForLine = bool(attrs.get("roleText") or role != controlTypes.Role.LANDMARK)
		speakExitForOther = True
	...
	if (
		...
		tableSeq.extend(getPropertiesSpeech(_tableID=tableID, rowCount=rowCount, columnCount=columnCount))
...
def getFormatFieldSpeech(  # noqa: C901
	...
	if formatConfig["reportTables"]:
		...
		tableSequence = getTableInfoSpeech(tableInfo, oldTableInfo, extraDetail=extraDetail)
		if tableSequence:
...
def getTableInfoSpeech(
	tableInfo: Optional[Dict[str, Any]], oldTableInfo: Optional[Dict[str, Any]], extraDetail: bool = False
) -> SpeechSequence:
