...
class SpeechManager(object):
	...
	def _queueSpeechSequence(self, inSeq: SpeechSequence, priority: Spri) -> bool:
		...
		log._speechManagerDebug(
			f"Current priority: {priority}," f" queLen: {0 if queue is None else len(queue.pendingSequences)}"
		)
		if not queue:
	...
	def _buildNextUtterance(self):
		...
		try:
			...
			log.error(f"Checking for cancellations failed, cancelling sequence: {utterance}", exc_info=True)
	...
	def _checkForCancellations(self, utterance: SpeechSequence) -> bool:
		...
		if utteranceIndex is None:
			raise IndexError(f"no utterance index({utteranceIndex}, cant save cancellable commands")
		...
		for item in cancellableItems:
			...
			if item.isCancelled:
				...
				log._speechManagerDebug(f"Speaking utterance with cancellable item, index: {utteranceIndex}")
	...
	def _removeCompletedFromQueue(self, index: int) -> Tuple[bool, bool]:  # noqa: C901
		...
		for seqIndex, seq in enumerate(self._curPriQueue.pendingSequences):
			...
			if isinstance(lastCommand, IndexCommand):
				if self._isIndexAAfterIndexB(index, lastCommand.index):
					...
					endOfUtterance = isinstance(
						self._curPriQueue.pendingSequences[seqIndex + 1][0], EndUtteranceCommand
					)
					if endOfUtterance:
			...
			log._speechManagerDebug("Unknown index. Probably from a previous utterance which was cancelled.")
			return False, False
		if endOfUtterance:
		...
		if _shouldCancelExpiredFocusEvents():
			cancellables = (
				item for seq in toRemove for item in seq if isinstance(item, _CancellableSpeechCommand)
			)
	...
	def _handleDoneSpeaking(self):
		log._speechManagerDebug(f"Synth done speaking, should push: {self._shouldPushWhenDoneSpeaking}")
	...
	def _switchProfile(self):
		...
		assert isinstance(
			command, ConfigProfileTriggerCommand
