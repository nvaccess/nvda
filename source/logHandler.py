...
def _unraisableExceptHook(unraisable: _UnraisableHookArgs) -> None:
	...
	log.exception(
		exc_info=(unraisable.exc_type, unraisable.exc_value, unraisable.exc_traceback), codepath=msg
	)


def _showwarning(message, category, filename, lineno, file=None, line=None):
	log.debugWarning(
		warnings.formatwarning(message, category, filename, lineno, line).rstrip(), codepath="Python warning"
	)
...
def initialize(shouldDoRemoteLogging=False):
	...
	if not shouldDoRemoteLogging:
		...
		logFormatter = Formatter(fmt="{codepath!s}:\n{message}", style="{")
