	...
	def _get_usbComPorts(self) -> list[dict[str, str]]:
		...
		for port in self.comPorts:
			if (usbId := port.get("usbID")) is None:
				continue
			if (usbDict := next((d for d in self.usbDevices if d.get("usbID") == usbId), None)) is not None:
...
class _Detector:
	...
	def _queueBgScan(
		self, usb: bool = False, bluetooth: bool = False, limitToDevices: Optional[List[str]] = None
	):
	...
	def _bgScan(self, usb: bool, bluetooth: bool, limitToDevices: Optional[List[str]]):
...
def driverHasPossibleDevices(driver: str) -> bool:
	...
	return bool(
		next(
			itertools.chain(
				getConnectedUsbDevicesForDriver(driver), getPossibleBluetoothDevicesForDriver(driver)
			),
			None,
		)
	)
...
class DriverRegistrar:
	...
	def addDeviceScanner(
		self, scanFunc: Callable[..., Iterable[Tuple[str, DeviceMatch]]], moveToStart: bool = False
	):
