...
AccessBridge_PropertyActiveDescendentChangeFP = CFUNCTYPE(
...
def _fixBridgeFuncs():
	...
	_fixBridgeFunc(
		BOOL, "getAccessibleContextFromHWND", HWND, POINTER(c_long), POINTER(JOBJECT64), errcheck=True
	)
	_fixBridgeFunc(HWND, "getHWNDFromAccessibleContext", c_long, JOBJECT64, errcheck=True)
	_fixBridgeFunc(
		BOOL, "getAccessibleContextAt", c_long, JOBJECT64, jint, jint, POINTER(JOBJECT64), errcheck=True
	)
	_fixBridgeFunc(
		BOOL, "getAccessibleContextWithFocus", HWND, POINTER(c_long), POINTER(JOBJECT64), errcheck=True
	)
	_fixBridgeFunc(
		BOOL, "getAccessibleContextInfo", c_long, JOBJECT64, POINTER(AccessibleContextInfo), errcheck=True
	)
	...
	_fixBridgeFunc(
		BOOL, "getAccessibleRelationSet", c_long, JOBJECT64, POINTER(AccessibleRelationSetInfo), errcheck=True
	)
	_fixBridgeFunc(
		BOOL,
	...
	_fixBridgeFunc(
		BOOL, "getAccessibleTextRect", c_long, JOBJECT64, POINTER(AccessibleTextRectInfo), jint, errcheck=True
	)
	_fixBridgeFunc(
		BOOL,
	...
	_fixBridgeFunc(
		BOOL, "getAccessibleTextRange", c_long, JOBJECT64, jint, jint, POINTER(c_char), c_short, errcheck=True
	)
	_fixBridgeFunc(
		BOOL,
	...
	_fixBridgeFunc(
		BOOL, "getCaretLocation", c_long, JOBJECT64, POINTER(AccessibleTextRectInfo), jint, errcheck=True
	)
	...
	_fixBridgeFunc(
		BOOL, "getAccessibleKeyBindings", c_long, JOBJECT64, POINTER(AccessibleKeyBindings), errcheck=True
	)
...
class JABContext(object):
	...
	def __eq__(self, jabContext):
		if self.vmID == jabContext.vmID and bridgeDll.isSameObject(
			self.vmID, self.accContext, jabContext.accContext
		):
			return True
		else:
			return False
	...
	def getAccessibleTextLineBounds(self, index):
		...
		startIndex = c_int()
		endIndex = c_int()
		bridgeDll.getAccessibleTextLineBounds(
			self.vmID, self.accContext, index, byref(startIndex), byref(endIndex)
		)
		...
		while not ok:
			bridgeDll.getAccessibleTextLineBounds(
				self.vmID, self.accContext, end, byref(startIndex), byref(endIndex)
			)
		...
		while not ok:
			bridgeDll.getAccessibleTextLineBounds(
				self.vmID, self.accContext, start, byref(startIndex), byref(endIndex)
			)
			tempStart = max(startIndex.value, 0)
			tempEnd = max(endIndex.value, 0)
			log.debug("line bounds: tempStart %s, tempEnd %s" % (tempStart, tempEnd))
	...
	def getTextAttributesInRange(self, startIndex, endIndex):
		...
		bridgeDll.getTextAttributesInRange(
			self.vmID, self.accContext, startIndex, endIndex, byref(attributes), byref(length)
		)
	...
	def getAccessibleTableInfo(self):
		...
		if bridgeDll.getAccessibleTableInfo(self.vmID, self.accContext, byref(info)):
			...
			info.jabCaption = (
				JABContext(hwnd=self.hwnd, vmID=self.vmID, accContext=info.caption) if info.caption else None
			)
			info.jabSummary = (
				JABContext(hwnd=self.hwnd, vmID=self.vmID, accContext=info.summary) if info.summary else None
			)
			info.jabContext = (
				JABContext(hwnd=self.hwnd, vmID=self.vmID, accContext=info.accessibleContext)
				if info.accessibleContext
				else None
			)
			info.jabTable = (
				JABContext(hwnd=self.hwnd, vmID=self.vmID, accContext=info.accessibleTable)
				if info.accessibleTable
				else None
			)
			return info
...
def event_stateChange(vmID, accContext, oldState, newState):
	...
	if "focused" in stateList or "selected" in stateList:
		...
		if (
			focus != obj
			and eventHandler.lastQueuedFocusObject != obj
			and obj.role in (controlTypes.Role.MENUITEM, controlTypes.Role.TAB, controlTypes.Role.MENU)
		):
			eventHandler.queueEvent("gainFocus", obj)
			return
	obj = (
...
def initialize():
	...
	try:
		bridgeDll = cdll.LoadLibrary(os.path.join(NVDAHelper.versionedLibPath, "windowsaccessbridge-32.dll"))
	except WindowsError:
		raise NotImplementedError("dll not available")
	_fixBridgeFuncs()
	if (
		# We should not attempt to write to disk from the secure desktop.
		not isRunningOnSecureDesktop() and config.isInstalledCopy() and not isBridgeEnabled()
	):
		enableBridge()
