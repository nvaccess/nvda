		...
		if os.access(portablePath, os.W_OK):
			executeParams = (
				'--create-portable --portable-path "{portablePath}" --config-path "{configPath}" -m'.format(
					portablePath=portablePath, configPath=WritePaths.configDir
				)
			)
		else:
...
class UpdateResultDialog(
		...
		if not remoteUpdateExists:
			...
			showAddonCompat = any(
				getIncompatibleAddons(
					currentAPIVersion=self.apiVersion, backCompatToAPIVersion=self.backCompatTo
				)
			)
			if showAddonCompat:
				message += "\n\n" + getAddonCompatibilityMessage()
				confirmationCheckbox = sHelper.addItem(
					wx.CheckBox(self, label=getAddonCompatibilityConfirmationMessage())
				)
				confirmationCheckbox.Bind(
					wx.EVT_CHECKBOX, lambda evt: self.updateButton.Enable(not self.updateButton.Enabled)
				)
			...
			self.updateButton.Bind(wx.EVT_BUTTON, lambda evt: self.onUpdateButton(pendingUpdateDetails[0]))
	...
	def onReviewAddonsButton(self, evt):
		...
		incompatibleAddons = addonGui.IncompatibleAddonsDialog(
			parent=self, APIVersion=self.apiVersion, APIBackwardsCompatToVersion=self.backCompatTo
		)
...
class UpdateAskInstallDialog(
	...
	def __init__(self, parent, destPath, version, apiVersion, backCompatTo):
		...
		showAddonCompat = any(
			getIncompatibleAddons(currentAPIVersion=self.apiVersion, backCompatToAPIVersion=self.backCompatTo)
		)
		if showAddonCompat:
			message += "\n" + getAddonCompatibilityMessage()
		...
		if not showAddonCompat:
			...
			self.confirmationCheckbox.Bind(
				wx.EVT_CHECKBOX, lambda evt: updateButton.Enable(not updateButton.Enabled)
			)
	...
	def onPostponeButton(self, evt):
		...
		try:
			...
			log.debugWarning(
				"Unable to rename the file from {} to {}".format(self.destPath, finalDest), exc_info=True
			)
			gui.messageBox(
...
def _updateWindowsRootCertificates():
	...
	crypt.CertGetCertificateChain(
		...
		None,
		ctypes.byref(
			CERT_CHAIN_PARA(cbSize=ctypes.sizeof(CERT_CHAIN_PARA), RequestedUsage=CERT_USAGE_MATCH())
		),
		0,
		None,
