...
class OffsetsTextInfo(textInfos.TextInfo):
	...
	def _get_boundingRects(self) -> List[locationHelper.RectLTWH]:  # noqa: C901
		...
		if inclusiveEndOffset > startOffset:
			...
			rects.append(locationHelper.RectLTWH.fromCollection(startLocation))
		intersectedRects = []
		for rect in rects:
	...
	def _calculateUniscribeOffsets(
		self, lineText: str, unit: str, relOffset: int
		...
		if helperFunc(
			uniscribeLineText, uniscribeLineLength, relOffset, ctypes.byref(relStart), ctypes.byref(relEnd)
		):
	...
	def getTextWithFields(self, formatConfig: Optional[Dict] = None) -> textInfos.TextInfo.TextWithFieldsT:
		...
		if self.detectFormattingAfterCursorMaybeSlow and not formatConfig["detectFormatAfterCursor"]:
			field, (boundStart, boundEnd) = self._getFormatFieldAndOffsets(
				self._startOffset, formatConfig, calculateOffsets=False
			)
			text = self.text
