		...
		try:
			...
			log.warning(
				"Could not execute function {func} defined in {module} module; kwargs: {kwargs}".format(
					func=func.__name__, module=func.__module__ or "unknown", kwargs=self.kwargs
				),
				exc_info=True,
			)
...
class FocusLossCancellableSpeechCommand(_CancellableSpeechCommand):
	...
	def isMenuItemOfCurrentFocus(self) -> bool:
		...
		if not (
			...
			in (oleacc.ROLE_SYSTEM_MENUITEM, IA2.IA2_ROLE_CHECK_MENU_ITEM, IA2.IA2_ROLE_RADIO_MENU_ITEM)
...
def shouldAcceptEvent(eventName, windowHandle=None):
	...
	key = (eventName, winUser.getWindowThreadProcessID(windowHandle)[0], wClass)
	...
	if (
		...
		or winUser.user32.GetWindowLongW(
			winUser.getAncestor(windowHandle, winUser.GA_ROOT), winUser.GWL_EXSTYLE
		)
		...
		if winUser.getClassName(gi.hwndFocus) == "Chrome_WidgetWin_0":
			...
			try:
				...
				try:
					...
					log.debugWarning(
						"Unable to normalize root Chromium element to focused ancestor", exc_info=True
					)
