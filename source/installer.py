...
def registerInstallation(
	...
	with winreg.CreateKeyEx(
		...
		for name, value in calculatedUninstallerRegInfo.items():
			...
			winreg.SetValueEx(k, name, None, regType, value)
	with winreg.CreateKeyEx(
		winreg.HKEY_LOCAL_MACHINE,
	...
	with winreg.CreateKeyEx(
		winreg.HKEY_LOCAL_MACHINE, config.RegistryKey.NVDA.value, 0, winreg.KEY_WRITE
	) as k:
		...
		if NVDAState._forceSecureModeEnabled():
			winreg.SetValueEx(k, config.RegistryKey.FORCE_SECURE_MODE_SUBKEY.value, None, winreg.REG_DWORD, 1)
...
def _createShortcutWithFallback(
	...
	try:
		createShortcut(
			path, targetPath, arguments, iconLocation, workingDirectory, hotkey, prependSpecialFolder
		)
	except Exception:
...
def unregisterInstallation(keepDesktopShortcut=False):
	try:
		winreg.DeleteKeyEx(
			winreg.HKEY_LOCAL_MACHINE, easeOfAccess.RegistryKey.APP.value, winreg.KEY_WOW64_64KEY
		)
	...
	try:
		...
		with winreg.CreateKeyEx(
			...
			k2 = winreg.CreateKeyEx(
				k, "OpenWithProgids\\%s" % addonHandler.NVDA_ADDON_PROG_ID, 0, winreg.KEY_WRITE
			)
			winreg.CloseKey(k2)
...
def unregisterAddonFileAssociation():
	try:
		...
		_deleteKeyAndSubkeys(
			winreg.HKEY_LOCAL_MACHINE, "Software\\Classes\\%s" % addonHandler.NVDA_ADDON_PROG_ID
		)
	except WindowsError:
...
def tryRemoveFile(path: str, numRetries: int = 6, retryInterval: float = 0.5, rebootOK: bool = False):
...
_nvdaExes = {"nvda.exe", "nvda_noUIAccess.exe", "nvda_uiAccess.exe", "nvda_dmp.exe", "nvda_slave.exe"}
...
def _deleteFileGroupOrFail(
	installDir: str, relativeFilepaths: Iterable[str], numTries: int = 6, retryWaitInterval: float = 0.5
):
	"""
...
def install(shouldCreateDesktopShortcut: bool = True, shouldRunAtLogon: bool = True):
	...
	_deleteFileGroupOrFail(
		installDir, _nvdaExes.union({"nvda_service.exe"}), numTries=6, retryWaitInterval=0.5
	)
...
def registerEaseOfAccess(installDir):
	with winreg.CreateKeyEx(
		...
		winreg.SetValueEx(appKey, "ApplicationName", None, winreg.REG_SZ, versionInfo.name)
		winreg.SetValueEx(appKey, "Description", None, winreg.REG_SZ, versionInfo.longName)
		winreg.SetValueEx(
			appKey,
			"Profile",
			None,
		...
		winreg.SetValueEx(appKey, "SimpleProfile", None, winreg.REG_SZ, "screenreader")
		winreg.SetValueEx(appKey, "ATExe", None, winreg.REG_SZ, "nvda.exe")
		winreg.SetValueEx(appKey, "StartExe", None, winreg.REG_SZ, os.path.join(installDir, "nvda.exe"))
		winreg.SetValueEx(appKey, "StartParams", None, winreg.REG_SZ, "--ease-of-access")
		winreg.SetValueEx(appKey, "TerminateOnDesktopSwitch", None, winreg.REG_DWORD, 0)
