...
def tryRemoveDir(
	...
	"""Attempt to remove a directory and all its contents, with retries and optional
	scheduling for deletion on reboot.

	The directory is first renamed to a temporary path in the same parent directory,
	freeing the original path immediately. Removal of the temporary directory is then
	attempted up to numRetries times with retryInterval seconds between each attempt.

	If all immediate removal attempts fail and rebootOK is True, all files and
	subdirectories are individually scheduled for deletion on reboot using MoveFileEx.
	The tree is walked bottom-up so that each directory is empty by the time Windows
	processes its deletion entry — MoveFileEx can only remove a directory on reboot if
	it is empty at that point.

	If removal cannot be completed or scheduled for reboot, the directory is renamed
	back to its original path and RetriableFailure is raised.

	:param path: The path of the directory to remove.
	:param numRetries: The number of times to retry removal before giving up.
	:param retryInterval: The time in seconds to wait between retries.
	:param rebootOK: If True, schedule the directory for deletion on next reboot if
		immediate removal fails.
	:raises RetriableFailure: If the directory could not be removed or scheduled for removal.
	"""
	parentDir = os.path.dirname(path) or "."
	# Create an empty temp directory in the same parent directory to use as the rename target.
	# Since it is empty, os.replace will succeed, mirroring the file removal pattern.
	tempPath = tempfile.mkdtemp(dir=parentDir)
	try:
		os.replace(path, tempPath)
	except (WindowsError, OSError):
		raise RetriableFailure("Failed to rename directory %s before removal" % path)
	for count in range(numRetries):
		try:
			shutil.rmtree(tempPath)
			return
			...
			log.debugWarning(
				f"Failed to delete directory {tempPath}, attempt {count}/{numRetries}",
				exc_info=True,
			)
		time.sleep(retryInterval)
	if rebootOK:
		log.debugWarning("Failed to delete directory %s, marking for delete on reboot" % tempPath)
		# Use escapes in a unicode string instead of raw.
		# In a raw string the trailing slash escapes the closing quote leading to a python syntax error.
		pathQualifier = "\\\\?\\"
		try:
			# MoveFileEx can only delete a directory on reboot when it is empty at that point.
			# Walk bottom-up: files are scheduled before their containing directories,
			# and subdirectories before their parents, so each directory will be empty
			# when Windows processes it.
			for dirpath, _subdirs, files in os.walk(tempPath, topdown=False):
				for filename in files:
					winKernel.moveFileEx(
						pathQualifier + os.path.join(dirpath, filename),
						None,
						winKernel.MOVEFILE_DELAY_UNTIL_REBOOT,
					)
				# Schedule this directory itself after all its contents.
				winKernel.moveFileEx(pathQualifier + dirpath, None, winKernel.MOVEFILE_DELAY_UNTIL_REBOOT)
		except WindowsError:
			log.debugWarning(f"Failed to mark directory {tempPath} for delete on reboot", exc_info=True)
		else:
			return
	try:
		os.replace(tempPath, path)
	except Exception:
		log.exception(f"Unable to rename directory back to {path} before retriable failure")
	raise RetriableFailure("Directory %s could not be removed" % path)
