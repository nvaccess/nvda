...
class BoundMethodWeakref(Generic[HandlerT]):
	...
	def __init__(self, target: HandlerT, onDelete: Optional[Callable[[BoundMethodWeakref], None]] = None):
		if onDelete:
...
class HandlerRegistrar(Generic[HandlerT]):
	...
	def __init__(self):
		...
		self._handlers = OrderedDict[
			HandlerKeyT, Union[BoundMethodWeakref[HandlerT], AnnotatableWeakref[HandlerT]]
		]()
	...
	def unregister(
		self, handler: Union[AnnotatableWeakref[HandlerT], BoundMethodWeakref[HandlerT], HandlerT]
	):
		if isinstance(handler, (AnnotatableWeakref, BoundMethodWeakref)):
			key = handler.handlerKey
		else:
