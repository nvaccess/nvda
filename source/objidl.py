...
class IOleWindow(IUnknown):
	...
	_methods_ = [
		COMMETHOD([], HRESULT, "GetWindow", (["out"], POINTER(HWND), "phwnd")),
		COMMETHOD([], HRESULT, "ContextSensitiveHelp", (["in"], BOOL, "fEnterMode")),
	]
...
IStream._methods_ = [
	...
	COMMETHOD(
		[], HRESULT, "Stat", (["out"], POINTER(tagSTATSTG), "pstatstg"), (["in"], c_ulong, "grfStatFlag")
	),
...
class IBindCtx(IUnknown):
	...
	_idlflags_ = []
	_methods_ = [
		COMMETHOD([], HRESULT, "RegisterObjectBound", (["in"], POINTER(IUnknown), "punk")),
		...
		COMMETHOD(
			[], HRESULT, "GetRunningObjectTable", (["out"], POINTER(POINTER(IRunningObjectTable)), "pprot")
		),
		COMMETHOD(
			[],
			HRESULT,
