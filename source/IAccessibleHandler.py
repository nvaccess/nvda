...
def processFocusNVDAEvent(obj,needsFocusedState=True):
	...
	if needsFocusedState and obj.windowClassName.startswith("Mozilla") and obj.IAccessibleRole in (ROLE_SYSTEM_COMBOBOX, ROLE_SYSTEM_DOCUMENT, ROLE_SYSTEM_LIST):
...
def initialize():
	global _IA2Dll, _IA2RegCooky
	#Check if IAccessible2 is already registered
	proxyIid=GUID()
	try:
		oledll.ole32.CoGetPSClsid(byref(IAccessible2._iid_),byref(proxyIid))
		isRegistered=True
	except:
		isRegistered=False
	if not isRegistered:
