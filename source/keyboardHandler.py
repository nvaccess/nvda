	...
	try:
		...
		if (
			...
			res = ctypes.windll.user32.ToUnicodeEx(
				vkCode, scanCode, keyStates, charBuf, len(charBuf), 0x4, hkl
			)
			if res > 0:
...
class KeyboardInputGesture(inputCore.InputGesture):
	...
	def _get_identifiers(self):
		...
		return ("kb({layout}):{key}".format(layout=self.layout, key=keyName), "kb:{key}".format(key=keyName))
	...
	def _reportToggleKey(self):
		...
		ui.message(
			"{key} {state}".format(
				key=localizedKeyLabels.get(key.lower(), key), state=_("on") if toggleState else _("off")
			)
		)
