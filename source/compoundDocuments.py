	...
	def _normalizeStartAndEnd(self):
		if (
			...
			and self._start.compareEndPoints(
				self._makeRawTextInfo(self._startObj, textInfos.POSITION_ALL), "endToEnd"
			)
			== 0
		):
		...
		if (
			...
			and self._end.compareEndPoints(
				self._makeRawTextInfo(self._endObj, textInfos.POSITION_FIRST), "startToStart"
			)
			== 0
		):
		...
		if end:
			if (
				self._end.compareEndPoints(
					self._makeRawTextInfo(self._endObj, textInfos.POSITION_ALL), "endToEnd"
				)
				== 0
			):
	...
	def getTextWithFields(self, formatConfig: Optional[Dict] = None) -> textInfos.TextInfo.TextWithFieldsT:
		...
		for ti in self._getTextInfos():
			for textWithEmbeddedObjectsItem in ti._iterTextWithEmbeddedObjects(
				True, formatConfig=formatConfig
			):
