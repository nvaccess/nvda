...
class DocumentWithTableNavigation(TextContainerObject, ScriptableObject):
	...
	def _getFirstOrLastTableCell(
		self, startPos: textInfos.TextInfo, cell: _TableCell, movement: _Movement, axis: _Axis
	) -> textInfos.TextInfo:
		"""
	...
	def _tableFindNewCell(
		...
		try:
			if movement in {_Movement.PREVIOUS, _Movement.NEXT}:
				...
				info = self._getFirstOrLastTableCell(self.selection, cell, movement, axis)
	...
	def _tableMovementScriptHelper(self, movement: _Movement = _Movement.NEXT, axis: Optional[_Axis] = None):
		# documentBase is a core module and should not depend on these UI modules and so they are imported
		# at run-time. (#12404)
