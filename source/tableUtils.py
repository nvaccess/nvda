...
class HeaderCellTracker:
	...
	def iterPossibleHeaderCellInfosFor(
		self,
		rowNumber,
		columnNumber,
		minRowNumber=None,
		...
		for key in self.listByRow:  # (self.listByColumn if columnHeader else self.listByRow):
			...
			if (
				(info.minColumnNumber and info.minColumnNumber > columnNumber)
				or (info.maxColumnNumber and info.maxColumnNumber < columnNumber)
				or (info.minRowNumber and info.minRowNumber > rowNumber)
				or (info.maxRowNumber and info.maxRowNumber < rowNumber)
			):
				# Skipping this possible header as the requested coordinates are outside the header's allowed range
				continue
			if (
				(minColumnNumber and minColumnNumber > info.columnNumber)
				or (maxColumnNumber and maxColumnNumber < info.columnNumber)
				or (minRowNumber and minRowNumber > info.rowNumber)
				or (maxRowNumber and maxRowNumber < info.rowNumber)
			):
