...
def __getattr__(attrName: str) -> Any:
	...
	if NVDAState._allowDeprecatedAPI():
		if attrName == "isObjectAboveLockScreen":
			log.warning("isObjectAboveLockScreen(obj) is deprecated. " "Instead use obj.isBelowLockScreen. ")
...
def _isObjectAboveLockScreen(obj: "NVDAObjects.NVDAObject") -> bool:
	log.error("This function is deprecated. " "Instead use obj.isBelowLockScreen. ")
...
def _isObjectBelowLockScreen(obj: "NVDAObjects.NVDAObject") -> bool:
	...
	if not isinstance(obj, Window):
		log.debug("Cannot detect if object is below lock app, considering object as safe. ")
...
def _isObjectBelowLockScreenCheckZOrder(objWindowHandle: int) -> bool:
	...
	try:
		...
		log.debugWarning("Couldn't determine lock screen and NVDA object relative z-order", exc_info=True)
		return False
...
def _isWindowBelowWindowMatchesCond(
	window: winUser.HWNDVal, matchCond: Callable[[winUser.HWNDVal], bool]
) -> bool:
	"""
	This is a risky hack.
