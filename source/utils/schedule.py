...
class ScheduleThread(threading.Thread):
	...
	def scheduleDailyJobAtStartUp(
		self, task: Callable, queueToThread: ThreadTarget, *args, **kwargs
	) -> schedule.Job:
		"""
		...
		try:
			job = self.scheduleDailyJob(
				task, self._calculateDailyTimeOffset(), queueToThread, *args, **kwargs
			)
	...
	def scheduleDailyJob(
		self, task: Callable, cronTime: str, queueToThread: ThreadTarget, *args, **kwargs
	) -> schedule.Job:
		"""
	...
	def scheduleJob(
		self, task: Callable, jobSchedule: schedule.Job, queueToThread: ThreadTarget, *args, **kwargs
	) -> schedule.Job:
		"""
		...
		match queueToThread:
			...
			case ThreadTarget.DAEMON:
				...
				def callJobOnThread(*args, **kwargs):  # noqa F811: lint bug with flake8 4.0.1 not recognizing case statement
					t = threading.Thread(
						target=task, args=args, kwargs=kwargs, daemon=True, name=f"{task.__name__}"
					)
