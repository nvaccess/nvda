...
class SecurePopen(PopenWithToken):
	...
	def __init__(
		...
		removePrivileges: bool = True,
		removeElevation: bool = True,
		restrictToken: bool = True,
		retainUserInRestrictedToken: bool = False,
		runAsLocalService: bool = False,
		applyUIRestrictions=True,
		isolateDesktop: bool = False,
		isolateWindowStation: bool = False,
		killOnDelete: bool = True,
		...
		"""
		log.debug(
			f"Preparing to launch secure process: {subprocess.list2cmdline(argv)}, options: {integrityLevel=}, {removePrivileges=}, {removeElevation=}, {restrictToken=}, {retainUserInRestrictedToken=}, {runAsLocalService=}, {isolateDesktop=}, {isolateWindowStation=}, {killOnDelete=}, {startSuspended=}, {hideCriticalErrorDialogs=}...",
		)
		if runAsLocalService:
			log.debug(
				"runAsLocalService requested, creating service logon token and ensuring secLogon is used to launch process...",
			)
			useSecLogon = True
			...
			if removeElevation and isTokenElevated(token):
				...
				log.debug(
					"Using current primary token as source token for child process. Not using secLogon for process launch.",
				)
				useSecLogon = False
		...
		restrictedSD = createRestrictedSecurityDescriptor(
			logonSidString,
		...
		if restrictToken and not retainUserInRestrictedToken:
			log.debug(
				"Ensuring restricted token has an appropriate default DACL  so  it can access its own sandbox directory and handles...",
			)
