...
class FeatureFlag:
	...
	def __init__(self, value: FlagValueEnum, behaviorOfDefault: FlagValueEnum):
		self.value = value
	...
	def __bool__(self) -> bool:
		if not isinstance(self.value, BoolFlag):
			raise NotImplementedError("Only BoolFlag supported. For other types use explicit checks")
		if self.isDefault():
...
def _validateConfig_featureFlag(
	value: Union[str, FeatureFlag, None], optionsEnum: str, behaviorOfDefault: str
) -> FeatureFlag:
...
def _transformSpec_AddFeatureFlagDefault(specString: str, **kwargs) -> str:
	...
	if "default=" in specString:
		raise VdtParamError(name_or_msg=f"Param 'default' not expected. {usage}", value=specString)

	optionsEnumKey = "optionsEnum"
	if optionsEnumKey not in kwargs:
		raise VdtParamError(name_or_msg=f"Param '{optionsEnumKey}' missing. {usage}", value=specString)
	...
	if behaviorOfDefaultKey not in kwargs:
		raise VdtParamError(name_or_msg=f"Param '{behaviorOfDefaultKey}' missing. {usage}", value=specString)
	...
	if len(kwargs) != 2:
		raise VdtParamError(
			name_or_msg=("Unexpected number of params." f" Got {kwargs}. {usage}"), value=specString
		)
