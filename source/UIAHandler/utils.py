...
def iterUIARangeByUnit(rangeObj, unit, reverse=False):
	...
	while relativeGTOperator(endRange.Move(unit, minRelativeDistance), 0):
		...
		pastEnd = relativeGTOperator(
			tempRange.CompareEndpoints(Endpoint_relativeEnd, rangeObj, Endpoint_relativeEnd), 0
		)
		if pastEnd:
		...
		try:
			...
			for (scope, cacheRequest, properties), handler in self._propertyChangedEventHandlers.items():
				self.clientObject.AddPropertyChangedEventHandler(
					element, scope, cacheRequest, handler, properties
				)
...
def _getConhostAPILevel(hwnd: int) -> WinConsoleAPILevel:
	...
	try:
		...
		textArea = UIAElement.buildUpdatedCache(textAreaCacheRequest).getCachedChildren().GetElement(0)
		UIATextPattern = textArea.GetCurrentPattern(UIAHandler.UIA_TextPatternId).QueryInterface(
		...
		if visiRanges.length == 1:
			...
			# information to UIA.
			if isinstance(
				visiRanges.GetElement(0).GetAttributeValue(UIAHandler.UIA_FontNameAttributeId), str
			):
