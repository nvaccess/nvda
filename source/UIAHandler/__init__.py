...
class UIAHandler(COMObject):
	...
	def terminate(self):
		...
		MTAThreadHandle = ctypes.wintypes.HANDLE(
			windll.kernel32.OpenThread(winKernel.SYNCHRONIZE, False, self.MTAThread.ident)
		)
		...
		try:
			...
			if config.conf["UIA"]["enhancedEventProcessing"]:
				...
				NVDAHelper.localLib.rateLimitedUIAEventHandler_create(
					self._com_pointers_[IUnknown._iid_], byref(self._rateLimitedEventHandler)
				)
			else:
				handler = self
	...
	def _registerGlobalEventHandlers(self, handler: "UIAHandler"):
		...
		for eventId in (
			...
			self.globalEventHandlerGroup.AddAutomationEventHandler(
				eventId, UIA.TreeScope_Subtree, self.baseCacheRequest, handler
			)
		if not utils._shouldSelectivelyRegister() and winVersion.getWinVer() >= winVersion.WIN10:
			...
			self.globalEventHandlerGroup.AddAutomationEventHandler(
				UIA.UIA_Text_TextChangedEventId, UIA.TreeScope_Subtree, self.baseCacheRequest, handler
			)
		...
		if isinstance(self.clientObject, UIA.IUIAutomation5):
			self.globalEventHandlerGroup.AddNotificationEventHandler(
				UIA.TreeScope_Subtree, self.baseCacheRequest, handler
			)
		if isinstance(self.clientObject, UIA.IUIAutomation6):
	...
	def _createLocalEventHandlerGroup(self, handler: "UIAHandler"):
		...
		for eventId in localEventHandlerGroupUIAEventIds:
			self.localEventHandlerGroup.AddAutomationEventHandler(
				eventId, UIA.TreeScope_Ancestors | UIA.TreeScope_Element, self.baseCacheRequest, handler
			)
	...
	def IUIAutomationEventHandler_HandleAutomationEvent(self, sender, eventID):
		...
		if isinstance(focus, NVDAObjects.UIA.UIA) and self.clientObject.compareElements(
			...
			if _isDebug():
				log.debug(f"HandleAutomationEvent: Ignoring event {NVDAEventName} for non native element")
			return
		...
		if not obj:
			...
			if not obj:
				...
				return
			if _isDebug():
				log.debug(f"handleAutomationEvent: created object {obj} ")
		if (NVDAEventName == "gainFocus" and not obj.shouldAllowUIAFocusEvent) or (
			...
			return
		if _isDebug():
			log.debug(f"handleAutomationEvent: queuing NVDA event {NVDAEventName} " f"for NVDAObject {obj} ")
	...
	def IUIAutomationFocusChangedEventHandler_HandleFocusChangedEvent(self, sender):
		...
		if isinstance(eventHandler.lastQueuedFocusObject, NVDAObjects.UIA.UIA):
			...
			try:
				...
				if _isDebug():
					log.debugWarning(
						"HandleFocusChangedEvent: Couldn't check for duplicate focus event ", exc_info=True
					)
		...
		try:
			...
			if _isDebug():
				log.debugWarning(
					"HandleFocusChangedEvent: Exception while creating NVDAObject ", exc_info=True
				)
			obj = None
		if not obj:
			if _isDebug():
				log.debug("handleFocusChangedEvent: Could not create an NVDAObject ")
			return
		if _isDebug():
			log.debug(f"Created object {obj} for element {self.getUIAElementDebugString(sender)}")
		if not obj.shouldAllowUIAFocusEvent:
			if _isDebug():
				log.debug("HandleFocusChangedEvent: NVDAObject chose to ignore event ")
			return
		if _isDebug():
			log.debug("handleFocusChangedEvent: Queuing NVDA gainFocus event " f"for obj {obj} ")
	...
	def IUIAutomationPropertyChangedEventHandler_HandlePropertyChangedEvent(
		...
		if not obj:
			...
			if not obj:
				...
				return
			if _isDebug():
				log.debug(f"handlePropertyChangeEvent: created object {obj} ")
		if _isDebug():
			log.debug(
	...
	def IUIAutomationNotificationEventHandler_HandleNotificationEvent(
		...
		if not obj:
			...
			return
		if _isDebug():
			log.debug("Queuing UIA_notification NVDA event " f"for NVDAObject {obj}")
	...
	def IUIAutomationActiveTextPositionChangedEventHandler_HandleActiveTextPositionChangedEvent(
		...
		try:
			...
			if _isDebug():
				log.debugWarning(
					"HandleActiveTextPositionChangedEvent: Exception while creating object: ", exc_info=True
				)
			return
		if not obj:
			if _isDebug():
				log.debug("HandleActiveTextPositionchangedEvent: Ignoring because no object: ")
			return
		if _isDebug():
			log.debug(
	...
	def _isUIAWindowHelper(self, hwnd: int, isDebug=False) -> bool:  # noqa: C901
		...
		if appModule and appModule.isGoodUIAWindow(hwnd):
			if isDebug:
				log.debug(f"appModule {appModule.appName} says to treat window as UIA")
			return True
		...
		if appModule and appModule.isBadUIAWindow(hwnd):
			if isDebug:
				log.debug(f"appModule {appModule.appName} says to not treat window as UIA")
			return False
	...
	def isUIAWindow(self, hwnd: int, isDebug: bool = False) -> bool:
		...
		if not v or (now - v[1]) > 0.5:
			v = (self._isUIAWindowHelper(hwnd, isDebug=isDebug), now)
	...
	def getNearestWindowHandle(self, UIAElement):
		...
		try:
			...
			log.debugWarning("error walking up to an element with a valid windowHandle", exc_info=True)
			return None
		try:
		...
		if _isDebug():
			log.debug(
				"Found ancestor element " f"with valid windowHandle {self.getWindowHandleDebugString(window)}"
			)
	...
	def isNativeUIAElement(self, UIAElement):
		...
		if processID == globalVars.appPid:
			if _isDebug():
				log.debug("element is local to NVDA, " "treating as non-native.")
			return False
		...
		if windowHandle:
			...
			if self._isNetUIEmbeddedInWordDoc(UIAElement):
				if _isDebug():
					log.debug("treating as native as is a netUI embedded in word doc. ")
				return True
			if (
				...
				if _isDebug():
					log.debug("treating as native as is in IE9 downloads list. ")
				return True
		if _isDebug():
