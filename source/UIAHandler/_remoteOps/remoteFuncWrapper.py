...
from typing import Generator, ContextManager, Callable, Concatenate, ParamSpec, TypeVar
import functools
import contextlib
from . import builder
...
class _BaseRemoteFuncWrapper:
	...
	def _execRawFunc(
		...
		**kwargs: _remoteFunc_paramSpec.kwargs,
		...
		main.addComment(f"Entering {func.__qualname__}{self.generateArgsKwargsString(*args, **kwargs)}")
...
class RemoteContextManager(_BaseRemoteFuncWrapper):
	...
	def _execRawFunc(
		self,
		func: Callable[
			Concatenate[_remoteFunc_self, _remoteFunc_paramSpec], ContextManager[_remoteFunc_return]
		],
		funcSelf: _remoteFunc_self,
		*args: _remoteFunc_paramSpec.args,
		**kwargs: _remoteFunc_paramSpec.kwargs,
