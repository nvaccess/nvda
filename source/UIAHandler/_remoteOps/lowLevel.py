...
from ctypes import oledll, byref, c_void_p, c_long, c_ulong, c_bool
...
PropertyId = enum.IntEnum(
	"PropertyId", {k[4:-10]: v for k, v in vars(UIA).items() if k.endswith("PropertyId")}
)


AttributeId = enum.IntEnum(
	"AttributeId", {k[4:-11]: v for k, v in vars(UIA).items() if k.endswith("AttributeId")}
)

StyleId = enum.IntEnum("StyleId", {k[8:]: v for k, v in vars(UIA).items() if k.startswith("StyleId")})
