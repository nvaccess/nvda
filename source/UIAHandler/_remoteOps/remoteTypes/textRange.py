...
class RemoteTextRange(RemoteExtensionTarget[POINTER(UIA.IUIAutomationTextRange)]):
	...
	def clone(self) -> RemoteTextRange:
		...
		self.rob.getDefaultInstructionList().addInstruction(
			instructions.TextRangeClone(result=result, target=self)
		)
		return result
	...
	def getEnclosingElement(self) -> RemoteElement:
		...
		self.rob.getDefaultInstructionList().addInstruction(
			instructions.TextRangeGetEnclosingElement(result=result, target=self)
		)
		return result
	...
	def getText(self, maxLength: RemoteInt | int) -> RemoteString:
		...
		self.rob.getDefaultInstructionList().addInstruction(
			instructions.TextRangeGetText(
				result=result, target=self, maxLength=RemoteInt.ensureRemote(self.rob, maxLength)
			)
		)
		return result
	...
	def expandToEnclosingUnit(self, unit: RemoteIntEnum[lowLevel.TextUnit] | lowLevel.TextUnit):
		self.rob.getDefaultInstructionList().addInstruction(
			instructions.TextRangeExpandToEnclosingUnit(
				target=self, unit=RemoteIntEnum.ensureRemote(self.rob, unit)
			)
		)
	...
	def getAttributeValue(
		self, attributeId: RemoteIntEnum[lowLevel.AttributeId] | lowLevel.AttributeId
	) -> RemoteVariant:
		result = RemoteVariant(self.rob, self.rob.requestNewOperandId())
		self.rob.getDefaultInstructionList().addInstruction(
			instructions.TextRangeGetAttributeValue(
				result=result, target=self, attributeId=RemoteIntEnum.ensureRemote(self.rob, attributeId)
			)
		)
		return result
...
class _RemoteTextRangeEndpoint(builder._RemoteBase):
	def __init__(
		self, rob: builder.RemoteOperationBuilder, textRangeLA: RemoteTextRangeLogicalAdapter, isStart: bool
	):
		super().__init__(rob)
	...
	def moveByUnit(
		self, unit: RemoteIntEnum[lowLevel.TextUnit] | lowLevel.TextUnit, count: RemoteInt | int
	) -> RemoteInt:
...
class RemoteTextRangeLogicalAdapter(builder._RemoteBase):
	def __init__(
		self, rob: builder.RemoteOperationBuilder, textRange: RemoteTextRange, reverse: bool = False
	):
		super().__init__(rob)
