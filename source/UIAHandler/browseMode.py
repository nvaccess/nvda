...
class UIADocumentWithTableNavigation(documentBase.DocumentWithTableNavigation):
	def _getTableCellAt(self, tableID, startPos, row, column):
		...
		UIACondition = UIAHandler.handler.clientObject.createPropertyCondition(
			UIAHandler.UIA_RuntimeIdPropertyId, tableIDArray
		)
...
class HeadingUIATextInfoQuickNavItem(browseMode.TextInfoQuickNavItem):
	def __init__(
		self, itemType: str, document: UIA, position: UIATextInfo, label: str | None = None, level: int = 0
	):
	...
	if UIAHandler.remote.isSupported():
		if position is None:
			...
			heading = UIAHandler.remote.findFirstHeadingInTextRange(
				searchArea._rangeObj, wantedLevel, reverse
			)
...
def UIAControlQuicknavIterator(
	itemType, document, position, UIACondition, direction="next", itemClass=UIATextRangeQuickNavItem
):
	...
	if direction == "up":
		...
		toPosition.MoveEndpointByRange(
			UIAHandler.TextPatternRangeEndpoint_Start, childRange, UIAHandler.TextPatternRangeEndpoint_Start
		)
		...
		toPosition = position._rangeObj.clone()
		toPosition.MoveEndpointByRange(
			UIAHandler.TextPatternRangeEndpoint_End, childRange, UIAHandler.TextPatternRangeEndpoint_End
		)
		zoomedOnce = False
		while True:
			children = toPosition.getChildren()
...
class UIABrowseModeDocumentTextInfo(
	browseMode.BrowseModeDocumentTextInfo, treeInterceptorHandler.RootProxyTextInfo
):
...
class UIABrowseModeDocument(UIADocumentWithTableNavigation, browseMode.BrowseModeDocumentTreeInterceptor):
	...
	def _iterNodesByType(self, nodeType, direction="next", pos=None):
		if nodeType.startswith("heading"):
			...
			return UIATextAttributeQuicknavIterator(
				ErrorUIATextInfoQuickNavItem, nodeType, self, pos, direction=direction
			)
		elif nodeType == "link":
			condition = UIAHandler.handler.clientObject.createPropertyCondition(
				UIAHandler.UIA_ControlTypePropertyId, UIAHandler.UIA_HyperlinkControlTypeId
			)
			...
			condition = UIAHandler.handler.clientObject.createPropertyCondition(
				UIAHandler.UIA_ControlTypePropertyId, UIAHandler.UIA_CheckBoxControlTypeId
			)
			return UIAControlQuicknavIterator(nodeType, self, pos, condition, direction)
		elif nodeType == "radioButton":
			condition = UIAHandler.handler.clientObject.createPropertyCondition(
				UIAHandler.UIA_ControlTypePropertyId, UIAHandler.UIA_RadioButtonControlTypeId
			)
			return UIAControlQuicknavIterator(nodeType, self, pos, condition, direction)
			...
			condition = UIAHandler.handler.clientObject.createPropertyCondition(
				UIAHandler.UIA_ControlTypePropertyId, UIAHandler.UIA_SeparatorControlTypeId
			)
			return UIAControlQuicknavIterator(nodeType, self, pos, condition, direction)
			...
			condition = UIAHandler.handler.clientObject.createNotCondition(
				UIAHandler.handler.clientObject.createPropertyCondition(
					UIAHandler.UIA.UIA_LandmarkTypePropertyId, 0
				)
			)
			return UIAControlQuicknavIterator(nodeType, self, pos, condition, direction)
			...
			condition = UIAHandler.handler.clientObject.CreateAndConditionFromArray(
				[
					UIAHandler.handler.clientObject.createPropertyCondition(
						UIAHandler.UIA.UIA_ControlTypePropertyId, UIAHandler.UIA.UIA_GroupControlTypeId
					),
	...
	def __contains__(self, obj):
		...
		if runtimeID.vt == VT_EMPTY:
			log.debugWarning("Could not get runtimeID of document. Most likely document is dead.")
			return False
		UIACondition = UIAHandler.handler.clientObject.createPropertyCondition(
			UIAHandler.UIA_RuntimeIdPropertyId, runtimeID
		)
