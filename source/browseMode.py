...
class BrowseModeTreeInterceptor(treeInterceptorHandler.TreeInterceptor):
	...
	def _quickNavScript(self, gesture, itemType, direction, errorMessage, readUnit):
		if itemType == "notLinkBlock":
			...
			def iterFactory(
				direction: str, pos: textInfos.TextInfo
			) -> Generator[TextInfoQuickNavItem, None, None]:
...
class ElementsListDialog(
	...
	def __init__(self, document):
		...
		# in the browse mode Elements List dialog.
		child = wx.RadioBox(
			self, wx.ID_ANY, label=_("Type:"), choices=tuple(et[1] for et in self.ELEMENT_TYPES)
		)
	...
	def onElementTypeChange(self, evt):
		...
		queueHandler.queueFunction(
			queueHandler.eventQueue, self.initElementType, self.ELEMENT_TYPES[elementType][0]
		)
...
class BrowseModeDocumentTreeInterceptor(
		...
		if not self.passThrough:
			if doSayAll:
				speech.speakObjectProperties(
					self.rootNVDAObject, name=True, states=True, reason=OutputReason.FOCUS
				)
		...
		if not isOverlapping:
			...
			if not self.passThrough:
				...
				if config.conf["virtualBuffers"]["autoFocusFocusableElements"]:
					...
					# changes.
					speech.speakObject(objPendingFocusBeforeActivate, OutputReason.CHANGE)
			else:
				self._replayFocusEnteredEvents()
				return nextHandler()
	...
	def _expandStyle(
		...
		compareResult = textRange.compareEndPoints(
			paragraphInfo, "endToEnd" if direction == documentBase._Movement.NEXT else "startToStart"
		)
		if compareResult != 0:
