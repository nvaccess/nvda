		...
		if isinstance(item, str):
			...
			if lastHwnd is not None and hwnd != lastHwnd:
				processFieldsAndRectsRangeReadingdirection(
					commandList, rects, windowStartIndex, windowStartOffset, index, lastEndOffset
				)
...
def processFieldsAndRectsRangeReadingdirection(
	commandList, rects, startIndex, startOffset, endIndex, endOffset
):
...
def initialize():
	...
	_getWindowTextInRect = CFUNCTYPE(
		c_long,
		c_long,
		c_long,
		c_bool,
...
class DisplayModelTextInfo(OffsetsTextInfo):
	...
	def _get__storyFieldsAndRects(
		self,
	) -> Tuple[List[textInfos.TextInfo.TextOrFieldsT], List[RectLTRB], List[int], List[int]]:
				...
				if baseline != lineBaseline:
					if lineBaseline is not None:
						processWindowChunksInLine(
							commandList, rects, lineStartIndex, lineStartOffset, index, lastEndOffset
						)
...
class EditableTextDisplayModelTextInfo(DisplayModelTextInfo):
	...
	def _findCaretOffsetFromLocation(
		self, caretRect: RectLTRB, validateBaseline: bool = True, validateDirection: bool = True
	):
	...
	def _getCaretOffset(self):
		...
		try:
			return self._findCaretOffsetFromLocation(
				caretRect, validateBaseline=True, validateDirection=False
			)
		except LookupError:
			pass
		...
		try:
			return self._findCaretOffsetFromLocation(
				caretRect, validateBaseline=False, validateDirection=False
			)
		except LookupError:
			raise RuntimeError
