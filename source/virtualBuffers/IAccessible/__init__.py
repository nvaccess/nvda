...
def update(obj):
	for w in filter(lambda x: not winUser.isWindow(x),runningTable):
		globalVars.log.debug("virtualBuffers.IAccessible.removeVirtualBuffer: removed %s at %s"%(runningTable[w],w))
		del runningTable[w]
		#globalVars.log.debug("virtualBuffers.IAccessible.update: trying to update with %s (%s)"%(hwnd,winUser.getClassName(hwnd)))
	firstObj=None
	if isinstance(obj,NVDAObjects.IAccessible.IAccessible):
		windowHandle=obj.windowHandle
		firstObj=obj
	...
	for curWindow in [windowHandle,winUser.getAncestor(windowHandle,winUser.GA_PARENT)]:
		...
		if len(possibles)>0:
			globalVars.log.debug("possibles: %s"%str(possibles))
		...
		if classString:
			...
			globalVars.log.debug("virtualBuffers.IAccessible.update: adding %s at %s (%s)"%(newClass,obj.windowHandle,className))
