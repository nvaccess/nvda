...
class virtualBuffer(textBuffer.textBufferObject):
	...
	def getIDEnterMessage(self,ID):
		...
		if (role==controlTypes.ROLE_LINK and vbc["reportLinks"]) or (role==controlTypes.ROLE_LIST and vbc["reportLists"]) or (role==controlTypes.ROLE_LISTITEM and vbc["reportListItems"]) or (role==controlTypes.ROLE_HEADING and vbc["reportHeadings"]) or (role==controlTypes.ROLE_TABLE and vbc["reportTables"]) or (role==controlTypes.ROLE_TABLEHEADER and vbc["reportTables"]) or (role==controlTypes.ROLE_TABLEFOOTER and vbc["reportTables"]) or (role==controlTypes.ROLE_TABLEROW and vbc["reportTables"]) or (role==controlTypes.ROLE_TABLECELL and vbc["reportTables"]) or (role==controlTypes.ROLE_GRAPHIC and vbc["reportGraphics"]) or (role==controlTypes.ROLE_FORM and vbc["reportForms"]) or ((role in [controlTypes.ROLE_BUTTON,controlTypes.ROLE_EDITABLETEXT,controlTypes.ROLE_EDITABLETEXT,controlTypes.ROLE_CHECKBOX,controlTypes.ROLE_RADIOBUTTON,controlTypes.ROLE_COMBOBOX]) and vbc["reportFormFields"]) or (role==controlTypes.ROLE_PARAGRAPH and vbc["reportParagraphs"]) or (role==controlTypes.ROLE_BLOCKQUOTE and vbc["reportBlockQuotes"]) or (role==controlTypes.ROLE_FRAME and vbc["reportFrames"]):
			msg=controlTypes.speechRoleLabels[info["role"]]
