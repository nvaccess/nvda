				...
				if not descNode:
					try:
						descNode = NVDAObjects.IAccessible.MSHTML.locateHTMLElementByID(
							self.obj.rootNVDAObject.HTMLNode.document, ariaDescribedById
						)
					except (COMError, NameError):
						descNode = None
				if descNode:
...
class MSHTML(VirtualBuffer):
	...
	def getNVDAObjectFromIdentifier(self, docHandle, ID):
		HTMLNode = NVDAObjects.IAccessible.MSHTML.locateHTMLElementByID(
			self.rootNVDAObject.HTMLNode.document, "ms__id%d" % ID
		)
		if not HTMLNode:
	...
	def _activateLongDesc(self, controlField):
		...
		self.rootNVDAObject.HTMLNode.document.parentWindow.open(
			longDesc, "_blank", "location=no, menubar=no, toolbar=no"
		)
