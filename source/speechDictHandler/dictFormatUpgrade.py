...
def createVoiceDictFileName(synthName, voiceName):
	...
	return fileNameFormat.format(synth=synthName, voice=api.filterFileName(voiceName))
...
def _doSynthVoiceDictBackupAndMove(synthName, oldFileNameToNewFileNameList=None):
	...
	if needsUpgrade:
		...
		# dicts diectory
		voiceDictGlob = os.path.join(WritePaths.speechDictsDir, "{synthName}*".format(synthName=synthName))
			...
			if oldFileNameToNewFileNameList:
				for oldFname, newFname in oldFileNameToNewFileNameList:
					if oldFname == actualBasename:
						log.debug(
							"renaming {} to {} and moving to {}".format(actualPath, newFname, newDictPath)
						)
						renameTo = newFname
						break
...
def _doEspeakDictUpgrade():
	...
	def getNextVoice():
		for ID, (oldName, newName) in espeakNameChanges.items():
			yield (createVoiceDictFileName(synthName, oldName), createVoiceDictFileName(synthName, newName))
