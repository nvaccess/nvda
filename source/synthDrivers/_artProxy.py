...
class ARTProxySynthDriver(SynthDriver):
	...
	def pause(self, switch: bool):
		...
		if self._connected:
			try:
				...
				log.debugWarning(f"ART synth {self.name} disconnected during pause - normal during shutdown")
				self._connected = False
			except Exception:
	...
	def _get_variant(self) -> str:
		"""Get current variant from ART."""
		return self._get_synth_property("variant", "")

	def _set_variant(self, value: str):
		"""Set variant in ART."""
		self._set_synth_property("variant", value, "setVariant")

	def _get_inflection(self) -> int:
		"""Get current inflection from ART."""
		return self._get_synth_property("inflection", 0)

	def _set_inflection(self, value: int):
		"""Set inflection in ART."""
		self._set_synth_property("inflection", value, "setInflection")

	def _get_language(self) -> Optional[str]:
		"""Get current language from ART."""
		try:
			current_voice = self.voice
			if current_voice and current_voice in self.availableVoices:
				return self.availableVoices[current_voice].language
		except Exception:
			log.exception(f"Error getting current language from {self.name}")
		return None

	def _set_language(self, language: Optional[str]):
		"""Set language in ART by finding appropriate voice."""
		# For now, this raises NotImplementedError to match core behavior
		# TODO: Implement voice switching based on language
		raise NotImplementedError

	def _get_availableLanguages(self):
		"""Get available languages from available voices."""
		try:
			return {self.availableVoices[v].language for v in self.availableVoices}
		except Exception:
			log.exception(f"Error getting available languages from {self.name}")
			return set()

	def _getAvailableVariants(self):
		"""Get available variants from ART."""
		if not self._connected:
			return OrderedDict()

		try:
			# Try to get variants from ART service
			if hasattr(self._synthService, "getAvailableVariants"):
				variantData = self._synthService.getAvailableVariants()
				if variantData:
					variants = OrderedDict()
					for vData in variantData:
						if not isinstance(vData, dict):
							continue
						variant_id = vData.get("id")
						if variant_id:
							display_name = vData.get("displayName", variant_id)
							language = vData.get("language")
							variants[variant_id] = VoiceInfo(variant_id, display_name, language)
					return variants
		except Exception:
			log.debugWarning(f"Error getting variants from ART synth {self.name}")
	...
	def _get_synth_property(self, propName: str, defaultValue: Any) -> Any:
		...
		try:
			...
			log.debugWarning(f"Communication error getting {propName}, using cached value")
			self._connected = False  # Mark as disconnected for retry
		except Exception:
			log.exception(f"Error getting {propName} from ART synth {self.name}")
	...
	def _set_synth_property(self, propName: str, value: Any, methodName: str) -> None:
		...
		try:
			...
			# Mark for retry
			self._pending_property_updates[propName] = value
		except Exception:
			log.exception(f"Error setting {propName} in ART synth {self.name}")

	def _sendPendingUpdates(self):
		"""Send any property updates that were cached while disconnected."""
		for propName, value in self._pending_property_updates.items():
			try:
				self._synthService.setProperty(propName, value)
			except Exception:
				log.exception(f"Error sending pending {propName} update")
	...
	def _serializeSpeechSequence(self, sequence: SpeechSequence) -> List[Dict[str, Any]]:
		...
		for item in sequence:
			if isinstance(item, str):
				...
				serialized.append({"type": "IndexCommand", "index": item.index})
			elif isinstance(item, CharacterModeCommand):
				serialized.append({"type": "CharacterModeCommand", "state": item.state})
			elif isinstance(item, BreakCommand):
				serialized.append({"type": "BreakCommand", "time": item.time})
			elif isinstance(item, LangChangeCommand):
				serialized.append({"type": "LangChangeCommand", "lang": item.lang})
			elif isinstance(item, PitchCommand):
				serialized.append({"type": "PitchCommand", "offset": item.offset})
			elif isinstance(item, RateCommand):
				serialized.append({"type": "RateCommand", "offset": item.offset})
			elif isinstance(item, VolumeCommand):
				serialized.append({"type": "VolumeCommand", "offset": item.offset})
			elif isinstance(item, PhonemeCommand):
				serialized.append({"type": "PhonemeCommand", "phoneme": item.phoneme})
			else:
