...
class ARTAddonProcess:
	...
	def _startProcessWithHandshake(self):
		...
		self.sp = SecurePopen(
			self._runtimeArgv, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, killOnDelete=True, hideCriticalErrorDialogs=True, createNoWindow=True,
			**(self.launchConfig_secure if isSecure else self.launchConfig_standard)
		)
