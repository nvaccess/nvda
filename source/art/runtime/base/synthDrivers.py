...
class SynthDriver(ABC):
	...
	def _registerWithCore(self):
		...
		try:
			...
			if result:
				...
				self.logger.error(
					f"Failed to register {self.name} with NVDA Core - registerSynthDriver returned False",
				)
					...
					settings_metadata.append(
						{
							...
							"displayNameWithAccelerator": getattr(
								setting,
								"displayNameWithAccelerator",
								"Unknown",
							),
	...
	def _notifySpeechDone(self):
		...
		if self._speechService:
			try:
				...
				if rpc_time > 0.1:
					self.logger.warning(
						f"SLOW: Speech done notification RPC took {rpc_time:.3f}s - possible hang!",
					)
				else:
	...
	def _sendAudioData(
		...
		start_time = time.time()
		self.logger.info(
			f"_sendAudioData called with {len(data)} bytes, {sampleRate}Hz, {channels}ch, {bitsPerSample}bit",
		)
		if self._speechService:
			try:
				...
				for i in range(0, len(data), MAX_CHUNK_SIZE):
					...
					if chunk_time > 0.5:
						self.logger.warning(
							f"SLOW: Audio chunk {chunk_num} RPC took {chunk_time:.3f}s - possible hang!",
						)
					elif chunk_time > 0.1:
				...
				if total_time > 1.0:
					self.logger.warning(
						f"SLOW: Total _sendAudioData took {total_time:.3f}s - this could cause issues!",
					)
			except Exception:
				total_time = time.time() - start_time
