...
class OffsetConverter(metaclass=ABCMeta):
	...
	def strToEncodedOffsets(
		...
		if strEnd is not None and strEnd < strStart:
			raise ValueError("strEnd=%d must be greater than or equal to strStart=%d" % (strEnd, strStart))
	...
	def encodedToStrOffsets(
		...
		if encodedEnd is not None and encodedEnd < encodedStart:
			raise ValueError(f"{encodedEnd=} must be greater than or equal to {encodedStart=}")
...
class WideStringOffsetConverter(OffsetConverter):
	...
	def encodedToStrOffsets(
		...
		decodedRange: str = self.encoded[bytesStart:correctedBytesEnd].decode(
			self._encoding, errors="surrogatepass"
		)
...
def getTextFromRawBytes(
	buf: bytes, numChars: int, encoding: Optional[str] = None, errorsFallback: str = "replace"
):
	"""
...
class UnicodeNormalizationOffsetConverter(OffsetConverter):
	...
	def encodedToStrOffsets(
		self, encodedStart: int, encodedEnd: int | None = None, raiseOnError: bool = False
	) -> int | Tuple[int]:
		super().encodedToStrOffsets(encodedStart, encodedEnd, raiseOnError)
		if encodedStart == 0:
			resultStart = 0
