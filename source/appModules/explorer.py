...
class AppModule(appModuleHandler.AppModule):
	...
	def _get_statusBar(self):
		...
		condition = clientObject.createPropertyCondition(
			UIAHandler.UIA_ControlTypePropertyId, UIAHandler.UIA_StatusBarControlTypeId
		)
		...
		for index, child in enumerate(obj.children):
			if (
				...
				if selected is not None:
					parts.append(" ".join([child.name] + ([selected.name] if selected is not None else [])))
			else:
	...
	def event_NVDAObject_init(self, obj: NVDAObject) -> None:
		...
		if (
			...
			and winUser.getClassName(winUser.getAncestor(obj.windowHandle, winUser.GA_PARENT))
			== "Address Band Root"
		):
