...
class BookPageViewTreeInterceptor(
	DocumentWithPageTurns, ReviewCursorManager, BrowseModeDocumentTreeInterceptor
):
	...
	def _iterEmbeddedObjs(self, hypertext, startIndex, direction):
		...
		for index in range(
			startIndex, hypertext.nHyperlinks if direction == "next" else -1, 1 if direction == "next" else -1
		):
			...
			for subObj in self._iterEmbeddedObjs(
				objHt, 0 if direction == "next" else objHt.nHyperlinks - 1, direction
			):
				yield subObj
