...
CLM_GETEXTRAIMAGE = (
	CLM_FIRST + 16
)  # wParam=hItem, lParam=MAKELPARAM(iColumn (0 based),0), returns iImage or 0xFF
...
CLM_GETISEARCHSTRING = (
	CLM_FIRST + 20
)  # lParam=(char*)pszStr, max 120 bytes, returns number of chars in string
MAXITEMTEXTLEN = 120
...
class AppModule(appModuleHandler.AppModule):
	...
	def chooseNVDAObjectOverlayClasses(self, obj, clsList):
		...
		if windowClass == "CListControl":
			...
			and obj.IAccessibleRole == oleacc.ROLE_SYSTEM_SCROLLBAR
			and obj.windowControlID in MESSAGEVIEWERS
		):
...
class mirandaIMContactList(IAccessible):
	def _get_name(self):
		...
		internalBuf = winKernel.virtualAllocEx(
			self.processHandle, None, MAXITEMTEXTLEN, winKernel.MEM_COMMIT, winKernel.PAGE_READWRITE
		)
		try:
		...
		if not name:
			...
			if tc and tc.role == controlTypes.Role.TABCONTROL:
				...
				for index in range(len(children)):
					if (children[index].role == controlTypes.Role.TAB) and (
						controlTypes.State.SELECTED in children[index].states
					):
