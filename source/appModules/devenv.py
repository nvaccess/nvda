	...
	def _getFormatFieldAtRange(self, textRange, formatConfig, ignoreMixedValues=False):
		...
		if lineNumberStr:
			try:
				...
				log.debugWarning(
					f"Couldn't parse {lineNumberStr} as integer to report a line number", exc_info=True
				)
		return formatField
...
class VsTextEditPane(EditableText, Window):
	...
	def _get_location(self):
		if not isinstance(self, UIA):
			return RectLTWH(self._window.Left, self._window.Top, self._window.Width, self._window.Height)
...
class ObjectsTreeItem(IAccessible):
	...
	def _get_positionInfo(self):
		return {"level": int(self.IAccessibleObject.accValue(self.IAccessibleChildID))}
