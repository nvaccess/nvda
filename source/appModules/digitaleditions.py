...
class BookContent(DocumentWithPageTurns, UIA):
	...
	def turnPage(self, previous=False):
		...
		oldPos = posSlider.UIAElement.getCurrentPropertyValueEx(
			UIAHandler.UIA_RangeValueValuePropertyId, True
		)
		KeyboardInputGesture.fromName("pageUp" if previous else "pageDown").send()
		if (
			posSlider.UIAElement.getCurrentPropertyValueEx(UIAHandler.UIA_RangeValueValuePropertyId, True)
			== oldPos
		):
			# No more pages.
			raise RuntimeError
