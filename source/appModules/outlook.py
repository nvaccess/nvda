...
def getContactString(obj):
	return ", ".join(
		[x for x in [obj.fullName, obj.companyName, obj.jobTitle, obj.email1address] if x and not x.isspace()]
	)
...
class CalendarView(IAccessible):
	...
	def _generateTimeRangeText(self, startTime, endTime):
		startText = winKernel.GetTimeFormatEx(
			winKernel.LOCALE_NAME_USER_DEFAULT, winKernel.TIME_NOSECONDS, startTime, None
		)
		endText = winKernel.GetTimeFormatEx(
			winKernel.LOCALE_NAME_USER_DEFAULT, winKernel.TIME_NOSECONDS, endTime, None
		)
		...
		if (
			...
			startDateText = winKernel.GetDateFormatEx(
				winKernel.LOCALE_NAME_USER_DEFAULT, winKernel.DATE_LONGDATE, startTime, None
			)
		...
		if endDate != startDate:
			...
			endText = "%s %s" % (
				winKernel.GetDateFormatEx(
					winKernel.LOCALE_NAME_USER_DEFAULT, winKernel.DATE_LONGDATE, endTime, None
				),
				endText,
			)
	...
	def _generateCategoriesText(appointment):
		...
		if (
			ctypes.windll.kernel32.GetLocaleInfoW(
				languageHandler.LOCALE_USER_DEFAULT, languageHandler.LOCALE.SLIST, separatorBuf, bufLength
			)
			== 0
		):
		...
		if self.appModule.outlookVersion >= 13 and self.appModule.nativeOm:
			...
			if s.count > 0:
				...
				startDate = winKernel.GetDateFormatEx(
					winKernel.LOCALE_NAME_USER_DEFAULT, winKernel.DATE_LONGDATE, selectedStartTime, None
				)
				startTime = winKernel.GetTimeFormatEx(
					winKernel.LOCALE_NAME_USER_DEFAULT, winKernel.TIME_NOSECONDS, selectedStartTime, None
				)
				endDate = winKernel.GetDateFormatEx(
					winKernel.LOCALE_NAME_USER_DEFAULT, winKernel.DATE_LONGDATE, selectedEndTime, None
				)
				endTime = winKernel.GetTimeFormatEx(
					winKernel.LOCALE_NAME_USER_DEFAULT, winKernel.TIME_NOSECONDS, selectedEndTime, None
				)
		...
		for index in range(cachedChildren.length):
			...
			if name and config.conf["documentFormatting"]["reportTableHeaders"] in (
				...
				columnHeaderItems = e.getCachedPropertyValueEx(
					UIAHandler.UIA_TableItemColumnHeaderItemsPropertyId, True
				)
			else:
