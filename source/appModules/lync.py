...
class NetUIRicherLabel(UIA):
	...
	def event_liveRegionChange(self):
		...
		if self.value is not None:
			...
			pattern = (
				r"^(?P<name>.+?): (?P<priority>.*?), , (?P<content>.+), (?P<status>.*?), (?P<timestamp>.+?)$"
			)
		...
		if (
			...
			and pretext == lastPretext
			and contentLinesLen > lastContentLinesLen
			and contentLines[:lastContentLinesLen] == lastContentLines
		):
