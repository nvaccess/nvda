...
class NetUIRicherLabel(UIA):
	...
	def event_liveRegionChange(self):
		...
		if self.value is None:
			# For some versions of Skype for Business, self.value is just None.
			# So we just look at self.name formatting to split content from person and timestamp.
			contentStartIndex = self.name.find(', , ') + 4
			contentEndIndex = self.name.rfind(', , ')
			content = self.name[contentStartIndex:contentEndIndex].replace('\r\n\r\n', '\n').strip()
		else:
			# For older versions of Skype for Business, the object's value just contains the content.
			...
			# Normalize the usage of end-of-line characters (name and value seem to expose them differently,
			# which would break comparison)
