...
class SymphonyTextInfo(IA2TextTextInfo):
	...
	def _getFormatFieldFromLegacyAttributesString(  # noqa: C901
		self, attribsString: str, offset: int
	...
	def _getFormatFieldAndOffsetsFromAttributes(
		self, offset: int, formatConfig: Optional[dict], calculateOffsets: bool
		...
		if attribsString and attribsString.startswith("Version:1;"):
			formatField = self._getFormatFieldFromLegacyAttributesString(attribsString, offset)
		else:
			formatField, (startOffset, endOffset) = super()._getFormatFieldAndOffsets(
				offset, formatConfig, calculateOffsets
			)

		return formatField, (startOffset, endOffset)

	def _getFormatFieldAndOffsets(
		self, offset: int, formatConfig: Optional[dict], calculateOffsets: bool = True
	) -> tuple[textInfos.FormatField, tuple[int, int]]:
		...
		try:
			if (
				obj.IAccessibleTextObject.QueryInterface(IA2.IAccessibleHypertext).hyperlinkIndex(offset)
				!= -1
			):
	...
	def announceSelectionChange(self):
		if self is api.getFocusObject():
			speech.speakObjectProperties(
				self, states=True, cellCoordsText=True, reason=controlTypes.OutputReason.CHANGE
			)
	...
	def _get_cellCoordsText(self):
		if self.hasSelection and controlTypes.State.FOCUSED in self.states:
			...
			return _("{firstAddress} through {lastAddress}").format(
				firstAddress=self._get_name(), lastAddress=lastAddress
			)
