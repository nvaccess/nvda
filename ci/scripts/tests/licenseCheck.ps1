...
if ($LastExitCode -ne 0) {
	Write-Output "FAIL: License check." >> $env:GITHUB_STEP_SUMMARY
	Write-Output "testFailExitCode=$LastExitCode" | Out-File -FilePath $Env:GITHUB_ENV -Encoding utf8 -Append
} else {
	Write-Output "PASS: License check." >> $env:GITHUB_STEP_SUMMARY
}
Write-Output "<details><summary>License check summary</summary>" >> $env:GITHUB_STEP_SUMMARY
