...
class NVDASpyLib:
	...
	def assignGesture(
		self, gesture: str, module: str, className: str, script: Optional[str], replace: bool = False
	):
		import inputCore
	...
	def override_translationString(self, invariantString: str, replacementString: str):
		...
		if not self.fakeTranslations:
			...
			class Translation_Fake(gettext.NullTranslations):
				...
				def __init__(self, originalTranslationFunction: Optional):
	...
	def wait_for_specific_speech_no_raise(
		...
		success, speechIndex = self._has_speech_occurred_before_timeout(
			speech, afterIndex, maxWaitSeconds, intervalBetweenSeconds
		)
		if not success:
			return None
		return speechIndex
