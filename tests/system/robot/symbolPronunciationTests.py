...
def _testDelayedDescription(expectDescription: bool = True) -> None:
	...
	if spoken[0] not in _CHARACTER_DESCRIPTIONS:
		raise AssertionError(f"First piece of speech not an expected character; got: '{spoken[0]}'")
	if expectDescription:
		if len(spoken) != 2:
			raise AssertionError(f"Expected character with description; got: '{spoken}'")
		_asserts.strings_match(spoken[1], _CHARACTER_DESCRIPTIONS[spoken[0]])
	else:
		if len(spoken) != 1:
			raise AssertionError(f"Expected single character; got: '{spoken}'")
...
def _doTest(
	...
	_builtIn.should_be_equal(
		actual, expectedSpeech, msg=f"actual vs expected. With symbolLevel {symbolLevel}"
	)
...
def test_ignoreBlankLinesForReportLineIndentation():
	...
	_doTestIgnoreBlankLines(
		...
		expectedSpeech=["def foo", "tab  hello", "blank", "world", "blank", "no indent  def bar", "blank"],
	)
