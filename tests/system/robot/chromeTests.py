...
def test_focus_mode_on_focusable_read_only_lists():
	...
	# Tab into the document, but browse mode won't yet be invoked
	actualSpeech = _chrome.getSpeechAfterKey("tab")
	_asserts.strings_match(
		actualSpeech,
		SPEECH_SEP.join([
		...
		message="focus mode - focus link in document but focus mode remains on"
	)

	# Tab to the time stamp link, which will finally turn off focus mode
	actualSpeech = _chrome.getSpeechAfterKey("tab")
	_asserts.strings_match(
		actualSpeech,
		SPEECH_CALL_SEP.join([
			SPEECH_SEP.join([
				"1:30 PM",  # name for link
				"link",  # role for link
			]),
			"Browse mode",  # Focus mode should be disabled automatically and be indicated
		]),
		message="focus mode - focus second link in document and turn off focus mode"
	)
...
# shift+Tab back to first link
	...
	_asserts.strings_match(
		...
		message="focus mode - focus first link in document with browse mode still on"
	)
