...
class TestSynthDriver_Logic(unittest.TestCase):
	...
	def test_normalizeLangCommand(self):
		...
		self.assertIn("Unable to find an eSpeak language for 'fake'", logContext.output[0])
...
class TestSynthDriver_Integration(unittest.TestCase):
	...
	def test_fallbackToBritishEnglish(self):
		...
		self.assertEqual("gmw\\en", self._driver.voice, msg="Language without locale not supported by eSpeak")
