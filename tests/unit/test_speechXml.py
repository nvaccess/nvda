...
class TestXmlBalancer(unittest.TestCase):
	...
	def test_standAloneTag(self):
		xml = self.balancer.generateXml([speechXml.StandAloneTagCommand("tag", {"attr": "val"}, "content")])
		self.assertEqual(xml, '<tag attr="val">content</tag>')

	def test_standAloneTagNoContent(self):
		xml = self.balancer.generateXml([speechXml.StandAloneTagCommand("tag", {"attr": "val"}, None)])
	...
	def test_attrEscaping(self):
		...
		"""
		xml = self.balancer.generateXml([speechXml.StandAloneTagCommand("tag", {"attr": '"v1"&"v2"'}, None)])
	...
	def test_setAttr(self):
		xml = self.balancer.generateXml([speechXml.SetAttrCommand("pitch", "val", 50), "text"])
		self.assertEqual(xml, '<pitch val="50">text</pitch>')

	def test_delAttrNoSetAttr(self):
		xml = self.balancer.generateXml([speechXml.DelAttrCommand("pitch", "val"), "text"])
		self.assertEqual(xml, "text")
	...
	def test_setAttrInterspersedText(self):
		...
		self.assertEqual(
			xml, '<pitch val="50">t1</pitch><pitch val="50"><volume val="60">t2</volume></pitch>'
		)
	...
	def test_delAttrUnbalanced(self):
		...
		self.assertEqual(
			xml, '<pitch val="50"><volume val="60">t1</volume></pitch><volume val="60">t2</volume>'
		)
	...
	def test_stopEnclosingText(self):
		...
		xml = self.balancer.generateXml(
			[speechXml.EncloseTextCommand("say-as", {}), "c", speechXml.StopEnclosingTextCommand(), "t"]
		)
