...
import unittest
import winUser
from orderedWinEventLimiter import OrderedWinEventLimiter
...
class TestOrderedWinEventLimiter(unittest.TestCase):
	...
	def test_limitEventsPerThread(self):
		...
		# TODO: repeated Id's (0 and 8) are EVENT_SYSTEM_FOREGROUND see test_maxFocusEvents
		expectedIds = [26, 24, 19, 18, 16, 11, 10, 9, 8, 8, 4, 3, 2, 1, 0, 0]
		self.assertEqual(expectedIds, windowIds)
		# TODO: BUG? Why isn't this MAX_WINEVENTS_PER_THREAD=10
		# perhaps because there are also 4 focus events... except then it should be 14?
