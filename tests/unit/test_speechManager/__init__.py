...
class TestSpeechIndexComparison(unittest.TestCase):
	...
	def testAllValuesHaveSymmetry(self):
		...
		self.assertEqual(
			len(indexesWithEquivalence), 1, msg=f"Indexes with neither true: {indexesWithEquivalence!r}"
		)
		...
		self.assertEqual(len(bothBefore), 0, msg=f"Indexes with both true: {bothBefore!r}")
...
class SayAllEmulatedTests(unittest.TestCase):
	...
	def test_standardSayAll(self):
		...
		with smi.expectation():
			seqNum = smi.speak(
				[callBack(expectedToBecomeIndex=1), "sequence 0  ", callBack(expectedToBecomeIndex=2)]
			)
		...
		seqNum = smi.speak(["sequence 2  ", callBack(expectedToBecomeIndex=5)])
		...
		with smi.expectation():
			...
			smi.pumpAllAndSendSpeechOnCallback(
				...
				seq=[callBack(expectedToBecomeIndex=7), "sequence 4  ", expectIndex(expectedToBecomeIndex=8)],
			)
	...
	def test_speechNotRepeated(self):
		...
		with smi.expectation():
			...
			smi.pumpAllAndSendSpeechOnCallback(
				...
				seq=[callBack(expectedToBecomeIndex=3), "sequence 2  ", expectIndex(expectedToBecomeIndex=4)],
			)
			# Now ensure there is no double speaking!
...
class InitialDevelopmentTests(unittest.TestCase):
	...
	def test3(self):
		...
		sequence = [
			PitchCommand(offset=50),
			"This is the first utterance in a higher pitch",
	...
	def test_6_SPRI(self):
		...
		interrupt1 = smi.speak(
			priority=speech.Spri.NEXT, seq=["6 7 8 9 10", smi.create_ExpectedIndex(expectedToBecomeIndex=3)]
		)
		interrupt2 = smi.speak(
			priority=speech.Spri.NEXT,
			seq=["11 12 13 14 15", smi.create_ExpectedIndex(expectedToBecomeIndex=4)],
		)
	...
	def test_7_SPRI(self, mock_BeepCommand_run):
		...
		with smi.expectation():
			smi.indexReached(3)
			smi.pumpAll()
			smi.expect_synthSpeak(sequence=postInterruption)
	...
	def test_8_SPRI(self):
		...
		with smi.expectation():
			first = smi.speak(
				priority=speech.Spri.NOW,
				seq=["First ", "utterance", smi.create_ExpectedIndex(expectedToBecomeIndex=1)],
			)
			smi.expect_synthSpeak(first)
			smi.expect_synthCancel()
		...
		with smi.expectation():
			second = smi.speak(
				priority=speech.Spri.NOW,
				seq=["Second ", "utterance", smi.create_ExpectedIndex(expectedToBecomeIndex=2)],
			)
	...
	def test_13_SPRI_interruptAfterIndexReached(self, mock_BeepCommand_run):
		...
		with smi.expectation():
			smi.indexReached(3)
			smi.pumpAll()
			resume = [smi.create_ExpectedProsodyCommand(firstSeq[0]), *firstSeq[3:]]
	...
	def test_10_SPRI_profiles(self):
		...
		with smi.expectation():
			first = smi.speak(
				["This is a normal utterance, text, text,", smi.create_ExpectedIndex(expectedToBecomeIndex=1)]
			)
			smi.expect_synthSpeak(first)
	...
	def test_11_SPRI_Profile(self):
		...
		with smi.expectation():
			interrupt = ["This is an interruption", smi.create_ExpectedIndex(expectedToBecomeIndex=2)]
