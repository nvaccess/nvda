...
class TestCalculate(unittest.TestCase):
	...
	def tearDown(self):
		...
		for attr in ("rawToBraillePos", "brailleToRawPos"):
			braille.getHandler().buffer.__dict__.pop(attr, None)

	def test_noCells(self):
		"""Check that, if list of braille cells is empty, offsets will be (0, 0, False)."""
		braille.getHandler().buffer.brailleCells = []
		braille.getHandler().buffer._calculateWindowRowBufferOffsets(0)
		expectedOffsets = [braille.buffers._WindowRowPositions(0, 0)]
		self.assertEqual(braille.getHandler().buffer._windowRowBufferOffsets, expectedOffsets)

	def test_firstPosition(self):
		"""Checks that first offset is equal to start parameter."""
		braille.getHandler().buffer.brailleCells = [1] * braille.getHandler().displaySize
		braille.getHandler().buffer._calculateWindowRowBufferOffsets(0)
		expectedOffsets = [
		...
		self.assertEqual(braille.getHandler().buffer._windowRowBufferOffsets, expectedOffsets)
		braille.getHandler().buffer._calculateWindowRowBufferOffsets(1)
		expectedOffsets = [
	...
	def test_end(self):
		...
		braille.getHandler().buffer.brailleCells = [1] * (braille.getHandler().displaySize - 10)
		braille.getHandler().buffer._calculateWindowRowBufferOffsets(0)
		expectedOffsets = [
			braille.buffers._WindowRowPositions(0, 20),
	...
	def test_textWrapFirstRowWithSpace(self):
		...
		cells = [1] * (braille.getHandler().displayDimensions.numCols - 5)
		cells.append(0)
		cells.extend([1] * (braille.getHandler().displayDimensions.numCols + 4))
		braille.getHandler().buffer.brailleCells = cells
		braille.getHandler().buffer._calculateWindowRowBufferOffsets(0)
		expectedOffsets = [
	...
	def test_textWrapSecondRowStartsWithSpace(self):
		...
		_setTextWrap(BrailleTextWrapFlag.AT_WORD_BOUNDARIES)
		cells = [1] * braille.getHandler().displayDimensions.numCols
		cells.append(0)
		cells.extend([1] * (braille.getHandler().displayDimensions.numCols - 1))
		braille.getHandler().buffer.brailleCells = cells
		braille.getHandler().buffer._calculateWindowRowBufferOffsets(0)
		expectedOffsets = [
			braille.buffers._WindowRowPositions(0, 20),
	...
	def test_none_hardCutsAtDisplayEdge(self):
		...
		braille.getHandler().buffer.brailleCells = [1] * 25
		braille.getHandler().buffer._calculateWindowRowBufferOffsets(0)
		self.assertEqual(
			braille.getHandler().buffer._windowRowBufferOffsets,
			[braille.buffers._WindowRowPositions(0, 20), braille.buffers._WindowRowPositions(20, 25)],
		)
		self.assertFalse(
			any(r.showContinuationMark for r in braille.getHandler().buffer._windowRowBufferOffsets),
		)
	...
	def test_markWordCuts_oneCellEarlierAndMarksRow(self):
		...
		self.assertEqual(
			braille.getHandler().buffer._windowRowBufferOffsets[0],
			braille.buffers._WindowRowPositions(0, 19, True),
		)
		self.assertTrue(braille.getHandler().buffer._windowRowBufferOffsets[0].showContinuationMark)
	...
	def test_markWordCuts_cleanRowHasNoMarker(self):
		...
		braille.getHandler().buffer.brailleCells = cells
		braille.getHandler().buffer._calculateWindowRowBufferOffsets(0)
		self.assertFalse(braille.getHandler().buffer._windowRowBufferOffsets[0].showContinuationMark)
	...
	def test_atWordOrSyllableBoundaries_success(self):
		...
		braille.getHandler().buffer.rawToBraillePos = list(range(len(cells)))
		braille.getHandler().buffer.brailleToRawPos = list(range(len(cells)))
		with (
			patch.object(
				braille.getHandler().buffer,
				"bufferPositionsToRawText",
				return_value="abcdefghijklmnopqrs",
			),
			patch.object(braille.getHandler().buffer, "_getLanguageAtBufferPos", return_value="en_US"),
			patch(
	...
	def test_atWordOrSyllableBoundaries_emptyPositions(self):
		...
		braille.getHandler().buffer.brailleToRawPos = list(range(len(cells)))
		with (
			patch.object(braille.getHandler().buffer, "bufferPositionsToRawText", return_value="word"),
			patch.object(braille.getHandler().buffer, "_getLanguageAtBufferPos", return_value="en_US"),
			patch(
				"braille.buffers.textUtils.hyphenation.getHyphenPositions",
				return_value=(),
	...
	def test_atWordOrSyllableBoundaries_unknownLanguage(self):
		...
		with (
			patch.object(braille.getHandler().buffer, "bufferPositionsToRawText", return_value="word"),
			patch.object(braille.getHandler().buffer, "_getLanguageAtBufferPos", return_value="zz_ZZ"),
			patch(
				"braille.buffers.textUtils.hyphenation.getHyphenPositions",
				return_value=(),
			),
