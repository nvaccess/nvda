...
class TestComplex(unittest.TestCase):
	...
	def test_group_replacement(self):
		...
		replaced = self._replace(string="1", pattern=r"(\d)", replacement="a")
		self.assertEqual(replaced, "a")

	def test_backslash_replacement(self):
		"""Test that backslashes get properly replaced"""
		replaced = self._replace(string="1", pattern=r"(\d)", replacement=r"\\")
	...
	def test_unknown_escape(self):
		...
		with self.assertRaises(LookupError):
			self._replace(string="1", pattern=r"(\d)", replacement=r"\a")
	...
	def test_missing_group(self):
		...
		with self.assertRaises(IndexError):
			self._replace(string="1", pattern=r"(\d)", replacement=r"\2")
	...
	def test_unterminated_escape(self):
		...
		"""
		with self.assertRaises(LookupError):
			self._replace(string="1", pattern=r"(\d)", replacement="\\")

	def test_group_replacements(self):
		"""Test that group references get properly replaced"""
		replaced = self._replace(string="bar.BAT", pattern=r"(([a-z]*)\.([A-Z]*))", replacement=r"\2>\1")
	...
	def test_multiple_group_replacement(self):
		...
		replaced = self._replace(
			string="bar.BAT", pattern=r"(baz)|(?P<foo>([a-z]*)\.([A-Z]*))", replacement=r"\2>\1", name="foo"
		)
		self.assertEqual(replaced, "BAT>bar")
