	...
	try:
		with winreg.OpenKey(
			...
			doxygenPath = '"%s"' % os.path.join(
				winreg.QueryValueEx(doxygenKey, "InstallLocation")[0], "Bin", "doxygen.exe"
			)
	except WindowsError:
		return "doxygen"
	return doxygenPath
				...
				for root, dirs, files in os.walk(node):
					for f in files:
						...
						pattern_check = reduce(
							lambda x, y: x or bool(fnmatch(filename, y)), file_patterns, False
						)
...
def generate(env):
	...
	env.AppendUnique(DOXYGEN=fetchDoxygenPath())
