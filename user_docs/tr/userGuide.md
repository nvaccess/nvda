...
* Uzaktan yardım veya iş birliği için [başka bir bilgisayara bağlanma ve onu NVDA ile kontrol etme](#RemoteAccess)
...
* Windows oturum açma ekranında ve diğer [güvenli ekranlarda](#SecureScreens) çalışabilme yeteneği
...
İngilizce harici, NVDA şu 55 dile çevrilmiştir: Afrikaanca, Arnavutça, Amharca, Arapça, Aragonca, Boşnakça, Bulgarca, Birmanca, Katalanca, Çince (basitleştirilmiş ve geleneksel), Hırvatça, Çekçe, Danca, Felemenkçe, Farsça, Fince, Fransızca, Galiçyaca, Gürcüce, Almanca (Almanya ve İsviçre), Yunanca, İbranice, Hintçe, Macarca, İzlandaca, İrlandaca, İtalyanca, Japonca, Kannada, Korece, Kırgızca, Litvanca, Makedonca, Moğolca, Nepalce, Norveççe, Lehçe, Portekizce (Brezilya ve Portekiz), Pencapça, Rumence, Rusça, Sırpça, Slovakça, Slovence, İspanyolca (Kolombiya ve İspanya), İsveççe, Tamilce, Tayca, Türkçe, Ukraynaca ve Vietnamca.
...
Nvda'nın geçici bir kopyası yüklenirken birkaç saniye beklemeniz gerekebilir.
Yüklendikten sonra, NVDA kurulum süreci boyunca konuşacaktır.
1. NVDA Kurulum penceresi lisans anlaşmasını görüntüleyecek bir düğmeyle açılır.
Lisans anlaşmasını okumak için `aşağıOk` tuşuna basın
...
1. NVDA'nın başarılı bir şekilde kurulduğunu gösteren bir iletişim kutusu açılır.
...
| işlev |Masa üstü kısayolu |Dizüstü Kısayolu |Açıklama|
|---|---|---|---|
|Yazılan karakterleri seslendir |`NVDA+2` |`NVDA+2` |NVDA'nın yazılan karakterlerin bildirip bildirmeyeceğini belirler |
|Yazılan sözcükleri seslendir |`NVDA+3` |`NVDA+3` | NVDA'nın yazılan sözcüklerin bildirip bildirmeyeceğini belirler |
...
"NVDA için Temel Eğitim" modülü ile başlamanızı öneririz.
...
Bu seçenek, çalışan kopyadaki kullanıcı konfigürasyonunun  mevcut oturum açmış kullanıcının NVDA konfigürasyonuna kopyalanıp kopyalanmayacağını belirler.
...
Varsayılan olarak, NVDA Masaüstü klavye düzenini kullanır; ancak, NVDA menüsünde Tercihler altında bulunan [NVDA Ayarları](#NVDASettings) iletişim kutusundaki Klavye kategorisinden Dizüstü düzenine geçebilirsiniz.
...
Bir parmakla bir kez dokunmak dokunuş olarak adlandırılır.
...
Komutların işlevleri girdi hareketleri açıkken çalışmaz.
...
Nesne dolaşım özelliği, ekrandaki tüm nesneler arasında tek tek dolaşmak yerine, hiyerarşik olarak gruplanmış nesneler arasında daha verimli bir şekilde dolaşma olanağı sunmaktadır.
Yani bir nesnenin içinde bulunan diğer nesnelere ulaşmak için o nesne içine girip dolaşmanız gerekmektedir.
Örneğin, not defteri uygulamasında menü çubuğu nesnesinin üzerine geldikten sonra nesnenin içine girip, dosya, düzen, görünüm, yardım gibi alt nesneler arasında dolaşabilirsiniz. Bir listeyle karşılaşıldığında, tüm liste ögeleri arasında dolaşmak yerine, bir liste ögesinin altındaki diğer ögeleri görmek için bu liste ögesinin alt nesnesi ya da nesnelerine giderek liste ögesi içinde dolaşmanız gerekecektir.
...
|İncelemede seçimin başına git |NVDA+alt+baş |NVDA+alt+baş |Yok |İnceleme imlecini seçili metnin ilk karakterine götürür|
|İncelemede seçimin sonuna git |NVDA+alt+son |NVDA+alt+son |Yok |İnceleme imlecini seçili metnin son karakterine götürür|
...
Masaüstü düzenini kullanırken temel metin inceleme komutlarını hatırlamanın en kolay yolu, bunları üç satır ve üç sütundan oluşan bir tablo gibi düşünmektir. Tabloda yukarıdan aşağıya satır, kelime ve karakter; soldan sağa ise önceki, mevcut ve sonraki şeklinde sıralanır.
...
| Ad |Kısayol |Açıklama|
|---|---|---|
|Metin ara |NVDA+control+f |Ara iletişim kutusunu açar|
|Sonrakini ara |NVDA+ff3 |Mevcut arama teriminin bir sonraki tekrarını arar|
|Öncekini ara |NVDA+shift+f3 |Geçerli arama teriminin önceki eşleşmesini arar|
...
Ancak NVDA’nın, belirli Gözatma Modu belgelerinde etkinleştirilebilen yerel girdi Modu özelliği vardır. Bu mod, Mozilla Firefox ve Chromium 134 veya daha yeni sürümlerine dayalı tarayıcılarda çalışır. Etkinleştirildiğinde, belgenin yerel seçimi NVDA’nın tarama kipi seçimini takip eder.
...
`NVDA+F5` tuşuna basmak tanıma sonucunu yeniler.
...
NVDA, bazı uygulamalarda belirli görevleri kolaylaştırmak veya ekran okuyucu kullanıcılarının erişemeyeceği işlevlere erişim sağlamak için kendi ek özelliklerini sunar.
...
| Ad |Kısayol |Açıklama|
|---|---|---|
|Sütun başlıklarını ayarla |`NVDA+shift+c` |Bir kez basıldığında, NVDA’ya bu hücrenin sütun başlıklarını içeren satırdaki ilk başlık hücresi olduğunu belirtir; böylece bu satırın altındaki sütunlar arasında gezinirken başlıklar otomatik olarak seslendirilir. İki kez basıldığında ayar sıfırlanır.|
|Satır başlıklarını ayarla |`NVDA+shift+r` |Bir kez basıldığında, NVDA’ya bu hücrenin satır başlıklarını içeren sütundaki ilk başlık hücresi olduğunu belirtir; böylece bu sutünun altındaki sataırlar arasında gezinirken başlıklar otomatik olarak seslendirilir. İki kez basıldığında ayar sıfırlanır|

<!-- KC:endInclude -->
Bu ayarlar, belge içinde yer imi olarak kaydedilir ve JAWS gibi başka ekran okuyucu yazılımlarla uyumludur.
Bu sayede, belgeyi daha sonra açan diğer ekran okuyucu kullanıcıları için satır ve sütun başlıkları otomatik olarak ayarlanmış olur.
...
<!-- KC:endInclude -->
Belgedeki tüm yorumlar ve diğer izlenen değişiklikler, Tür olarak Açıklamalar seçildiğinde NVDA Öğeler Listesinde de listelenebilir.
...
|---|---|---|
|Sütun başlıklarını ayarla |`NVDA+shift+c` |Bir kez basıldığında, NVDA’ya bu hücrenin sütun başlıklarını içeren satırdaki ilk başlık hücresi olduğunu belirtir; böylece bu satırın altındaki sütunlar arasında gezinirken başlıklar otomatik olarak seslendirilir. İki kez basıldığında ayar sıfırlanır.|
|Satır başlıklarını ayarla |`NVDA+shift+r` |Bir kez basıldığında, NVDA’ya bu hücrenin satır başlıklarını içeren sütundaki ilk başlık hücresi olduğunu belirtir; böylece bu sütunun altındaki satırlar arasında gezinirken başlıklar otomatik olarak seslendirilir. İki kez basıldığında ayar sıfırlanır.|
...
Bağlantılar ve görseller için tek tuşla gezinme desteklenir ancak bu özellik yalnızca mevcut sayfada kullanılabilir.
Bağlantıya göre gezinme aynı zamanda dipnotları da içerir.

NVDA, erişilebilir matematik içeren kitapların matematiksel içeriğinin okunması ve etkileşimli olarak gezinmesi için erken destek sağlar.
Daha fazla bilgi için lütfen [Matematiksel İçerik Okuma](#ReadingMath) bölümüne bakın.

#### Metin Seçimi {#KindleTextSelection}

Kindle, seçilen metin üzerinde sözlük tanımı elde etmek, notlar ve vurgular eklemek, metni panoya kopyalamak ve web'de arama yapmak dahil olmak üzere çeşitli işlevleri gerçekleştirmenize yardımcı olabilir.
...
<!-- KC:endInclude -->
Bunu hiçbir metin seçilmeden yaparsanız, imlecin üzerinde olduğu kelime için seçenekler gösterilecektir.

#### Kullanıcı Notları {#KindleUserNotes}

Bir kelime veya metin parçasıyla ilgili not ekleyebilirsiniz.
Bunun için öncelikle ilgili metni seçin ve yukarıda anlatıldığı gibi seçme seçeneklerine erişin.
Ardından Not Ekle'yi seçin.

Tarama kipinde okurken, NVDA bu notları yorum olarak bildirir.

Bir notu görüntülemek, düzenlemek veya silmek için:

1. İmleci notu içeren metne götürün.
1. Yukarıda açıklandığı şekilde seçme seçeneklerine erişin.
1. Notu Düzenle'yi seçin.

### Azardi {#Azardi}

<!-- KC:beginInclude -->
Kitap tablo görünümündeyken:

| Ad|Kısayol tuşu |Açıklama|
|---|---|---|
|Enter |enter |Seçili kitabı açar.|
|Uygulama tuşu |uygulama tuşu |Seçilen kitabın içerik menüsünü açar.|
...
İşaretlendiğinde, NVDA’dan çıkmaya çalıştığınızda aşağıdaki seçenekleri sunan bir iletişim kutusu görünür:

* Kapat
* Yeniden başlat
* Eklentileri devre dışı bırakarak ve günlük tutma seviyesini tamir olarak ayarlayarak yeniden başlat
* Hata ayıklamayı etkinleştirerek yeniden başlat
* bekleyen güncellemeleri yükle (varsa).
...
Eğer bekleyen bir güncelleme varsa, NVDA'dan çık iletişim kutusundan, NVDA menüsünden ya da Yardım menüsünde güncellemeleri kontrol et diyerek elle de kurabilirsiniz.

##### Güncelleme kopyası {#UpdateMirror}

Bu denetimler, NVDA güncellemelerini kontrol etmek için alternatif bir bağlantı belirlemeye yarar.
Bu seçenek NV Access NVDA güncelleme sunucusuna erişimin yavaş veya erişilemez olduğu yerlerde faydalı olabilir.

Salt okunur metin kutusu mevcut kopya adresini gösterir.
Eğer güncelleme kopyası yerine NV Access güncelleme sunucusu kullanılıyorsa, "Kopya yok" ifadesi görüntülenir.

Güncelleme kopyasını değiştirmek isterseniz, [Güncelleme Kopyası Ayarları iletişim kutusunu](#SetURLDialog) açmak için "Değiştir..." düğmesine basın.

Güncelleme kopyası kullanırken, kopya sahibinin tüm [güncelleme kontrolleriyle gönderilen bilgilere](#GeneralSettingsCheckForUpdates) erişimi olduğunu unutmayın.
Bir güncelleme kopyası ayarlamadan önce, bilgilerinizi nasıl işleyecekleri konusunda bilgi almak için kopya sahibiyle veri işleme politikaları hakkında iletişime geçebilirsiniz.

##### Ekranın Tümünü Oku veya Braille ile Okuma sırasında kapanmasını önle {#PreventDisplayTurningOff}

Bu onay kutusu etkinleştirildiğinde, tümünü oku veya braille okuma sırasında kaydırma düğmelerine basıldığında ekranın açık kalmasını sağlar.
Böylece  “tümünü oku” esnasında ekranın beklenmedik şekilde kilitlenmesi önlenmiş olur.
Bu seçenek varsayılan olarak etkindir.
Pil ömrünüzün kısaldığını fark ediyorsanız, bu seçeneği devre dışı bırakmanız önerilir.
...
Konuşma Ayarları kategorisindeki ilk seçenek Değiştir... düğmesidir.
Bu düğmeye bastığınızda, etkin ses sentezleyicisini seçmenizi sağlayan [Sentezleyici Seç](#SelectSynthesizer) iletişim penceresi açılır.
...
Eğer Otomatik seçilirse, tablo NVDA’nın güncel diline göre otomatik olarak seçilir.
...
Bu durumda, braille sistemi odak ve imleci izlemez.
...
| . {.hideHeaderRow} |.|
|---|---|
|Seçenekler |Varsayılan (Hiçbir zaman), Hiçbir zaman, Yalnızca otomatik olarak bağlandığında, Her zaman|
...
##### Paragraf başlangıç işareti {#BrailleParagraphStartMarkers}

"paragraf bazında oku" işaretliyse, seçilen başlangıç işareti paragrafın başlangıcını belirtmek için gösterilir.
Bu seçenek özellikle yapılandırılmış belgeler veya kitaplar gibi büyük metin parçalarını okumak için kullanılan uygulamalarda yararlı olabilir.
Bu tür belgelerde paragrafların nerede başladığını bilmek, içeriğin yapısını anlamak veya paragraf konumuna göre yer imleri veya ek açıklamalar yazmak için faydalı olabilir.

Seçenekler arasında, ince bir paragraf sonu olarak iki boşluk kullanılması ve daha belirgin bir paragraf simgesi olan pilcrow (¶) kullanılması yer alır.

| . {.hideHeaderRow} |.|
|---|---|
|Seçenekler |Paragraf başlangıç ​​işareti yok, Çift boşluk ( ), Pilcrow (¶)|
|Varsayılan |Paragraf başlangıç işareti yok|
...
##### Biçimlendirme Görünümü {#BrailleFormattingDisplay}

Bu ayar, NVDA'nın braillede metin formatını nasıl görüntüleyeceğini belirler.
Bu seçenek yalnızca NVDA'nın [braille yazı tipi niteliklerini göster](#DocumentFormattingFontAttributes) ayarlandığında etkili olur.

| . {.hideHeaderRow} |.|
|---|---|
| Seçenekler | Varsayılan (Liblouis), Liblouis, Etiketler |
| Varsayılan| Liblouis |

| Seçenek | Davranış |
|---|---|
| Liblouis | Yerel Braille biçimlendirmesini kullanın. Bu seçeneğin yalnızca kalın, italik ve altı çizili metni göstereceğini ve yalnızca seçilen braille tablosunun bu nitelikleri göstermeyi desteklemesi durumunda gösterileceğini unutmayın. |
| [Etiketler](#BrailleFormattingDisplayTags) | Metin biçimlendirmesinin nasıl ve nerede değiştiğini açıklayan etiketleri kullanır. |

###### Etiketler {#BrailleFormattingDisplayTags}

"Biçimlendirme görünümü" "Etiketler" olarak ayarlandığında, biçimlendirmede bir değişiklik algılandığında braille alfabesinde bir biçimlendirme etiketi görüntülenir.
Bu etiketler ⣋ ile başlar ve ⣙ ile biter.
Biçimlendirme etiketi, metin biçimlendirmesini tanımlayan bir veya daha fazla sembol içerecektir.
Aşağıdaki semboller tanımlanmıştır:

| Sembol | Anlam |
|---|---|
| ⠃ ("b") | Kalın metin başlangıcı |
| ⡃ ("b" with dot 7) | kalın metin sonu |
| ⠊ ("i") | italik başlangıç |
| ⡊ ("i" with dot 7) | italik sonu |
| ⠥ ("u") | Altı çizili başlangıç |
| ⡥ ("u" with dot 7) | Altı çizili son |
| ⠎ ("s")| üstü çizili başlangıç |
| ⡎ ("s" with dot 7) | Üstü çizili son |

##### Karakter karakter gezinirken karakteri bildir {#BrailleSpeakOnRouting}

Bu etkinleştirilirse, NVDA, braille imleç yönlendirme tuşlarıyla yönlendirme yaparken imlecin bulunduğu karakteri otomatik olarak seslendirecektir.

Bu seçeneği herhangi bir yerden değiştirmek için lütfen [Girdi Hareketleri iletişim kutusunu](#InputGestures) kullanarak özel bir hareket atayın.

##### Satır veya paragraf gezinirken sesli okuma {#BrailleSpeakOnNavigating}

Bu seçenek etkinleştirildiğinde, braille ekranı üzerindeki gezinme kontrolleriyle ulaşılan satır veya paragrafların NVDA tarafından sesli okunması sağlanır.

Bu seçeneği istediğiniz yerden değiştirmek için, [Girdi Hareketleri iletişim kutusu](#InputGestures) Braille bölümünden bir hareket atayabilirsiniz.

##### Mümkün olduğunda sözcükleri bölmemeye çalış {#BrailleSettingsWordWrap}

Bu seçenek etkinleştirilirse, braille ekranının sonuna sığmayacak kadar büyük olan bir sözcük bölünmeyecektir.
Bunun yerine ekranın sonunda bir miktar boş alan bırakılacaktır.
Ekranı kaydırdığınızda sözcüğün tamamını okuyabilirsiniz.
Bu seçenek bazen "kelime kaydırma" olarak da bilinir.
Sözcük tek başına bile ekrana sığmayacak kadar büyükse yine de bölünecektir.

Eğer bu seçenekdevre dışı bırakılırsa, sözcüğün görüntülenebilecek kısmı görüntülenecek, ancak geri kalanı kesilecektir.
Ekranı kaydırdığınızda sözcüğün tamamını okuyabilirsiniz.

Bu seçeneği etkinleştirmek daha akıcı okumalar yapmanızı sağlayabilir, ancak ekranı daha fazla kaydırmanız gerekir.

##### Unicode normalleştirme {#BrailleUnicodeNormalization}

Bu seçenek etkinleştirildiğinde, braille ekranda braille ile yazılan metin üzerinde unicode normalleştirme gerçekleştirilir.
Bu seçenek belirli bir braille tablosunda bilinmeyen ve sosyal medyada yaygın olarak kullanılan kalın ve italik karakterler gibi uyumlu bir alternatifi olan braille karakterleri ile karşılaştığınızda işe yarar.
Unicode normalleştirmenin diğer yararları [ilgili konuşma Unicode normalleştirme bölümünde](#SpeechUnicodeNormalization) daha detaylı olarak açıklanmıştır.

Unicode normalleştirmeyi herhangi bir yerden değiştirmek için lütfen [Girdi Hareketleri iletişim kutusunu](#InputGestures) kullanarak özel bir hareket atayın.
...
##### Kaydırma sırasında konuşmayı kesme {#BrailleSettingsInterruptSpeech}

Bu seçenek, Braille ekranı geriye/ileriye kaydırıldığında konuşmanın kesilip kesilmeyeceğini belirler.
Önceki/sonraki satır komutları her zaman konuşmayı keser.

Braille okurken devam eden konuşma dikkat dağıtıcı olabilir.
Bundan ötürü ekranı kaydırırken konuşmayı kesen bu seçenek varsayılan olarak etkindir.

Bu seçeneğin devre dışı bırakılması, Braille okurken konuşmayı da duymanızı sağlar.
...
* NVDA solda ve uygulamalar iki kanalda: NVDA sesi sol kanalda çalarken, diğer uygulamalar sesleri hem sol hem sağ kanalda verir.
...
##### Yazılan Karakterleri Seslendir {#KeyboardSettingsSpeakTypedCharacters}

Kısayol: `NVDA+2`

Bu seçenek, NVDA'nın klavyeden yazdığınız karakterleri ne zaman seslendireceğini belirler.
Kullanılabilir seçenekler:

* Kapalı: NVDA yazılan karakterleri seslendirmez.
* Sadece yazı alanlarında: NVDA sadece, yazı girilebilen alanlarda yazılan karakterleri seslendirir.
* Her zaman: NVDA tüm yazılan karakterleri seslendirir.

<!-- KC:setting -->

##### Yazılan Sözcükleri Seslendir {#KeyboardSettingsSpeakTypedWords}

Kısayol: `NVDA+3`

Bu seçenek, NVDA'nın klavyeden yazdığınız kelimeleri ne zaman seslendireceğini belirler.
Kullanılabilir seçenekler:

* Kapalı: NVDA yazılan kelimeleri seslendirmez.
* Sadece yazı alanlarında: NVDA sadece, yazı girilebilen alanlarda yazılan kelimeleri seslendirir.
* Her zaman: NVDA tüm yazılan kelimeleri seslendirir.
...
Bu seçenek varsayılan olarak açıktır. Ancak örneğin UniKey yazılımıyla Vietnamca yazan kullanıcılar gibi bazı kullanıcılar, karakterlerin yanlış girilmesine neden olabileceğinden bu özelliği devre dışı bırakmak isteyebilirler.

##### Çoklu tuşa basma zaman aşımı {#MultiPressTimeout}

Bazı NVDA klavye hareketleri, aynı tuşa hızlı bir şekilde art arda kaç kez basıldığına bağlı olarak farklı eylemler gerçekleştirir.
Bunun bir örneği "Nesne sunucusunun üzerinde bulunan karakteri bildir" komutudur.
Bu komut, bir kez basıldığında karakteri, iki kez basıldığında karakterin fonetik açıklamasını ve üç kez basıldığında karakterin sayısal değerini bildirir.
Bu seçenek, aynı tuşa yeniden basıldığında ilk basışın devamı olarak değil, yeni bir hareket olarak kabul edilmesi için zaman aşımını ayarlar.
Örnek komut için, çok kısa bir zaman aşımı, fonetik açıklama yerine iki kez basıldığında geçerli karakterin iki kez bildirilmesine neden olur.
Varsayılan zaman aşımı 500 MS. (yarım saniyedir.)
Zaman aşımınını artırmak, özellikle yapışkan tuş kullananlar veya fiziksel engeli olan kişiler için faydalı olabilir.
...
Varsayılan olarak açık olan bu seçenek, aday listesinin görünmesi veya sayfa değiştirmesi sırasında tüm görünen adayların otomatik olarak okunup okunmayacağını seçmenizi sağlar.
Bu seçeneğin açık olması, chinese New ChangJie veya Boshiami gibi pictographic girdi metodları için faydalı olabilir.
Ancak Çin Yeni Fonetik gibi ses temelli giriş yöntemlerinde, tüm semboller aynı şekilde okunacağından ve her adayı ok tuşlarıyla tek tek seçerek karakter açıklamalarını dinlemek daha faydalı olacağından, bu seçeneği kapatmanız önerilir.
...
Seçimin ok tuşlarıyla değiştirilebildiği giriş yöntemlerinde (ör. Çin Yeni Fonetik), bu seçenek gereklidir, fakat bazı yöntemlerde kapalı olması daha verimli yazım sağlayabilir.
...
Çince gibi yerel ayarlar için, seçilen karakterin ek açıklamalarının duyurulması bu seçenekten etkilenmez.
...
Okuma veya ön kompozisyon verileri bir sembolle birleştirildikten sonra, çoğu giriş yöntemi bu sembolü, belgeye eklenmeden önce geçici bir kompozisyon dizisine yerleştirir.
Bu seçenek, bileşim dizesinde yeni karakterler göründükçe NVDA’nın karakterleri bildirip bildirmeyeceğini kontrol eder.
Bu seçenek varsayılan olarak açıktır.
...
Varsayılan olarak etkin olan bu seçenek, bir NVDA komutu olmayan veya genel olarak komut tuşu olarak kabul edilmeyen hareketlerin (ör. tuşlara basmak), odağınızda olan belgeye iletilip iletilmeyeceğini belirlemenizi sağlar.
...
* Yazı tipi
  * Yazı tipi adı
  * Yazı tipi boyutu
  * Yazı tipi nitelikleri [(Kapalı, Konuşma, Braille, Konuşma ve Braille)](#DocumentFormattingFontAttributes)
  * Üst ve alt simgeler
  * Vurgu
...
* Öğeler
  * Başlıklar
  * Linkler
  * Bağlantı türü (aynı sayfa içinde hedef)
  * Grafikler
  * Listeler
  * Alıntılar
  * Gruplandırmalar
...
##### Yazı tipi nitelikleri {#DocumentFormattingFontAttributes}

Bu seçenek kalın, italik, altı çizili ve üstü çizili gibi belirli yazı tipi özelliklerinin nasıl bildirileceğini seçmenize yarar.
Yazı tipi nitelikleri seçim kutusunda dört seçenek bulunur:

* Kapalı: NVDA bu yazı tipi özelliklerini bildirmez.
* Konuşma: NVDA bu yazı tipi nitelikleri değiştiğinde bildirir.
* Braille: NVDA bu nitelikleri braille olarak gösterecektir.
Bunların tam olarak nasıl görüntüleneceği [NVDA'nın braille ayarlarında](#BrailleFormattingDisplay) yapılandırılabilir.
* Konuşma ve Braille: NVDA, yazı tipi niteliklerini yukarıdaki yöntemlerin her ikisini de kullanarak bildirecektir.
...
* Konuşma: Konuşma seçilirse, girinti miktarı değiştiğinde NVDA örneğin “on iki boşluk” veya “dört sekme” gibi bir bildirim yapar.
* Tonlar: Tonlar seçildiğinde, girinti miktarındaki değişim, işitsel tonlarla belirtilir.
...
##### Otomatik güncellemeler {#AutomaticAddonUpdates}
...
Varsayılan olarak eklenti güncellemeleriyle ilgili bildirimler sadece aynı [kanalda](#AddonStoreFilterChannel) (örn. kararlı - stable, beta veya geliştirici - dev) güncellemesi bulunan eklentiler için yapılır.
Eklenti güncelleme kanallarını [her bir eklenti için ayrı ayrı](#AddonStoreUpdateChannel) veya [tüm eklentiler için genel olarak](#DefaultAddonUpdateChannel) yapılandırabilirsiniz.

“Otamatik Güncelle” olarak ayarlandığında, eklentiler arka planda otomatik olarak güncellenir.
Güncellemeler tamamlandığında NVDA’yı yeniden başlatmanız istenir.

| . {.hideHeaderRow} |.|
|---|---|
|Seçenekler |Bildirim (Varsayılan), Otomatik Güncelle, Devre Dışı |
|Varsayılan |Bildir |

|Seçenek |Davranış |
|---|---|
|Bildir |Eklentiler için güncelleme olduğunda bildir |
|Otomatik Güncelle |Eklentileri arka planda otomatik güncelle |
|Devre Dışı |Eklenti güncellemelerini otomatik olarak kontrol etme |

##### Varsayılan Güncelleme Kanalı {#DefaultAddonUpdateChannel}

[Otomatik eklenti güncellemeleri](#AutomaticAddonUpdates) etkinleştirildiğinde, varsayılan olarak eklentiler yalnızca aynı [kanala](#AddonStoreFilterChannel) güncellenir.
Örneğin, yüklenmiş bir beta sürüm sadece yeni bir beta sürüm mevcutsa güncellenir.
Bu ayar tüm eklentiler için varsayılan güncelleme kanalını belirlemektedir.
Ayrıca [Eklenti Mağazası'ndan](#AddonStoreUpdateChannel) tek bir eklentinin güncelleme kanalını ayrı ayrı değiştirebilirsiniz.

| . {.hideHeaderRow} |.|
|---|---|
| Seçenekler | Aynı (Varsayılan), Herhangi, Güncellenmesin, Kararlı (Stable), Beta veya geliştirici (dev), Beta, Geliştirici (Dev) |
| Varsayılan | Aynı |

| Seçenek | Davranış |
|---|---|
| Aynı | Eklentiler bulundukları kanalda kalır ve güncellenmez |
| Herhangi | Eklentiler, hangi kanalda olursa olsun otomatik olarak en son sürüme güncellenir |
| Güncellenmesin | Eklentiler otomatik olarak güncellenmez, her birini tek tek elle güncellemeniz gerekir |
| Kararlı | Eklentiler yalnızca kararlı (stabil) sürümlere otomatik olarak güncellenir |
| Beta veya geliştirici | Eklentiler, beta veya geliştirici sürümlerine otomatik olarak güncellenir |
| Beta | Eklentiler sadece beta sürümlerine otomatik olarak güncellenir |
| Geliştirici | Eklentiler sadece geliştirici (dev) sürümlerine otomatik olarak güncellenir |

##### Uyumsuz eklentilerin otomatik güncellenmesine izin ver {#AllowIncompatibleAddonUpdates}

Bu ayar, mevcut NVDA sürümüyle tam olarak uyumlu olmayan eklentilerin de otomatik olarak güncellenmesini sağlar.
Varsayılan olarak bu ayar kapalıdır; otomatik güncellemeler yalnızca mevcut NVDA sürümüyle uyumlu olarak işaretlenen eklenti sürümlerine yapılır.
Yine de, eğer uyumlu bir sürüm yayınlanırsa, otomatik güncellemeyle eklentiniz uyumlu sürüme yükseltilir.
Bu özelliği açmak, özellikle yeni çıkan, geriye dönük uyumluluğu bozan (yılın ilk büyük sürümleri gibi) güncellemeler geçişinde yararlı olabilir.
Bu özellik eklentilerin uyumluluğunu test eden alfa ve beta kullanıcıları için de oldukça faydalıdır.

##### Kopya sunucusu {#AddonStoreMetadataMirror}

Bu ayarlar, Eklenti Mağazası verilerini farklı bir adresten indirmek için alternatif bir adres belirlemenizi sağlar.
Eğer NV Access Eklenti Mağazası sunucusuna erişim yavaş ya da mümkün değilse, bu işlev yardımcı olabilir.

Salt okunur kutuda mevcut kopya adresi gösterilmektedir.
Eğer herhangi bir güncelleme kopyası kullanılmıyorsa "Kopya yok" ifadesi görüntülenir.

Eklenti Mağazası'nın kopya adresini değiştirmek için "Değiştir..." düğmesine tıklayarak [Eklenti Mağazası Kopya Adresini Ayarla penceresini](#SetURLDialog) açabilirsiniz.

#### Uzaktan Erişim Ayarları {#RemoteSettings}

Bu bölümde, [Uzaktan Erişim](#RemoteAccess) özelliklerini ayarlayabilirsiniz.

##### Uzaktan Erişimi Etkinleştir {#RemoteEnable}

Bu onay kutusu NVDA’nın Uzaktan Erişim özelliğini açıp kapatır.

Onay kutusu işaretli değilse, Uzaktan Erişim özelliği devre dışıdır.

Dikkat: Kullanıcı Hesabı Denetimi (UAC) gibi güvenlik ekranlarında uzaktan oturuma devam etmek için bu ayarın açık olması, değişikliklerin kaydedilmesi ve ardından [ayarların NVDA’nın sistem klasörüne kopyalanması](#GeneralSettingsCopySettings) gerekir.
Uzaktan Erişim sadece yüklü NVDA kopyasıyla başlatılan oturumlarda güvenli ekranlarda kullanılablilir.
Örneğin, taşınabilir NVDA ile başlatılan bir uzaktan erişim oturumu güvenli ekranda devam edemez.

Aşağıdaki seçenekler yalnızca Uzaktan Erişim aktifse görünür:

##### Kontrol edilen bir bilgisayardan bağlantı kesileceğinde onay iste {#RemoteConfirmDisconnect}

Bu seçenek açıkken, kontrol edilen bilgisayardan bağlantıyı koparmadan önce NVDA sizden onay ister.

Eğer onay kutusu işaretliyse, NVDA bağlantıyı kesmeden önce onayınızı alır.
Kapalıysa, bağlantı onay almadan kesilir.

##### NVDA başlatıldığında otomatik olarak bağlan {#RemoteAutoconnect}

Bu seçenekle, NVDA açılır açılmaz otomatik olarak bir uzaktan erişim oturumu başlatabilirsiniz.
Bu seçenek örneğin evden uzaktayken evdeki bilgisayarınızı yönetmek için kullanışlı olabilir.

Ancak, bu özelliği açtığınızda bilgisayarınızın güvenliği açısından dikkatli olmanız önerilir.

Aşağıdaki gelişmiş seçenekler için [Uzaktan Erişim Oturumu Kurulumu](#RemoteAccessSetup) bölümüne bakabilirsiniz.

##### Mod {#RemoteAutoconnectMode}

Otomatik bağlantılar için bağlantı modunu seçin.

Bu seçenek sadece [NVDA başlatıldığında otomatik olarak bağlan](#RemoteAutoconnect) aktifleştirildiyse kullanılır.

|Seçenek | Davranış |
|---|---|
|Bu bilgisayar kontrol edilsin | Bu bilgisayar “kontrol edilen” olarak kullanılır; başka bir bilgisayar üzerinden bu bilgisayara tam erişim sağlanır. |
|Başka bir bilgisayarı kontrol et | Bu bilgisayar “kontrol eden” olarak kullanılır; uzaktaki bilgisayarı komutlarla yönetebilirsiniz. |

##### Sunucu {#RemoteAutoconnectServer}

Otomatik bağlantılar için sunucu türünü seçin.

Bu ayar sadece [NVDA başlatıldığında otomatik olarak bağlan](#RemoteAutoconnect) seçeneği aktifse görünür.

|Seçenek | Davranış |
|---|---|
|Mevcut sunucuyu kullan | Bağlantı için var olan bir Uzaktan Erişim sunucusunu kullanır (önerilir). |
|Yerel olarak barındır| NVDA’yı doğrudan uzaktan erişim sunucusu yapar (ileri seviye), ek ağ ayarları gerekebilir. |

##### Host {#RemoteAutoconnectHost}

Otomatik bağlantı için kullanmak istediğiniz Uzaktan Erişim sunucusunun adresini buraya yazın.
Sunucu 6837 dışında bir port kullanıyorsa, adresin sonuna iki nokta ve port numarasını ekleyin (ör: `ornek.com:1234`).

Bu alan yalnızca [Otomatik bağlan](#RemoteAutoconnect) ve [Sunucu](#RemoteAutoconnectServer) “Mevcut sunucuyu kullan” olarak ayarlanmışsa geçerlidir.

##### Port {#RemoteAutoconnectPort}

Otomatik bağlantı için kullanmak istediğiniz TCP portunu buraya yazın.

Bu seçenek sadece [Otomatik bağlan](#RemoteAutoconnect) ve [Sunucu](#RemoteAutoconnectServer) “Yerel olarak barındır” şeklinde ise geçerlidir.

##### Anahtar {#RemoteAutoconnectKey}

Otomatik bağlantı için kullanmak istediğiniz anahtarı buraya yazın.

Bu seçenek yalnızca [Otomatik bağlan](#RemoteAutoconnect) etkinse geçerlidir.

##### Tüm güvenilen parmak izlerini sil {#RemoteDeleteFingerprints}

Bu düğme, daha önce güvenilen tüm Uzaktan Erişim sunucularının parmak izlerini unutmanıza olanak tanır.
Böylece, daha önce bağlandığınız tüm bilinmeyen sunucular için tekrar onay istenir.
Güvenilen parmak izleri silinmeden önce sizden onay alınır.

Bu seçenek, yalnızca yapılandırmanızda güvenilir parmak izleri varsa kullanılabilir.
...
#### Kopya sunucusunu ayarla iletişim kutusu {#SetURLDialog}

Bu pencere, NVDA’yı [güncellerken](#GeneralSettingsCheckForUpdates) ya da [Eklenti Mağazası’nı kullanırken](#AddonsManager) kullanılacak güncelleme kopya adresi belirtmenizi sağlar.
bu işlemler için NV Access sunucularına erişim yavaş ya da mümkün değilse kopya adresi ayarlamak yararlı olabilir.

* [NVDA güncelleme kopyası](#UpdateMirror) ayarlanırken bu pencerenin başlığı “NVDA Güncelleme Kopyasını Ayarla” şeklindedir.
* [Eklenti Mağazası güncelleme kopyası](#AddonStoreMetadataMirror) ayarlanırken başlık “Eklenti Mağazası Kopya Sunucusunu Ayarla” olur.

##### Adres {#SetURLTextbox}

Buraya kullanmak istediğiniz kopyanın web adresini yazın.
Sadece http ve https adresleri desteklenir.
Gizliliğiniz için, kullanılabilir olduğunda NV Access HTTPS bağlantıları kullanmanızı önerir.

Varsayılan sunucuyu kullanmak için boş bırakın.

##### Test {#SetURLTest}

Girdiğiniz kopya adresini test etmek için bu düğmeyi kullanın.
Testin başarılı olması için internet bağlantınız olması gerekir.
Kopya adresini kaydetmeden önce her zaman test etmeniz önerilir.
...
* Sadece gerekli olduğunda: Microsoft Word nesne modüllerinin kullanılabilir olmadığı durumlarda
...
##### Konuşmadaki sessizliği kırp {#TrimLeadingSilenceSpeech}

Etkinleştirildiğinde, NVDA konuşmaların başında bululabilecek sessizliği kırparak bazı sentezleyicilerde tepki süresini düşürebilir.
Varsayılan olarak bu seçenek etkindir ve yalnızca konuşmanın başındaki sessizliği etkiler.
Bir sentezleyiciyi kullanırken bazı gerekli sessizliklerin de kaybolduğunu fark ederseniz (ör. iki cümle arasındaki duraklama), bu özelliği kapatarak sorunu çözebilirsiniz.

##### SAPI 4 ses çıkışı için WASAPI kullan {#UseWASAPIForSAPI4}

Bu seçenek, Microsoft Speech API versiyon 4 (SAPI 4) seslerinin çıktılarını Windows Audio Session API (WASAPI) üzerinden vermesini sağlar.
Böylece SAPI 4 sesleri, ses bastırma, baştaki sessizlikleri kırpma ve ses cihazını uyanık tutma gibi ek özelliklerle uyumlu şekilde çalışabilir.
Ancak, bazı SAPI 4 sesleri mevcut WASAPI desteğiyle düzgün çalışmayabilir.
Kullandığınız SAPI 4 sesi çalışmayı durdurursa, bu seçeneği devre dışı bırakabilirsiniz.
...
* Düzey alanı ise, işaretin seslendirilmesini istediğiniz en düşük noktalama seslendirme düzeyini belirlemenize imkan verir (hiç biri, bağzıları, çoğu veya tümü).
...
Bu nedenle, Değiştirme alanında normal ters bölü işaretleri ikiyle yazılmalıdır, örneğin "a\\\\b" yazarsanız "a\\b" ile değişim elde edilir.
...
| Ad |Masaüstü Kısayol tuşu |Dizüstü Kısayol tuşu |Tarif|
|---|---|---|---|
|Konfigurasyonu kaydet |`NVDA+kontrol+c` |`NVDA+control+c` |Geçerli ayarlarınızı NVDA'dan çıkarken kaybolmasın diye kaydeder.|
|Kaydedilmiş konfigürasyona dön |`NVDA+kontrol+r` |`NVDA+control+r` |Bir kez bastığınızda ayarınızı son kaydettiğiniz hâle döndürür. Üç kez bastığınızda fabrika ayarlarına döner.|
...
## Uzaktan Erişim {#RemoteAccess}

NVDA'nın dahili Uzaktan Erişim özelliğini kullanarak, NVDA çalıştıran başka bir bilgisayarı kontrol edebilir, ya da bilgisayarınızın kontrol edilmesini sağlayabilirsiniz.
Böylece, birisine yardım etmek veya yardım almak, işbirliği yapmak veya uzaktan kendi bilgisayarınıza erişmek daha kolay olur.

Güvenliğiniz için Uzaktan Erişim varsayılan olarak devre dışıdır.
Uzaktan Erişim'i [Uzaktan Erişim ayarları](#RemoteSettings) penceresinden etkinleştirebilirsiniz.

### Başlarken {#RemoteAccessGettingStarted}

Başlamadan önce, NVDA'nın her iki bilgisayarda da yüklendiğinden ve çalıştırıldığından emin olun.
Ayrıca [Uzaktan Erişim ayarları](#RemoteSettings) bölümündeki Uzaktan Erişimi Etkinleştir kutusunun iki bilgisayarda da işaretli olduğundan emin olun.
Sonrasında, Uzaktan Erişim ek bir indirme veya kurulum gerektirmeden NVDA'nın Araçlar menüsünden kullanılabilir.

NVDA [güvenli modda](#SecureMode) çalışıyorsa, manuel olarak yeni bir Uzaktan Erişim oturumu başlatamazsınız.
Ancak, [NVDA başladıktan sonra otomatik bağlan](#RemoteAutoconnect) ayarıyla yapılandırıldıysa, NVDA otomatik olarak yeni bir Uzaktan Erişim oturumu başlatacaktır.
Güvenli modda olsanız da bağlantıyı manuel olarak kesebilirsiniz.

### Uzaktan Erişim Oturumu Ayarlama {#RemoteAccessSetup}

Uzaktan Erişim Oturumları Uzaktan Erişim Bağlantısı iletişim kutusu aracılığıyla yapılandırılır.
NVDA menüsünü açın, Araçlar’ı seçin, ardından "Uzaktan Erişim" ve "Bağlan..."’ı seçin.
Veya herhangi bir yerden `nvda+alt+r` tuşlarına basın.

#### Mod Seçimi (Kontrol edilen veya Kontrol eden) {#RemoteAccessMode}

Hangi bilgisayarın kontrol edileceğine (kontrol edilen bilgisayar) ve hangisinin kontrol edeceğine (kontrol eden bilgisayar) karar vermeniz gerekir.

Mod seçim kutusunda , bu bilgisayarın başkası tarafından mı kontrol edileceğine (bir başkası bilgisayarınıza erişir) yoksa başka bir bilgisayarı sizin mi kontrol edeceğinize (siz başka bir bilgisayara erişirsiniz) karar verebilirsiniz.

* Bilgisayarınızda size yardımcı olacak başka bir kişi varsa, "Bu bilgisayarın kontrol edilmesine izin ver" seçeneğini seçin.
* Eğer başkasına yardım edecekseniz, "Başka bir bilgisayarı kontrol et" i seçin.

#### Mevcut bir sunucuya bağlanma (ana bilgisayar ve anahtar bilinmesi gerekir) {#RemoteAccessJoin}

Bu durumda, diğer kişi bağlantıyı bir aktarma sunucusu üzerinden başlattı ya da bağlantıya yerel olarak ev sahipliği yapıyor.

1. Sunucu için, "Mevcut olanı kullan" seçeneğini seçin.
1. Sunucu Adresi alanına, size verilen sunucu adresini (gerekirse port numarasıyla birlikte) girin.
  * Bağlantı yapılacak portu da yazmak isterseniz, sunucu adresinden sonra iki nokta üst üste (":") ve port numarasını ekleyin.
Örneğin: `example.com:1234`
1. Anahtar alanına, size verilen anahtarı yazın.
Anahtarı tam olarak verildiği gibi yazmaya dikkat edin.
1. Tamam'a basın.

#### Aktarma Sunucusu üzerinden Bağlantı Başlatma {#RemoteAccessConnectRelay}

Başka birinin internet üzerinden size bağlanmasına izin vermek istiyorsanız, aktarma sunucusu kullanmak en kolay yoldur. Bunun için:

1. Sunucu olarak "Mevcut olanı kullan"ı seçin.
1. Sunucu olarak, bir aktarma sunucusunun adını girin, örneğin `nvdaremote.com`.
Bu adresi diğer kişiye vermeniz gerekecektir.
  * Bağlantı yapılacak portu da yazmak isterseniz, sunucu adresinden sonra iki nokta üst üste (":") ve port numarasını ekleyin.
Örneğin: `example.com:1234`
1. Anahtar için: Uzun ve tahmin edilmesi zor bir anahtar oluşturun veya otomatik üretmek için "Anahtar Oluştur" düğmesine basın.
Bu anahtarı karşı tarafa iletmeniz gerekir.
  * Anahtar oturumun kimliği ve parolası olarak işlev gördüğünden, güvenlik için tahmin edilmesinin zor olması çok önemlidir.
1. Tamam'a basın.
1. Karşı tarafın bağlantı yapabilmesi için bağlantı detaylarını (ör. sunucu ve anahtar) karşı tarafa iletmeniz gerekir.

#### NVDA'yı Sunucu Olarak Kullanma {#RemoteAccessConnectSelfHosted}

Eğer bir aktarma sunucusu kullanmak istemiyorsanız veya yerel ağ üzerinden bağlanacaksanız:

1. Sunucu olarak "Yerel olarak barındır"ı seçin.
1. İnternet üzerinden bağlanıyorsanız, "Harici ip Al" düğmesine basarak harici (internetten görülebilir) IP adresinizi görebilirsiniz.
Bu harici IP'yi karşı tarafa vermelisiniz.
1. Yerel ağdan bağlanıyorsanız, karşı tarafa yerel IP adresinizi vermelisiniz.
Bunun nasıl bulunacağı bu belgede anlatılmamaktadır.
1. Port alanına, gelen uzak bağlantılar için kullanmak istediğiniz portu yazın.
Karşı tarafın da aynı portu bilmesi gerekir.
  * Port  başka bir hizmet tarafından kullanılmamalıdır.
  * Varsayılan olarak, uzaktan erişim 6837 numaralı portu kullanır.
  * İnternet üzerinden bağlanıyorsa, portun açılması gerekebilir.
  Burada nasıl port açılacağı anlatılmaz.
  Port açmak için ağ yöneticinizle iletişime geçmeniz gerekebilir.
1. Anahtar için: Uzun ve tahmin edilmesi zor bir anahtar oluşturun veya "Anahtar Oluştur" düğmesiyle otomatik üretin.
Bu anahtarı karşı tarafa iletmeniz gerekir.
  * Anahtar hem oturum kimliği hem de parola olduğundan, tahmin edilmesinin zor olması önemlidir.
1. Tamam'a basın.
1. Karşı tarafın bağlantı yapabilmesi için bağlantı detaylarını (ör. sunucu ve anahtar) karşı tarafa iletmeniz gerekir.

### Uzaktan Erişimi Kullanma {#RemoteAccessUsage}

Bir uzaktan oturum başladığında, uzaktaki bilgisayarı veya kendi bilgisayarınızı kontrol etme, panoyu paylaşma ve uzak oturumu susturma gibi işlemler yapabilirsiniz:

* `NVDA+alt+tab` tuşuna basarak uzaktaki bilgisayarı veya kendi bilgisayarınızı kontrol etme arasında geçiş yapabilirsiniz.
* Panonuzdaki metni, NVDA menüsünden Araçlar'a, ardından "Uzaktan Erişim" ve "Panoyu gönder"i seçerek karşı bilgisayara gönderebilirsiniz.
* Kontrol eden bilgisayar olarak bağlıysanız, uzak bilgisayarın konuşmasını kendi bilgisayarınızda susturmak için NVDA menüsünden Araçlar > Uzaktan Erişim > "Uzağı sustur"u seçebilirsiniz.
* Kontrol eden bilgisayar olarak, karşı bilgisayara `kontrol+alt+delete` göndermek için NVDA menüsünden Araçlar > Uzaktan Erişim > "kontrol+alt+delete gönder"i seçin.
  Bunun çalışabilmesi için, kontrol edilen bilgisayarda kurulu bir NVDA olması gerekir.
  Ayrıca, kontrol edilen bilgisayarın ayarlarına bağlı olarak bu işlev her zaman çalışmayabilir.

### Uzaktan Erişim Hızlı Başlangıç Kısayol özeti {#RemoteAccessGestures}

<!-- KC:beginInclude -->
| Ad |Tuş |Açıklama|
|---|---|---|
| Uzaktan bağlantıyı aç/kapat | `NVDA+alt+r` | Yeni bir uzak erişim oturumu başlatır, veya zaten açıksa oturumu sonlandırır. |
| Kontrolü Değiştir | `NVDA+alt+tab` | Uzaktaki bilgisayarı kontrol ile kendi bilgisayarınıza dönme arasında değişim yapar. |
| Bağlan | Yok | Yeni bir Uzaktan Erişim oturumu başlatır. [Güvenli modda](#SecureMode) kullanılamaz. |
| Bağlantı linkini kopyala | Yok | Uzak oturumun bağlantısını panoya kopyalar. |
| Bağlantıyı kes | Yok | Mevcut Uzaktan Erişim oturumunu bitirir. |
| Uzağı sustur | Yok | Uzak bilgisayardan gelen sesi susturur veya tekrar açar. |
| Panoyu gönder | Yok | Panonuzdaki içeriği uzaktaki bilgisayara yollar. |
<!-- KC:endInclude -->
...
#### Eklenti listesini sütuna göre sıralama {#AddonStoreSortByColumn}

Varsayılan olarak,eklentiler adlarına göre sıralanır.
Sütuna göre sırala seçim kutusu ayrı ayrı sekmedeki sıralamaları ayarlamak için kullanılabilir.
Örneğin, eklentileri yayıncı, mevcut sürüm vb. Tarafından sıralamak isteyebilirsiniz.
Eklentiler artan veya azalan şekilde sıralanabilir.
...
İndirme veya kurulum başarısız olursa, kurulumu yeniden deneyebilirsiniz.
Eklenti mağazasından çıkmadan önce yüklemeyi iptal de edebilirsiniz.
...
Eğer eklenti "etkin" ise devre dışı bırakabilir, "devre dışı" ise tekrar etkinleştirebilirsiniz.
...
#### Otomatik güncelleme kanalının değiştirilmesi {#AddonStoreUpdateChannel}

Eklentilerin otomatik güncelleme kanallarını [kurulu ve güncellenebilir eklentiler sekmesinden](#AddonStoreFilterStatus) yönetebilirsiniz.
[Eklenti otomatik güncellemeleri](#AutomaticAddonUpdates) etkin olduğunda, eklentiler varsayılan olarak [yüklenirken kullanılan kanalda](#DefaultAddonUpdateChannel) güncellenir.
Bir eklentinin eylemler menüsünden "Güncelleme kanalı" alt menüsünü kullanarak, eklentinin otomatik olarak güncelleneceği kanalı değiştirebilirsiniz.

| Seçenek | Davranış |
|---|---|
| Varsayılan | Eklenti, [varsayılan güncelleme kanalını](#DefaultAddonUpdateChannel) takip eder |
| Aynı | Eklenti mevcut kanalında kalır |
| Herhangi | Eklenti, kanal fark etmeksizin en son sürüme otomatik olarak güncellenir |
| Güncellenmesin | Eklenti otomatik olarak güncellenmez |
| Kararlı | Eklenti yalnızca kararlı sürümlere otomatik güncellenir |
| Beta veya geliştirici | Eklenti yalnızca beta veya geliştirici sürümlere otomatik güncellenir |
| Beta | Eklenti yalnızca beta sürümlere otomatik güncellenir |
| Geliştirici | Eklenti yalnızca geliştirici sürümlere otomatik güncellenir |
...
Bazen, NVDA'nın düzgün çalışmamasına neden olan Windows Kayıt Defteri ile sorunlar ortaya çıkabilir.
Bu, örneğin belirli programların (Adobe Reader veya MathPlayer  gibi), Windows güncellemelerinin vb. yüklenmesinden veya kaldırılmasından kaynaklanabilir.
COM kayıt tamir aracı, kayıt defterindeki erişilebilirlik girişlerini onararak bu sorunları çözmeye çalışır.

Bu aracın düzeltebileceği sorunlardan bazıları şunlardır:

* Firefox veya Edge gibi tarayıcılarda gezinirken, Thunderbird, Windows Gezgini, Görev Çubuğu ve diğer programlar gibi posta programlarında gezinirken "bilinmeyen" veya "bölme" denmesi
* NVDA'nın kullanılabilir yerlerde tarama kipi ve odak KİPLERİ arasında geçiş yapamaması
* Daha önce isimleri konuşulan, aniden sadece "düğme" olarak GÖRÜLEN düğmeler.
* Tarama kipinde tarayıcılarda gezinirken NVDA'nın çok yavaş çalışması

Bu araç, Windows kayıt defterindeki girişleri düzelttiğinden, tıpkı bir program yüklerken olduğu gibi  çalışmak için yönetici erişimi gerektirir.
Çoğu kullanıcıda olduğu gibi UAC (Kullanıcı Hesabı Denetimi) açıksa, aracı sorunsuz çalıştırabilmeniz için UAC tarafından gösterilen pencereleri takip etmeniz gerekir.
...
NVDA ile kullanmak üzere satın alıp indirebileceğiniz ücretsiz ve ticari konuşma sentezleyicilerin daha kapsamlı bir listesi için lütfen [ekstra sesler sayfasına](https://github.com/nvaccess/nvda/wiki/ExtraVoices) bakın.
...
HID kullanmayan ekranlara USB üzerinden bağlanıyorsanız öncelikle üretici tarafından sağlanan USB sürücülerini yüklemelisiniz.
VarioUltra ve Pronto! HID'yi kullanır.
Refreshabraille ve Orbit Reader 20, uygun şekilde yapılandırıldığı takdirde HID'yi kullanabilir.

Orbit Reader 20'nin USB seri modu şu anda yalnızca Windows 10 ve sonraki sürümlerde desteklenmektedir.
Bunun yerine genellikle USB HID kullanılmalıdır.

NVDA'da bu ekranlara ilişkin temel atamalar aşağıda verilmiştir.
Bu tuşların nerede bulunabileceğine ilişkin açıklamalar için lütfen ekranınızın kullanma klavuzlarına bakın.
<!-- KC:beginInclude -->
...
[HumanWare](https://www.humanware.com/) tarafından sunulan Brailliant BI ve B serisi ekranlar, örneğin BI 14, BI 32, BI 20X, BI 40, BI 40X ve B 80, USB veya Bluetooth üzerinden bağlandığında desteklenmektedir.
...
Aşağıda, NVDA ile Brailliant BI/B ve BrailleNote Touch ekranları için klavye kısayolları yer almaktadır.
Tuşların yeri ve tarifi için Braille ekranın dokümantasyonunu inceleyin.
...
BrailleNote Touch modeli için lütfen [Brailliant BI Serisi / BrailleNote Touch](#HumanWareBrailliant) bölümüne bakın.
...
|Say all |`f14`|
|İnceleme imleci altındaki mevcut karakteri bildirir |`f15`|
...
### Standart HID Braille ekranları {#HIDBraille}

Bu, 2018'de Microsoft, Google, Apple ve NV Access dahil çeşitli yardımcı teknoloji şirketleri tarafından kabul edilen yeni Standart HID Braille Spesifikasyonu için deneysel bir sürücüdür.
Herhangi bir üretici tarafından oluşturulan gelecekteki tüm Braille Ekran modellerinin, üreticiye özel Braille sürücülerine olan ihtiyacı ortadan kaldıracak bu standart protokolü kullanması umulmaktadır.

NVDA'nın otomatik braille ekran algılama özelliği, bu protokolü destekleyen tüm ekranları da tanıyacaktır.

Bu ekranlar için geçerli tuş atamaları aşağıda verilmiştir.
<!-- KC:beginInclude -->

| Ad |kısayol tuşu|
|---|---|
|Braille ekranını geri kaydır|pan left or rocker up|
|Braille ekranını ileri kaydır|pan right or rocker down|
|Braille hücresine git |routing set 1|
|Braille taşımayı aç/kapatto |up+down|
|yukarı ok tuşu |joystick up, dpad up or space+dot1|
|aşağı ok tuşu|joystick down, dpad down or space+dot4|
|sol ok tuşu |space+nokta3, joystick sola veya dpad sola|
|sağ ok tuşu|space+dot6, joystick right or dpad right|
|şift+sekme tuşu |space+dot1+dot3|
|Sekme tuşu |space+dot4+dot6|
|alt tuşu |space+dot1+dot3+dot4 (space+m)|
|escape tuşu |space+dot1+dot5 (space+e)|
|enter tuşu |dot8, joystick center or dpad center|
|windows tuşu |space+dot3+dot4|
|alt+tab tuşu |space+dot2+dot3+dot4+dot5 (space+t)|
|NVDA menüsü |space+dot1+dot3+dot4+dot5 (space+n)|
|windows+d tuşu (tüm uygulamaları simge durumuna küçült) |space+dot1+dot4+dot5 (space+d)|
|tümünü oku |space+dot1+dot2+dot3+dot4+dot5+dot6|
...
Dot Inc tarafından geliştirilen Dot Pad, yenilenebilir dokunsal grafikler ve braille gösterebilen bir cihazdır.
NVDA, bu cihazda özel braille satırında tek satırlık braille veya dokunsal grafik alanında çok satırlı braille gösterebilir.
A300 modeli, her biri 20 hücreden oluşan 8 satıra sığabilecek şekilde 120 x 80 noktalık bir dokunsal grafik alanına sahiptir.

NVDA'nın bu sürücüsündeki Braille Ayarları'ndaki Braille Hedefi seçeneği ile, braillenin özel satırda mı yoksa grafik alanında mı gösterileceğini yapılandırabilirsiniz.

Kaydırma (panning) tuşları desteklenir, ancak cihazdaki sınırlı buton sayısı nedeniyle diğer komutlar ve konumlama (routing) işlevleri şu anda mevcut değildir.

NVDA'da Dot Pad sürücüsünü seçerken, Dot Pad'in bağlı olduğu USB/Bluetooth sanal seri portunu manuel olarak tanımlamanız gerekmektedir.
Bu sürücü otomatik algılamayı desteklemez.

Donanım sınırlamalarından dolayı, eliniz cihazın üstündeyken Dot Pad tüm noktaları tam olarak yenilemeyebilir, lütfen gezinti sırasında elinizi tamamen kaldırıp yalnızca güncelleme tamamlandıktan sonra tekrar dokunarak okumaya başlayın.
Donanım sınırlamalarından dolayı, eliniz cihazın üstündeyken Dot Pad tüm noktaları tam olarak yenilemeyebilir, lütfen gezinti sırasında elinizi tamamen kaldırıp yalnızca güncelleme tamamlandıktan sonra tekrar dokunarak okumaya başlayın.
...
| Ad |Kısayol|
|---|---|
|Braille ekranı geri kaydır | `pan_left` |
|Braille ekranı ileri kaydır | `pan_right` |
...
* NVDA menüsünden, kullanıcı kılavuzu dosyası gibi harici belgeleri açma.
...
|`-n LANGUAGE` |`--lang=LANGUAGE` |Yapılandırılmış NVDA dilini geçersiz kılar. Geçerli kullanıcı varsayılanı için "Windows", İngilizce için "en" gibi değerler alır.|
|`-m` |`--minimal` |Ses, arayüz, başlangıç mesajı vb olmaz|
|`-s` |`--secure` |NVDA'yı [Güvenli Mod](#SecureMode)'da başlatır|
|`-d` |`--disable-addons` |Eklentiler çalışmaz.|
...
|Yok |`--create-portable-silent` |NVDA’nın taşınabilir bir kopyasını oluşturur (yeni kopyayı başlatmaz). `--portable-path` seçeneğinin belirtilmesi gerekir. Bu seçenek, dolu dizinlere yazarken uyarıları atlar ve dosyaları uyarı vermeden üzerine yazabilir.|
|None |--`portable-path=PORTABLEPATH` |taşınabilir kopyanın oluşturulacağı adres|

NVDA’yı --install-silent komut satırı seçeneğiyle sessizce kurabileceğiniz gibi, kaldırıcıya /S komutunu vererek de sessizce kaldırabilirsiniz.

NVDA kaldırıcısı `uninstall.exe` olarak adlandırılır ve NVDA’nın kurulu olduğu dizinde bulunur. 64 bit Windows’ta `%ProgramFiles(x86)%\nvda`, 32 bit Windows’ta ise `%ProgramFiles%\nvda` yolundadır.

Aşağıda, NVDA’nın kaldırıcısının komut satırı seçenekleri verilmiştir:

| Kısa |Uzun |Açıklama|
|---|---|---|
|`/S` |Yok |NVDA'yı sessizce kaldırır. |
