...
Bu sürümde NVDA uzaktan erişim desteği eklendi. Bu özellik sayesinde NVDA çalıştıran uzaktaki bir bilgisayarı NVDA çalıştıran bir cihaz ile kontrol edebilirsiniz.

Konuşma ile ilgili birçok geliştirme yapıldı. Özellikle SAPI 4, SAPI 5 ve OneCore seslerinin tepki hızıyla ilgili geliştirme yapıldı.
SAPI 5'te hız artırıcı ve otomatik dil değiştirme destekleniyor.
SAPI 4 sesleri ses zayıflaması, konuşmaların başındaki sessizliklerin kesilmesi ve ses cihazını uyanık tutma özelliklerini destekliyor.

Eklenti mağazasının otomatik güncelleme sistemi geliştirildi. Otomatik güncellemeler için kanal seçebiliyor ve otomatik güncellemeleri arka planda yapabiliyorsunuz.

OCR sonucunu elle yenilemek ve OCR sonucunun düzenli olarak yenilenmesini açıp kapatmak için komutlar eklendi.

Yerel seçim Chrome ve Edge'de kullanılabiliyor.

Microsoft Office ve LibreOffice desteği geliştirildi. Özellikle kısayol tuşlarıyla yapılan daha fazla eylem seslendiriliyor.

Varsayılan braille girdi ve çıktı tabloları NVDA diline göre ayarlanabiliyor.
Dot Inc tarafından üretilen Dot Pad dokunsal grafik cihazı için çoklu satırlı braille cihazı olarak yerleşik destek eklendi.
Braille ekranlar için, özellikle bağlantı konusunda başka küçük düzeltmeler yapıldı.

LibLouis braille translator, eSpeak - NG ve Unicode CLDR güncellendi.
Yeni Kıpti, küçültülmüş Çivi Yazısı ve Portekizce 6 noktalı Bilgisayar Braille tabloları eklendi.
...
Lütfen güvenlik sorunlarını NVDA'nın [güvenlik politikası](https://github.com/nvaccess/nvda/blob/master/security.md) uyarınca sorumlu bir şekilde bildirin.

* Cihaz kilitlendiğinde braille ekranlarda potansiyel olarak hassas bilgilerin gösterilmesi önlendi.
([GHSA-8f8q-2jc3-6rf4](https://github.com/nvaccess/nvda/security/advisories/GHSA-8f8q-2jc3-6rf4))
* Kurulumun, dizininden istenmeyen DLL'leri yüklemesini önler.
([GHSA-qf5h-qw92-rx2f](https://github.com/nvaccess/nvda/security/advisories/GHSA-qf5h-qw92-rx2f))

### Yeni Özellikler

* Eklenti mağazası:
  * Otomatik güncellemeler (#3208):
    * Eklentiler için otomatik güncelleme kanalları değiştirilebiliyor.
      * Kurulu eklentiler için "güncelleme kanalı" alt menüsü aracılığıyla otomatik güncelleme kanalları seçilebiliyor.
      * Varsayılan otomatik güncelleme kanalı, NVDA ayarlarındaki Eklenti Mağazası panelinden ayarlanabiliyor.
    * Otomatik güncellemeler arka planda yapılabiliyor.
      * NVDA ayarlarındaki Eklenti Mağazası panelinden "Otomatik güncellemeler" ayarını "Otomatik Olarak Güncelle" olarak değiştirerek etkinleştirilebilir.
    * Otomatik güncellemelerle, uyumsuz eklentiler başka yeni ve uyumsuz bir sürüme güncellenebiliyor.
      * NVDA ayarlarındaki Eklenti Mağazası panelinden etkinleştirilebilir.
  * Eklentilerin kurulumunu iptal etmek için eylem eklendi. (#15578, @hwf1324)
  * Eklentinin indirilmesi/kurulumu başarısız olursa kurulumu yeniden denemek için eylem eklendi. (#17090, @hwf1324)
  * Eklenti listeleri, yayın tarihi de dahil olmak üzere sütunlara göre artan ve azalan sırayla sıralanabiliyor. (#15277, #16681, @nvdaes)
* Konuşma:
  * Microsoft Speech API sürüm 5 (SAPI5) ve Microsoft Speech Platform sesleri kullanılırken otomatik dil değiştirme destekleniyor. (#17146, @gexgd0419)
  * Microsoft Speech API sürüm 5 (SAPI5) ve 6 kata kadar hızı destekleyen Microsoft Speech Platform sesleri kullanılırken hız artırma destekleniyor. (#17606, @gexgd0419)
* Web tarayıcılar:
  * NVDA tarama kipinin yerel seçim kipi (`NVDA+SHIFT+f10`) Google Chrome, Edge ve Chromium 134 veya üzeri sürüm tabanlı diğer uygulamalarda destekleniyor. (#17838)
  * Mozilla Firefox'ta, NVDA bir metin parçası içeren web adresi ziyaret edildiğinde vurgulanan metni bildirecek. (# 16910, @jcsteh)
  * NVDA bir link aynı sayfada bir konuma yönlendirdiğinde bunu seslendirebilecek. (#141, @LeonarddeR, @nvdaes)
* Microsoft Office:
  * PowerPoint metin kutularında düzenleme yaparken, `alt+yukarı ok` /`alt+ aşağı ok` tuşları ile cümleler arasında dolaşabileceksiniz. (#17015, @LeonarddeR)
  * Word'de, seçimi genişletmek veya daraltmak için Word komutları kullanılırken (`f8` veya `şift+ f8`) seçim güncellemesi bildirilecek . (#3293, @CyrilleB79)
  * Word 16.0.18226 ve üzeri sürümlerde veya Word nesne modeli kullanılırken, NVDA bir başlık daraltıldığında konuşma ve braille ile bildirecek. (#17499)
  * UIA ile Word'de, yerel geliştirilmiş tablo dolaşım komutları (`alt+baş`, `alt+son`, `alt+ önceki sayfa` ve `alt+ sonraki sayfa`) kullanıldığında, imleç hareketi seslendirilecek. (#17867, @CyrilleB79)
  * Word'de "odağı seslendirme" komutu kullanıldığında, varsa ve nesne açıklamalarını bildirme etkinleştirildiyse belge düzeni okunacak. (# 15088, @nvdaes)
  * Word ve Outlook'ta daha fazla kısayolun sonucu bildiriliyor:
    * yazı tipi biçimlendirme kısayolları (#10271, @CyrilleB79)
    * başlığın daraltılması veya genişletilmesi (#17545, @CyrilleB79)collapse or expand heading (#17545, @CyrilleB79)
* LibreOffice:
  * İlgili klavye kısayolları kullanılarak Writer'daki yazı tipi boyutu artırılıp azaltıldığında, NVDA yeni yazı tipi boyutunu bildirecek. (#6915, @michaelweghorn)
  * Writer 25.2 veya üzeri sürümlerde, ilgili klavye kısayolu kullanılarak "Gövde Metni" veya başlık paragraf stili uygulandığında NVDA yeni paragraf stilini seslendirecek. (#6915, @michaelweghorn)
  * İlgili klavye kısayolu kullanılarak Writer'da çift altı çizgili kipi değiştirildiğinde, NVDA yeni durumu (" çift altı çizgili açık "/" çift altı çizgili kapalı ") seslendirecek. (#6915, @michaelweghorn)
  * NVDA LibreOffice 25.8 ve üzeri sürümlerde ilk satır girintisini bildirebiliyor. (#13052, @michaelweghorn)
* OCR:
  * Tanıma sonucu penceresinde, tanınan içeriği `NVDA+f5` ile elle yenileyebilirsiniz. (#17715, @CyrilleB79)
  * Windows OCR sonucunu düzenli olarak yenileme özelliğini açıp kapatmak için kısayol tuşu atanmamış bir hareket eklendi. (#16897)
* NVDA uzaktan destek eklentisine dayanan uzaktan erişim işlevi NVDA'ya entegre edildi. (#4390, @ctoth, @ tspivey, @ daiverd, NVDA uzaktan destek eklentisine katkıda bulunanlar ve maddi destekte bulunanlar)
* NVDA güncellemeleri ve Eklenti Mağazası için kullanılacak kopya web adreslerini belirlemek artık mümkün. (# 14974, # 17151, #17310, @christopherpross)
* PDF'lerde matematik desteği eklendi. (#9288, @NSoiffer)
  * TeX/LaTeX'in daha yeni sürümleriyle oluşturulan bazı dosyalar gibi ilişkilendirilmiş MathML formülleri desteklenmektedir.
  * Şu anda yalnızca Foxit Reader ve Foxit Editor'da desteklenmektedir.
* NVDA Braille dolaşım tuşlarıyla gezinirken mevcut satırı veya paragrafı sesli okumak üzere ayarlanabiliyor. (#17053, @nvdaes)
* NVDA örneğin Visual Studio Code'da `alt+yukarı ok` veya `alt+aşağı ok` hareketleri kullanılırken imleç değişikliklerini bildirebiliyor. (#17652, @LeonarddeR)
* İnceleme imlecini seçilen metnin ilk ve son karakterine taşımak için komutlar eklendi. Sırasıyla `NVDA+alt+baş` ve `NVDA+alt+son` tuşlarına atandı. (# 17299, @nvdaes)
* Tümünü oku veya braille ile okuma sırasında ekranın kapanmasını önlemek için genel bir ayar eklendi.
Bu seçenek varsayılan olarak etkinleştirildi ancak pilin daha hızlı tükenmesine neden olabilir. (#17649, @LeonarddeR)
* Boşnakça dil desteği eklendi. (#17953)
* Adobe Acrobat'ta, NVDA Microsoft Word'ün yeni sürümlerinde oluşturulan pdf belggelerindeki matematik denklemlerini okuyup etkileşime geçebiliyor. (#18056)

### Değişiklikler

* Bileşen güncellemeleri:
  * LibLouis Braille translator [3.33.0](https://github.com/liblouis/liblouis/releases/tag/v3.33.0) sürümüne güncellendi. (#17469, # 17768, @LeonarddeR, @codeofdusk)
    * Yeni Kıpti, sıkıştırılmış Çivi Yazısı ve Portekizce 6 noktalı Bilgisayar Braille tabloları eklendi.
  * CLDR 46.0 sürümüne güncellendi. (#17484, @OzancanKaratas)
  * ESpeak NG 1.52.0 sürümüne güncellendi. (#17056)
* NVDA arayüz değişiklikleri:
  * NVDA kurulumu başlatıldığında ses çalmayacak. (#14068)
  * Linkin yönlendirdiği adresi bildirme, Karakter biçimlendirme bilgileri ve Konuşma seçimi iletişim kutularında kullanıcı kolaylığı için "Kapat" ve "Kopyala" düğmeleri bulunuyor. (#17018, @XLTechie)
  * Çıkış iletişim kutusunda NVDA'yı aynı anda eklentiler devre dışı bırakılmış ve tamir günlüğü etkinleştirilmiş olarak yeniden başlatabileceksiniz. (#11538, @CyrilleB79)
  * Ses çıkışı için WASAPI'yi iptal etme özelliği kaldırıldı. (#16080)
  * "Sistem odağı otomatik olarak odaklanabilir öğelere taşınsın " tarama kipi ayarı kaldırıldı, özellik devre dışı bırakıldı. (#17598)
  * NVDA yalnızca mevcut sürümle uyumsuz bir eklenti API'ına sahip yeni bir sürüme güncelleme yaparken eklenti uyumsuzluğu konusunda uyaracak. (#17071 ,#17506)
  * "Katkıda bulunanlar" dosyası NVDA menüsünden kaldırıldı. (#16922)
  * Okunabilirliği artırmak için NVDA lisansı HTML olarak biçimlendirildi. (#17600)
  * En sık kullanılan komut satırı seçeneklerinin kısa sürümleri eklendi: `-- disable - addons` için `- d` ve `-- lang` için `- n`.
  Komut satırı bayraklarında önek eşleşmesi, örneğin `--disable-addons` için `-- di` kullanımı desteklenmemektedir. (#11644, @CyrilleB79)
  * Com Kayıt tamir Aracındaki Değişiklikler: (#12355, @XLTechie)
    * Araç, uyarı yerine amacının daha kullanıcı dostu açıklamasıyla başlıyor. (#12351)
    * İlk pencereden `çıkış` tuşu veya `alt+f4` tuşlarıyla çıkılabiliyor. (#10799)
    * com kayıtlarını yeniden tamir etmeyi denerken nadiren bir Windows hatası olması durumunda, hata da dahil olmak üzere kullanıcıya bir mesaj gösterilecek.
* Konuşma:
  * Microsoft Speech API version 5 ve Microsoft Speech Platform sesleri ses çıkışı için WASAPI kullanacak. Bu durum seslerin yanıt hızını artırabilir. (#13284, @gexgd0419)
  * Yanıt hızlarını artırmak için OneCore sesleri, SAPI5 sesleri ve bazı üçüncü taraf ses eklentileri kullanılırken konuşmanın başındaki sessizlik kesilecek. (#17614, @gexgd0419)
  * Microsoft Speech API version 4 sesleri ses çıkışı için WASAPI kullanacak. Bu sayede ses zayıflaması, konuşmanın başındaki sessizliğin kesilmesi ve ses cihazını uyanık tutma gibi özellikler desteklenebilecek.
  Bu, kullandığınız SAPI 4 ses ile çalışmazsa Gelişmiş ayarlardan SAPI 4 için WASAPI'yi devre dışı bırakabilirsiniz. (#17718, #17801, @gexgd0419)
  * Unicode Normalleştirmesi konuşma için varsayılan olarak etkinleştirildi. (#17017, @ LeonarddeR).
    * Bu işlevi, NVDA Ayarları iletişim kutusunun Konuşma kategorisinden yine de devre dışı bırakabilirsiniz.
* "yazarken karakterleri seslendir" ve "yazarken sözcükleri seslendir" klavye ayarlarının üç seçeneği var: Kapalı, Yalnızca yazım alanlarında ve Her zaman. (#17505, @Cary-rowen)
  * Varsayılan olarak, "yazarken karakterleri seslendir" seçeneği "Yalnızca yazım alanlarında" olarak ayarlandı.
* Varsayılan braille girdi ve çıktı tabloları NVDA diline göre ayarlanabiliyor. (# 17306, # 16390, #290, @nvdaes)

### Hata düzeltmeleri

* Konuşma:
  * Bazı SAPI5 sentezleyicileri kullanılırken ilk cümlenin sonunda sürekli okumanın (tümünü okurken) durması sorunu giderildi. (#16691, @gexgd0419)
  * SAPI5 ve SAPI4 sentezleyici sesleri arasında geçiş yapmak için sentezleyici ayar zinciri kullanıldığında,  hız ve ses seviyesi gibi ses ayarları varsayılana sıfırlanmayacak. (# 17693, #2320, @ gexgd0419)
  * Bazı SAPI4 seslerinin (örneğin IBM TTS Çince) yüklenememesi sorunu giderildi. (#17726, @gexgd0419)
* Braille:
  * Dot Inc tarafından üretilen Dot Pad dokunsal grafik cihazı için çoklu satırlı braille cihazı olarak yerleşik destek eklendi.
       (#17007)
  * Seika Not alma cihazı kullanılırken, noktalı boşluk ve boşluk hareketleri Giriş Hareketleri iletişim kutusunda doğru şekilde görüntülenecek. (#17047, @school510587)
  * NVDA, braille ekran bağlı değilken ekran konuşma çıkışı braille kipi kullanıldığında hata seslerini çalmayacak veya günlük dosyasına gereksiz veri yazmayacaktır. (#17092, @ Emil-18)
  * Standart HID Braille Ekran sürücüsü, Braille ekran sürücüsü olarak özellikle seçildiğinde ve Braille ekran listesi açıldığında, NVDA HID sürücüsünü seçilen sürücü yok olarak göstermek yerine seçilen sürücü olarak doğru bir şekilde gösterecek. (# 17537, @LeonarddeR)
  * Humanware Brailliant sürücüsü artık doğru bağlantı uç noktasını seçmede daha güvenilir. Sonuç olarak bağlantı daha kararlı olacak ve daha az hata ile karşılaşılacak. (# 17537, @LeonarddeR)
  * Geliştirici scratchpad dizinindeki özel braille tabloları, eklentiler devre dışı bırakıldığında düzgün bir şekilde yok sayılacak. (# 17565, @LeonarddeR)
  * bazı USB Braille ekranların NVDA tarafından algılanmamasına neden olan bir sorun düzeltildi. (#18114, @christiancomaschi)
* Microsoft Office:
  * "Linkin yönlendirdiği web adresini oku" komutu Word, Outlook, Excel ve PowerPoint'te eski nesne modeli kullanılırken beklendiği gibi çalışacak. (#17292, #17362, #17435, @CyrilleB79)
  * Excel'de, öge listesi iletişim kutusunda (`NVDA+f7`) İngilizce olmayan sistemlerde yorum veya formüllerin listelenememesi sorunu giderildi. (#11366, @CyrilleB79)
  * PowerPoint iyileştirmeleri:
    * Metin, emoji gibi geniş karakterler içerdiğinde imleç seslendirmesi bozulmayacak. (#17006 , @LeonarddeR)
    * Karakter konumu seslendirmesi,  (örneğin `NVDA+ sil` tuşlarına basıldığında) doğru biçimde yapılıyor. (#9941, @LeonarddeR)
    * NVDA slayt gösterisi başladığında ve "Sayfa yüklendiğinde otomatik olarak hepsini okumaya başla  " tarama kipi ayarı devre dışıysa, sayfanın tümünü otomatik olarak okumayacak. (#17488, @LeonarddeR)
* LibreOffice:
  * Çince için Microsoft Pinyin Giriş Yöntemi kullanıldığında ve önceki sürümü kullanmak için Pinyin uyumluluk seçeneği etkinleştirildiğinde, bir IME açılır penceresi görüntülenirken Writer'da (ve potansiyel olarak diğer uygulamalarda) yazı yazmak hata tetiklemeyecek. (#17198, @ michaelweghorn)
  * İletişim kutularındaki onay kutularının mevcut durumu (işaretli/işaretli değil) sadece konuşmayla değil, braille olarak da gösterilecek. (#17218, @michaelweghorn)
* Matematik:
  * Bazı web öğeleri için matematik okuması düzeltildi.
  Özellikle, span içindeki MathML ve `role=" math "` özelliği olan diğer öğeler. (#15058)
  * Zengin dolaşım için Yalnızca MathML içermeyen bir görsel ve alternatif metinle temsil edilen matematik denklemleri, içeriksiz matematik yerine normal grafikler gibi ele alınacak. Kullanıcı bunlara `g` tuşuyla atlayabilecek ve alternatif metinde ok tuşlarıyla karakter dolaşımı yapabilecek. (#16007)
* IDE'ler:
  * Android Studio veya IntelliJ Idea'daki belirli kaynak dosyalardaki tüm metinler seçilirken NVDA kilitlenmeyecek. (# 17418, @thgcode)
  * Visual Studio Code'da NVDA cümle dolaşımı için `alt+yukarı ok` ve `alt+aşağı ok` hareketlerini ele geçirmeyecek. (#17082, @LeonarddeR)
  * Visual Studio Code'da belirli bölüm öğeleri düzenlenebilir kontroller olarak doğru şekilde tanınacak. (# 17573, @Cary-rowen)
  * Not Defteri ve diğer UIA belgelerinde ve Windows 11 'deki Notepad++ belgelerinde, son satır boşsa "braille sonraki satır komutu" imleci son satıra taşıyacak.
  Herhangi bir belgede imleç son satırdaysa, bu komut kullanıldığında satırın sonuna taşınacak. (# 17251, #17430, @ nvdaes)
* Konfigürasyon profilleri:
  * Braille, ilişkilendirilen bir konfigürasyon profili ile "tümünü oku" etkinleştirildiğinde de çalışacak. (#17163, @LeonarddeR)
  * Bazı ayarların değeri temel konfigürasyondaki değere eşit olsa bile ayarların etkin yapılandırma profiline özellikle kaydedilmesi sorunu giderildi. (#17157, @ leonarddeR)
* Sözcükleri harf harf okurken, unicode normalleştirme artık daha doğru şekilde çalışıyor:
  * Normalleştirilmiş bir karakteri okuduktan sonra NVDA sonraki karakterleri yanlış bir şekilde "normal" olarak okumayacak. (#17286, @LeonarddeR)
  * Kompozit karakterler (é gibi) doğru şekilde okunacak. (#17295, @LeonarddeR)
* NVDA, Thunderbird arama sonuçları sayfasındaki açılır alt menü öğelerini okuyabilecek. (#4708, @thgcode)
* Com Kayıt tamir Aracı başarısız olduğunda başarı bildirmiyor. (#12355, @XLTechie)
* Öğeler mevcutken pencere kapatıldığında NVDA Windows 11 pano geçmişi girişlerini duyurmayacak. (#17308, @josephsl)
* Göz atılabilir bir mesaj açılırken pluginler yeniden yüklenirse, NVDA ardından gelen odak hareketlerini de bildirebilecek. (#17323, @CyrilleB79)
* Sesli iletişim için Skype, Discord, Signal ve telefon bağlantısı gibi uygulamalar kullanılırken NVDA konuşması ve seslerin ses seviyesi düşmeyecek. (#17349, @jcsteh)
* Anlık görüntü değişkenleri alınırken bir hata oluşması durumunda NVDA Python Konsolunun açılması başarısız olmayacak. (# 17391, @CyrilleB79)
* Web tarayıcılarda, düzenlenebilir metin kontrollerinde metin seçimindeki değişiklikler bazen bildirilemiyordu. Bu sorun düzeltildi. (#17501, @ jcsteh)
* Anchor linkleri sanal imlecin yerleştirildiği aynı nesneye yönlendirdiğinde , NVDA bağlantı hedefine kaydırma yapamıyordu. Bu sorun düzeltildi. (#17669, @nvdaes)
* Explorer yeniden başlatıldığında NVDA Vurgulayıcı Penceresi simgesi görev çubuğunda sabit durmuyor. (#17696, @hwf1324)
...
Bu sürümde Microsoft Office, braille ve belge formatlarına yönelik birkaç iyileştirme yapıldı.

Word ve Excel'de, yorum hareketi iki kez yapıldığında yorum veya not pencerede okunabiliyor.
PowerPoint'te metin seçmek için inceleme imleci seçim komutlarını kullanabiliyorsunuz.
Ayrıca Word'de nesne modeli kullanılırken tablo satır veya sütunlarının başlıkları braille ile gösterildiğinde, NVDA gereksiz karakterler göstermiyor.

NVDA yazı tipi özniteliklerini seslendirirken veya braille ile gösterirken farklı şekilde davranmak üzere ayarlanabiliyor.

Saati veya tarihi seslendirme gibi tuşların peşpeşe basıldığı komutların gerektirdiği zaman aşımı süresini ayarlamak için yeni ayar eklendi.

NVDA'nın metin özniteliklerini braille olarak nasıl göstereceğini ayarlayabiliyor ve NVDA'yı paragraf başlarını braille olarak göstermek üzere ayarlayabiliyorsunuz.
Braille ekran tuşlarıyla imleç yönlendirildiğinde NVDA imlecin altındaki harfi seslendirebiliyor.
Braille ekran tuşlarıyla imleç yönlendirilmesinin güvenilirliği arttırıldı, PowerPoint'te braille ekran tuşlarıyla imleç yönlendirme için destek eklendi.
HID braille aracılığıyla çok satırlı bir braille ekran kullanıldığında, tüm satırların tüm hücreleri kullanılacak.
Otomatik bluetooth braille taraması sırasında yeniden başlatıldığında, NVDA kararsız davranmayacak.

NVDA'nın desteklediği asgari Poedit sürümü 3.5 oldu.

ESpeak NG güncellendi, Faroece ve Xextan dilleri için destek eklendi.

LibLouis güncellendi, Tay dili ve tek hücreli aksanlı harf destekli uluslararası Yunanca braille tabloları eklendi.

Ayrıca Firefox'ta fare izleme ve isteğe bağlı konuşma modu dahil olmak üzere birkaç düzeltme yapıldı.

### Yeni özellikler

* Yeni braille özellikleri:
  * NVDA'nın bazı metin biçimi özniteliklerini braille olarak gösterme şeklini değiştirmek artık mümkün.
    Mevcut seçenekler:
    * Liblouis (varsayılan): Seçilen braille tablosunda tanımlanan biçimlendirme işaretleri kullanılır.
    * Etiketler: Bazı yazı tipi özniteliklerinin başlayıp bittiğini göstermek için başlangıç ve bitiş etiketleri kullanılır. (#16864)
  * "Paragraf bazında oku" seçeneği etkinleştirildiğinde, NVDA paragraf başlangıçlarını braille olarak göstermek üzere ayarlanabiliyor. (#16895, @nvdaes)
  * İmleç braille ekran tuşlarıyla yönlendirildiğinde NVDA imlecin altındaki karakteri otomatik olarak okuyabiliyor. (#8072, @LeonarddeR)
    * Bu seçenek varsayılan olarak devre dışıdır.
      "İmleç braille ekran tuşlarıyla yönlendirildiğinde imleç altındaki karakteri seslendir" seçeneğini NVDA'nın braille ayarları iletişim kutusundan etkinleştirebilirsiniz.
* Microsoft Word'ün yorum komutuna ve Microsoft Excel'in notlar komutuna, içeriğin pencerede gösterilmesi için iki kez basılabiliyor. (#16800, #16878, @Cary-Rowen)
* NVDA yazı tipi özniteliklerini seslendirirken veya braille ile gösterirken farklı şekilde davranmak üzere ayarlanabiliyor. (#16755)
* Birden fazla kez peşpeşe basılması gereken komutlar için zaman aşımı süresi ayarlanabiliyor. Bu özellikle kas engeli olanlar için kullanışlı olabilir. (#11929, @CyrilleB79)

### Değişiklikler

* NVDA açıkken bir güncelleme kurulmaya başlandığında da `-c`/`--config-path` ve `--disable-addons` komut satırı seçenekleri etkili olacak. (#16937)
* Bileşen güncellemeleri:
  * LibLouis Braille translator [3.31.0](https://github.com/liblouis/liblouis/releases/tag/v3.31.0) sürümüne güncellendi. (#17080, @LeonarddeR, @codeofdusk)
    * İspanyolca braillede rakamların gösterimi düzeltildi.
    * Yeni braille tabloları:
      * Tay dili derece1
      * Yunanca uluslararası braille (1 hücreli aksanlı harfler)
    * Adı değiştirilen tablolar:
      * "Tay dili 6 noktalı" tablosunun adı, tutarlı olması için "Tay dili derece 0" olarak değiştirildi.
      * Mevcut "Yunanca uluslararası braille" tablosunun adı, iki Yunanca sistem arasındaki farkın anlaşılması için "Yunanca uluslararası braille (2 hücreli aksanlı harfler)" olarak değiştirildi.
  * ESpeak NG 1.52-dev commit `961454ff` sürümüne güncellendi. (#16775)
    * Faroece ve Xextan dilleri eklendi.
* Standart HID braille sürücüsüyle çok satırlı braille ekranlar kullanıldığında tüm satırlardaki hücreler kullanılacak. (#16993, @alexmoon)
* NVDA'nın Poedit desteğinin kararlılığı geliştirildi. Bundan dolayı NVDA asgari olarak Poedit'in 3.5 sürümünü destekleyecek. (#16889, @LeonarddeR)

### Hata düzeltmeleri

* Braille düzeltmeleri:
  * Microsoft PowerPoint'te metin imlecini hareket ettirmek için braille ekran imleç yönlendirme tuşlarını kullanmak mümkün. (#9101)
  * UI otomasyonu olmadan Microsoft Word kullanıldığında, NVDA satır ve sütun başlıklarını ayarlama komutlarıyla tanımlanan tablo başlıklarını braille ile gösterirken gereksiz karakterler göstermiyor. (#7212)
  * Seika Notetaker sürücüsü boşluk, geri tuşu ve boşluk / geri tuşu ile noktaları doğru şekilde braille olarak gösteriyor. (#16642, @school510587)
  * İmleç yönlendirmesi bir satırda bir veya birden fazla unicode varyasyon seçicisi veya bozuk karakter içerdiği durumda çok daha güvenilir bir şekilde çalışıyor. (#10960, @mltony, @LeonarddeR)
  * Bazı boş yazı alanlarında braille ekran ileri kaydırıldığında NVDA hata vermiyor. (#12885)
  * NVDA otomatik braille bluetooth taraması sırasında yeniden başlatıldığında kararsız davranmıyor. (#16933)
* Microsoft PowerPoint'te metin seçmek için inceleme imleci seçim komutları kullanılabiliyor. (#17004)
* NVDA, isteğe bağlı konuşma modunda Outlook'ta ileti açıldığında, herhangi bir tarayıcıda yeni sayfa yüklendiğinde veya PowerPoint slayt gösterisinde yeni slayt görüntülendiğinde konuşmuyor. (#16825, @CyrilleB79)
* Mozilla Firefox'ta fare linkin öncesindeki veya sonrasındaki metne taşındığında metin daha doğru bir şekilde okunuyor. (#15990, @jcsteh)
* NVDA nadiren mesajları pencerede göstermekte başarısız olmuyor. Mesajların pencerede gösterilmesi durumuna `NVDA+f` tuşuna iki kez basmak örnek verilebilir. (#16806, @LeonarddeR)
* Eklenti güncellemeleri beklemedeyken NVDA'nın güncellenmesi eklentilerin kaldırılmasına neden olmuyor. (#16837)
* Microsoft Excel 365'te veri doğrulaması açılır listeleriyle etkileşimde bulunulabiliyor. (#15138)
* VS Code'da büyük dosyalar arasında dolaşırken NVDA eskisi kadar yavaş tepki vermiyor. (#17039)
* NVDA özellikle Microsoft Word ve Microsoft Outlook'ta tarama kipi etkinken herhangi bir ok tuşu uzun süre basılı tutulduğunda yanıtsız kalmıyor. (#17039)
* NVDA, Java uygulamalarının çok satırlı yazı alanlarında imleç sondan ikinci satırdayken son satırı okumuyor. (#17027)
...
Bu sürüm, otomatik eklenti güncelleme bildirimiyle ilgili bir hatayı düzeltmek için yayınlanan bir yama sürümüdür.

### Hata düzeltmeleri

* Eklenti güncellemeleri otomatik olarak denetlenirken, NVDA kötü bağlantılarda donmuyor. (#17036)
...
* Eklenti mağazası:
  ...
  * Eklenti kurulumundan sonra NVDA'nın yeniden başlatılana kadar konuşmamasına neden olan bir hata düzeltildi. (#14525)
...
Braille için yeni özellikler ve komutlar eklendi. Yeni braille ekranlar için destek eklendi.
...
* Deneysel geliştirilmiş ses yönetimi:
  ...
  * WASAPI kullanımı gelişmiş ayarlardan etkinleştirilebilir.
  Buna ek olarak, WASAPI etkinleştirilmişse aşağıdaki gelişmiş ayarlar da değiştirilebilir:
...
Lütfen güvenlik sorunlarını <info@nvaccess.org> adresine bildirin.

### Güvenlik güncellemeleri

* Addressed security advisory `GHSA-xc5m-v23f-pgr7`.
  * Artık Noktalama imla sözlüğü güvenli modda kullanılamaz.
...
* Addressed security advisory `GHSA-354r-wr4v-cx28`. (#13488)
  * Artık NVDA Güvenli modda çalışırken hata ayıklama günlükleriyle yeniden başlatılamaz.
  * Artık NVDA güvenli modda çalışırken güncelenemez.
* Addressed security advisory `GHSA-wg65-7r23-h6p9`. (#13489)
  * Artık Girdi Hareketleri iletişim kutusu güvenli modda açılamaz.
  * Artık Varsayılan, Geçici ve Ses sözlük iletişim kutuları güvenli modda açılamaz.
* Addressed security advisory `GHSA-mvc8-5rv9-w3hx`. (#13487)
  * Artık wx GUI inspection aracı güvenli modda devredışıdır.
...
* Güvenlik düzeltmesi: Windows 10 ve Windows 11'de kilit ekranının dışında nesne incelemesinin kullanılması engellendi. (#13328)
* Güvenlik düzeltmesi: Eklenti yöneticisi iletişim kutusu artık güvenli ekranlarda kullanılamaz. (#13059)
* Güvenlik düzeltmesi: NVDA yardımı artık güvenli ekranlarda kullanılamaz. (#13353)

## 2021.3.1

Bu sürüm, 2021.3 sürümünde görülen birkaç küçük hatayı düzeltir.

### Değişiklikler

* Yeni HID Braille protokolü, başka bir braille ekran sürücüsü kullanılabilir  ise birincil olarak kullanılmaz. (#13153)
* Yeni HID Braille protokolü, ayarlar iletişim kutusunun gelişmiş bölümünden devre dışı bırakılabilir. (#13180)

### Hata Düzeltmeleri

* Sınır imleri önceden olduğu gibi braillede kısaltıldı. #13158
* Humanware Brailliant ve APH Mantis Q40 braille ekranlarını bluetoothta kullanırken oluşabilecek kararsız otomatik ekran algılama sorunu düzeltildi. (#13153)

## 2021.3

Bu sürüm, yeni HID Braille spesifikasyonu için destek sunar.
Bu spesifikasyon, ayrı sürücülere ihtiyaç duymadan Braille Ekranları için desteği standartlaştırmayı amaçlamaktadır.
Yeni Rusça ve Tshivenda tabloları da dahil olmak üzere LibLouis ve eSpeak-NG için güncellemeler içerilmiştir.
Kararlı NVDA sürümlerinde hata sesleri, ayarlar iletişim kutusunda gelişmiş panelinde bulunan yeni bir seçenek aracılığıyla etkinleştirilebilir.
Word'de tümünü oku komutu bulunulan konumu göstermek amacıyla görünümü de kaydırıyor.
Office'i UIA ile kullanırken birçok iyileştirme yapıldı.
UIA düzeltmelerinden biri, Outlook'un artık iletilerde daha fazla düzen tablosu türünü yok saymasını sağlıyor.

Önemli notlar:

Güvenlik sertifikamızda yapılan bir güncelleme nedeniyle, NVDA 2021.2 güncellemeleri kontrol ettiğinde az sayıda kullanıcı hata alabiliyor.
NVDA şimdi Windows'tan güvenlik sertifikalarını güncellemesini istiyor, bu da gelecekte bu hatayı önleyecektir.
Etkilenen kullanıcıların bu güncellemeyi manuel olarak indirmeleri gerekecektir.

### Yeni özellikler

* Hücre kenarlıklarının stilinin bildirim ayarını değiştirmek için bir girdi hareketi eklendi. (#10408)
* Braille Ekranları için desteği standartlaştırmayı amaçlayan yeni HID Braille spesifikasyonu desteği. (#12523)
  * Bu özelliği destekleyen cihazlar, NVDA tarafından otomatik olarak algılanacaktır.
  * NVDA'nın bu spesifikasyonla ilgili uygulamasına ilişkin teknik ayrıntılar için bkz. https://github.com/nvaccess/nvda/blob/master/devDocs/hidBrailleTechnicalNotes.md
* VisioBraille Vario 4 Braille ekranı için destek eklendi. (#12607)
* NVDA'nın herhangi bir sürümünü kullanırken hata bildirimleri etkinleştirilebilir (gelişmiş ayarlar). (#12672)
* Windows 10 ve sonraki sürümlerde NVDA, Ayarlar ve Microsoft Store gibi uygulamalarda arama terimleri girildiğinde öneri sayısını bildirir. (#7330, #12758, #12790)
* Tablo navigasyonu artık PowerShell'de Out-GridView cmdlet'i kullanılarak oluşturulan ızgara denetimlerinde destekleniyor. (#12928)

### Değişiklikler

* Espeak-ng 1.51-dev commit `74068b91bcd578bd7030a7a6cde2085114b79b44` sürümüne güncellendi. (#12665)
* NVDA'nın tercih ettiği dili destekleyen yüklü OneCore sesleri yoksa, NVDA varsayılan olarak eSpeak'e geçer. (#10451)
* OneCore sesleri sürekli olarak problem çıkarıyorsa, sentezleyici olarak eSpeak'e dönülür. (#11544)
* Durum çubuğunu `NVDA+end` ile okurken, inceleme imleci artık bu konuma taşınmaz.
Bu işlevselliğe ihtiyacınız varsa, lütfen Girdi Hareketleri iletişim kutusundaki Nesne dolaşımı kategorisindeki ilgili komut için bir hareket atayın. (#8600)
* Zaten açık olan bir ayarlar iletişim kutusunu açarken, NVDA, hata vermek yerine odağı mevcut iletişim kutusuna taşıyor. (#5383)
* liblouis braille çevirmen [3.19.0](https://github.com/liblouis/liblouis/releases/tag/v3.19.0) sürümüne güncellendi. (#12810)
  * Yeni braille tabloları: Rusça derece 1, Tshivenda derece 1, Tshivenda derece 2
* "işaretli içerik" veya "işrl" yerine artık "vurgulanan" veya "vrgl" konuşma ve braille için kullanılacak. (#12892)
* NVDA artık diyaloglar gerekli bir eylemi beklerken (örneğin Onayla/İptal) diyalogu kapatmaya kalkmayacaktır. (#12984)

### Hata düzeltmeleri

* Kontrol, insert gibi klavye değiştirici tuşların takibi watchdog kurtarılırken daha kararlı. (#12609)
* Belirli sistemlerde NVDA güncellemelerini kontrol etmek tekrar mümkün; Örneğin. temiz Windows yüklemeleri. (#12729)
* NVDA, UI otomasyonunu kullanırken Microsoft Word'de boş tablo hücrelerini doğru şekilde bildiriyor. (#11043)
* Web'deki ARIA veri ızgara hücrelerinde, Escape tuşu artık ızgaraya iletilecek ve artık odak modunu kasıtlı olarak kapatmayacak. (#12413)
* Chrome'da bir tablonun başlık hücresini okurken, sütun adının iki kez bildirimi düzeltildi. (#10840)
* NVDA, değerlerinin metinsel bir temsiline sahip olan UIA kaydırıcıları için artık sayısal bir değer bildirmemektedir. (UIA ValuePattern artık RangeValuePattern yerine tercih ediliyor). (#12724)
* NVDA artık UIA kaydırıcılarının değerini her zaman yüzdeye dayalı olarak ele almıyor.
* UI Otomasyonu aracılığıyla erişildiğinde Microsoft Excel'de bir hücrenin konumunun bildirimi, Windows 11'de tekrar düzgün çalışıyor. (#12782)
* NVDA artık geçersiz Python yerel ayarları belirlememektedir. (#12753)
* Devre dışı bırakılan bir eklenti kaldırılır ve ardından yeniden yüklenirse yeniden etkinleştiriliyor. (#12792)
* Eklenti klasörünün yeniden adlandırıldığı veya dosyaların açıldığı eklentilerin güncellenmesi ve kaldırılmasıyla ilgili hatalar düzeltildi. (#12792, #12629)
* Microsoft Excel elektronik tablo kontrollerine erişmek için UI Otomasyonu kullanıldığında, NVDA artık tek bir hücre seçildiğinde artık gereksiz bir şekilde bildirim yapmıyor. (#12530)
* LibreOffice Writer'da, örneğin onay diyaloglarında olduğu gibi, daha fazla diyalog metni otomatik olarak okunuyor. (#11687)
* UI otomasyonu aracılığıyla Microsoft Word'de tarama kipinde okuma/gezinme artık belgenin her zaman kaydırılmasını, böylece geçerli tarama kipi konumunun görünür olmasını ve odak kipindeki düzeltme iminin konumunun tarama kipi konumunu doğru bir şekilde yansıtmasını sağlıyor. (#9611)
* UI otomasyonu aracılığıyla Microsoft Word'de Tümünü oku komutu verildiğinde, belge artık otomatik olarak kaydırılıyor ve düzeltme iminin konumu doğru şekilde güncelleniyor. (#9611)
* Outlook'ta e-postaları okurken ve NVDA, UI Otomasyonu ile mesaja erişirken, belirli tablolar artık düzen tabloları olarak işaretleniyor, bu da artık varsayılan olarak bildirilmeyecekleri anlamına geliyor. (#11430)
* Ses cihazlarını değiştirirken nadir görülen bir hata düzeltildi. (#12620)
* Edebi braille tablolarıyla giriş, düzenleme alanlarında daha güvenilir çalışmalıdır. (#12667)
* Windows sistem tepsisi takviminde gezinirken, NVDA artık haftanın gününü tam olarak bildiriyor. (#12757)
* Tayvan - Microsoft Word'de Microsoft Quick gibi bir Çince giriş yöntemi kullanırken, braille ekranını ileri ve geri kaydırmak artık yanlış bir şekilde orijinal düzeltme imi konumuna geri dönmeye devam etmiyor. (#12855)
* UIA üzerinden Microsoft Word belgelerine erişirken, yine cümle ile gezinme (alt+aşağı ok / alt+yukarı ok) mümkün. (#9254)
* UIA ile MS Word'e erişirken, artık paragraf girintisi bildiriliyor. (#12899)
* UIA ile MS Word'e erişirken, değişiklik izleme komutu ve diğer bazı yerelleştirilmiş komutlar artık Word'de bildiriliyor. (#12904)
* 'Açıklama' 'içerik' veya 'ad' ile eşleştiğinde yinelenen braille ve konuşma düzeltildi. (#12888)
* UIA etkinleştirilmiş MS Word'de, yazarken yazım hatası seslerinin daha doğru çalınması söz konusu. (#12161)
* Windows 11'de NVDA, programlar arasında geçiş yapmak için Alt+Tab tuşlarına basıldığında artık "pano" demeyecektir. (#12648)
* Yeni Modern Yorumlar yan izleme bölmesi, belgeye UIA aracılığıyla erişilmediğinde artık MS Word'de desteklenmektedir. Yan izleme bölmesi ile belge arasında hareket etmek için alt+f12 tuşlarına basın. (#12982)
...
Bu sürüm, ön Windows 11 desteği sunar.
Windows 11 henüz piyasaya sürülmemiş olsa da, bu sürüm Windows 11'in önizleme sürümlerinde test edilmiştir.
Bu, Ekran Perdesi için önemli bir düzeltme de içeriyor, (önemli notlara bakın).
COM Kayıt Düzeltme Aracı artık NVDA çalışırken daha fazla sorunu çözebiliyor.
Sentezleyici eSpeak ve braille çevirmeni LibLouis için güncellemeler var.
Özellikle braille desteği ve Windows terminalleri, hesap makinesi, emoji paneli ve pano geçmişi için çeşitli hata düzeltmeleri ve iyileştirmeler de var.
...
Buna Windows 10 Insider ve Windows 11 dahildir.
...
* ARIA bilgi notları için deneysel destek::
  * aria-detayları ile bir nesnenin ayrıntılarının özetinin okunması için bir komut ekler. (#12364)
  * Gelişmiş ayarlar kategorisine nesnenin detaylarının bildirimi için bir ayar ekler. (#12439)
* Windows 10 Sürüm 1909 ve sonraki sürümlerde (Windows 11 dahil), NVDA, Dosya Gezgini'nde arama yaparken öneri sayısını duyurur. (#10341, #12628)
* Microsoft Word'de, NVDA artık basıldığında girinti ve asılı girinti kısayollarının sonucunu bildirir. (#6269)

### Değişiklikler

* Espeak-ng güncellendi, 1.51-dev commit `ab11439b18238b7a08b965d1d5a6ef31cbb05cbb`. (#12449, #12202, #12280, #12568)
* Belge formatları kategorisinde kullanıcı tercihlerinde makaleler etkinleştirilmişse, NVDA içerikten sonra "makale" der. (#11103)
* liblouis braille çevirmeni güncellendi, [3.18.0](https://github.com/liblouis/liblouis/releases/tag/v3.18.0). (#12526)
  * Yeni braille tabloları: Bulgarca 1. derece, Birmanca 1. derece, Birmanca 2. derece, Kazakça 1. derece, Kmerce 1. derece, Kuzey Kürtçe 1. derece, Sepedi derece 1, Sepedi derece 2, Sesotho derece 1, Sesotho derece 2, Setsvana derece 1, Setsvana 2. derece, Tatar derece 1, Vietnam derece 0, Vietnam derece 2, Güney Vietnam derece 1, Xhosa derece 1, Xhosa derece 2, Yakut derece 1, Zulu derece 1, Zulu derece 2
* Windows 10 OCR, Windows OCR olarak yeniden adlandırıldı. (#12690)

### Hata düzeltmeleri

* Windows 10 Hesap Makinesi'nde NVDA, braille ekranında hesap makinesi ifadelerini bildirir.(#12268)
* Windows 10 sürüm 1607 ve sonraki sürümlerdeki terminal programlarında, bir satırın ortasına karakter eklerken veya silerken, düzenleme imlecinin sağındaki karakterler artık okunmaz. (#3200)
  * Diff Match Patch artık varsayılan olarak etkin. (#12485)
* Braille girişi, aşağıdaki kısaltmalı tablolarla düzgün çalışır: Arapça, İspanyolca, Urduca, Çince (Çin, Mandarin). (#12541)
* COM Kayıt Düzeltme Aracı artık özellikle 64 bit Windows'ta daha fazla sorunu çözmektedir. (#12560)
* Nippon Telesoft'tan Seika Notetaker braille cihazı için düğme kullanımında iyileştirmeler. (#12598)
* Windows emoji paneli ve pano geçmişi bildirimlerinde iyileştirmeler. (#11485)
* Bengal alfabesi karakter açıklamaları güncellendi. (#12502)
* Yeni bir NVDA işlemi başlatıldığında NVDA güvenli bir şekilde sonlandırılır. (#12605)
* Braille Ekranını Seç iletişim kutusundan Handy Tech braille ekran sürücüsünün yeniden seçilmesi artık hatalara neden olmaz. (#12618)
* Windows sürüm 10.0.22000 veya üstü, Windows 10 olarak değil, Windows 11 olarak tanınır. (#12626)
* Ekran perdesi desteği, 10.0.22000'e kadar olan Windows sürümleri için düzeltildi ve sınandı. (#12684)
* Girdi hareketlerini filtrelerken hiçbir sonuç gösterilmiyorsa, girdi hareketi yapılandırma iletişim kutusu beklendiği gibi çalışmaya devam eder. (#12673)
* Bazı bağlamlarda bir alt menünün ilk menü öğesinin duyurulmamasına  neden olan bir hata düzeltildi. (#12624)
...
* Chromium tabanlı tarayıcılar için (Edge gibi) UIA için erken destek. (#12025)
* Microsoft Excel için   isteğe bağlı UI Otomasyonu  deneysel desteği. Yalnızca Microsoft Excel derlemesi 16.0.13522.10000 veya üzeri için önerilir. (#12210)
* NVDA Python Konsolunda çıktı üzerinde  daha kolay navigasyon. (#9784)
  * alt + yukarı / aşağı ok, önceki / sonraki çıktı sonucuna gider (seçim için shift tuşunu ekleyin).
  * control + l çıktı panosunu temizler.
* NVDA artık, mevcutsa Microsoft Outlook'ta bir randevuya atanan kategorileri bildiriyor. (#11598)
* Nippon Telesoft'tan Seika Notetaker braille ekranı için destek. (#11514)

### Değişiklikler

* Tarama kipinde, kontroller artık tanımlayıcıları üzerine braille imleci yönlendirmesiyle etkinleştirilebilir (yani, bir bağlantı için "bln" üzerine braille imleci ile gelinip etkinleştirilebilir). Bu, özellikle örneğin etiketsiz onay kutularının etkinleştirilmesi için kullanışlı olabilir. (#7447)
* NVDA artık ekran perdesi etkinse kullanıcının Windows 10 OCR gerçekleştirmesini engeller. (#11911)
* Unicode Ortak Yerel Veri Deposu (CLDR) 38.1'e güncellendi. (#11943)
* Sembol sözlüğüne daha fazla matematiksel sembol eklendi. Bunlar Türkçe için çoktan eklenmişti. (#11467)
* Kullanıcı rehberi, ne yeni dosyası ve çabuk komut listesi artık yenilenmiş bir görünüme sahip. (#12027)
* Microsoft Word gibi desteklemeyen uygulamalarda ekran düzenini değiştirmeye çalışırken artık "Desteklenmiyor" deniyor. (#7297)
* Gelişmiş ayarlar kategorisindeki 'Süresi dolan odak olayları için konuşmayı iptal etmeyi dene' seçeneği artık varsayılan olarak etkindir. (#10885)
  * Bu davranış, bu seçenek "Hayır" olarak ayarlanarak devre dışı bırakılabilir.
  * Web uygulamaları (Gmail gibi) artık odağı hızla hareket ettirirken güncel olmayan bilgiler söylenmez.
* liblouis braille çevirmen  [3.17.0](https://github.com/liblouis/liblouis/releases/tag/v3.17.0) sürümüne güncellendi. (#12137)
...
* Outlook 2010'daki ileti listesi bir kez daha okunabilir hale getirildi. (#12241)
* Windows 10 sürüm 1607 ve sonraki sürümlerdeki terminal programlarında, bir satırın ortasına karakter eklerken veya silerken, imlecin sağındaki karakterler artık okunmaz. (#3200)
  * Bu deneysel düzeltme NVDA'nın gelişmiş ayarlar panelinde   diff algorithm  Diff Match Patch olarak ayarlanarak manuel olarak etkinleştirilmelidir.
* MS Outlook'ta, mesaj gövdesinden konu alanına geçiş için shift + tab basıldığında yanlış mesafe bildirimi artık gerçekleşmemelidir. (#10254)
* Python Konsolunda, boş olmayan bir giriş satırının başına girinti için bir sekme eklemek ve bir giriş satırının ortasında sekme tamamlama gerçekleştirmek artık desteklenmektedir. (#11532)
* Biçimlendirme bilgileri ve diğer taranabilir mesajlar, ekran düzeni kapatıldığında artık beklenmeyen boş satırlar oluşturmaz. (#12004)
* Artık UIA etkinleştirildiğinde MS Word'deki açıklamaları okumak mümkündür. (#9285)
* Visual Studio ile etkileşimde performans iyileştirildi. (#12171)
* NVDA'yı sağdan sola düzen ile kullanırken eksik öğeler gibi grafiksel hatalar düzeltildi. (#8859)
* GUI düzen yönü, sistem yerel ayarına değil, NVDA diline göre belirleniyor. (#638)
  * sağdan sola diller için bilinen sorun: etiketlere / kontrollere sahip gruplama kliplerinin sağ kenarlığı. (#12181)
* Python yerel ayarı, tercihlerde seçilen dile tutarlı bir şekilde uyacak şekilde ayarlanır ve varsayılan dil kullanılırken gerçekleşir. (#12214)
* TextInfo.getTextInChunks, NVDA günlük görüntüleyici gibi Zengin Düzenleme kontrollerinde çağrıldığında artık donmaz. (#11613)
* NVDA'yı Windows 10 1803 ve 1809'da de_CH gibi yerel ayar adında alt çizgi içeren dillerde kullanmak bir kez daha mümkündür. (#12250)
* WordPad'de, üst simge / alt simge bildirimi ayarı beklendiği gibi çalışır. (#12262)
* NVDA, eski odak kaybolursa ve aynı konumdaki yeni odakla değiştirilirse, bir web safyfasındaki yeni odaklanmış içeriği bildirmekte artık başarısız olmaz. (#12147)
* Karşılık gelen seçenek etkinleştirilirse, tüm Excel hücreleri için üstü çizili, üst simge ve alt simge biçimlendirmesi artık rapor edilmektedir. (#12264)
...
* Yeni Chromium tabanlı Microsoft Edge'de adres çubuğu gibi metin alanları artık boşken de duyuruluyor. (#12474)
* Seika Braille sürücüsü düzeltildi. (#10787)
...
Bu sürüm, yeni Çince Giriş yöntemlerini, Liblouis güncellemesini ve artık odak modunda da çalışan ögeler listesi (NVDA + f7) gibi yenilikler içeriyor.
Yanı sıra, bağlama duyarli yardim artık NVDA iletişim kutularında F1'e basıldığında kullanılabiliyor.
Sembol telaffuz kuralları, konuşma sözlüğü, braille mesajları ve tümünü okurken atlayarak okumada iyileştirmeler yapıldı.
Mail, Outlook, Teams, Visual Studio, Azure Data Studio, Foobar2000 için hata düzeltmeleri ve iyileştirmeler var.
Web'de Google Dokümanlar için iyileştirmeler ve ARIA için daha fazla destek sunuluyor.
Ayrıca aşağıda okuyacağınız üzere birçok önemli hata düzeltmesi ve iyileştirmeden söz edilebilir.

### Yeni özellikler

* NVDA iletişim kutularında F1'e basmak artık yardım dosyasında en alakalı bölümü açıyor. (#7757)
* Microsoft SQL Server Management Studio ve Visual Studio 2017 ve sonraki sürümlerinde otomatik tamamlama önerileri (IntelliSense) desteği. (#7504)
* Sembol telaffuzu: Karmaşık bir sembol tanımında gruplama desteği ve bir değiştirme kuralında grup referanslarını destekleyerek onları daha basit ve daha güçlü hale getiriyor. (#11107)
* Artık geçersiz düzenli ifade ((regular expression) ikamelerine sahip konuşma Sözlüğü girişleri oluşturmaya çalışırken kullanıcılar uyarılıyor. (#11407)
  * Özellikle gruplama hataları tespit ediliyor.
* Windows 10'da yeni Çince Geleneksel Hizli ve Pinyin giriş yöntemleri için destek eklendi. (#11562)
* Sekme başlıkları artık f tuşu ile hizli gezinilebilen form alanları olarak kabul ediliyor. (#10432)
* Işaretli (vurgulanan) metnin raporlamasını değiştirmek için bir komut eklendi; Varsayılan olarak ilişkilendirilmiş bir hareket yok. (#11807)
* NVDA'yi sessizce kurarken, sağlanan yapılandirmayı kullanıcı hesabına otomatik olarak kopyalamanıza izin veren --copy-portable-config komut satırı parametresi eklendi. (#9676)
* Braille yönlendirme artık fare kullanıcıları için Braille Görüntüleyici ile destekleniyor, fareyle bir braille hücresine yönlendirebilirsiniz. (#11804)
* NVDA artık Humanware Brailliant BI 40X ve 20X cihazlarını hem USB hem de Bluetooth araciligiyla otomatik olarak algılayacaktır. (#11819)

### değişiklikler

* Liblouis braille çevirmeni 3.16.1 sürümüne güncellendi:
...
* Tarama kipinde tümünü oku  ile okurken, sonrakini bul ve öncekini bul komutları, okumayi durdurmaz, tümünü okurken atlamaya izin ver seçeneği işaretlenmişse artık    okuma sonraki veya önceki bulunan terimden sonra devam eder. (#11563)
* HIMS braille ekranları için F3, Boşluk + nokta 148 olarak yeniden değiştirildi. (#11710)
* "Braille mesaj zaman asimi" ve "Mesajları süresiz göster" seçeneklerinin kullanıcı deneyimi iyileştirmeleri. (#11602)
* Web tarayıcıları ve tarama kipini destekleyen diğer uygulamalar için Öge Listesi iletişim kutusu (NVDA + F7) artık odak kipindeyken de kullanılabilir. (#10453)
* ARIA canli bölgelerinde yapılan güncellemeler artık dinamik içerik değişikliklerinin raporlanmasi devre dışı bırakıldığında engelleniyor. (#9077)
* NVDA artık kopyalanan metinden önce "Panoya kopyalandi" ifadesini söylüyor. (#6757)
* Disk yönetiminde grafiksel görünüm tablosunun sunumu iyileştirildi. (#10048)
* Kontrol devre dışı bırakıldığında, kontrol etiketleri de artık devre dışı bırakılır (grileşir). (#11809)
* CLDR emoji notu sürüm 38'e güncellendi. (#11817)
* Dahili "Odak Vurgulamasi" özelligi "görsel vurgu" olarak yeniden adlandırıldı. (#11700)

### Hata düzeltmeleri

* NVDA, Hizli Günlük girişi uygulamasini kullanirken düzenleme alanlarıyla artık doğru sekilde çalışıyor. (#8996)
* Toplam süre yoksa (ör. Canli yayin oynatirken) Foobar2000'de geçen süreyi  doğru olarak bildiriyor. (#11337)
* NVDA artık web sayfalarındaki düzenlenebilir içerikteki ögelerde aria-roledescription özelligini kapsiyor. (#11607)
* "liste" artık Google Dokümanlar'daki bir listenin her satirinda veya Google Chrome'daki diger düzenlenebilir içerikte duyurulmuyor. (#7562)
* Web'deki düzenlenebilir içerikte bir liste ögesinden digerine karakter veya kelime bazinda navigasyon yaparken, yeni liste ögesine gelindiği artık bildiriliyor. (#11569)
* NVDA artık, düzenleme imleci Google Dokümanlar'daki bir liste ögesinin veya web üzerindeki diger düzenlenebilir içerigin sonundaki bir bağlantının sonuna yerleştirildiğinde doğru satırı okuyor. (#11606)
* Windows 7'de, masaüstünden başlat menüsünü açip kapatmak artık odagi doğru sekilde ayarliyor. (#10567)
* "Süresi dolan odak olaylarını iptal etme girişimi" etkinleştirildiğinde, sekme başlığı artık Firefox'ta sekmeler arasinda geçiş yapılırken tekrar duyuruluyor. (#11397)
* NVDA, SAPI5 Ivona sesleriyle konuşurken bir listeye bir karakter yazdiktan sonra bir liste ögesini artık okuyabiliyor. (#11651)
* Windows 10 Mail 16005.13110 ve sonraki sürümlerde e-postaları okurken tarama kipini kullanmak yine mümkün. (#11439)
* Harposoftware.com'dan SAPI5 Ivona seslerini kullanirken, NVDA artık konfigürasyonu kaydedebiliyor, sentezleyicileri değiştirebiliyor ve yeniden başlatıldıktan sonra artık sessiz kalmiyor. (#11650)
* artık HIMS ekranlarındaki bir braille klavyeden bilgisayar braille'ine 6 rakamini girmek mümkün. (#11710)
* Azure Data Studio'da önemli performans iyileştirmeleri. (#11533, #11715)
* "Süresi dolan odak olayları için konuşmayi iptal etme girişimi" etkinleştirildiğinde, NVDA Bul iletişim kutusunun başlığı  okunabiliyor. (#11632)
* NVDA bilgisayar uyandırıldığında odak bir Microsoft Edge belgesindeyse artık donmuyor. (#11576)
* Tarama kipinin tekrar islevsel olmasi için MS Edge'de bir bağlam menüsünü kapattiktan sonra sekmeye basmak veya odagi hareket ettirmek gerekmiyor. (#11202)
* NVDA, Tortoise SVN gibi 64 bitlik bir uygulama içindeki liste görünümlerindeki ögeleri artık okuyabiliyor. (#8175)
* ARIA agaç ızgaraları artık hem Firefox hem de Chrome'da tarama kipinde normal tablolar olarak gösteriliyor. (#9715)
* Geriye doğru arama artık NVDA + shift + F3 araciligiyla 'öncekini bul' ile başlatılabilir. (#11770)
* Bir NVDA komutu, komutun iki yürütmesi arasinda ilgisiz bir tusa basilmasi durumunda artık tekrarlanmış olarak degerlendirilmez. (#11388)
* Internet Explorer'da güçlü ve vurgulu etiketleri NVDA'nin Belge Biçimlendirme ayarlarında Vurgu bildirimi  kapalıysa artık  bildirilmiyor. (#11808)
* Excel'de hücreler arasinda gezinirken az sayida kullanıcının yaşadığı birkaç saniyelik bir donma artık olmuyor. (#11818)
* Microsoft Teams 1.3.00.28xxx gibi sürüm numaralarına sahip derlemelerde, NVDA artık yanlis odaklanmış bir menü nedeniyle sohbetlerde veya Teams kanallarında mesajları okuyabiliyor. (#11821)
* Google Chrome'da ayni anda hem yazim hem de dil bilgisi hatasi olarak isaretlenen metin, NVDA tarafindan uygun sekilde hem yazim hem de dil bilgisi hatasi olarak bildiriliyor. (#11787)
* Outlook'u (Fransizca yerel ayar) kullanirken, 'Tümünü yanitla' (control + shift + R) kisayolu yeniden çalışıyor. (#11196)
* Visual Studio'da, mevcut seçili IntelliSense ögesi hakkinda ek ayrıntılar sağlayan IntelliSense araç ipuçları artık yalnizca bir kez bildiriliyor. (#11611)
* Windows 10 Hesap Makinesi'nde, yazılan karakterlerin  bildirimi devre dışı bırakılmışsa NVDA hesaplamaların ilerlemesini bildirmez. (#9428)
...
* When moving by character in plain text controls (such as Notepad) or browse mode, 32 bit emoji characters consisting of two UTF-16 code points (such as ðŸ¤¦) will now read properly. (#8782)
...
* In the User Guide, when a <!-- KC:setting command is used for a setting which has a common key for all layouts, the key may now be placed after a full-width colon (ï¼š) as well as the regular colon (:). (#5739) -->
...
* If the synthesizer supports it, speech should now pause for parentheses and the en dash (â€“). (#3799)
...
* Mouse tracking and touch exploration in Internet Explorer and other MSHTML controls (including many Windows 8 applications) is now much more accurateÂ  on high DPI displays or when document zoom is changed. (#3494)
...
* NVDA no longer causes certain programs to crash when moving the mouse over their rich edit (TRichEdit) controls. Programs include Jarte 5.1 and BRfÃ¡cil. (#3693, #3603, #3581)
