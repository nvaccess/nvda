...
+ NVDA Kurulumu penceresinde ilk odaklı öğe lisans koşullarıdır.
Dilerseniz Aşağı OK tuşuna basarak lisans koşullarını okuyabilirsiniz.
+ "Kabul ediyorum" onay kutusuna gitmek için ``tab`` tuşuna basın, ardından işaretlemek için ``boşluk çubuğu``na basın.
+ Seçenekler arasında gezinmek için ``tab`` tuşuna basın, ardından istediğiniz seçeneğin üzerinde ``enter`` tuşuna basın.
+
...
+ Kurulum ekranında lisans sözleşmesini kabul etmek için onay kutusunun işaretli olduğundan emin olun.
+ ``Tab`` tuşuna, ardından "NVDA'yı bu bilgisayara kur" düğmesine basın.
+ Sonrasında, Windows oturum açma sırasında NVDA'yı kullanma ve bir masa üstü kısayolu oluşturma seçenekleri sizi karşılayacaktır.
...
+ Sonra devam etmek için ``enter`` tuşuna basın.
+ "Bu uygulamanın PC'nizde değişiklik yapmasına izin vermek istiyor musunuz?" diye soran bir Windows "Kullanıcı Hesabı Denetimi (UAC)" iletişim kutusu görüntülenir.
+ UAC istemini kabul etmek için ``alt+y`` tuşlarına basın.
+ NVDA yüklenirken bir aşama çubuğu görüntülenir.
Bu işlem sırasında NVDA giderek daha yüksek perdeli bir bip sesi çıkarır.
Bu süreç genellikle hızlıca tamamlanır.
+ NVDA kurulumunun başarılı olduğunu bildiren bir iletişim kutusu görüntülenir.
...
+ "NVDA'ya Hoş Geldiniz" iletişim kutusu açılır ve NVDA bir karşılama mesajı okur.
...
+ "Büyük harf kilidini NVDA tuşu olarak kullan" seçeneğine gitmek için ``tab`` tuşuna basın.
...
+ Bu ekranda dolaşmak ve diğer seçenekleri ayarlamak için ``tab`` ve ``Boşluk Çubuğu``' tuşlarını kullanın.
Bu seçenekler NVDA'nın otomatik olarak başlayıp başlamayacağını belirler.
+ Sonrasında bu iletişim kutusunu kapatmak için ``enter`` tuşuna basın.
+
...
+ Tarama kipinden girdi hareketleri iletişim kutusunu açın.
+ Tarama kipi bölümünden "sonraki sekmeye geçer" öğesini bulun.
+ Bulunan hareket için ``y`` tuşunu atayın.
+ Tarama kipi bölümünden "Önceki sekmeye geçer" öğesini bulun.
+ Bulunan hareket için ``shift+y`` tuşunu atayın.
+
...
+ İnceleme imlecini matematiksel içerik üzerine getirin.
...
+ Sonra, aşağıdaki komutu gerçekleştirin:
+
...
%kc:setting
==== Sentezleyici Seç iletişim kutusunu aç ====
...
%kc:setting
==== Braille Ekran Seçimi iletişim kutusunu aç ====
...
%kc:setting
==== Belge Formatları ayarlarını aç ====
...
==== Developer Scratchpad Dizini'nden özel kod yüklemeyi etkinleştir ====
...
==== Geliştirici Scratchpad Dizinini Aç ====
...
+++ Brailliant BI 14 için Tuş atamaları +++ 
%kc:beginInclude
|| Ad | Tuş |
