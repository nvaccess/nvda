...
+ Het gedownloade bestanduitvoeren 
...
+ Tegelijk met het  NVDA dialoogvenster van de Launcher verschijnt de licentieovereenkomst.
Druk op ``PijlOmlaag`` om, desgewenst, de licentieovereenkomst te lezen.
+ Druk op ``tab`` om naar het "Ik ga akkoord" selectievakje te gaan. Druk nu op de ``spatiebalk`` om dit vakje aan te kruisen.
+ Druk ``tab`` om de verschillende opties te verkennen, druk dan ``enter`` bij de gewenste optie.
+
...
+ Zorg dat het selectievakje van het dialoogvenster van de launcher, waarmee u aangeeft akkoord te gaan met de licentieovereenkomst, is aangevinkt.
+ ``Tab`` naar, en activeer de knop "Installeer NVDA op deze computer".
+ Vervolgens komen er opties om NVDA te gebruiken voor aanmelding bij Windows en om een snelkoppeling voor het bureaublad aan te maken.
...
+ Er verschijnt nueen UAC(User Account Control)"-dialoogvenster  met de vraag Wilt u toestaan dat deze app wijzigingen aan uw PC? aanbrengt".
+ Druk op ``alt+`j`` om de UAC-prompt te accepteren.
+ Het verloop van de installatie wordt via een voortgangsbalk weergegeven.
...
+ Er verschijnt een  dialoogvenster  met de mededeling dat de installatie van NVDA geslaagd is.
...
+ U komt nu in het scherm "Welkom bij NVDA" en u krijgt een welkomsboodschap te horen.
...
+ Druk op ``tab`` om naar "Gebruik ``capsLock`` als een NVDA-programmatoets" te gaan.
...
+ Gebruik ``tab`` en ``spatiebalk`` om de verdere opties op dit scherm aan te passen.
Hiermee stelt u in of NVDA automatisch moet opstarten.
+ NVDA is nu actief en door op ``enter`` te drukken zal het dialoogvenster worden gesloten.
+
...
+ Open het dialoogvenster invoerhandelingen  vanuit bladermodus.
+ Zoek  het item "naar volgende tab gaan" in de  sectie Bladermodus.
+ Wijs de toets ``y`` toe voor de gevonden handeling.
+ Zoek  het item "naar vorige tab gaan".
+ Wijs ``shift+y`` toe voor de gevonden handeling.
+
...
+ verplaatst u de cursor naar de tekst waar de aantekening zich bevindt,
+ roept u het Windows contextmenu op met de opties voor de selectie,
+ kiest u "Aantekening bewerken".
+
...
%kc:setting
==== Dialoogvenster synthesizer selecteren openen ====
...
%kc:setting
==== Dialoogvenster brailleleesregel selecteren openen ====
...
%kc:setting
==== Objectweergaveinstellingen openen ====
...
%kc:setting
==== Bladermodusinstellingen openen ====
...
%kc:setting
==== Documentopmaakinstellingen openen ====
...
+ Vergewis je ervan dat je de [nieuwste driver hebt geïnstalleerd via https://www.papenmeier-rehatechnik.de/en/service/downloadcenter/software/articles/software-braille-devices.html].
+ Open Apparaatbeheer in Windows
+ Ga in de lijst naar beneden tot  je "USB Controllers" of "USB apparaten" tegenkomt.
+ Selecteer "Papenmeier Braillex USB Apparaat".
+ Open Eigenschappen en kies het tabblad "Geavanceerd".
