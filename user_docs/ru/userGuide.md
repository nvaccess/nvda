...
|Читать текст в буфере обмена |`NVDA+c` |`NVDA+c` |Читает любой текст в буфере обмена. При двойном нажатии текст читается по символьно. При тройном нажатии читается фонетическое описание каждого символа|
...
В основном это полезно в местах (включая консоли Windows), где нет [системной каретки](#SystemCaret).
...
|Искать вперёд |NVDA+F3 |Искать в документе следующее вхождение текста, который вы искали в прошлый раз|
|Искать назад |NVDA+Shift+F3 |Искать в документе предыдущее вхождение текста, который вы искали в прошлый раз|

<!-- KC:endInclude -->

### Быстрая навигация {#SingleLetterNavigation}

Для быстрой навигации по определённым элементам документа в режиме обзора, NVDA предоставляет набор однобуквенных команд.
...
В этом диалоговом окне имеется также поле редактирование, которое позволяет вам отфильтровывать элементы для облегчения поиска определённого элемента на странице.
...
Для получения дополнительной информации о доступных командах и настройках для чтения и навигации по математическому содержимому обратитесь к документации математического компонента, который вы установили.
...
Если вы случайно зажмёте клавиши-модификаторы, то повторное выполнение этой команды приведёт к отпусканию модификатора.
...
* Сплошным синим цветом подсвечивается совмещение системного фокуса и объекта навигатора (например, когда объект навигатора [следует за системным фокусом](#ReviewCursorFollowFocus)).
...
После завершения распознавания, результат будет представлен в документе в режиме обзора и вы сможете читать полученную информацию курсорными клавишами.
...
В NVDA имеются свои дополнительные возможности в некоторых приложениях для облегчения определённых задач или для получения доступа к функционалу, который иначе был бы недоступен пользователям NVDA.
...
| Название |Комбинация клавиш |Описание|
|---|---|---|
|Установить заголовки столбцов |NVDA+Shift+C |Одиночное нажатие указывает NVDA, что это первая ячейка заголовка в строке, содержащей заголовки столбцов, которые автоматически читаются при перемещении по столбцам под этой строкой. Двойное нажатие сбрасывает настройку.|
|Установить заголовки строк |NVDA+Shift+R |Одиночное нажатие указывает NVDA, что это первая ячейка заголовка в столбце, содержащем заголовки строк, которые автоматически читаются при перемещении по строкам за этим столбцом. Двойное нажатие сбрасывает настройку.|
...
Выбор диаграммы и нажатие Enter или кнопки "Перейти к" фокусирует диаграмму для навигации и её чтения при помощи клавиш-стрелок
...
#### Чтение примечаний (заметок) {#ExcelReportingComments}
...
| Название |Комбинация клавиш |Описание|
|---|---|---|
|Читать примечания для переводчиков |`control+shift+a` |Сообщает о любых примечаниях для переводчиков. При двойном нажатии отображает их в режиме обзора.|
...
Из любого места диалога вы также можете перейти на одну категорию вперёд, нажав `ctrl+tab`, или на одну категорию назад, нажав `shift+ctrl+tab`.
...
* Предупреждение отладки: Будут журналироваться отладочные предупреждения не вызванные серьёзными ошибками при работе программы.
...
При нажатии этой кнопки, ваши текущие сохранённые настройки NVDA будут скопированы в папку системных настроек, и NVDA будет использовать их на экране входа в систему, на экране контроля учётных записей (UAC) и на других [защищённых экранах](#SecureScreens).
Прежде всего, чтобы удостовериться, что все ваши настройки сохранены, нажмите NVDA+Control+C или выберите в главном меню NVDA пункт "Сохранить конфигурацию".
...
Если этот флажок установлен, то он указывает NVDA говорить слово "Большая" перед любой заглавной буквой при чтении символов по отдельности, например, при посимвольном чтении.
...
Отложенное описание символов будет отменено, если в это время будет произнесён другой текст или если вы нажмете клавишу `control`.
...
Режимы, не отмеченные флажком, исключаются.
По умолчанию все режимы включены.

Например, если вам не нужны  режимы "сигналы" и "выключено", нужно снять флажки с этих двух режимов и оставить отмеченными флажки "речь" и "по требованию".
...
Этот комбинированный список позволяет вам выбрать синтезатор речи, который будет использоваться в NVDA для вывода речи.
...
Категория "Брайль" в диалоге настроек NVDA содержит параметры, позволяющие вам настраивать взаимодействие NVDA с брайлевским дисплеем.
Эта категория содержит следующие опции:
...
Следующая опция, на которую вы попадёте в этой категории настроек, это комбинированный список брайлевских таблиц вывода.
В этом комбинированном списке вы найдёте брайлевские таблицы для разных языков, стандарты и степени сокращения брайля.
...
Обратите внимание, что эта опция будет работать только с брайлевским дисплеем, оснащённым 8-клавишной клавиатурой в стиле Perkins, и если такая возможность поддерживается драйвером брайлевского дисплея.
...
В этом случае, когда объект навигатора или просмотровый курсор изменяет свою позицию из-за явных действий пользователя, брайлевский вывод временно привязывается к просмотровому курсору, пока не произойдёт перемещение системного фокуса или каретки.
...
Команды перехода на следующую или предыдущую строку будут перемещать также по абзацам соответственно.
...
Разделение звука позволяет передавать речь NVDA в одном канале (например, левом), а звуки остальных приложений воспроизводить в другом канале (например, правом).
...
При меньших значениях этого параметра звук может обрываться чаще, так как устройство может слишком быстро перейти в режим ожидания, что приведёт к  обрезанию начала следующей произносимой фразы.
...
Перед тем как продолжить (нажав кнопку "Да"), убедитесь, что у вас включён брайлевский или речевой вывод, и вы сможете управлять компьютером без использования экрана.
...
Категория "Клавиатура" диалога настроек NVDA содержит параметры, определяющие поведение NVDA при использовании системной клавиатуры.
Эта категория содержит следующие опции:
...
Флажки в этом списке указывают, какие клавиши на клавиатуре должны использоваться в качестве [клавиш-модификаторов NVDA](#TheNVDAModifierKey). Для выбора доступны следующие варианты:
...
Категория "Мышь" диалога настроек NVDA позволяет включать режим отслеживания указателя мыши, звуковые сигналы при перемещении мыши, а также другие параметры взаимодействия с мышью.
Эта категория содержит следующие опции:
...
Если этот флажок установл	ен, то NVDA будет читать текст, расположенный в данный момент под указателем мыши. Это позволит вам находить нужную информацию физически перемещая мышь, вместо использования объектной навигации.
...
* Воспроизводить звуковые сигналы: Этот параметр предписывает NVDA издавать звуковые сигналы при изменении индикаторов выполнения. Чем выше сигнал по высоте, тем ближе индикатор выполнения к окончанию.
...
Если вы свернёте или переключитесь на другое окно, NVDA будет по прежнему озвучивать изменения индикатора выполнения, так что вы можете заниматься другими делами и при этом отслеживать окно с индикатором выполнения.
...
В этом поле редактирования устанавливается число строк на странице, на которое вы будете перемещаться в режиме обзора при нажатии клавиш PageUp или PageDown.
...
Это может быть проще для понимания при построчной навигации по странице и облегчить взаимодействие с элементами для некоторых пользователей.
...
Пожалуйста, ознакомьтесь со [списками привязки клавиш BRLTTY](http://mielke.cc/brltty/doc/KeyBindings/) для получения информации о сопоставлении команд BRLTTY с элементами управления брайлевских дисплеев.
<!-- KC:beginInclude -->
