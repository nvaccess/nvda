...
В этом выпуске представлен удалённый доступ к NVDA, позволяющий управлять удаленным компьютером под управлением NVDA с другого устройства под управлением NVDA.

В SAPI 4, SAPI 5 и OneCore voices значительно улучшена речь, в частности, скорость отклика.
В SAPI 5 теперь поддерживается повышение скорости и автоматическое переключение языков.
Голоса SAPI 4 теперь поддерживают приглушение звука, подрезку звукового сопровождения и поддержание аудиоустройства в рабочем состоянии.
...
Теперь стало проще вручную обновлять распознавание текста и переключать автоматическое обновление с помощью новых команд.
...
Таблицы Брайля для ввода и вывода по умолчанию теперь можно определять на основе языка NVDA.
...
* Речь:
  * Автоматическое переключение языков теперь поддерживается при использовании Microsoft Speech API версии 5 (SAPI5) и Microsoft Speech Platform voices. (#17146, @gexgd0419)
  * Ускорение теперь поддерживается при использовании Microsoft Speech API версии 5 (SAPI5) и платформы Microsoft Speech Voices, которая поддерживает ускорение до 6 раз. (#17606, @gexgd0419)
* Веб-браузеры:
...
В этой версии исправлены ошибки, возникавшие при работе с дисплеями Брайля и чтении математических выражений в браузерах на движке Chromium.

### Исправления ошибок

* Исправлена ошибка с чтением математических выражений в браузерах Chromium (Chrome, Edge). (#17421, @NSoiffer)
* Устройства Humanware Brailliant BI 40X с встроенным ПО версии 2.4 теперь работают должным образом. (#17518, @bramd)

## 2024.4.1

Это патч, исправляющий ошибку при сохранении словарей речевых символов.

### Исправления

* Исправлена ошибка, из-за которой речевые словари символов не сохранялись и диалоговое окно не закрывалось. (#17344)
...
* Теперь поддерживаются контексты сообщений Gettext. Это позволяет определить несколько переводов для одного сообщения на английском языке в зависимости от контекста. (#1524)
  ...
  * Это поддерживается как для NVDA, так и для дополнений.
  ...
  * Смотрите http://www.gnu.org/software/gettext/manual/html_node/Contexts.html#Contexts для получения дополнительной информации.
...
* NVDA теперь может давать команды синтезаторам речи переключать языки для определённых фрагментов речи.
  * Чтобы поддерживать это, драйверы должны обрабатывать речь.Команда LangChangeCommand в последовательностях, предшествующих SynthDriver.speak().
...
* Драйвер синтезатора речи теперь получает последовательность текста и речевых команд вместо просто текста и индекса.
  * Это позволяет создавать встроенные индексы, изменять параметры и т.д.
  * Драйверы должны реализовывать SynthDriver.speak() вместо SynthDriver.speakText() и SynthDriver.speakCharacter().
  * Старые методы будут использоваться, если SynthDriver.speak() не реализован, но они устарели и будут удалены в следующем выпуске.
* gui.execute() была удалена. Вместо нее следует использовать wx.CallAfter().
* модуль gui.scriptUI был удален.
  * Для диалоговых окон сообщений используйте wx.CallAfter(gui.MessageBox, ...).
  * Для всех остальных диалогов следует использовать реальные wx-диалоги.
...
* В некоторых элементах управления NVDA теперь может определять цвета. автоматическое сообщение цвета включается в диалоге настроек форматирования документа. Также информацию о цвете можно получить, используя команду сообщения форматирования документа (NVDA+f).
  * Изначально это поддерживается стандартными элементами управления редактируемым текстом IAccessible2 (например, в приложениях Mozilla), элементами управления RichEdit (например, в Wordpad) и текстовыми элементами IBM Lotus Symphony.
...
* Любой пользовательский ввод теперь представляется экземпляром класса inputCore.InputGesture. (#601)
  * Каждый источник входных данных создаёт подклассы базового класса InputGesture.
  * Нажатия клавиш на клавиатуре контролируются обработчиком клавиш.Класс KeyboardInputGesture.
  * Нажатия кнопок, колёсика и других элементов управления брайлевского дисплея относятся к подклассам шрифта Брайля.Класс BrailleDisplayGesture. Эти подклассы предоставляются каждым драйвером брайлевского дисплея.
...
* Все скрипты теперь принимают аргументом экземпляр класса InputGesture вместо key press.
  * Нажатия клавиш могут быть отправлены в операционную систему с помощью метода жеста send().
...
* Теперь каждая локаль может предоставить файл сопоставления ввода для изменения сочетаний для конкретного языка. (#810)
  * Языковые карты жестов должны быть размещены в locale\LANG\gestures.ini, где LANG - это код языка.
  * Смотрите inputCore.GlobalGestureMap для получения подробной информации о формате файла.
* Новые классы поведения объектов NVDA (NVDA object behavior) LiveText и Terminal реализуют поддержку автоматического чтения нового текста. См. эти классы в пакете NVDAObjects.behaviors для более подробной информации.
  * Класс наложения NVDAObjects.window.DisplayModelLiveText может использоваться для объектов, которые должны извлекать текст, введённый на дисплее.
  * Примеры использования приведены в модулях приложений mirc и putty.
...
* Объекты класса SynthDriver теперь должны реализовывать свойство language, которое указывает язык текущего голоса.
  * Базовая реализация использует язык, указанный в объектах VoiceInfo в availableVoices. Это подходит для большинства синтезаторов, которые поддерживают один язык для каждого голоса.
* Драйверы брайлевских дисплеев теперь могут поддерживать ввод с дисплея (кнопки, колёсико и пр.):
  * Драйверы могут предоставить глобальные жесты ввода для добавления привязок к сценариям в NVDA.
  * Они также могут предоставлять свои собственные сценарии для выполнения определённых функций дисплея.
  * Для получения более подробной информации обратитесь к braille.BrailleDisplayDriver , для ознакомления с примерами обратитесь к документации существующих драйверов брайлевских дисплеев.
...
* Все драйверы брайлевских дисплеев теперь должны наследоваться от braille.BrailleDisplayDriver, а не от braille.BrailleDisplayDriverWithCursor.
  * Теперь управление курсором осуществляется вне драйвера.
  * Существующим драйверам нужно только соответствующим образом изменить инструкцию class и переименовать свой метод _display в display.
...
* Теперь, при помощи NVDA можно просматривать большую часть текста, отображаемого на экране. (#40, #643)
  * Это позволяет озвучивать элементы управления, которые не предоставляют информацию более прямыми и надежными способами.
  * К элементам управления, доступным благодаря этой функции, относятся: некоторые пункты меню с отображаемыми значками (например, меню "Открыть с помощью" для файлов в Windows XP) (#151), редактируемые текстовые поля в приложениях Windows Live (#200), список ошибок в Outlook Express (#582), редактируемый текстовый элемент управления в TextPad (#605), списки в Eudora, множество элементов управления в Australian E-tax и панель формул в Microsoft Excel.
...
* Теперь озвучиваются заголовки строк и столбцов в таблицах. Эта опция доступна в настройках форматирования документа.
  * В настоящее время это поддерживается в документах приложений Mozilla, таких как Firefox (версии 3.6.11 и более поздних версий) и Thunderbird (версии 3.1.5 и более поздних версий). (#361)
* Введены команды для плоского просмотра: (#58)
  * NVDA+7 на цифровом блоке переключает в режим плоского просмотра, помещая просмотровый курсор в положение текущего объекта, что позволяет просматривать экран (или документ, если он находится в пределах одного объекта) с помощью команд просмотра текста.
  * NVDA+1 на цифровом блоке перемещает просмотровый курсор на объект, представленный текстом, в положение просмотрового курсора, позволяя вам перемещаться по объекту с этой точки.
...
* В NVDA теперь есть встроенный синтезатор eSpeak, разработанный Джонатаном Даддингтоном. Он очень отзывчив и лёгок, и поддерживает множество языков. Синтезаторы Sapi по-прежнему можно использовать, но eSpeak будет использоваться по умолчанию. eSpeak не требует установки специального программного обеспечения, поэтому его можно использовать с NVDA на любом компьютере, на USB-накопителе или где угодно. Для получения дополнительной информации об eSpeak или поиска других версий перейдите по ссылке https://espeak.sourceforge.net/.
  ...
  * Для получения дополнительной информации о eSpeak или поиска других версий перейдите по ссылке http://espeak.sourceforge.net/.
...
* Добавлены аргументы командной строки для NVDA:
  * -m, --minimal: не воспроизводить звуки запуска/завершения работы и не показывайть интерфейс при запуске, если это настроено.
  * -q, --quit: закрыть любой другой, уже запущенный экземпляр NVDA, а затем завершить работу
  * -s, --stderr-file fileName: указать, где NVDA должна размещать неперехваченные ошибки и исключения
  * -d, --debug-file fileName: указать, где NVDA должна размещать сообщения отладки
  * -c, --config-file: указать альтернативный файл конфигурации
  * -h, -help: показать справочное сообщение со списком аргументов командной строки
