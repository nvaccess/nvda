...
* Веб-браузеры:
  * Режим исходного выделения в режиме обзора NVDA (`nvda+shift+f10`) теперь поддерживается в Google Chrome, Edge и других браузерах, основанных на Chromium 134 или новее. (#17838)
  * В Mozilla Firefox NVDA сообщит подсвеченный текст, когда URL содержит посещённый текстовый фрагмент.(#16910, @jcsteh)
  * NVDA теперь может сообщить, когда направление ссылки указывает на текущую страницу. (#141, @LeonarddeR, @nvdaes)
* Microsoft Office:
  * При редактировании текстовых полей в PowerPoint вы можете перемещаться по предложениям с помощью `alt+стрелкаВверх`/`alt+стрелка Вниз`. (#17015, @LeonarddeR)
  * В Word обновление выделения теперь сообщается при использовании команд Word для расширения или уменьшения выделения (`f8` или `shift+f8`). (#3293, @CyrilleB79)
  * В Word 16.0.18226 и выше или при использовании модели объекта Word NVDA теперь сообщит, если заголовок свёртывается как речью, так и по брайлю. (#17499)
  * В Word с UIA, при использовании команд расширенной навигации по таблицы (`alt+home`, `alt+end`, `alt+pageUp` и `alt+pageDown`), теперь будет сообщаться перемещение каретки. (#17867, @CyrilleB79)
  * В Word при использовании команды "Объявлять фокус" будет объявлено, если эта информация будет доступна и включена в объявление описания объектов. (#15088, @nvdaes)
...
* OCR:
  * В результате распознавания `nvda+f5` вручную обновляет распознанное содержимое.(#17715, @CyrilleB79)
  * Добавлен неназначенный жест, чтобы переключать периодическое обновление результата Windows OCR. (#16897)
...
* NVDA теперь может сообщать об изменениях каретки при нажатии на жесты `alt+upArrow` или `alt+downArrow', например, в Visual Studio Code. (#17652, @LeonarddeR)
* Добавлены команды для перемещения просмотрового курсора на первый и последний символы выделенного текста, назначенные на `nvda+alt+home` и на `nvda+alt+end`. (#17299, @nvdaes)
* Добавлен параметр в категории общее, чтобы предотвратить выключение дисплея в режиме непрерывного чтения или во время чтения по брайлю.
...
* Обновления компонентов:
  * Обновлён LibLouis Braille translator до [3.33.0](https://github.com/liblouis/liblouis/releases/tag/v3.33.0). (#17469, #17768, @LeonarddeR, @codeofdusk)
...
* Изменения интерфейса NVDA:
  ...
  * Были добавлены краткие версии наиболее часто используемых параметров командной строки: `-d` для `--disable-addons` и `-n` для `--lang`.
  Соответствие префикса на флагах командной строки, например, Использование `--di` для`-disable-addons 'больше не поддерживается. (#11644, @CyrilleB79)
  * Изменения в Средстве восстановления COM-объектов: (#12355, @XLTechie)
    * Теперь всё начинается с более удобного объяснения его цели, а не предупреждения. (#12351)
    * Из первоначального окна теперь можно выйти спомощью `escape` или `alt+f4`. (#10799)
    * Теперь оно будет показывать сообщение пользователю, включая ошибку, в редком событии ошибки Windows при попытке перерегистрации.
* Речь:
  ...
  * Нормализация юникода теперь включена по умолчанию для вывода речи. (#17017, @LeonarddeR).
    * Вы по -прежнему можете отключить эту функциональность в категории речи диалога настройки NVDA.
* Настройки клавиатуры для проговаривания вводимых символов и проговаривания вводимых слов теперь имеют три варианта: выключено, только в элементах управления редактирования и всегда. (#17505, @Cary-rowen)
  * По умолчанию, "Проговаривать вводимые символы" теперь настроен на "Только в элементах управления редактирования".
* Брайлевские таблицы ввода и вывода по умолчанию теперь могут быть определены на основе языка NVDA. (#17306, #16390, #290, @nvdaes)
...
* Microsoft Office:
  ...
  * В Excel диалог списка элементов (`nvda+f7`) больше не может перечислять комментарии или формулы в некоторых неанглийских системах. (#11366, @CyrilleB79)
  * Улучшения в PowerPoint:
    ...
    * Объявления о положении символа теперь является точным (например, при нажатии `NVDA+delete`. (#9941, @LeonarddeR)
    * NVDA больше не начинает говорить всё при запуске слайд -шоу, а настройка режима обзора "Автоматическое чтение страницы при её загрузке" отключена. (#17488, @LeonarddeR)
* LibreOffice:
...
* Математика:
  * Чтение математики было исправлено для некоторых веб -элементов.
  В частности, MathML внутри span и другие элементы, которые имеют атрибут `role="math"`. (#15058)
  * Математические выражения, представленные только изображением и альтернативным текстом без MathML для расширенной навигации, теперь рассматриваются как обычные изображения, а не как математические уравнения без содержимого, что позволяет пользователю переходить к ним с помощью `g` и перемещаться клавишами-стрелками в альтернативном тексте по символам. (#16007)
* Интегрированные среды разработки:
  ...
  * В Visual Studio Code NVDA больше не использует жесты `alt+стрелка вверх` и `alt+стрелка вниз' для навигации по предложениям. (#17082, @LeonarddeR)
  * Некоторые элементы раздела теперь правильно распознаются как редактируемые элементы управления в Visual Studio Code. (#17573, @Cary-rowen)
...
* При написании, нормализация Unicode теперь работает более уместно:
  ...
  * Составные символы (такие как é) теперь сообщаются правильно. (#17295, @LeonarddeR)
* NVDA может прочитать всплывающие элементы подменю на странице результатов поиска Thunderbird. (#4708, @thgcode)
* Средство восстановления COM-объектов больше не сообщает об успехе при неудачи. (#12355, @XLTechie)
* NVDA больше не будет объявлять записи истории буфера обмена в Windows 11 при закрытии окна, пока присутствуют элементы. (#17308, @josephsl)
* Если плагины перезагружаются при открытом просмотровым сообщением, NVDA больше не будет сообщать о последующих движениях фокуса. (#17323, @CyrilleB79)
* При использовании таких приложений, как Skype, Discord, Signal и Phone Link for audio communication, громкость речи и звуков NVDA больше не уменьшается. (#17349, @jcsteh)
* Открытие консоли Python NVDA больше не будет проходить в случае, если возникнет ошибка при получении переменных снимков. (#17391, @CyrilleB79)
* В веб -браузерах об изменениях в выделении текста иногда не сообщалось в элементах управления редактируемым текстом. (#17501, @jcsteh)
* Когда anchor-ссылки указывают на тот же объект, что и виртуальная каретка, NVDA больше не может прокручивать направление ссылки. (#17669, @nvdaes)
* Значок окна NVDA Highlight больше не зафиксирована на панели задач после перезапуска проводника. (#17696, @hwf1324)
...
* Обновления компонентов:
  ...
  * Обновлён configobj до 5.1.0 commit `8be5462`. (#17328)
  * Обновлён pre-commit до 4.0.1. (#17260)
...
* `ui.browseableMessage` Теперь можно вызвать с параметрами, чтобы предоставить кнопку для копирования в буфер обмена, и/или кнопку для закрытия окна. (#17018, @XLTechie)
* Несколько дополнений для определения типов ссылок (#16994, @LeonarddeR, @nvdaes)
  * Новый модуль `utils.urlUtils` с различными функциями для определения типов ссылок
  * Было добавлено новое состояние `INTERNAL_LINK` в `controlTypes.states.State`
  * Было добавлено новое свойство `linkType` в `NVDAObject`.
  По умолчанию оно запрашивает `treeInterceptor`, если он есть.
  * Объект `BrowseModeTreeInterceptor` имеет новое свойство `documentUrl`
  * Объект `BrowseModeTreeInterceptor` имеет новый метод `getLinkTypeInDocument`, который принимает URL для проверки типа ссылки объекта
  * В `globalCommands` была добавлена вспомогательная функция `toggleBooleanValue`.
...
* Добавлена [конфигурация рабочего пространства VS Code для NVDA](https://nvaccess.org/nvaccess/vscode-nvda) как подмодуль git. (#17003)
* Была добавлена новая функция `gui.guiHelper.wxCallOnMain`, которая позволяет безопасно и синхронно вызывать функции wx из потоков без GUI и получать их возвращаемые значения. (#17304)
* Новое сообщение API диалога было добавлено в `gui.message`. (#13007)
  * Добавлены классы: `ReturnCode`, `EscapeCode`, `DialogType`, `Button`, `DefaultButton`, `DefaultButtonSet`, `MessageDialog`.
* В модуль `brailleTables` была добавлена функция `getDefaultTableForCurrentLang` (#17222, @nvdaes)
* Добавлен класс данных `updateCheck.UpdateInfo`, который собирает метаданные об обновлениях NVDA. (#17310, @christopherpross)
* Получение свойства `labeledBy` теперь работает для:
  * объекты в приложениях, реализующие соотношение `labelled-by' iaccessible2. (#17436, @michaelweghorn)
  * Элементы UIA, поддерживающие соответствующее свойство UIA `LabeledBy`. (#17442, @michaelweghorn)
* Добавлена возможность ассоциировать `wx.ComboBox` и метку `wx.StaticText` с использованием `gui.guiHelper.associateElements`. (#17476)
* Добавлены следующие точки расширения (#17428, @ctoth):
  * `inputCore.decide_handleRawKey`: Вызывается при каждом нажатии клавиши
  * `speech.extensions.post_speechPaused`: вызывается, когда речь приостановлена или возобновлена
* Изменения регистрации автоматического обнаружения брайлевского дисплея в `bdDetect.DriverRegistrar`: (#17521, @LeonarddeR)
  * Добавлен метод `addUsbDevice` для регистрации по одному USB -устройству за раз.
  * Добавлен параметр `matchFunc` в `addUsbDevices`, который также доступен в `addUsbDevice'.
    * Таким образом, обнаружение устройства может быть далее ограничено в тех случаях, когда комбинация VID/PID используется несколькими устройствами по нескольким драйверам или, например, устройство HID предлагает несколько конечных точек.
    * Смотрите Документацию метода, а также примеры драйверов albatross и brailliantB для получения дополнительной информации.
* Добавлена новая функция `utils.mmdevice.getOutputDevices`, чтобы перечислить устройства вывода звука. (#17678)
* Добавлена новая точка расширения `pre_synthSpeak` в `synthDriverHandler`, которая будет вызвана до того, как речевой менеджер вызовет `speak` текущего синтезатора. (#17648)
* NVDA поддерживает Атрибут объекта IAccessible2 `text-indent`. (#13052, @michaelweghorn)
* Когда запускается скрипт с использованием `gesture.send` с жестом, который включает в себя `numLock` в качестве модификатора, Num Lock больше не выключается. (#10827, @CyrilleB79)
...
* Константа `addonStore.network.BASE_URL` была удалена.
...
* Константа `updateCheck.CHECK_URL` была удалена.
Поскольку URL -адрес проверки обновления NVDA теперь настраивается непосредственно в NVDA, замена не планируется. (#17151)
* `NVDAObjects.UIA.winConsoleUIA.WinTerminalUIA` был удален без публичной замены. (#14047, #16820, @codeofdusk)
* `NVDAObjects.IAccessible.ia2TextMozilla.FakeEmbeddingTextInfo` был удалён. (#16768, @jcsteh)
* Следующие символы в `appModules.soffice` были переименованы (#6915, @michaelweghorn):
  * `SymphonyDocument.announceToolbarButtonToggle` в `SymphonyDocument.announceFormattingGestureChange`
  * `SymphonyDocument.script_toggleTextAttribute` в `SymphonyDocument.script_changeTextFormatting`
* Аргумент ключевого слова `space` для `brailleDisplayDrivers.seikantk.InputGesture` теперь ожидает `int`, а не `bool`. (#17047, @school510587)
* Раздел конфигурации `[upgrade]`, включающий `[upgrade][newLaptopKeyboardLayout]` был удалён. (#17191)
* `updateCheck.checkForUpdate` теперь возвращает объект `UpdateInfo` вместо словаря. (#17310, @christopherpross)
* Конструкторы `updateCheck.UpdateResultDialog` и `updateCheck.UpdateDownloader` были обновлены, чтобы взять объекты `UpdateInfo` вместо словарей метаданных. (#17310, @christopherpross)
* Из -за выхода на пенсию поддержки winmm в NVDA (#17496, #17532, #17678):
  * Следующие символы были удалены из `nvwave` без замены: `CALLBACK_EVENT`, `CALLBACK_FUNCTION`, `CALLBACK_NULL`, `HWAVEOUT`, `LPHWAVEOUT`, `LPWAVEFORMATEX`, `LPWAVEHDR`, `MAXPNAMELEN`, `MMSYSERR_NOERROR`, `usingWasapiWavePlayer`, `WAVEHDR`, `WAVEOUTCAPS`, `waveOutProc`, `WAVE_MAPPER`, `WHDR_DONE`, `WinmmWavePlayer`, и `winmm`.
  * Следующие символы были удалены из `nvwave`: `getOutputDeviceNames`, `outputDeviceIDToName`, `outputDeviceNameToID`.
  Вместо этого используйте `utils.mmdevice.getOutputDevices`.
  * `nvwave.WasapiWavePlayer` был переименован в `WavePlayer`.
  Кроме того, сигнатура метода `__init__` была изменена следующим образом:
    * Параметру `outputDevice` теперь должны передаваться только строковые аргументы.
    * Были удалены устаревшие параметры `closeWhenIdle` и `buffered`.
  * `gui.settingsDialogs.AdvancedPanelControls.wasapiComboBox` был удалён.
  * Ключ `WASAPI` был удалён из раздела `audio` в спецификации конфигурации.
  * Ключ конфигурации `config.conf["speech"]["outputDevice"]` был удален.
...
* В `NVDAObjects.window.scintilla.ScintillaTextInfo`, если текст не выделен, метод `collapse` переопределяется, чтобы развернуть в строке, если параметр `end` установлен в `True` (#17431, @nvdaes)
* Следующие символы были удалены без замены: `languageHandler.getLanguageCliArgs`, `__main__.quitGroup` и `__main__.installGroup` . (#17486, @CyrilleB79)
* Соответствие префикса на флагах командной строки, например, Использование `--di` для `--disable-addons` больше не поддерживается. (#11644, @CyrilleB79)
* Аргумент ключевого слова `useAsFallBack` в `bdDetect.DriverRegistrar` был переименован в `useAsFallback`. (#17521, @LeonarddeR)
* Настройка конфигурации `[addonStore][showWarning]` была удалена.
Вместо этого используйте `addonStore.dataManager.addonDataManager.storeSettings.showWarning`. (#17597)
* `ui.browseableMessage` теперь принимает параметр `sanitizeHtmlFunc`.
Это умолчание для `nh3.clean` с аргументами по умолчанию.
...
* `updateCheck.UpdateAskInstallDialog` больше не выполняет автоматическое действие, когда нажимают кнопки обновления или отложения обновления.
Вместо этого было добавлено свойство `callback`, которое возвращает функцию, которая выполняет соответствующее действие при вызове с возвратным значением из диалога. (#17582)
* Диалоги, открытые с помощью `gui.runScriptModalDialog`, теперь в NVDA признаны модальными. (#17582)
* Следующие символы API, связанные с настройкой "Automatically set system focus to focusable elements" были удалены без замены: (#17598)
...
* По этой причине голоса SAPI5 теперь используют `nvwave.WavePlayer` для вывода звука:
  * `synthDrivers.sapi5.SPAudioState` был удалён.
  * `synthDrivers.sapi5.SynthDriver.ttsAudioStream` был удалён.
* Экземпляры `autoSettingsUtils.driverSetting.DriverSetting` с `id`, который начинается с подчёркивания (_), больше не отображаются в настройках NVDA. (#17599)
* Изменена конфигурация эха набора клавиатуры с логического значения на целочисленное. (#17505, @Cary-rowen)
  * `config.conf["keyboard"]["speakTypedCharacters"]` и `config.conf["keyboard"]["speakTypedWords"]` Теперь используют целочисленные значения.
  * Добавлены enum `TypingEcho` в `config.configFlags`, чтобы представлять эти режимы, 0=Выключено, 1=Только в элементах управления редактирования, 2=Всегда.
  * `gui.settingsDialogs.KeyboardSettingsPanel.wordsCheckBox` и `gui.settingsDialogs.KeyboardSettingsPanel.charsCheckBox` были удалены.
* Функция `winUser.paint` была переименована из `painStruct` в `paintStruct`. Это исправляет ошибку, когда передача `PAINTSTRUCT` приводила к возникновению исключения. (#17744)
* `documentationUtils.getDocFilePath` и `installer.getDocFilePath` больше не ищут файлы `.txt` в локальных папках документации. (#17911, @CyrilleB79)
* `config.conf[«documentFormatting»][«reportFontAttributes»]` был удалён, вместо него используйте `config.conf[«documentFormatting»][«fontAttributeReporting»]`. (#18066)
* `config.conf[«speech»][«includeCLDR»]` был удалён, проверьте/измените, содержит ли `config.conf[«speech»][«symbolDictionaries»]` вместо  `«cldr»`. (#18066)

#### Устаревания

* Точка расширения `braille.filter_displaySize` устарела.
Вместо неё используйте `braille.filter_displayDimensions`. (#17011)
* Функции `gui.message.messageBox` и `gui.runScriptModalDialog` и класс `gui.nvdaControls.MessageDialog` устарели.
Вместо этого используйте `gui.message.MessageDialog`. (#17582)
* Следующие символы устарели (#17486, @CyrilleB79):
  * `NoConsoleOptionParser`, `stringToBool`, `stringToLang` в `__main__`; вместо этого, используйте те же символы в `argsParsing`.
  * `__main__.parser`; вместо этого используйте `argsParsing.getParser()`.
* `bdDetect.DeviceType` устарел в пользу `bdDetect.ProtocolType` и `bdDetect.CommunicationType`, чтобы принять во внимание тот факт, что как скрытые, так и серийные общения могут происходить над USB и Bluetooth. (#17537 , @LeonarddeR)
...
* Исправлена ошибка, из-за которой математические атрибуты считывались в Adobe Reader, что приводило к искажению речи и Брайля. (#17980)Fixed math attributes being read in Adobe Reader, this resulted in poor or wrong speech and braille. (#17980)
* Исправления в Windows 11:
...
* Для Microsoft Office 16.0.15000 и выше в Windows 11 NVDA будет использовать автоматизацию пользовательского интерфейса для доступа к документам Microsoft Word по умолчанию, обеспечивая значительное повышение производительности по сравнению со старой объектной моделью доступа. (#13437)
  * Это включает в себя документы в самом Microsoft Word, а также считыватель сообщений и композитор в Microsoft Outlook.
...
* Библиотека брайлевской трансляции Liblouis обновлена до версии 3.16.1:
  ...
  * Добавлены таблицы русского литературного брайля и русского литературного брайля (подробная)
...
* NVDA во всех web-браузерах теперь сообщает наличие выделенного (подсвеченного) содержимого (обёрнутого тегом &lt;mark&gt; или имеющего атрибут role="mark"), как через речевой, так и через брайлевский вывод. (#11436)
  * Это может быть включено и выключено новой опцией форматирования документов NVDA для выделения.
...
* Удаление специфичных для NVDA Python-объектов, найденных циклическим сборщиком мусора, теперь отражается в журнале NVDA, помогая избавляться от появления циклических ссылок в коде NVDA. (#11499)
  * Большинство классов NVDA отслеживаются, включая NVDAObjects, appModules, GlobalPlugins, SynthDrivers, и TreeInterceptors.
  * Класс, который необходимо отслеживать, должен унаследовать от garbageHandler.TrackedObject.
...
* В Internet Explorer, Google Chrome и Mozilla Firefox, теперь возможно перемещаться по статьям и группам с помощью комманд быстрой навигации. Клавиши на соответствующие команды могут быть назначены в диалоге жестов ввода, при открытии его из документа с активным режимом обзора. (#9485, #9227)
  * Илюстрации также сообщаются. Они считаются объектами и, следовательно, им доступна однобуквенная навигация с помощью o.
...
* Добавлена функция затемнения экрана, которая будучи активированной на системах начиная с Windows 8, позволяет сделать экран компьютера полностью чёрным. (#7857)
  * Добавлена команда затемнения экрана (однократный вызов включает временное затемнение экрана до следующей перезагрузки NVDA, а двойной - активирует этот режим на постоянной основе), по умолчанию на эту команду нет назначенной комбинации клавиш.
  * Также может быть включено и настроено в категории 'зрение' диалога настроек NVDA.
* В NVDA добавлена возможность подсветки отдельных элементов экрана. (#971, #9064)
  * Подсветку системного фокуса, объекта навигатора и курсора режима обзора можно настроить в категории зрения диалога настроек NVDA.
  * Примечание: эта функция несовместима с дополнением Focus Highlight, но его использование всё равно возможно при отключении встроенной подсветки.
...
* NVDA больше не зависит от pyWin32, а такие модули как win32api и win32con больше недоступны для дополнений. (#9639)
  ...
  * константы win32con должны быть определены в ваших файлах.
...
* экземпляры класса synthDriverHandler.SynthDriver больше не поддерживают методы speakText и speakCharacter.
  * Эта функциональность обрабатывается SynthDriver.speak.
...
* Следующие функции модуля speech теперь возвращают речевую последовательность. (#10098)
  ...
  * getSpeechTextForProperties теперь вызывается getPropertiesSpeech
...
* Проведена чистка кодовой базы. (#9548)
  * Удалены модули приложений:
    * Windows XP sound recorder.
    * Klango Player, который является заброшенным программным обеспечением.
  * configobj.validate была удалена.
  ...
  * config.getConfigDirs была удалена.
  * config.ConfigManager.getConfigValidationParameter была заменена функцией getConfigValidation
  ...
  * Удалены ряд классов synthDriverHandler.SynthSetting.
    * Сохраняемые ранее для обратной совместимости (#8214), но теперь признанные полностью устаревшими.
    * Драйверы использующие классы SynthSetting должны быть обновлены для использования классов DriverSetting.
  * Некоторый устаревший код был удален, особенно:
    * Поддержка списка сообщений Outlook Pre 2003.
    * Класс наложений для классического меню пуск, существовавшие только в Windows Vista и раньше.
    * Заброшена поддержка Skype 7, так как он точно больше не работает.
* Реализован фреймворк для создания поставщиков средств зрительной коррекции (vision enhancement providers); модулей, которые могут изменять содержимое экрана, при необходимости основываясь на данных о расположении объектов полученных от NVDA. (#9064)
  * Дополнения могут складывать свои собственные поставщики в папке visionEnhancementProviders.
  ...
  * Поставщики зрительной коррекции могут быть активированы и настроены в категории 'зрение' диалога настроек NVDA.
...
* Внешние модули (глобальные плагины и модули приложений) теперь с меньшей вероятностью могут помешать созданию экземпляров NVDAObjects.
  * Исключения, вызванные методами "chooseNVDAObjectOverlayClasses" и "event_NVDAObject_init", теперь правильно отлавливаются и регистрируются.
...
* Профили конфигурации NVDA теперь могут вручную активироваться и деактивироваться назначаемыми пользователем жестами. (#4209)
  * Жест должен быть настроен в диалоге Жесты ввода".
...
* В категорию дополнительных настроек добавлен флажок "Автоматически устанавливать системный фокус на фокусируемые элементы" (NVDA+8), позволяющий отключить перемещение системного фокуса вслед за курсором виртуального буфера режима обзора. (#2039) Это отключение не требуется для большинства web-страниц, но оно может помочь решить следующие проблемы:
  * Эффект резиновой полосы: NVDA спорадически отменяет последний клавиш в режиме просмотра, прыгая в предыдущее место.
...
* Добавлена поддержка автоматического определения подключений для брайлевских дисплеев. (#1271)
  * В настоящее время поддерживаются дисплеи ALVA, Baum/HumanWare/APH/Orbit, Eurobraille, Handy Tech, Hims, SuperBraille и HumanWare BrailleNote и Brailliant BI/B.
...
* Улучшена поддержка брайлевского вывода в Microsoft Word с активированным интерфейсом UI Automation:
  * Подобно другим многострочным текстовым полям, при установки каретки в начале документа, брайлевский дисплей теперь сначало отображает первый символ этого документа. (#8406)
...
* В диалоге выхода из NVDA Теперь возможно перезагружать NVDA или перезагружать NVDA с отключёнными дополнениями. (#4057)
  * NVDA также можно запускать с отключенными дополнениями с помощью параметра командной строки --disable-addons.
...
* Microsoft Excel: обратная связь речи и Брайля для ещё большего количества горячих клавиш перемещения и выделения, (#4211) включая:
  ...
  * Расширенное выделение (вышеуказанные клавиши с добавлением Shift); и
...
* Несколько исправлений в поддержке для доступных приложений Java:
  * Первоначально находящийся в фокусе тип управления в фрейме или диалоге, теперь удаётся объявить, если фрейм или диалог выходит на передний план. (#3753)
...
* Драйверы брайлевских дисплеев теперь поддерживают выбор порта вручную. (#426)
  * Это наиболее полезно для брайлевских дисплеев, которые поддерживают соединение через устаревший последовательный порт.
  * Это делается с использованием метода getPossiblePorts Class в классе Brailledisplaydriver.
* Теперь поддерживается ввод Брайля с брайлевских клавиатур. (#808)
  * Ввод Брайля включается в класс brailleInput.BrailleInputGesture или его подкласс.
  * Подклассы braille.BrailleDisplayGesture (как реализовано в драйверах брайлевских дисплеев) также могут наследовать от brailleInput.BrailleInputGesture. Это позволяет обрабатывать брайлевские команды отображения и ввода.
...
* NVDA теперь может поддерживать ввод азиатских символов с использованием методов ввода IME и текстовых служб во всех приложениях, в том числе:
  * Объявление и навигация списков кандидатов;
  * Объявление и навигация по композитным строкам; и
  * Объявление о строках чтения.
...
* Экспериментальная поддержка сенсорных экранов в Windows 8, включая:
  * Чтение текста прямо под пальцем при перемещении его вокруг
  * Многие жесты для выполнения навигации по объектам, просмотра текста и других команд NVDA.
...
* В Adobe Reader теперь можно сообщать номера страниц. (#2534)
  * В Reader XI сообщается, что метки страниц представлены, где присутствуют, отражающие изменения в нумерации страниц в разных разделах и т. Д. В более ранних версиях это невозможно, и сообщается только о последовательных номерах страниц.
...
* Теперь поддерживаются контексты сообщений Gettext. Это позволяет определить несколько переводов для одного сообщения на английском языке в зависимости от контекста. (#1524)
  * Это делается с использованием функции pgettext(context, message).
  ...
  * Для xgettext передайте аргумент командной строки --keyword=pgettext:1c,2, чтобы включить контексты сообщений.
...
* Теперь в документах Microsoft Word содержится гораздо больше информации, в том числе:
  * Встроенная информация, такая как сноска и конечные номера, уровни заголовка, существование комментариев, уровни вложенности таблицы, ссылки и цвет текста;
  * Объявления при вводе разделов документов, таких как история комментариев, сноски и истории, а также истории заголовка и нижнего колонтитула.
...
* Объекты TextInfo теперь имеют метод active() для активации позиции, представленной TextInfo.
  * Это используется брайлем для активации позиции, используя клавиши маршрутизации курсора на брайлевском дисплее. Однако в будущем могут быть другие вызовы.
...
* Некоторые константы управления и вывода были переименованы или перемещены. (#228)
  * константы speech.REASON_* были перемещены в controlTypes.
