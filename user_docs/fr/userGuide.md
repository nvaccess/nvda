...
<!-- KC:title: Résumé des commandes NVDA NVDA_VERSION -->
...
* Possibilité de [se connecter et contrôler un autre ordinateur exécutant NVDA](#RemoteAccess) pour assistance à distance ou collaboration
...
* Possibilité de s'exécuter lors de la connexion Windows et [autres écrans sécurisés](#SecureScreens)
...
En plus de l'anglais, NVDA a été traduit en 55 langues, y compris : afrikaans, albanais, amharique, arabe, aragonais, bosniaque, bulgare, birman, catalan, chinois (simplifié et traditionnel), croate, tchèque, danois, néerlandais, farsi, finnois, français, galicien, géorgien, allemand (Allemagne et Suisse), grec, hébreu, hindi, hongrois, islandais, irlandais, italien, japonais, kannada, coréen, kirghiz, lituanien, macédonien, mongol, népalais, norvégien, polonais, portugais (Brésil et Portugal), pendjabi, roumain, russe, serbe, slovaque, slovène, espagnol (Colombie et Espagne), suédois, tamoul, thaï, turc, ukrainien et vietnamien.
...
Vous devrez peut-être attendre quelques secondes pendant qu'une copie temporaire de NVDA se charge.
Une fois chargé, NVDA parlera tout au long du processus.
1. La fenêtre de lancement de NVDA apparaît avec un bouton pour ouvrir le contrat de licence.
Appuyez sur la touche `flèche vers le bas` pour lire le contrat de licence.
...
1. Une boîte de dialogue apparaît pour confirmer que l'installation de NVDA a réussi.
...
| Nom |Touche ordinateur de bureau |Touche ordinateur portable |Description|
|---|---|---|---|
|Prononcer les caractères tapés | `NVDA+2` | `NVDA+2` | Contrôle la manière dont NVDA annonce les caractères que vous tapez. |
|Prononcer les mots tapés | `NVDA+3` | `NVDA+3` | Contrôle la manière dont NVDA annonce les mots que vous tapez. |
...
|Revenir en arrière |`maj+lettre` |Appuyez sur `maj` et n'importe laquelle des lettres ci-dessus pour aller à l'élément précédent de ce type|
...
Pour obtenir de l'aide sur NVDA, appuyez sur `NVDA+n` pour ouvrir le menu, puis sur `a` pour obtenir de l'aide.
...
Nous recommandons de commencer par le module "Formation de base pour NVDA".
...
Avant de pouvoir appuyer sur le bouton Continuer, vous devrez utiliser la case à cocher pour confirmer que vous comprenez que ces extensions seront désactivées.
...
Cette option vous permet de choisir si NVDA doit ou non copier la configuration utilisateur de la version actuellement exécutée de NVDA vers la configuration de l'utilisateur actuellement connecté, pour la version installée de NVDA.
...
Par défaut, NVDA est configuré pour utiliser la disposition bureau, mais vous pouvez opter pour la disposition ordinateur portable dans la catégorie Clavier de la boîte de dialogue [Paramètres NVDA](#NVDASettings), que vous trouverez sous Préférences dans le menu NVDA.
...
<!-- KC:beginInclude -->
Pour activer/désactiver le support d'interaction tactile, appuyez sur NVDA+contrôle+alt+t.
<!-- KC:endInclude -->
...
Quand vous avez trouvé la touche sur laquelle vous voulez appuyer, double-tapez la touche ou retirez votre doigt, selon les options définies dans la catégorie [Paramètres d'Interaction Tactile](#TouchInteraction)des Paramètres NVDA.
...
|Aller au début de la sélection dans la révision |`NVDA+alt+début` |`NVDA+alt+début` |Aucun |Déplace le curseur de revue sur le premier caractère du texte sélectionné|
|Aller à la fin de la sélection en cours de révision |`NVDA+alt+fin` |`NVDA+alt+fin` |Aucun |Déplace le curseur de revue jusqu’au dernier caractère du texte sélectionné|
...
<!-- KC:beginInclude -->
Pour activer ou désactiver la navigation rapide pour le document courant, appuyez sur NVDA+maj+espace.
<!-- KC:endInclude -->
...
Pour interagir avec ces objets, vous pouvez appuyer sur Entrée sur eux.
...
Cependant, NVDA dispose d'un mode de sélection native qui peut être activé dans certains documents en mode navigation qui le supportent (Mozilla Firefox et les navigateurs basés sur Chromium 134 ou plus récent), ce qui fait que la sélection native dans le document suit la sélection du mode navigation de NVDA.
...
Quand on utilise le braille abrégé, le texte est inséré quand on appuie sur espace ou entrée à la fin d'un mot.
...
Par exemple, si vous utilisez un code braille dont les nombres doivent commencer par un indicateur numérique et appuyez sur retour arrière pour aller à la fin d'un nombre, vous devrez entrer à nouveau l'indicateur numérique pour entrer des chiffres supplémentaires.
...
Bien que NVDA soit en premier lieu destiné à des personnes aveugles ou malvoyantes qui utilisent principalement la parole ou le braille pour accéder à un ordinateur, il offre aussi des facilités internes pour modifier le contenu de l'écran.
...
Un appui sur `NVDA+f5` actualise le résultat de la reconnaissance.
...
<!-- KC:beginInclude -->
Pour reconnaître le texte dans l'objet navigateur courant en utilisant la reconnaissance optique de caractères de Windows, appuyez sur NVDA+r.
<!-- KC:endInclude -->

## Fonctionnalités Spécifiques à Certaines Applications {#ApplicationSpecificFeatures}

NVDA fournit ses propres fonctionnalités additionnelles dans certaines applications pour rendre certaines tâches plus faciles ou pour donner accès à des fonctionnalités qui ne seraient pas accessibles autrement.
...
| Nom |Touche |Description|
|---|---|---|
|Définir les en-têtes de colonne |`NVDA+maj+c` |Un appui indique à NVDA qu'il s'agit de la première cellule de la ligne qui contient les en-têtes de colonne, qui doivent être annoncées automatiquement lors du déplacement entre les colonnes sous cette ligne. Un double appui efface ce paramétrage.|
|Définir les en-têtes de ligne |`NVDA+maj+r` |Un appui indique à NVDA qu'il s'agit de la première cellule de la ligne qui contient les en-têtes de ligne, qui doivent être annoncées automatiquement lors du déplacement entre les lignes sous cette colonne. Un double appui efface ce paramétrage.|
...
<!-- KC:beginInclude -->
Pour activer ou désactiver le mode navigation sous Microsoft Word, appuyez sur NVDA+espace.
<!-- KC:endInclude -->
...
<!-- KC:beginInclude -->
Pour annoncer tout commentaire à la position actuelle du curseur, appuyez sur `NVDA+alt+c`.
Un double appui affiche l'information en mode navigation.
<!-- KC:endInclude -->
Tous les commentaires pour le document ainsi que les autres suivis de modifications peuvent aussi être listés dans la liste d'éléments de NVDA en choisissant le type Annotations.
...
Cela signifie que d'autres utilisateurs de revues d'écran ouvrant ce classeur ultérieurement auront automatiquement les titres des lignes et colonnes déjà définis.
...
<!-- KC:beginInclude -->
Pour accéder à la liste d'éléments sous Excel, appuyez sur NVDA+f7.
<!-- KC:endInclude -->
...
<!-- KC:beginInclude -->
Pour annoncer toute note pour la cellule actuellement en focus, appuyez sur `NVDA+alt+c`.
Un double appui affiche l'information en mode navigation.
...
<!-- KC:beginInclude -->
Pour ajouter ou éditer une note, dans une cellule ayant le focus, appuyez sur maj+f2.
<!-- KC:endInclude -->
...
NVDA offre un support amélioré pour Poedit 3.5 ou plus récent.
...
<!-- KC:beginInclude -->
Une fois que vous avez sélectionné le texte, appuyez sur la touche applications ou maj+f10 pour afficher les options disponibles pour travailler avec la sélection.
<!-- KC:endInclude -->
...
Si on l'exécute, ce raccourci ouvrira le dialogue Paramètres directement dans cette catégorie particulière.
...
Quand elle est cochée, un dialogue apparaît lorsque vous voulez quitter NVDA, vous proposant les choix suivants :

* quitter
* redémarrer
* redémarrer avec les extensions désactivées et le journal activé en mode débogage
* redémarrer avec le journal activé en mode débogage
* installer la mise à jour en attente (le cas échéant)
...
##### Mettre à jour le miroir {#UpdateMirror}

Ces contrôles vous permettent de spécifier une URL alternative à utiliser pour vérifier les mises à jour de NVDA.
Cela peut être utile dans les endroits où l’accès au serveur de mise à jour NV Access NVDA est lent ou indisponible.

La zone de texte en lecture seule affiche l'URL du miroir actuel.
Si aucun miroir n’est utilisé (c’est-à-dire que le serveur de mise à jour de NV Access est utilisé), "Aucun miroir" s’affiche.

Si vous souhaitez modifier le miroir de mise à jour, appuyez sur le bouton "Modifier..." pour ouvrir la boîte de dialogue [Définir le miroir de mise à jour](#SetURLDialog).

Veuillez noter que lors de l’utilisation d’un miroir de mise à jour, l’opérateur du miroir a accès à toutes les [informations envoyées avec les vérifications de mise à jour](#GeneralSettingsCheckForUpdates).
Contactez l’opérateur du miroir de mise à jour pour obtenir des détails sur ses politiques de traitement des données afin de vous assurer que vous êtes à l’aise avec la manière dont vos informations seront traitées avant de configurer un miroir de mise à jour.

##### Empêcher la désactivation de l'écran pendant dire tout ou pendant la lecture en braille {#PreventDisplayTurningOff}

Cette case à cocher, lorsqu’elle est activée, garantit que l’écran reste allumé lors de la lecture avec Dire Tout ou avec le braille (par exemple lorsque vous appuyez sur les boutons de défilement).
Cela évite la situation où l’écran se verrouille de manière inattendue pendant la lecture en mode Dire Tout.
Cette option est activée par défaut.
Pensez à désactiver cette option si vous constatez une plus courte autonomie de batterie.
...
La catégorie "Parole" dans le dialogue Paramètres de NVDA, contient des options vous permettant de modifier le synthétiseur vocal ainsi que les caractéristiques de la voix pour le synthétiseur choisi.
...
La première option dans la catégorie Paramètres de Parole est le bouton Modifier...
Ce bouton ouvre la boîte de dialogue [Choisir le Synthétiseur](#SelectSynthesizer), qui vous permet de choisir le synthétiseur vocal actif.
...
Si vous utilisez un synthétiseur tiers, vous pourrez peut-être modifier cette valeur aussi, si la voix que vous aurez choisie prend cela en charge.
...
Le dialogue Synthétiseur, qui s'ouvre en activant le bouton Modifier dans la catégorie Parole du dialogue Paramètres, permet de choisir le synthétiseur de parole qui sera utilisé par NVDA.
...
Si vous voulez modifier rapidement les paramètres vocaux sans passer par la catégorie Voix du dialogue Paramètres, il y a quelques touches de commandes qui vous permettent de le faire de n'importe où quand NVDA est en fonction.
<!-- KC:beginInclude -->
...
Dans la catégorie Braille du dialogue Paramètres, le bouton Modifier... active le dialogue [Choisir l'Afficheur Braille](#SelectBrailleDisplay) qui vous permet de choisir l'afficheur braille actif.
Ce dialogue s'ouvre par-dessus le dialogue Paramètres.
...
Si vous sélectionnez "Automatique", la table sera sélectionnée en fonction de la langue actuelle de NVDA.
...
##### Marqueur de début de paragraphe {#BrailleParagraphStartMarkers}

Si l'option "Lire par paragraphe" est cochée, le marqueur de début sélectionné sera affiché pour indiquer le début d'un paragraphe.
Cela peut être particulièrement utile dans les applications utilisées pour lire de longs passages de texte, comme des documents structurés ou des livres.
Dans de tels documents, savoir où commencent les paragraphes peut être utile pour comprendre la structure du contenu ou pour définir des signets ou des annotations en fonction de la position du paragraphe.

Les options comprennent l'utilisation de deux espaces comme saut de paragraphe subtil et le symbole de paragraphe, pied-de-mouche (¶), comme symbole plus évident.

| . {.hideHeaderRow} |.|
|---|---|
|Options |Aucun marqueur de début de paragraphe, Deux espaces (  ), Pied-de-mouche (¶)|
|Aucun marqueur de début de paragraphe (défaut)|
...
##### Affichage de la mise en forme {#BrailleFormattingDisplay}

Ce paramètre détermine comment NVDA affichera la mise en forme du texte en braille.
Cette option n'a d'effet que si NVDA est paramétré pour [afficher les attributs de police en braille](#DocumentFormattingFontAttributes).

| . {.hideHeaderRow} |.|
|---|---|
| Options | Défaut (Liblouis), Liblouis, Balises |
| Défaut | Liblouis |

| Option | Comportement |
|---|---|
| Liblouis | Utilisez la mise en forme braille native. Veuillez noter que cette option n'indiquera que le texte en gras, italique et souligné, et seulement si la table braille sélectionnée prend en charge l'indication de ces attributs. |
| [Balises](#BrailleFormattingDisplayTags) | Utilisez des balises qui décrivent comment et où la mise en forme du texte change. |

###### Balises {#BrailleFormattingDisplayTags}

Lorsque "Affichage de la mise en forme" est réglé sur "Balises", une balise de mise en forme s'affiche en braille lorsqu'un changement de mise en forme est détecté.
Ces balises commencent par ⣋ et se terminent par ⣙.
Une balise de mise en forme contiendra un ou plusieurs symboles qui décrivent le formatage du texte.
Les symboles suivants sont définis :

| Symbole | Signification |
|---|---|
| ⠛ ("g") | Début gras |
| ⡛ ("g" avec le point 7) | Fin gras |
| ⠊ ("i") | Début italique |
| ⡊ ("i" avec le point 7) | Fin italique |
| ⠎ ("s") | Début souligné |
| ⡎ ("s" avec le point 7) | Fin souligné |
| ⠃ ("b")| Début barré |
| ⡃ ("b" with dot 7) | Fin barré |

##### Annoncer le caractère lors du routage du curseur dans le texte {#BrailleSpeakOnRouting}

Si cette option est activée, NVDA annoncera automatiquement le caractère au niveau du curseur lors du routage vers celui-ci avec les touches de routage du curseur braille.

Pour basculer cette option depuis n'importe où, veuillez attribuer un geste personnalisé à l'aide de la [boîte de dialogue Gestes de commandes](#InputGestures).

##### Retour vocal lors de la navigation par ligne ou par paragraphe {#BrailleSpeakOnNavigating}

L’activation de cette option amenera NVDA à annoncer les lignes ou les paragraphes atteints à l’aide des commandes de navigation de l’afficheur Braille.

Pour activer cette option depuis n’importe où, veuillez attribuer un geste personnalisé à "Active ou désactive la lecture à voix haute lors de la navigation par lignes ou paragraphes en braille" dans la section "Braille" de la [boîte de dialogue Gestes de commande](#InputGestures).
...
Le flot de parole peut être une distraction lors de la lecture du braille.
...
Le dialogue Choisir l'Afficheur Braille, qui peut être ouvert en activant le bouton Modifier... dans la catégorie Braille du dialogue Paramètres, vous permet de choisir l'afficheur Braille qui sera utilisé par NVDA.
Quand vous aurez choisi votre afficheur braille, vous pourrez appuyer sur le bouton OK et NVDA chargera l'afficheur sélectionné.
...
* NVDA à gauche et les applications dans les deux canaux : NVDA parlera dans le canal de gauche, tandis que les autres applications diffuseront leur son dans les canaux gauche et droit.
...
Notez que cocher ou décocher la case "Activer la Mise en évidence" modifiera également l'état des trois autres cases à cocher en conséquence.
...
Cette option contrôle la manière dont NVDA annonce les caractères que vous saisissez au clavier.
Les options possibles sont :

* Désactivé : NVDA n’annoncera pas les caractères saisis.
* Seulement dans les contrôles d’édition : NVDA annoncera uniquement les caractères saisis dans les contrôles d’édition et autres zones où du texte peut être saisi.
* Toujours : NVDA annoncera tous les caractères saisis.
...
Cette option contrôle la manière dont NVDA annonce les mots que vous tapez au clavier.
Les options possibles sont :

* Désactivé : NVDA n’annoncera pas les mots saisis.
* Seulement dans les contrôles d’édition : NVDA annoncera uniquement les mots saisis dans les contrôles d’édition et autres zones où du texte peut être saisi.
* Toujours : NVDA annoncera tous les mots saisis.
...
##### Délai d'expiration pour les appuis multiples {#MultiPressTimeout}

Certains gestes de clavier NVDA effectuent différentes actions en fonction du nombre de fois que l'on appuie rapidement sur une même touche.
Un exemple est la commande "Annoncer le caractère courant de l'objet navigateur".
Cette commande annonce le caractère lors du premier appui, une description phonétique du caractère lors du second appui et la valeur numérique du caractère lors du troisième appui.
Cette option configure le délai après lequel un appui supplémentaire sur la même touche lancera un nouveau geste, plutôt que d'être considérée comme un appui supplémentaire pour le premier.
Pour la commande citée en example, avec un délai d'expiration trop court, deux appuis annonceront deux fois le caractère courant, au lieu de la description phonétique.
Le délai d'expiration par défaut est de 500 ms, soit une demi-seconde.
Augmenter ce délai peut être particulièrement utile pour les personnes utilisant les touches rémanentes ou souffrant d'un handicap physique.
...
Quand elle est décochée, vous devez double-taper sur la touche du clavier tactile pour l'appuyer.
...
Cette option permet de choisir si oui ou non NVDA doit annoncer tous les symboles quand ils apparaissent dans la chaîne de composition.</target>
Cette option est activée par défaut.
...
#### Mise en forme des documents {#DocumentFormattingSettings}
...
* Police,
  * Nom de la police
  * Taille de la police,
  * Attributs de la police [(Désactivé, Parole, Braille, Parole et braille)](#DocumentFormattingFontAttributes)
...
* Éléments
  * Titres
  * Liens
  * Type de lien (destination sur la même page)
  * Graphiques
  * Listes
  * Citations
  * Groupes
...
##### Attributs de la police {#DocumentFormattingFontAttributes}

Cette option vous permet de sélectionner la manière dont certains attributs de police, tels que le gras, l'italique, le souligné et le barré, sont signalés.
La liste déroulante des attributs de police propose quatre options :

* Désactivé : NVDA ne signalera pas ces attributs de police.
* Parole : NVDA annoncera quand ces attributs de police changeront.
* Braille : NVDA affichera ces attributs en braille.
La manière exacte dont ils sont affichés peut être configurée dans [Paramètres braille de NVDA] (#BrailleFormattingDisplay).
* Parole et braille : NVDA signalera les attributs de police en utilisant les deux méthodes ci-dessus.
...
##### Mises à jour automatiques {#AutomaticAddonUpdates}
...
Par défaut, les notifications n’apparaîtront que pour les extensions avec des mises à jour disponibles dans le même [canal](#AddonStoreFilterChannel) (par exemple, stable, bêta ou dev).
Vous pouvez configurer les canaux de mise à jour des extensions [individuellement pour chaque extension](#AddonStoreUpdateChannel) ou pour [toutes les extensions](#DefaultAddonUpdateChannel).

Lorsque l’option "Mettre à jour automatiquement" est sélectionnée, les extensions seront automatiquement mises à jour en arrière-plan.
Vous serez invité à redémarrer NVDA une fois les mises à jour terminées.

| . {.hideHeaderRow} |.|
|---|---|
|Options |Notifier (Défaut), Mettre à jour automatiquement, Désactivé |
|Défaut |Notifier |

|Option |Comportement |
|---|---|
|Notifier |Notify Notifier lorsque des mises à jour d'extensions sont disponibles |
|Mettre à jour automatiquement |Mettre à jour automatiquement les extensions en arrière-plan |
|Désactivé |Ne pas vérifier automatiquement les mises à jour des extensions |

##### Canal de mise à jour par défaut {#DefaultAddonUpdateChannel}

Lorsque les [mises à jour automatiques des extensions](#AutomaticAddonUpdates) sont activées, par défaut, les extensions sont mises à jour uniquement vers le même [canal](#AddonStoreFilterChannel).
Par exemple, une version bêta installée ne sera mise à jour que vers une version bêta plus récente.
Cette option définit le canal de mise à jour par défaut pour toutes les extensions.
Vous pouvez également modifier le canal de mise à jour d’une [extension spécifique individuellement à partir de la boutique d’extensions](#AddonStoreUpdateChannel).

| . {.hideHeaderRow} |.|
|---|---|
| Options | Identique (Défaut), Tous, Ne pas mettre à jour, Stable, Beta ou dev, Beta, Dev |
| Défaut | Identique |

| Option | Comportement |
|---|---|
| Identique | Les extensions resteront sur le même canal |
| Tous | Les extensions seront automatiquement mis à jour vers la dernière version, quel que soit le canal |
| Ne pas mettre à jour | Les extensions ne seront pas automatiquement mise à jour par défaut, vous devez les activer individuellement |
| Stable | Les extensions seront automatiquement mises à jour vers des versions stables |
| Beta ou dev | Les extensions seront automatiquement mises à jour vers des versions beta ou dev |
| Beta | Les extensions seront automatiquement mises à jour vers des versions beta |
| Dev | Les extensions seront automatiquement mises à jour vers des versions dev |

##### Permettre aux mises à jour automatique d'installer des extensions incompatibles {#AllowIncompatibleAddonUpdates}

Ce paramètre active les mises à jour automatiques des extensions qui peuvent ne pas être entièrement compatibles avec la version actuelle de NVDA.
Par défaut, cette option est désactivée, ce qui signifie que les mises à jour automatiques ne seront mises à niveau que vers les versions complémentaires marquées comme compatibles avec la version actuelle de NVDA.
Les mises à jour automatiques mettront toujours à jour une version d’extension incompatible vers une version compatible lors de sa publication.
L’activation de cette option peut être utile pour passer à l’utilisation de versions de rupture d’extension (la première version de l’année).
Ceci est particulièrement utile pour les testeurs alpha et bêta, qui testent la compatibilité des extensions au cours des premières étapes d’une version de rupture d’extension.

##### Miroir {#AddonStoreMetadataMirror}

Ces contrôles vous permettent de spécifier une URL alternative à partir de laquelle télécharger les données de lêaddon store.
Cela peut être utile dans les endroits où l’accès au serveur NV Access Add-on Store est lent ou indisponible.

La zone de texte en lecture seule affiche l’URL du miroir actuel.
Si aucun miroir n’est utilisé (c’est-à-dire que le serveur de NV Access Add-on Store est utilisé), "Aucun miroir" s’affiche.

Si vous souhaitez modifier le miroir de l’add-on store, appuyez sur le bouton "Modifier..." pour ouvrir la boîte de dialogue [Définir le miroir de l’add-on store](#SetURLDialog).

#### Paramètres d'accès à distance {#RemoteSettings}

Cette catégorie vous permet de configurer le comportement de [l’accès à distance](#RemoteAccess).

##### Activer l'Accès à distance {#RemoteEnable}

Utilisez cette case à cocher pour activer ou désactiver la fonction d’accès à distance de NVDA.

Lorsque cette option n’est pas cochée, la fonctionnalité d’accès à distance est totalement indisponible.

Veuillez noter que pour continuer une session d’accès à distance sur des écrans sécurisés tels que les invites de contrôle de compte d’utilisateur, vous devez activer ce paramètre, enregistrer les modifications, puis [copier vos paramètres dans le répertoire de configuration système de NVDA](#GeneralSettingsCopySettings).
L'accès à distance ne poursuivra que les sessions démarrées par la copie installée de NVDA sur les bureaux sécurisés.
Cela signifie par exemple que vous ne pouvez pas démarrer une session d'accès à distance avec une copie portable de NVDA et poursuivre la session sur des écrans sécurisés.

Les options suivantes ne sont disponibles que si l’accès à distance est activé.

##### Confirmation de la déconnexion pour l'ordinateur contrôlé {#RemoteConfirmDisconnect}

Cette option contrôle si une confirmation est requise avant de se déconnecter d'une session d'accès à distance lorsque connecté en tant qu'ordinateur contrôlé.

Si coché, NVDA demandera une confirmation avant de se déconnecter.
Si décoché, NVDA se déconnectera immédiatement sans confirmation.

##### Se connecter automatiquement après le démarrage de NVDA {#RemoteAutoconnect}

Cette option vous permet d’établir automatiquement une session d’accès à distance au démarrage de NVDA.
Cela pourrait être utile, par exemple, pour contrôler votre ordinateur personnel lorsque vous n’êtes pas à la maison.

Soyez prudent lorsque vous activez cette option, car cela peut augmenter le risque d’accès non autorisé à votre ordinateur.

Pour plus de détails sur les options suivantes, veuillez vous référer à la section [Configuration d'une session d'accès à distance](#RemoteAccessSetup)

##### Mode {#RemoteAutoconnectMode}

Sélectionnez le mode de connexion pour les connexions automatiques.

Cette option n’est disponible que lorsque [Se connecter automatiquement après le démarrage de NVDA](#RemoteAutoconnect) est coché.

|Option |Comportement |
|---|---|
|Permettre le contrôle de cet ordinateur |Ceci permet à une personne de contrôler votre ordinateur à distance à partir de son ordinateur, comme si elle était assise devant. |
|Contrôler un autre ordinateur |Utilisez cet ordinateur comme l'ordinateur "contrôleur". Ceci vous permet d'exécuter des commandes sur l'ordinateur distant. |

##### Serveur {#RemoteAutoconnectServer}

Sélectionnez le type de serveur pour les connexions automatiques.

Cette option n’est disponible que lorsque [Se connecter automatiquement après le démarrage de NVDA](#RemoteAutoconnect) est coché.

|Option |Comportement |
|---|---|
|Utiliser un serveur existant |Utilisez un serveur d'accès à distance existant pour gérer la connexion (recommandée). |
|Héberger localement |Use this instance of NVDA as the Remote Access server (advanced). This option may require network configuration that is out of scope for this manual. |

##### Hôte {#RemoteAutoconnectHost}

Utilisez ce champ pour définir l’URL du serveur d’accès à distance que vous souhaitez utiliser pour les connexions automatiques.
Si le serveur utilise un port autre que 6837, ajoutez-le après deux points (par exemple, "example.com:1234").

Cette option n’est disponible que lorsque [Se connecter automatiquement après le démarrage de NVDA](#RemoteAutoconnect) est coché et que [Serveur](#RemoteAutoconnectServer) est défini sur "Utiliser un serveur existant".

##### Port {#RemoteAutoconnectPort}

Utilisez ce champ pour définir le port TCP que vous souhaitez utiliser pour les connexions automatiques.

Cette option n’est disponible que lorsque [Se connecter automatiquement après le démarrage de NVDA](#RemoteAutoconnect) est coché et que [Serveur](#RemoteAutoconnectServer) est défini sur "Héberger localement".

##### Clé {#RemoteAutoconnectKey}

Utilisez ce champ pour définir la clé (mot de passe) que vous souhaitez utiliser pour les connexions automatiques.

This option is only available when [Automatically connect after NVDA starts](#RemoteAutoconnect) is checked.

##### Supprimer toutes les empreintes digitales de confiance {#RemoteDeleteFingerprints}

Ce bouton vous permet d’oublier les empreintes digitales de tous les serveurs d’accès à distance précédemment approuvés.
Cela signifie qu'on vous demandera à nouveau si vous souhaitez vous connecter à tous les serveurs d'accès à distance non reconnus, même ceux auxquels vous vous êtes déjà connecté.
Il vous sera demandé de confirmer avant que toutes les empreintes digitales de confiance ne soient supprimées.

Cette option n’est disponible que si des empreintes digitales fiables sont stockées dans votre configuration.
...
#### Dialogue Définir un miroir {#SetURLDialog}

Cette boîte de dialogue vous permet de spécifier l’URL d’un miroir à utiliser lors de la [mise à jour de NVDA](#GeneralSettingsCheckForUpdates) ou [de l’utilisation du magasin de modules complémentaires](#AddonsManager).
Cela peut être utile dans les endroits où l’accès aux serveurs NV Access pour ces fonctions est lent ou indisponible.

* Lors de la configuration du [miroir de mise à jour NVDA](#UpdateMirror), le titre de cette boîte de dialogue sera "Définir le miroir de mise à jour de NVDA".
* Lors de la configuration du [miroir de l’add-on store](#AddonStoreMetadataMirror), le titre de cette boîte de dialogue sera "Définir le miroir pour l'Add-on Store".

##### URL {#SetURLTextbox}

Saisissez ici l’URL (adresse Web) du miroir que vous souhaitez utiliser.
Seules les URL HTTP et HTTPS sont prises en charge.
Pour votre confidentialité, NV Access recommande d’utiliser les URL HTTPS dans la mesure du possible.

Laissez ce champ vide pour utiliser le serveur NV Access par défaut.

##### Tester... {#SetURLTest}

Appuyez sur ce bouton pour tester l’URL miroir que vous avez saisie.
Vous devez être connecté à Internet pour que le test réussisse.
Il est recommandé de toujours tester l’URL avant de l’enregistrer.
...
##### Supprimer le silence au début du flux audio de la parole {#TrimLeadingSilenceSpeech}

Lorsqu’il est activé, NVDA supprime le silence au début de l’audio vocal, ce qui peut améliorer la réactivité de certains synthétiseurs vocaux.
Cette option est activée par défaut et ne devrait affecter que le silence au début du discours.
Si vous constatez que certaines périodes de silence nécessaires manquent également (par exemple, une pause entre deux phrases) lors de l’utilisation d’une extension de synthétiseur vocal, vous pouvez désactiver complètement cette fonctionnalité pour résoudre le problème.

##### Utiliser WASAPI pour la sortie audio SAPI 4 {#UseWASAPIForSAPI4}

Cette option permet aux voix Microsoft Speech API version 4 (SAPI 4) de produire de l’audio via l’API Windows Audio Session (WASAPI).
Cela peut permettre aux voix SAPI 4 de fonctionner avec davantage de fonctionnalités, telles que l’atténuation audio, la réduction du silence principal et le maintien de l’appareil audio en veille.
Cependant, certaines voix SAPI 4 peuvent ne pas fonctionner avec l’implémentation WASAPI actuelle.
Si vous constatez que la voix SAPI 4 que vous utilisez cesse de fonctionner, vous pouvez désactiver cette option.

| . {.hideHeaderRow} |.|
|---|---|
|Options | Par défaut (activé), Désactivé, Activé|
|Défaut |Activé|
...
Dans le dialogue qui apparaît, saisissez le symbole et appuyez sur le bouton OK.
...
Pour ajouter un geste d'émulation, sélectionnez la catégorie Touches émulées du clavier système et appuyez sur le bouton Ajouter.
Puis, sur le clavier, appuyez sur la touche que vous voulez émuler.
...
| Nom |Ordinateur de bureau |Ordinateur portable |Description|
|---|---|---|---|
|Enregistrer la configuration |`NVDA+contrôle+c` |`NVDA+contrôle+c` |Enregistre votre configuration actuelle afin qu’elle ne soit pas perdue lorsque vous quittez NVDA|
|Rétablir la configuration |`NVDA+contrôle+r` |`NVDA+contrôle+r` |Un appui réinitialise votre configuration à la dernière sauvegarde. Trois appuis la réinitialise aux paramètres par défaut.|
...
## Accès à distance {#RemoteAccess}

Grâce à la fonction d’accès à distance intégrée de NVDA, vous pouvez contrôler un autre ordinateur exécutant NVDA ou autoriser quelqu’un à contrôler votre ordinateur.
Cela permet de fournir ou de recevoir facilement de l’aide, de collaborer ou d’accéder à votre propre ordinateur à distance.

L’accès à distance est désactivé par défaut pour votre sécurité.
Vous pouvez activer l'accès à distance [dans les paramètres d'accès à distance](#RemoteSettings).

### Commencer {#RemoteAccessGettingStarted}

Avant de commencer, assurez-vous que NVDA est installé et en cours d’exécution sur les deux ordinateurs.
Assurez vous également que la case Activer l’accès à distance [dans les paramètres d’accès à distance](#RemoteSettings) est cochée sur les 2 ordinateurs.
La fonctionnalité d’accès à distance sera alors disponible à partir du menu Outils de NVDA ; aucun téléchargement ou installation supplémentaire n’est nécessaire.

Si NVDA fonctionne en [mode sécurisé](#SecureMode), vous ne pouvez pas initier manuellement une nouvelle session d'accès à distance.
Cependant, NVDA démarrera toujours automatiquement une nouvelle session d'accès à distance s'il est configuré pour le faire avec le paramètre [Se connecter automatiquement après le démarrage de NVDA](#RemoteAutoconnect).
Vous pouvez toujours vous déconnecter manuellement en mode sécurisé.

### Configurer une session d’accès à distance {#RemoteAccessSetup}

Les sessions d’accès à distance sont configurées depuis le dialogue Accès à distance.
Ouvrez le menu NVDA et sélectionnez Outils, puis "Accès à distance", puis "Se connecter…".
Ou, depuis n’importe où, appuyez sur `NVDA+alt+r`.

#### Sélection du mode (ordinateur de contrôle ou contrôlé) {#RemoteAccessMode}

Vous devrez décider quel ordinateur sera contrôlé (l’ordinateur contrôlé) et lequel contrôlera l'autre (l’ordinateur de contrôle).

la liste déroulante mode vous permet de choisir si cet ordinateur sera contrôlé (une autre personne interagira avec votre ordinateur) ou sera l'ordinateur de contrôle (vous interagirez avec l’ordinateur de quelqu’un d’autre).

* Si vous avez besoin de l’aide d’une autre personne avec votre ordinateur, choisissez "Autoriser le contrôle de cet ordinateur".
* Si vous souhaitez aider quelqu’un avec son ordinateur, choisissez "Contrôler un autre ordinateur".

#### Connexion à un serveur existant (hôte et clé fournis) {#RemoteAccessJoin}

Dans ce cas, l’autre personne a soit démarré la connexion via un serveur relais, soit héberge la connexion localement.

1. Pour le serveur, choisissez "Utiliser un serveur existant".
1. Dans le champ Hôte, saisissez l’adresse de l’hôte (et le port s’il est également fourni).
  * Vous pouvez inclure le port sur lequel vous souhaitez vous connecter en ajoutant deux points (":") et le numéro de port à l’adresse de l’hôte.
Par exemple, `example.com:1234`.
1. Dans le champ Clé, saisissez la clé fournie.
Veillez à saisir la clé exactement comme indiqué.
1. Appuyez sur OK.

#### Démarrer la connexion via un serveur relais {#RemoteAccessConnectRelay}

Si vous autorisez une autre personne à se connecter à vous via Internet, la solution la plus simple est d’utiliser un serveur relais. Pour cela :

1. Pour le serveur, choisissez "Utiliser un serveur existant".
1. Pour l’hôte, indiquez le nom d’hôte d’un serveur relais, tel que "nvdaremote.com".
Vous devrez fournir cette adresse à l’autre personne.
  * Vous pouvez inclure le port sur lequel vous connecter en ajoutant deux points (":") et le numéro de port à l’hôte.
Par exemple, `example.com:1234`.
1. Pour la clé : trouvez une clé longue et difficile à deviner, ou appuyez sur le bouton "Générer une clé" pour en générer une automatiquement.
Vous devrez la fournir à l’autre personne.
  * Notez que la clé agit à la fois comme identifiant de session et comme mot de passe, il est donc très important qu’elle soit difficile à deviner.
1. Appuyez sur OK.
1. Fournissez les détails de connexion (tels que l’hôte et la clé) à l’autre personne afin qu’elle puisse se connecter.

#### Utiliser NVDA comme serveur {#RemoteAccessConnectSelfHosted}

Si vous ne souhaitez pas utiliser de serveur relais ou si vous vous connectez via un réseau local :

1. Pour le serveur, choisissez "Héberger localement".
1. Si vous vous connectez via Internet, vous pouvez appuyer sur le bouton "Obtenir l’adresse IP externe" pour afficher votre adresse IP externe (visible sur Internet).
Vous devrez fournir cette IP externe à l’autre personne.
1. Si vous vous connectez via un réseau local, vous devrez fournir à l’autre personne votre adresse IP locale.
Sa localisation n’est pas abordée dans cette documentation.
1. Pour le port, entrez le port que vous souhaitez utiliser pour les connexions à distance entrantes.
L’autre personne en aura également besoin.
  * Il doit s’agir d’un port qui n’est utilisé par aucun autre service.
  * Par défaut, l’accès à distance utilise le port 6837.
  * Si vous vous connectez via Internet, le port devra peut-être être transféré.
  Cette documentation ne couvre pas la manière de transférer des ports.
  Vous devrez peut-être contacter votre administrateur réseau pour ce faire.
1. Pour la clé : trouvez une clé longue et difficile à deviner, ou appuyez sur le bouton "Générer une clé" pour en générer une automatiquement.
Vous devrez la fournir à l’autre personne.
  * Notez que la clé agit à la fois comme identifiant de session et comme mot de passe, il est donc très important qu’elle soit difficile à deviner.
1. Appuyez sur OK.
...
### Utiliser l’Accès à distance {#RemoteAccessUsage}

Une fois qu'une session d'accès à distance est active, vous pouvez basculer entre le contrôle de l'ordinateur distant et le vôtre, partager votre presse-papiers et couper le son de la session distante :

* Appuyez sur `NVDA+alt+tab` pour basculer entre l'ordinateur contrôlé et votre ordinateur.
* Envoyez du texte depuis votre presse-papiers vers l'autre ordinateur en ouvrant le menu NVDA, en sélectionnant Outils, "Accès à distance", puis "Envoyer le presse-papiers".
* Si vous êtes connecté en tant qu’ordinateur de contrôle, coupez le son de la parole l’ordinateur distant sur votre ordinateur local en ouvrant le menu NVDA, puis en sélectionnant Outils, puis "Accès à distance", puis "Couper le son distant".
* Si vous êtes connecté en tant qu’ordinateur de contrôle, envoyez `contrôle+alt+suppr` à l’ordinateur contrôlé en ouvrant le menu NVDA, puis en sélectionnant Outils, puis "Accès à distance", puis "Envoyer Envoyer Ctrl+Alt+Suppr".
  Pour que cela fonctionne, l’ordinateur contrôlé doit exécuter une copie installée de NVDA.
  De plus, selon les paramètres de l’ordinateur contrôlé, cette fonction peut ne pas toujours fonctionner.

### Résumé des commandes clés d’accès à distance {#RemoteAccessGestures}

<!-- KC:beginInclude -->
| Nom |Touche |Description|
|---|---|---|
| Activer ou désactiver la connexion d’accès à distance | `NVDA+alt+r` | Démarre une nouvelle session d'accès à distance, ou s'en déconnecte si une session est déjà en cours. |
| Basculer le contrôle | `NVDA+alt+tab` | Alterner entre le contrôle de l’ordinateur distant et celui de l’ordinateur local. |
| Se connecter | Aucune | Démarre une nouvelle session d'accès à distance. Non disponible en [mode sécurisé](#SecureMode). |
| Copier le lien | Aucune | Copie un lien vers la session d'accès à distance dans le presse-papiers. |
| Déconnecter | Aucune | Termine la session d'accès à distance en cours. |
| Couper le son distant | Aucune | Coupe ou rétablit la parole provenant de l'ordinateur distant. |
| Envoyer le presse-papiers | Aucune | Envoie le contenu du presse-papiers à l'ordinateur distant. |
<!-- KC:endInclude -->
...
#### Trier la liste des extensions par colonne {#AddonStoreSortByColumn}

Par défaut, la liste des extensions est triée par nom d’affichage des extensions.
La zone de liste déroulante "Trier par colonne" peut être utilisée pour trier la liste en fonction des colonnes disponibles pour chaque onglet.
Par exemple, vous souhaiterez peut-être trier les extensions par éditeur, version disponible, etc.
Les extensions peuvent être triés par ordre croissant ou décroissant.
...
Si le téléchargement ou l’installation échoue, vous pouvez réessayer l’installation.
Il est également possible d’annuler l’installation avant de quitter l’Add-on Store.
...
Vous pouvez désactiver une extension si son statut indique qu’elle est "activée" ou l’activer si l’extension est "désactivée".
...
#### Changement du canal de mise à jour automatique {#AddonStoreUpdateChannel}

Vous pouvez gérer les canaux de mise à jour automatique des extensions à partir des [onglets extensions installées et pouvant être mises à jour](#AddonStoreFilterStatus).
Lorsque les [mises à jour automatiques des extensions](#AutomaticAddonUpdates) sont activées, les extensions seront mises à jour vers le même [canal](#AddonStoreFilterChannel) à partir duquel elles ont été installées par [défaut](#DefaultAddonUpdateChannel).
Depuis le menu d’actions d'une extension, en utilisant le sous-menu "Canal de mise à jour", vous pouvez modifier les canaux vers lesquels une extension sera automatiquement mise à jour.

| Option | Comportement |
|---|---|
| Défaut | les extensions suivront le [canal de mise à jour par défaut](#DefaultAddonUpdateChannel) |
| Identique | L'extension restera sur le même canal |
| Tous | L'extension sera mise à jour à la dernière version, quel que soit le canal |
| Ne pas mettre à jour | L'extension ne sera pas mise à jour automatiquement |
| Stable | L'extension sera automatiquement mise à jour vers des versions stables |
| Beta ou dev | L'extension sera automatiquement mise à jour vers des versions beta ou dev |
| Beta | L'extension sera automatiquement mise à jour vers des versions beta |
| Dev | L'extension sera automatiquement mise à jour vers des versions dev |
...
Parfois, des problèmes peuvent survenir avec le registre Windows, ce qui entraîne un comportement anormal de NVDA.
Cela peut être dû, par exemple, à l’installation ou à la désinstallation de certains programmes (tels qu’Adobe Reader ou Math Player), ainsi qu’aux mises à jour de Windows et à d’autres évènements.
L’outil de correction d’enregistrement COM tente de résoudre ces problèmes en réparant les entrées d’accessibilité dans le registre.

Les types de problèmes que cet outil peut résoudre incluent :

* NVDA signale "inconnu" ou "volet" lors de la navigation dans des navigateurs tels que Firefox ou Edge, des programmes de messagerie tels que Thunderbird, Windows Explorer, la barre des tâches et d’autres programmes.
* NVDA ne parvient pas à basculer entre le mode formulaire et le mode navigation lorsque vous vous y attendez.
* Les boutons dont le nom était auparavant prononcé sont soudainement signalés uniquement comme "bouton".
* NVDA devient très lent lors de la navigation dans les navigateurs lors de l’utilisation du mode navigation.

Étant donné que cet outil corrige les entrées dans le registre Windows, il nécessite un accès administrateur pour fonctionner, tout comme lors de l’installation d’un programme.
Si vous avez l’UAC (User Access Control) activé, comme la plupart des utilisateurs, vous devrez suivre les invites présentées par l’UAC pour exécuter l’outil avec succès.
...
Pour une liste encore plus complète de synthétiseurs gratuits et commerciaux que vous pouvez acheter et télécharger pour une utilisation avec NVDA, veuillez consulter la [page des voix supplémentaires](https://github.com/nvaccess/nvda/wiki/ExtraVoices) (en anglais).
...
* Série HumanWare Brailliant BI/B
* HumanWare BrailleNote
* SuperBraille
* Série Optelec ALVA 6
* Série Braille Sense/Braille EDGE/Smart Beetle/Sync Braille de HIMS
...
### Série Optelec ALVA 6/Convertisseur de Protocole {#OptelecALVA}
...
### Série Brailliant BI/B de HumanWare / BrailleNote Touch {#HumanWareBrailliant}

Les terminaux de la série Brailliant BI & B de [HumanWare](https://www.humanware.com/), incluant le BI 14, BI 32, BI 20X, BI 40, BI 40X et B 80 sont supportés quand ils sont connectés via USB ou Bluetooth.
...
Voici les affectations de touches des afficheurs Braille Brailliant BI/B et BrailleNote avec NVDA.
Veuillez consulter la documentation du terminal pour savoir où se situent ces touches.
...
### Afficheurs Braille Seika {#Seika}
...
La plupart des afficheurs ont une barre d'accès facile Easy Access Bar (EAB) qui permet des opérations intuitives et rapides.
...
Garder enfoncée une des routines du haut puis appuyer sur l'EAB sur les appareils de la Série-C permet d'émuler le second cran.
Les terminaux de la série Live n'ont qu'une ligne de routines et l'EAB n'a qu'un cran par direction.
Le second cran peut être émulé en pressant une des touches de routage et en pressant l'EAB dans la direction correspondante.
...
Pour le BrailleNote Touch, veuillez vous référer à la section [Brailliant BI Series / BrailleNote Touch](#HumanWareBrailliant).
...
|flèche gauche | espace + point 3, joystick gauche ou pavé directionnel gauche|
...
Le Dot Pad de Dot Inc est un appareil capable d’afficher des graphiques tactiles actualisables et du braille.
NVDA peut afficher soit une seule ligne de braille sur la ligne d’affichage braille dédiée des appareils, soit plusieurs lignes de braille sur sa zone graphique tactile.
Le modèle A300 dispose d’une zone graphique tactile de 120 par 80 points, pouvant contenir 8 lignes de 20 cellules chacune.

Vous pouvez configurer si NVDA affiche le braille sur la ligne d’affichage braille dédiée ou sur la zone graphique tactile via l’option Destination braille dans les paramètres braille de NVDA pour ce pilote.

Les touches de panoramique sont prises en charge, mais en raison du nombre limité de boutons sur l’appareil, d’autres commandes et capacités de routage ne sont actuellement pas disponibles.

Lors de la sélection du pilote Dot Pad dans NVDA, vous devez sélectionner manuellement le port série virtuel USB / Bluetooth auquel le Dot Pad est connecté.
Ce pilote ne prend pas en charge la détection automatique.

Veuillez noter qu’en raison de limitations matérielles, le Dot Pad n’actualisera pas correctement tous les points lorsque votre main est sur l’appareil.
Assurez-vous de retirer complètement votre main de l’appareil lorsque vous naviguez avec NVDA et ne recommencez à lire qu’une fois la mise à jour terminée.

<!-- KC:beginInclude -->

| Nom |Touche|
|---|---|
|Défilement arrière de l'afficheur braille | `défilement_gauche` |
|Défilement avant de l'afficheur braille | `défilement_droite` |
...
* Ouverture de documents externes à partir du menu NVDA, tels que le fichier du guide de l’utilisateur.
...
|`-n LANGUAGE` |`--lang=LANGUAGE` |Remplacer la langue configurée de NVDA. Définir sur "Windows" pour la langue par défaut de l'utilisateur actuel, "en" pour l'anglais, etc.|
|`-m` |`--minimal` |Pas de sons, pas d'interface, pas de message de démarrage etc.|
|`-s` |`--secure` |Démarre NVDA en [Mode Sécurisé](#SecureMode)|
|`-d` |`--disable-addons` |Les extensions n'auront aucun effet|
...
|None |`--create-portable-silent` | Crée une copie portable de NVDA (sans démarrer la nouvelle copie). Nécessite la spécification de `--portable-path`. Cette option supprime les avertissements lors de l’écriture dans des répertoires non vides et peut écraser des fichiers sans avertissement.|
|Aucune |`--portable-path=PORTABLEPATH` |L'emplacement où la copie portable sera créée|

Tout comme vous pouvez installer NVDA silencieusement en passant l’option de ligne de commande `--install-silent` à NVDA, vous pouvez le désinstaller silencieusement en passant l’argument `/S` au programme de désinstallation.

Le programme de désinstallation de NVDA s’appelle `uninstall.exe` et se trouve sous le répertoire d’installation de NVDA, `%ProgramFiles(x86)%\nvda` sur Windows 64 bits ou `%ProgramFiles%\nvda` sur Windows 32 bits.

Voici les options de ligne de commande pour le programme de désinstallation de NVDA :

| Courte |Longue |Description|
|---|---|---|
|`/S` |Aucune |Désinstalle NVDA silencieusement. |

### Paramètres système globaux {#SystemWideParameters}
