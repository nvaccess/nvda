...
|Lire la sélection |`NVDA+maj+flècheHaut` |`NVDA+maj+s` |Lit tout texte sélectionné|
|Lire le texte du presse-papiers |`NVDA+c` |`NVDA+c` |Lit tout texte dans le presse-papiers|
...
|Annoncer le focus |`NVDA+tab` |`NVDA+tab` |Annonce le contrôle actuellement en focus. Un double appui épelle l'information|
...
|Annoncer le lien de destination |`NVDA+k` |`NVDA+k` |Un appui annonce l'URL de destination du lien à la position courante du curseur ou du focus. Deux appuis l'affichent dans une fenêtre pour une revue plus attentive|
...
Si vous utilisez NVDA sur un système possédant un écran tactile, et utilisant Windows 8 ou supérieur, vous pouvez aussi contrôler NVDA directement via l'écran tactile.
...
| Nom |Touche |Description|
|---|---|---|
|Annonce de la fenêtre de commentaires |contrôle+maj+c |Annonce tout commentaire dans la fenêtre de commentaires.|
|Annonce des notes pour les traducteurs |contrôle+maj+a |Annonce les notes pour les traducteurs.|
...
#### Général (NVDA+contrôle+g) {#GeneralSettings}
...
##### Autoriser le projet NVDA à collecter certaines informations statistiques d'utilisation {#GeneralSettingsGatherUsageStats}
...
#### Parole (NVDA+contrôle+v) {#SpeechSettings}
...
#### Choix du Synthétiseur (NVDA+contrôle+s) {#SelectSynthesizer}
...
Raccourci : NVDA+maj+d

Sous Windows 8 et au-delà, cette option vous permet de choisir si NVDA doit baisser le volume des autres applications quand il parle, ou tout le temps quand il est en cours d'exécution.
...
#### Choisir l'Afficheur Braille (NVDA+contrôle+a) {#SelectBrailleDisplay}
...
Pour activer ou désactiver le Rideau d'Écran de n'importe où, veuillez assigner un geste personnalisé en utilisant le dialogue [Gestes de Commandes](#InputGestures).
...
#### Clavier (NVDA+contrôle+k) {#KeyboardSettings}
...
Si aucune touche n'est choisie pour être la touche NVDA, il peut être impossible d'accéder à certaines commandes NVDA.
Par conséquent, le dialogue Paramètres affichera un message d'erreur si aucune touche n'est sélectionnée en appuyant sur "OK".
Après avoir fermé le message d'erreur, vous devrez sélectionner au moins une touche avant de pouvoir appuyer sur "OK" pour fermer le dialogue correctement.
...
#### Souris (NVDA+contrôle+m) {#MouseSettings}
...
##### Quand la souris entre dans un objet, annoncer son type {#MouseSettingsRole}

Quand cette case est cochée, NVDA annonce le type de l'objet quand la souris y entre.
...
Cette catégorie, disponible seulement sur un ordinateur exécutant Windows 8 et au-delà et possédant des capacités tactiles, vous permet de configurer la manière dont NVDA interagit avec l'écran tactile.
Elle contient les options suivantes :
...
#### Présentation des Objets (NVDA+contrôle+o) {#ObjectPresentationSettings}
...
#### Mode Navigation (NVDA+contrôle+b) {#BrowseModeSettings}
...
Cette option ne prend effet que lorsque "Utiliser WASAPI pour la sortie audio" est activé.

##### Volume des sons NVDA {#SoundVolume}

Ce potentiomètre vous permet de régler le volume des sons et des bips de NVDA.
Ce paramètre ne prend effet que lorsque "Utiliser WASAPI pour la sortie audio" est activé et que "Le volume des sons NVDA suit le volume de la voix" est désactivé.
...
Pour un tutoriel d'introduction, veuillez consulter [https://docs.python.org/fr/3.7/howto/regex.html].
...
1. Extensions et Add-on Store + [AddonsManager]
...
Sélectionnez une extension dans l'onglet "Extensions disponibles" ou "Extensions pouvant être mises à jour".
...
Comme pour l'installation, NVDA doit être redémarré pour que l'extension soit complètement supprimée.
 Jusqu'à ce que vous le fassiez, un statut "En attente de suppression" sera affiché pour cette extension dans la liste.
...
Vous pouvez parcourir les extensions incompatibles disponibles à l'aide des [onglets d'extensions disponibles et pouvant être mises à jour](#AddonStoreFilterStatus).
...
La visionneuse du journal, située dans "Outils" du menu NVDA, vous permet de consulter tous les événements enregistrés depuis votre dernier démarrage de NVDA.
Un appui sur NVDA+F1 ouvre la visionneuse du journal et affiche des informations pour le développeur à propos de l'objet navigateur courant.

En plus de lire le contenu, vous pouvez également sauvegarder une copie du journal ou réactualiser la visionneuse de manière à voir les événements les plus récents.
Ces actions sont disponibles dans le menu de la visionneuse.
...
Cependant, si vous cliquez ou mettez le focus sur la visionneuse, NVDA arrêtera momentanément de mettre à jour le texte, vous pourrez ainsi sélectionner ou copier le contenu.
...
Pour une liste encore plus importante de synthétiseurs gratuits ou du commerce que vous pouvez acheter et télécharger pour utilisation avec NVDA, veuillez consulter la page [https://www.nvda-project.org/wiki/ExtraVoices](https://www.nvda-project.org/wiki/ExtraVoices).
...
Sous Windows 7, 8 et 8.1, NVDA utilise eSpeak NG par défaut ([Windows OneCore](#OneCore) est utilisé sous Windows 10 et versions ultérieures par défaut).
 ...
 * Cette page contient beaucoup de fichiers tant pour la reconnaissance de parole que pour la synthèse vocale.
...
Veuillez consulter cet article de Microsoft pour une liste de voix disponibles : https://support.microsoft.com/fr-fr/windows/annexe-a-langues-et-voix-prises-en-charge-4486e345-7730-53da-fcfe-55cc64300f01
...
Si vous ne les avez pas déjà, vous pourrez les obtenir sur https://www2.freedomscientific.com/downloads/focus-40-blue/focus-40-14-blue-downloads.asp.
...
#### Assignations des Touches pour tous les Modèles {#toc307}
...
#### Assignations de Touches pour les Brailliant BI 32, BI 40 et B 80 {#toc308}
...
#### Assignations de Touches pour le Brailliant BI 14 {#toc309}
...
Si vous connectez votre afficheur braille par USB, vous devrez installer les pilotes USB de HIMS sur votre système.
Vous pouvez les télécharger ici : http://www.himsintl.com/upload/HIMS_USB_Driver_v25.zip
...
|alt+flècheGauche |point2+point7|
...
|alt+flècheDroite |point5+point7|
...
|alt+flècheHaut |point2+point3+point7|
...
|alt+flècheBas |point5+point6+point7|
...
Vous pouvez trouver plus d'informations concernant ces afficheurs sur https://en.seika-braille.com/down/index.html.
...
|touche entrer |RJ centre, pointt8|
...
|Choisir entre les modes de parole désactivé, bip and parole |`attribute2+attribute4`|
...
|`-f NOMDUFICHIERJOURNAL` |`--log-file=NOMDUFICHIERJOURNAL` |Le fichier où les messages du journal doivent être écrits|
|`-l NIVEAUDEJOURNALISATION` |`--log-level=NIVEAUDEJOURNALISATION` |Le niveau minimal de messages journalisés (débogage 10, entrées/sorties 12, avertissements de débogage 15, info 20, désactivé 100)|
|`-c CHEMINDECONFIG` |`--config-path=CHEMINDECONFIG` |Le chemin vers lequel tous les paramètres de NVDA sont stockés|
...
Si vous avez besoin d'informations complémentaires ou d'aide concernant l'utilisation de NVDA, veuillez visiter le site NVDA à NVDA_URL.
