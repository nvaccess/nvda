...
* Add-on Store:
  * Mises à jour automatiques (#3208):
...
* Navigateurs web:
  * Le mode de sélection native de NVDA en mode navigation (`NVDA+maj+f10`) est désormais pris en charge dans Google Chrome, Edge et d'autres applications basées sur Chromium 134 ou plus récent. (#17838)
...
* Microsoft Office:
  ...
  * Dans Word, lors de l'utilisation de la commande indiquant où se trouve le focus, la mise en page du document sera annoncée si cette information est disponible et que la description des objets est activée. (#15088, @nvdaes)
  * Le résultat de davantage de raccourcis est maintenant annoncé dans Word et Outlook :
    * raccourcis de mise en forme de police (#10271, @CyrilleB79)
    * réduire ou développer un titre (#17545, @CyrilleB79)
* LibreOffice:
  * Lorsque vous augmentez ou diminuez la taille de la police dans Writer en utilisant les raccourcis clavier correspondants, NVDA annonce la nouvelle taille de police. (#6915, @michaelweghorn)
  * Lorsque vous appliquez le style de paragraphe corps de texte" ou un titre avec le raccourci clavier correspondant dans Writer 25.2 ou une version plus récente, NVDA annonce le nouveau style de paragraphe. (#6915, @michaelweghorn)
  * Lorsque vous activez ou désactivez le double soulignement dans Writer à l'aide du raccourci clavier correspondant, NVDA annonce le nouvel état ("soulignage double activé" / "soulignage double désactivé"). (#6915, @michaelweghorn)
  * NVDA peut maintenant indiquer l'indentation de première ligne dans LibreOffice 25.8 et versions ultérieures. (#13052, @michaelweghorn)
* Reconnaissance Optique de Caractères :
  * Dans un résultat de reconnaissance, `NVDA+f5` actualise manuellement le contenu reconnu. (#17715, @CyrilleB79)
  * Ajout d'un geste de commandes non assigné pour activer ou désactiver l'actualisation périodique du résultat de la Reconnaissance Optique de Caractères de Windows. (#16897)
* La fonctionnalité d'accès à distance, basée sur l'extension NVDA Remote, a été intégrée à NVDA. (#4390, @ctoth, @tspivey, @daiverd, contributeurs et financeurs de NVDA Remote)
* Il est maintenant possible de spécifier des URLs miroirs à utiliser pour les mises à jour de NVDA et l'Addon Store. (#14974, #17151, #17310, @christopherpross)
* La prise en charge des formules mathématiques dans les PDF a été ajoutée. (#9288, @NSoiffer)
  * Cela fonctionne pour les formules avec du MathML associé, comme certains fichiers générés par les versions plus récentes de TeX/LaTeX.
  * Actuellement, cette fonctionnalité n'est prise en charge que dans Foxit Reader et Foxit Editor.
* NVDA peut maintenant être configuré pour annoncer la ligne ou le paragraphe courant lors de la navigation avec les touches de navigation braille. (#17053, @nvdaes)
* NVDA est maintenant capable de signaler les changements de curseur lors de l'exécution des gestes de commande `alt+flècheHaut` ou `alt+flècheBas`, par exemple dans Visual Studio Code. (#17652, @LeonarddeR)
* Ajout de commandes pour déplacer le curseur de revue au premier et au dernier caractère du texte sélectionné, assignées respectivement à `NVDA+alt+début` et `NVDA+alt+fin`. (#17299, @nvdaes)
* Ajout d'un paramètre général pour empêcher l'extinction de l'écran pendant l'utilisation du mode "dire tout" ou la lecture en braille.
Cette option est activée par défaut, mais peut entraîner une décharge plus rapide de la batterie. (#17649, @LeonarddeR)
* NVDA est maintenant traduit en bosniaque. (#17953)
* Dans Adobe Acrobat, NVDA peut désormais lire et interagir avec les équations mathématiques dans les documents PDF générés par les versions récentes de Microsoft Word. (#18056)

### Changements

* Mises à jour de composants:
  * Mise à jour du convertisseur braille LibLouis vers [3.33.0](https://github.com/liblouis/liblouis/releases/tag/v3.33.0). (#17469, #17768, @LeonarddeR, @codeofdusk)
    * Ajout de nouvelles tables braille pour le copte, le cunéiforme compact et le braille informatique portugais 6 points.
  * Mise à jour de CLDR vers la version 46.0. (#17484, @OzancanKaratas)
  * Mise à jour de eSpeak NG vers 1.52.0. (#17056)
* Modifications de l'interface de NVDA :
  * L'installateur de NVDA ne joue plus de son au lancement. (#14068)
  * Les boîtes de dialogue d'annonce de la destination d'un lien, d'informations sur la mise en forme des caractères et de lecture de la sélection incluent maintenant des boutons "Fermer" et "Copier" pour plus de commodité. (#17018, @XLTechie)
  * La boîte de dialogue de sortie permet maintenant de redémarrer NVDA avec les extensions désactivées et la journalisation de débogage activée simultanément. (#11538, @CyrilleB79)
  * La possibilité de ne pas utiliser WASAPI pour la sortie audio a été supprimée. (#16080)
  * Le paramètre du mode Navigation "Positionnement automatique du focus système aux éléments susceptibles d'être mis en focus" a été supprimé, ce comportement est maintenant désactivé. (#17598)
  * NVDA n'avertira désormais de l'incompatibilité des extensions que lors de la mise à jour vers une nouvelle version dont l'API d'extension est incompatible avec la version actuelle. (#17071, #17506)
  * Le fichier "Contributeurs" a été supprimé du menu NVDA. (#16922)
  * La licence NVDA est maintenant formatée en HTML pour améliorer sa lisibilité. (#17600)
  * Des versions abrégées des options de ligne de commande les plus couramment utilisées ont été ajoutées : `-d` pour `--disable-addons` et `-n` pour `--lang`.
  La correspondance par préfixe pour les options de ligne de commande, par exemple l'utilisation de `--di` pour `--disable-addons`, n'est plus prise en charge. (#11644, @CyrilleB79)
  * Modifications au niveau de l'outil de correction des enregistrements COM : (#12355, @XLTechie)
    * Il démarre maintenant avec une explication plus conviviale de son objectif, au lieu d'un avertissement. (#12351)
    * La fenêtre initiale peut maintenant être fermée avec `échap` ou `alt+f4`. (#10799)
    * Il affichera désormais un message à l'utilisateur en incluant l'erreur, dans le cas rare où une erreur Windows se produirait lors des tentatives de réenregistrement COM.
* Parole:
  * Les voix Microsoft Speech API version 5 et Microsoft Speech Platform utilisent maintenant WASAPI pour la sortie audio, ce qui peut améliorer leur réactivité. (#13284, @gexgd0419)
  * Le silence au début de la parole sera désormais supprimé lors de l'utilisation des voix OneCore, SAPI5 et certaines extensions de voix tierces pour améliorer leur réactivité. (#17614, @gexgd0419)
  * Les voix Microsoft Speech API version 4 utilisent maintenant WASAPI pour la sortie audio, afin qu'elles puissent fonctionner avec des fonctionnalités telles que l'atténuation de l'audio, la suppression du silence initial et le maintien en éveil du périphérique audio.
  Si cela ne fonctionne pas avec votre voix SAPI 4, vous pouvez désactiver WASAPI pour SAPI 4 dans les paramètres avancés. (#17718, #17801, @gexgd0419)
  * La normalisation Unicode est maintenant activée par défaut pour la sortie vocale. (#17017, @LeonarddeR).
    * Vous pouvez toujours désactiver cette fonctionnalité dans la catégorie Parole de la boîte de dialogue des Paramètres de NVDA.
* Les paramètres clavier contrôlant l'éccho des caractères et des mots pendant la saisie ont maintenant trois options : Désactivé, Seulement dans les zones d'édition, et Toujours. (#17505, @Cary-rowen)
  * Par défaut, "écho clavier par caractères" est maintenant défini sur "Seulement dans les zones d'édition".
* Les tables braille de saisie et d'affichage par défaut peuvent maintenant être déterminées en fonction de la langue de NVDA. (#17306, #16390, #290, @nvdaes)

### Corrections de bugs

* Parole:
  * Correction d'un problème où la lecture continue (Dire tout) s'arrêtait à la fin de la première phrase lors de l'utilisation de certains synthétiseurs SAPI5. (#16691, @gexgd0419)
  * Les paramètres vocaux, tels que la vitesse et le volume, ne seront plus réinitialisés aux valeurs par défaut lors de l'utilisation de la boucle des paramètres synthétiseur pour changer de voix dans les synthétiseurs SAPI5 et SAPI4. (#17693, #2320, @gexgd0419)
  * Correction d'un problème où certaines voix SAPI4 (par exemple IBM TTS Chinois) ne pouvaient pas être chargées. (#17726, @gexgd0419)
* Braille:
  * Prise en charge native du périphérique graphique tactile Dot Pad de Dot Inc en tant qu'afficheur braille multi-lignes. (#17007)
  * Lors de l'utilisation des blocs-notes Seika, les gestes utilisant la barre d'espace et la barre d'espace avec des points sont maintenant affichés correctement dans la boîte de dialogue Gestes de commande. (#17047, @school510587)
  * NVDA ne devrait plus émettre de sons d'erreur ou surcharger inutilement le fichier journal lors de l'utilisation du mode d'affichage de la sortie vocale en braille lorsqu'aucun afficheur braille n'est connecté. (#17092, @Emil-18)
  * Lorsque le pilote d'affichage braille HID standard est explicitement sélectionné comme pilote d'afficheur braille et que la liste des afficheurs braille est ouverte, NVDA identifie correctement le pilote HID comme étant le pilote sélectionné au lieu de n'afficher aucun pilote sélectionné. (#17537, @LeonarddeR)
  * Le pilote Humanware Brailliant est maintenant plus fiable dans la sélection du bon point de connexion, ce qui se traduit par une meilleure stabilité de connexion et moins d'erreurs. (#17537, @LeonarddeR)
  * Les tables braille personnalisées dans le Scratchpad sont maintenant correctement ignorées lors de l'exécution avec les extensions désactivées. (#17565, @LeonarddeR)
  * Correction d'un problème où certains afficheurs braille USB n'étaient pas correctement détectés par NVDA. (#18114, @christiancomaschi)
* Microsoft Office:
  * La commande d'Annonce de l'URL de destination d'un lien fonctionne maintenant comme prévu lors de l'utilisation du modèle d'objet hérité dans Word, Outlook, Excel et PowerPoint. (#17292, #17362, #17435, @CyrilleB79)
  * Dans Excel, la boîte de dialogue liste d'éléments (`NVDA+f7`) n'échoue plus à lister les commentaires ou formules sur certains systèmes non anglophones. (#11366, @CyrilleB79)
  * Améliorations dans PowerPoint:
    * L'annonce du curseur ne dysfonctionnne plus lorsque le texte contient des caractères larges, comme les émojis. (#17006, @LeonarddeR)
    * L'annonce de la position du caractère est désormais précise lors de l'appui sur `NVDA+suppr` par exemple. (#9941, @LeonarddeR)
    * NVDA ne démarre plus le mode "dire tout" lors du démarrage d'un diaporama quand le paramètre du mode Navigation "Lecture automatique au chargement de la page" est désactivé. (#17488, @LeonarddeR)
* LibreOffice:
  * Lors de l'utilisation de la méthode de saisie Microsoft Pinyin pour le chinois et de l'activation de l'option de compatibilité Pinyin pour utiliser la version précédente, la saisie dans Writer (et potentiellement d'autres applications) ne déclenche plus d'erreur lorsqu'une popup IME est affichée. (#17198, @michaelweghorn)
  * L'état actuel des cases à cocher (coché/non coché) est désormais également signalé en braille et pas seulement vocalement dans les boîtes de dialogue. (#17218, @michaelweghorn)
* Formules mathématiques:
  * La lecture des formules mathématiques a été corrigée pour certains éléments web.
  Plus précisément, pour le MathML à l'intérieur de balises span et autres éléments qui ont l'attribut `role="math"`. (#15058)
  * Les équations mathématiques représentées uniquement par une image et un texte alternatif sans MathML pour la navigation enrichie sont maintenant traitées comme des images normales plutôt que comme des formules mathématiques sans contenu, permettant à l'utilisateur de les atteindre avec `g` et de parcourir le texte alternatif caractère par caractère. (#16007)
* IDEs:
  * NVDA ne plante plus lors de la sélection de tout le texte dans certains fichiers sources dans Android Studio ou IntelliJ Idea. (#17418, @thgcode)
  * Dans Visual Studio Code, NVDA n'intercepte plus les gestes `alt+flècheHaut` et `alt+flècheBas` pour la navigation par phrase. (#17082, @LeonarddeR)
  * Certains éléments de section sont maintenant correctement reconnus comme des contrôles modifiables dans Visual Studio Code. (#17573, @Cary-rowen)
  * Dans Notepad et autres documents UIA ainsi que dans les documents Notepad++ sous Windows 11, la "commande braille ligne suivante" déplacera le curseur vers la dernière ligne si la dernière ligne est vide.
  Dans n'importe quel document, si le curseur est sur la dernière ligne, il sera déplacé à la fin lors de l'utilisation de cette commande. (#17251, #17430, @nvdaes)
* Profils de configuration:
  * Le braille ne dysfonctionne plus lors de l'activation de "dire tout" avec un profil de configuration associé. (#17163, @LeonarddeR)
  * Correction d'un problème où certains paramètres étaient explicitement enregistrés dans le profil de configuration actif même lorsque la valeur de ce paramètre était égale à la valeur dans la configuration de base. (#17157, @leonarddeR)
* Lors de l'épellation, la normalisation Unicode fonctionne maintenant de manière plus appropriée:
  * Après avoir prononcé un caractère normalisé, NVDA n'annonce plus incorrectement les caractères suivants comme normalisés. (#17286, @LeonarddeR)
  * Les caractères composites tels que é sont maintenant annoncés correctement. (#17295, @LeonarddeR)
* NVDA est désormais capable de lire les éléments du sous-menu contextuel sur la page des résultats de recherche de Thunderbird. (#4708, @thgcode)
* L'outil de correction des enregistrements COM ne signale plus un succès en cas d'échec. (#12355, @XLTechie)
* NVDA n'annoncera plus les entrées de l'historique du presse-papiers de Windows 11 lors de la fermeture de la fenêtre quand des éléments sont présents. (#17308, @josephsl)
* Si les modules sont rechargés alors qu'un message navigable est ouvert, NVDA n'omettra plus d'annoncer les déplacements ultérieurs du focus. (#17323, @CyrilleB79)
* Lors de l'utilisation d'applications comme Skype, Discord, Signal et Phone Link pour la communication audio, le volume de la voix et des sons de NVDA ne diminue plus. (#17349, @jcsteh)
* L'ouverture de la Console Python NVDA ne sera plus bloquée si une erreur se produit lors de la récupération des variables d'instantané. (#17391, @CyrilleB79)
* Dans les navigateurs web, les modifications de la sélection de texte sont maintenant toujours correctement annoncées dans les contrôles de texte modifiables. (#17501, @jcsteh)
* Lorsque des liens d'ancrage pointent vers le même objet que celui où est placé le curseur virtuel, NVDA n'échoue plus à faire défiler vers la destination du lien. (#17669, @nvdaes)
* L'icône de NVDA n'est plus fixée dans la barre des tâches après le redémarrage de l'Explorateur. (#17696, @hwf1324)

### Changements pour les développeurs

Veuillez consulter [le guide du développeur](https://www.nvaccess.org/files/nvda/documentation/developerGuide.html#API) pour obtenir des informations sur le processus de dépréciation et de suppression de l'API de NVDA.

* Remarque : il s'agit d'une version de rupture de compatibilité de l'API des extensions.
Les extensions devront être re-testés et leur manifeste mis à jour.
* Mises à jour de composants:
  * Mise à jour de Ruff vers 0.8.1. (#17102, #17260, #17473)
  * Mise à jour de Comtypes vers 1.4.6. (#17061, @LeonarddeR)
  * Mise à jour de wxPython vers 4.2.2. (#17181, @dpy013)
  * Mise à jour de SCons vers 4.8.1. (#17254)
  * Mise à jour de sphinx vers 8.1.2 et de sphinx-rtd-theme vers 3.0.1. (#17284, @josephsl)
  * Mise à jour de Robot Framework vers 7.1.1. (#17329, @josephsl)
  * Mise à jour de configobj vers 5.1.0 commit `8be5462`. (#17328)
  * Mise à jour de pre-commit vers 4.0.1. (#17260)
  * Mise à jour de typing-extensions vers 4.12.2. (#17438, @josephsl)
  * Mise à jour de licensecheck vers 2024.3. (#17440, @josephsl)
  * Mise à jour de markdown vers 3.7. (#17459, @josephsl)
  * Mise à jour de nh3 vers 0.2.19. (#17465, @josephsl)
  * Mise à jour de nuitka vers 2.5.4. (#17458, @josephsl)
  * Mise à jour de schedule vers 1.2.2. (#17455, @josephsl)
  * Mise à jour de requests vers 2.32.3. (#17456, @josephsl)
* `ui.browseableMessage` peut maintenant être appelé avec des options pour présenter un bouton de copie dans le presse-papiers, et/ou un bouton pour fermer la fenêtre. (#17018, @XLTechie)
* Plusieurs ajouts pour identifier les types de liens (#16994, @LeonarddeR, @nvdaes)
  * Un nouveau module `utils.urlUtils` avec différentes fonctions pour déterminer les types de liens
  * Un nouvel état `INTERNAL_LINK` a été ajouté à `controlTypes.states.State`
  * Une nouvelle propriété `linkType` a été ajoutée à `NVDAObject`.
  Elle interroge le `treeInterceptor` par défaut, le cas échéant.
  * L'objet `BrowseModeTreeInterceptor` possède une nouvelle propriété `documentUrl`
  * L'objet `BrowseModeTreeInterceptor` possède une nouvelle méthode `getLinkTypeInDocument` qui accepte une URL pour vérifier le type de lien de l'objet
  * Une fonction utilitaire `toggleBooleanValue` a été ajoutée à `globalCommands`.
  Elle peut être utilisée dans des scripts pour signaler le résultat lorsqu'un booléen est basculé dans `config.conf`
* Suppression de l'obligation d'indenter les listes de paramètres de fonction de deux tabulations dans les normes de codage de NVDA, pour être compatible avec le linting automatique moderne. (#17126, @XLTechie)
* Ajout de la [configuration d'espace de travail VS Code pour NVDA](https://nvaccess.org/nvaccess/vscode-nvda) en tant que sous-module git. (#17003)
* Une nouvelle fonction `gui.guiHelper.wxCallOnMain` a été ajoutée permettant d'appeler en toute sécurité et de manière synchrone des fonctions wx à partir de threads non GUI, et d'obtenir leur valeur de retour. (#17304)
* Une nouvelle API de boîte de dialogue de message a été ajoutée à `gui.message`. (#13007)
  * Classes ajoutées: `ReturnCode`, `EscapeCode`, `DialogType`, `Button`, `DefaultButton`, `DefaultButtonSet`, `MessageDialog`.
* Dans le module `brailleTables`, une fonction `getDefaultTableForCurrentLang` a été ajoutée (#17222, @nvdaes)
* Ajout d'une classe de données `updateCheck.UpdateInfo`, qui encapsule les métadonnées concernant les mises à jour de NVDA. (#17310, @christopherpross)
* La récupération de la propriété `labeledBy` fonctionne maintenant pour:
  * les objets dans les applications implémentant la relation `labelled-by` IAccessible2. (#17436, @michaelweghorn)
  * Les éléments UIA prenant en charge la propriété UIA `LabeledBy` correspondante. (#17442, @michaelweghorn)
* Ajout de la possibilité d'associer `wx.ComboBox` et une étiquette `wx.StaticText` en utilisant `gui.guiHelper.associateElements`. (#17476)
* Ajout des points d'extension suivants (#17428, @ctoth):
  * `inputCore.decide_handleRawKey`: appelé à chaque pression de touche
  * `speech.extensions.post_speechPaused`: appelé lorsque la parole est mise en pause ou reprise
* Modifications de l'enregistrement de la détection automatique d'afficheur braille dans `bdDetect.DriverRegistrar`: (#17521, @LeonarddeR)
  * Ajout de la méthode `addUsbDevice` pour enregistrer un périphérique USB à la fois.
  * Ajout du paramètre `matchFunc` à `addUsbDevices` qui est également disponible sur `addUsbDevice`.
    * De cette façon, la détection des périphériques peut être davantage limitée dans les cas où une combinaison VID/PID est partagée par plusieurs périphériques à travers plusieurs pilotes, ou encore lorsqu'un périphérique HID offre plusieurs points de terminaison.
    * Consultez la documentation de la méthode ainsi que des exemples dans les pilotes albatross et brailliantB pour plus d'informations.
* Ajout d'une nouvelle fonction, `utils.mmdevice.getOutputDevices`, pour énumérer les périphériques de sortie audio. (#17678)
* Ajout d'un nouveau point d'extension `pre_synthSpeak` dans `synthDriverHandler`, qui sera appelé avant que le gestionnaire de parole n'appelle `speak` du synthétiseur actuel. (#17648)
* NVDA prend en charge l'attribut d'objet IAccessible2 `text-indent`. (#13052, @michaelweghorn)
* Lorsqu'un script utilisant `gesture.send` est déclenché avec un geste qui inclut `numLock` comme modificateur, Num Lock n'est plus désactivé. (#10827, @CyrilleB79)

#### Changements avec rupture de l'API

Il s'agit de changements d'API entraînant une rupture de compatibilité.
Veuillez ouvrir un ticket GitHub si votre extension rencontre un problème lors de la mise à jour vers la nouvelle API.

* La constante `addonStore.network.BASE_URL` a été supprimée.
Étant donné que l'URL de base de l'Addon Store est désormais configurable directement dans NVDA, aucun remplacement n'est prévu. (#17099)
* La constante `updateCheck.CHECK_URL` a été supprimée.
Étant donné que l'URL de vérification des mises à jour NVDA est désormais configurable directement dans NVDA, aucun remplacement n'est prévu. (#17151)
* `NVDAObjects.UIA.winConsoleUIA.WinTerminalUIA` a été supprimé sans remplacement public. (#14047, #16820, @codeofdusk)
* `NVDAObjects`NVDAObjects.IAccessible.ia2TextMozilla.FakeEmbeddingTextInfo` a été supprimé. (#16768, @jcsteh)
* Les symboles suivants dans `appModules.soffice` ont été renommés (#6915, @michaelweghorn):
  * `SymphonyDocument.announceToolbarButtonToggle` en `SymphonyDocument.announceFormattingGestureChange`
  * `SymphonyDocument.script_toggleTextAttribute` en `SymphonyDocument.script_changeTextFormatting`
* L'argument clé `space` pour `brailleDisplayDrivers.seikantk.InputGesture` attend maintenant un `int` au lieu d'un `bool`. (#17047, @school510587)
* La section de configuration `[upgrade]` incluant `[upgrade][newLaptopKeyboardLayout]` a été supprimée. (#17191)
* `updateCheck.checkForUpdate` renvoie maintenant un objet `UpdateInfo` au lieu d'un dictionnaire. (#17310, @christopherpross)
* Les constructeurs de `updateCheck.UpdateResultDialog` et `updateCheck.UpdateDownloader` ont été mis à jour pour prendre des objets `UpdateInfo` au lieu de dictionnaires de métadonnées. (#17310, @christopherpross)
* En raison de l'abandon du support winmm de NVDA (#17496, #17532, #17678):
  * Les symboles suivants ont été supprimés de `nvwave` sans remplacements: `CALLBACK_EVENT`, `CALLBACK_FUNCTION`, `CALLBACK_NULL`, `HWAVEOUT`, `LPHWAVEOUT`, `LPWAVEFORMATEX`, `LPWAVEHDR`, `MAXPNAMELEN`, `MMSYSERR_NOERROR`, `usingWasapiWavePlayer`, `WAVEHDR`, `WAVEOUTCAPS`, `waveOutProc`, `WAVE_MAPPER`, `WHDR_DONE`, `WinmmWavePlayer`, et `winmm`.
  * Les symboles suivants ont été supprimés de `nvwave`: `getOutputDeviceNames`, `outputDeviceIDToName`, `outputDeviceNameToID`.
  Utilisez `utils.mmdevice.getOutputDevices` à la place.
  * `nvwave.WasapiWavePlayer` a été renommé en `WavePlayer`.
  De plus, la signature de sa méthode `__init__` a changé comme suit :
    * Le paramètre `outputDevice` ne reçoit désormais que des arguments de type string.
    * Les paramètres obsolètes `closeWhenIdle` and `buffered` ont été supprimés.
  * `gui.settingsDialogs.AdvancedPanelControls.wasapiComboBox` a été supprimé.
  * La clé `WASAPI` a été supprimée de la section `audio` de la spécification de configuration.
  * La clé de configuration `config.conf["speech"]["outputDevice"]` a été supprimée.
    Elle a été remplacée par `config.conf["audio"]["outputDevice"]`, qui stocke un ID de périphérique de point de terminaison audio Windows core. (#17547)
* Dans `NVDAObjects.window.scintilla.ScintillaTextInfo`, si aucun texte n'est sélectionné, la méthode `collapse` est surchargée pour étendre à la ligne si le paramètre `end` est défini sur `True` (#17431, @nvdaes)
* Les symboles suivants ont été supprimés sans remplacement: `languageHandler.getLanguageCliArgs`, `__main__.quitGroup` et `__main__.installGroup`. (#17486, @CyrilleB79)
* La correspondance par préfixe sur les options de ligne de commande, par exemple l'utilisation de `--di` pour `--disable-addons`, n'est plus prise en charge. (#11644, @CyrilleB79)
* L'argument clé `useAsFallBack` de `bdDetect.DriverRegistrar` a été renommé en `useAsFallback`. (#17521, @LeonarddeR)
* Le paramètre de configuration `[addonStore][showWarning]` a été supprimé.
Utilisez plutôt `addonStore.dataManager.addonDataManager.storeSettings.showWarning`. (#17597)
* `ui.browseableMessage` prend maintenant un paramètre `sanitizeHtmlFunc`.
Celui-ci prend par défaut `nh3.clean` avec les arguments par défaut.
Cela signifie que tout HTML passé à `ui.browseableMessage` en utilisant `isHtml=True` est maintenant assaini par défaut.
Pour modifier les règles d'assainissement, comme la mise en liste blanche de balises ou d'attributs, créez une fonction qui appelle `nh3.clean` avec les paramètres souhaités. (#16985)
* `updateCheck.UpdateAskInstallDialog` n'effectue plus automatiquement une action lorsque les boutons de mise à jour ou de report sont pressés.
À la place, une propriété `callback` a été ajoutée, qui renvoie une fonction qui effectue l'action appropriée lorsqu'elle est appelée avec la valeur de retour de la boîte de dialogue. (#17582)
* Les boîtes de dialogue ouvertes avec `gui.runScriptModalDialog` sont maintenant reconnues comme modales par NVDA. (#17582)
* Les symboles d'API suivants liés au paramètre "Définir automatiquement le focus système sur les éléments pouvant recevoir le focus" ont été supprimés sans remplacement: (#17598)
...
* Étant donné que les voix SAPI5 utilisent maintenant `nvwave.WavePlayer` pour sortir l'audio: (#17592, @gexgd0419)
  * `synthDrivers.sapi5.SPAudioState` a été supprimé.
  * `synthDrivers.sapi5.SynthDriver.ttsAudioStream` a été supprimé.
* Les instances de `autoSettingsUtils.driverSetting.DriverSetting` avec un `id` commençant par un tiret bas (_) ne sont plus affichées dans les paramètres de NVDA. (#17599)
* Changement de la configuration de l'écho de frappe du clavier de valeurs booléennes à des valeurs entières. (#17505, @Cary-rowen)
  * `config.conf["keyboard"]["speakTypedCharacters"]` et `config.conf["keyboard"]["speakTypedWords"]` utilisent maintenant des valeurs entières.
  * Ajout de l'énumération `TypingEcho` dans `config.configFlags` pour représenter ces modes, 0=Désactivé, 1=Uniquement dans les zones d'édition, 2=Toujours.
  * `gui.settingsDialogs.KeyboardSettingsPanel.wordsCheckBox` et `gui.settingsDialogs.KeyboardSettingsPanel.charsCheckBox` ont été supprimés.
* `winUser.paint` a été renommé de `painStruct` à `paintStruct`, corrigeant un bug où le passage d'un `PAINTSTRUCT` provoquait une exception. (#17744)
* `documentationUtils.getDocFilePath` et `installer.getDocFilePath` ne recherchent plus les fichiers `.txt` dans les dossiers de documentation locale. (#17911, @CyrilleB79)
* `config.conf["documentFormatting"]["reportFontAttributes"]` a été supprimé, utilisez `config.conf["documentFormatting"]["fontAttributeReporting"]` à la place. (#18066)
* `config.conf["speech"]["includeCLDR"]` a été supprimé, vérifiez/modifiez si `config.conf["speech"]["symbolDictionaries"]` contient `"cldr"` à la place. (#18066)

#### Dépréciations

* Le point d'extension `braille.filter_displaySize` est déprécié.
Veuillez utiliser `braille.filter_displayDimensions` à la place. (#17011)
* Les fonctions `gui.message.messageBox` et `gui.runScriptModalDialog`, ainsi que la classe `gui.nvdaControls.MessageDialog` sont dépréciées.
Utilisez `gui.message.MessageDialog` à la place. (#17582)
* Les symboles suivants sont dépréciés (#17486, @CyrilleB79):
  * `NoConsoleOptionParser`, `stringToBool`, `stringToLang` dans `__main__`; utilisez les mêmes symboles dans `argsParsing` à la place.
  * `__main__.parser`; utilisez `argsParsing.getParser()` à la place.
* `bdDetect.DeviceType` est déprécié en faveur de `bdDetect.ProtocolType` et `bdDetect.CommunicationType` pour prendre en compte le fait que les communications HID et Serial peuvent avoir lieu via USB et Bluetooth. (#17537, @LeonarddeR)
...
Cette version corrective vise à résoudre un bug lors de l'enregistrement des dictionnaires de symboles vocaux.
...
NVDA peut désormais annoncer le caractère à la position du curseur lors d'un appui sur une touche de routage curseur.
La fiabilité des touches de routage curseur a  été améliorée et celles-ci sont désormais prise en charge dans PowerPoint.
...
* Nouvelles fonctionnalités braille :
  ...
  * Lors de l'appui sur une touche de routage curseur, NVDA peut désormais annoncer automatiquement le caractère à la position du curseur. (#8072, @LeonarddeR)
...
* La stabilité du support de Poedit par NVDA a été améliorée, avec comme effet de bord que la version minimale de Poedit supportée est désormais la 3.5. (#16889, @LeonarddeR)
...
* L'exécution de SCons avec le paramètre `--all-cores` sélectionnera automatiquement le nombre maximum de cœurs de processeur disponibles. (#16943, #16868, @LeonarddeR)
...
* Correction d'un problème avec les attributs mathématiques lus dans Adobe Reader et qui entraînait une vocalisation et un affichage braille incorrects. (#17980)
* Correctifs pour Windows 11 :
...
* De nouvelles commandes de navigation de tableau ont été ajoutées. (#957)
  * `control+alt+début/fin` pour atteindre la première/dernière colonne.
  * `control+alt+pagePréc/PageSuiv` pour atteindre la première/dernière ligne.
...
* Pour Microsoft Office 16.0.15000 et supérieur sous Windows 11, NVDA utilisera UI Automation par défaut pour accéder aux documents Microsoft Word, offrant un considérable gain de performances pour l'accès à tout les objets. (#13437)
  * Cela inclut les documents dans Microsoft Word lui-même, mais aussi le lecteur de message et la fenêtre de rédaction de Microsoft Outlook.
...
* Cette version rompt la compatibilité avec les extensions existantes.
* Cette version abandonne également la prise en charge d'Adobe Flash.
...
* Mise à jour du transcripteur Braille Liblouis à la version 3.16.1 :
  * Résout plusieurs plantages
  * Ajoute la table braille grade 1 en bachkir
  * Ajout de la table braille informatique 8 points en copte
  * Ajout des tables braille littéraire russe et braille littéraire russe (détaillée)
  * Ajout de la table braille abrégé en afrikaans
  * Suppression de la table braille intégral en russe
...
* Le texte marqué (surligné) est maintenant signalé en Braille et vocalement dans tous les navigateurs Web. (#11436)
  * Ceci peut être activé et désactivé par une nouvelle option de formatage de document NVDA pour la surbrillance.
...
* Les objets spécifiques à NVDA trouvés par le ramasse miettes cyclique de Python sont maintenant conservés lorsqu'ils permettent de supprimer les références cycliques de NVDA. (#11499)
  * La majorité des classes de NVDA sont suivies, notamment les NVDAObjects, appModules, GlobalPlugins, SynthDrivers et TreeInterceptors.
  * Une classe qui doit être suivie devrait hériter de garbageHandler.TrackedObject.
...
* Dans Internet Explorer, Google Chrome et Firefox, vous pouvez maintenant naviguer par article et groupe en utilisant des scripts de navigation rapide. Ces scripts ne sont pas assignés par défaut et peuvent l'être via le dialogue "Gestes de commande" lorsque celui-ci est ouvert à partir d'un document en mode navigation. (#9485, #9227)
  * Les figures sont également signalées. Elles sont considérées comme des objets et donc navigables avec la touche de navigation rapide o.
...
* Ajout du rideau d'écran qui, quand il est activé, rend l'écran totalement noir sous Windows 8 et supérieur (#7857)
  * Ajout d'un script pour activer le rideau d'écran (jusqu'au prochain redémarrage avec une pression, ou toujours pendant l'exécution de NVDA avec deux pressions), aucun geste de commande par défaut n'est assigné.
  * Peut être activé et configuré via la catégorie 'vision' dans la boîte de dialogue des paramètres de NVDA.
* Ajout de la fonctionnalité surlignage d'écran à NVDA. (#971, #9064)
  * La mise en surbrillance du focus, de l'objet navigateur et de la position du curseur en mode navigation peut être activée et configurée via la catégorie 'vision' dans la boîte de dialogue des paramètres de NVDA.
  * Remarque: Cette fonctionnalité est incompatible avec l'addon de mise en surbrillance du focus, cependant, l'addon peut toujours être utilisé lorsque la surbrillance intégrée est désactivée.
...
* Comme NVDA ne dépend plus de pywin32, les modules tels que win32api et win32con ne sont plus disponibles pour les extensions. (#9639)
  * les appels win32api peuvent être remplacés par des appels directs aux fonctions dll win32 via ctypes.
  * les constantes win32con doivent être définies dans vos fichiers.
...
* speakText et speakCharacter ne sont plus supportés dans synthDriver.
  * Cette fonctionnalité est gérée par SynthDriver.speak.
...
* La classe SynthDriver ne peut plus renvoyer d'index via la propriété lastIndex.
  * À la place, ils devraient notifier l'action synthDriverHandler.synthIndexReached avec l'index, une fois que tous les précédents audios ont fini de jouer avant cet index.
...
* Plusieurs fonctions de parole ont été modifiées pour retourner des séquences de parole (#10098)
  ...
  * getSpeechTextForProperties est maintenant appelé getPropertiesSpeech
...
* Ajout d'un module textUtils pour faciliter la différenciation entre les chaînes python 3 et les chaînes Windows unicode. (#9545)
  * Consultez la documentation du module et le module textInfos.offsets pour des exemples d'implémentations.
* Fonctionnalités obsolètes supprimées (#9548)
  * AppModules supprimés:
    * Enregistreur vocal de Windows XP.
    * Klango Player, qui est un logiciel abandonné.
  * wrapper configobj.validate supprimé.
    * Le nouveau code devrait utiliser from configobj import validate au lieu de import validate
  * textInfos.Point et textInfos.Rect remplacés respectivement par locationHelper.Point et locationHelper.RectLTRB.
  ...
  * Plusieurs classes synthDriverHandler.SynthSetting ont été supprimées.
    * Auparavant conservées pour la rétrocompatibilité (#8214), maintenant considérées comme obsolètes.
    * Les pilotes qui utilisaient les classes SynthSetting doivent être mis à jour pour utiliser les classes DriverSetting.
  * Certains codes obsolètes ont été supprimés, notamment:
    * La prise en charge des listes de messages Outlook antérieurs à 2003.
    * Une classe de superposition pour le menu Démarrer classique, présent uniquement dans Windows Vista et versions antérieures.
    * Abandon de la prise en charge de Skype 7, qui ne fonctionne définitivement plus.
* Ajout d'un cadre pour créer des fournisseurs d'amélioration de la vision; modules pouvant modifier le contenu de l'écran, éventuellement en fonction des informations fournies par NVDA concernant l'emplacement des objets. (#9064)
  * Les extensions peuvent regrouper leurs propres fournisseurs dans un dossier visionEnhancementProviders.
  * Consultez les modules vision et visionEnhancementProviders pour l'implémentation du framework et les exemples respectifs.
  * Les fournisseurs d'amélioration de la vision sont activés et configurés via la catégorie 'vision' dans la boîte de dialogue des paramètres de NVDA.
...
* Introduction de displayModel.UNIT_DISPLAYCHUNK comme une unité constante spécifique de textInfos DisplayModelTextInfo. (#10165)
  * Cette nouvelle constante permet de parcourir le texte dans un DisplayModelTextInfo d'une manière qui ressemble davantage à la façon dont les fragments de texte sont enregistrés dans le modèle sous-jacent.
...
* Les modules externes (appModules et globalPlugins) ont désormais moins de chances d’interrompre la création de NVDAObjects.
  * Les exceptions causées par les méthodes "chooseNVDAObjectOverlayClasses" et "event_NVDAObject_init" sont maintenant correctement interceptées et enregistrées.
...
* Vous pouvez maintenant désactiver le mode navigation par défaut en désactivant la nouvelle option " Activer le mode navigation au chargement de la page " dans les paramètres du mode navigation de NVDA. (#8716)
  * Notez que lorsque cette option est désactivée, vous pouvez toujours activer manuellement le mode navigation en appuyant sur NVDA+espace.
...
* Les profils de configuration peuvent maintenant être activés manuellement avec des gestes. (#4209)
  * Le geste doit être configuré dans la boîte de dialogue "Gestes de commandes".
* Dans Eclipse, ajout du support pour l'auto-complétion dans l'éditeur de code. (#5667)
  * De plus, les informations Javadoc peuvent être lues depuis l'éditeur lorsqu'elles sont présentes en utilisant NVDA+d.
* Ajout d'une option expérimentale au panneau Paramètres Avancés permettant d'empêcher le focus système de suivre le curseur du mode navigation (Mettre automatiquement le focus système sur les éléments pouvant être focalisés). (#2039) Bien que cette option ne puisse pas être désactivée pour tous les sites Web, cela peut également résoudre :
  * Effet élastique: NVDA annule sporadiquement la dernière frappe en mode navigation en revenant à l'emplacement précédent.
  * Les zones d'édition capturent le focus système lors du défilement à travers elles sur certains sites web.
  * Les raccourcis clavier en mode navigation répondent lentement.
...
* Le résultat de la dernière commande exécutée dans la console NVDA Python est désormais accessible depuis la variable _ (souligné). (#9782)
  * Notez que cela masque la fonction de traduction gettext également appelée "_". Pour accéder à la fonction de traduction : del _
...
* Ajout d'une option dans les paramètres de la souris de NVDA pour que NVDA supporte les situations où la souris est contrôlée par une autre application. (#8452)
  * Cela permettra à NVDA de suivre la souris lorsqu'un système est contrôlé à distance à l'aide de TeamViewer ou d'autres logiciels de contrôle à distance.
...
* Sous Mozilla Firefox et Google Chrome, le mode navigation annonce maintenant l'élément sélectionné dans les listes déroulantes et les arborescences.
  * Cela fonctionne dans Firefox 66 et les versions ultérieures.
  * Cela ne fonctionne pas pour certaines zones de liste (Contrôles de sélection HTML) dans Chrome.
...
* NVDA ne charge plus automatiquement les appModules personnalisés, les modules globaux et les pilotes braille ou de synthétiseur depuis le répertoire de configuration utilisateur de NVDA. Ce code devrait plutôt être packagé comme une extension avec une information de version correcte, pour être sûr que du code incompatible n'est pas exécuté avec la version courante de NVDA. (#9238)
  * Pour les développeurs qui doivent tester du code pendant son développement, activez le répertoire du bloc-notes du développeur de NVDA dans la catégorie Avancé des paramètres de NVDA, et placez votre code dans le répertoire 'scratchpad' situé dans le répertoire de configuration utilisateur de NVDA lorsque cette option est activée.
...
* Les auteurs d'extensions peuvent maintenant fournir des informations de compatibilité de version de NVDA dans le manifests de l'extension. (#6275, #9055)
  * minimumNVDAVersion : La version minimale de NVDA requise pour qu'une extension fonctionne correctement.
  * lastTestedNVDAVersion : La dernière version de NVDA avec laquelle une extension a été testée.
...
* gui.nvdaControls contient maintenant deux classes pour créer des listes accessibles avec des cases à cocher. (#7325)
  ...
  * AutoWidthColumnCheckListCtrl ajoute des cases à cocher accessibles à un AutoWidthColumnListCtrl, qui est lui-même basé sur wx.ListCtrl.
...
* Ajout de la détection automatique des terminaux braille en arrière-plan. (#1271)
  * Les plages braille ALVA, Baum/HumanWare/APH/Orbit, Eurobraille, Handy Tech, Hims, SuperBraille et HumanWare BrailleNote et Brailliant BI/B sont actuellement prises en charge.
  * Vous pouvez activer cette fonctionnalité en sélectionnant l'option automatique dans la liste des plages braille dans la boîte de dialogue de sélection de l'afficheur braille de NVDA.
...
* Plusieurs améliorations du support braille sous Microsoft Word avec UI Automation activé:
  * Comme pour les autres champs de texte multiligne, lorsqu'on se trouve au début d'un document en Braille, l'affichage est maintenant déplacé de manière à ce que le premier caractère du document se trouve au début de l'affichage. (#8406)
  * Réduction de la présentation trop verbeuse du focus à la fois en parole et en braille lors de la mise en focus d'un document Word. (#8407)
  * Le routage du curseur en braille fonctionne désormais correctement dans une liste d'un document Word. (#7971)
  * Les puces/numéros nouvellement insérés dans un document Word sont correctement annoncés à la fois en braille et vocalement. (#7970)
...
* Quelques changements ont été apportés au module hwPortUtils : (#1271)
  * listUsbDevices renvoie maintenant des dictionnaires contenant des informations sur les appareils, y compris le hardwareID et le devicePath.
  * Les dictionnaires renvoyés par listComPorts contiennent désormais une entrée usbID pour les ports COM avec des informations VID/PID USB dans leur identifiant matériel.
...
* Vous pouvez maintenant demander à être notifié des actions de sauvegardes/réinitialisations de configuration via les nouvelles actions config.pre_configSave, config.post_configSave, config.pre_configReset, et config.post_configReset. (#7598)
  * config.pre_configSave est utilisé pour être notifié lorsque la configuration de NVDA est sur le point d'être sauvegardée, et config.post_configSave est appelé après que la configuration a été sauvegardée.
  * config.pre_configReset et config.post_configReset incluent un indicateur de valeurs par défaut d'usine pour spécifier si les paramètres sont rechargés depuis le disque (false) ou réinitialisés aux valeurs par défaut (true).
...
* Vous pouvez maintenant utiliser contrôle, majuscule, alt, windows et NVDA depuis votre clavier braille et combiner ces modifieurs avec la saisie braille (ex : presser contrôle+s). (#7306)
  * Vous pouvez assigner ces nouveaux bascules de modificateurs en utilisant les commandes trouvées sous Touches émulées du clavier dans la boîte de dialogue des gestes de commandes.
...
* Il est maintenant possible d'interagir avec les diagrammes sous Microsoft Word et Microsoft Powerpoint, de la même manière que sous Microsoft Excel. (#7046)
  * Dans Microsoft Word : Lorsque vous êtes en mode navigation, déplacez le curseur vers un graphique intégré et appuyez sur Entrée pour interagir avec celui-ci.
  * Dans Microsoft PowerPoint lors de l'édition d'une diapositive : appuyez sur Tab pour atteindre un objet graphique, puis appuyez sur Entrée ou Espace pour interagir avec le graphique.
  * Pour arrêter d'interagir avec un graphique, appuyez sur Échap.
...
* Il est maintenant possible d'utiliser la saisie braille avec un terminal ALVA 640 Comfort braille. (#7733)
  * La fonctionnalité de saisie braille de NVDA peut être utilisée avec ceux-ci ainsi qu'avec d'autres afficheurs BC6 ayant un firmware 3.0.0 et supérieur.
...
* Les pilotes pour les terminaux HIMS Braille Sense/Braille EDGE/Smart Beetle et Hims Sync Braille ont été remplacés par un pilote unique. Le nouveau pilote sera automatiquement activé pour les utilisateurs des anciens pilotes. (#7459)
  * Certaines touches, notamment les touches de défilement, ont été réaffectées pour suivre les conventions utilisées par les produits Hims. Consultez le guide de l'utilisateur pour plus de détails.
* Quand vous tapez sur le clavier à l'écran via l'interaction tactile, par défaut vous devez maintenant toucher deux fois chaque touche comme vous le feriez pour activer tout autre contrôle.
  * Pour utiliser le mode existant de "saisie tactile" où il suffit de lever le doigt de la touche pour l'activer, activez cette option dans la nouvelle boîte de dialogue des paramètres d'interaction tactile accessible depuis le menu Préférences.
* Il n'est plus nécessaire de joindre le braille au focus ou au curseur de revue explicitement car cela se fera automatiquement par défaut. (#2385)
  * Notez que l'attachement automatique à la revue ne se produira que lors de l'utilisation d'une commande de curseur de revue ou de navigation par objet. Le défilement n'activera pas ce nouveau comportement.
...
* Ajout d'un indicateur booléen caché à la section braille dans la configuration : "outputPass1Only". (#7301, #7693, #7702)
  * Ce paramètre est défini sur true par défaut. S'il est false, les règles multi-passes de liblouis seront utilisées pour la sortie braille.
...
* Les gestes pour afficheur braille qui émulent des modificateurs de touches du clavier du système (comme contrôle et alt) peuvent maintenant être combinées avec d'autres touches émulées sans définition explicite. (#6213)
  * Par exemple, si vous avez une touche sur votre plage braille liée à la touche alt et une autre touche de l'afficheur à flècheBas, la combinaison de ces touches entraînera l'émulation de alt+flècheBas.
* La classe braille.BrailleDisplayGesture possède maintenant une propriété model. si elle est fournie, l'appui d'une touche générera un identificateur additionnel de geste spécifique au modèle de terminal. Cela permet à l'utilisateur d'affecter des gestes commande spécifiques limités à un modèle de terminal.
  * Consultez le pilote baum comme exemple pour cette nouvelle fonctionnalité.
...
* NVDA peut maintenant utiliser la fonctionnalité de reconnaissance de caractères incluse dans Windows 10 pour reconnaître le texte d'images ou d'applications inaccessibles. (#7361)
  * La langue peut être définie à partir de la nouvelle boîte de dialogue OCR Windows 10 dans les Préférences de NVDA.
  * Pour reconnaître le contenu de l'objet navigateur actuel, appuyez sur NVDA+r.
  * Consultez la section Reconnaissance de contenu du Guide de l'utilisateur pour plus de détails.
* Vous pouvez maintenant choisir les informations contextuelles affichées sur un terminal braille quand un objet prend le focus en utilisant le nouveau paramètre "Présentation contextuelle du focus" dans le dialogue Paramètres Braille. (#217)
  * Par exemple, les options "Remplir l'affichage pour les changements de contexte" et "Seulement lors du défilement vers l'arrière" peuvent rendre le travail avec les listes et les menus plus efficace, car les éléments ne changeront pas continuellement de position sur l'affichage.
  * Consultez la section sur le paramètre "Présentation du contexte de focus" dans le Guide de l'utilisateur pour plus de détails et d'exemples.
...
* Par défaut, NVDA n'affiche maintenant que la partie de l'information contextuelle qui a changé quand un objet prend le focus. (#217)
  * Auparavant, il montrait toujours autant d'informations de contexte que possible, indépendamment du fait que vous ayez vu les mêmes informations de contexte auparavant.
  * Vous pouvez revenir à l'ancien comportement en modifiant le nouveau paramètre "Présentation du contexte de focus" dans la boîte de dialogue Paramètres Braille sur "Toujours remplir l'affichage".
...
* NVDA doit maintenant être construit avec Visual Studio Community 2015. Visual Studio Express n'est plus supporté. (#7110)
  * Les outils et SDK Windows 10 sont maintenant également requis, qui peuvent être activés lors de l'installation de Visual Studio.
  * Consultez la section Dépendances installées du readme pour plus de détails.
...
* La propriété InputGesture.identifiers a été modifiée de manière à ne plus être normalisée. (#6945)
  * Les sous-classes n'ont plus besoin de normaliser les identifiants avant de les renvoyer depuis cette propriété.
  * Si vous voulez des identifiants normalisés, il existe maintenant une propriété InputGesture.normalizedIdentifiers qui normalise les identifiants renvoyés par la propriété identifiers.
...
* Du code obsolète a été supprimé :
  * Constantes `speech.REASON_*` : `controlTypes.REASON_*` doit être utilisé à la place. (#6846)
  * `i18nName` pour les paramètres de synthèse : `displayName` et `displayNameWithAccelerator` doivent être utilisés à la place. (#6846, #5185)
  * `config.validateConfig`. (#6846, #667)
  * `config.save` : `config.conf.save` doit être utilisé à la place. (#6846, #667)
...
* Il y a maintenant une architecture de test unitaire pour NVDA. (#7026)
  * Les tests unitaires et l'infrastructure sont situés dans le répertoire tests/unit. Consultez la docstring dans le fichier tests\unit\init.py pour plus de détails.
  * Vous pouvez exécuter les tests en utilisant "scons tests". Consultez la section "Running Tests" de readme.md pour plus de détails.
  * Si vous soumettez une pull request pour NVDA, vous devriez d'abord exécuter les tests et vous assurer qu'ils réussissent.
...
* Le constructeur de nvwave.WavePlayer possède maintenant un mot-clé argument wantDucking qui spécifie si l'audio en arrière-plan devrait être atténuée durant l'émission de l'audio. (#3830)
  * Lorsque ceci est activé (ce qui est le cas par défaut), il est essentiel que WavePlayer.idle soit appelé au moment approprié.
* Entrées/sorties améliorées pour les afficheurs braille : (#5609)
  * Les pilotes d'affichage braille thread-safe peuvent se déclarer comme tels en utilisant l'attribut BrailleDisplayDriver.isThreadSafe. Un pilote doit être thread-safe pour bénéficier des fonctionnalités suivantes.
  * Les données sont écrites sur les pilotes d'affichage braille thread-safe en arrière-plan, améliorant ainsi les performances.
  * hwIo.Serial étend pyserial pour appeler un callable quand des données sont reçues, au lieu que les pilotes aient à interroger.
  * hwIo.Hid fournit une prise en charge pour les plages braille communiquant via USB HID.
  * hwPortUtils et hwIo peuvent éventuellement fournir une journalisation de débogage détaillée, incluant les périphériques trouvés et toutes les données envoyées et reçues.
* plusieurs nouvelles propriétés sont accessible depuis les gestes sur un écran tactile : (#5652)
  * Les objets MultitouchTracker contiennent maintenant une propriété childTrackers qui contient les MultiTouchTrackers dont le tracker était composé. Par exemple, un double tapotement à 2 doigts a des trackers enfants pour deux tapotements à 2 doigts. Les tapotements à 2 doigts eux-mêmes ont des trackers enfants pour deux tapotements.
  * Les objets MultiTouchTracker contiennent maintenant également une propriété rawSingleTouchTracker si le tracker était le résultat d'un seul doigt effectuant un tapotement, un glissement ou un survol. Le SingleTouchTracker permet d'accéder à l'ID sous-jacent attribué au doigt par le système d'exploitation et de savoir si le doigt est toujours en contact au moment actuel.
  * Les TouchInputGestures ont maintenant des propriétés x et y, supprimant la nécessité d'accéder au tracker pour des cas triviaux.
  * Les TouchInputGestures contiennent maintenant une propriété preheldTracker, qui est un objet MultitouchTracker représentant les autres doigts maintenus pendant que cette action était effectuée.
* Deux nouveaux gestes tactiles peuvent être émis : (#5652)
  * Tapotements et maintenus multiples (par exemple double tapotement et maintien)
  * Un identifiant généralisé avec le nombre de doigts supprimé pour les maintenus (par exemple hold+hover pour 1finger_hold+hover).
...
* Support de l'édition du texte enrichi sous Google Chrome et les navigateurs basés sur Chrome tels que Google Docs avec le support braille activé. (#2634)
  * Cela nécessite Chrome version 47 ou ultérieure.
...
* Sous Windows 8 et au-delà, NVDA démarre maintenant beaucoup plus tôt quand il est configuré pour démarrer après la connexion à Windows. (#308)
  * Si vous avez activé cela en utilisant une version précédente de NVDA, vous devrez le désactiver et le réactiver pour que le changement prenne effet. Suivez cette procédure :
    1. Ouvrez la boîte de dialogue des paramètres généraux.
    1. Décochez la case Démarrer automatiquement NVDA après ma connexion à Windows.
    1. Appuyez sur le bouton OK.
    1. Ouvrez à nouveau la boîte de dialogue des paramètres généraux.
    1. Cochez la case Démarrer NVDA après ma connexion.
    1. Appuyez sur le bouton OK.
...
* Plutôt qu'un simple attribut i18nName, synthDriverHandler.SynthSetting possède maintenant un displayNameWithAccelerator séparé et des attributs displayName pour éviter l'annonce du raccourci dans la boucle des paramètres synthétiseur dans certaines langues.
  * Pour la rétrocompatibilité, dans le constructeur, displayName est facultatif et sera dérivé de displayNameWithAccelerator s'il n'est pas fourni. Cependant, si vous avez l'intention d'avoir un accélérateur pour un paramètre, les deux doivent être fournis.
  * L'attribut i18nName est obsolète et pourrait être supprimé dans une future version.
...
* Des commandes de parole ont été implémentées pour insérer une pause entre les mots et pour changer la hauteur, le volume et le débit. Voir les commandes BreakCommand, PitchCommand, VolumeCommand et RateCommand dans le module speech. (#4674)
  * Il existe également speech.PhonemeCommand pour insérer une prononciation spécifique, mais les implémentations actuelles ne prennent en charge qu'un nombre très limité de phonèmes.
...
* Dans Microsoft Word, les en-têtes de colonnes de tableaux sont annoncés automatiquement pour les tableaux où la ligne d'en-têtes a été explicitement spécifiée par l'auteur via les propriétés de tableau de Microsoft Word. (#4510)
  * Cependant, pour les tableaux où des lignes ont été fusionnées, cela ne fonctionnera pas automatiquement. Dans cette situation, vous pouvez toujours définir manuellement les en-têtes de colonne dans NVDA avec NVDA+maj+c.
...
* Il est maintenant possible de redémarrer NVDA normalement ou avec les modules complémentaires désactivés depuis le dialogue d'arrêt de NVDA. (#4057)
  * NVDA peut également être démarré avec les extensions désactivées en utilisant l'option de ligne de commande --disable-addons.
...
* Pour les exécutables qui hébergent beaucoup d'applications différentes (ex : javaw.exe), du code peut maintenant être fourni pour charger des modules spécifiques pour chaque application au lieu de charger le même module pour toutes les applications hébergées. (#4360)
  * Consultez la documentation du code pour appModuleHandler.AppModule pour plus de détails.
  * La prise en charge de javaw.exe est implémentée.
...
* Support du Calendrier sous Microsoft Outlook 2007 et plus récent (#2943) incluant :
  * Annonce de l'heure actuelle lors des déplacements avec les touches fléchées.
  * Indication si l'heure sélectionnée se trouve dans un rendez-vous.
  * annonce du rendez-vous sélectionné lors de l'appui sur la touche tab.
  * Filtrage intelligent de la date afin d'annoncer la date uniquement si la nouvelle heure sélectionnée ou le nouveau rendez-vous est à un jour différent du précédent.
* Support amélioré de la boîte de réception et des autres listes de messages sous Outlook 2010 et plus récent (#3834) incluant :
  * La possibilité de désactiver l'annonce des en-têtes de colonnes (de, objet, etc.) en désactivant l'option Annoncer les en-têtes de lignes et de colonnes des tableaux dans les paramètres de Mise en forme des documents.
  * La possibilité d'utiliser les commandes de navigation dans les tableaux (contrôle + alt + flèches) pour se déplacer à travers les colonnes.
...
* Support amélioré de la lecture automatique des titres de lignes et de colonnes sous Microsoft Excel (#3568) incluant :
  * Prise en charge des plages de noms définis dans Excel pour identifier les cellules d'en-tête (compatible avec le lecteur d'écran Jaws).
  * Les commandes définir l'en-tête de colonne (NVDA+maj+c) et définir l'en-tête de ligne (NVDA+maj+r) stockent désormais les paramètres dans la feuille de calcul afin qu'ils soient disponibles lors de la prochaine ouverture de la feuille et seront accessibles à d'autres lecteurs d'écran qui prennent en charge le système de plages de noms définis.
  * Ces commandes peuvent désormais être utilisées plusieurs fois par feuille pour définir différents en-têtes pour différentes zones.
...
* Microsoft Excel : retour vocal et braille pour beaucoup plus de raccourcis de déplacement de sélection (#4211) incluant :
  * Déplacement vertical de page (pageUp et pageDown);
  * Déplacement horizontal de page (alt+pageUp et alt+pageDown);
  * Extension de la sélection (les touches ci-dessus avec Maj ajoutée); et
  * Sélection de la région actuelle (contrôle+maj+8).
...
* Microsoft Powerpoint : quand on déplace des formes dans une diapositive avec les flèches, la position courante de la forme est maintenant annoncée (#4214) incluant :
  * La distance entre la forme et chacun des bords de la diapositive est annoncée.
  * Si la forme couvre ou est couverte par une autre forme, alors la distance de chevauchement et la forme chevauchée sont annoncées.
  * Pour signaler ces informations à tout moment sans déplacer une forme, appuyez sur la commande d'annonce de l'emplacement (NVDA+suppr).
  * Lors de la sélection d'une forme, si elle est couverte par une autre forme, NVDA signalera qu'elle est masquée.
* La commande d'annonce de position (NVDA+effacement) est plus spécifique au contexte dans certaines situations. (#4219):
  * Dans les champs d'édition standard et en mode navigation, la position du curseur en pourcentage dans le contenu et ses coordonnées à l'écran sont annoncées.
  * Sur les formes dans les présentations PowerPoint, la position de la forme par rapport à la diapositive et aux autres formes est annoncée.
  * Appuyer deux fois sur cette commande produira le comportement précédent d'annoncer les informations d'emplacement pour l'ensemble du contrôle.
...
* Il est maintenant possible de changer le raccourci clavier utilisé pour démarrer NVDA pour les localisations où le raccourci normal pose problème. (#2209)
  * Cela est fait via gettext.
  * Notez que le texte pour l'option Créer un raccourci sur le bureau dans la boîte de dialogue d'installation de NVDA, ainsi que la touche de raccourci dans le Guide de l'utilisateur, doivent également être mis à jour.
...
* Amélioration du support des ARIA Live Regions dans Firefox et les autres applications basées sur Mozilla Gecko :
  * Prise en charge des mises à jour aria-atomic et filtrage des mises à jour aria-busy. (#2640)
  * Le texte alternatif (comme l'attribut alt ou aria-label) est inclus s'il n'y a pas d'autre texte utile. (#3329)
  * Les mises à jour de région dynamique ne sont plus désactivées si elles se produisent en même temps que le déplacement du focus. (#3777)
...
* Plusieurs correctifs pour le support des applications Java accessibles :
  * Le contrôle initialement focalisé dans un cadre ou une boîte de dialogue n'échoue plus à être annoncé lorsque le cadre ou la boîte de dialogue passe au premier plan. (#3753)
  * Les informations de position inutiles ne sont plus annoncées pour les boutons radio (par exemple 1 sur 1). (#3754)
  * Meilleure annonce des contrôles JComboBox (html n'est plus annoncé, meilleure annonce des états développé et réduit). (#3755)
  * Lors de l'annonce du texte des boîtes de dialogue, certains textes qui étaient auparavant manquants sont maintenant inclus. (#3757)
  * Les modifications du nom, de la valeur ou de la description du contrôle focalisé sont maintenant annoncées plus précisément. (#3770)
...
* Dans la console Python, on peut maintenant appuyer sur Tab pour avoir une auto-complétion. (#433)
  * S'il existe plusieurs possibilités, vous pouvez appuyer une seconde fois sur la touche tab pour choisir à partir d'une liste.
...
* Le mode revue à plat a été remplacé par les modes revue d'objet, de document ou d'écran. (#2996)
  * La revue d'objet examine le texte uniquement dans l'objet navigateur, la revue de document examine tout le texte dans un document en mode navigation (s'il y en a) et la revue de l'écran examine le texte sur l'écran pour l'application actuelle.
  * Les commandes qui déplaçaient auparavant vers/depuis la revue basculent maintenant entre ces nouveaux modes de revue.
  * L'objet navigateur suit automatiquement le curseur de revue de sorte qu'il reste l'objet le plus profond à la position du curseur de revue lorsqu'il est en modes de revue de document ou de l'écran.
  * Après avoir basculé en mode revue de l'écran, NVDA restera dans ce mode jusqu'à ce que vous reveniez explicitement au mode revue de document ou d'objet.
  * En mode revue de document ou d'objet, NVDA peut automatiquement basculer entre ces deux modes selon que vous vous déplacez ou non dans un document en mode navigation.
...
* L'inclusion du module bisect dans les binaires est maintenant dépréciée et pourrait être supprimée dans une future version. (#3368)
  * Les extensions qui dépendent de bisect (y compris le module urllib2) devraient être mis à jour pour inclure ce module.
...
* Les afficheurs Braille peuvent maintenant supporter la sélection manuelle du port. (#426)
  * Cela est particulièrement utile pour les anciens afficheurs braille qui prennent en charge la connexion via un port série.
  * Cela se fait en utilisant la méthode de classe getPossiblePorts sur la classe BrailleDisplayDriver.
* La saisie en braille via un clavier braille est maintenant supportée. (#808)
  * La saisie braille est englobée par la classe brailleInput.BrailleInputGesture ou une sous-classe de celle-ci.
  * Les sous-classes de braille.BrailleDisplayGesture (telles qu'implémentées dans les pilotes d'affichage braille) peuvent également hériter de brailleInput.BrailleInputGesture. Cela permet aux commandes d'affichage et à la saisie braille d'être gérées par la même classe de geste.
...
* NVDA supporte maintenant la saisie des caractères asiatiques en utilisant les méthodes IME et les services de saisie de texte dans toutes les applications, incluant :
  * Annonce et navigation des listes de candidats;
  * Annonce et navigation des chaînes de composition ; et
  * Annonce des chaînes de lecture.
...
* Support expérimentale des écrans tactiles sous Windows 8, incluant :
  * Lecture du texte directement sous votre doigt lorsque vous le déplacez
  * De nombreux gestes pour effectuer la navigation par objet, la revue de texte et d'autres commandes NVDA.
...
* Dans Adobe Reader, les numéros de pages peuvent maintenant être rapportés. (#2534)
  * Dans Reader XI, les étiquettes de page sont annoncées lorsqu'elles sont présentes, reflétant les changements de numérotation des pages dans différentes sections, etc. Dans les versions antérieures, cela n'est pas possible et seuls les numéros de page séquentiels sont annoncés.
...
* Les messages de contexte Gettext sont maintenant supportés. Cela permet de pouvoir définir plusieurs traductions pour un message en Anglais selon le contecte. (#1524)
  * Cela se fait en utilisant la fonction pgettext(contexte, message).
  * Cela est pris en charge à la fois pour NVDA lui-même ainsi que pour les extensions.
  * xgettext et msgfmt de GNU gettext doivent être utilisés pour créer tous les fichiers PO et MO. Les outils Python ne prennent pas en charge les contextes de message.
  * Pour xgettext, passez l'argument de ligne de commande --keyword=pgettext:1c,2 pour permettre l'inclusion des contextes de message.
  * Voir http://www.gnu.org/software/gettext/manual/html_node/Contexts.html#Contexts pour plus d'informations.
...
* Dans les documents Microsoft Word, beaucoup plus d'informations sont maintenant annoncées, parmi lesquelles :
  * Informations en ligne telles que les numéros de notes de bas de page et de fin, les niveaux de titre, l'existence de commentaires, les niveaux d'imbrication des tableaux, les liens et la couleur du texte;
  * Annonce lors de l'entrée dans des sections de document telles que l'historique des commentaires, l'historique des notes de bas de page et de fin, et l'historique des en-têtes et pieds de page.
...
* Les objets Textinfo ont maintenant une méthode activate() pour activer la position représentée par le Textinfo.
  * Cela est utilisé par le braille pour activer la position en utilisant les touches de routage du curseur sur un afficheur braille. Cependant, il pourrait y avoir d'autres appelants à l'avenir.
...
* Plusieurs contrôles et valeurs de constantes ont été renommés ou déplacés. (#228)
  ...
  * Dans controlTypes, speechRoleLabels et speechStateLabels ont été renommés respectivement en roleLabels et stateLabels.
...
* NVDA peut maintenant dire aux synthétiseurs de changer de langue pour certaines portions de texte.
  * Pour prendre cela en charge, les pilotes doivent gérer speech.LangChangeCommand dans les séquences passées à SynthDriver.speak().
  * Les objets SynthDriver doivent également fournir l'argument de langue aux objets VoiceInfo (ou remplacer l'attribut language pour récupérer la langue actuelle). Sinon, la langue de l'interface utilisateur de NVDA sera utilisée.
...
* Les pilotes des synthèses vocales adoptent maintenant une séquence de commandes en mode texte et la parole lors de la synthétisation, et non seulement du texte et un index.
  * Cela permet d'intégrer des index, des changements de paramètres, etc.
  * Les pilotes doivent implémenter SynthDriver.speak() au lieu de SynthDriver.speakText() et SynthDriver.speakCharacter().
  * Les anciennes méthodes seront utilisées si SynthDriver.speak() n'est pas implémenté, mais elles sont obsolètes et seront supprimées dans une future version.
...
* gui.scriptUI a été supprimé.
  ...
  * Pour toutes les autres boîtes de dialogue, de vraies boîtes de dialogue wx devraient être utilisées à la place.
  * Une nouvelle fonction gui.runScriptModalDialog() simplifie l'utilisation des boîtes de dialogue modales depuis des scripts.
...
* Les couleurs peuvent maintenant être annoncées pour certains contrôles. L'annonce automatique peut être configurée dans le dialogue Mise en forme des documents du menu Préférences. Elles peuvent aussi être annoncées à la demande grâce à la commande NVDA+f.
  * Initialement, cela est pris en charge dans les contrôles de texte modifiables IAccessible2 standard (comme dans les applications Mozilla), les contrôles RichEdit (comme dans Wordpad) et les contrôles de texte IBM Lotus Symphony.
...
* Toute la frappe venant de l'utilisateur est maintenant représentée par une instance de inputCore.InputGesture. (#601)
  * Chaque source d'entrée est une sous-classe de la classe de base InputGesture.
  * Les touches pressées sur le clavier du système sont englobées par la classe keyboardHandler.KeyboardInputGesture.
  * Les appuis sur les boutons, molettes et autres contrôles sur un afficheur braille sont englobés par des sous-classes de la classe braille.BrailleDisplayGesture. Ces sous-classes sont fournies par chaque pilote d'afficheur braille.
...
* Tous les scripts prennent maintenant une instance de InputGesture au lieu d'une pression de touche.
  * Les KeyboardInputGestures peuvent être envoyés au système d'exploitation en utilisant la méthode send() du geste.
...
* Les locales peuvent maintenant fournir un fichier de mappage de gestes d'entrée pour que l'on puisse ajouter de nouvelles affectations ou remplacer des affectations existantes pour les scripts n'importe où dans NVDA. (#810)
  * Les configurations de gestes de commandes locales doivent être placées dans locale\LANG\gestures.ini, où LANG est le code de langue.
  * Consultez inputCore.GlobalGestureMap pour les détails du format de fichier.
* Les nouveaux comportements LiveText et Terminal NVDAObject facilitent l'annonce automatique du nouveau texte. Voir ces classes dans NVDAObjects.behaviors pour plus de détails. (#936)
  * La classe de superposition NVDAObjects.window.DisplayModelLiveText peut être utilisée pour les objets qui doivent récupérer le texte écrit sur l'affichage.
  * Consultez les AppModules mirc et putty pour des exemples d'utilisation.
...
* Les objets SynthDriver fournissent maintenant un attribut langue qui spécifie la langue de la voix en cours.
  * L'implémentation de base utilise la langue spécifiée sur les objets VoiceInfo dans availableVoices. Cela convient à la plupart des synthétiseurs qui prennent en charge une langue par voix.
* Les pilotes d'afficheurs braille ont été améliorés pour permettre aux boutons, roulettes et autres contrôles d'être reconnus par les scripts NVDA :
  * Les pilotes peuvent fournir une configuration globale de gestes de commandes pour ajouter des liaisons pour des scripts n'importe où dans NVDA.
  * Ils peuvent également fournir leurs propres scripts pour effectuer des fonctions spécifiques à l'affichage.
  * Consultez braille.BrailleDisplayDriver pour plus de détails et les pilotes d'affichage braille existants pour des exemples.
...
* Tous les pilotes d'afficheurs braille devraient maintenant utiliser braille.BrailleDisplayDriver au lieu de braille.BrailleDisplayDriverWithCursor.
  * Le curseur est maintenant géré en dehors du pilote.
  * Les pilotes existants doivent seulement modifier leur déclaration de classe en conséquence et renommer leur méthode _display en display.
...
* NVDA peut maintenant accéder à la plupart du texte écrit à l'écran. (#40, #643)
  * Cela permet la lecture des contrôles qui n'exposent pas d'informations de manière plus directe/fiable.
  * Les contrôles rendus accessibles par cette fonctionnalité comprennent : certains éléments de menu qui affichent des icônes (par exemple, le menu Ouvrir avec sur les fichiers dans Windows XP) (#151), les champs de texte modifiables dans les applications Windows Live (#200), la liste des erreurs dans Outlook Express (#582), le contrôle de texte modifiable dans TextPad (#605), les listes dans Eudora, de nombreux contrôles dans l'E-tax australien et la barre de formule dans Microsoft Excel.
...
* Dans les tableaux, les titres de lignes ou de colonnes peuvent maintenant être annoncés. Ceci est configurable depuis le dialogue Mise en forme des documents du menu Préférences.
  * Actuellement, cela est pris en charge dans les documents des applications Mozilla telles que Firefox (version 3.6.11 et ultérieure) et Thunderbird (version 3.1.5 et ultérieure). (#361)
* Introduction de commandes pour la revue à plat : (#58)
  * NVDA+pavnum7 bascule vers la revue, plaçant le curseur de revue à la position de l'objet actuel, vous permettant d'examiner l'écran (ou un document si vous êtes dans l'un d'eux) avec les commandes de revue de texte.
  * NVDA+pavnum1 déplace le curseur de revue dans l'objet représenté par le texte à la position du curseur de revue, vous permettant de naviguer par objet à partir de ce point.
...
* NVDA dispose maintenant d'une synthèse vocale intégrée appelée eSpeak, développée par Jonathan Duddington. Elle est très réactive, légère et elle prend en charge beaucoup de langues différentes. Les synthèses vocales Sapi peuvent encore être utilisées, mais eSpeak sera utilisée par défaut.
  * eSpeak ne dépend d'aucun logiciel spécial à installer, il peut donc être utilisé avec NVDA sur n'importe quel ordinateur, sur une clé USB, ou n'importe où.
  * Pour plus d'informations sur eSpeak, ou pour trouver d'autres versions, visitez http://espeak.sourceforge.net/.
...
* Ajout d'arguments en ligne de commande à NVDA :
  * -m, --minimal : ne pas jouer les sons de démarrage/sortie et ne pas afficher l'interface au démarrage si configuré ainsi.
  * -q, --quit : quitter toute autre instance de NVDA déjà en cours d'exécution puis quitter
  * -s, --stderr-file nomFichier : spécifier où NVDA doit placer les erreurs et exceptions non interceptées
  * -d, --debug-file nomFichier : spécifier où NVDA doit placer les messages de débogage
  * -c, --config-file : spécifier un fichier de configuration alternatif
  * -h, -help : afficher un message d'aide listant les arguments de ligne de commande
