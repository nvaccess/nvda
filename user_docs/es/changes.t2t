...
% Inserta nuevos elementos de lista aquí según la tabla de alias de appModule debería mantenerse al fondo de esta lista
...
- ``gui.quit`` ha sido puesto como obsoleto. Utiliza ``wx.CallAfter(mainFrame.onExitCommand, None)`` directamente en su lugar.
  -
% Inserta nuevos elementos de lista aquí como el alias appModule la tabla debería permanecer en la parte inferior de esta lista
...
- Un ScriptableObject roto (tal como un GlobalPlugin que pierda una llamada a su clase base ' método __init__ ) ya no rompe el manejo de scripts de NVDA. (#5446)
