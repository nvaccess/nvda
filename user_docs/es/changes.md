...
Las nuevas convenciones sugeridas son:
...
* `gui.quit` ha sido puesto como obsoleto para eliminarlo en 2023.1. (#13498)
  * Utiliza `wx.CallAfter(mainFrame.onExitCommand, None)` directamente en su lugar.
...
* Se ha eliminado la asignación `controlTypes.isCurrentLabels`. (#11782)
  * En su lugar, utiliza la propiedad `displayString` en un valor del enum `controlTypes.IsCurrent`. EG `controlTypes.IsCurrent.YES.displayString`
* La propiedad `NVDAObject` (y derivados) ahora devuelve estrictamente `controlTypes.IsCurrent`. (#11782)
  * `isCurrent` ya no es Opcional, y por tanto no devolverá None, cuando un objeto no es actual `controlTypes.IsCurrent.NO` se devuelve.
* Se ha eliminado `controlTypes.isCurrentLabels`, en su lugar se utiliza la propiedad `displayString` en un valor del enum `controlTypes.IsCurrent`. (#11782)
  * Ej.: `controlTypes.IsCurrent.YES.displayString`
