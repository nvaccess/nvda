# NVDA NVDA_VERSION:n käyttöopas
...
* Tuki yleisille sovelluksille, kuten internet-selaimille sekä sähköpostiasiakas-, internet-keskustelu- ja toimisto-ohjelmille
...
* Automaattinen hiiren alla olevan tekstin puhuminen ja valinnainen hiiren sijainnin ilmaiseminen piippauksilla
...
* Mahdollisuus [yhdistää toiseen NVDA:ta käyttävään tietokoneeseen ja hallita sitä](#RemoteAccess) etätuen vastaanottamista tai yhteistyön tekemistä varten
...
* Käännökset 55 kielelle
* Tuki modernien Windows-käyttöjärjestelmien 32- ja 64-bittisille versioille
...
* Tallennustilaa vähintään 500 Mt.

#### Suositeltavat järjestelmävaatimukset {#RecommendedSystemRequirements}

* Käyttöjärjestelmät: 64-bittiset Windows 10:n, 11:n ja Server 2022:n versiot.
  * Sekä AMD64- että ARM64-versioita tuetaan.
* Tallennustilaa vähintään 500 Mt.
...
NVDA on käännetty englannin lisäksi seuraaville 55 kielelle: afrikaans, albania, amhara, arabia, aragonia, bosnia, burma, espanja (Kolumbia ja Espanja), farsi, galicia, georgia, heprea, hindi, hollanti, islanti, iiri, italia, japani, katalaani, kannada, kiina (perinteinen ja yksinkertaistettu), kirgiisi, korea, kroatia, kreikka, liettua, makedonia, mongolia, nepali, norja, portugali (Brasilia ja Portugali), puola, punjabi, ranska, romania, ruotsi, saksa (Saksa ja Sveitsi), serbia, slovakki, sloveeni, suomi, tamili, tanska, thai, tšekki, turkki, ukraina, unkari, venäjä ja vietnam.
...
### Lisenssi ja tekijänoikeustiedot {#LicenseAndCopyright}
...
Poikkeukset on esitelty lisenssidokumentin luvuissa "Non-GPL Components in Plugins and Drivers" ja "Microsoft Distributable Code".
...
Voit [tarkastella koko lisenssiä](https://www.gnu.org/licenses/old-licenses/gpl-2.0.html) saadaksesi lisätietoja.
Saat lisätietoja poikkeuksista lisenssidokumentista, jonka voit avata NVDA-valikon Ohje-alivalikosta.
...
Sinun on ehkä odotettava muutama sekunti NVDA:n tilapäisversion käynnistymistä.
Kun se on käynnistynyt, NVDA puhuu koko prosessin ajan.
1. Näyttöön tulee NVDA:n asennusohjelman ikkuna, jossa on painike lisenssisopimuksen avaamista varten.
Lue sopimus painamalla `Nuoli alas` -näppäintä.
...
Asennuksen aikana NVDA toistaa yhä korkeammaksi muuttuvan piippauksen.
Tämä prosessi on usein nopea, eikä sitä välttämättä huomaa.
1. Näyttöön tulee valintaikkuna, jossa näytetään vahvistus NVDA:n asennuksen onnistumisesta.
...
| Nimi | Näppäinkomento pöytäkoneissa | Näppäinkomento kannettavissa | Kuvaus|
|---|---|---|---|
|Puhu kirjoitetut merkit | `NVDA+2` | `NVDA+2` | Määrittää, missä tilanteessa NVDA puhuu kirjoittamasi merkit. |
|Puhu kirjoitetut sanat | `NVDA+3` | `NVDA+3` | Määrittää, missä tilanteessa NVDA puhuu kirjoittamasi sanat. |
...
Useimmat NVDA:n toiminnot voidaan ottaa käyttöön tai muuttaa NVDA:n asetuksista.
...
Mikäli et kuule mitään tällaista tai jos kuulet Windowsin virheäänen tai joukon laskevia ääniä, NVDA:ssa on mahdollisesti virhe, josta on ilmoitettava kehittäjille.
Ohjeet virheiden ilmoittamiseen löytyvät NVDA:n verkkosivustolta.
...
NVDA käyttää oletusarvoisesti pöytäkoneiden näppäinasettelua, mutta kannettaville tarkoitettuun asetteluun voidaan vaihtaa Näppäimistö-kategoriasta [NVDA:n asetukset](#NVDASettings) -valintaikkunasta, johon pääsee NVDA-valikon Mukautukset-alivalikosta.
...
* Siirtymällä vaihtoehtoisesti ilmoitusalueelle painamalla `Win+B`, `Nuoli alas` -näppäimellä NVDA-kuvakkeen kohdalle ja avaamalla pikavalikon painamalla `sovellusnäppäintä`, joka löytyy useimmista näppäimistöistä oikeanpuoleisen Ctrl-näppäimen vasemmalta puolelta.
...
|NVDA-valikko | NVDA+N | NVDA+N | Kaksoisnapautus kahdella sormella | Avaa NVDA-valikon, jonka kautta voit muuttaa asetuksia, käyttää työkaluja ja lukea ohjetta jne.|
...
* Piippaukset: NVDA korvaa normaalin puheen lyhyillä piippauksilla.

Piippaukset-tilasta voi olla hyötyä, kun päätteessä tulostuu jonkin komennon seurauksena paljon tekstiä, mutta et ole kiinnostunut sen sisällöstä vaan pelkästään siitä, että tekstin tulostuminen ruudulle jatkuu, tai muissa tilanteissa, joissa tieto tekstin ilmestymisestä ruudulle on tärkeämpää kuin itse sisältö.
...
Esimerkkeinä tällaisista voisivat olla äänen nauhoittaminen, näytön suurennuksen käyttäminen, kokous, puhelu tai vaihtoehto Piippaukset-tilalle.
...
NVDA:n avulla voidaan tutkia järjestelmää ja liikkua siinä useilla tavoilla sekä normaalia vuorovaikutusta että tekstin tarkastelua käyttäen.
...
Tästä on eniten hyötyä tilanteissa, joissa [järjestelmäkohdistin](#SystemCaret) ei ole käytettävissä, kuten  Windowsin komentokonsoleissa.
...
|Siirrä tarkastelukohdistin valinnan alkuun | `NVDA+Alt+Home` | `NVDA+Alt+Home` | Ei mitään | Siirtää tarkastelukohdistimen valitun tekstin ensimmäiseen merkkiin.|
|Siirrä tarkastelukohdistin valinnan loppuun | `NVDA+Alt+End` | `NVDA+Alt+End` | Ei mitään | Siirtää tarkastelukohdistimen valitun tekstin viimeiseen merkkiin.|
...
Hyvä keino tekstin tarkastelun peruskomentojen muistamiseen pöytäkoneen näppäinasettelua käytettäessä on ajatella niitä 3 x 3 -ruudukkona, jossa on ylhäältä alas rivi, sana ja merkki sekä vasemmalta oikealle edellinen, nykyinen ja seuraava.
...
NVDA tarjoaa käyttäjille tavan ymmärtää, missä hiiri on ruudun mittojen suhteen toistamalla hiiren nykyiset koordinaatit piippauksina.
Mitä ylempänä hiiri on ruudulla, sitä korkeampia piippaukset ovat.
...
Voit halutessasi ottaa ne käyttöön [Hiiri](#MouseSettings)-kategoriasta [Asetukset](#NVDASettings)-valintaikkunasta, johon pääsee NVDA:n Mukautukset-valikosta.
...
|Päivitä selaustila-asiakirja | NVDA+F5 | Lataa uudelleen nykyisen asiakirjan sisällön. (Hyödyllinen, jos sivulta vaikuttaa puuttuvan jotakin. Komento ei ole käytettävissä Microsoft Wordissa tai Outlookissa.)|
|Etsi | NVDA+Ctrl+F | Avaa valintaikkunan, jossa voidaan etsiä tekstiä nykyisestä asiakirjasta. Katso lisätietoja [Tekstin etsiminen](#SearchingForText) -osiosta.|
...
Kun tekstiä valitaan selaustilassa `Vaihto+Nuolinäppäimillä`, valintaa ei oletusarvoisesti tehdä varsinaisessa sovelluksessa vaan NVDA:n muodostamassa asiakirjan selaustilanäkymässä.
Tämä tarkoittaa, että valinta ei näy näytöllä, ja tekstin kopiointi `Ctrl+C`-näppäinyhdistelmällä kopioi sisällön vain yksinkertaisessa NVDA:n tuottamassa tekstimuodossa. Toisin sanoen esimerkiksi taulukoiden muotoilu tai linkit eivät kopioidu.
NVDA:ssa on kuitenkin alkuperäinen valintatila, joka voidaan ottaa käyttöön tuetuissa selaustila-asiakirjoissa (Mozilla Firefoxissa sekä missä tahansa Chromium 134:ään pohjautuvassa tai sitä uudemmassa selaimessa), mikä saa asiakirjan alkuperäisen valinnan seuraamaan NVDA:n selaustilan valintaa.
...
`NVDA+F5:n` painaminen päivittää tunnistustuloksen.
...
NVDA:ssa on joillekin sovelluksille omia lisäkomentoja tiettyjen tehtävien helpottamiseksi tai mahdollistamaan pääsyn sellaisiin toimintoihin, joiden käyttäminen ei ole ruudunlukuohjelman käyttäjille muutoin mahdollista.
...
| Nimi | Näppäinkomento | Kuvaus|
|---|---|---|
|Määritä sarakeotsikot | `NVDA+Vaihto+C` | Yksi painallus merkitsee nykyisen solun rivin ensimmäiseksi sarakeotsikoksi. Se luetaan automaattisesti liikuttaessa tämän rivin alapuolella olevien sarakkeiden välillä. Kahdesti painaminen nollaa tämän asetuksen.|
|Määritä riviotsikot | `NVDA+Vaihto+R` | Yksi painallus merkitsee nykyisen solun sarakkeen ensimmäiseksi riviotsikoksi. Se luetaan automaattisesti liikuttaessa kyseisen sarakkeen oikealla puolella olevien rivien välillä. Kahdesti painaminen nollaa tämän asetuksen.|
...
Tähän asiakirjaan lisätyt rivi- ja sarakeotsikot ovat siis automaattisesti muiden ruudunlukijoiden käyttäjien käytössä.
...
<!-- KC:beginInclude -->
Puhu kommentit kohdistimen nykyisestä kohdasta painamalla `NVDA+Alt+C`.
Kahdesti painettaessa ne näytetään selaustilassa.
<!-- KC:endInclude -->
Kaikki asiakirjan kommentit ja muut jäljitettävät muutokset voidaan näyttää myös elementtilistassa, kun tyypiksi valitaan Merkinnät.
...
Tähän työkirjaan lisätyt rivi- ja sarakeotsikot ovat siis automaattisesti muiden ruudunlukijoiden käyttäjien käytössä.
...
<!-- KC:beginInclude -->
Lue muistiinpanot nykyisestä solusta painamalla `NVDA+Alt+C`.
Kahdesti painettaessa ne näytetään selaustilassa.
...
NVDA tarjoaa parannetun tuen Poedit 3.5:lle tai sitä uudemmille versioille.
...
NVDA tarjoaa tuen Windowsin komentokonsolille, jota käytetään komentokehotteessa, PowerShellissä ja Windowsin Linux-alijärjestelmässä.
...
Useimpia asetuksia on mahdollista muuttaa valintaikkunoista, joihin pääsee NVDA-valikon Mukautukset-alivalikosta.
Monet näistä asetuksista löytyvät useita kategorioita sisältävästä [Asetukset-valintaikkunasta](#NVDASettings).
...
Kun tämä asetus on käytössä, NVDA:ta suljettaessa tulee näkyviin valintaikkuna, jossa ovat käytettävissä seuraavat vaihtoehdot:

* Sulje
* Käynnistä uudelleen
* Käynnistä uudelleen, poista lisäosat käytöstä ja ota virheenkorjausloki käyttöön
* Käynnistä uudelleen ja ota virheenkorjausloki käyttöön
* Asenna odottavat päivitykset (mikäli sellaisia on)
...
Voit asentaa odottavat päivitykset myös manuaalisesti NVDA:n sulkemisvalintaikkunasta (mikäli se on käytössä), NVDA-valikosta tai suorittaessasi Ohje-valikosta päivitystarkistuksen.

##### Päivityspeilipalvelin {#UpdateMirror}

Näiden säätimien avulla voit määrittää vaihtoehtoisen URL-osoitteen, jota käytetään NVDA-päivitysten tarkistamiseen.
Tästä voi olla hyötyä tilanteissa, joissa yhteys NV Accessin NVDA-päivityspalvelimeen on hidas tai sitä ei ole lainkaan.

Nykyisen peilipalvelimen URL-osoite näytetään vain luku -tyyppisessä tekstikentässä.
Mikäli peilipalvelinta ei käytetä (ts. käytössä on NV Accessin päivityspalvelin), kentässä näytetään teksti "Ei peilipalvelinta".

Jos haluat vaihtaa päivityspeilipalvelinta, avaa [Määritä päivityspeilipalvelin -valintaikkuna](#SetURLDialog) painamalla "Vaihda..."-painiketta.

Huom: Päivityspeilipalvelinta käytettäessä sen ylläpitäjällä on pääsy kaikkeen [päivitystarkistusten yhteydessä lähetettävään tietoon](#GeneralSettingsCheckForUpdates).
Ota yhteyttä peilipalvelimen ylläpitäjään saadaksesi lisätietoja tietojenkäsittelykäytännöistä ja varmistaaksesi, että olet tyytyväinen siihen, miten tietojasi käsitellään, ennen kuin otat kyseisen palvelimen käyttöön.

##### Estä näytön sammuminen jatkuvassa luvussa tai pistenäytöllä luettaessa {#PreventDisplayTurningOff}

Tämä asetus varmistaa käytössä ollessaan, että näyttö pysyy päällä jatkuva luku -toimintoa käytettäessä tai pistenäytöllä luettaessa (esim. vierityspainikkeita painamalla).
Tämä estää tilanteen, jossa näyttö lukittuu odottamattomasti jatkuvaa lukua käytettäessä.
Asetus on oletusarvoisesti käytössä.
Harkitse asetuksen käytöstä poistamista, mikäli akkukesto lyhenee.
...
Puhe-asetuskategorian ensimmäinen vaihtoehto on "Vaihda..."-painike.
Tämä painike avaa [Valitse syntetisaattori](#SelectSynthesizer) -valintaikkunan, josta valitaan käytettävä puhesyntetisaattori.
...
##### Tehostettu puhenopeus {#SpeechSettingsRateBoost}
...
Jos isojen kirjainten äänenkorkeuden muuttamista ei tueta, harkitse sen sijaan asetusten [Ilmaise isot kirjaimet sanomalla "iso"](#SpeechSettingsSayCapBefore) ja/tai [Ilmaise isot kirjaimet piippauksella](#SpeechSettingsBeepForCaps) käyttämistä.
...
##### Ilmaise isot kirjaimet piippauksella {#SpeechSettingsBeepForCaps}

Jos tämä valintaruutu on valittuna, NVDA piippaa lyhyesti aina isoa kirjainta puhuttaessa.
...
Esimerkiksi jos et tarvitse "piippaukset"- ja "ei puhetta" -tiloja, sinun tulisi poistaa niiden valinta ja pitää sekä "puhe käytössä" että "pyydettäessä" valittuina.
...
Jos valitset "Automaattinen"-vaihtoehdon, taulukko valitaan NVDA:n nykyisen kielen perusteella.
...
Huom: Tästä vaihtoehdosta on hyötyä vain, jos pistenäytössä on Perkins-tyylinen -näppäimistö ja näytön ajuri tukee tätä ominaisuutta.
...
Tässä tapauksessa pistenäyttö ei seuraa järjestelmän kohdistusta eikä ----kohdistinta.
...
##### Kappaleen aloitusmerkki {#BrailleParagraphStartMarkers}

Jos "Lue kappaleittain" on valittuna, kappaleen alku ilmaistaan valitulla aloitusmerkillä.
Tämä voi olla erityisen hyödyllistä sovelluksissa, joissa luetaan suuria tekstikokonaisuuksia, kuten jäsenneltyjä asiakirjoja tai kirjoja.
Tällaisissa asiakirjoissa kappaleiden aloituskohdan tietäminen voi auttaa ymmärtämään sisällön rakennetta tai asettamaan kirjanmerkkejä tai merkintöjä kappaleiden sijainnin perusteella.

Vaihtoehtoina ovat kahden välilyönnin käyttö hienovaraisena kappaleenvaihtona sekä kappalemerkin (¶) käyttö selvempänä merkkinä.

| . {.hideHeaderRow} |.|
|---|---|
|Asetukset | Ei kappaleen aloitusmerkkiä, Kaksi välilyöntiä (  ), Kappalemerkki (¶)|
|Oletus | Ei kappaleen aloitusmerkkiä|
...
##### Muotoilun näyttö {#BrailleFormattingDisplay}

Tämä asetus määrittää, miten NVDA näyttää tekstin muotoilun pistekirjoituksella.
Tämä asetus vaikuttaa vain, jos NVDA on määritetty [näyttämään fontin määreet pistekirjoituksella](#DocumentFormattingFontAttributes).

| . {.hideHeaderRow} |.|
|---|---|
| Asetukset | Oletus (Liblouis), Liblouis, Tunnisteet |
| Oletus | Liblouis |

| Asetus | Toiminta |
|---|---|
| Liblouis | Käytä alkuperäistä pistekirjoitusmuotoilua. Huomaa, että tämä vaihtoehto merkitsee vain lihavoidun, kursivoidun ja alleviivatun tekstin, ja vain, jos valittu pistetaulukko tukee näiden määreiden merkitsemistä. |
| [Tunnisteet](#BrailleFormattingDisplayTags) | Käytä tunnisteita, jotka osoittavat, miten ja missä tekstin muotoilu muuttuu. |

###### Tunnisteet {#BrailleFormattingDisplayTags}

Kun "Muotoilun näyttö" -asetukseksi on määritetty "Tunnisteet", pistekirjoituksella näytetään muotoilutunniste, kun muotoilumuutos havaitaan.
Tunnisteet alkavat merkillä ⣋ ja päättyvät merkkiin ⣙.
Muotoilutunniste sisältää yhden tai useamman symbolin, jotka kuvailevat tekstin muotoilua.
Seuraavat symbolit on määritelty:

| Symboli | Merkitys |
|---|---|
| ⠃ ("b") | Lihavointi alkaa |
| ⡃ ("b" pisteellä 7) | Lihavointi päättyy |
| ⠊ ("i") | Kursiivi alkaa |
| ⡊ ("i" pisteellä 7) | Kursiivi päättyy |
| ⠥ ("u") | Alleviivaus alkaa |
| ⡥ ("u" pisteellä 7) | Alleviivaus päättyy |
| ⠎ ("s") | Yliviivaus alkaa |
| ⡎ ("s" pisteellä 7) | Yliviivaus päättyy |

##### Puhu merkki siirrettäessä kohdistinta kosketuskohdistinnäppäimillä {#BrailleSpeakOnRouting}

Jos tämä on käytössä, NVDA puhuu automaattisesti kohdistimen kohdalla olevan merkin käyttäessäsi pistenäytön kosketuskohdistinnäppäimiä.

Vaihda tätä asetusta milloin tahansa määrittämällä mukautettu näppäinkomento [Näppäinkomennot-valintaikkunaa](#InputGestures) käyttäen.

##### Puhu riveittäin tai kappaleittain liikuttaessa {#BrailleSpeakOnNavigating}

Kun tämä asetus on käytössä, NVDA puhuu rivit ja kappaleet, joihin siirrytään pistenäytön siirtymispainikkeilla.

§Ota tämä asetus käyttöön mistä tahansa määrittämällä oma näppäinkomento "speakOnNavigatingByUnit"-skriptille [Näppäinkomennot-valintaikkunan](#InputGestures) "Pistekirjoitus"-osiossa.

##### Vältä sanojen katkaisua {#BrailleSettingsWordWrap}

Jos tämä asetus on käytössä, sanaa, joka on liian pitkä mahtuakseen pistenäytön rivin loppuun, ei katkaista.
Sen sijaan pisterivin loppuun jää hieman tyhjää tilaa.
Voit lukea koko sanan vierittämällä näyttöä.
Tätä kutsutaan myös automaattiseksi rivitykseksi.
...
Voit lukea sanan lopun vierittämällä näyttöä.
...
Tämä on hyödyllistä, kun vastaan tulee pistekirjoitusmerkkejä, joita ei ole määritelty tietyssä pistetaulukossa ja joilla on yhteensopiva vaihtoehto, kuten sosiaalisessa mediassa yleisesti käytetyt lihavoidut ja kursivoidut merkit.
Muita Unicode-normalisoinnin hyötyjä käsitellään tarkemmin [puheasetusten vastaavassa osiossa](#SpeechUnicodeNormalization).
...
Tarkista [Tuetut pistenäytöt -luvusta](#SupportedBrailleDisplays), tukeeko kyseinen ajuri näytön automaattista tunnistusta.
...
Tästä syystä on suositeltavaa kytkeä koneeseen vain yksi tietyn tyyppinen/valmistajan pistenäyttö kerrallaan.
...
##### Pidä äänilaite aktiivisena puheen jälkeen {#AudioAwakeTime}

Tässä muokkausruudussa määritetään, kuinka kauan NVDA pitää äänilaitetta aktiivisena puheen loputtua.
Tämä auttaa NVDA:ta estämään puheen häiriöitä, kuten tilanteita, joissa osa sanasta jää pois.
Tämä johtuu usein äänilaitteiden (erityisesti Bluetooth- ja muiden langattomien laitteiden) siirtymisestä valmiustilaan.
Tästä voi olla hyötyä myös muissa tilanteissa, kuten käytettäessä NVDA:ta virtuaalikoneessa (esim. Citrix Virtual Desktopissa) tai tietyissä kannettavissa tietokoneissa.

Jos arvo on liian pieni, äänilaite saattaa siirtyä valmiustilaan liian aikaisin, jolloin seuraavan puhejakson alku jää kuulumatta.
Liian suuren arvon käyttö saattaa puolestaan tyhjentää äänilaitteen akun nopeammin, koska laite pysyy aktiivisena pidempään, vaikka ääntä ei toisteta.
...
Tämä asetus määrittää, missä tilanteessa NVDA puhuu kirjoittamasi merkit.
Seuraavat vaihtoehdot ovat käytettävissä:

* Ei käytössä: NVDA ei puhu kirjoitettuja merkkejä.
...
* Aina: NVDA puhuu kaikki kirjoitetut merkit.
...
Tämä asetus määrittää, missä tilanteessa NVDA puhuu kirjoittamasi sanat.
Seuraavat vaihtoehdot ovat käytettävissä:

* Ei käytössä: NVDA ei puhu kirjoitettuja sanoja.
...
* Aina: NVDA puhuu kaikki kirjoitetut sanat.
...
##### Salli pikanäppäimet jatkuvassa luvussa {#KeyboardSettingsSkimReading}
...
##### Piippaa kirjoitettaessa pieniä kirjaimia Caps Lockin ollessa käytössä {#KeyboardSettingsBeepLowercase}

Kun tämä asetus on käytössä, kirjoitettaessa kirjaimia Vaihto-näppäin pohjassa kuuluu varoituspiippaus, Jos Caps Lock on päällä.
...
##### Ilmaise kirjoitusvirheet kirjoitettaessa merkkiäänellä {#KeyboardSettingsAlertForSpellingErrors}
...
##### Usean näppäinpainalluksen aikakatkaisu {#MultiPressTimeout}

Osa NVDA-näppäinkomennoista suorittaa eri toimintoja sen mukaan, kuinka monta kertaa samaa näppäintä painetaan.
Esimerkkinä tästä on "Puhu tarkastelukohdistimen kohdalla oleva merkki" -komento.
Tämä komento ilmoittaa kerran painettaessa merkin, kahdesti painettaessa sen foneettisen kuvauksen ja kolmesti painettaessa numeerisen arvon.
Tämä asetus määrittää ajan, jonka jälkeen saman näppäimen lisäpainallus tulkitaan uudeksi komennoksi sen sijaan, että sitä käsiteltäisiin ensimmäisen komennon seuraavana painalluksena.
Liian lyhyt aikakatkaisu aiheuttaa esimerkkikomennon tapauksessa sen, että kaksi painallusta ilmoittaa nykyisen merkin kahdesti sen foneettisen kuvauksen sijaan.
Oletusarvoinen aikakatkaisu on 500 ms, ts. puoli sekuntia.
Aikakatkaisun pidentäminen voi olla erityisen hyödyllistä henkilöille, jotka käyttävät alas jääviä näppäimiä tai joilla on fyysinen vamma.
...
Hiiri-kategoriassa NVDA:n voi määrittää seuraamaan hiirtä, ilmaisemaan hiiren koordinaatit piippauksilla sekä muuttaa muita hiiren käyttöön liittyviä asetuksia.
Tämä kategoria sisältää seuraavat asetukset:
...
##### Ilmaise hiiren koordinaatit piippauksilla {#MouseSettingsAudio}

Kun tämä valintaruutu on valittuna, NVDA piippaa hiiren liikkuessa, jotta käyttäjä saa selville, missä hiiri on ruudun kokoon suhteutettuna.
Piippaukset ovat sitä korkeampia, mitä ylempänä ruudulla hiiri on.
...
##### Äänikoordinaatttien voimakkuutta säädetään ruudun kirkkauden perusteella {#MouseSettingsBrightness}

Jos Ilmaise hiiren koordinaatit piippauksilla -asetus on käytössä, tämän valintaruudun valitseminen tarkoittaa, että äänikoordinaattien voimakkuutta säädetään sen mukaan, miten kirkas ruutu on hiirikohdistimen alla.
...
* Piippaa: NVDA piippaa aina edistymispalkin muuttuessa. Mitä korkeampi piippaus, sitä lähempänä loppua edistymispalkki on.
* Piippaa ja puhu: NVDA sekä piippaa että puhuu edistymispalkin päivittyessä.
...
##### Lue kaikki ehdokkaat automaattisesti {#InputCompositionReportAllCandidates}

Tällä asetuksella, joka on oletusarvoisesti käytössä, voidaan valita, luetaanko kaikki näkyvissä olevat ehdokkaat automaattisesti niiden luettelon tullessa näkyviin tai sen sivua vaihdettaessa.
...
Uuden foneettisen kiinan kaltaisissa foneettisissa syöttömenetelmissä saattaa kuitenkin olla hyödyllisempää poistaa asetus käytöstä, sillä kaikki symbolit kuulostavat samalta, ja luettelokohteissa on liikuttava yksitellen lisätietojen saamiseksi kunkin ehdokkaan merkkikuvauksista.

##### Lue valittu ehdokas {#InputCompositionAnnounceSelectedCandidate}

Tällä asetuksella, joka on oletusarvoisesti käytössä, voidaan valita, lukeeko NVDA valitun ehdokkaan ehdokasluettelon tullessa näkyviin tai valintaa muutettaessa.
Tämä on tarpeen esim. uuden foneettisen kiinan kaltaisissa syöttömenetelmissä, joissa valintaa voidaan muuttaa nuolinäppäimillä, mutta joissakin kirjoittaminen saattaa olla tehokkaampaa, kun asetus on poistettu käytöstä.
Huom: Vaikka tämä asetus ei ole käytössä, tarkastelukohdistin sijoitetaan silti valitun ehdokkaan kohdalle, mikä mahdollistaa objektinavigoinnin tai tekstintarkastelukomentojen käyttämisen kyseessä olevan tai muiden ehdokkaiden lukemiseen.

##### Ilmoita lyhyet merkkikuvaukset aina ehdokkaita luettaessa {#InputCompositionCandidateIncludesShortCharacterDescription}

Tällä asetuksella, joka on oletusarvoisesti käytössä, voidaan valita, lukeeko NVDA lyhyen kuvauksen kustakin ehdokkaan merkistä joko ehdokasta valittaessa tai automaattisesti luettaessa ehdokasluettelon tullessa näkyviin.
Huom: Asetus ei vaikuta valitun ehdokkaan lisämerkkikuvausten lukemiseen kiinan kaltaisissa kielissä.
...
Joissakin syöttömenetelmissä, kuten uudessa foneettisessa kiinassa ja uudessa ChangJiessa, on lukumerkkijono, jota kutsutaan joskus myös esikoostemerkkijonoksi.
Tällä asetuksella voidaan valita, ilmoittaako NVDA lukumerkkijonoon kirjoitettavat uudet merkit.
Asetus on oletusarvoisesti käytössä.
Huom: Jotkin vanhemmat syöttömenetelmät, kuten kiinalainen ChangJie, eivät välttämättä käytä lukumerkkijonoa esikoostemerkkien säilyttämiseen, vaan käyttävät niiden sijaan suoraan koostemerkkijonoa. Katso seuraavaa asetusta koostemerkkijonon lukemisen määrittämistä varten.

##### Ilmoita koostemerkkijonon muutokset {#InputCompositionCompositionStringChanges}

Kun luku- tai esikoostetiedot on yhdistetty kelvolliseksi kuvakirjoitussymboliksi, useimmat syöttömenetelmät sijoittavat sen tilapäisesti talteen koostemerkkijonoon muiden yhdistettyjen symbolien kanssa ennen niiden lopullista asiakirjaan lisäämistä.
Tällä asetuksella voidaan valita, lukeeko NVDA uudet symbolit niiden tullessa näkyviin koostemerkkijonoon.
Asetus on oletusarvoisesti käytössä.
...
* Fontti
  * Fontin nimi
  * Fontin koko
  * Fontin määreet [(Ei käytössä, Puhe, Pistekirjoitus, Puhe ja pistekirjoitus)](#DocumentFormattingFontAttributes)
  * Ylä- ja alaindeksit
  * Korostus
...
* Sivut ja välit
  * Sivunumerot
  * Rivinumerot
  * Rivien sisennysten ilmoittaminen [(ei käytössä, puheella, piippauksilla, puheella ja piippauksilla)](#DocumentFormattingSettingsLineIndentation)
...
* Elementit
  * Otsikot
  * Linkit
  * Linkin tyyppi (kohde samalla sivulla)
  * Grafiikat
  * Luettelot
...
##### Fontin määreet {#DocumentFormattingFontAttributes}

Tämän asetuksen avulla voit valita, miten tietyt fontin määreet, kuten lihavointi, kursiivi, alleviivaus ja yliviivaus, ilmoitetaan.
Fontin määreet -yhdistelmäruudussa on neljä vaihtoehtoa:

* Ei käytössä: NVDA ei ilmoita fontin määreitä.
* Puhe: NVDA ilmoittaa, kun fontin määreet muuttuvat.
* Pistekirjoitus: NVDA näyttää määreet pistekirjoituksella.
Määreiden näyttötapa voidaan määrittää [NVDA:n pistekirjoitusasetuksissa](#BrailleFormattingDisplay).
* Puhe ja pistekirjoitus: NVDA ilmoittaa fontin määreet molemmilla yllä mainituista tavoista.
...
* piippauksilla: Jos tämä on valittuna, piippaukset ilmaisevat sisennysten määrän sen muuttuessa.
Piippauksen äänenkorkeus nousee jokaisen välilyönnin kohdalla, ja sarkainmerkki vastaa neljää välilyöntiä.
* puheella ja piippauksilla: Tämän ollessa valittuna sisennykset ilmoitetaan molempia yllä olevia asetuksia käyttäen.
...
##### Automaattiset päivitykset {#AutomaticAddonUpdates}
...
Oletusarvoisesti vain sellaisista lisäosista ilmoitetaan, joiden päivitykset ovat saatavilla samassa [kanavassa](#AddonStoreFilterChannel) (esim. vakaa, beeta tai kehitys).
Voit määrittää päivityskanavat [erikseen kullekin lisäosalle](#AddonStoreUpdateChannel) tai yhteisesti [kaikille](#DefaultAddonUpdateChannel).

Kun "Päivitä automaattisesti" on valittuna, lisäosat päivittyvät automaattisesti taustalla.
Sinua pyydetään käynnistämään NVDA uudelleen, kun päivitykset on suoritettu.

| . {.hideHeaderRow} |.|
|---|---|
|Asetukset | Ilmoita (Oletus), Päivitä automaattisesti, Ei käytössä |
|Oletus | Ilmoita |

|Asetus | Toiminta |
|---|---|
|Ilmoita | Ilmoita, kun lisäosapäivityksiä on saatavilla |
|Päivitä automaattisesti | Päivitä lisäosat automaattisesti taustalla |
|Ei käytössä | Älä tarkista lisäosapäivityksiä automaattisesti |

##### Oletuspäivityskanava {#DefaultAddonUpdateChannel}

Kun [lisäosien automaattiset päivitykset](#AutomaticAddonUpdates) ovat käytössä, lisäosat päivittyvät oletusarvoisesti vain samalle [kanavalle](#AddonStoreFilterChannel).
Esimerkiksi asennettu beetaversio päivittyy vain uudempaan beetaversioon.
Tämä asetus määrittää kaikkien lisäosien oletuspäivityskanavan.
Voit myös vaihtaa päivityskanavan [lisäosakaupasta tietylle yksittäiselle lisäosalle](#AddonStoreUpdateChannel).

| . {.hideHeaderRow} |.|
|---|---|
| Asetukset | Sama (Oletus), Mikä tahansa, Älä päivitä, Vakaa, Beeta tai kehitys, Beeta, Kehitys |
| Oletus | Sama |

| Asetus | Toiminta |
|---|---|
| Sama | Lisäosat pysyvät omilla kanavillaan. |
| Mikä tahansa | Lisäosat päivittyvät automaattisesti uusimpaan versioon kanavasta riippumatta. |
| Älä päivitä | Lisäosat eivät päivity automaattisesti, vaan päivitys on otettava käyttöön erikseen kullekin lisäosalle. |
| Vakaa | Lisäosat päivittyvät automaattisesti vakaisiin versioihin. |
| Beeta tai kehitys | Lisäosat päivittyvät automaattisesti beeta- tai kehitysversioihin. |
| Beeta | Lisäosat päivittyvät automaattisesti beetaversioihin. |
| Kehitys | Lisäosat päivittyvät automaattisesti kehitysversioihin. |

##### Salli automaattisten päivitysten asentaa yhteensopimattomia lisäosia {#AllowIncompatibleAddonUpdates}

Tämä asetus mahdollistaa automaattisen päivittämisen sellaisiin lisäosiin, jotka eivät välttämättä ole täysin yhteensopivia nykyisen NVDA-version kanssa.
Tämä on oletusarvoisesti poissa käytöstä, mikä tarkoittaa, että automaattiset päivitykset päivittää vain sellaisiin lisäosiin, jotka ovat yhteensopivia nykyisen NVDA-version kanssa.v
Automaattiset päivitykset päivittää silti yhteensopimattoman lisäosaversion yhteensopivaksi, kun sellainen julkaistaan.
Tämän ottaminen käyttöön voi olla hyödyllistä siirryttäessä käyttämään lisäosien yhteensopimattomuuden aiheuttavia NVDA-versioita (vuoden ensimmäinen julkaisu).
Tämä on erityisen hyödyllistä alfa- ja beetatestaajille, jotka testaavat lisäosien yhteensopivuutta lisäosat rikkovien NVDA-versioiden kehityksen alkuvaiheessa.

##### Peilipalvelin {#AddonStoreMetadataMirror}

Näillä säätimillä voit määrittää vaihtoehtoisen URL-osoitteen, josta lisäosakaupan tiedot ladataan.
Tämä voi olla hyödyllistä alueilla, joissa NV Accessin lisäosakaupan palvelimen käyttö on hidasta tai pääsyä ei ole lainkaan.

Nykyisen peilipalvelimen URL-osoite näytetään vain luku -tyyppisessä tekstikentässä.
Jos peilipalvelin ei ole käytössä (ts. käytetään NV Accessin lisäosakaupan palvelinta), tekstikentässä näytetään "Ei peilipalvelinta".

Mikäli haluat vaihtaa lisäosakaupan peilipalvelinta, avaa [Määritä lisäosakaupan peilipalvelin -valintaikkuna](#SetURLDialog) painamalla "Vaihda..."-painiketta.

#### Etäkäyttö {#RemoteSettings}

Tämä kategoria mahdollistaa [etäkäytön](#RemoteAccess) toiminnan määrittämisen.

##### Salli etäkäyttö {#RemoteEnable}

Ota NVDA:n etäkäyttötoiminto käyttöön tai poista se käytöstä tällä valintaruudulla.

Kun se ei ole valittuna, etäkäyttötoiminto ei ole käytettävissä.

Huom: Jos haluat jatkaa etäkäyttöistuntoa suojatuissa ruuduissa, kuten käyttäjätilien valvonnan kehotteissa, sinun on otettava tämä asetus käyttöön, tallennettava muutokset ja [kopioitava sen jälkeen asetuksesi NVDA:n järjestelmäasetusten hakemistoon](#GeneralSettingsCopySettings).
Etäkäyttö jatkaa suojatuilla työpöydillä vain asennetun NVDA-version aloittamia istuntoja.
Tämä tarkoittaa, että NVDA:n massamuistiversiota ei voi käyttää etäkäyttöistunnon aloittamiseen ja sen jatkamiseen suojatuissa ruuduissa.

Seuraavat asetukset ovat käytettävissä vain, jos etäkäyttö on sallittu.

##### Pyydä vahvistus yhteyden katkaisulle, kun tätä konetta hallitaan {#RemoteConfirmDisconnect}

Tämä asetus määrittää, pyydetäänkö etäkäyttöistunnon lopettamiselle vahvistus, kun tätä konetta hallitaan.

Kun  se on valittuna, NVDA pyytää vahvistuksen ennen yhteyden katkaisua.
Mikäli asetus ei ole valittuna, NVDA katkaisee yhteyden heti pyytämättä vahvistusta.

##### Yhdistä automaattisesti NVDA:n käynnistyessä {#RemoteAutoconnect}

Tämä asetus mahdollistaa etäkäyttöistunnon aloittamisen automaattisesti NVDA:n käynnistyessä.
Tämä voi olla hyödyllistä esimerkiksi silloin, kun haluat hallita kotitietokonettasi ollessasi poissa kotoa.

Ole varovainen ottaessasi tätä asetusta käyttöön, sillä se saattaa lisätä tietokoneesi luvattoman käytön riskiä.

Lisätietoja seuraavista asetuksista on luvussa [Etäkäyttöistunnon määrittäminen](#RemoteAccessSetup)

##### Tila {#RemoteAutoconnectMode}

Valitse automaattisen yhdistämisen yhteystila.

Tämä asetus on käytettävissä vain, kun [Yhdistä automaattisesti NVDA:n käynnistyessä](#RemoteAutoconnect) on valittuna.

|Asetus | Toiminta |
|---|---|
|Salli tämän koneen hallinta | Käytä tätä tietokonetta "hallittavana" koneena. Tämä sallii "hallitsevaa" tietokonetta käyttävän henkilön käyttää tätä konetta ikään kuin hän istuisi sen ääressä. |
|Hallitse toista konetta | Käytä tätä tietokonetta "hallitsevana" koneena. Tämä sallii sinun suorittaa komentoja etäkoneessa. |

##### Palvelin {#RemoteAutoconnectServer}

Valitse tämä automaattista yhdistämistä varten.

Tämä asetus on käytettävissä vain, kun [Yhdistä automaattisesti NVDA:n käynnistyessä](#RemoteAutoconnect) on valittuna.

|Asetus | Toiminta |
|---|---|
|Käytä olemassa olevaa | Käytä olemassa olevaa etäkäyttöpalvelinta yhteyden välittämiseen (suositellaan). |
|Isännöi paikallisesti | Käytä tätä NVDA-versiota etäkäyttöpalvelimena (edistyneille käyttäjille). Tämä asetus saattaa edellyttää sellaisia verkon määrityksiä, joita ei käsitellä tässä käyttöoppaassa. |

##### Isäntä {#RemoteAutoconnectHost}

Määritä tässä kentässä etäkäyttöpalvelimen URL-osoite, jota käytetään automaattisissa yhteyksissä.
Jos palvelin käyttää muuta porttia kuin 6837, lisää se kaksoispisteen jälkeen (esim. palvelin.com:1234).

Tämä asetus on käytettävissä vain, kun [Yhdistä automaattisesti NVDA:n käynnistyessä](#RemoteAutoconnect) on valittuna ja [palvelimeksi](#RemoteAutoconnectServer) on määritetty "Käytä olemassa olevaa".

##### Portti {#RemoteAutoconnectPort}

Määritä tässä kentässä TCP-portti, jota käytetään automaattisissa yhteyksissä.

Tämä asetus on käytettävissä vain, kun [Yhdistä automaattisesti NVDA:n käynnistyessä](#RemoteAutoconnect) on valittuna ja [palvelimeksi](#RemoteAutoconnectServer) on määritetty "Isännöi paikallisesti".

##### Avain {#RemoteAutoconnectKey}

Määritä tässä kentässä avain (salasana), jota käytetään automaattisissa yhteyksissä.

Tämä asetus on käytettävissä vain, kun [Yhdistä automaattisesti NVDA:n käynnistyessä](#RemoteAutoconnect) on valittuna.

##### Poista kaikki luotetut sormenjäljet {#RemoteDeleteFingerprints}

Tämän painikkeen painaminen saa NVDA:n unohtamaan kaikki aiemmin luotettujen etäkäyttöpalvelinten sormenjäljet.
Tämä tarkoittaa, että sinulta kysytään uudelleen, haluatko muodostaa yhteyden kaikkiin valtuuttamattomiin etäkäyttöpalvelimiin – myös sellaisiin, joihin olet aiemmin muodostanut yhteyden.
NVDA pyytää vahvistuksen sormenjälkien poistolle.

Tämä asetus on käytettävissä vain, jos asetuksiisi on tallennettu luotettuja sormenjälkiä.
...
#### Määritä peilipalvelin -valintaikkuna {#SetURLDialog}

Tässä valintaikkunassa voit määrittää peilipalvelimen URL-osoitteen, jota käytetään [NVDA:ta päivitettäessä](#GeneralSettingsCheckForUpdates) tai [lisäosakauppaa käytettäessä](#AddonsManager).
Tämä voi olla hyödyllistä alueilla, joilla NV Accessin palvelinten käyttö näitä toimintoja varten on hidasta tai ei ole lainkaan mahdollista.

* Kun määrität [NVDA:n päivityspeilipalvelinta](#UpdateMirror), tämän valintaikkunan otsikko on "Määritä NVDA:n päivityspeilipalvelin".
* Kun määrität [lisäosakaupan peilipalvelinta](#AddonStoreMetadataMirror), tämän valintaikkunan otsikko on "Määritä lisäosakaupan peilipalvelin".

##### URL-osoite {#SetURLTextbox}

Syötä tähän sen peilipalvelimen URL-osoite, jota haluat käyttää.
Vain HTTP- ja HTTPS-osoitteita tuetaan.
Yksityisyytesi suojaamiseksi NV Access suosittelee käyttämään HTTPS-osoitteita aina kun mahdollista.

Käytä oletusarvoista NV Accessin palvelinta jättämällä tämä tyhjäksi.

##### Testaa... {#SetURLTest}

Testaa antamasi URL-osoitteen toimivuus painamalla tätä painiketta.
Internet-yhteyden on oltava käytettävissä, jotta testi onnistuu.
Suosittelemme aina testaamaan URL-osoitteen toimivuuden ennen sen tallentamista.
...
Tämä asetus sallii käytössä ollessaan NVDA:n ladata mukautettuja sovellusmoduuleita, yleisliitännäisiä, pistenäyttö- ja syntetisaattoriajureita sekä näönparannuksen tarjoajia erityisestä kehittäjille tarkoitetusta scratchpad-nimisestä hakemistosta, joka sijaitsee NVDA:n käyttäjäkohtaisessa asetushakemistossa.
...
##### Poista hiljaisuus puheen alusta {#TrimLeadingSilenceSpeech}

Kun tämä asetus on käytössä, NVDA poistaa hiljaisuutta puheen alusta, mikä saattaa parantaa joidenkin puhesyntetisaattoreiden vastetta.
Asetus on oletusarvoisesti käytössä, ja sen pitäisi vaikuttaa vain puheen alussa olevaan hiljaisuuteen.
Jos huomaat, että myös tarpeellisia hiljaisuusjaksoja (esim. tauko kahden lauseen välillä) puuttuu puhesyntetisaattorilisäosaa käytettäessä, voit ratkaista ongelman poistamalla tämän ominaisuuden käytöstä.

##### Käytä WASAPIa SAPI 4:n äänentoistoon {#UseWASAPIForSAPI4}

Tämä asetus mahdollistaa Microsoft Speech API versio 4 (SAPI 4) -äänien äänentoiston Windows Audio Session API:n (WASAPI) kautta.
Tämä voi mahdollistaa SAPI 4 -äänien toimimisen useampien ominaisuuksien kanssa, kuten äänenvaimennuksen, alun hiljaisuuden poistamisen ja äänilaitteen hereillä pitämisen.
Jotkin SAPI 4 -äänet eivät kuitenkaan välttämättä toimi nykyisen WASAPI-toteutuksen kanssa.
Mikäli käyttämäsi SAPI 4 -ääni lakkaa toimimasta, voit poistaa tämän asetuksen käytöstä.
...
##### Ilmaise lokiin tallentuvat virheet merkkiäänellä {#PlayErrorSound}
...
##### Tekstikappalenavigoinnin säännöllinen lauseke {#TextParagraphRegexEdit}

Tämä kenttä mahdollistaa säännöllisen lausekkeen mukauttamisen tekstikappaleiden tunnistamiseksi selaustilassa.
[Tekstikappaleiden navigointikomento](#TextNavigationCommand) etsii tämän säännöllisen lausekkeen mukaisia kappaleita.

### Sekalaiset asetukset {#MiscSettings}

[Asetukset](#NVDASettings)-valintaikkunan lisäksi NVDA-valikosta löytyvä Mukautukset-alivalikko sisältää myös useita muita kohteita, jotka on kuvailtu alla.

#### Puhesanastot {#SpeechDictionaries}

Mukautukset-valikosta löytyvä Puhesanastot-valikko sisältää valintaikkunoita, joista voidaan muuttaa tapaa, jolla NVDA lausuu tietyt sanat tai ilmaisut.
...
Viimeisenä on joukko valintapainikkeita, joiden avulla on mahdollista määrittää, täsmääkö korvattava merkkijono kaikkialla, vain kokonaisena sanana vai käsitelläänkö sitä säännöllisenä lausekkeena.
...
Voit tutustua aiheeseen [Pythonin säännöllisten lausekkeiden oppaassa](https://docs.python.org/3.11/howto/regex.html).
...
* Lähetä merkki syntetisaattorille -yhdistelmäruudusta voit määrittää, milloin varsinainen symboli (vastakohtana sen korvaavalle tekstille) lähetetään syntetisaattorille.
...
Monimutkaisten symbolien tapauksessa Korvaava teksti -kentässä on ehkä oltava joitakin ryhmäviittauksia täsmättävään tekstiin. Esim. koko päivämäärää vastaavan korvattavan tekstin \1, \2 ja \3 tulisi näkyä kentässä, jotta ne korvataan päivämäärän vastaavilla osilla.
...
Tätä asetusta voidaan kuitenkin myös muuttaa Yleiset asetukset -valintaikkunassa, joka löytyy Mukautukset-valikosta kohdasta Asetukset.
...
## Etäkäyttö {#RemoteAccess}

NVDA:n sisäänrakennetun etäkäyttötoiminnon avulla voit hallita toista NVDA:ta käyttävää tietokonetta tai antaa toisen henkilön hallita omaa konettasi.
Tämä tekee avun tarjoamisesta tai vastaanottamisesta, yhteistyön tekemisestä tai oman tietokoneen etäkäytöstä helppoa.

Turvallisuutesi vuoksi etäkäyttö on oletusarvoisesti estetty.
Voit sallia etäkäytön [etäkäyttöasetuksissa](#RemoteSettings).

### Aloittaminen {#RemoteAccessGettingStarted}

Varmista ennen aloittamista, että NVDA on asennettu ja käynnissä molemmissa tietokoneissa.
Varmista myös, että Salli etäkäyttö -valintaruutu on valittuna [etäkäyttöasetuksissa](#RemoteSettings) molemmilla tietokoneilla.
Etäkäyttötoiminto on sen jälkeen käytettävissä NVDA:n Työkalut-valikosta, eikä lisälatauksia tai -asennuksia tarvita.

Etäkäyttöistuntoa ei voi aloittaa manuaalisesti, jos NVDA on käynnissä [suojatussa tilassa](#SecureMode).
NVDA aloittaa kuitenkin automaattisesti uuden istunnon, jos se on siten määritetty [Yhdistä automaattisesti NVDA:n käynnistyessä](#RemoteAutoconnect) -asetuksella.
Yhteyden manuaalinen katkaisu on edelleen mahdollista suojatussa tilassa.

### Etäkäyttöistunnon määrittäminen {#RemoteAccessSetup}

Etäkäyttöistunnot määritetään etäkäyttöyhteyden valintaikkunassa.
Avaa NVDA-valikko, valitse Työkalut, sitten "Etäkäyttö" ja lopuksi "Yhdistä...".
Voit myös painaa missä tahansa `NVDA+Alt+R`.

#### Tilan valinta (hallitseva tai hallittava) {#RemoteAccessMode}

Sinun on päätettävä, kumpaa tietokonetta hallitaan (hallittava kone) ja kumpi toimii hallitsevana (hallitseva kone).

Tila-yhdistelmäruudusta voit valita, hallitaanko tätä tietokonetta (toinen henkilö käyttää konettasi) vai hallitsetko sinä toista tietokonetta (käytät toisen henkilön konetta).

* Jos tarvitset toisen henkilön apua tietokoneesi käytössä, valitse "Salli tämän koneen hallinta".
* Jos aiot auttaa toista henkilöä hänen tietokoneensa käytössä, valitse "Hallitse toista konetta".

#### Yhdistä olemassa olevaan palvelimeen (isäntä ja avain tarvitaan) {#RemoteAccessJoin}

Tässä tapauksessa toinen henkilö on joko muodostanut yhteyden reitityspalvelimen kautta tai isännöi yhteyttä paikallisesti.

1. Valitse Palvelin-yhdistelmäruudusta "Käytä olemassa olevaa".
1. Syötä Isäntä-kenttään isännän URL-osoite sekä portti, mikäli se on tiedossa.
  * Lisää tarvittaessa porttinumero isännän URL-osoitteen perään kaksoispisteellä (":") erotettuna.
Esimerkiksi `palvelin.com:1234`.
1. Syötä Avain-kenttään saamasi avain.
Varmista, että Kirjoitat avaimen täsmälleen annetussa muodossa.
1. Paina OK.

#### Yhdistäminen reitityspalvelimen kautta {#RemoteAccessConnectRelay}

Jos haluat sallia toisen henkilön muodostaa yhteyden koneeseesi internetin kautta, helpoin vaihtoehto on käyttää reitityspalvelinta. Tämä tehdään seuraavasti:

1. Valitse Palvelin-yhdistelmäruudusta "Käytä olemassa olevaa".
1. Syötä Isäntä-kenttään reitityspalvelimen isäntänimi, esim. `nvdaremote.com`.
Sinun täytyy antaa tämä osoite henkilölle, joka aikoo hallita konettasi.
  * Lisää tarvittaessa porttinumero isäntänimen perään kaksoispisteellä (":") erotettuna.
Esimerkiksi `palvelin.com:1234`.
1. Avain: keksi pitkä ja vaikeasti arvattava avain tai paina "Luo avain" -painiketta, jolloin sellainen luodaan automaattisesti.
Sinun täytyy antaa se henkilölle, joka aikoo hallita konettasi.
  * Huom: Avain toimii sekä istunnon tunnisteena että salasanana, joten sen on oltava vaikeasti arvattava.
1. Paina OK.
1. Anna yhteyden tiedot (kuten isäntänimi ja avain) henkilölle, joka aikoo hallita konettasi, jotta hän voi muodostaa yhteyden.

#### NVDA:n käyttäminen palvelimena {#RemoteAccessConnectSelfHosted}

Jos et halua käyttää reitityspalvelinta tai mikäli muodostat yhteyden lähiverkon kautta:

1. Valitse Palvelin-yhdistelmäruudusta "Isännöi paikallisesti".
1. Jos yhteys muodostetaan internetin kautta, voit painaa "Hae ulkoinen IP-osoite" -painiketta, jolloin ulkoinen (internetissä näkyvä) IP-osoitteesi näytetään.
Sinun täytyy antaa se henkilölle, joka aikoo hallita konettasi.
1. Jos yhteys muodostetaan lähiverkon kautta, sinun tulee antaa toiselle henkilölle paikallinen IP-osoitteesi.
Sen selvittämistä ei käsitellä tässä käyttöoppaassa.
1. Syötä Portti-kenttään sen portin numero, jota haluat käyttää saapuville etäkäyttöyhteyksille.
Myös konettasi hallitseva henkilö tarvitsee tämän.
  * Portti ei saa olla minkään muun palvelun käytössä.
  * Etäkäytön oletusportti on 6837.
  * Jos yhteys muodostetaan internetin kautta, portin uudelleenohjaus voi olla tarpeen.
  Aihetta ei käsitellä tässä käyttöoppaassa.
  Ota tarvittaessa yhteyttä verkon ylläpitäjään.
1. Avain: keksi pitkä ja vaikeasti arvattava avain tai paina "Luo avain" -painiketta, jolloin sellainen luodaan automaattisesti.
Sinun täytyy antaa se henkilölle, joka aikoo hallita konettasi.
  * Huom: Avain toimii sekä istunnon tunnisteena että salasanana, joten sen on oltava vaikeasti arvattava.
1. Paina OK.
...
### Etäkäytö käyttäminen {#RemoteAccessUsage}

Kun etäkäyttöistunto on aktiivinen, voit vaihtaa etäkäytettävän ja oman koneesi hallinnan välillä, jakaa leikepöydän sekä mykistää etäistunnon.

* Vaihda etäkoneen hallinnan ja oman koneesi välillä painamalla `NVDA+Alt+Sarkain`.
* Lähetä leikepöydällä oleva teksti toiselle koneelle avaamalla NVDA-valikko, valitsemalla Työkalut, sitten "Etäkäyttö" ja lopuksi "Lähetä leikepöytä".
* Mikäli olet muodostanut yhteyden hallitsevana koneena, mykistä etäkoneen puhe omalta koneeltasi avaamalla NVDA-valikko, valitsemalla Työkalut, sitten Etäkäyttö ja lopuksi "Mykistä etäpuhe".
* Jos olet muodostanut yhteyden hallitsevana koneena, lähetä `Ctrl+Alt+Del`-komento hallittavalle koneelle avaamalla NVDA-valikko, valitsemalla Työkalut, sitten Etäkäyttö ja lopuksi "Lähetä Ctrl+Alt+Del".
  Jotta tämä toimisi, hallittavassa koneessa on oltava käynnissä NVDA:n asennettu versio.
  Tämä toiminto ei välttämättä toimi kaikissa tilanteissa, sillä se riippuu hallittavan koneen asetuksista.

### Yhteenveto etäkäytön näppäinkomennoista {#RemoteAccessGestures}

<!-- KC:beginInclude -->
| Nimi | Näppäinkomento | Kuvaus|
|---|---|---|
| Yhdistä tai katkaise yhteys | `NVDA+Alt+R` | Aloittaa uuden etäkäyttöistunnon tai katkaisee käynnissä olevan istunnon yhteyden. |
| Vaihda hallintaa | `NVDA+Alt+Sarkain` | Vaihtaa etä- ja paikallisen koneen hallinnan välillä. |
| Yhdistä | Ei mitään | Aloittaa uuden etäkäyttöistunnon. Ei käytettävissä [suojatussa tilassa](#SecureMode). |
| Kopioi linkki | Ei mitään | Kopioi etäistunnon linkin leikepöydälle. |
| Katkaise yhteys | Ei mitään | Lopettaa käynnissä olevan etäkäyttöistunnon. |
| Mykistä etäkone | Ei mitään | Mykistää etäkoneelta tulevan puheen tai poistaa sen mykistyksen. |
| Lähetä leikepöytä | Ei mitään | Lähettää leikepöydän sisällön etäkoneelle. |
<!-- KC:endInclude -->
...
#### Lisäosaluettelon lajittelu sarakkeen perusteella {#AddonStoreSortByColumn}

Lisäosaluettelo lajitellaan oletusarvoisesti lisäosien näyttönimen perusteella.
"Lajitteluperuste"-yhdistelmäruutua voidaan käyttää luettelon lajitteluun kunkin välilehden sarakkeiden perusteella.
Saatat esimerkiksi haluta lajitella lisäosat julkaisijan, saatavilla olevan version jne. mukaan.
Lajittelu on mahdollista nousevaan tai laskevaan järjestykseen.
...
Mikäli lataus tai asennus epäonnistuu, voit yrittää asennusta uudelleen.
Asennuksen voi myös peruuttaa ennen lisäosakaupan sulkemista.
...
Voit poistaa lisäosan käytöstä, mikäli sen tila on "Käytössä", tai ottaa sen käyttöön, mikäli tilana on "Ei käytössä".
...
#### Automaattisten päivitysten kanavan vaihtaminen {#AddonStoreUpdateChannel}

Automaattisten lisäosapäivitysten kanavia voidaan hallita [asennettujen ja päivitettävien lisäosien välilehdiltä](#AddonStoreFilterStatus).
Kun [automaattiset lisäosapäivitykset](#AutomaticAddonUpdates) ovat käytössä, lisäosat päivittyvät  samalle [kanavalle](#AddonStoreFilterChannel), josta ne asennettiin [oletusarvoisesti](#DefaultAddonUpdateChannel).
Lisäosan toimintovalikon "Päivityskanava"-alivalikosta voit vaihtaa kanavaa, jolle lisäosa päivittyy automaattisesti.

| Asetus | Toiminta |
|---|---|
| Oletus | Lisäosa seuraa [oletusarvoista päivityskanavaa](#DefaultAddonUpdateChannel). |
| Sama | Lisäosa pysyy samalla kanavalla. |
| Mikä tahansa | Lisäosa päivittyy automaattisesti uusimpaan versioon kanavasta riippumatta. |
| Älä päivitä | Lisäosa ei päivity automaattisesti. |
| Vakaa | Lisäosa päivittyy automaattisesti vakaisiin versioihin. |
| Beeta tai kehitys | Lisäosa päivittyy automaattisesti beeta- tai kehitysversioihin. |
| Beeta | Lisäosa päivittyy automaattisesti beetaversioihin. |
| Kehitys | Lisäosa päivittyy automaattisesti kehitysversioihin. |
...
Joskus Windowsin rekisterissä voi ilmetä ongelmia, jotka saavat NVDA:n toimimaan epänormaalisti.
Tämä voi johtua esimerkiksi tiettyjen ohjelmien (kuten Adobe Readerin tai Math Playerin) asentamisesta tai poistamisesta, Windowsin päivityksistä tai muista järjestelmätapahtumista.
COM-rekisteröintien korjaustyökalu yrittää ratkaista tällaiset ongelmat korjaamalla rekisterin saavutettavuusmerkinnät.

Korjattavat ongelmat voivat olla seuraavanlaisia:

* NVDA ilmoittaa "tuntematon" tai "ruutu" liikuttaessa selaimissa (esim. Firefox tai Edge), sähköpostiohjelmissa (esim. Thunderbird), Windowsin Resurssienhallinnassa, tehtäväpalkissa tai muissa ohjelmissa.
* NVDA ei vaihda vuorovaikutustilasta selaustilaan odotetulla tavalla.
* Painikkeet, joiden nimet puhuttiin aiemmin, ilmoitetaan yllättäen vain sanalla "painike".
* NVDA on erittäin hidas selaimissa selaustilaa käytettäessä.

Koska tämä työkalu korjaa Windowsin rekisterimerkintöjä, sen käyttö edellyttää järjestelmänvalvojan oikeuksia, aivan kuten ohjelman asentaminenkin.
Mikäli käyttäjätilien valvonta on käytössä, sinun on seurattava sen kehotteita, jotta työkalun käyttö onnistuu.
...
Kattavampi luettelo ilmaisista ja kaupallisista syntetisaattoreista, joita on mahdollista ostaa ja ladata NVDA:lla käytettäväksi, löytyy [lisä-äänien sivulta](https://github.com/nvaccess/nvda/wiki/ExtraVoices).

### eSpeak NG {#eSpeakNG}

[ESpeak NG](https://github.com/espeak-ng/espeak-ng) -puhesyntetisaattori on sisäänrakennettu NVDA:han, eikä se tarvitse muita erikoisajureita tai -komponentteja.
...
Lisää uusia OneCore-ääniä menemällä Windowsin järjestelmäasetuksiin (Win+I) ja valitsemalla sieltä ensin "Aika ja kieli" ja sitten "Puhe".
...
Näiden näyttöjen käyttämiseksi on asennettava Freedom Scientificin pistenäyttöajurit.
...
|Win+Sarkain-näppäinyhdistelmä | `väli+pisteet 2, 3 ja 4`|
...
|Vaihda Windows-näppäimen tilaa | `väli+pisteet 4 ja 8`|
...
|Vaihda Windows- ja Vaihto-näppäinten tilaa | `väli+pisteet 4, 7 ja 8`|
...
|Win+D-näppäinyhdistelmä (pienennä kaikki sovellukset) | `väli+pisteet 1, 2, 3, 4, 5 ja 6`|
...
Kun pariliitos on tehty tätä apuohjelmaa käyttäen ja NVDA ei tunnista laitettasi, suosittelemme käyttämään laiteparin muodostamiseen Windowsin Bluetooth-asetuksia.
...
|Lue päivämäärä/kellonaika | `sp2+sp3`|
|NVDA-valikko | `sp1+sp3`|
|Win+D-näppäinyhdistelmä (pienennä kaikki sovellukset) | `sp1+sp4`|
|Win+B-näppäinyhdistelmä (siirrä kohdistus ilmoitusalueelle) | `sp3+sp4`|
...
* Baum: SuperVario, PocketVario, VarioUltra, Pronto!, SuperVario 2, Vario 340
* HumanWare: Brailliant, BrailleConnect, Brailliant 2
* APH: Refreshabraille
* Orbit: Orbit Reader 20

Muutkin Baumin valmistamat pistenäytöt voivat toimia, vaikkei niitä olekaan testattu.

Mikäli tietokoneeseen kytketään USB:llä pistenäyttö, joka ei käytä HID:tä, valmistajan toimittamat ajurit on asennettava ensin.
...
|`Win+D`-näppäinyhdistelmä (pienennä kaikki sovellukset) | `väli+pisteet 1, 4 ja 5` (`väli+d`)|
...
Seuraavassa ovat Brailliant BI/B- ja BrailleNote Touch -näyttöjen näppäinkomennot NVDA:ta käytettäessä.
...
|Win+D-näppäinyhdistelmä (pienennä kaikki sovellukset) | `c1`, `c4` ja `c5` (komento d)|
...
|Win+B-näppäinyhdistelmä | `pisteet 1 ja 2+f1`|
|Win+D-näppäinyhdistelmä | `pisteet 1, 4 ja 5+f1`|
...
* Laiteajurit on asennettava ennen Seika v3-, 4-, 5- ja 80-näyttöjen käyttöä.
Ajurit ovat ladattavissa [valmistajan verkkosivulta.](https://en.seika-braille.com/down/index.html)
...
* Seika Notetaker -pistenäytön käyttämiseen ei tarvita lisäajureita.
...
|Windows-näppäin | `askelpalautin+RJ keskikohta`|
...
Portti, johon laite on kytketty, tulee valita sen jälkeen, kun pistenäytön ajuri on valittu [Valitse pistenäyttö](#SelectBrailleDisplay) -valintaikkunasta.
...
#### Pistenäppäimistön toiminnot {#EurobrailleBraille}
...
Windows 10 ja uudemmat tunnistavat näytön tietokoneeseen kytkettäessä, mutta vanhempia Windows-versioita käytettäessä USB-ajureiden asennus voi olla tarpeen.
...
Käytettävää pistenäyttöä ja porttia määritettäessä on syytä kiinnittää huomiota ohjeisiin, erityisesti jos käytetään USB-pistenäyttöä ja valmistajan ajurit on jo asennettu.
...
|`Win+D` (pienennä kaikki sovellukset) | `attribute1`|
|`Win+E` (tämä tietokone) | `attribute2`|
|`Win+B` (siirrä kohdistus ilmoitusalueelle) | `attribute3`|
|`Win+I` (Windowsin asetukset) | `attribute4`|
...
Dot Inc:in Dot Pad on laite, joka pystyy näyttämään sekä kohografiikkaa että pistekirjoitusta.
NVDA voi näyttää joko yhden pistekirjoitusrivin laitteen omalla pisterivillä tai useita pistekirjoitusrivejä sen kohografiikka-alueessa.
A300-mallissa on 120 x 80 pisteen kohografiikka-alue, johon mahtuu 8 riviä, joissa kussakin on 20 solua.

Voit määrittää, näytetäänkö pistekirjoitus omalla pisterivillään vai kohografiikka-alueella valitsemalla NVDA:n pistekirjoitusasetuksista tämän ajurin "Pistekirjoituksen kohde" -asetuksen.

panorointinäppäimiä tuetaan, mutta laitteen rajoitetun näppäinmäärän vuoksi muita komentoja ja kohdistimen siirtämistoimintoja ei ole tällä hetkellä käytettävissä.

Kun Dot Padin ajuri valitaan NVDA:ssa, virtuaalinen USB- tai Bluetooth-sarjaportti, johon laite on kytketty, täytyy valita manuaalisesti.
Tämä ajuri ei tue automaattista tunnistusta.

Huom: Laitteiston rajoitusten vuoksi Dot Pad ei päivitä kaikkia pisteitä oikein, kun kätesi on laitteen päällä.
Varmista, että nostat kätesi kokonaan pois laitteelta NVDA:lla navigoidessasi, ja aloita lukeminen vasta, kun kaikki pisteet ovat täysin päivittyneet.

<!-- KC:beginInclude -->

| Nimi | Näppäinkomento|
|---|---|
|Vieritä taaksepäin | `pan_left` |
|Vieritä eteenpäin | `pan_right` |
...
NVDA sallii mukautettujen lisäosien asentamisen, jotka voivat suorittaa mielivaltaista koodia myös silloin, kun NVDA:lla on järjestelmänvalvojan oikeudet.
NVDA sallii myös käyttäjien suorittaa omaa koodiaan Python-konsolin kautta.
...
* Ulkoisten asiakirjojen, kuten käyttöopastiedoston, avaaminen NVDA-valikosta.
...
Järjestelmänvalvojien oletetaan rajoittavan luvattomien ohjelmien, kuten NVDA:n massamuistiversioiden, käyttöä järjestelmissään.

Käyttäjät muokkaavat usein NVDA-profiiliaan tarpeisiinsa sopivaksi.
Tähän saattaa sisältyä mukautettujen lisäosien asentaminen ja asetusten määritys, jotka tulisi tarkistaa erillään NVDA:sta.
Suojattu tila estää NVDA:n asetusten muuttamisen, joten varmista, että asetukset on määritetty halutulla tavalla ennen suojatun tilan käyttöönottoa.
...
NVDA käyttää asetuksia varten järjestelmäprofiilia ollessaan käynnissä suojatussa ruudussa.
...
Valitsimet voidaan antaa pikakuvakkeesta käynnistettäessä (syötetään kuvakkeen Ominaisuudet-valintaikkunassa olevaan Kohde-kenttään), Suorita-valintaikkunasta (Käynnistä-valikko -> Suorita tai Win+R) tai Windowsin komentokonsolista.
...
|`-n KIELI` | `--lang=KIELI` | Ohita NVDA:n asetuksissa määritetty kieli. Määritä kieleksi "Windows" käyttääksesi nykyistä käyttäjän oletusarvoa, "en" englantia jne.|
|`-m` | `--minimal` | Ei ääniä, käyttöliittymää tai käynnistysilmoitusta jne.|
|`-s` | `--secure` | Käynnistää NVDA:n [suojatussa tilassa](#SecureMode).|
|`-d` | `--disable-addons` | Poistaa lisäosat käytöstä.|
|Ei mitään | `--debug-logging` | Ottaa käyttöön virheenkorjaus-lokitason vain nykyisessä istunnossa. Tämä asetus korvaa minkä tahansa muun annetun lokitason argumentin (`--loglevel`, `-l`), myös lokin käytöstä poistavan valitsimen.|
...
|Ei mitään | `--create-portable-silent` | Luo NVDA:n massamuistiversion käynnistämättä sitä luonnin jälkeen. Parametri `--portable-path` on myös määritettävä. Tämä valitsin estää varoitukset kirjoitettaessa ei-tyhjiin hakemistoihin ja saattaa korvata tiedostoja varoituksetta.|
|Ei mitään | `--portable-path=POLKU` | Hakemistopolku, johon massamuistiversio luodaan.|

Aivan kuten NVDA voidaan asentaa hiljaisesti antamalla sille `--install-silent`-komentorivivalitsin, se voidaan poistaa hiljaisesti antamalla poisto-ohjelmalle `/S`-valitsin.

NVDA:n poisto-ohjelma on nimeltään `uninstall.exe` ja se sijaitsee NVDA:n asennushakemistossa, joka on 64-bittisessä Windowsissa `%ProgramFiles(x86)%\NVDA` tai 32-bittisessä `%ProgramFiles%\NVDA`.

Seuraavassa ovat NVDA:n poisto-ohjelman komentorivivalitsimet:

| Lyhyt | Pitkä | Kuvaus|
|---|---|---|
|`/S` | Ei mitään | Poista NVDA hiljaisesti. |
