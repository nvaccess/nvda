...
Windows 文字認識の手動リフレッシュ、自動更新の切り替えなどを行う新しいコマンドが追加されました。
...
* ウェブブラウザ:
  * NVDA ブラウズモードのネイティブ選択モード (`NVDA+shift+f10`) が Google Chrome、Edge、およびChromium 134以降に基づく他のアプリでサポートされるようになりました。 (#17838)
...
* Microsoft Office:
  * PowerPointのテキストボックスで編集する場合、`alt+upArrow`/`alt+downArrow`で文ごとに移動できるようになりました。(#17015, @LeonarddeR)
  * Word では、Word のコマンドを使用して選択範囲を拡張または縮小する場合（`f8` または `shift+f8`）、選択範囲の更新が報告されるようになりました。（#3293, @CyrilleB79）
  * Word 16.0.18226以降またはWordオブジェクトモデルを使用している場合、NVDAは見出しが折りたたまれているかどうかを音声と点字の両方で報告するようになりました。（#17499）
  * Word の UIAで、ネイティブの拡張テーブルナビゲーションコマンド (`alt+home`, `alt+end`, `alt+pageUp`, `alt+pageDown`) を使用する場合、テキストカーソルの移動が報告されるようになりました。 (#17867, @CyrilleB79)
  ...
  * より多くのショートカットキー操作の結果がWordとOutlookで報告されるようになりました:
    * フォント書式ショートカット (#10271, @CyrilleB79)
    * 見出しの折りたたみまたは展開 (#17545, @CylleB79）
* LibreOffice:
  * 対応するキーボードショートカットを使用して Writer のフォントサイズを増減すると、NVDA は新しいフォントサイズを報告します。(#6915, @michelweghorn)
  * Writer 25.2以降で対応するキーボードショートカットを使用して「本文」または見出しの段落スタイルを適用すると、NVDAは新しい段落スタイルを報告するようになりました。（#6915, @michaelweghorn）
  * Writerで対応するキーボードショートカットを使用して二重下線を切り替えると、NVDAは新しい状態（「二重下線オン」/「二重下線オフ」）を報告するようになりました。（#6915, @michaelweghorn）
  * NVDA は LibreOffice 25.8 以降で最初の行インデントを報告できるようになりました。 (#13052, @michelweghorn)
* 文字認識:
  * 認識結果において `NVDA+f5` を押すと、認識されたコンテンツを手動で更新します。(#17715, @CyrilleB79)
...
* NVDAは、Visual Studio Codeなどで `alt+上矢印` または `alt+下矢印` のジェスチャーを押した際に、テキストカーソルの位置の変更を報告できるようになりました。（#17652, @LeonarddeR）
* 選択されたテキストの最初または最後の文字にレビューカーソルを移動するコマンドが追加され、それぞれ `NVDA+alt+home` と `NVDA+alt+end` に割り当てられました。（#17299, @nvdaes）
...
* コンポーネントの更新:
  * LibLouis点訳エンジンを[3.33.0](https://github.com/liblouis/liblouis/releases/tag/v3.33.0)に更新しました。(#17469, @LeonarddeR, @codeofdusk)
    * コプト語、縮約の楔形文字、およびポルトガル語6点コンピューター点字の点字テーブルが追加されました。
  * Unicode CLDRを 46.0 に更新しました。 (#17484, @OzancanKaratas)
  * eSpeak NG をバージョン 1.52.0 に更新しました。(#17056)
* NVDA のユーザーインターフェイスの変更点:
  ...
  * よく使用されるコマンドラインオプションの短縮版が追加されました：`--disable-addons` の短縮版として `-d`、`--lang`の短縮版として`-n`が追加されました。
  コマンドラインフラグの接頭辞マッチング（例えば`--disable-addons`に対して`--di`を使用するなど）はサポートされなくなりました。(#11644, @CyrilleB79)
  * COM登録修復ツールの変更点: （#12355, @XLTechie）
    * 警告の代わりに、よりわかりやすい目的の説明から始まるようになりました。（#12351）
    * 最初のウィンドウは `escape` または `alt+f4` で終了できるようになりました。(#10799)
...
* 音声:
  * 一部のSAPI5音声合成エンジンを使用している場合に、連続読み上げ（すべて読み上げ）が最初の文の末尾で停止する問題を修正しました。(#16691, @gexgd0419)
  * 簡単音声設定を使用してSAPI5およびSAPI4音声合成エンジンのあいだで音声を切り替える際に、速度や音量などが既定の値にリセットされなくなりました。(#17693, #2320, @gexgd0419)
  * 一部のSAPI4音声（例：IBM TTS中国語）が読み込めない問題を修正しました。(#17726, @gexgd0419)
* 点字ディスプレイ:
...
* Microsoft Office:
  ...
  * Excelでは、要素リストダイアログ（`NVDA+f7`）が一部の非英語システムでコメントや数式を一覧に表示できなかった問題が解消されました。（#11366、@CyrilleB79）
  * PowerPointの改善:
    * テキストに絵文字などのマルチバイト文字が含まれている場合、テキストカーソル移動の報告が正しく動作するようになりました。 (#17006 , @LeonarddeR)
    * 文字の位置の報告が正確になりました（例：`NVDA+delete` を押した場合）。（#9941, @LeonarddeR）
    * ブラウズモード設定の「ページ読み込み時に自動的にすべてを読み上げ」が無効になっている場合、NVDAはスライドショーの開始時にすべての読み上げを開始しなくなりました。（#17488, @LeonarddeR）
* LibreOffice:
...
* 数式:
  ...
  * リッチナビゲーション用のMathMLがなく、画像とalt属性のテキストだけで表現された数式は、内容のない数式ではなく通常の画像として扱われるようになりました。これにより、ユーザーは `g` キーでそれらにジャンプでき、alt属性のテキストを文字単位で矢印キーで移動できるようになりました。(#16007)
* IDE:
  * Android StudioまたはIntelliJ Ideaで特定のソースファイル内のすべてのテキストを選択するとNVDAがクラッシュしていた問題を修正しました。 (#17418, @thgcode)
  * Visual Studio Code において、NVDA は `alt+upArrow` と `alt+downArrow` ジェスチャーをハイジャックしなくなりました。(#17082, @LeonarddeR)
...
* 設定プロファイル:
  * 関連する設定プロファイルで「すべて読み上げ」を有効にしても、点字が機能しなくなる問題が解消されました。（#17163, @LeonarddeR）
  * 設定の値がベース設定の値と同じであっても、特定の設定が明示的にアクティブな設定プロファイルに保存されていた問題を修正しました。（#17157, @leonarddeR）
* スペルチェック時に、Unicode 正規化がより適切に機能するようになりました:
  * 正規化された文字を報告した後、NVDAが後続の文字を誤って正規化されたものとして報告する問題が解消されました。（#17286, @LeonarddeR）
  * 合成文字（éなど）が正しく報告されるようになりました。（#17295, @LeonarddeR）
...
* `synthDriverHandler` に新しい拡張ポイント`pre_synthSpeak`が追加されました。これは音声マネージャーが現在の音声合成エンジンの`speak`を呼び出す前に呼ばれます。(#17648)
* NVDA は `text-indent` IAccessible2 オブジェクト属性をサポートしました。 (#13052, @michelweghorn)
* `gesture.send`を使用するスクリプトが`numLock`を修飾キーとして含むジェスチャーで実行された場合、NumLockがオフになることがなくなりました。(#10827, @CyrilleB79)
