...
* 遠隔サポートや共同作業のために [NVDAを実行している他のPCに接続して制御する]（#RemoteAccess）機能
* インストールなしでUSBメモリーや他の携帯型メディアから実行が可能
* 簡単に利用できるトーキングインストーラー
* 54ヶ国語への翻訳
* 32ビット版および64ビット版の最新のWindowsへの対応
* Windowsのサインインおよび[その他のセキュアデスクトップ](#SecureScreens)への対応
...
NVDAは現在、英語以外の55の言語に翻訳されています: アフリカーンス語、アルバニア語、アムハラ語、アラビア語、アラゴン語、ボスニア語、ブルガリア語、ビルマ語、カタロニア語、中国語 (簡体字、繁体字)、クロアチア語、チェコ語、デンマーク語、オランダ語、ファルシー語、フィンランド語、フランス語、ガリシア語、グルジア語、ドイツ語(ドイツ、スイス)、ギリシャ語、ヘブライ語、 ヒンディー語、ハンガリー語、アイスランド語、アイルランド語、イタリア語、カンナダ語、韓国語、キルギス語、リトアニア語、マケドニア語、モンゴル語、ネパール語、ノルウェー語、ポーランド語、ポルトガル語 (ブラジルとポルトガル) パンジャビ、ルーマニア語、ロシア語、セルビア語、スロバキア語、スロベニア語、スペイン語(コロンビアとスペイン)、スウェーデン語、タミル語、タイ語、トルコ語、ウクライナ語、ベトナム語。
...
NVDA の一時的な環境の読み込みが完了するまで数秒待つ必要があります。
NVDA の読み込みが終わると、NVDA がインストールの操作を音声で読み上げます。
1. NVDA インストーラーのウィンドウが表示されます。ここには「使用許諾契約」を開くボタンがあります。
...
| 名称 |デスクトップ用キー |ラップトップ用キー |説明|
|---|---|---|---|
|入力文字の読み上げ | `NVDA+2` | `NVDA+2` | NVDAがキーボードから入力された文字を読み上げるかどうかを設定します。 |
|入力単語の読み上げ | `NVDA+3` | `NVDA+3` | NVDAがキーボードから入力された単語を読み上げるかどうかを設定します。 |
...
NVDA について情報を得るには `NVDA+N` で NVDA メニューを開いて `H` を押します。NVDA日本語版のこのサブメニューには「NVDA日本語版の説明」があります。この中で [NVDA日本語版のウェブサイト](https://www.nvda.jp/) と [NVDA日本語版ガイドブック](https://nvdajp-book.readthedocs.io/ja/latest/) を紹介しています。
...
このオプションでは、現在実行中のポータブル版NVDAの設定を、現在サインイン中のユーザーの設定としてコピーし、インストールされるNVDAで使用するかどうかを選択できます。
...
既定では、NVDAはデスクトップ配列を使用するように設定されていますが、NVDAメニューの「設定」にある[NVDA設定](#NVDASettings)ダイアログの「キーボード」カテゴリで、ラップトップ配列に切り替えることもできます。
...
|選択範囲の先頭に移動 | `NVDA+Alt+Home` | `NVDA+Alt+Home` | なし | レビューカーソルを選択されたテキストの先頭の文字に移動します。|
|選択範囲の最後に移動 | `NVDA+Alt+End` | `NVDA+Alt+End` | なし | レビューカーソルを選択されたテキストの最後の文字に移動します。|
...
ただし、NVDAには「ネイティブ選択モード」があり、これを有効にできるブラウズモード対応のドキュメント（Mozilla Firefoxや、Chromium 134以降をベースとしたブラウザーなど）では、NVDAのブラウズモードでの選択がドキュメントのネイティブな選択に反映されるようになります。
...
`NVDA+f5` を押すと認識結果のドキュメントが更新されます。
...
NVDAは、特定のタスクを簡単に実行できるようにしたり、スクリーンリーダーに配慮した操作方法が用意されていない機能を利用可能にしたりするために、アプリごとに特別な機能を提供しています。
...
チェックすると、NVDA を終了しようとすると以下のの選択肢のダイアログが表示されます。

* 終了
* 再起動
* アドオンを無効にしてデバッグログを有効にして再起動
* デバッグログを有効化して再起動
* 保留中の更新を実行(該当する場合)
...
保留中の更新は、有効になっている場合は NVDA の終了ダイアログから、または NVDA メニューや「ヘルプ」メニューの「更新の確認」から手動で実行することもできます。

##### 更新のミラー {#UpdateMirror}

これらのコントロールでは、NVDA の更新を確認するために使用する代替の URL を指定できます。
これは、NV Access の NVDA 更新サーバーへのアクセスが遅い場合や利用できない場所で役立つことがあります。

読み取り専用テキストボックスは、現在のミラー URL を表示します。
ミラーが使用されていない場合(通常の更新サーバーが使用されている場合)、「ミラーなし」と表示されます。

更新ミラーを変更したい場合は、「変更...」ボタンを押して、[更新ミラーの設定ダイアログ](#SetURLDialog)を開きます。

なお、更新ミラーを使用する場合、ミラーの運営者は[更新チェックのときに送信されるすべての情報](#GeneralSettingsCheckForUpdates)にアクセスできることにご注意ください。

ご自身の情報がどのように扱われるかを確認したい場合は、更新ミラーを設定する前に、ミラーの運営者にデータの取り扱い方針を問い合わせてください。

##### 読み上げ中と点字表示中のディスプレイのオフを防止 {#PreventDisplayTurningOff}

このチェックボックスを有効にすると、「すべて読み上げ」や点字で読み上げているとき（例：スクロールボタンを押した場合など）に、ディスプレイが消えずに表示されたままになります。
これにより、ディスプレイが突然ロックされる状況を回避します。
このオプションは初期状態でチェックされています。
バッテリーの持ちが悪くなっている場合は、このオプションを無効にすることを検討してください。
...
音声設定カテゴリの最初のオプションは「変更」ボタンです。
このボタンを押すと、[音声エンジンの選択](#SelectSynthesizer)ダイアログが開き、使用する音声エンジンを選択できます。
...
「自動」を選択すると、NVDAの現在の言語に従ってテーブルが選択されます。
...
##### 行や段落の移動で読み上げ {#BrailleSpeakOnNavigating}

このオプションを有効にすると、点字ディスプレイのナビゲーション操作で移動した行や段落がNVDAによって読み上げられるようになります。

このオプションをどこからでも切り替えられるようにするには、[入力ジェスチャーのダイアログ](#InputGestures)の「点字」セクションにある「点字で行や段落を移動するときに読み上げる」にカスタムジェスチャーを割り当てます。
...
各ドライバーの点字ディスプレイの自動検出への対応状況は [サポートされている点字ディスプレイ](#SupportedBrailleDisplays) で確認してください。
...
* 左チャンネルでNVDA、両方のチャンネルでアプリ: NVDAは左チャンネルで音声を出力し、他のアプリは左右両方のチャンネルで音を再生します。
...
このオプションでは、キーボードで入力した文字をNVDAが読み上げるタイミングを設定します。
使用可能なオプションは次のとおりです:

* オフ: NVDAは入力した文字を読み上げません。
* エディットコントロールのみ: NVDAは、エディットコントロールや文字を入力できる他の領域で入力された文字のみを読み上げます。
* 常に: NVDAは入力した文字を常に読み上げます。
...
このオプションでは、キーボードで入力した単語をNVDAが読み上げるタイミングを設定します。
使用可能なオプションは次のとおりです:

* オフ: NVDAは入力した単語を読み上げません。
* エディットコントロールのみ: NVDAは、エディットコントロールや文字を入力できる他の領域で入力された単語のみを読み上げます。
* 常に: NVDAは入力した単語を常に読み上げます。
...
このオプションは既定でオンになっていますが、UniKeyという入力ソフトを使ってベトナム語を入力するユーザーなど、一部のユーザーは無効にしたほうがよい場合があります。これを有効にしていると、正しく文字が入力されないことがあります。
...
##### 自動更新 {#AutomaticAddonUpdates}

このオプションが「通知」に設定されている場合、NVDAの起動後にアドオンの更新が利用可能であればアドオンストアが通知します。
このチェックは24時間ごとに行われます。
既定では、通知は同じ[チャンネル](#AddonStoreFilterChannel)（例：安定版、ベータ版、開発版）で更新が利用可能なアドオンに対してのみ行われます。
アドオンの更新チャンネルは、[各アドオンごとに個別に](#AddonStoreUpdateChannel)または[すべてのアドオンに対して一括で](#DefaultAddonUpdateChannel)設定できます。

「自動的に更新」に設定すると、アドオンは自動的にバックグラウンドで更新されます。
更新が完了するとNVDAを再起動するように求められます。

| . {.hideHeaderRow} |.|
|---|---|
|選択肢 | 通知(既定の値)、自動的に更新、無効 |
|既定の値 |通知 |

|選択肢 |動作 |
|---|---|
|通知 | アドオンで更新が利用可能になったら通知する |
|自動更新|バックグラウンドでアドオンを自動的に更新する |
|無効 |アドオンの更新を自動的にチェックしない |

##### 既定の更新チャンネル {#DefaultAddonUpdateChannel}

[アドオンの自動更新](#AutomaticAddonUpdates)が有効になっている場合、既定ではアドオンは同じ[チャンネル](#AddonStoreFilterChannel)の更新のみ適用されます。
たとえば、インストールされているベータ版は、新しいベータ版にのみ更新されます。
このオプションは、すべてのアドオンの既定の更新チャンネルを設定します。
[アドオンストアから個別に特定のアドオン](#AddonStoreUpdateChannel)の更新チャンネルを変更することもできます。

| . {.hideHeaderRow} |.|
|---|---|
| 選択肢 | 同じ（既定）、すべて、更新しない、安定版、ベータ版または開発版、ベータ版、開発版 |
| 既定 | 同じ |

| 選択肢 | 動作 |
|---|---|
| 同じ | アドオンはそのチャンネルに残ります |
| すべて | アドオンは、チャンネルに関係なく自動的に最新バージョンに更新されます |
| 更新しない | アドオンは既定で自動的に更新されません。個別に自動更新を有効にする必要があります。 |
| 安定版 | アドオンは自動的に安定版に更新されます |
| ベータ版または開発版 | アドオンは自動的にベータ版または開発版に更新されます |
| Beta | アドオンは自動的にベータ版に更新されます |
| Dev | アドオンは自動的に開発版に更新されます |

##### 自動更新で互換性のないアドオンのインストールを許可 {#AllowIncompatibleAddonUpdates}

この設定を有効にすると、現在のNVDAのバージョンと完全には互換性がない可能性のあるアドオンも自動的に更新されます。
既定ではこの設定は無効になっており、自動更新は現在のNVDAのバージョンと互換性があるとマークされたアドオンのバージョンにのみ適用されます。
自動更新は、互換性のないアドオンであっても、互換性のあるバージョンがリリースされれば、そのバージョンに更新されます。
この設定を有効にすると、アドオン互換性に大きな変更があるリリース（その年の最初のリリース）へ移行する際に役立つ場合があります。
この設定は、アドオン互換性に大きな変更が加えられるリリースの初期段階でアドオンの互換性をテストしているアルファ版やベータ版のテスターにとって特に有用です。

##### ミラーサーバー {#AddonStoreMetadataMirror}

これらのコントロールを使用すると、アドオンストアのデータをダウンロードするための代替URLを指定できます。
これは、NV Access のアドオンストアのサーバーへのアクセスが遅い場合や利用できない場所で役立つことがあります。

読み取り専用テキストボックスは、現在のミラー URL を表示します。
ミラーが使用されていない場合(通常のアドオンストアサーバーが使用されている場合)、「ミラーなし」と表示されます。

アドオンストアのミラーを変更したい場合は、「変更...」ボタンを押して、[ミラーの設定ダイアログ](#SetURLDialog)を開きます。

#### リモートアクセスの設定 {#RemoteSettings}

このカテゴリでは、[リモートアクセス](#RemoteAccess)の動作を設定できます。

##### リモート アクセスを有効にする {#RemoteEnable}

NVDAのリモートアクセス機能を有効または無効にするには、このチェックボックスを使用します。

これがチェックされていない場合、リモートアクセス機能は完全に無効化されます。

ユーザーアカウント制御のプロンプトなどのセキュア画面でリモートアクセスのセッションを継続するには、この設定を有効にし、変更を保存したうえで、[現在保存されている情報をサインインおよびセキュアデスクトップで使用](#GeneralSettingsCopySettings)する必要がありますのでご注意ください。
リモートアクセスは、セキュアデスクトップにおいては、NVDAのインストール版の環境で開始されたセッションのみを継続します。
例えば、 NVDAのポータブル版でリモートアクセスのセッションを開始した場合には、セキュア画面でセッションを継続できません。

以下のオプションは、リモートアクセスが有効な場合にのみ使用できます。

##### 制御されるPCの場合に切断の前に確認 {#RemoteConfirmDisconnect}

このオプションは、制御されるPCとして接続されているときにリモートアクセスセッションからの切断の前に確認が必要かどうかを設定します。

チェックすると、NVDAは切断の前に確認をします。
チェックなしにすると、NVDAは確認せずにすぐに切断します。

##### NVDA起動後に自動的に接続 {#RemoteAutoconnect}

このオプションを使用すると、NVDAの起動時に自動的にリモートアクセスのセッションを確立します。
たとえば、自宅から離れているときに自宅のPCを操作する場合などに便利です。

このオプションを有効にすると、PCへの不正アクセスのリスクが高まる可能性があるため、ご注意ください。

以下のオプションの詳細については、「リモートアクセスセッションの設定」(#RemoteAccessSetup)を参照してください。

##### モード {#RemoteAutoconnectMode}

自動接続のモードを選択します。

このオプションは、[NVDA 起動後に自動的に接続](#RemoteAutoconnect)が有効になっている場合にのみ利用できます。


|選択肢 | 動作 |
|---|---|
|このPCの操作を許可 | このPCを「操作される側」として使用します。これにより「操作する側」のPCのユーザーが、このPCを目の前で操作しているかのように使用できるようになります。
 |
|別のPCを操作 | このPCを「操作する側」として使用します。これにより、リモートPC上でコマンドを実行できるようになります。 |

##### サーバー {#RemoteAutoconnectServer}
...
|選択肢 | 動作 |
|---|---|
|既存のサーバーを使用 | 既存のリモートアクセスサーバーを経由して接続します（推奨）。 |
|ローカルでホスト | このNVDAをリモートアクセスサーバーとして使用します（上級者向け）。このオプションを使用するには、本マニュアルの範囲外となるネットワーク設定が必要になる場合があります。
 |

##### ホスト {#RemoteAutoconnectHost}

このフィールドを使用して、自動接続に使用するリモートアクセスサーバーの URL を設定します。
サーバーが 6837 以外のポートを使用している場合は、コロンの後に (例えば `example.com:1234`) を含めます。

このオプションは、[NVDA 開始後に自動的に接続する](#RemoteAutoconnect)がチェックされていて、 [Server](#RemoteAutoconnectServer) が「既存のサーバーを使用」に設定されている場合にのみ使用できます。

##### ポート {#RemoteAutoconnectPort}

このフィールドを使用して、自動接続に使用する TCP ポートを設定します。

このオプションは、[NVDA 開始後に自動的に接続する](#RemoteAutoconnect)がチェックされていて、 [Server](#RemoteAutoconnectServer) が「ローカルでホスト」に設定されている場合にのみ使用できます。

##### 鍵 {#RemoteAutoconnectKey}

このフィールドを使用して、自動接続に使用する鍵(パスワード)を設定します。

このオプションは、[NVDA 起動後に自動的に接続](#RemoteAutoconnect)が有効になっている場合にのみ利用できます。

##### 信頼できるフィンガープリントをすべて削除 {#RemoteDeleteFingerprints}

このボタンを使用すると、以前に信頼されていたすべてのリモートアクセスサーバーのフィンガープリントを削除できます。
これにより、以前に接続したことがあるサーバーも含め、未承認のすべてのリモートアクセスサーバーに接続するかどうかを再び確認されるようになります。
すべての信頼済みフィンガープリントを削除する前に、確認が求められます。

このオプションは、信頼できるフィンガープリントが保存されている場合にのみ使用できます。
...
#### ミラー設定ダイアログ {#SetURLDialog}

このダイアログでは、[NVDAの更新](#GeneralSettingsCheckForUpdates)や[アドオンストアの使用](#AddonsManager)時に使用するミラーのURLを指定できます。
これらの機能に必要な標準のサーバーへのアクセスが遅い、または利用できない場所で役立つ場合があります。

* [NVDAの更新ミラー](#UpdateMirror)を設定する場合、このダイアログのタイトルは「NVDA更新ミラーの設定」になります。
* [アドオンストアのミラー](#AddonStoreMetadataMirror)を設定する場合、このダイアログのタイトルは「アドオンストアのミラーサーバーの設定」になります。

##### URL {#SetURLTextbox}

使用したいミラーのURL(ウェブアドレス)を入力します。
HTTP と HTTPS の URL のみがサポートされます。
プライバシーを守るために、可能な限り HTTPS の URL を使用することをお勧めします。

既定のサーバーを使用する場合は空白のままにしてください。

##### テスト {#SetURLTest}

入力したミラー URL をテストするには、このボタンを押します。
テストを成功させるにはインターネットに接続する必要があります。
URLを保存する前に常にテストすることをお勧めします。
...
##### 音声の先頭の無音部分をトリミング {#TrimLeadingSilenceSpeech}

このオプションを有効にすると、NVDA は音声の冒頭にある無音部分を取り除き、一部の音声エンジンで応答性が向上する場合があります。
このオプションは既定で有効になっており、音声の冒頭の無音部分のみに影響します。
音声エンジンのアドオンを使用している際に、文と文の間のポーズなど、必要な無音部分まで省略されてしまう場合は、この機能を無効にすることで問題が解決することがあります。

##### SAPI4 オーディオ出力にWASAPIを使用 {#UseWASAPIForSAPI4}

このオプションを使用すると、Microsoft Speech API バージョン 4 (SAPI 4) の音声エンジンがWindows Audio Session API (WASAPI) 経由でオーディオを出力できるようになります。
これにより、SAPI 4 の音声でも、オーディオのダッキング、先頭の無音部分の除去、オーディオデバイスのスリープ防止など、より多くの機能を使用できるようになります。
ただし、現在の WASAPI 実装は一部の SAPI 4 音声では動作しない場合があります。
使用している SAPI 4 音声が機能しなくなった場合は、このオプションを無効にすることができます。
...
## リモートアクセス {#RemoteAccess}

NVDAに内蔵されているリモートアクセス機能を使用すると、NVDAを実行している別のPCを制御したり、誰かにあなたのPCを制御してもらったりできます。
これにより、サポートを提供したり受けたり、共同作業をしたり、離れた場所にある自分のPCにアクセスしたりすることが容易になります。

セキュリティのために、既定ではリモートアクセスは無効になっています。
[リモートアクセス設定](#RemoteSettings)で、リモートアクセスを有効にできます。

### リモートを使い始めるには {#RemoteAccessGettingStarted}

始める前に、両方のPCにNVDAがインストールされ、実行されていることを確認してください。
また、両方のPCで[リモートアクセス設定](#RemoteSettings)のリモートアクセス有効化のチェックボックスがチェックされていることを確認してください。
リモートアクセス機能はNVDAのツールメニューから利用できます。追加のダウンロードやインストールは必要ありません。

NVDAが[セキュアモード](#SecureMode)で実行されている場合、新しいリモートアクセスセッションを手動で開始することはできません。
ただし、NVDAの起動後に[自動接続](#RemoteAutoconnect)する設定が有効になっている場合、NVDAは引き続きリモートアクセスセッションを自動的に開始します。

セキュアモードでは手動で切断することができます。

### リモートアクセスセッションの設定 {#RemoteAccessSetup}

リモートアクセスセッションは、「リモートアクセス接続」ダイアログで設定します。
NVDAメニューを開き「ツール」「リモートアクセス」「接続」の順に選択します。
または `NVDA+alt+r` を押します。

#### モード選択 (制御する、または制御される) {#RemoteAccessMode}

どちらを制御されるPCにして、どちらを制御するPCにするかを決める必要があります。

モードのコンボボックスでは、このPCが制御される(別の人があなたのPCを操作する)側になるか、制御する(他の人のPCをあなたが操作する)側になるかを選びます。

* 別の人にあなたのPCの操作をサポートしてもらいたい場合は、「このPCが制御されることを許可」を選択します。
* あなたが誰かのPCの操作を支援したい場合は、「別のPCを制御」を選択します。

#### 既存のサーバーに接続する (ホストと鍵を与えられている場合) {#RemoteAccessJoin}

これは、他の人が中継サーバーを介して開始した接続がすでにあるか、ローカルに接続をホストする場合です。

1. サーバーは「既存のサーバーを使用」を選択します。
1. ホストのフィールドに、ホストのアドレスを入力します。指定されている場合はポートの情報も追加します。
  * 「ホスト」の後にコロン（「:」）とポート番号を追加することで、接続先のポートを指定できます。
例えば `example.com:1234` のように指定できます。
1. 鍵のフィールドには、受け取った鍵を入力します。
与えられた鍵を正確に入力するように注意してください。
1. OK を押します。

#### リレーサーバー経由で接続を開始する {#RemoteAccessConnectRelay}

他の人がインターネット上であなたに接続することを許可する場合、最も簡単なオプションは、リレーサーバーを使用することです。これを行うには:

1. サーバーは「既存のサーバーを使用」を選択します。
1. ホストのフィールドには `nvdaremote.com` のようなリレーサーバーのホスト名を指定します。
このアドレスを他の人に伝える必要があります。
  * 「ホスト」の後にコロン（「:」）とポート番号を追加することで、接続先のポートを指定できます。
例えば `example.com:1234` のように指定できます。
1. 鍵の入力: 文字数が多くて推測が難しい鍵を設定します。または「鍵を生成」ボタンを押して自動的に生成したものを使います。
このアドレスを他の人に伝える必要があります。
  * 鍵はセッションの識別子であり、同時にパスワードとしても機能します。そのため、推測が難しいことが非常に重要です。
1. OK を押します。
1. 他の人が接続できるように、ホストや鍵などの接続情報を提供してください。

#### NVDA をサーバーとして使用 {#RemoteAccessConnectSelfHosted}

リレーサーバーを使用したくない場合、またはローカルネットワーク経由で接続したい場合:

1. サーバーの場合は、「ローカルでホスト」を選択します。
1. インターネット経由で接続する場合は、「外部IPを取得」ボタンを押すと、インターネット上でアクセス可能な外部IPアドレスが表示されます。
この外部IPアドレスを他の人に伝える必要があります。
1. ローカルネットワークを介して接続する場合は、他の人にローカルIPアドレスを提供する必要があります。
ローカルネットワークのアドレスについてはこのドキュメントでは説明しません。
1. ポートには、リモートアクセスの接続を待ち受けるポート番号を入力します。
接続する側の人にもこの情報を伝える必要があります。
  * ポート番号は、他のサービスで使用されていないものである必要があります。
  * 既定ではリモートアクセスはポート番号 6837 を使用します。
  * インターネット経由で接続する場合は、ポート転送が必要になる場合があります。
  このドキュメントでは、ポート転送について説明しません。
  お使いの環境のネットワーク管理者に問い合わせてください。
1. 鍵の入力: 文字数が多くて推測が難しい鍵を設定します。または「鍵を生成」ボタンを押して自動的に生成したものを使います。
このアドレスを他の人に伝える必要があります。
  * 鍵はセッションの識別子であり、同時にパスワードとしても機能します。そのため、推測が難しいことが非常に重要です。
1. OK を押します。
...
### リモートアクセスを使用する {#RemoteAccessUsage}

セッションが開始されると、リモートPCと自分のPCの操作を切り替えたり、クリップボードを共有したり、リモートアクセスセッションの音声をミュートにしたりできます。

* `NVDA+Alt+Tab` を押すと、リモートPCの操作と自分のPCの操作の切り替えができます。
* クリップボードの内容を相手のPCに送信するには、NVDAメニューを開き、「ツール」、「リモートアクセス」、「クリップボードを送信」の順に選択します。
* 別のPCを制御する側として接続している場合に、制御する側のPCからリモートPCの音声出力をミュートするには、NVDAメニュー「ツール」「リモートアクセス」から「リモートのミュート」を選択します。

* 制御する側のPCとして接続している場合は、NVDA メニューを開き、「ツール」「リモートアクセス」「Control+Alt+Delete を送信」を選択することで、操作される側のPCに `Control+Alt+Delete` を送信できます。
  この機能を使うためには、制御される側のPCが NVDA のインストール版を実行している必要があります。
  また、制御される側のPCの設定によっては、この機能が動作しない場合があります。

### リモートアクセスのキーコマンドの概要 {#RemoteAccessGestures}

<!-- KC:beginInclude -->
| 名前 |キー |説明|
|---|---|---|
| リモート接続を切り替える | `NVDA+alt+r` | 新しいリモートアクセスセッションを開始するか、セッションが進行中の場合は、セッションから切断します。 |
| 制御対象の切り替え | `NVDA+alt+tab` | リモートとローカルのPCの制御を切り替えます。 |
| Connect | None | 新しいリモートアクセスセッションを開始します。[secure mode](#SecureMode) で利用できません。 |
| リンクのコピー | なし | リモートセッションへのリンクをクリップボードにコピーします。 |
| 切断 | なし | 既存のリモートアクセスセッションを終了します。 |
| リモートのミュート | なし | リモートPCからの音声をミュートまたはミュート解除します。 |
| クリップボードの送信 | なし | クリップボードの内容をリモートPCに送信します。 |
<!-- KC:endInclude -->
...
#### アドオン一覧の並べ替え基準 {#AddonStoreSortByColumn}

既定では、アドオンの一覧はアドオンの表示名の順に並びます。
「並び替え基準」コンボボックスを使用して、各タブで使用可能な列に基づいて一覧を並べ替えることができます。
例えば、アドオンを開発者や利用可能なバージョンなどで並べ替えることができます。
アドオンは昇順または降順で並べ替えることができます。
...
ダウンロードまたはインストールが失敗した場合は、インストールを再試行できます。
アドオンストアを終了する前にインストールをキャンセルすることもできます。
...
アドオンの状態が「有効」である場合はアドオンを無効にし、「無効」である場合は有効にすることができます。
...
#### 自動更新チャンネルの変更 {#AddonStoreUpdateChannel}

アドオンの自動更新チャンネルは、[インストール済みまたは更新可能](#AddonStoreFilterStatus) タブから管理できます。
[アドオンの自動更新](#AutomaticAddonUpdates)が有効になっている場合、アドオンは[既定](#DefaultAddonUpdateChannel)でインストール元と同じ[チャンネル](#AddonStoreFilterChannel)に更新されます。
アドオンのアクションメニューから、サブメニュー「更新チャンネル」を使用して、アドオンの自動更新に使うチャンネルを変更できます。

| 選択肢 | 動作 |
|---|---|
| 既定 | アドオンは[既定の更新チャンネル](#DefaultAddonUpdateChannel)に従います |
| 同じ | アドオンは同じチャンネルのままになります |
| すべて | アドオンは、チャンネルに関係なく自動的に最新バージョンに更新されます |
| 更新しない | アドオンは自動的に更新されません |
| 安定版 | アドオンは自動的に安定版に更新されます |
| ベータ版または開発版 | アドオンは自動的にベータ版または開発版に更新されます |
| ベータ版 | アドオンは自動的にベータ版に更新されます |
| 開発版 | アドオンは自動的に開発版に更新されます |
...
場合によっては、Windowsレジストリで問題が発生し、NVDAの動作がおかしくなることがあります。
これは、例えば Adobe Reader や Math Player などの特定のアプリをインストールまたはアンインストールした場合に発生することがあります。また、Windows のアップデートやその他のイベントが原因となることもあります。
COM 登録修正ツールは、レジストリのアクセシビリティエントリを修復することによって、これらの問題の解決を試みます。

このツールが修正できる問題の種類は次のとおりです。

* Firefox や Edge などのブラウザー、Thunderbird などのメールアプリ、エクスプローラー、タスクバー、その他のアプリ内を移動しているときに、NVDA が「不明」や「ペイン」などと読み上げることがあります。
* NVDAがフォーカスモードとブラウズモードを切り替えることに失敗します。
* 以前は名前を読み上げていたボタンを突然「ボタン」としか報告できなくなります。
* ブラウズモードを使用しているときにブラウザーで移動すると、NVDAが非常に遅くなります。

このツールは Windows レジストリの項目を修正するため、アプリをインストールする場合と同様に管理者権限が必要です。
通常のユーザー環境ではUAC（ユーザーアカウント制御）が有効です。この場合、このツールを正常に実行するために、UAC によって表示されるプロンプトに従う必要があります。
...
NVDAで使用できる、さらに多くの無料および商用の音声エンジンを購入・ダウンロードしたい場合は、[追加音声のページ](https://github.com/nvaccess/nvda/wiki/ExtraVoices)をご覧ください。
...
この規格に対応した多くの音声エンジンは、複数のメーカーやウェブサイトから購入もしくは無料で入手できる他、少なくともひとつ以上の音声がすでにシステムにプリインストールされています。
...
[HumanWare](https://www.humanware.com/) の Brailliant BI シリーズおよび B シリーズの点字ディスプレイ（BI 14、BI 32、BI 20X、BI 40、BI 40X、B 80 を含む）は、USB または Bluetooth 接続で使用できます。
...
BrailleNote Touch については、[Brailliant BI シリーズ / BrailleNote Touch](#HumanWareBrailliant) のセクションを参照してください。
...
|左矢印キー | スペース+3の点、ジョイスティック左、または方向パッド左|
...
Dot Inc の Dot Pad は、触知できるグラフィックと点字を表示できるデバイスです。
NVDA は、デバイスに搭載された点字専用の表示行に1行の点字を表示することも、触知グラフィックの領域に複数行の点字を表示することもできます。
A300モデルには、120×80ドットの触覚グラフィックの領域があり、20セル×8行の点字表示として使用できます。

NVDAのこのドライバー用の点字設定にある「点字の表示先」オプションで、点字を専用の点字表示行に表示するか、触知グラフィックの領域に表示するかを設定できます。

パンニングキーはサポートされていますが、デバイス上のボタンが制限されているため、他のコマンドやタッチカーソル機能は現在使用できません。

NVDAでDot Padドライバを選択する場合は、Dot Padが接続されているUSB/Bluetooth仮想シリアルポートを手動で選択する必要があります。
このドライバーは自動検出をサポートしていません。

ハードウェアの制限により、手がデバイスに触れている間は Dot Pad がすべてのドットを正しく更新できない場合があることにご注意ください。
NVDAで操作する際は、デバイスから手を完全に離し、表示が完全に更新されてから読み始めるようにしてください。

<!-- KC:beginInclude -->

| 名前 | キー|
|---|---|
|点字表示をスクロールして戻る | `pan_left` |
|点字表示をスクロールして進む | `pan_right` |
...
* NVDAメニューからユーザーガイドファイルなどの外部ドキュメントを開くこと。
...
|`-n LANGUAGE` | `--lang=LANGUAGE` | 設定されているNVDAの言語を上書きします。"Windows" を指定すると現在のユーザーの既定の言語に、"en" を指定すると英語になります。|
|`-m` |`--minimal` |サウンド、インターフェイス、開始メッセージなどを出しません|
|`-s` |`--secure` |[セキュアモード](#SecureMode) でNVDAを起動|
|`-d` | `--disable-addons` | アドオンを無効化|
...
|なし | `--create-portable-silent` | NVDAのポータブル版を作成します（コピーされたNVDAを起動しません）。`--portable-path` の指定が必要です。このオプションを使用すると、空でないディレクトリへの書き込み時の警告が表示されず、ファイルが警告なしに上書きされる場合があります。|
|なし |`--portable-path=PORTABLEPATH` |ポータブル版を作成する場所|

`--install-silent` コマンドラインオプションを指定してNVDAをサイレントインストールできるのと同様に、アンインストーラーに `/S` コマンドを渡すことでサイレントアンインストールも可能です。

NVDAのアンインストーラーは `uninstall.exe` という名前で、NVDAのインストールされたフォルダにあります。64ビット版Windowsでは `%ProgramFiles(x86)%\nvda`、32ビット版Windowsでは `%ProgramFiles%\nvda` に配置されています。

以下は NVDA アンインストーラーのコマンドラインオプションです:

| 短い書き方 | 長い書き方 | 説明|
|---|---|---|
|`/S` | なし | NVDAをサイレントアンインストールします。 |
