...
また、設定を変える、コミュニティに参加する、情報を得る、などについても説明します。
...
+ NVDA インストーラーの「使用許諾契約」などが書かれたウィンドウが表示されます。
必要な場合は ``下矢印`` を押して使用許諾契約をお読みください。
+ ``Tab`` を押して「同意する」チェックボックスに移動します。さらに ``スペース`` を押して「チェック」に切り替えます。
+ ``Tab`` を押して選択肢を確認します。必要に応じて項目を選んで ``Enter`` を押します。
+
...
+ インストーラーのダイアログで「同意する」チェックボックスをチェックします。
+ ``Tab`` を押して「このコンピューターにNVDAをインストール」を選び、Enter で実行します。
+ Windows サインイン画面でNVDAを使うこと、デスクトップにショートカットを作成することなどに関するオプションの画面になります。
これらは既定でチェックされています。
必要であれば ``Tab`` とスペースキーを使い、これらのオプションを変更します。必要がなければそのままにします。
+ ``Enter`` を押して次に進みます。
+ Windows の "ユーザーアカウント制御 (UAC)" ダイアログが表示されます。「このアプリがデバイスに変更を加えることを許可しますか？」と質問されます。
+ ``Alt+Y`` を押してユーザーアカウント制御を許可します。
+ プログレスバーが表示され NVDA のインストールが行われます。
その間、NVDA がプログレスバーの表示を、ビープ音がだんだん高くなることによって報告します。
インストールにかかる時間が短い場合は、この報告に気づかないかも知れません。
+ NVDA のインストールが終わると確認のダイアログボックスが表示されます。
「インストールされたNVDAを実行するにはOKを押してください」というメッセージがあります。
``Enter`` を押してインストールされたNVDAを実行します。
+ "ようこそNVDAへ" というダイアログが表示され、 NVDA がメッセージを表示します。
...
+ ``Tab`` を押すと「無変換をNVDA制御キーとして使用」「変換をNVDA制御キーとして使用」「EscapeをNVDA制御キーとして使用」に移動します。さらに``Tab`` を押すと "英語キーボードのCapsLockキーをNVDA制御キーとして使用" に移動します。
...
+ ``Tab`` とスペースキーでその他の NVDA の設定を変更できます。
例えば NVDA を自動的に開始するかどうかを選べます。
+ ``Enter`` を押してダイアログを閉じても、そのまま NVDA を使うことができます。
+
...
ある[オブジェクト #Object]に[フォーカス #SystemFocus]があり、その中で文字単位の移動や編集ができるときには、テキスト内の移動にテキストカーソルを使います。テキストカーソルは Windows の機能で、「文字入力カーソル」「挿入ポイント」「システムキャレット」と呼ばれることもあります。
...
++ レビューモード ++[ReviewModes]
NVDAのレビューコマンドは、レビューモードを切り替えることで、現在のナビゲーターオブジェクト、現在のドキュメント、画面のいずれかの内容をレビューできます。
...
+ ブラウズモードで入力ジェスチャーダイアログを開きます。
+ ブラウズモードのグループで「次のタブに移動」を探します。
+ 見つかったジェスチャーに ``y`` キーを割り当てます。
+ 「前のタブに移動」を探します。
+ 見つかったジェスチャーに ``shift+y`` キーを割り当てます。
+
...
この設定は、仮想マシン内でNVDAを実行する場合（例：Citrix Virtual Desktop）や、特定のラップトップで使用する場合にも役立つかもしれません。

値を小さく設定すると、デバイスが早くスタンバイモードに入るため、音声がより頻繁に切れる可能性があります。これにより、次の音声の開始部分が切れることがあります。
値を大きく設定しすぎると、音声を出力していない間もデバイスが長時間有効になるため、音声出力デバイスのバッテリーが早く消耗する可能性があります。
...
既定ではオーディオ分割は無効になっており、NVDAを含むすべてのアプリが左右両方のチャンネルで音を出力します。
...
- オーディオ分割が無効：NVDAと他のアプリの両方が左右両方のチャンネルで音を出力します。
- 左チャンネルでNVDA、右チャンネルでアプリ：NVDAは左チャンネルで音声を出力し、他のアプリは右チャンネルで音を出力します。
- 右チャンネルでNVDA、左チャンネルでアプリ：NVDAは右チャンネルで音声を出力し、他のアプリは左チャンネルで音を出力します。
-

NVDAの設定には、より多くのオーディオ分割モードを選択できるコンボボックスがあります。
NVDAを除くすべてのアプリの音量を調整したい場合は、[専用のコマンド #OtherAppVolume]の使用を検討してください。
...
- オーディオ分割が無効：NVDAとアプリの両方が左右両方のチャンネルで音を再生します。
...
==== 他のアプリの音量 ====[OtherAppVolume]

このスライダーを使用すると、NVDA以外の現在実行中のすべてのアプリの音量を調整できます。
この音量設定は、この設定が変更された後に起動した場合でも、他のすべてのアプリのオーディオ出力に適用されます。
この音量は、以下のキーボードコマンドを使用して、どこからでも制御することもできます。

%kc:beginInclude
|| 名前 | キー | 説明 |
| アプリの音量を上げる | ``NVDA+alt+pageUp`` | NVDA以外のすべてのアプリの音量を上げます。 |
| アプリの音量を下げる | ``NVDA+alt+pageDown`` | NVDA以外のすべてのアプリの音量を下げます。 |

%kc:endInclude

この機能は、高度な設定で[オーディオ出力にWASAPIを使用しない #WASAPI]状態でNVDAを起動した場合は利用できません。

==== 他のアプリのミュート ====[MuteApplications]

このチェックボックスを使用すると、NVDA以外のすべてのアプリをミュートできます。
このミュート設定は、この設定が変更された後に起動した場合でも、音を出力する他のすべてのアプリに適用されます。
以下のキーボードコマンドを使用して、どこからでもミュートの切り替えができます。

%kc:beginInclude
|| 名前 | キー | 説明 |
| 他のアプリのミュートの切り替え | ``NVDA+alt+delete`` | NVDA以外のすべてのアプリをミュートまたはミュート解除します。 |
...
%kc:setting
==== NVDA設定ダイアログ「書式とドキュメント情報」カテゴリを開く ====
Key: ``NVDA+Ctrl+D``
...
|| . {.hideHeaderRow} | . |
| 選択肢 | 既定 (アプリによる処理), アプリによる処理, 改行, 連続改行 |
| 既定の値 | アプリによる処理 |
...
|| . {.hideHeaderRow} | . |
| 選択肢 | 既定(差分アルゴリズム), 差分アルゴリズム, UIオートメーションの通知 |
| 既定の値 | 差分アルゴリズム |
...
注意:
 ...
 例えば、エミュレートされたキー入力 'a' に 'Ctrl+M' の入力ジェスチャーを設定すると、
 アプリが受け取れる入力が 'Ctrl+A' となる場合があります。
 -
...
サインインまたはUAC画面でのNVDAの設定を変更したい場合は、いったん現在起動中のNVDAを使ってサインイン画面のNVDA用の設定を作ってから、NVDA設定ダイアログの一般カテゴリ「現在保存されている情報をサインインおよびセキュアデスクトップで使用する」ボタンを使用して設定をコピーしてください。
...
キーワードがアドオンID、名前、配布者、開発者、または説明に含まれていれば、アドオンを検索結果に表示します。
...
次の手順を試してください：
+ [最新のドライバー https://www.papenmeier-rehatechnik.de/en/service/downloadcenter/software/articles/software-braille-devices.html] をインストールしていることを確認してください。
+ Windowsデバイスマネージャーを開きます。
+ リストを下にスクロールして、「USBコントローラー」または「USBデバイス」を探します。
+ 「Papenmeier Braillex USB Device」を選択します。
+ プロパティを開き、「詳細設定」タブに切り替えます。
