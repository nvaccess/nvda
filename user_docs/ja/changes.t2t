...
- ユニットテストのフレームワークが追加されました。(#7026)
 - ユニットテストとそのインフラストラクチャは tests/unit ディレクトリにあります。詳細は tests\unit\__init__.py の docstring を参照してください。
...
%- gui.execute() has been removed. wx.CallAfter() should be used を使います。
%- gui.scriptUI has been removed.
% - For message dialogs, use wx.CallAfter(gui.messageBox, ...).
% - For all other dialogs, real wx dialogs should be used を使います。
