...
بهبودهای زیادی در گفتار وجود دارد؛ به ویژه در زمینه‌ی پاسخگویی با صداهای SAPI 4، SAPI 5 و OneCore.
تقویت سرعت و تغییر خودکار زبان اکنون در SAPI 5 پشتیبانی می‌شود.
صداهای SAPI 4 اکنون از کم کردن خودکار صدا، حذف سکوت‌های ابتدایی و بیدار نگه داشتن دستگاه صوتی پشتیبانی می‌کنند.
...
جدول‌های بریل ورودی و خروجی اکنون می‌توانند بر اساس زبان NVDA تعیین شوند.
پشتیبانی بومی برای دستگاه گرافیکی لمسی Dot Pad از شرکت Dot به عنوان یک نمایشگر بریل چند‌خطی اضافه شده است.
اشکالات جزئی دیگری برای نمایشگرهای بریل، به ویژه در زمینه اتصال رفع شده است.

مترجم بریل LibLouis، eSpeak-NG و Unicode CLDR بروزرسانی شده‌اند.
جدول‌های بریل رایانه‌ای جدید برای قبطی، خط میخی مختصر و پرتغالی 6 نقطه در دسترس هستند.
...
لطفاً مشکلات امنیتی را با رعایت [سیاست امنیتی](https://github.com/nvaccess/nvda/blob/master/security.md) NVDA به‌طور مسئولانه افشا کنید.

* از نمایش اطلاعات حساس احتمالی بر روی نمایشگرهای بریل هنگامی که دستگاه قفل است جلوگیری می‌کند.
([GHSA-8f8q-2jc3-6rf4](https://github.com/nvaccess/nvda/security/advisories/GHSA-8f8q-2jc3-6rf4))
* از بارگذاری DLL‌های ناخواسته توسط نصب‌کننده از پوشه‌ی خود جلوگیری می‌کند.
([GHSA-qf5h-qw92-rx2f](https://github.com/nvaccess/nvda/security/advisories/GHSA-qf5h-qw92-rx2f))

### امکانات جدید

* فروشگاه افزونه‌ها:
  * بروزرسانی‌های خودکار (#3208):
  ...
  * عملی برای لغو نصب افزونه‌ها اضافه شد. (#15578, @hwf1324)
  * عملی برای تلاش دوباره برای نصب افزونه در صورت ناموفق بودن دانلود یا نصب افزونه افزوده شد. (#17090, @hwf1324)
  * فهرست افزونه‌ها را می‌توانید بر اساس ستون، از جمله تاریخ انتشار، به ترتیب صعودی و نزولی مرتب کنید. (#15277, #16681, @nvdaes)
* گفتار:
  * تغییر خودکار زبان حالا هنگام استفاده از نسخه‌ی 5 رابط برنامه‌نویسی گفتار مایکروسافت (SAPI5) و صداهای پلتفرم گفتار مایکروسافت پشتیبانی می‌شود. (#17146, @gexgd0419)
  * تقویت سرعت حالا هنگام استفاده از نسخه‌ی 5 رابط برنامه‌نویسی گفتار مایکروسافت (SAPI5) و صداهای پلتفرم گفتار مایکروسافت پشتیبانی می‌شود که تا سرعت ۶ برابر را پشتیبانی می‌کند. (#17606, @gexgd0419)
* مرورگرهای وب:
  * حالت انتخاب بومی در حالت مرور NVDA (`NVDA+shift+f10`) حالا در گوگل کروم، اِج و سایر برنامه‌های مبتنی بر کرومیوم نسخه 134 یا جدیدتر پشتیبانی می‌شود. (#17838)
  * در موزیلا فایرفاکس، NVDA متن برجسته‌شده را زمانی که یک URL حاوی یک قطعه‌ی متن بازدید می‌شود، اعلام خواهد کرد. (#16910, @jcsteh)
  * NVDA حالا می‌تواند اعلام کند وقتی که مقصد یک لینک به صفحه فعلی اشاره دارد. (#141, @LeonarddeR, @nvdaes)
* مایکروسافت آفیس:
  * هنگام ویرایش در جعبه‌های متن پاورپوینت، حالا می‌توانید با استفاده از `alt+جهت‌نمای بالا` و `alt+جهت‌نمای پایین` جمله به جمله حرکت کنید. (#17015, @LeonarddeR)
  * در Word، بروزرسانی انتخاب، زمانی که از فرمان‌های Word برای افزایش یا کاهش انتخاب (`f8` یا `shift+f8`) استفاده می‌کنید، حالا اعلام می‌شود. (#3293, @CyrilleB79)
  * در Word نسخه‌ی ۱۶.۰.۱۸۲۲۶ و بالاتر یا هنگام استفاده از مدل شیء Word، NVDA حالا  اگر یک سرنوشتار بسته شده باشد در گفتار و بریل اعلام می‌کند. (#17499)
  * در Word با UIA، وقتی که از فرمان‌های توسعه‌یافته‌ی بومی پیمایش جدول (`alt+home`، `alt+end`، `alt+pageUp` و `alt+pageDown`) استفاده می‌کنید، حرکت مکان‌نما حالا اعلام خواهد شد. (#17867, @CyrilleB79)
  * در Word، هنگامی که از فرمان «اعلام فکوس» استفاده می‌کنید، چیدمان سند اعلام خواهد شد، اگر اطلاعاتش موجود و اعلام توضیحات اشیاء فعال باشد. (#15088, @nvdaes)
  * نتیجه‌ی میانبرهای بیشتری حالا در Word و Outlook اعلام می‌شود:
    * میانبرهای قالب‌بندی قلم‌ها (#10271, @CyrilleB79)
    * بستن یا باز کردن سرنوشتارها (#17545, @CyrilleB79)
* LibreOffice:
  * هنگام افزایش یا کاهش اندازه قلم در Writer با استفاده از میانبرهای صفحه‌کلید مربوط، NVDA اندازه‌ی جدید قلم را اعلام می‌کند. (#6915, @michaelweghorn)
  * هنگام اِعمال سبْک پاراگراف «متن اصلی» یا یک سرنوشتار با استفاده از میانبر صفحه‌کلید مربوط در Writer نسخه‌ی ۲۵.۲ یا جدیدتر، NVDA سبْک جدید پاراگراف را اعلام می‌کند. (#6915, @michaelweghorn)
  * هنگام تغییر وضعیت زیرخط دوتایی در Writer با استفاده از میانبر صفحه‌کلید مربوط، NVDA وضعیت جدید را اعلام می‌کند («زیرخط دوتایی روشن»/«زیرخط دوتایی خاموش»). (#6915, @michaelweghorn)
  * NVDA حالا می‌تواند تورفتگی خط اول را در LibreOffice نسخه‌ی ۲۵.۸ و بالاتر اعلام کند. (#13052, @michaelweghorn)
* نویسه‌خوان نوری (OCR):
  * در یک نتیجه‌ی بازشناسی، `NVDA+f5` بطور  دستی محتوای شناسایی‌شده را نو‌سازی می‌کند. (#17715, @CyrilleB79)
  * یک فرمان اختصاص‌نیافته به صفحه‌کلید افزوده شد تا بطور متناوب نتیجه‌ی OCR ویندوز را نو‌سازی کند. (#16897)
* قابلیت دسترسی از راه دور، بر اساس افزونه‌ی NVDA Remote، در NVDA یکپارچه شده است. (#4390, @ctoth, @tspivey, @daiverd، مشارکت‌کنندگان و حامیان مالی NVDA Remote)
* اکنون امکان مشخص کردن نشانی‌های اینترنتی جایگزین برای استفاده در بروزرسانی‌های NVDA و فروشگاه افزونه‌ها وجود دارد. (#14974, #17151, #17310, @christopherpross)
* پشتیبانی از ریاضی در PDF‌ها افزوده شد. (#9288, @NSoiffer)
...
* اکنون می‌توانید NVDA را طوری تنظیم کنید که هنگام پیمایش با کلیدهای پیمایشی بریل، خط یا پاراگراف فعلی را بخواند. (#17053, @nvdaes)
* حالا NVDA می‌تواند تغییرات مکان‌نما را هنگام فشردن کلیدهای ترکیبی `alt+جهت‌نمای بالا` یا `alt+جهت‌نمای پایین`، مثلا در Visual Studio Code اعلام کند. (#17652, @LeonarddeR)
* فرمان‌های جدیدی برای بردن مکان‌نمای بازبینی به اولین و آخرین نویسه‌ی متن انتخاب‌شده اضافه شده است که به ترتیب به `NVDA+alt+home` و `NVDA+alt+end` اختصاص یافته‌اند. (#17299, @nvdaes)
* یک تنظیم عمومی اضافه شده است تا از خاموش شدن نمایشگر در هنگام استفاده از قابلیت «خواندن همه» یا خواندن با بریل جلوگیری کند.
این گزینه به صورت پیش‌فرض فعال است، ولی ممکن است منجر به افزایش مصرف باتری شود. (#17649, @LeonarddeR)
* اکنون NVDA به زبان بوسنیایی ترجمه شده است. (#17953)
* در Adobe Acrobat، NVDA حالا می‌تواند معادلات ریاضی در اسناد PDF که توسط نسخه‌های اخیر Microsoft Word تولید شده‌اند را بخواند و با آن‌ها تعامل داشته باشد. (#18056)

### تغییرات

* بروزرسانی اجزای برنامه:
  * برگرداننده‌ی بریل LibLouis به نسخه‌ی [۳.۳۳.۰](https://github.com/liblouis/liblouis/releases/tag/v3.33.0) بروز شد. (#17469, #17768, @LeonarddeR, @codeofdusk)
    * جدول‌های جدید قبطی، خط میخی فشرده و پرتغالی بریل رایانه‌ای ۶ نقطه اضافه شد.
  * CLDR به نسخه‌ی ۴۶.۰ بروز شد. (#17484, @OzancanKaratas)
  * ESpeak NG به نسخه‌ی ۱.۵۲.۰ بروز شد. (#17056)
* تغییرات رابط کاربری NVDA:
  * نصب‌کننده‌ی NVDA دیگر هنگام اجرا صدا پخش نمی‌کند. (#14068)
  * پنجره‌های «اعلام مقصد لینک»، «اطلاعات قالب‌بندی نویسه» و «انتخاب گفتار» اکنون برای راحتی کاربر، دارای دکمه‌های «بستن» و «کپی» هستند. (#17018، @XLTechie)
  * پنجره‌ی خروج اکنون به شما اجازه می‌دهد NVDA را به طور همزمان با افزونه‌های غیرفعال و فعال بودن ثبت اشکال‌زدایی راه‌اندازی مجدد کنید. (#11538، @CyrilleB79)
  * امکان انصراف از استفاده از WASAPI برای خروجی صدا حذف شده است. (#16080)
  * گزینه‌ی «تنظیم خودکار فکوس سیستم روی گزینه‌های فکوس‌پذیر» در حالت مرور حذف و این رفتار حالا غیرفعال شده است. (#17598)
  * NVDA اکنون فقط زمانی درباره‌ی ناسازگاری افزونه‌ها هشدار می‌دهد که به نسخه جدیدی بروز شود که رابط برنامه‌نویسی افزونه‌های آن با نسخه‌ی فعلی ناسازگار است. (#17071, #17506)
  * فایل «مشارکت‌کنندگان» از منوی NVDA حذف شده است. (#16922)
  * مجوز NVDA اکنون برای خوانایی بهتر به قالب HTML درآمده است. (#17600)
  * نسخه‌های کوتاه‌تر از رایج‌ترین گزینه‌های خط فرمان اضافه شده‌اند: `-d` for `--disable-addons` and `-n` for `--lang`.
  تطبیق پیشوندی روی فلگ‌های خط فرمان، مثلاً استفاده از `--di` به جای `--disable-addons` دیگر پشتیبانی نمی‌شود. (#11644, @CyrilleB79)
  * تغییرات در ابزار تعمیر رجیستری: (#12355, @XLTechie)
    * اکنون به جای یک هشدار، با توضیحی کاربرپسندتر درباره هدف خود شروع می‌شود. (#12351)
    * می‌توانید حالا پنجره‌ی اولیه را با کلیدهای `escape`» یا `Alt+F4` ببندید. (#10799)
...
* گفتار:
  * اکنون صداهای Microsoft Speech API نسخه ۵ و Microsoft Speech Platform برای خروجی صدا از WASAPI استفاده می‌کنند که ممکن است باعث بهبود پاسخ‌دهی این صداها شود. (#13284, @gexgd0419)
  * حالا سکوت ابتدای گفتار هنگام استفاده از صداهای OneCore، صداهای SAPI5 و برخی افزونه‌های صدای شخص ثالث حذف می‌شود تا پاسخ‌دهی آن‌ها بهبود یابد. (#17614, @gexgd0419)
  * اکنون صداهای Microsoft Speech API نسخه ۴ برای خروجی صدا از WASAPI استفاده می‌کنند تا بتوانند با قابلیت‌هایی مانند کم کردن صدای سایر برنامه‌ها، حذف سکوت ابتدایی و بیدار نگه داشتن دستگاه صوتی کار کنند.
  اگر این قابلیت با صدای SAPI 4 شما کار نمی‌کند، می‌توانید WASAPI را برای SAPI 4 در تنظیمات پیشرفته غیرفعال کنید. (#17718, #17801, @gexgd0419)
  * عادی‌سازی یونی‌کد حالا به طور پیش‌فرض برای خروجی گفتار فعال است. (#17017, @LeonarddeR).
    * هنوز هم می‌توانید این قابلیت را در دسته‌ی گفتار تنظیمات NVDA غیرفعال کنید.
* تنظیمات صفحه‌کلید برای «خواندن نویسه‌های تایپ‌شده» و «خواندن واژه‌های تایپ‌شده» اکنون سه گزینه دارند: خاموش، فقط در کنترل‌های ویرایشی، و همیشه. (#17505, @Cary-rowen)
  * به طور پیش‌فرض، گزینه‌ی «خواندن نویسه‌های تایپ‌شده» اکنون روی «فقط در کنترل‌های ویرایشی» تنظیم شده است.
* جدول‌های بریل ورودی و خروجی پیش‌فرض اکنون می‌توانند بر اساس زبان NVDA تعیین شوند. (#17306, #16390, #290, @nvdaes)

### رفع اشکال

* گفتار:
  * مشکلی که در آن، خواندن پیوسته (خواندن همه) هنگام استفاده از برخی موتورهای سخنگوی SAPI5 در پایان جمله اول متوقف می‌شد، برطرف شد. (#16691, @gexgd0419)
  * پارامترهای صدا، مانند سرعت و حجم، دیگر هنگام تغییر بین صداها با استفاده از حلقه‌ی تنظیمات موتور سخنگو در سخنگوهای SAPI5 و SAPI4 به حالت پیش‌فرض بازنشانی نمی‌شوند. (#17693, #2320, @gexgd0419)
  * یک مشکل برطرف شد که در آن برخی از صداهای SAPI4 (مثلاً IBM TTS چینی) قابل بارگذاری نبودند. (#17726, @gexgd0419)
* بریل:
  * پشتیبانی بومی از دستگاه گرافیک لمسی Dot Pad شرکت Dot به عنوان یک نمایشگر بریل چندخطّی. (#17007)
  * هنگام استفاده از یادداشت‌بردار Seika، ژست‌های فاصله و فاصله با نقطه‌ها اکنون به‌درستی در پنجره‌ی فرمان‌های ورودی نمایش داده می‌شوند. (#17047, @school510587)
  * NVDA دیگر نباید هنگام استفاده از حالت بریل خروجی گفتار نمایشگر، زمانی که هیچ نمایشگر بریلی متصل نیست، صدای خطا پخش کند یا فایل گزارش را پر از پیام کند. (#17092, @Emil-18)
  * زمانی که درایور استاندارد نمایشگر بریل HID صراحتا به‌عنوان درایور نمایشگر بریل انتخاب شده باشد و فهرست نمایشگرهای بریل باز شود، NVDA به‌درستی درایور HID را به‌عنوان درایور انتخاب‌شده شناسایی می‌کند و دیگر گزینه «هیچ درایوری انتخاب نشده» را نمایش نمی‌دهد. (#17537, @LeonarddeR)
  * درایور Humanware Brailliant اکنون در انتخاب نقطه اتصال صحیح قابل‌اعتمادتر شده است، که این موضوع باعث پایداری بهتر اتصال و خطاهای کمتر می‌شود. (#17537, @LeonarddeR)
  * جدول‌های بریل سفارشی در چرک‌نویس برنامه‌نویس اکنون زمانی که افزونه‌ها غیرفعال هستند، به‌درستی نادیده گرفته می‌شوند. (#17565, @LeonarddeR)
  * مشکلی که در آن برخی نمایشگرهای بریل USB به درستی توسط NVDA شناسایی نمی‌شدند رفع شد. (#18114, @christiancomaschi)
* مایکروسافت آفیس:
  * فرمان «اعلام مقصد URL یک لینک» اکنون هنگام استفاده از مدل شیء قدیمی در برنامه‌های Word، Outlook، Excel و PowerPoint همان‌طور که انتظار می‌رود کار می‌کند. (#17292, #17362, #17435, @CyrilleB79)
  * در اکسل، پنجره‌ی فهرست سازه‌ها (NVDA+f7) دیگر در برخی سیستم‌های غیر‌انگلیسی‌زبان، نمایش توضیحات یا فرمول‌ها را با مشکل مواجه نمی‌کند. (#11366, @CyrilleB79)
  * بهبودی‌هایی در پاورپوینت:
    * اعلام مکان‌نما دیگر زمانی که متن شامل نویسه‌های عریض مانند ایموجی باشد، دچار مشکل نمی‌شود. (#17006 , @LeonarddeR)
    * اعلام موقعیت نویسه حالا (مثلاً هنگام فشردن `NVDA+delete`) دقیق است. (#9941, @LeonarddeR)
    * NVDA دیگر هنگام شروع نمایش اسلاید، زمانی که تنظیم «خواندن خودکار همه هنگام بارگذاری صفحه» در حالت مرور غیرفعال است، خواندن همه را به‌طور خودکار آغاز نمی‌کند. (#17488, @LeonarddeR)
* LibreOffice:
  * هنگامی که از روش ورودی پین‌یین مایکروسافت برای زبان چینی استفاده می‌کنید و گزینه‌ی سازگاری پین‌یین را برای استفاده از نسخه‌ی قبلی فعال می‌کنید، تایپ کردن در Writer (و احتمالاً سایر برنامه‌ها) در حالی که پنجره پاپ‌آپ IME نمایش داده می‌شود، دیگر باعث ایجاد خطا نمی‌شود. (#17198, @michaelweghorn)
  * وضعیت فعلی کادرهای علامت‌زدنی (علامت‌دار یا بدون علامت) در پنجره‌های محاوره‌ای اکنون علاوه بر گفتار، در بریل نیز گزارش می‌شود. (#17218, @michaelweghorn)
* ریاضی:
  * خواندن ریاضی برای برخی از سازه‌های وب اصلاح شده است.
  به طور خاص، MathML که داخل سازه‌ی span و دیگر سازه‌هایی قرار دارد که دارای ویژگی `role="math"` هستند. (#15058)
  * معادلات ریاضی که فقط به صورت یک تصویر و متن جایگزین بدون MathML برای ناوبری پیشرفته نمایش داده می‌شوند، اکنون مانند تصاویر معمولی در نظر گرفته می‌شوند، نه به عنوان ریاضی بدون محتوا. این کار به کاربر اجازه می‌دهد با کلید `g` به آن‌ها پرش کند و بتواند با کلیدهای جهت‌نما، متن جایگزین را کاراکتر به کاراکتر مرور کند. (#16007)
* محیط‌های توسعه‌ی یکپارچه (IDEها):
  * NVDA دیگر هنگام انتخاب تمام متن در برخی فایل‌های منبع در IDEهای Android Studio یا IntelliJ متوقف نمی‌شود. (#17418, @thgcode)
  * در Visual Studio Code، NVDA دیگر کلیدهای میانبر `alt+جهت‌نمای بالا` و `alt+جهت‌نمای پایین` را برای پیمایش جمله‌ها به صورت اختصاصی در اختیار نمی‌گیرد. (#17082, @LeonarddeR)
  * برخی از سازه‌های section اکنون به‌درستی به عنوان کنترل‌های قابل ویرایش در Visual Studio Code شناسایی می‌شوند. (#17573, @Cary-rowen)
  * در Notepad و سایر اسناد UIA، و همچنین اسناد Notepad++ در ویندوز ۱۱، اگر خط آخر خالی باشد، فرمان «خط بعدی در بریل» مکان‌نما را به خط آخر منتقل می‌کند.
  در هر سندی، اگر مکان‌نما روی خط آخر باشد، با استفاده از این فرمان، به انتهای خط منتقل خواهد شد. (#17251, #17430, @nvdaes)
* پروفایل‌های پیکربندیConfiguration profiles:
  * بریل دیگر هنگام فعال کردن «خواندن همه» با یک پروفایل پیکربندی مرتبط، دچار اختلال نمی‌شود. (#17163, @LeonarddeR)
  * مشکلی رفع شد که در آن برخی تنظیمات حتی زمانی که مقدار آن‌ها با مقدار موجود در پیکربندی پایه برابر بود، صراحتا در پروفایل پیکربندی فعال ذخیره می‌شدند. (#17157, @leonarddeR)
* هنگام هجی کردن، عادی‌سازی یونی‌کد اکنون به شکل مناسب‌تری عمل می‌کند:
  * پس از اعلام یک نویسه عادی‌سازی‌شده، NVDA دیگر به اشتباه نویسه‌های بعدی را به عنوان عادی‌سازی‌شده اعلام نمی‌کند. (#17286, @LeonarddeR)
  * نویسه‌های ترکیبی (مانند é) اکنون درست اعلام می‌شوند. (#17295, @LeonarddeR)
* NVDA قادر است آیتم‌های زیرمنوی پاپ‌آپ را در صفحه نتایج جستجوی Thunderbird بخواند. (#4708, @thgcode)
* ابزار تعمیر رجیستری دیگر در صورت عدم موفقیت، به اشتباه پیام موفقیت نمایش نمی‌دهد. (#12355, @XLTechie)
* NVDA دیگر هنگام بستن پنجره، زمانی که مورد‌هایی وجود دارند، ورودی‌های تاریخچه کلیپ‌بورد ویندوز ۱۱ را اعلام نخواهد کرد. (#17308, @josephsl)
* اگر افزونه‌ها زمانی که یک پیام قابل مرور باز است، دوباره بارگذاری شوند، NVDA دیگر در اعلام جابجایی‌های بعدی فکوس دچار مشکل نخواهد شد. (#17323, @CyrilleB79)
* هنگام استفاده از برنامه‌هایی مانند اسکایپ، دیسکورد، سیگنال و Phone Link برای ارتباط صوتی، صدای گفتار و صداهای NVDA دیگر کاهش نمی‌یابد. (#17349, @jcsteh)
* باز کردن میز فرمان پایتون NVDA دیگر در صورتی که هنگام دریافت متغیرهای snapshot خطایی رخ دهد، با مشکل مواجه نخواهد شد. (#17391, @CyrilleB79)
* در مرورگرهای وب، تغییرات انتخاب متن دیگر گاهی اوقات در کنترل‌های متنی قابل ویرایش اعلام نمی‌شوند. (#17501, @jcsteh)
* وقتی لینک‌های لنگر به همان شیئی اشاره می‌کنند که مکان‌نمای مجازی روی آن قرار دارد، NVDA دیگر در اسکرول کردن به مقصد پیوند دچار مشکل نمی‌شود. (#17669, @nvdaes)
* آیکون پنجره برجسته‌ساز NVDA دیگر پس از راه‌اندازی مجدد Explorer در نوار وظیفه ثابت نمی‌مانَد. (#17696, @hwf1324)
...
* گزینه‌ی `config.conf["documentFormatting"]["reportFontAttributes"]` حذف شده است، به جای آن از `config.conf["documentFormatting"]["fontAttributeReporting"]` استفاده کنید. (#18066)
* گزینه‌ی `config.conf["speech"]["includeCLDR"]` حذف شده است، به جای آن بررسی یا ویرایش کنید که آیا `config.conf["speech"]["symbolDictionaries"]` شامل `"cldr"` است یا نه. (#18066)
