...
- Ein defektes ScriptableObject (z. B. ein GlobalPlugin, bei dem ein Aufruf der "__init__"-Methode dessen Basisklasse fehlt) unterbricht nicht mehr die Skriptbehandlung von NVDA. (#5446)
...
- Es gibt jetzt ein Unit Test Framework für NVDA. (#7026)
 - Unit-Tests und Infrastruktur befinden sich im Verzeichnis tests/unit. Sehen Sie sich den Docstring in der Datei tests\unit\__init__.py für weitere Informationen an.
...
- In Windows 8 und neuer startet NVDA nun wesentlich früher, wenn es so konfiguriert wird, dass es nach der Anmeldung ausgeführt wird. (#308)
 - Wenn die Option zum Start nach der Anmeldung mit einer vorherigen Version aktiviert wurde, müssen Sie sie aus- und wieder einschalten, damit die Änderungen wirksam werden. Folgen Sie dieser Anleitung:
  ...
  + Deaktivieren Sie die Option "NVDA automatisch nach der Anmeldung starten".
  + Betätigen Sie den Schalter "OK".
  + Öffnen Sie die Allgemeinen Einstellungen erneut.
  + Aktivieren Sie die Option "NVDA automatisch nach der Anmeldung starten".
  + Betätigen Sie den Schalter "OK".
...
- Unterklassen des SAPI5-Treibers können "_getVoiceTokens" überschreiben und "__init__" erweitern, um benutzerdefinierte "voice tokens" zu unterstützen. So kann mit "sapi.spObjectTokenCategory" eine Stimme von einem benutzerdefinierten Standort in der Registrierungsdatenbank abgerufen werden.
