...
Mit dieser Version wird der Fernzugriff via NVDA eingeführt, mit dem Sie einen entfernten Computer, auf dem NVDA läuft, von einem anderen Gerät mit NVDA aus steuern können.

Es gibt viele Verbesserungen bei der Sprachausgabe, insbesondere bei der Reaktionsfähigkeit mit SAPI 4, SAPI 5 und OneCore-Stimmen.
Erhöhung der Geschwindigkeit und automatische Sprachumschaltung werden jetzt in SAPI 5 unterstützt.
SAPI 4-Stimmen unterstützen jetzt die Reduzierung der Audio-Quellen, Ausblendung der führenden Stille und halten das Audiogerät aktiv.

Das System für die automatischen Updates im Store für die NVDA-Erweiterungen wurde verbessert. Sie können nun Kanäle auswählen und automatische Updates im Hintergrund ausführen lassen.

New commands have been added to manually refresh an OCR result, and to toggle periodically refreshing OCR results.

Die native Auswahl ist jetzt in Google Chrome und Microsoft Edge verfügbar.

Die Unterstützung für Microsoft Office und LibreOffice wurde verbessert, insbesondere durch die Mitteilung weiterer Tastenkombinationen.

Standardisierte Eingabe- und Ausgabe-Braille-Tabellen können nun basierend auf die Sprache in NVDA festgelegt werden.
Native support has been added for the Dot Pad tactile graphics device from Dot Inc as a multiline braille display.
Es wurden weitere kleinere Korrekturen für Braillezeilen vorgenommen, insbesondere im Hinblick auf die Konnektivität.

LibLouis Braille-Übersetzer, eSpeak-NG und Unicode CLDR wurden aktualisiert.
Neue koptische, kompakte Keilschrift- und portugiesische 6-Punkt-Computer-Braille-Schrift sind verfügbar.

### Wichtige Hinweise

* Diese Version beeinträchtigt die Kompatibilität bestehender NVDA-Erweiterungen.

### Sicherheitskorrekturen

Bitte melden Sie Sicherheitsprobleme in NVDA umgehend gemäß der [Sicherheitsrichtlinien](https://github.com/nvaccess/nvda/blob/master/security.md).
...
* Store für NVDA-Erweiterungen:
  * Automatische Updates (#3208):
    * Automatische Update-Kanäle für NVDA-Erweiterungen können nun geändert werden.
      * Automatische Update-Kanäle können für installierte NVDA-Erweiterungen über das Untermenü "Update-Kanal" ausgewählt werden.
...
* Support for math in PDFs has been added. (#9288, @NSoiffer)
  ...
  * Derzeit wird dies nur in Foxit Reader und Foxit Editor unterstützt.
...
* Komponenten-Updates:
  * LibLouis Braille-Übersetzer auf [3.33.0](https://github.com/liblouis/liblouis/releases/tag/v3.33.0) aktualisiert. (#17469, #17768, @LeonarddeR, @codeofdusk)
    * Neue koptische, kompakte Keilschrift- und portugiesische 6-Punkt-Computer-Braille-Tabellen hinzugefügt.
  * CLDR auf Version 46.0 aktualisiert. (#17484, @OzancanKaratas)
  * eSpeak NG auf 1.52.0 aktualisiert. (#17056)
* Änderungen an der NVDA-Oberfläche:
  ...
  * Die Datei zu den Mitwirkenden wurde aus dem NVDA-Menü entfernt. (#16922)
  * Die Lizenz für NVDA ist jetzt in HTML formatiert, um die Lesbarkeit zu verbessern. (#17600)
...
* Sprachausgaben:
  ...
  * Die Unicode-Normalisierung ist nun standardmäßig für die Sprachausgabe aktiviert. (#17017, @LeonarddeR).
...
* Microsoft Office:
  ...
  * Verbesserungen in Microsoft PowerPoint:
...
* Hinweis: Dies ist eine Version, die die Kompatibilität der API der NVDA-Erweiterungen aufhebt.
...
* Due to the retirement of NVDA's winmm support (#17496, #17532, #17678):
  ...
  * `nvwave.WasapiWavePlayer` wurde umbenannt in `WavePlayer`.
  <target/>
  ...
  * `gui.settingsDialogs.AdvancedPanelControls.wasapiComboBox` wurde entfernt.
...
* Because SAPI5 voices now use `nvwave.WavePlayer` to output audio: (#17592, @gexgd0419)
  * `synthDrivers.sapi5.SPAudioState` wurde entfernt.
  * `synthDrivers.sapi5.SynthDriver.ttsAudioStream` wurde entfernt.
...
* Changed keyboard typing echo configuration from boolean to integer values. (#17505, @Cary-rowen)
  * Für `config.conf["keyboard"]["speakTypedCharacters"]` und `config.conf["keyboard"]["speakTypedWords"]` werden jetzt ganzzahlige Werte verwendet.
  * Added `TypingEcho` enum in `config.configFlags` to represent these modes, 0=Off, 1=Only in edit controls, 2=Always.
  * `gui.settingsDialogs.KeyboardSettingsPanel.wordsCheckBox` und `gui.settingsDialogs.KeyboardSettingsPanel.charsCheckBox` wurden entfernt.
...
* Der Erweiterungspunkt `braille.filter_displaySize` ist veraltet.
Please use `braille.filter_displayDimensions` instead. (#17011)
* Die Funktionen `gui.message.messageBox` und `gui.runScriptModalDialog` sowie die Klasse `gui.nvdaControls.MessageDialog` sind veraltet.
Use `gui.message.MessageDialog` instead. (#17582)
* Die folgenden Symbole sind veraltet (#17486, @CyrilleB79):
  * `NoConsoleOptionParser`, `stringToBool`, `stringToLang` in `__main__`; benutzen Sie stattdessen die gleichen Symbole in `argsParsing`.
  * `__main__.parser`; verwenden Sie stattdessen `argsParsing.getParser()`.
...
* Die Kommandozeilenoptionen `-c`/`--config-path` und `--disable-addons` werden nun beachtet, wenn ein Update aus NVDA heraus gestartet wird. (#16937)
* Komponenten-Updates:
...
* NVDA schlägt nicht mehr gelegentlich fehl, um ein Fenster zum Nachlesen zu öffnen (z. B. durch zweimaliges Drücken von `NVDA+F`). (#16806, @LeonarddeR)
...
* Eine `.editorconfig`-Datei wurde zum NVDA-Repository hinzugefügt, damit verschiedene IDEs standardmäßig die grundlegenden NVDA-Code-Stilregeln übernehmen können. (#16795, @LeonarddeR)
* Unterstützung für benutzerdefinierte Symbol-Wörterbücher für Sprachausgaben wurde hinzugefügt. (#16739, #16823, @LeonarddeR)
  * Wörterbücher können in landesspezifischen Ordnern in einer NVDA-Erweiterung bereitgestellt werden, z. B. `locale\en`.
  * Wörterbuch-Metadaten können zu einem optionalen Abschnitt `symbolDictionaries` in der Manifest-Datei der NVDA-Erweiterung hinzugefügt werden.
  * Bitte lesen Sie den Abschnitt [Benutzerdefinierte Symbol-Wörterbücher für Sprachausgabe im Entwicklerhandbuch](https://www.nvaccess.org/files/nvda/documentation/developerGuide.html#AddonSymbolDictionaries) für weitere Details.
* Es ist nun möglich, Objekte umzuleiten, die von Bildschirmkoordinaten abgerufen werden, indem die Methode `NVDAObject.objectFromPointRedirect` verwendet wird. (#16788, @Emil-18)
* Wenn Sie SCons mit dem Parameter `--all-cores` ausführen, wird automatisch die maximale Anzahl verfügbarer CPU-Kerne ausgewählt. (#16943, #16868, @LeonarddeR)
...
* Der Konfigurationsschlüssel `bool` `[documentFormatting][reportFontAttributes]` ist veraltet und wird in 2025.1 entfernt, verwenden Sie stattdessen `[fontAttributeReporting]`. (#16748)
  * Die neue Taste hat einen `int`-Wert, der mit einem `OutputMode`-`Enum` übereinstimmt, mit Optionen für Sprachaugabe, Braille, Sprachausgabe und Braille und Ausgeschaltet.
  * Über die API können Sie wie bisher den Wert `bool` verwenden oder den `OutputMode` überprüfen, wenn sie speziell mit Sprachausgabe oder Braille umgehen.
...
* LibreOffice:
  ...
  * Das Anzeigen der Statuszeile mit `NVDA+Ende` funktioniert jetzt auch in Dialogfeldern in LibreOffice 2024.2 und neuer. (#15591, @michaelweghorn)
...
* Microsoft Office:
  * In Excel mit deaktivierter UIA wird die Brailleschrift aktualisiert und der Inhalt der aktiven Zelle gesprochen, wenn `Strg+Y`, `Strg+Z` oder `Alt+RÜcktaste` gedrückt wird. (#15547)
  * In Word mit deaktiviertem UIA wird die Brailleschrift aktualisiert, wenn `Strg+V`, `Strg+X`, `Strg+Y`, `Strg+Z`, `Alt+Rücktaste`, `RÜcktaste` oder `Strg+Rücktaste` gedrückt wird.
  ...
  * In Word wird die Zielzelle korrekt angezeigt, wenn Sie die Word-Eigenen Befehle zur Tabellennavigation verwenden. Hierzu gehören `Alt+Pos1`, `Alt+Ende`, `Alt+Bild nach oben` und `Alt+Bild nach unten`. (#15805, @CyrilleB79)
...
* Wenn ein Skript mit dem Skript-Dekorator definiert wird, kann das boolesche Argument `speakOnDemand` angegeben werden, um zu kontrollieren, ob ein Skript im Sprachmodus "Bei Bedarf" sprechen soll. (#481, @CyrilleB79)
...
* Wenn NVDA mit dem Parameter `--lang=Windows` gestartet wird, ist es wieder möglich, das Dialogfeld für die Allgemeinen Einstellungen in NVDA zu öffnen. (#14407)
...
* Das Build-System von NVDA holt jetzt alle Python-Abhängigkeiten mit pip und speichert sie in einer virtuellen Python-Umgebung. Dies geschieht alles transparent.
  ...
  * So führen Sie Systemtests durch: Führen Sie `runsystemtests.bat [<extra robot options>]` aus
  * Um Linting durchzuführen, führen Sie `runlint.bat <base branch>` aus
...
* Veraltete Funktionen wurden entfernt. (#9548)
  ...
  * Wrapper configobj.validate entfernt.
  ...
  * Eigenschaft inputCore.InputGesture.logIdentifier wurde entfernt.
    * Verwenden Sie stattdessen _get_identifiers in inputCore.InputGesture.
  * synthDriverHandler.SynthDriver.speakText/speakCharacter wurden entfernt.
  * Mehrere Klassen in synthDriverHandler.SynthSetting entfernt.
  ...
  * Alter Code wurde entfernt, insbesondere:
...
* Wenn im Benutzerhandbuch der Befehl <!-- KC:setting für eine Einstellung verwendet wird, die eine gemeinsame Tastenkombination für alle Tastatur-Layouts besitzt, wird nun eine Spalte in voller Breite verwendet. (#5739) -->
...
* NVDA bringt jetzt den integrierten Synthesizer eSpeak mit, der von Jonathan Duddington entwickelt wurde. Espeak ist Schnell, klein und unterstützt viele unterschiedliche Sprachen. SAPI-Sprachen werden zwar immer noch unterstützt, eSpeak wird jedoch standardmäßig verwendet.
  * eSpeak benötigt keine spezielle Software, die installiert werden muss, und kann daher mit NVDA auf jedem Computer, auf einem USB-Stick oder überall verwendet werden.
  * Weitere Informationen zu eSpeak oder zu anderen Versionen finden Sie unter http://espeak.sourceforge.net/.
...
* Kommandozeilen-Parameter zu NVDA hinzugefügt:
  * -m, --minimal: Keine NVDA-Sounds beim Starten und Beenden abspielen und die Benutzeroberfläche beim Starten nicht anzeigen, wenn dies so eingestellt ist.
  * -q, --quit: Sämtliche bereits laufende Instanzen von NVDA beenden
  * -s, --stderr-file fileName: Ablageort für nicht abgefangene Fehler von NVDA und Ausnahmen
  * -d, --debug-file fileName: Ablageort der Debug-Meldungen von NVDA
  * -c, --config-file: Eine alternative Konfigurationsdatei angeben
  * -h, --help: Eine Hilfemeldung mit einer Auflistung der Kommandozeilenargumente anzeigen
