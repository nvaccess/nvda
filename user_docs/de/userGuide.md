...
| Name |Desktop-Tastenkombination |Laptop-Tastenkombination |Beschreibung|
|---|---|---|---|
|Alles vorlesen |`NVDA+Pfeiltaste nach unten` |`NVDA+A` |Beginnt an der aktuellen Position mit dem Lesen und zieht  den Fokus dabei mit|
...
|Tastenzuordnungen ansagen |`NVDA+4` |`NVDA+4` |Wenn diese Funktion aktiviert ist, nennt NVDA alle Tasten, die keine Buchstaben sind und die Sie auf der Tastatur eingeben. Dazu gehören auch Tastenkombinationen wie Steuerung plus ein weiterer Buchstabe.|
...
Es ist auch möglich, den ersten oder letzten Wert der aktuellen Sprachausgaben-Einstellung zu setzen, indem Sie benutzerdefinierte Tastenzuordnungen im [Dialogfeld für die Tastenzuordnungen](#InputGestures), dort die Kategorie "Sprachausgabe" auswählen und diese dann zuweisen.
...
Das Desktop-Layout nutzt den Nummernblock (wenn dieser ausgeschaltet ist).
...
* Alternativ können Sie auch die Taskleiste aufrufen, indem Sie die Tastenkombination `Windows+B` drücken, mit den `Pfeiltasten` zum NVDA-Symbol navigieren und das Kontextmenü öffnen, indem Sie die `Kontextmenü`-Taste drücken, die sich auf den meisten Tastaturen neben der rechten Strg-Taste befindet.
...
|---|---|---|---|---|
|NVDA starten oder neustarten |Strg+Alt+N |Strg+Alt+N |Keine |Startet NVDA vom Desktop (neu), sofern diese Tastenkombination während der NVDA-Installation aktiviert wurde. Dies ist eine Windows-spezifische Verknüpfung und kann daher im Dialogfeld für die Tastenzuordnungen nicht neu zugewiesen werden.|
...
|---|---|---|
|Datum und Uhrzeit mitteilen |NVDA+F12 |Bei einmal Drücken wird das Datum und bei zweimal Drücken wird die Uhrzeit ausgegeben|
...
Der Standardmodus ist "Sprechen", der in Situationen vorliest, die bei der Verwendung eines Screenreaders zu erwarten sind.
...
| Name |Desktop-Tastenkombination |Laptop-Tastenkombination |Beschreibung|
|---|---|---|---|
|Aktuellen Fokus mitteilen |NVDA+Tab-Taste |NVDA+Tab-Taste |Teilt das aktuell hervorgehobene Objekt mit. Bei zweimal Drücken werden die Informationen buchstabiert|
...
|Statuszeile mitteilen |NVDA+Ende |NVDA+Umschalt+Ende |Liest die Statusleiste vor, falls vorhanden. Durch zweimaliges Drücken werden die Informationen buchstabiert. Durch dreimaliges Drücken werden die Informationen in die Zwischenablage kopiert|
|Kurztaste mitteilen |`Umschalt+Nummernblock 2` |`NVDA+Strg+Umschalt+Punkt` |Gibt das Tastenkürzel (Taste) des aktuell fokussierten Objekts aus|
...
| Name |Desktop-Tastenkombination |Laptop-Tastenkombination |Beschreibung|
|---|---|---|---|
|Alles vorlesen |NVDA+Pfeiltaste nach unten |NVDA+A |Liest von der aktuellen Position des System-Cursors bis zum Textende und zieht dabei den System-Cursor mit|
|Aktuelle Zeile vorlesen |NVDA+Pfeiltaste nach oben |NVDA+L |Liest die Zeile, auf der sich der System-Cursor befindet. Wird diese Tastenkombination zweimal gedrückt, wird die Zeile buchstabiert, wird die Tastenkombination 3-mal gedrückt, wird die Zeile phonetisch buchstabiert.|
|Markierten Text vorlesen |NVDA+Umschalt+Pfeiltaste nach oben |NVDA+Umschalt+S |Liest den markierten Text vor, sofern vorhanden|
|Textformatierungen mitteilen |NVDA+F |NVDA+F |Gibt die Textformatierungen unter dem System-Cursor aus. Bei zweimal Drücken werden diese Informationen im Lesemodus angezeigt|
|Linkziel mitteilen |`NVDA+K` |`NVDA+K` |Durch einmaliges Drücken wird die Ziel-Adresse des Links an der aktuellen Cursor- oder Fokusposition ausgegeben. Durch zweimaliges Drücken wird ein Fenster zur genaueren Überprüfung angezeigt|
...
|Nächsten Satz vorlesen |Alt+Pfeiltaste nach unten |Alt+Pfeiltaste nach unten |Zieht die Schreibmarke zum nächsten Satz und gibt ihn aus (nur in Microsoft Word und Microsoft Outlook unterstützt).|
|Vorherigen Satz vorlesen |Alt+Pfeiltaste nach oben |Alt+Pfeiltaste nach oben |Zieht die Schreibmarke zum vorherigen Satz und gibt ihn aus (nur in Microsoft Word und Microsoft Outlook unterstützt).|
...
|Aktuelles Wort unter dem NVDA-Cursor mitteilen |Nummernblock 5 |NVDA+Strg+Punkt |Keine |Gibt das aktuelle Wort im Text, welches sich beim NVDA-Cursor befindet, aus. Bei zweimaliges Drücken wird es buchstabiert, bei dreimaliges Drücken wird das Wort phonetisch buchstabiert.|
...
Zum Navigieren mit der Maus sollte eine richtige Maus oder ein Track-Pad benutzt werden. Folgende Tastenzuordnungen stehen in NVDA zur Verfügung:
...
|An der Maus-Position nach oben scrollen |Keine |Keine |Keine |Scrollt das Mausrad an der aktuellen Maus-Position nach oben|
|An der Maus-Position nach unten scrollen |Keine |Keine |Keine |Scrollt das Mausrad an der aktuellen Maus-Position nach unten|
|An der Maus-Position nach links scrollen |Keine |Keine |Keine |Scrollt das Mausrad an der aktuellen Maus-Position nach links|
|An der Maus-Position nach rechts scrollen |Keine |Keine |Keine |Scrollt das Mausrad an der aktuellen Maus-Position nach rechts|
|Mauszeiger zum aktuellen Navigator-Objekt ziehen |NVDA+Nummernblock-Schrägstrich |NVDA+Umschalt+M |Keine |Zieht die Maus zum aktuellen Standort des Navigator-Objektes oder des NVDA-Cursors|
|Zum Objekt unter dem Mauszeiger navigieren |NVDA+Nummernblock-Stern |NVDA+Umschalt+N |Keine |Zieht das Navigator-Objekt zur Objektposition an der Mausposition|
...
NVDA verfügt jedoch über einen nativen Auswahlmodus, der in bestimmten Dokumenten im LeseModus aktiviert werden kann, die ihn unterstützen können (Mozilla Firefox und jeder Browser, der auf Chromium 134 oder höher basiert), wodurch die native Auswahl des Dokuments der NVDA-Auswahl im Durchsuchen-Modus folgt.
...
Sie können diese Zuweisungen ändern oder neue emulierte Tasten über den [Dialogfeld für die Tastenzuordnungen](#InputGestures) hinzufügen.
...
Um diese Umschalt-Tasten zu verwenden, drücken Sie zunächst den Befehl (oder die Befehlsfolge) für die Modifikatortasten, die Sie drücken möchten.
...
Wenn Sie in Braille-Schrift tippen, werden Ihre Eingaben mit den Umschalt-Tasten so übersetzt, als ob Sie die Punkte 7+8 gedrückt hätten.
...
Durch Drücken von 'NVDA+f5' wird das Erkennungsergebnis aktualisiert.
...
NVDA bietet für einige Anwendungen eigene zusätzliche Funktionen, um bestimmte Aufgaben zu erleichtern oder den Zugriff auf Funktionen zu ermöglichen, die Benutzern von Screenreadern sonst nicht zugänglich sind.
...
| Name |Tastenkombination |Beschreibung|
|---|---|---|
|Spaltenüberschriften setzen |'NVDA+Umschalt+c' |Wenn Sie diese Taste einmal drücken, wird NVDA mitgeteilt, dass dies die erste Kopfzelle in der Zeile ist, die Spaltenüberschriften enthält, die automatisch angesagt werden sollten, wenn zwischen den Spalten unter dieser Zeile gewechselt wird. Durch zweimaliges Drücken wird die Einstellung gelöscht.|
|Zeilenüberschriften setzen |'NVDA+Umschalt+R' |Wenn Sie diese Taste einmal drücken, wird NVDA mitgeteilt, dass dies die erste Kopfzelle in der Spalte ist, die Zeilenüberschriften enthält, die automatisch angesagt werden sollten, wenn nach dieser Spalte zwischen den Zeilen gewechselt wird. Durch zweimaliges Drücken wird die Einstellung gelöscht.|

<!-- KC:endInclude -->
Diese Zuweisungen werden im Dokument als Lesezeichen gesetzt. Dies ist kompatibel zu anderen Bildschirmlesern wie JAWS.
Das bedeutet, dass Benutzer anderer Screenreader, die dieses Dokument zu einem späteren Zeitpunkt öffnen, automatisch die Zeilen- und Spaltenüberschriften bereits festgelegt haben.
...
<!-- KC:beginInclude -->
Um Kommentare an der aktuellen Cursor-Position mitzuteilen, drücken Sie `NVDA+Alt+C`.
Zweimaliges Drücken zeigt die Informationen in einem Fenster zum Nachlesen an.
<!-- KC:endInclude -->
Alle Kommentare für das Dokument können zusammen mit anderen nachverfolgten Änderungen auch in der NVDA-Elementliste aufgelistet werden, wenn Anmerkungen als Typ ausgewählt werden.
...
Das bedeutet, dass Benutzer anderer Screenreader, die diese Arbeitsmappe zu einem späteren Zeitpunkt öffnen, die Zeilen- und Spaltenüberschriften automatisch bereits festgelegt haben.
...
<!-- KC:beginInclude -->
Um alle Notizen für die aktuell fokussierte Zelle mitzuteilen, drücken Sie `NVDA+Alt+C`.
Zweimaliges Drücken zeigt die Informationen in einem Fenster zum Nachlesen an.
...
Diese Tastenkombination kann nicht im Dialogfeld für die Tastenzuordnungen geändert werden.
...
Anschließend können Sie mit der Leertaste oder mit der Eingabetaste den Interaktionsmodus aktivieren um mit dem Formularfeld umzugehen.
...
| Name |Tastenkombination |Beschreibung|
|---|---|---|
|Verbleibende Zeit mitteilen |Strg+Umschalt+R |Gibt die verbleibende Zeit aus, sofern ein Titel abgespielt wird.|
|Verstrichene Zeit mitteilen |Strg+Umschalt+E |Gibt die verstrichene Zeit des aktuellen Titels aus, sofern ein Titel abgespielt wird.|
|Titellänge mitteilen |Strg+Umschalt+T |Gibt die Länge des aktuellen Titels aus, sofern ein Titel abgespielt wird.|
...
| Name |Tastenkombination |Beschreibung|
|---|---|---|
|Letzte Nachricht mitteilen |NVDA+Strg+1 bis 4 |Gibt die zur Nummer passende Nachricht aus; NVDA+Strg+2 gibt beispielsweise die vorletzte Nachricht aus.|

<!-- KC:endInclude -->

### Poedit {#Poedit}

NVDA bietet erweiterte Unterstützung für Poedit 3.5 oder neuer.

<!-- KC:beginInclude -->

| Name |Tastenkombination |Beschreibung|
|---|---|---|
|Anmerkungen für Übersetzer mitteilen |`Strg+Umschalt+A` |Gibt alle Anmerkungen für Übersetzer aus. Bei zweimaligem Drücken werden die Anmerkungen im Lesemodus angezeigt|
|Kommentare mitteilen |`Strg+Umschalt+C` |Gibt alle Kommentare im Kommmentarbereich aus. Zweimaliges Drücken zeigt die Kommentare im Lesemodus|
|Alten Quelltext mitteilen |`Strg+Umschalt+O` |Gibt den alten Quelltext aus, falls vorhanden. Bei zweimaligem Drücken wird der Text im Lesemodus angezeigt|
|Übersetzungswarnung mitteilen |`Strg+Umschalt+W` |Gibt evtl. vorhandene Übersetzungswarnung aus. Bei zweimaligem Drücken wird die Warnung im Lesemodus angezeigt|
...
Markieren Sie dazu zuerst den Text mit dem im Lesemodus üblichen Tasten oder Tastenkombinationen (z. B. Umschalt+Pfeiltasten).
<!-- KC:beginInclude -->
...
Die meisten Dialogfelder in NVDA unterstützen die Kontexthilfe.
<!-- KC:beginInclude -->
...
Beachten Sie, dass standardmäßig nicht alle Einstellungskategorien mittels Tastenkombination, Tastenzuordnungen, etc. aufgerufen werden können.
Wenn Sie häufig auf Kategorien zugreifen, für die es keine eigenen Tastenkombinationen gibt, können Sie im Dialogfeld für die [Tastenzuordnungen](#InputGestures) einen benutzerdefinierten Tastenbefehl für diese Kategorie hinzufügen.
...
Wenn diese Option aktiviert ist, wird beim Versuch, NVDA zu beenden, ein Dialogfeld angezeigt, das die folgenden Möglichkeiten bietet:

* exit
* restart
* restart with add-ons disabled and debug logging enabled
* restart with debug logging enabled
* ausstehende Updates installieren (falls vorhanden).
...
Sofern diese Option aktiviert ist, verwendet NV Access die Informationen aus den Daten zur Update-Abfrage, um die Anzahl der NVDA-Benutzer einschließlich bestimmter demografischer Daten wie Betriebssystem und Herkunftsland zu ermitteln.
...
* Eine eindeutige ID für den aktuellen NVDA-Benutzer, diese ändert sich einmal pro Monat.
* Eingestellte Landessprache in NVDA
* Art der Instanz von NVDA (portabel oder installiert)
...
Sie können das ausstehende Update auch manuell über das Dialogfeld "NVDA beenden" (falls aktiviert), über das NVDA-Menü oder durch Ausführen einer neuen Überprüfung über das Menü "Hilfe" installieren.

##### Update Mirror {#UpdateMirror}

Mit diesen Steuerelementen können Sie eine alternative URL angeben, die für die Suche nach Updates für NVDA verwendet werden soll.
Dies kann an Speicherorten nützlich sein, an denen der Zugriff auf den NV Access NVDA-Updateserver langsam oder nicht verfügbar ist.

Das schreibgeschützte Textfeld zeigt die aktuelle Mirror-URL an.
Wenn kein Spiegel verwendet wird (d. h. der NV Access-Updateserver verwendet wird), wird "Kein Mirror" angezeigt.

Wenn Sie den Update-Spiegel ändern möchten, drücken Sie die Taste "Ändern..." , um das Dialogfeld [Update-Mirror einstellen](#SetURLDialog) zu öffnen.

Bitte beachten Sie, dass bei der Verwendung eines Update-Mirrors der Betreiber des Mirrors Zugriff auf alle [Informationen hat, die mit Update-Prüfungen gesendet werden](#GeneralSettingsCheckForUpdates).
Wenden Sie sich an den Betreiber des Update-Spiegels, um Einzelheiten zu dessen Datenverarbeitungsrichtlinien zu erhalten, um sicherzustellen, dass Sie mit der Art und Weise, wie Ihre Informationen behandelt werden, vertraut sind, bevor Sie einen Update-Spiegel einrichten.

##### Prevent display from turning off during say all or reading with braille {#PreventDisplayTurningOff}

Wenn dieses Kontrollkästchen aktiviert ist, wird sichergestellt, dass die Anzeige eingeschaltet bleibt, wenn es mit "Alle" oder mit Brailleschrift gelesen wird (z. B. beim Drücken von Scroll-Tasten).
Dadurch wird vermieden, dass der Bildschirm während einer Say-All-Sitzung unerwartet gesperrt wird.
Diese Option ist standardmäßig aktiviert.
Erwägen Sie, diese Option zu deaktivieren, wenn ihr Akku nur noch eine Kurze Lebensdauer hat.
...
Die erste Option in der Kategorie Spracheinstellungen ist der Schalter Ändern....
Diese Schaltfläche aktiviert den Dialog [Synthesizer auswählen](#SelectSynthesizer), in dem Sie den aktiven Sprachsynthesizer auswählen können.
...
##### Unicode-Normalisierung {#SpeechUnicodeNormalization}

Wenn diese Option aktiviert ist, wird der von NVDA vorgelesene Text einer Unicode-Normalisierung unterzogen.
...
1. Aufteilung einiger Ligaturen, einschließlich "ĳ" (Ligatur ij) in ihre Zweibuchstabenform ("ij").

1. Feste Anordnung von Modifikatoren in zusammengesetzten Zeichen, z. B. im Althebräischen.

Um die Unicode-Normalisierung von überall aus umzuschalten, weisen Sie bitte einen benutzerdefinierten Tastenbefehl über das Dialogfeld für die [Tastenzuordnungen](#InputGestures) zu.
...
Beachten Sie, dass diese Einstellung nur verfügbar ist, wenn "[Unicode-Normalisierung](#SpeechUnicodeNormalization)" aktiviert ist.
...
Um die Einbindung von Unicode-Konsortiumsdaten von überall her zu aktivieren, weisen Sie bitte eine Tastenkombination über das Dialogfeld für die [Tastenzuordnungen](#InputGestures) zu.
...
|In größeren Schritten die aktuellen Sprachausgaben-Einstellung verringern |`NVDA+Strg+Seite nach unten` |`NVDA+Umschalt+Strg+Seite nach unten` |Verringert den Wert der aktuellen Sprachausgaben-Einstellung, auf der Sie sich befinden, in größeren Schritten. Wenn Sie sich z. B. auf einer Stimmen-Einstellung befinden, springt der Wert alle 20 Stimmen rückwärts; wenn Sie sich auf einer Schieberegler-Einstellung befinden, springt der Wert um bis zu 20 % rückwärts|
...
Wenn Sie "Automatisch" auswählen, wird die Tabelle entsprechend der aktuellen Sprache von NVDA ausgewählt.
...
Um die Anzeige der Meldungen von überall aus einzuschalten, weisen Sie bitte einen benutzerdefinierten Tastenbefehl zu, indem Sie das Dialogfeld für die [Tastenzuordnungen](#InputGestures) verwenden.
...
In diesem Fall folgt die Brailleschrift nicht dem Systemfokus und dem Systemcursor.
...
Diese Option ist standardmäßig auf "Niemals" eingestellt, das bedeutet, der Cursor wird bei der Weiterleitung des NVDA-Cursors nicht mitgezogen.
...
Um den System-Cursor zu ziehen, wenn Sie den NVDA-Cursor von einer beliebigen Stelle aus weiterleiten, weisen Sie bitte einen benutzerdefinierten Tastenbefehl zu, indem Sie das Dialogfeld für die [Tastenzuordnungen](#InputGestures) verwenden.
...
##### Absatzanfangsmarkierung {#BrailleParagraphStartMarkers}

Wenn "Absatzweises vorlesen" aktiviert ist, wird die ausgewählte Startmarkierung angezeigt, um den Beginn eines Absatzes zu kennzeichnen.
Dies kann vor allem bei Anwendungen hilfreich sein, mit denen große Textmengen gelesen werden, wie strukturierte Dokumente oder Bücher.
In solchen Dokumenten kann es nützlich sein zu wissen, wo Absätze beginnen, um die Struktur des Inhalts zu verstehen oder um Lesezeichen oder Anmerkungen auf der Grundlage der Absatzposition zu setzen.

Zu den Optionen gehören die Verwendung von zwei Leerzeichen als subtiler Absatzumbruch und das Absatzsymbol (¶) als offensichtlicheres Zeichen.

| . {.hideHeaderRow} |.|
|---|---|
|Optionen |Keine Absatzanfangsmarkierung, Doppeltes Leerzeichen (  ), Absatzmarke (¶)|
|Standard |Keine Absatzanfangsmarkierung|

##### Kontextbezogene Informationen anzeigen {#BrailleSettingsFocusContextPresentation}

Mit dieser Option können Sie festlegen wann kontextbezogene Informationen auf der Braillezeile angezeigt werden sollen, sobald ein Objekt fokussiert wird.
Kontextbezogene Informationen sind z. B. Informationen über übergeordnete Objekte.
...
Um die Einstellung von überall aus zu ändern, weisen Sie eine Tastenkombination mit Hilfe des Dialogs [Tastenzuordnungen](#InputGestures) zu.
...
Um die Auswahl von einer beliebigen Stelle aus umzuschalten, weisen Sie bitte einen benutzerdefinierten Tastenbefehl zu, indem Sie das dialogfeld für die [Tastenzuordnungen](#InputGestures) verwenden.
...
##### Formattierung auf der Braillezeile ausgeben {#BrailleFormattingDisplay}

Diese Einstellung legt fest, wie NVDA die Textformatierung in Braille anzeigt.
Diese Option wirkt sich nur aus, wenn NVDA auf [Schrift-Attribute in Braille anzeigen](#DocumentFormattingFontAttributes) eingestellt ist.

| . {.hideHeaderRow} |.|
|---|---|
| Optionen | Standard (Liblouis), Liblouis, Tags |
| Standard | Liblouis |

| Option | Verhalten |
|---|---|
| Liblouis | Native Braille-Formatierung verwenden. Beachten Sie, dass diese Option nur fetten, kursiven und unterstrichenen Text anzeigt, und auch nur, wenn die ausgewählte Braille-Tabelle die Anzeige dieser Attribute unterstützt. |
| [Tags](#BrailleFormattingDisplayTags) | Verwenden Sie Tags, die beschreiben, wie und wo sich die Textformatierung ändert. |

###### Tags {#BrailleFormattingDisplayTags}

Wenn "Formatierungsanzeige" auf "Tags" eingestellt ist, wird ein Formatierungs-Tag in Braille angezeigt, wenn eine Änderung der Formatierung erkannt wird.
Diese Tags beginnen mit ⣋ und enden mit ⣙.
Ein Tag zur Formatierung enthält ein oder mehrere Symbole, die die Textformatierung beschreiben.
Die folgenden Symbole sind definiert:

| Symbol | Bedeutung |
|---|---|
| ⠃ ("b") | Fett (am Anfang) |
| ⡃ ("b" mit Punkt 7) | Fett (am Ende) |
| ⠊ ("i") | Kursiv (am Anfang) |
| ⡊ ("i" mit Punkt 7) | Kursiv (am Ende) |
| ⠥ ("u") | Unterstrichen (am Anfang) |
| ⡥ ("u" mit Punkt 7) | Unterstrichen (am Ende) |
| ⠎ ("s")| Durchgestrichen (am Anfang) |
| ⡎ ("s" mit Punkt 7) | Durchgestrichen (am Ende) |

##### Zeichen vorlesen beim Navigieren im Text {#BrailleSpeakOnRouting}

Wenn diese Option aktiviert ist, teilt NVDA automatisch das Zeichen an der Cursor-Position mit, wenn es mit den Cursor-Routing-Tasten auf der Braillezeile angesteuert wird.

Um diese Option von überall aus zu aktivieren, weisen Sie bitte einen benutzerdefinierten Tastenbefehl über das [Dialogfeld für die Tastenzuordnungen](#InputGestures) zu.

##### Speak when navigating by line or paragraph {#BrailleSpeakOnNavigating}

Wenn Sie diese Option aktivieren, spricht NVDA Zeilen oder Absätze, die mit den Navigationssteuerelementen der Braillezeile erreicht wurden.

Um diese Option von überall aus umzuschalten, weisen Sie bitte "speakOnNavigatingByUnit" im Abschnitt "Braille" des Dialogfelds [Eingabegesten](#InputGestures) eine benutzerdefinierte Geste zu.

##### Wortumbrüche vermeiden, falls möglich {#BrailleSettingsWordWrap}

Wenn diese Option aktiviert ist, wird ein Wort, das zu lang ist und auf der Braillezeile keinen Platz am Ende mehr findet, nicht mehr angezeigt.
Stattdessen wird am Ende auf der Braillezeile nichts angezeigt.
Wenn Sie auf der Braillezeile navigieren, können Sie das ganze Wort lesen.
Dies wird manchmal als "Wortumbruch" bezeichnet.
Wenn das Wort zu lang ist, um allein auf die Braillezeile zu passen, muss es trotzdem angezeigt werden.

Wenn diese Funktion deaktiviert ist, wird ein möglichst großer Teil des Wortes angezeigt, der Rest wird jedoch abgeschnitten.
Wenn Sie auf der Braillezeile navigieren, können Sie den Rest des Wortes lesen.

Die Aktivierung dieser Funktion kann ein flüssigeres Lesen ermöglichen, erfordert aber in der Regel ein häufigeres Navigieren auf der Braillezeile.
...
Dies ist von Vorteil, wenn man auf Zeichen in Braille stößt, die in einer bestimmten Braille-Tabelle nicht vorkommen und für die es eine kompatible Alternative gibt, wie z. B. die fett und kursiv gedruckten Zeichen, die häufig in Social Media verwendet werden.
...
##### Unterbrechung der Sprachausgabe beim Navigieren {#BrailleSettingsInterruptSpeech}

Diese Einstellung legt fest, ob die Sprachausgabe unterbrochen werden soll, wenn auf der Braillezeile vorwärts oder rückwärts navigiert wird.
Befehle für die vorherige bzw. nächste Zeile unterbrechen immer die Sprachausgabe.

Das ständige Vorlesen von der Sprachausgabe kann beim Lesen auf der Braillezeile ablenken.
...
Wenn Sie diese Option deaktivieren, können Sie der Sprachaugabe zuhören und gleichzeitig auf der Braillezeile lesen.
...
* Keine Reduzierung der Lautstärke anderer Audio-Quellen: NVDA reduziert die Lautstärke anderer Audio-Quellen nicht.
* Nur beim Vorlesen und bei Sounds die Lautstärke anderer Audio-Quellen reduzieren: NVDA reduziert die Lautstärke anderer Audio-Quellen nur bei Verwendung der Sprachausgabe oder sofern NVDA-Sounds wiedergegeben werden. Dies funktioniert möglicherweise nicht mit allen Sprachausgaben.
* Immer die Lautstärke anderer Audio-Quellen reduzieren: NVDA hält die Lautstärke anderer Audio-Quellen während der gesamten Laufzeit von NVDA relativ niedrig.

Diese Option ist nur verfügbar, sofern NVDA installiert wurde.
Die Unterstützung zur Reduzierung der Lautstärke anderer Audio-Quellen für portable und temporäre NVDA-Versionen ist nicht möglich.
...
* NVDA auf der linken Seite und Anwendungen auf beiden Kanälen: NVDA spricht im linken Kanal, während andere Anwendungen Töne sowohl im linken als auch im rechten Kanal abspielen.
...
Diese Option steuert, wann NVDA Zeichen ankündigt, die Sie auf der Tastatur eingeben.
Folgende Optionen stehen zur Verfügung:

* Aus: NVDA kündigt eingegebene Zeichen nicht an.
* Nur in Bearbeitungssteuerelementen: NVDA sagt nur Zeichen an, die in Bearbeitungssteuerelementen und anderen Bereichen eingegeben wurden, in denen Text eingegeben werden kann.
* Immer: NVDA sagt alle eingegebenen Zeichen an.
...
Diese Option steuert, wann NVDA Wörter ansagt, die Sie auf der Tastatur eingeben.
Folgende Optionen stehen zur Verfügung:

* Aus: NVDA sagt eingegebene Wörter nicht an.
* Nur in Bearbeitungssteuerelementen: NVDA sagt nur Wörter an, die in Bearbeitungssteuerelementen und anderen Bereichen eingegeben wurden, in denen Text eingegeben werden kann.
* Immer: NVDA sagt alle eingegebenen Wörter an.
...
##### Navigation während "Alles vorlesen" erlauben {#KeyboardSettingsSkimReading}
...
Wenn aktiviert, hören Sie einen kurzen Warnton, wenn Sie einen Buchstaben mit der Umschalt-Taste drücken, während die Dauergroßschreibung eingeschaltet ist.
Es ist generell nicht gebräuchlich, Buchstaben mittels der Umschalt-Taste groß zu schreiben, während die Dauergroßschreibung aktiviert ist und geschieht meistens nur deshalb, weil man nicht erkennt, dass die Dauergroßschreibung aktiv ist.
...
Diese Option ist standardmäßig aktiviert, obwohl bestimmte Benutzer sie möglicherweise deaktivieren möchten, z. B. diejenigen, die Vietnamesisch mit der UniKey-Eingabesoftware eingeben, da dies zu einer falschen Zeicheneingabe führt.

##### Zeitüberschreitung bei mehrfachem Tastendruck {#MultiPressTimeout}

Einige Tastenzuordnungen in NVDA führen unterschiedliche Aktionen aus, je nachdem, wie oft dieselbe Taste in schneller Folge gedrückt wird.
Ein Beispiel hierfür ist der Befehl "Aktuelles Zeichen des Navigator-Objekts mitteilen".
Dieser Befehl teilt das Zeichen mit, wenn es einmal gedrückt wird, eine phonetische Beschreibung des Zeichens, wenn es zweimal gedrückt wird, und den numerischen Wert des Zeichens, wenn es dreimal gedrückt wird.
Mit dieser Option können Sie die Zeitspanne festlegen, nach der ein weiterer Tastendruck ein neuer Tastenbefehl auslöst und nicht als erneuter Druck der ersten Taste gewertet wird.
Bei dem Beispielbefehl führt eine zu kurze Zeitspanne dazu, dass bei zwei Betätigungen das aktuelle Zeichen doppelt mitgeteilt wird und nicht die phonetische Beschreibung.
Die Zeitspanne beträgt standardmäßig 500 Millisekunden (ms), d. h. eine halbe Sekunde.
Eine Erhöhung dieser Zeitspanne kann insbesondere für Personen nützlich sein, die leicht klebrige Tasten verwenden oder eine körperliche Behinderung haben.
...
Um die Auflösung der Texteinheiten von überall her umzuschalten, weisen Sie bitte eine Tastenkombination über das Dialogfeld für die [Tastenzuordnungen](#InputGestures) zu.
...
Um den einfachen Darstellungsmodus per Tastenkombination ein- und auszuschalten, können Sie den entsprechenden Befehl an eine Tastenkombination zuweisen. Verwenden Sie hierfür das Dialogfeld [Tastenzuordnungen](#InputGestures).
...
Mit dieser Option, die standardmäßig aktiviert ist, können Sie auswählen, ob alle sichtbaren Kandidaten automatisch gemeldet werden sollen, wenn eine Kandidatenliste angezeigt wird oder ihre Seite geändert wird.
...
Bei phonetischen Eingabemethoden, wie z. B. Chinesisch Neue Phonetik, kann es jedoch sinnvoller sein, diese Option zu deaktivieren, da alle Symbole gleich klingen und Sie die Pfeiltasten verwenden müssen, um durch die Listenelemente einzeln zu navigieren, um mehr Informationen aus den Zeichenbeschreibungen für jeden Kandidaten zu erhalten.
...
Für Eingabemethoden, bei denen die Auswahl mit den Pfeiltasten geändert werden kann (z. B. Chinesisch Neue Phonetik), ist dies erforderlich, aber für einige Eingabemethoden kann es effizienter sein, mit ausgeschalteter Option zu tippen.
...
Mit dieser Option können Sie auswählen, ob NVDA neue Symbole melden soll, wenn sie in der Kompositionszeichenfolge angezeigt werden.
Diese Option ist standardmäßig aktiviert.
...
Um die Ausgabe von Layout-Tabellen schnell ein- oder auszuschalten, müssen Sie das Dialogfeld für die [Tastenzuordnungen](#InputGestures) verwenden, um eine Tastenkombination zuzuweisen.
...
##### Automatisch bei Änderungen im Fokus den Interaktionsmodus einschalten {#BrowseModeSettingsAutoPassThroughOnFocusChange}

Diese Option ermöglicht bei Änderungen den Interaktionsmodus automatisch zu aktivieren.
Zum Beispiel: Sie befinden sich auf einer Webseite und landen beim Drücken der Tab-Taste auf einem Formularfeld, schaltet NVDA in den Interaktionsmodus um, sofern diese Option eingeschaltet ist.

##### Beim Navigieren mit dem System-Cursor automatisch den Interaktionsmodus einschalten {#BrowseModeSettingsAutoPassThroughOnCaretMove}

Wenn diese Option eingeschaltet ist, schaltet NVDA den Interaktionsmodus beim Navigieren mit den Pfeiltasten automatisch ein oder aus.
Wenn Sie beispielsweise auf eine Webseite nach unten navigieren und auf einem Eingabefeld treffen, schaltet NVDA automatisch in den Interaktionsmodus um.
...
Mit dieser Option können Sie standardmäßig auswählen, ob Gesten (z. B. Tastendrücke), die nicht zu einem NVDA-Befehl führen und im Allgemeinen nicht als Befehlstaste betrachtet werden, daran gehindert werden sollen, zu dem Dokument zu gelangen, auf das Sie sich gerade konzentrieren.
...
* Schriftart
  * Name der Schriftart
  * Schriftgröße
  * Schrift-Attribute [(Ausgeschaltet, Über Sprachausgabe, In Braille, Über Sprachausgabe und in Braille)](#DocumentFormattingFontAttributes)
...
Um diese Einstellungen überall ändern zu können, verwenden Sie bitte das Dialogfeld für die [Tastenzuordnungen](#InputGestures) um eigene Tastenkombinationen hinzuzufügen.

##### Schrift-Attribute {#DocumentFormattingFontAttributes}

Mit dieser Option können Sie festlegen, wie bestimmte Schrift-Attribute, wie z. B. fett, kursiv, unterstrichen und durchgestrichen, mitgeteilt werden sollen.
Das Kombinationsfeld für die Schrift-Attribute bietet vier Optionen:

* Ausgeschaltet: NVDA teilt diese Schrift-Attribute nicht mit.
* Sprachausgabe: NVDA teilt mit, wenn sich diese Schrift-Attribute ändern.
* Braille: NVDA zeigt diese Attribute auf der Braillezeile an.
Wie sie genau angezeigt werden, kann in den [Braille-Einstellungen](#BrailleFormattingDisplay) in NVDA konfiguriert werden.
* Sprachausgabe und Braille: NVDA teilt Schrift-Attribute mit beiden oben genannten Methoden mit.
...
* Töne: Wenn "Töne" ausgewählt ist, geben Töne an, um wie viel sich der Einzug geändert hat.
...
Sie können von überall aus zwischen den verfügbaren Absatz-Eigenschaften umschalten, indem Sie eine Taste im Dialogfeld für die [Tastenzuordnungen](#InputGestures) zuweisen.

#### Die Einstellungen für den Store {#AddonStoreSettings}

In dieser Kategorie können Sie das Verhalten des Stores anpassen.

##### Automatic updates {#AutomaticAddonUpdates}

Wenn diese Option auf "Benachrichtigen" eingestellt ist, werden Sie über den Store benachrichtigt, nach dem Start von NVDA, wenn Updates zu NVDA-Erweiterungen verfügbar sind.
Diese Überprüfung wird alle 24 Stunden durchgeführt.
Standardmäßig werden Benachrichtigungen nur für Add-ons mit Updates angezeigt, die im selben [Kanal](#AddonStoreFilterChannel) verfügbar sind (z. B. stable, beta oder dev).
Sie können Add-on-Updatekanäle [individuell für jedes Add-on](#AddonStoreUpdateChannel) oder für [alle Add-ons](#DefaultAddonUpdateChannel) konfigurieren.

Wenn diese Option auf "Automatisch aktualisieren" eingestellt ist, werden Add-ons automatisch im Hintergrund aktualisiert.
Wenn die Updates abgeschlossen sind, werden Sie aufgefordert, NVDA neu zu starten.

| . {.hideHeaderRow} |.|
|---|---|
|Options |Notify (Default), Update Automatically, Disabled |
|Standard |Benachrichtigen |
...
Wenn [Automatische Add-On-Updates](#AutomaticAddonUpdates) aktiviert ist, werden Add-Ons standardmäßig nur auf denselben [Kanal](#AddonStoreFilterChannel) aktualisiert.
Beispielsweise wird eine installierte Beta-Version nur auf eine neuere Beta-Version aktualisiert.
Mit dieser Option wird der standardmäßige Updatekanal für alle Add-ons festgelegt.
Sie können auch den Update-Kanal für ein [spezifisches Add-on einzeln aus dem Add-on Store](#AddonStoreUpdateChannel) ändern.
...
Diese Einstellung ermöglicht automatische Updates für Add-ons, die möglicherweise nicht vollständig mit der aktuellen Version von NVDA kompatibel sind.
Standardmäßig ist dies deaktiviert, was bedeutet, dass automatische Updates nur auf Add-on-Versionen aktualisiert werden, die als kompatibel mit der aktuellen Version von NVDA markiert sind.
Automatische Updates aktualisieren eine inkompatible Add-on-Version weiterhin auf eine kompatible Version, wenn sie veröffentlicht wird.
Wenn Sie diese Option aktivieren, kann dies nützlich sein, um auf die Verwendung von Add-On-Breaking-Releases (die erste Version des Jahres) umzusteigen.
Dies ist besonders nützlich für Alpha- und Beta-Tester, die die Kompatibilität von Add-ons in den frühen Phasen eines Add-on-Breaking-Release testen.

##### Mirror server {#AddonStoreMetadataMirror}

Mit diesen Steuerelementen können Sie eine alternative URL angeben, von der Add-On-Store-Daten heruntergeladen werden sollen.
Dies kann an Speicherorten nützlich sein, an denen der Zugriff auf den NV Access Add-on Store-Server langsam oder nicht verfügbar ist.

Das schreibgeschützte Textfeld zeigt die aktuelle Mirror-URL an.
Wenn kein Spiegel verwendet wird (d. h. der NV Access Add-on Store-Server verwendet wird), wird "Kein Mirror" angezeigt.

Wenn Sie den Add-on-Store-Spiegel ändern möchten, klicken Sie auf die Schaltfläche "Ändern..." , um den Dialog [Add-on-Store-Mirror einstellen](#SetURLDialog) zu öffnen.

#### Remote Access Settings {#RemoteSettings}

In dieser Kategorie können Sie das Verhalten von [Fernzugriff](#RemoteAccess) konfigurieren.

##### Enable Remote Access {#RemoteEnable}

Aktivieren oder deaktivieren Sie dieses Kontrollkästchen, um die Fernzugriffsfunktion von NVDA zu aktivieren oder zu deaktivieren.

Wenn diese Option deaktiviert ist, ist die Funktion für den Fernzugriff nicht verfügbar.

Bitte beachten Sie, dass Sie diese Einstellung aktivieren, die Änderungen speichern und dann [Ihre Einstellungen in das Systemkonfigurationsverzeichnis von NVDA kopieren] müssen, um eine Remotezugriffssitzung auf sicheren Bildschirmen wie Eingabeaufforderungen der Benutzerkontensteuerung fortzusetzen (#GeneralSettingsCopySettings).
...
Die folgenden Optionen sind nur verfügbar, wenn der Remotezugriff aktiviert ist.
...
Mit dieser Option können Sie beim Starten von NVDA automatisch eine RAS-Sitzung einrichten.
Dies kann z.B. nützlich sein, um Ihren Heimcomputer zu steuern, wenn Sie nicht zu Hause sind.

Seien Sie vorsichtig, wenn Sie diese Option aktivieren, da dies das Risiko eines unbefugten Zugriffs auf Ihren Computer erhöhen kann.
...
Wählen Sie den Verbindungsmodus für automatische Verbindungen aus.

Diese Option ist nur verfügbar, wenn [Nach NVDA-Start automatisch verbinden](#RemoteAutoconnect) aktiviert ist.
...
Wählen Sie den Servertyp für automatische Verbindungen aus.
...
Verwenden Sie dieses Feld, um die URL des RAS-Servers festzulegen, den Sie für automatische Verbindungen verwenden möchten.
Wenn der Server einen anderen Port als 6837 verwendet, geben Sie diesen hinter einem Doppelpunkt an (z. B. `beispiel.de:1234`).
...
Um von überall aus durch die verfügbaren Sprachen zu wechseln, weisen Sie bitte einen Tastenbefehl zu, indem Sie das Dialogfeld für die [Tastenzuordnungen](#InputGestures) verwenden.

##### Regelmäßige Aktualisierung von erkannten Inhalten {#Win10OcrSettingsAutoRefresh}

Wenn dieses Kontrollkästchen aktiviert ist, aktualisiert NVDA den erkannten Inhalt automatisch, wenn ein Erkennungsergebnis den Fokus hat.
...
Wenn diese Option aktiviert ist, sollte NVDA weiterhin reagieren, wenn es mit vielen UIA-Ereignissen überflutet wird, z. B. große Textmengen in einem Terminalfenster.
...
In Windows 10 Version 1709 hat Microsoft [der Konsole](https://devblogs.microsoft.com/commandline/whats-new-in-windows-console-in-windows-10-fall-creators-update/) Unterstützung für die API der UIA hinzugefügt und damit die Leistung und Stabilität für Screenreader, die diese unterstützen, erheblich verbessert.
...
Sie müssen den Dialogfeldern eigene Tastenkombinationen zuweisen, falls Sie die Wörterbücher per Tastenkürzel aufrufen wollen. Verwenden Sie hierzu das Dialogfeld für die [Tastenzuordnungen](#InputGestures).
...
Dieses Konbinationsfeld enthält drei Optionen:
  * Niemals: Das Sonderzeichen wird nicht an die Sprachausgabe gesendet.
  * Immer: Das Symbol wird an die Sprachausgabe gesendet.
...
#### Die Tastenzuordnungen {#InputGestures}

In diesem Dialogfeld können Sie Tastenzuordnungen (Tastenkombinationen, Tasten an einer Braillezeile, Aktionen auf einem Touchscreen) an NVDA-Funktionen zuweisen.
...
Um einen Tastenbefehl zu einem Befehl hinzuzufügen, wählen Sie den Befehl aus und klicken Sie auf die Schaltfläche Hinzufügen.
...
Bei Tastenkombinationen kann es beispielsweise sinnvoll sein, das Tastatur-Layout anzugeben, in dem diese Tastenkombination wirksam werden soll (Desktop/Laptop).
...
Die folgenden Tastenzuordnungen sind auch hilfreich:
<!-- KC:beginInclude -->
...
Zum Korrekturlesen ist es sinnvoll, Einrückungen und Schrift-Attribute angesagt zu bekommen.
...
* Alles vorlesen; wird ausgelöst, sobald Sie den Befehl "Alles vorlesen" verwenden.
...
Um ein Profil zu bearbeiten, das an den Trigger "Alles vorlesen" gekoppelt ist, müssen Sie dieses Profil zuvor [Manuell aktivieren](#ConfigProfileManual).
...
Sie können auch eine Tastenkombination dafür im Dialogfeld für die [Tastenzuordnungen](#InputGestures) definieren.

#### Aktivieren eines Konfigurationsprofils mit Tastenzuordnungen {#ConfigProfileGestures}
...
Sie können Tastenzuordnungen für ein Profil im Dialogfeld für die [Tastenzuordnungen](#InputGestures) definieren.
...
Um das Einstellungs-Verzeichnis von überall her zu öffnen, können Sie das Dialogfeld für die [Tastenzuordnungen](#InputGestures) verwenden, um dieser Funktion eine Taste zuzuordnen.
...
Um von überall aus auf den Store für NVDA-Erweiterungen zuzugreifen, weisen Sie einen benutzerdefinierten Tastenbefehl zu, indem Sie das Dialogfeld für die [Tastenzuordnungen](#InputGestures) verwenden.
...
Standardmäßig werden Sie nach dem Start von NVDA benachrichtigt, sofern Updates für NVDA-Erweiterungen verfügbar sind.
Um mehr über darüber zu erfahren und zu konfigurieren, lesen Sie bitte den Abschnitt ["Update-Benachrichtigungen"](#AutomaticAddonUpdates).
...
| Name |Tastenkombination |Beschreibung|
|---|---|---|
|Protokoll-Betrachter öffnen |`NVDA+F1` |Öffnet den Protokoll-Betrachter und zeigt Entwickler-Informationen zum aktuellen Navigator-Objekt an.|
|Teil des Protokolls in die Zwischenablage kopieren |`NVDA+Strg+Umschalt+F1` |Wenn diese Tastenkombination einmal gedrückt wird, wird ein Startpunkt zum Erfassen des Protokollinhalts festgelegt. Wenn Sie  die Tastenkombination ein zweites Mal drücken, wird der Protokollinhalt ausgehend vom Startpunkt in die Zwischenablage kopiert.|
...
Um den Sprachausgaben-Betrachter per Tastenkombination ein- oder auszuschalten, müssen Sie über das Dialogfeld für die [Tastenzuordnungen](#InputGestures) eine eigene Tastenkombination zuweisen.
...
Braillezeilen haben typischerweise Tasten zum Vorwärts- oder Rückwärtsnavigieren. Um das Scrollen mit dem Braille-Betrachter zu ermöglichen, verwenden Sie das Dialogfeld für die [Tastenzuordnungen](#InputGestures), um Tastenkombinationen zuzuweisen, welche die "Braillezeile rückwärtsnavigieren" und "Braillezeile vorwärtsnavigieren" simulieren.
...
Um den Braille-Betrachter von einer beliebigen Stelle aus umzuschalten, weisen Sie bitte einen benutzerdefinierten Tastenbefehl zu, indem Sie das [Dialogfeld für die Tastenzuordnungen](#InputGestures) verwenden.
...
| Name |Tastenkombination |Beschreibung|
|---|---|---|
|Plugins neu laden |`NVDA+Strg+F3` |Lädt die globalen Plugins und Anwendungsmodule in NVDA neu.|
|Geladenes Anwendungsmodul und Programm mitteilen |`NVDA+Strg+F1` |Gibt (falls vorhanden) den Namen des Anwendungsmoduls sowie den Namen der ausführbaren Datei aus, die der fokussierten Anwendung zugeordnet ist.|
...
|Zum aktuellen Braille-Modul navigieren |Routing-Taste|
|Umschalt+Tab-Taste |Leertaste+Punkt1+Punkt2|
|Tab-Taste |Leertaste+Punkt4+Punkt5|
|Pfeiltaste nach oben |Leertaste+Punkt1|
|Pfeiltaste nach unten |Leertaste+Punkt4|
|Strg+Pfeiltaste nach links |Leertaste+Punkt2|
|Strg+Pfeiltaste nach rechts |Leertaste+Punkt5|
...
|Alt+Tab-Taste |Leertaste+Punkt2+Punkt3+Punkt4+Punkt5|
|Alt+Umschalt+Tab-Taste |Leertaste+Punkt1+Punkt2+Punkt5+Punkt6|
|Windows-Taste+Tab-Taste |Leertaste+Punkt2+Punkt3+Punkt4|
...
|Windows-Taste+M (Alle Fenster minimieren) |Leertaste+Punkt1+Punkt2+Punkt3+Punkt4+Punkt5+Punkt6|
|Aktuelle Zeile mitteilen |Leertaste+Punkt1+Punkt4|
...
|---|---|
|Auf der Braillezeile zur vorherigen Zeile navigieren |linker Kippschalter oben, rechter Kippschalter oben|
|Auf der Braillezeile zur nächsten Zeile navigieren |linker Kippschalter unten, rechter Kippschalter unten|
...
|Zum aktuellen Braille-Modul navigieren |Routing-Taste|
|Textformatierung am aktuellen Braille-Modul mitteilen |Obere Routing-Taste|
|HID-Tastatursimulation ein- oder ausschalten |T1+spEingabe|
|NVDA-Cursor zur ersten Zeile navigieren |T1+T2|
|NVDA-Cursor zur letzten Zeile navigieren |T4+T5|
|Kopplung der Braillezeile konfigurieren |T1+T3|
|Titelzeile mitteilen |etouch2|
|Statuszeile mitteilen |etouch4|
|Umschalt+Tab-Taste |Sp1|
|Alt-Taste |Sp2, Alt|
|Escape-Taste |Sp3|
|Tab-Taste |Sp4|
...
|Datum und Uhrzeit mitteilen |Sp2+Sp3|
|NVDA-Menü |Sp1+Sp3|
|Windows-Taste+M (Alle Fenster minimieren) |Sp1+Sp4|
|Windows-Taste+B (Infobereich anzeigen) |Sp3+Sp4|
|Windows-Taste |Sp1+Sp2, Windows|
|Alt+Tab-Taste |Sp2+Sp4|
|Strg+Pos1 |T3+SpUp|
|Strg+Ende |T3+SpDown|
|Pos1 |T3+SpLeft|
|Ende |T3+SpRight|
...
Die meisten Braillezeilen von [HandyTech](https://www.handytech.de/) werden von NVDA via seriellem Port, USB und Bluetooth unterstützt.
Bei älteren USB-Braillezeilen werden Sie den Universaltreiber installieren müssen.
...
Folgende Tastenkombinationen bei diesen Braillezeilen sind in NVDA zugeordnet.
...
|Auf der Braillezeile zur vorherigen Zeile navigieren |UP|
|Auf der Braillezeile zur nächsten Zeile navigieren |DN|
|Cursor zum aktuellen Braille-Modul ziehen |Route|
...
|Auf der Braillezeile zur vorherigen Zeile navigieren |B2|
|Auf der Braillezeile zur nächsten Zeile navigieren |B5|
...
|Auf der Braillezeile zur vorherigen Zeile navigieren |Auf|
|Auf der Braillezeile zur nächsten Zeile navigieren |Ab|
|Cursor zum aktuellen Braille-Modul ziehen |Routing-Taste|
...
|Pfeiltaste nach links |Leertaste+Punkt3|
|Pfeiltaste nach rechts |Leertaste+Punkt6|
|Umschalt+Tab-Taste |Leertaste+Punkt1+Punkt3|
|Tab-Taste |Leertaste+Punkt4+Punkt6|
|Alt-Taste |Leertaste+Punkt1+Punkt3+Punkt4 (Leertaste+M)|
|Escape-Taste |Leertaste+Punkt1+Punkt5 (Leertaste+E)|
|Eingabetaste |Punkt8|
|Windows-Taste |Leertaste+Punkt3+Punkt4|
|Alt+Tab-Taste |Leertaste+Punkt2+Punkt3+Punkt4+Punkt5 (Leertaste+T)|
|NVDA-Menü |Leertaste+Punkt1+Punkt3+Punkt4+Punkt5 (Leerttaste+N)|
|Windows-Taste+M (Alle Fenster minimieren) |Leertaste+Punkt1+Punkt4+Punkt5 (Leertaste+D)|
|Alles vorlesen |Leertaste+Punkt1+Punkt2+Punkt3+Punkt4+Punkt5+Punkt6|
...
|Windows-Taste+M (Alle Fenster minimieren) |C1+C4+C5 (Befehl D)|
|Alles vorlesen |C1+C2+C3+C4+C5+C6|
...
|Zur vorherigen Zeile navigieren |Rollen oben auf der linken Seite und rollen oben auf der rechten Seite|
|Zur nächsten Zeile navigieren |Rollen unten auf der linken Seite und rollen unten auf der rechten Seite|
|NVDA-Cursor zur vorherigen Zeile navigieren |Pfeiltaste nach oben auf der rechten Seite|
|NVDA-Cursor zur nächsten Zeile navigieren |Pfeiltaste nach unten auf der rechten Seite|
|NVDA-Cursor zum vorherigen Zeichen navigieren |Pfeiltaste nach links auf der rechten Seite|
|NVDA-Cursor zum nächsten Zeichen navigieren |Pfeiltaste nach rechts auf der rechten Seite|
...
|Pfeiltaste nach links |Punkt3+Leertaste, Pfeiltaste nach links auf der linken Seite|
...
|Pfeiltaste nach oben |Punkt1+Leertaste, Pfeiltaste nach oben auf der linken Seite|
...
|Auf der Braillezeile rückwärts navigieren |Links|
|Auf der Braillezeile vorwärts navigieren |Rechts|
|Auf der Braillezeile zur vorherigen Zeile navigieren |B3|
|Auf der Braillezeile zur nächsten Zeile navigieren |B4|
...
|Auf der Braillezeile rückwärts navigieren |Links|
|Auf der Braillezeile vorwärts navigieren |Rechts|
|Alles vorlesen |Leertaste+Rücktaste|
|NVDA-Menü |Links+Rechts|
|Auf der Braillezeile zur vorherigen Zeile navigieren |LJ oben|
|Auf der Braillezeile zur nächsten Zeile navigieren |LJ unten|
|Braillezeile koppeln umschalten |LJ mittig|
|Tab-Taste |LJ rechts|
|Umschalt+Tab-Taste |LJ links|
...
|Rücktaste |Punkt7|
|Seite nach oben |Leertaste+LJ rechts|
|Seite nach unten |Leertaste+LJ links|
|Pos1 |Leertaste+LJ oben|
|Ende |Leertaste+LJ unten|
...
1. Navigieren Sie in der Liste nach unten zu "USB-Controller" oder "USB-Geräte".
1. Wählen Sie "Papenmeier Braillex USB-Gerät" aus.
...
|Auf der Braillezeile zur nächsten Zeile navigieren |Ab|
|Cursor zum aktuellen Braille-Modul ziehen |Routing-Taste|
|Zeichen unter dem Cursor vorlesen |L1|
|Führt die Standardaktion aus |L2|
|Koppelt Braillezeile wahlweise an Fokus oder Anzeige |R2|
|Titelleiste des aktiven Fensters vorlesen |L1+Auf|
|Statusleiste des aktiven Fensters vorlesen |L2+Ab|
|Das Navigator-Objekt zum übergeordneten Objekt ziehen |Auf2|
|Das Navigator-Objekt zum ersten beinhaltenden Objekt ziehen |Ab2|
|Das Navigator-Objekt zum nächsten Objekt ziehen |Rechts2|
|Das Navigator-Objekt zum vorherigen Objekt ziehen |Links2|
|Textformatierungen am aktuellen Braille-Modul mitteilen |Obere Routing-Taste|
...
|Pfeiltaste nach oben |Leertaste+Punkt2|
|Pfeiltaste nach links |Leertaste+Punkt1|
|Pfeiltaste nach rechts |Leertaste+Punkt4|
|Pfeiltaste nach unten |Leertaste+Punkt5|
|Strg |LT+Punkt2|
|Alt-Taste |LT+Punkt3|
...
|Auf der Braillezeile zur nächsten Zeile navigieren |Ab|
|Cursor zum aktuellen Braille-Modul ziehen |Routing-Taste|
|Zeichen unter dem NVDA-Cursor mitteilen |L1|
|Aktuelles Navigator-Objekt aktivieren |L2|
|Titelleiste mitteilen |L1+Auf|
|Statusleiste mitteilen |L2+Ab|
|Zum übergeordneten Objekt navigieren |Auf2|
|Zum ersten untergeordneten Objekt navigieren |Ab2|
|Zum vorherigen Objekt navigieren |Links2|
|Zum nächsten Objekt navigieren |Rechts2|
|Textformatierungen am Braille-Modul mitteilen |Obere Routing-Taste|

BRAILLEX Tiny

| Name |Taste|
|---|---|
|Aktuelles Zeichen am NVDA-Cursor mitteilen |L1|
|Aktuelles Navigator-Objekt aktivieren |L2|
|Auf der Braillezeile rückwärts navigieren |Links|
|Auf der Braillezeile vorwärts navigieren |Rechts|
|Auf der Braillezeile zur vorherigen Zeile navigieren |Auf|
...
|Zum übergeordneten Objekt navigieren |R1+Auf|
|Zum ersten untergeordneten Objekt navigieren |R1+Ab|
|Zum vorherigen Objekt navigieren |R1+Links|
|Zum nächsten Objekt navigieren |R1+Rechts|
|Textformatierungen am Braille-Modul mitteilen |obere Routing-Taste|
|Titelleiste mitteilen |L1+Auf|
|Statusleiste mitteilen |L2+Ab|
...
|Auf der Braillezeile vorwärts navigieren |Rechts|
|Auf der Braillezeile zur nächsten Zeile navigieren |Ab|
|Zum nächsten Objekt navigieren |Links2|
|Zum übergeordneten Objekt navigieren |Auf2|
|Zum ersten untergeordneten Objekt navigieren |Ab2|
|Zum vorherigen Objekt navigieren |Rechts2|
...
Das Scrollrad besteht aus vier punktförmigen Pfeiltasten, einer mittleren Taste sowie dem eigentlichen Rad, das Sie mit und gegen den Uhrzeigersinn drehen können.
...
Die folgenden Befehle sind im Braillenote QT verfügbar, wenn die Braille-Eingabe nicht aktiviert ist.
...
| Name |Taste|
|---|---|
|Pfeiltaste nach oben |Pfeil auf|
|Pfeiltaste nach unten |Pfeil ab|
|Pfeiltaste nach links |Pfeil links|
|Pfeiltaste nach rechts |Pfeil rechts|
|Eingabetaste |mittlere Taste|
|Tab-Taste |Rad im Uhrzeigersinn drehen|
|Umschalt+Tab-Taste |Rad gegen den Uhrzeigersinn drehen|
...
|Position des NVDA-Cursors mitteilen |F0|
...
#### Tastenzuordnungen für b.note {#Eurobraillebnote}
...
|Zum aktuellen Braille-Modul wechseln |`routing`|
|Textformatierungen am Braille-Modul mitteilen |`doubleRouting`|
...
#### Tastenzuordnungen für Esys {#Eurobrailleesys}
...
#### Tastenzuordnungen für Esytime {#EurobrailleEsytime}
...
| Name |Taste|
|---|---|
|Auf der Braillezeile rückwärts navigieren |Aufwärts|
|Auf der Braillezeile vorwärts navigieren |Abwärts|
|Auf der Braillezeile zur vorherigen zeile navigieren |Nach links|
|Auf der Braillezeile zur nächsten Zeile navigieren |nach rechts|
|Cursor zum aktuellen Braille-Modul ziehen |Routing-Taste|
...
|Eingabehilfe umschalten |`learn` (Modus zum Lernen der Tastenzuordnungen ein- oder ausschalten)|
...
|Datum und Uhrzeit mitteilen |`f9`|
...
* Speichern der Tastenzuordnungen auf dem Speichermedium
...
| . {.hideHeaderRow} |.|
|---|---|
|`nvda -mc CONFIGPATH` |Dies startet NVDA mit deaktivierten Sounds und ohne Meldung beim Starten und der angegebenen Konfiguration|
|`nvda -mc CONFIGPATH --disable-addons` |Gleiches wie oben, aber mit deaktivierten NVDA-Erweiterungen|
...
| . {.hideHeaderRow} |.|
|---|---|
|`nvda -l 10` |Legt fest, dass NVDA mit der Protokollierungsstufe Debug starten soll|
|`nvda --log-file=C:\\NVDA.log` |Speichert das Protokoll in die Datei `C:\\NVDA.log`|
|`nvda --log-level=20 -f C:\\NVDA.log` |Legt die Protokollierungsstufe fest auf Informationen und speichert das Protokoll in die Datei `C:\\NVDA.log`|
...
|Keine |`--create-portable` |Erstellt eine portable Version (und startet die neue Version). Erfordert die Angabe von `--portable-path`|
...
|---|---|---|
|`/S` |Keine |Deinstalliert NVDA ohne Rückmeldungen. |
