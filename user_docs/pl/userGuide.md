...
* Systemy operacyjne: wsparcie wszystkich 32-bitowych i 64-bitowych wersji systemu Windows 7, Windows 8, Windows 8.1, Windows 10 i Windows 11 (w tym systemów operacyjnych dla serwerów od Windows Server 2008 R2).
 * Dla Windows 7, NVDA wymaga Service Pack 1 lub wyższego.
 * Dla Windows Server 2008 R2, NVDA wymaga Service Pack 1 lub wyższego.
...
Jeżeli chcesz, Naciskaj `strzałkę wdół` aby przeczytać umowę licencyjną.
...
|Czytaj zaznaczenie |`NVDA+shift+strzałka w górę` |`NVDA+shift+s` |czyta każdy zaznaczony tekst|
|Czytaj tekst w schowku |`NVDA+c` |`NVDA+c` |Czyta jakikolwiek tekst w schowku|
...
|Odczytaj fokus |`NVDA+tab` |`NVDA+tab` |Czyta aktualną kontrolkę w fokusie.  dwukrotne naciśnięcie przeliteruje informacje|
...
Jeżeli chcesz nosić NVDA ze sobą na użądzeniu pendrive lub innym urządzeniu zapisywalnym, w takim razie trzeba stworzyć kopię przenośną.
...
Jednakże, jeżeli chcesz skopiować NVDA na użądzenie tylko do odczytu takie jak CD, powinieneś skopiować tylko pobrany pakiet.
...
* Niemożliwość automatycznego uruchamiania podczas i po logowaniu.
* Niemożliwość uruchamiania programów wymagających prawa administratora, do póki, oczywiście NVDA nie jest uruchomiony z tymi prawami (niezalecane).
* Niemożliwość odczytu ekranów kontroli konta użytkownika (UAC) podczas próby uruchomienia aplikacji z prawami administratora.
* Windows 8 i nowsze: niemożliwość wsparcia ekranów dotykowych.
* Windows 8 i nowsze: niemożliwość dostarczania funkcji takich jak tryb przeglądania i czytanie wpisanych znaków w aplikacjach Microsot store.
* Windows 8 i nowsze: przyciszanie dźwięku nie jest wspierane.
...
Dla zainstalowanych kopii, NVDA domyślnie przechowuje konfigurację w podfolderze roaming folderu dane aplikacji aktualnego użytkownika (np. "C:\Users\<uzytkownik>\AppData\Roaming").
...
Możesz przeczytać więcej informacji o zbieranych danych w ustawieniach ogólnych w [sekcji im poświęconej](#GeneralSettingsGatherUsageStats).
...
### Nawigacja kursorem systemowym {#toc55}
...
MathPlayer jest bezpłatnie dostępny do pobrania z : https://www.dessci.com/en/products/mathplayer/.
Po zainstalowaniu MathPlayera, trzeba ponownie uruchomić NVDA.
...
Można ją pobrać ze strony https://www.dessci.com/en/products/mathtype/
* Adobe Reader:
...
Więcej informacji o klawiszach nawigacji, zawiera [dokumentacja MathPlayer](https://www.dessci.com/en/products/mathplayer/navigation_commands.htm)
...
##### Otwiera ustawienia ogólne {#toc104}
...
##### Zezwalaj projektowi NVDA na zbieranie statystyk użytkowania programu {#GeneralSettingsGatherUsageStats}
...
##### Otwiera ustawienia mowy {#toc117}
...
Gdy ta opcaje jest włączona, NVDA wypowie opis znaku podczas poruszania się po znakach.
...
##### Otwiera okno dialogowe wybór syntezatora {#toc137}
...
Nigdy
: Opcje
Domyślnie (Nigdy), Nigdy, Tylko podczas przywoływania automatycznego, zawsze
...
##### Otwiera okno dialogowe wybor monitora brajlowskiego {#toc160}
...
##### Otwiera ustawienia dźwięku {#toc165}
...
Ta opcja odnosi skutek gdy opcja "Używaj wasapi do wyjścia audio" jest włączona.
...
Jeżeli chcesz przełączać stan kurtyny z dowolnego miejsca, wymagane jest przypisanie odpowiedniego skrótu na ekranie ["Zdarzenia wejścia"](#InputGestures).
...
##### Otwiera ustawienia klawiatury {#toc175}
...
W związku z tym, po kliknięciu w oknie preferencji NVDA przycisku Ok lub Zastosuj, pojawi się stosowny komunikat o błędzie, jeśli wszystkie pola wyboru klawiszy NVDA są nieoznaczone.
...
##### Otwiera ustawienia myszy {#toc188}
...
##### Podaj typ obiektu wskazanego myszą {#MouseSettingsRole}
...
Ta kategoria ustawień, , dostępna tylko na komputerach z systemem Windows 8 i nowszymi z ekranem dotykowym, pozwala skonfigurować w jaki sposób NVDA obsługuje ekrany dotykowe.
Zawiera następujące ustawienia:
...
##### Otwiera opcje prezentacji obiektów {#toc205}
...
##### Otwiera ustawienia trybu czytania {#toc223}
...
##### Otwiera ustawienia formatowania dokumentów {#toc237}
...
| . {.hideHeaderRow} |.|
|---|---|
|Opcje |Domyślnie (zarządzane przez aplikacje), zarządzane przez aplikacje, jeden rozdział wiersza, rozdział wielowierszowy|
|Domyślny |Zarządzane przez aplikację|
...
| . {.hideHeaderRow} |.|
|---|---|
|Opcje |Automatycznie, selektywnie, globalnie|
...
Jest to funkcja eksperymentalna, i niektóre funkcje programu Microsooft Excel mogą być niedostępne.
...
Implementacja UIA w programie Microsoft Excel UI automation zmienia się z wersję na wersje, i wersje starsze niż 16.0.13522.10000 mogą nie dostarczać żadnych korzystnych informacji.

##### Obsługa konsoli systemu Windows {#AdvancedSettingsConsoleUIA}

| . {.hideHeaderRow} |.|
|---|---|
|Opcje |Automatyczne, UIA gdy jest to możliwe, przestarzałe|
...
Po więcej informacji zapraszamy na stronę [https://docs.python.org/3.7/howto/regex.html].
 ...
 * Przypisanie polecenia NVDA korzystającego z klawiszy modyfikujących do emulowanego skrótu pozbawionego 
 modyfikatorów. Na przykład, przypisanie emulowanego polecenia 'A' do polecenia 'CTRL+M' może skończyć się 
...
Lista niezgodnych lub zainstalowanych dodatków może być filtrowana wedłóg ich stanu.
...
Naciśnięcie NVDA+F1 otworzy podgląd logów i wyświetli informacje dla programistów o aktualnym obiekcie nawigatora.
...
Gdy klikniesz lub umieścisz fokus wewnątrz podglądu, NVDA tymczasowo zatrzyma aktualizację tekstu, dzięki czemu możesz łatwo zaznaczyć lub skopiować istniejące treści.
...
W systemie Windows 7, 8 i 8.1, NVDA używa domyślnie syntezatora eSpeak NG ([Głosy Windows OneCore](#OneCore) natomiast, są używane w systemie Windows 10 i nowszych jego wersjach.
...
* Microsoft Speech Platform - Runtime Languages (Wersja 11): https://www.microsoft.com/download/en/details.aspx?id=27224
...
Aby sprawdzić listę dostępnych głosów, prosimy przeczytać artykuł na stronie firmy Microsoft: https://support.microsoft.com/en-us/windows/appendix-a-supported-languages-and-voices-4486e345-7730-53da-fcfe-55cc64300f01
...
Jeśli nie zostały one jeszcze zainstalowane, to należy je pobrać ze strony: https://www2.freedomscientific.com/downloads/focus-40-blue/focus-40-14-blue-downloads.asp.
...
#### Przypisania klawiszy dla wszystkich modeli {#toc318}
...
#### Przypisania klawiszy dla Brailliant BI 32, BI 40 and B 80 {#toc319}
...
#### Przypisania klawiszowe dla Brailliant BI 14 {#toc320}
...
Możesz je pobrać stąd: http://www.himsintl.com/upload/HIMS_USB_Driver_v25.zip
...
Więcej informacji o tych monitorach można znaleźć na stronie https://en.seika-braille.com/down/index.html.
...
Te linijki nie są jeszcze obsługiwane przez funkcję NVDA automatycznego wykrywania linijki brajlowskiej.
...
|Przełącza między trbami mowy wyłączony, sygnały i mowa |`attribute2+attribute4`|
...
Aby zapobiec zmianę konfiguracji i dodatków bezpośrednio przez użytkownika, dostęp użytkownikowi musi być zabroniony.
...
|-l PożiomLogowania |--log-level=LOGLEVEL |Najniższy poziom zapisywanych zdarzeń do podglądu logu (debugowanie 10, wejście/wyjście 12, debugowanie powiadomienie 15, info 20, wyłączone 100)|
