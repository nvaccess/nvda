...
* Możliwość [łączenia się i kontrolowania innego komputera z NVDA](#RemoteAccess) w celu uzyskania zdalnej pomocy lub współpracy
...
* Możliwość uruchamiania podczas logowania do systemu Windows i [innych bezpiecznych pulpitów](#SecureScreens)
...
Oprócz języka angielskiego, NVDA zostało przetłumaczone na 55 języków, w tym: afrykanerski, albański, amharski, arabski, aragoński, bośniacki, bułgarski, birmański, kataloński, chiński (uproszczony i tradycyjny), chorwacki, czeski, duński, niderlandzki, perski, fiński, francuski, galicyjski, gruziński, niemiecki (Niemcy i Szwajcaria), grecki, hebrajski, hindi, węgierski, islandzki, irlandzki, włoski, japoński, kannada, koreański, kirgiski, litewski, macedoński, mongolski, nepalski, norweski, polski, portugalski (Brazylia i Portugalia), pendżabski, rumuński, rosyjski, serbski, słowacki, słoweński, hiszpański (Kolumbia i Hiszpania), szwedzki, tamilski, tajski, turecki, ukraiński i wietnamski.
...
Może być konieczne odczekanie kilku sekund, podczas gdy ładuje się tymczasowa kopia NVDA.
Po uruchomieniu, mowa będzie aktywna podczas całego procesu.
1. Instalator NVDA podczas uruchomienia wyświetla przycisk, za pomocą którego można zapoznać się z warunkami licencji.
Naciśnij strzałkę w dół, aby zapoznać się z treścią licencji.
...
1. Pojawi się okno dialogowe potwierdzające pomyślną instalację NVDA.
...
| Nazwa |Polecenie dla komputerów stacjonarnych |Polecenie dla komputerów przenośnych |opis|
|---|---|---|---|
|Czytaj pisane znaki |`NVDA+2` |`NVDA+2` |przełącza odczytywanie wpisywanych znaków. |
|Czytaj pisane słowa |`NVDA+3` |`NVDA+3` przełącza odczytywanie wpisywanych słów. |
...
Zalecamy rozpoczęcie od modułu "Szkolenie podstawowe z obsługi NVDA".
...
Ta opcja pozwala wybrać, czy NVDA ma skopiować konfigurację użytkownika z aktualnie uruchomionego NVDA do konfiguracji dla obecnie zalogowanego użytkownika, do użycia w zainstalowanej kopii NVDA.
...
Domyślnie NVDA używa układu klawiatury desktop, ale można to zmienić w kategorii klawiatura [ustawień NVDA](#NVDASettings), znajdujących się w podmenu opcje w menu NVDA.
...
Dotknięcie jednym palcem jest nazywane dotknięciem.
...
Gdy pomoc klawiatury jest włączona, wciskane klawisze nie będą interpretowane jako polecenia.
...
|Przejdź do początku zaznaczenia w trybie przeglądu |`NVDA+alt+home` |`NVDA+alt+home` |None |przenosi kursor przeglądu do pierwszego znaku zaznaczenia|
|Przejdź do końca zaznaczenia w trybie przeglądu |`NVDA+alt+end` |`NVDA+alt+end` |None |Przenosi kursor przeglądu do ostatniego znaku zaznaczenia|
...
Dobrym sposobem na zapamiętanie podstawowych poleceń przeglądu tekstu podczas korzystania z układu Desktop jest wyobrażenie ich sobie jako siatki trzy na trzy, gdzie od góry do dołu to linia, słowo i znak, a od lewej do prawej poprzedni, bieżący i następny.
...
|---|---|---|
|Znajdź tekst |NVDA+control+f |otwiera dialog wyszukiwania|
|Znajdź następny |NVDA+f3 |Poszukuje następnego wystąpienia ostatnio poszukiwanego tekstu|
|Znajdź poprzedni |NVDA+shift+f3 |znajduje poprzednie wystąpienie wyszukiwanej frazy|
...
NVDA posiada jednak funkcję kopiowania z zachowaniem formatowania, która może być użyta w obsługiwanych dokumentach w trybie przeglądania (Mozilla Firefox, lub każda przeglądarka oparta na Chromium 134 lub nowszym). Powoduje ona, że kopiowanie z zachowaniem formatowania podąża za zaznaczeniem w trybie przeglądu NVDA.
...
Naciśnięcie `NVDA+f5` odświeża wynik rozpoznawania.
...
NVDA oferuje własne dodatkowe funkcje dla niektórych aplikacji, aby ułatwić wykonywanie określonych zadań lub zapewnić dostęp do funkcji, które inaczej byłyby niedostępne dla użytkowników czytników ekranu.
...
| Polecenie |Klawisz |Opis|
|---|---|---|
|Ustaw nagłówek kolumn |`NVDA+shift+c` |Jednokrotne naciśnięcie informuje NVDA, że jest to pierwsza komórka wiersza zawierającego nagłówki kolumn, które powinny być automatycznie ogłaszane podczas poruszania się między kolumnami poniżej tego wiersza. Dwókrotne naciśnięcie wyczyści to ustawienie.|
|Ustaw nagłówek wierszy |`NVDA+shift+r` |Jednokrotne naciśnięcie informuje NVDA, że jest to pierwsza komórka kolummy zawierającej nagłówki wierszy, które powinny być automatycznie ogłaszane podczas poruszania się między wierszami poniżej tej kolumny. Dwókrotne naciśnięcie wyczyści to ustawienie.|

<!-- KC:endInclude -->
Te ustawienia zostaną zapisane w dokumencie jako zakładki, kompatybilne z innymi programami czytania ekranu np. Jaws.
Oznacza to, że użytkownicy innych czytników ekranu, którzy otworzą ten dokument w późniejszym terminie, będą mieli już ustawione nagłówki wierszy i kolumn.
...
<!-- KC:endInclude -->
Wszystkie komentarze do dokumentu, wraz z poprawkami w ramach śledzenia zmian mogą być wyświetlone na liście elementów NVDA, jeśli jako typ wybrano adnotacje.
...
Jeśli pole jest zaznaczone, podczas zamykania NVDA pojawi się okno dialogowe z następującymi opcjami:

* wyjdź
* uruchom ponownie
* uruchom w trybie debugowania z wyłączonymi dodatkami
* uruchom w trybie debugowania
* zainstaluj oczekujące aktualizacje (jeśli są obecne).
...
Można również ręcznie zainstalować oczekującą aktualizację wybierając odpowiednią opcję z okna dialogowego "zakończ NVDA" (jeśli jest włączone), z menu NVDA, lub sprawdzając aktualizacje w menu pomoc.

##### Serwer aktualizacji {#UpdateMirror}

Te ustawienia pozwalają określić adres URL, z którego będą pobierane aktualizacje NVDA.
Może to być przydatne w miejscach, gdzie dostęp do serwera NVAccess jest utrudniony lub niemożliwy.

Pole edycji tylko do odczytu zawiera aktualny adres URL.
Jeśli serwer lustrzany nie jest używany, czyli aktualizacje pobierane są z serwera NVAccess, pole edycji nie wyświetla adresu URL.

W celu zmiany serwera aktualizacji, należy nacisnąć przycisk "Zmień..." aby otworzyć [okno dialogowe ustaw serwer aktualizacji](#SetURLDialog).

Należy pamiętać, że administrator serwera otrzymuje wszystkie [informacje wysyłane wraz ze sprawdzeniem aktualizacji](#GeneralSettingsCheckForUpdates).
Aby uzyskać szczegóły dotyczące przetwarzania danych, skontaktuj się z administratorem serwera w celu upewnienia się, że dane będą przetwarzane w akceptowalny i bezpieczny sposób przed ustawieniem danego serwera aktualizacji.

##### Zapobiegaj wyłączaniu się monitora podczas korzystania z opcji czytaj wszystko lub czytania brajla {#PreventDisplayTurningOff}

Ta opcja, jeśli jest włączona, utrzymuje monitor aktywnym podczas korzystania z opcji czytaj wszystko lub czytania brajla, na przykład poprzez naciskanie przycisków przewijania.
Zapobiega to nieoczekiwanemu wygaszeniu ekranu podczas korzystania z opcji czytaj wszystko.
Ta opcja jest domyślnie włączona.
Ta opcja może powodować szybsze rozładowywanie baterii.
...
Pierwszą opcją w kategorii ustawień poświęconej mowie jest przycisk zmień.
Ten przycisk wywołuje okno [wyboru syntezatora](#SelectSynthesizer), które pozwala na wybranie syntezatora mowy do użycia.
...
| . {.hideHeaderRow} |.|
|---|---|
|Opcje |domyślne (Włączone), Włączone, Wyłączone|
|Domyślne |Włączone|
...
Wybranie opcji "Automatycznie" spowoduje, że tablica zostanie określona na podstawie bieżącego języka NVDA.
...
W tym przypadku brajl nie będzie podążał za fokusem i kursorem.
...
##### Mów podczas nawigacji po liniach i akapitach {#BrailleSpeakOnNavigating}

Ta opcja spowoduje, że NVDA wypowie linie i akapity, które będą się pojawiać na monitorze brajlowskim podczas nawigacji po tekście.

Aby móc szybko przełączać tę opcję, dodaj własny skrót do zdarzenia "Mów podczas nawigacji po liniach i akapitach" w sekcji "Brajl" w [zdarzeniach wejścia](#InputGestures).
...
* NVDA po lewej, a aplikacje w obu kanałach: NVDA będzie mówić w lewym kanale, podczas gdy inne aplikacje będą odtwarzać dźwięki w obu kanałach, lewym i prawym.
...
Ta opcja przełącza wypowiadanie wpisywanych znaków przez NVDA.
Dostępne opcje to:

* Wyłączone: NVDA nie będzie czytać wpisywanych znaków.
* Tylko w polach edycji: NVDA będzie odczytywać wpisywane znaki tylko w polach edycji oraz tam, gdzie można wpisać tekst.
* Zawsze: NVDA będzie czytać wszystkie wpisywane znaki.
...
Ta opcja przełącza odczytywanie wpisywanych słów przez NVDA.
Dostępne opcje to:

* Wyłączone: NVDA nie będzie odczytywać wpisywanych słów.
* Tylko w polach edycji: NVDA będzie odczytywać wpisywane słowa w polach edycji oraz tam, gdzie można wpisać tekst.
* Zawsze: NVDA będzie odczytywać wszystkie wpisywane słowa.
...
Ta opcja jest domyślnie włączona, choć niektórzy użytkownicy mogą chcieć ją wyłączyć, na przykład osoby piszące po wietnamsku za pomocą oprogramowania do pisania UniKey, ponieważ może to powodować błędne wprowadzanie znaków.
...
Ta opcja (domyślnie włączona) pozwala wybrać, czy wszyscy widoczni kandydaci mają być automatycznie zgłaszani po pojawieniu się listy kandydatów lub zmianie jej strony.
...
Jednak w przypadku fonetycznych metod wprowadzania, takich jak Chinese New Phonetic, może być bardziej przydatne wyłączenie tej opcji, ponieważ wszystkie symbole będą brzmiały tak samo i będzie należało używać klawiszy strzałek do indywidualnego nawigowania po elementach listy, aby uzyskać więcej informacji z opisów znaków dla każdego kandydata.
...
Dla metod wprowadzania, gdzie wybór można zmieniać za pomocą klawiszy strzałek (takich jak Chinese New Phonetic), jest to konieczne, ale wyłączenie tej opcji dla niektórych metod wprowadzania może być bardziej efektywne.
...
Ta opcja pozwala wybrać, czy NVDA ma zgłaszać nowe symbole pojawiające się podczas kompozycji ciągu znaków.
Opcja jest domyślnie włączona.
...
Opcja ta, włączona domyślnie, pozwala wybrać, czy polecenia (takie jak naciśnięcia), które nie skutkują wydaniem polecenia NVDA i nie są ogólnie uważane za takie, powinny być przechwytywane przed przejściem do dokumentu, na którym aktualnie się skupiasz.
...
* Elementów
  * Nagłówków
  * Linków
  * Typ linku (odnośnik do bieżącej strony)
  * Obrazków
  * List
  * Cytatów
  * Grup
...
* Mowa: Jeśli tryb mowy jest wybrany, a poziom wcięć się zmienia, NVDA powinno powiedzieć coś w stylu "dwanaście spacji" lub "cztery razy tab."
* Dźwięki: Jeśli wybrano tryb dźwięków, a poziom wcięć się zmienia, będzie on oznajmiany dźwiękami.
...
##### Automatyczne aktualizacje {#AutomaticAddonUpdates}
...
Domyślnie, powiadomienia pojawią się wyłącznie dla dodatków z tego samego [kanału](#AddonStoreFilterChannel) (NP. Stabilny, beta lub dev).
Możesz skonfigurować kanały aktualizacji [indywidualnie dla każdego dodatku](#AddonStoreUpdateChannel) lub [wszystkich dodatków](#DefaultAddonUpdateChannel).

Gdy wybierzesz "Aktualizuj automatycznie", dodatki zaktualizują się samoistnie w tle.
Gdy aktualizacja dodatków dobiegnie końca, zostaniesz poproszony o zrestartowanie NVDA.

| . {.hideHeaderRow} |.|
|---|---|
|Opcje |Powiadamiaj (Domyślnie), Aktualizacja automatyczna, Wyłączone |
|Domyślnie |powiadom |

|Opcja |Zachowanie |
|---|---|
|Powiadamiaj |Powiadamia, gdy aktualizacje dodatków są dostępne |
|Automatyczna aktualizacja |Automatycznie aktualizuje dodatki w tle |
|Wyłączone |nie sprawdza aktualizacji dodatków |

##### Domyślny kanał aktualizacji {#DefaultAddonUpdateChannel}

Jeśli [Automatyczne aktualizacje](#AutomaticAddonUpdates) są włączone, domyślnie dodatki aktualizują się wyłącznie z tego samego [kanału](#AddonStoreFilterChannel).
NP. Zainstalowana wersja beta zaktualizuje się wyłącznie do najnowszej dostępnej bety.
Ta opcja ustawia domyślny kanał aktualizacji dla wszystkich dodatków.
Możesz również indywidualnie zmienić kanał aktualizacji dla [wybranego dodatku w Add-on Store](#AddonStoreUpdateChannel).

| . {.hideHeaderRow} |.|
|---|---|
| Opcje | Tożsamy, (Domyślnie), Wszystkie,, Nie aktualizuj, Stabilny, Beta lub dev, Beta, Dev |
| Domyślny | Tożsamy |

| Opcja | Zachowanie |
|---|---|
| Tożsamy | Dodatki zostają na swoim kanale |
| Dowolny | Dodatki zostaną automatycznie zaktualizowane do najnowszej wersji bez względu na kanał |
| Nie aktualizuj | Dodatki domyślnie nie będą aktualizowane. Aby to zmienić, musisz włączyć aktualizację dla każdego ręcznie |
| Stabilny | Dodatki będą automatycznie aktualizowane do wersji stabilnych |
| Beta lub dev | Dodatki będą automatycznie aktualizowane do wersji beta lub dev |
| Beta | Dodatki będą automatycznie aktualizowane do wersji beta |
| Dev | Dodatki będą automatycznie aktualizowane do wersji dev |

##### Zezwalaj na automatyczne aktualizacje, aby instalować niezgodne dodatki {#AllowIncompatibleAddonUpdates}

To ustawienie włącza automatyczne aktualizacje dla dodatków, które mogą nie być w pełni zgodne z obecną wersją NVDA.
Domyślnie, to ustawienie jest wyłączone. Oznacza to, że automatycznej aktualizacji poddane zostaną wyłącznie te dodatki, które są oznaczone jako zgodne z obecną wersją NVDA.
Aktualizacje automatyczne nadal będą aktualizować niezgodną wersję dodatku do zgodnej wersji po jej wydaniu.
Włączenie tej opcji może być przydatne do przełączenia się na korzystanie z wersji powodujących niezgodność dodatków (co pierwsza wersja w roku).
Jest to szczególnie przydatne dla testerów wersji alfa oraz beta, testujących zgodność dodatków na wczesnych etapach wersji powodującej niezgodność dodatków.

##### Serwer lustrzany {#AddonStoreMetadataMirror}

Te kontrolki pozwalają na określenie adresu, z którego metadane dla Add-on store będą pobierane.
Może to być przydatne w lokalizacjach, w których dostęp do serwera NV Access Add-on Store jest wolny lub niedostępny.

W polu edycyjnym tylko do odczytu pokazuje się aktualnie używany adres serwera lustrzanego.
Jeżeli nie jest używany serwer lustrzany, czyli używany jest serwer organizacji NV Access, wyświetla siękomunikat "brak serwera lustrzanego".

Jeśli chcesz zmienić adres serwera lustrzanego, naciśnij przycisk "zmień...", aby otworzyć [okno dialogowe "ustaw adres serwera lustrzanego"](#SetURLDialog).

#### Ustawienia dostępu zdalnego {#RemoteSettings}

Ta kategoria umożliwia konfigurację opcji [pomocy zdalnej](#RemoteAccess).

##### Włącz pomoc zdalną {#RemoteEnable}

Użyj tego pola wyboru, aby włączyć lub wyłączyć funkcję pomocy zdalnej NVDA.

Jeśli to pole jest odznaczone, opcja pomocy zdalnej jest całkowicie wyłączona.

Prosimy pamiętać, że aby używać pomocy zdalnej na bezpiecznych ekranach takich jak NP. Kontrola konta użytkownika, musisz włączyć to ustawienie, zapisać konfigurację, a następnie [skopiować ustawienia do folderu konfiguracji systemowej NVDA](#GeneralSettingsCopySettings).
Zdalny dostęp będzie kontynuowany tylko na komputerach, na których sesje zostały rozpoczęte przez zainstalowaną kopię NVDA na bezpiecznych pulpitach.
Oznacza to, że nie możesz, na przykład, rozpocząć sesji zdalnego dostępu z przenośną kopią NVDA i kontynuować sesji na bezpiecznych ekranach.

Gdy Pomoc zdalna jest aktywowana, dostępne są następujące opcje.

##### Pytaj przed rozłączeniem podczas kontrolowania {#RemoteConfirmDisconnect}

Ta opcja określa, czy potwierdzenie jest wymagane przed rozłączeniem z komputerem zdalnym, gdy jest podłączony jako kontrolowany.

Po zaznaczeniu NVDA poprosi o potwierdzenie przed rozłączeniem.
Gdy ta opcja nie jest zaznaczona, NVDA rozłączy się natychmiast bez potwierdzenia.

##### Łącz automatycznie z serwerem kontroli przy starcie NVDA {#RemoteAutoconnect}

Ta opcja umożliwia Ci automatyczne rozpoczynanie sesji pomocy zdalnej przy starcie NVDA.
Może być ona użyteczna NP. gdy chcesz kontrolować swój komputer domowy, znajdując się po za domem.

Prosimy o ostrożność przy zaznaczaniu niniejszej opcji, ponieważ może zwiększyć ona ryzyko nieautoryzowanego dostępu do Twojego komputera.

Aby uzyskać więcej informacji na temat następujących opcji, zapoznaj się z sekcją [Konfigurowanie sesji dostępu zdalnego](#RemoteAccessSetup)

##### Tryb {#RemoteAutoconnectMode}

Wybierz tryb łączenia dla automatycznego startu pomocy zdalnej.

Ta opcja dostępna jest wyłącznie, gdy [Automatyczne łączenie z serwerem po starcie NVDA](#RemoteAutoconnect) jest włączone.

|Opcja |Zachowanie |
|---|---|
|Zezwalaj na kontrolę tego komputera |Używaj tego komputera jako "kontrolowanego". To pozwala osobie siedzącej przy "kontrolującym" komputerze na używanie tego komputera tak jakby siedziała blizko niego. |
|Kontroluj inny komputer |Używaj swojego komputera jako"kontrolującego". Ta opcja umożliwia wykonywanie poleceń na komputerze zdalnym. |

##### Serwer {#RemoteAutoconnectServer}

Wybierz typ serwera dla połączeń automatycznych.

Ta opcja jest dostępna wyłącznie, gdy zezwolimy na [automatyczne łączenie po starcie NVDA](#RemoteAutoconnect).

|Opcja |Zachowanie |
|---|---|
|Użyj serwera zewnętrznego |Użyj istniejącego serwera pomocy zdalnej do pośredniczenia podczas połączenia (rekomendowane). |
|Serwer lokalny |Użyj tej instancji NVDA jako serwera pomocy zdalnej (zaawansowane). Ta opcja może wymagać konfiguracji sieci, która nie jest tematem tego podręcznika. |

##### Host {#RemoteAutoconnectHost}

Użyj tego pola, aby wprowadzić adres serwera pomocy zdalnej, z którego chcesz skorzystać dla połączeń automatycznych.
Jeśli serwer używa innego portu niż 6837, wprowadź go po dwukropku (NP. `example.com:1234`).

Ta opcja jest dostępna wyłącznie, gdy zezwolimy na [automatyczne łączenie po starcie NVDA](#RemoteAutoconnect), a opcja [Serwer](#RemoteAutoconnectServer) ustawiona jest na "Używaj serwera zewnętrznego ".

##### Port {#RemoteAutoconnectPort}

Użyj tego pola, aby wprowadzić port TCP, z którego chcesz korzystać podczas połączeń automatycznych.

Ta opcja jest dostępna wyłącznie, gdy zezwolimy na [automatyczne łączenie po starcie NVDA](#RemoteAutoconnect), a opcja [Serwer](#RemoteAutoconnectServer) ustawiona jest jako "Serwer lokalny".

##### Klucz {#RemoteAutoconnectKey}

Użyj tego pola, aby wprowadzić klucz (hasło), którego chcesz używać dla połączeń automatycznych.

Ta opcja jest dostępna wyłącznie, gdy zezwolimy na [automatyczne łączenie po starcie NVDA](#RemoteAutoconnect).

##### Usuń wszystkie zaufane odciski palców {#RemoteDeleteFingerprints}

Niniejszy przycisk, pozwala na usunięcie wszystkich zapamiętanych wcześniej odcisków palców dla zaufanych serwerów.
Oznacza to, że użytkownik zostanie ponownie zapytany, czy połączyć się ze wszystkimi nierozpoznanymi serwerami dostępu zdalnego, nawet tymi, z którymi wcześniej się łączył.
Zostaniesz poproszony o potwierdzenie, zanim wszystkie zaufane odciski palców zostanąusunięte.

Opcja ta będzie dostępna wyłącznie, gdy w konfiguracji przechowywane są zaufane odciski palców.
...
#### Okno dialogowe "ustaw adres serwera lustrzanego" {#SetURLDialog}

To okno dialogowe pozwala określić adres serwera lustrzanego, który ma być używany podczas [aktualizacji NVDA](#GeneralSettingsCheckForUpdates) lub [korzystania ze sklepu z dodatkami](#AddonsManager).
Może to być przydatne w lokalizacjach, w których dostęp do serwerów NV Access dla tych funkcji jest wolny lub niedostępny.

* Podczas ustawiania [serwera lustrzanego do aktualizacji NVDA](#UpdateMirror), tytuł tego okna dialogowego będzie brzmiał "ustaw serwer lustrzany do aktualizacji NVDA".
* Podczas ustawiania [serwera lustrzanego dla Add-on store](#AddonStoreMetadataMirror), tytuł tego okna dialogowego będzie brzmiał "Ustaw serwer lustrzany dla Add-on store".

##### Adres {#SetURLTextbox}

Tutaj wpisz adres serwera lustrzanego, którego chcesz użyć.
Wspierane są tylko adresy HTTP i HTTPS.
Ze względu na Twoją prywatność NV Access zaleca używanie adresów URL HTTPS, gdy tylko jest to możliwe.

Aby użyć domyślnego serwera organizacji NV Access, pozostaw to pole puste.

##### Przetestuj... {#SetURLTest}

Naciśnij ten przycisk, aby sprawdzić poprawność wpisanego adresu serwera lustrzanego.
Aby test zakończył się pomyślnie, musisz mieć połączenie z Internetem.
Zawsze jest zalecane sprawdzanie adresu serwera lustrzanego przed zapisywaniem.
...
* Tylko w stosownych przypadkach: gdy model obiektowy programy Microsoft Word nie jest dostępny
...
##### Przycinaj początkową ciszę podczas generowania mowy {#TrimLeadingSilenceSpeech}

Gdy włączone, NVDA usunie ciszę po rozpoczęciu transmisji mowy, co może poprawić responsywność niektórych syntezatorów.
Niniejsza opcja jest domyślnie włączona, i powinna wpływać wyłącznie na ciszę podczas rozpoczęcia generacji mowy.
Jeśli zauważysz, że potrzebne przerwy (NP. pauzy między dwoma zdaniami) podczas używania syntezatora będącego dodatkiem do NVDA są również wycinane, możesz całkowicie wyłączyć tąfunkcję aby wyeliminować ten błąd.

##### Używaj WASAPI dla wyjścia audio SAPI 4 {#UseWASAPIForSAPI4}

Ta opcja umożliwia głosom Microsoft Speech API w wersji 4 (SAPI 4) odtwarzanie dźwięku za pomocą Windows Audio Session API (WASAPI).
Pozwoli to na działanie głosów SAPI 4 z większą ilością funkcji, takich jak przyciszanie dźwięku podczas generowania mowy, usuwanie ciszy z początków generowanego audio oraz pozostawiania urządzeń dźwiękowych w stanie wzbudzenia.
Jednakże, niektóre głosy SAPI 4 mogą nie działać z obecną implementacją WASAPI.
Gdy zauważysz, że używany przez Ciebie głos SAPI 4 przestał działać, możesz wyłączyć tą funkcję.
...
Zwykłe ukośniki wsteczne zaś powinni być napisane dwukrotnie, NP.  "a\\\\b" musi być wpisane, w celu zamiany na  "a\\b".
...
| Działanie |Skróty Klawiszowe Desktopa |Skróty Klawiszowe Laptopa |Opis|
|---|---|---|---|
|Zapisz ustawienia |`NVDA+control+c` |`NVDA+control+c` |Zapisuje Twoje obecne ustawienia, co zapobiega utraceniu ich po wyłaczeniu NVDA|
|Przywróć zapisane ustawienia |`NVDA+control+r` |`NVDA+control+r` |Jednokrotne naciśnięcie przywraca ostatnią zapisaną konfigurację, natomiast trzykrotne, resetuje NVDA do ustawień fabrycznych.|
...
## Pomoc zdalna {#RemoteAccess}

Przy pomocy wbudowanej funkcji Pomocy zdalnej NVDA, możesz kontrolować inny komputer używający tego czytnika ekranu, lub zezwolić na kontrolę Twojego komputera innej osobie.
Ułatwia to udzielenie lub otrzymanie pomocy, współpracę, lub zdalny dostęp do Twojego własnego komputera z innego miejsca.

Dla Twojego bezpieczeństwa, Pomoc zdalna jest domyślnie wyłączona.
Dostęp zdalny można włączyć [w ustawieniach dostępu zdalnego](#RemoteSettings).

### Szybki start {#RemoteAccessGettingStarted}

Zanim zaczniesz, upewnij się, że NVDA jest zainstalowane i uruchomione na obu komputerach.
Upewnij się również, że pole wyboru Włącz dostęp zdalny [w ustawieniach dostępu zdalnego](#RemoteSettings) jest zaznaczone na obu komputerach.
Funkcja zdalnego dostępu będzie wtedy dostępna w menu Narzędzia w NVDA; Nie ma potrzeby pobierania dodatkowych plików ani instalacji.

Jeśli NVDA jest uruchomiony w [trybie bezpiecznym](#SecureMode), nie można ręcznie zainicjować nowej sesji dostępu zdalnego.
Jednak NVDA nadal będzie automatycznie uruchamiać nową sesję dostępu zdalnego, jeśli zostanie skonfigurowana do tego z ustawieniem [Automatycznie łącz po uruchomieniu NVDA](#RemoteAutoconnect).
Nadal możesz ręcznie rozłączyć się w trybie bezpiecznym.

### Konfigurowanie sesji dostępu zdalnego {#RemoteAccessSetup}

Sesje dostępu zdalnego konfiguruje się za pomocą okna dialogowego Połączenie dostępu zdalnego.
Otwórz menu NVDA i wybierz Narzędzia, następnie "Zdalny dostęp", a następnie "Połącz...".
Lub z dowolnego miejsca naciśnij 'NVDA+alt+r'.

#### Wybór trybu (kontrolujący lub kontrolowany) {#RemoteAccessMode}

Musisz zdecydować, który komputer będzie kontrolowany (komputer kontrolowany), a który będzie kontrolował (komputer kontrolujący).

Lista rozwijana tryb pozwala wybrać, czy ten komputer będzie kontrolowany (inna osoba będzie wchodzić w interakcję z komputerem użytkownika), czy kontrolujący (użytkownik będzie wchodzić w interakcję z komputerem innej osoby).

* Jeśli potrzebujesz innej osoby do pomocy przy komputerze, wybierz opcję "Zezwalaj na kontrolowanie tego komputera".
* Jeśli zamierzasz pomóc komuś z jego komputerem, wybierz "Kontroluj inny komputer".

#### Nawiązywanie połączenia z istniejącym serwerem (dostarczony host i klucz) {#RemoteAccessJoin}

W takim przypadku druga osoba albo nawiązała połączenie za pośrednictwem serwera przekazującego, albo hostuje połączenie lokalnie.

1. W polu Serwer wybierz opcję "Użyj istniejącego".
1. W polu Host (Host) wprowadź adres hosta (i port, jeśli został również podany).
  * Możesz dołączyć port, z którym chcesz się połączyć, wpisując dwukropek (":") i numer portu do adresu hosta.
Na przykład "example.com:1234".
1. W polu Klucz wprowadź podany klucz.
Uważaj, aby wpisać klucz dokładnie tak, jak podano.
1. Naciśnij przycisk OK.

#### Rozpoczynanie połączenia za pośrednictwem serwera przekazującego {#RemoteAccessConnectRelay}

Jeśli zezwalasz innej osobie na łączenie się z Tobą przez Internet, najprostszą opcją jest użycie serwera przekazującego. Aby to zrobić:

1. W polu Serwer wybierz opcję "Użyj istniejącego".
1. W polu host podaj nazwę hosta serwera przekazującego, na przykład "nvdaremote.com".
Będziesz musiał podać ten adres drugiej osobie.
  * Możesz dołączyć port, z którym chcesz się połączyć, wpisując dwukropek (":") i numer portu do adresu hosta.
NP. `example.com:1234`.
1. Dla klucza: wymyśl długi, trudny do odgadnięcia klucz lub naciśnij przycisk "Generuj klucz", aby został automatycznie wygenerowany.
Będziesz musiał przekazać to drugiej osobie.
  * Należy pamiętać, że klucz pełni zarówno funkcję identyfikatora sesji, jak i hasła, dlatego bardzo ważne jest, aby był trudny do odgadnięcia.
1. Naciśnij przycisk OK.
1. Podaj szczegóły połączenia (takie jak host i klucz) drugiej osobie, aby mogła się połączyć.

#### Używanie NVDA jako serwera {#RemoteAccessConnectSelfHosted}

Jeśli nie chcesz korzystać z serwera przekazującego lub łączysz się za pośrednictwem sieci lokalnej:

1. W polu Serwer wybierz opcję "Hostuj lokalnie".
1. Jeśli łączysz się przez Internet, możesz nacisnąć przycisk "Pobierz zewnętrzny adres IP", aby wyświetlić zewnętrzny adres IP (widoczny w Internecie).
Będziesz musiał podać ten zewnętrzny adres IP drugiej osobie.
1. Jeśli łączysz się przez sieć lokalną, musisz podać drugiej osobie swój lokalny adres IP.
znajdowanie tej informacji nie jest omówione w tym podręczniku.
1. W polu Port wprowadź port, którego chcesz używać dla przychodzących połączeń zdalnych.
Druga osoba również będzie tego potrzebować.
  * Powinien to być port, który nie jest używany przez żadnych innych usług.
  * Domyślnie dostęp zdalny używa portu 6837.
  * W przypadku nawiązywania połączenia przez Internet może być konieczne przekierowanie portu.
  ten podręcznik nie obejmuje sposobu przekierowywania portów.
  W tym celu może być konieczne skontaktowanie się z administratorem sieci.
1. Dla klucza: wymyśl długi, trudny do odgadnięcia klucz lub naciśnij przycisk "Generuj klucz", aby został automatycznie wygenerowany.
Będziesz musiał przekazać to drugiej osobie.
  * Należy pamiętać, że klucz pełni zarówno funkcję identyfikatora sesji, jak i hasła, dlatego bardzo ważne jest, aby był trudny do odgadnięcia.
1. Naciśnij przycisk OK.
...
### Korzystanie z pomocy zdalnej {#RemoteAccessUsage}

Gdy sesja dostępu zdalnego jest aktywna, można przełączać się między sterowaniem komputerem zdalnym a własnym, udostępniać schowek i wyciszać sesję zdalną:

* Naciśnij `NVDA+alt+tab` aby przełączać się między kontrolowanym i Twoim komputerem.
* Wyślij tekst ze schowka do innego komputera, otwierając menu NVDA, a następnie wybierając Narzędzia, następnie "Zdalny dostęp", a następnie "Wyślij schowek".
* Jeśli jest podłączony jako komputer sterujący, wycisz mowę komputera zdalnego na komputerze lokalnym, otwierając menu NVDA, a następnie wybierając Narzędzia, następnie "Zdalny dostęp", a następnie "Wycisz zdalne".
* Jeśli jest podłączony jako komputer kontrolujący, wyślij "control+alt+delete" do kontrolowanego komputera, otwierając menu NVDA, następnie wybierając Narzędzia, następnie "Zdalny dostęp", a następnie "Wyślij control+alt+delete".
  Aby to zadziałało, na kontrolowanym komputerze musi być zainstalowana kopia NVDA.
  Dodatkowo, w zależności od ustawień kontrolowanego komputera, funkcja ta może nie zawsze działać.

### Podsumowanie skrótów klawiszowych dla pomocy zdalnej {#RemoteAccessGestures}

<!-- KC:beginInclude -->
| Nazwa |skrót |Opis|
|---|---|---|
| Przełącz Połączenie zdalne | 'NVDA+alt+r' | Rozpoczyna nową sesję dostępu zdalnego lub, jeśli sesja jest już w toku, rozłącza się z nią. |
| Przełącz kontrolę | `NVDA+alt+tab` | Przełącza między kontrolą innego i naszego komputera. |
| Połącz się | Brak | Uruchamia nową sesję dostępu zdalnego. Niedostępne w [trybie bezpiecznym](#SecureMode). |
| Skopiuj link | Brak | Kopiuje link sesji pomocy zdalnej do schowka. |
| Rozłącz | Brak | Kończy obecną sesję pomocy zdalnej. |
| Wycisz pomoc zdalną | Brak | Wycisza lub odcisza komunikaty przychodzące z kontrolowanej maszyny. |
| Wyślij schowek | Brak | Wysyła zawartość schowka do komputera zdalnego. |
<!-- KC:endInclude -->
...
#### Sortowanie listy dodatków według kolumn {#AddonStoreSortByColumn}

Dodatki są domyślnie sortowane według nazwy.
Listy rozwijanej "Sortuj według kolumny" można użyć do sortowania listy według dostępnych kolumn dla każdej karty właściwości.
Na przykład możesz zechcieć sortować dodatki według wydawcy, dostępnej wersji itp.
Dodatki mogą być sortowane rosnąco albo malejąco.
...
Jeżeli pobieranie albo instalacja zakończą się niepowodzeniem, możesz je powtórzyć.
Istnieje również możliwość anulowania instalacji dodatku przed zamknięciem NVDA Add-on Store.
...
Możesz wyłączyć dodatek, jeśli stan dodatku wskazuje, że jest "włączony" lub włączyć go, jeśli dodatek jest "wyłączony".
...
#### Zmiana kanału do automatycznych aktualizacji {#AddonStoreUpdateChannel}

Kanałami aktualizacji dla dodatków można zarządzać z poziomu [kart właściwości zainstalowane dodatki i dodatki do zaktualizowania](#AddonStoreFilterStatus).
Gdy włączona jest opcja [Automatyczne aktualizacje dodatków](#AutomaticAddonUpdates), dodatki będą aktualizowane do tego samego [kanału](#AddonStoreFilterChannel), z którego zostały zainstalowane [domyślnie](#DefaultAddonUpdateChannel).
Z menu "Działania" gdy dodatek jest wybrany, istnieje możliwość zmiany kanału aktualizacji z którego dodatek będzie aktualizowany, używając menu rozwijane "kanał aktualizacji".

| Opcja | Zachowanie |
|---|---|
| Domyślny | Dodatek będzie aktualizowany z [domyślnego kanału aktualizacji](#DefaultAddonUpdateChannel) |
| ten sam | Dodatek pozostanie na tym samym kanale aktualizacji |
| Jakikolwiek | Dodatek będzie zaktualizowany do najnowszej wersji, bez względu na kanał |
| Nie aktualizuj | Dodatek nie będzie automatycznie aktualizowany |
| Stabilny | Dodatek będzie aktualizowany na wersje stabilne |
| Beta lub dev | Dodatek będzie aktualizowany do wersji beta lub dev |
| Beta | Dodatek będzie aktualizowany do wersji beta |
| Dev | dodatek będzie automatycznie aktualizowany do wersji dev |
...
Czasami mogą wystąpić problemy z rejestrem systemu Windows, które powodują, że NVDA zachowuje się w nieoczekiwany sposób.
Może to być spowodowane na przykład instalacją lub odinstalowywaniem niektórych programów (takich jak Adobe Reader lub Math Player), a także aktualizacjami systemu Windows i innymi zdarzeniami.
Narzędzie naprawiania błędów rejestracji COM próbuje rozwiązać te problemy, naprawiając wpisy ułatwień dostępu w rejestrze.

Rodzaje problemów, które to narzędzie może rozwiązać, obejmują:

* Nieoczekiwany odczyt przez NVDA "nieznane" lub "okno", podczas nawigacji w przeglądarkach Firefox lub Edge, w programach pocztowych NP. Thunderbird, w Exploratorze Windows, na pasku zadań i w innych programach.
* Problem z niepowodzeniem przełączenia się do trubów czytania lub fokusu gdy jest to potrzebne.
* Przyciski, których nazwy wcześniej były wymawiane, a potem zaczęły być wymawiane jako "przycisk".
* Powolność NVDA podczas poruszania się po przeglądarkach używając tryb przeglądania.

Ponieważ to narzędzie koryguje wpisy w rejestrze systemu Windows, do działania wymaga praw administratora, podobnie jak podczas instalowania jakiegokolwiek programu.
Jeśli masz włączoną kontrolę dostępu do konta użytkownika (UAC), tak jak większość użytkowników, będziesz musiał postępować zgodnie z instrukcjami wyświetlanymi przez UAC, aby pomyślnie uruchomić narzędzie.
...
Aby uzyskać jeszcze bardziej obszerną listę darmowych i komercyjnych syntezatorów, które możesz kupić i pobrać do użytku z NVDA, zobacz [stronę z listą dodatkowych głosów](https://github.com/nvaccess/nvda/wiki/ExtraVoices).
...
Monitory brajlowskie serii Brailliant BI and B firmy [HumanWare](https://www.humanware.com/), włączając w to BI 14, BI 32, BI 20X, BI 40, BI 40X and B 80, są wspierane gdy są podłączone przez port USB oraz Bluetooth.
...
Poniżej znajdują się skróty klawiszowe dla monitorów brajlowskich Brailliant BI/B oraz BrailleNote touch do użytku z programem NVDA.
Aby odnaleźć opisywane klawisze, zajrzyj do dokumentacji urządzenia.
...
Informacje na temat BrailleNote Touch można znaleźć w rozdziale [Brailliant BI Series / BrailleNote Touch](#HumanWareBrailliant).
...
|strzałka w lewo |spacja+punkt3, joystick w lewo lub dpad left|
...
Urządzenie Dot Pad firmy Dot Inc to monitor brajlowski, który potrafi wyświetlać brajl i rysunki.
NVDA może wyświetlać zarówno pojedynczą linię brajla na dedykowanej linijce monitora brajlowskiego, jak i wiele linii brajla na obszarze graficznym.
Model A300 ma dotykowy obszar graficzny o wymiarach 120 na 80 punktów, w którym zmieści się 8 linii po 20 komórek każda.

Możesz skonfigurować, czy NVDA wyświetla brajl w dedykowanej linii monitora brajlowskiego, albo na obszarze graficznymdotykowej, za pomocą opcji punkt końcowy brajla w ustawieniach brajla dla tego sterownika.

Klawisze do przewijania są wspierane, ale z powodu ograniczonej ilości przycisków na urządzeniu, inne polecenia i możliwość przywoływania komórek brajlowskich nie jest wspierana.

Gdy wybierzesz sterownik Dot Pad w NVDA, powinieneś ręcznie wybrać port USB albo wirtualny port szeregowy bluetooth, do którego Dot Pad został podłączony.
Ten sterownik nie wspiera funkcję automatycznego wykrywania.

Należy pamiętać, że ze względu na ograniczenia sprzętowe Dot Pad nie odświeży poprawnie wszystkich kropek, gdy ręka znajduje się na urządzeniu.
Upewnij się, że całkowicie oderwiesz rękę od urządzenia podczas nawigacji za pomocą NVDA i zacznij czytać ponownie dopiero po pełnym odświeżeniu treści na urządzeniu.

<!-- KC:beginInclude -->

| Nazwa |Skrót|
|---|---|
|Przewiń monitor brajlowski wstecz | `pan_left` |
|Przewiń monitor brajlowski w przód | `pan_right` |
...
* Otwieranie zewnętrznych dokumentów z menu NVDA, takich jak plik z instrukcją obsługi.
...
|`-n JEZYK` |`--lang=JEZYK` |Nadpisuję bieżący język NVDA. Ustaw tą opcje na  "Windows" dla aktualej domyślnej wartości użytkownika, "en" dla angielskiego, itd.|
|`-m` |`--minimal` |Bez dźwięku, interfejsu, informacji początkowej etc.|
|`-s` |`--secure` |Uruchamia NVDA w [Trybie bezpiecznym](#SecureMode)|
|`-d` |`--disable-addons` |Dodatki bie będą funkcjonować|
...
|Brak |`--create-portable-silent` |Tworzy kopię przenośną NVDA (bez uruchamiania nowej kopii). Wymaga określenia `--portable-path`. Ta opcja powoduje, że ostrzeżenia podczas tworzenia plików w pełnych katalogach będą wyciszone a pliki mogą być nadpisane bez ostrzeżenia.|
|Brak |`--portable-path=SCIEZKA` |Ścieżka, w której zostanie utworzona przenośna kopia|

Tak jak możesz zainstalować NVDA po cichu, przekazując opcję wiersza poleceń `--install-silent` do NVDA, tak samo można ją po cichu odinstalować, przekazując polecenie `/S` do deinstalatora.

Deinstalator programu NVDA nazywa się `uninstall.exe` i znajduje się w katalogu instalacyjnym NVDA, `%ProgramFiles(x86)%\nvda` w 64-bitowym Windowsie, lub `%ProgramFiles%\nvda` w 32-bitowym Windowsie.

Poniżej znajdują się opcje wiersza poleceń dla deinstalatora NVDA:

| Krótka |długa |Opis|
|---|---|---|
|`/S` |brak |Cicho usuwa NVDA. |
