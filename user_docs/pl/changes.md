...
* Add-on store:
  ...
  * Listy dodatków można sortować według kolumn, w tym daty publikacji, w porządku rosnącym i malejącym. (#15277, #16681, @nvdaes)
* Mowa:
...
* NVDA jest teraz tłumaczone na język bośniacki. (#17953)
...
* IDE:
  ...
  * W Notatniku i innych dokumentach UIA oraz dokumentach Notepad++ w systemie Windows 11, jeśli ostatni wiersz jest pusty, polecenie "przenieś do następnego wiersza" przeniesie kursor do ostatniego wiersza.
...
* Dodano gesty dla monitorów brajlowskich Tivomatic Caiku Albatross. (#14844, #15002)
  ...
  * wWłączanie/wyłączanie kursora brajla
  * przełączanie stanu "Wskaźnik zaznaczenia brajlowskiego wyświetlacza"
  * przełączanie w trybie "przemieszczania kursoru systemowego podczas užywania kursora przeglādu". (#15122)
...
Są one opcjonalne ze względu na zgodność z NVDA, ale są zalecane lub wymagane do przesłania do Sklepu z dodatkami. (#14754)
  ...
  * užywaj formatu `<major>.<minor>.<patch>` format dla pola wersji(wymagade dla Add-on datastore).
...
* Ta wersja narusza zgodność z istniejącymi dodatkami.
...
* Ta wersja narusza zgodność z istniejącymi dodatkami.
* W tym wydaniu porzuca się również obsługę programu Adobe Flash.
...
* Zaktualizowano Liblouis do wersji 3.16.1:
  * Rozwiązuje wiele problemów z awariami
  * Dodaje tabelę brajlowską stopnia 1 w języku baszkirskim
  * Dodano koptyjską tabelę brajlowską z 8 punktami
  * Dodaje tabele rosyjskiego brajla literackiego i rosyjskiego brajla literackiego (szczegółowego)
  * Dodano tabelę brajlowską klasy afrikaans 2
  * Usuwa rosyjską tabelę brajlowską stopnia 1
...
* Obiekty specyficzne dla NVDA, które są znajdowane przez cykliczny garbage collector Pythona, są teraz rejestrowane podczas usuwania przez kolektor, aby pomóc w usuwaniu cykli referencyjnych z NVDA. (#11499)
  ...
  * Klasa, która musi być śledzona, powinna dziedziczyć po garbageHandler.TrackedObject.
...
* Dodano kurtynę ekranu, która po włączeniu sprawia, że cały ekran jest w systemie Windows 8 i nowszych. (#7857)
  * Dodano skrypt włączający kurtynę ekranową (do następnego restartu za pomocą jednego naciśnięcia lub zawsze, gdy NVDA działa z dwoma naciśnięciami), nie jest przypisany żaden domyślny gest.
...
* Klasy SynthDriver nie powinny już uwidaczniać indeksu za pośrednictwem właściwości lastIndex.
  * Zamiast tego powinni powiadomić akcję synthDriverHandler.synthIndexReached za pomocą indeksu, gdy wszystkie poprzednie dźwięki zakończą odtwarzanie przed tym indeksem.
...
* Kilka funkcji mowy zostało zmienionych w celu zwrócenia sekwencji mowy. (#10098)
  * getControlFieldSpeech (Mowa w polu getControlField)
  * getFormatFieldSpeech (mowa w polu)
...
* Dodano moduł textUtils, aby uprościć różnice między ciągami Pythona 3 a ciągami Unicode systemu Windows. (#9545)
  * Zobacz dokumentację modułu i moduł textInfos.offsets dla przykładowych implementacji.
...
* gui.nvdaControls zawiera teraz dwie klasy do tworzenia list z ułatwieniami dostępu z polami wyboru. (#7325)
  * CustomCheckListBox jest dostępną podklasą wx. Pole listy.
...
* Jeśli chcesz udostępnić widżet wx, który jeszcze tego nie zrobił, możesz to zrobić za pomocą instancji gui.accPropServer.IAccPropServer_impl. (#7491)
  * Zobacz implementację gui.nvdaControls.ListCtrlAccPropServer, aby uzyskać więcej informacji.
...
* Dodano obsługę automatycznego wykrywania monitorów brajlowskich w tle. (#1271)
  ...
  * Zapoznaj się z dokumentacją, aby uzyskać dodatkowe informacje.
...
* Kilka ulepszeń obsługi alfabetu Braille'a w programie Microsoft Word z włączoną automatyzacją interfejsu użytkownika:
  ...
  * Zmniejszono zbyt szczegółową prezentację fokusu zarówno w mowie, jak i w alfabecie Braille'a podczas ustawiania ostrości na dokumencie programu Word. (#8407)
  * Przekierowywanie kursora w alfabecie Braille'a działa teraz poprawnie na liście w dokumencie programu Word. (#7971)
...
* Wprowadzono pewne zmiany w module hwPortUtils: (#1271)
  ...
  * Słowniki dostarczane przez listComPorts zawierają teraz również wpis usbID dla portów COM z informacją o USB VID/PID w ich identyfikatorze sprzętowym.
...
* Możesz teraz zarejestrować się, aby otrzymywać powiadomienia o akcjach zapisywania/resetowania konfiguracji za pośrednictwem nowych akcji config.pre_configSave, config.post_configSave, config.pre_configReset i config.post_configReset. (#7598)
  ...
  * config.pre_configReset i config.post_configReset zawiera flagę ustawień fabrycznych, która określa, czy ustawienia mają być ponownie ładowane z dysku (false), czy resetowane do wartości domyślnych (true).
...
* Teraz możliwa jest interakcja z wykresami w programach Microsoft Word i Microsoft PowerPoint, podobnie jak w przypadku istniejącej obsługi wykresów w programie Microsoft Excel. (#7046)
  ...
  * W programie Microsoft PowerPoint podczas edytowania slajdu: naciśnij Tab do obiektu wykresu i naciśnij Enter lub spację, aby wejść w interakcję z wykresem.
...
* Nie jest już konieczne wyraźne wiązanie brajla w celu skupienia lub przeglądania, ponieważ domyślnie dzieje się to automatycznie. (#2385)
  * Należy pamiętać, że automatyczne tethering z recenzją będzie miał miejsce tylko w przypadku korzystania z kursora recenzji lub polecenia nawigacji po obiekcie. Przewijanie nie aktywuje tego nowego zachowania.
...
* Dodano ukrytą flagę logiczną do sekcji brajlowskiej w konfiguracji: "outputPass1Only". (#7301, #7693, #7702)
  * Ta flaga jest domyślnie ustawiona na wartość true. Jeśli wartość false, do wyjścia brajlowskiego zostaną użyte reguły wieloprzebiegowe liblouis.
...
* Gesty monitora brajlowskiego, które emulują modyfikatory klawiatury systemowej (takie jak control i alt), mogą być teraz łączone z innymi emulowanymi klawiatury systemowej bez wyraźnej definicji. (#6213)
  * Na przykład, jeśli masz na wyświetlaczu powiązany z alt i inny wyświetlania z downArrow, połączenie tych spowoduje emulację alt+downArrow.
...
* Możesz teraz wybrać, jakie informacje kontekstowe mają być wyświetlane na monitorze brajlowskim, gdy obiekt jest aktywny, korzystając z nowego ustawienia "Prezentacja kontekstu ostrości" w oknie dialogowym Ustawienia brajla. (#217)
  * Na przykład opcje "Wypełnij ekran w przypadku zmian kontekstu" i "Tylko podczas przewijania do tyłu" mogą sprawić, że praca z listami i menu będzie bardziej wydajna, ponieważ elementy nie będą stale zmieniać swojego położenia na wyświetlaczu.
...
* Usunięto niektóre przestarzałe kody:
  * – Mowa. REASON_*' stałe: zamiast tego należy użyć 'controlTypes.REASON_*'. (#6846)
...
* Obecnie istnieje framework do testów jednostkowych dla NVDA. (#7026)
  ...
  * Jeśli przesyłasz żądanie ściągnięcia dla NVDA, powinieneś najpierw uruchomić testy i upewnić się, że zakończyły się one pomyślnie.
...
* Ulepszone I/O dla monitorów brajlowskich: (#5609)
  * Bezpieczne wątkowo sterowniki monitorów brajlowskich mogą deklarować się jako takie za pomocą atrybutu BrailleDisplayDriver.isThreadSafe. Sterownik musi być bezpieczny wątkowo, aby można było korzystać z następujących funkcji.
  * Dane są zapisywane w bezpiecznych wątkowo sterownikach monitorów brajlowskich w tle, co zwiększa wydajność.
  * hwIo.Serial rozszerza pyserial tak, aby wywoływał obiekt po odebraniu danych, zamiast gdy sterowniki muszą odpytywać.
  * hwIo.Hid zapewnia wsparcie dla monitorów brajlowskich komunikujących się za pośrednictwem USB HID.
...
* Istnieje kilka nowych właściwości dostępnych z gestów ekranu dotykowego: (#5652)
  * Obiekty MultitouchTracker zawierają teraz właściwość childTrackers, która zawiera obiekty MultiTouchTrackers, z których składał się moduł śledzący. Na przykład dwukrotne stuknięcie 2 palcami ma moduły śledzące dzieci dla dwóch stuknięć 2 palcami. Same stuknięcia 2 palcami mają podrzędne urządzenia śledzące na dwa dotknięcia.
  * Obiekty MultiTouchTracker zawierają teraz również właściwość rawSingleTouchTracker, jeśli moduł śledzący powstał w wyniku stuknięcia, przesunięcia lub najechania palcem. SingleTouchTracker umożliwia dostęp do podstawowego identyfikatora przypisanego do palca przez system operacyjny oraz do tego, czy palec jest nadal w kontakcie w danym momencie.
...
* Można emitować dwa nowe gesty ekranu dotykowego: (#5652)
  * Liczba mnoga stuknięcia i przytrzymania (np. dwukrotne stuknięcie i przytrzymanie)
  * Uogólniony identyfikator z usuniętą liczbą palców dla blokad (np. przytrzymanie + najechanie kursorem na 1finger_hold + najechanie kursorem).
...
* W Windows 8 i nowszych, NVDA uruchamia się teraz znacznie wcześniej, gdy jest skonfigurowany do uruchamiania po zalogowaniu się do Windows. (#308)
  * Jeśli włączyłeś tę funkcję za pomocą poprzedniej wersji NVDA, będziesz musiał ją wyłączyć i włączyć ponownie, aby zmiana zaczęła obowiązywać. Postępuj zgodnie z tą procedurą:
    ...
    1. Ponownie otwórz okno dialogowe Ustawienia ogólne.
...
* Nagłówki kolumn tabeli w programie Microsoft Word są automatycznie raportowane dla tabel, w których wiersz nagłówka został jawnie określony przez autora za pomocą właściwości tabeli programu Microsoft Word. (#4510)
  * Jednak w przypadku tabel, w których wiersze zostały scalone, nie będzie to działać automatycznie. W tej sytuacji nadal możesz ustawić nagłówki kolumn ręcznie w NVDA za pomocą NVDA+shift+c.
...
* Obsługa kalendarza w programie Microsoft Outlook 2007 i nowszych (#2943), w tym:
  ...
  * Inteligentne filtrowanie daty, aby ogłosić datę tylko wtedy, gdy nowo wybrana godzina lub termin przypada na inny dzień niż poprzedni.
* Ulepszona obsługa skrzynki odbiorczej i innych list wiadomości w programie Microsoft Outlook 2010 i nowszych (#3834), w tym:
  * Możliwość wyciszenia nagłówków kolumn (od, temat itp.) przez wyłączenie opcji Nagłówki wierszy i kolumn tabeli raportu w ustawieniach formatowania dokumentu.
  * Możliwość korzystania z poleceń nawigacji po tabeli (control + alt + strzałki) w celu poruszania się po poszczególnych kolumnach.
...
* Ulepszona obsługa automatycznego odczytywania nagłówków kolumn i wierszy w programie Microsoft Excel (#3568), w tym:
  * Obsługa zdefiniowanych przez program Excel zakresów nazw w celu identyfikacji komórek nagłówka (kompatybilna z czytnikiem ekranu Jaws).
...
* Microsoft Excel: informacje zwrotne dotyczące mowy i alfabetu Braille'a dla wielu innych skrótów ruchu zaznaczania (#4211), w tym:
  * Ruch strony w pionie (pageUp i pageDown);
  * Ruch strony w poziomie (alt+pageUp i alt+pageDown);
...
* Microsoft PowerPoint: podczas przenoszenia kształtów wokół slajdu za pomocą strzałek jest teraz zgłaszana bieżąca lokalizacja kształtu (#4214), w tym:
  ...
  * Jeśli kształt zakrywa inny kształt lub jest przez niego pokryty, podawana jest odległość nałożona i nakładający się kształt.
...
* Polecenie lokalizacji raportu (NVDA+delete) jest w niektórych sytuacjach bardziej zależne od kontekstu. (#4219)
  * W standardowych polach edycyjnych i trybie przeglądania podawane jest procentowe położenie kursora w treści i jej współrzędnych ekranu.
  ...
  * Dwukrotne naciśnięcie tego polecenia spowoduje wcześniejsze zachowanie polegające na zgłaszaniu informacji o lokalizacji dla całego formantu.
...
* Możliwa jest teraz zmiana skrótu używanego do uruchamiania NVDA dla lokalizacji, w których normalny skrót powoduje problemy. (#2209)
  ...
  * Zauważ, że tekst opcji Utwórz skrót na pulpicie w oknie dialogowym Zainstaluj NVDA, jak również skrótu w Podręczniku użytkownika, również muszą zostać zaktualizowane.
...
* Ulepszona obsługa regionów ARIA Live w Firefoksie i innych aplikacjach Mozilla Gecko:
  * Obsługa aktualizacji aria-atomic i filtrowanie aktualizacji aria-busied. (#2640)
  * Tekst alternatywny (taki jak atrybut alt lub aria-label) jest dołączany, jeśli nie ma innego użytecznego tekstu. (#3329)
  * Aktualizacje regionu Live nie są już wyciszane, jeśli występują w tym samym czasie, co ruch fokusu. (#3777)
...
* Kilka poprawek w obsłudze dostępnych aplikacji Java:
  * Początkowo aktywna kontrolka w ramce lub oknie dialogowym nie jest już niezgłaszana, gdy klatka lub okno dialogowe znajdzie się na pierwszym planie. (#3753)
  ...
  * Podczas zgłaszania tekstu okien dialogowych uwzględniany jest teraz tekst, którego wcześniej brakowało. (#3757)
...
* Płaski podgląd został zastąpiony trybami recenzji obiektów, dokumentów i ekranu. (#2996)
  * Przegląd obiektu przegląda tekst tylko w obiekcie nawigatora, przegląd dokumentu przegląda cały tekst w dokumencie w trybie przeglądania (jeśli istnieje), a przegląd ekranu przegląda tekst na ekranie dla bieżącej aplikacji.
  ...
  * Po przełączeniu w tryb podglądu ekranu, NVDA pozostanie w tym trybie, dopóki jawnie nie przełączysz się z powrotem do trybu przeglądania dokumentów lub obiektów.
  * W trybie przeglądania dokumentu lub obiektu, NVDA może automatycznie przełączać się między tymi dwoma trybami w zależności od tego, czy poruszasz się po dokumencie w trybie przeglądania, czy nie.
...
* Obsługiwane jest teraz wprowadzanie brajlowskie z klawiatur brajlowskich. (#808)
  ...
  * Podklasy alfabetu Braille'a. BrailleDisplayGesture (zaimplementowany w sterownikach monitorów brajlowskich) może również dziedziczyć po BrailleInput.BrailleInputGesture. Dzięki temu polecenia wyświetlania i dane wejściowe brajla mogą być obsługiwane przez tę samą klasę gestów.
...
* NVDA może teraz obsługiwać wprowadzanie znaków azjatyckich przy użyciu IME i metod wprowadzania usługi tekstowej we wszystkich aplikacjach, w tym:
  * Raportowanie i nawigacja po listach kandydackich;
...
* Konteksty wiadomości Gettext są teraz obsługiwane. Pozwala to na zdefiniowanie wielu tłumaczeń dla jednej wiadomości w języku angielskim w zależności od kontekstu. (#1524)
  ...
  * xgettext i msgfmt z GNU gettext muszą być używane do tworzenia dowolnych plików PO i MO. Narzędzia języka Python nie obsługują kontekstów komunikatów.
...
* Znacznie więcej informacji jest teraz podawanych w dokumentach programu Microsoft Word, w tym:
  ...
  * Raportowanie podczas wprowadzania sekcji dokumentu, takich jak komentarze, artykuły, przypisy dolne i końcowe oraz artykuły nagłówka i stopki.
...
* Obiekty TextInfo mają teraz metodę activate() do aktywowania pozycji reprezentowanej przez obiekt TextInfo.
  * Jest to używane przez brajl do aktywacji pozycji za pomocą przywoływania kursora na monitorze brajlowskim. Jednak w przyszłości mogą pojawić się inni dzwoniący.
...
* NVDA może teraz instruować syntezatory mowy, aby przełączały języki dla określonych sekcji mowy.
  * Aby to umożliwić, kierowcy muszą obsługiwać mowę. LangChangeCommand w sekwencjach poprzedzających SynthDriver.speak().
  * SynthDriver powinny również dostarczać argument language do obiektów VoiceInfo (lub zastąpić atrybut language w celu pobrania bieżącego języka). W przeciwnym razie używany będzie język interfejsu użytkownika NVDA.
...
* Sterowniki syntezatora otrzymują teraz sekwencję poleceń tekstowych i głosowych do mówienia, a nie tylko tekst i indeks.
  * Pozwala to na osadzanie indeksów, zmian parametrów itp.
...
* Kolory mogą być teraz zgłaszane dla niektórych elementów sterujących. Automatyczne ogłaszanie można skonfigurować w oknie dialogowym Preferencje formatowania dokumentu. Można go również zgłaszać na żądanie za pomocą polecenia formatowania tekstu raportu (NVDA+f).
  * Początkowo jest to obsługiwane w standardowych edytowalnych formantach tekstowych IAccessible2 (takich jak w aplikacjach Mozilli), kontrolkach RichEdit (takich jak w Wordpadzie) i kontrolkach tekstowych IBM Lotus Symphony.
...
* Wszystkie dane wejściowe od użytkownika są teraz reprezentowane przez inputCore.InputGesture wystąpienie. (#601)
  * Każde źródło danych wejściowych podklasuje klasę bazową InputGesture.
...
* Sterowniki monitorów brajlowskich zostały ulepszone, aby umożliwić powiązanie przycisków, kółek i innych elementów sterujących ze skryptami NVDA:
  * Sterowniki mogą dostarczyć globalną mapę gestów wejściowych, aby dodać powiązania dla skryptów w dowolnym miejscu NVDA.
  * Mogą również udostępniać własne skrypty do wykonywania określonych funkcji wyświetlania.
  * Zobacz alfabet Braille'a. BrailleDisplayDriver w celu uzyskania szczegółowych informacji oraz przykłady istniejących sterowników monitorów brajlowskich.
...
* NVDA może teraz pobrać większość tekstu zapisanego na ekranie. (#40, #643)
  ...
  * Formanty dostępne za pomocą tej funkcji obejmują: niektóre elementy menu, które wyświetlają ikony (np. menu Otwórz za pomocą plików w systemie Windows XP) (#151), edytowalne pola tekstowe w aplikacjach Windows Live (#200), listę błędów w programie Outlook Express (#582), edytowalny formant tekstowy w programie TextPad (#605), listy w programie Eudora, wiele kontrolek w australijskim e-podatku oraz pasek formuły w programie Microsoft Excel.
...
* Wprowadzono polecenia dla płaskiego przeglądu: (#58)
  * NVDA+numpad7 przełącza się na płaski przegląd, umieszczając kursor recenzji w pozycji bieżącego obiektu, co pozwala na przejrzenie ekranu (lub dokumentu, jeśli się w nim znajduje) za pomocą poleceń przeglądu tekstu.
...
* Przestań ogłaszać niektóre bezużyteczne informacje, gdy zmieni się fokus, takie jak panele główne, panele warstwowe i panele przewijania w aplikacjach Java lub Lotus.
...
* CapsLock, Insert NumPad i Extended Insert mogą być używane jako modyfikujący NVDA. Ponadto, jeśli jeden z tych jest używany, dwukrotne naciśnięcie bez naciskania innych spowoduje wysłanie do systemu operacyjnego, tak jak to miało miejsce w przypadku naciśnięcia bez uruchomionego NVDA. Aby jeden z tych stał się modyfikującym NVDA, zaznacz jego pole wyboru w oknie dialogowym Ustawienia klawiatury (wcześniej nazywanym oknem dialogowym echa klawiatury).
...
* NVDA ma teraz wbudowany syntezator o nazwie eSpeak, opracowany przez Jonathana Duddington.It jest bardzo responsywny i lekki oraz obsługuje wiele różnych języków. Syntezatory Sapi mogą być nadal używane, ale eSpeak będzie używany domyślnie.
  * eSpeak nie wymaga zainstalowania żadnego specjalnego oprogramowania, więc może być używany z NVDA na dowolnym komputerze, na pendrive'ie USB lub w dowolnym miejscu.
...
* wirtualne są ładowane tylko wtedy, gdy fokus znajduje się na oknie, które ma zostać załadowane. Rozwiązuje to niektóre problemy z włączeniem okienka podglądu w programie Outlook Express.
* Dodano argumenty wiersza poleceń do NVDA:
  ...
  * -q, --quit: zamyka wszystkie już uruchomione instancje NVDA, a następnie kończy działanie
  * -s, --stderr-file nazwa_pliku: określa, gdzie NVDA powinna umieszczać nieprzechwycone błędy i wyjątki
  * -d, --debug-file nazwa_pliku: określa, gdzie NVDA powinna umieszczać komunikaty debugowania
