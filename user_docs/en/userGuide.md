...
* Part of word: The pattern must be preceded or followed by an alphanumeric character or underscore to match. This is useful for matching parts of compound words. For example, if you replace "bird" with "frog" using part of word matching, it would match "birds", "bluebird", "bluebirds" and "birdsong", but not the standalone word "bird" (since it has neiter no adjacent alphanumeric characters).
