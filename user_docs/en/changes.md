...
* Fixed missing MathML attributes for `mspace` in PDF documents. Also passed along attributes for elementary math elements. This allows math addons to potentially use those attributes for better speech and braille. These attributes are present when reading an HTML page, so it bring PDF back parity. (#18508)
