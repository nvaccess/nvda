...
+ Viene visualizzata la finestra di NVDA con il contratto di licenza.
Premere ``freccia giù`` per leggere il contratto di licenza, se lo si desidera.
+ Premere ``tab`` per passare alla casella di controllo "Accetto", quindi premere la ``barra spaziatrice`` per selezionarla.
+ Premere ``tab`` per spostarsi tra le opzioni, quindi premere ``invio`` sull'opzione desiderata.
+
...
+ Dal programma di installazione, assicurarsi che la casella di controllo per accettare la licenza sia selezionata.
+ Spostarsi con il tasto ``Tab`` e attivare il pulsante "Installa NVDA su questo computer".
+ Successivamente, ci sono le opzioni per utilizzare NVDA durante l'accesso a Windows e per creare un collegamento sul desktop.
...
+ Viene visualizzata una finestra di dialogo "Controllo account utente (UAC)" di Windows con una richiesta simile a questa: "Vuoi consentire a questa app di apportare modifiche al tuo PC?".
+ Premere ``alt+s`` per accettare.
+ Durante l'installazione di NVDA, apparirà una barra di avanzamento che si riempie sino al termine.
...
+ Viene visualizzata una finestra di dialogo per confermare che l'installazione di NVDA è andata a buon fine.
Il messaggio consiglia di "Premere OK per avviare la copia installata".
Premere ``invio`` per avviare la copia installata.
+ Viene visualizzata la finestra di dialogo "Benvenuto in NVDA" e lo screen reader ne leggerà il contenuto.
...
+ Premere ``tab`` per spostarsi su  "Utilizza ``Blocca maiuscole`` come tasto NVDA".
...
+ Utilizzare ``tab`` e ``barra spazio`` per regolare le altre opzioni su questa schermata.
Si tratta di stabilire se NVDA debba avviarsi automaticamente.
+ Premere ``invio`` per chiudere la finestra di dialogo con NVDA ora in esecuzione.
+
...
+++Esplorazione schermo (modalità in linea)+++
...
+ Spostare il cursore di controllo in un contenuto di tipo matematico.
...
+ Poi, utilizzare il comando seguente:
+
...
+ Spostare il cursore sul testo contenente la nota.
+ Accedere alle opzioni inerenti la selezione come descritto sopra.
+ Scegliere Modifica Nota.
+
...
==== Consenti a NV Access di raccogliere statistiche sull'utilizzo dello screen reader ====
...
==== Descrizione ritardata dei caratteri al movimento del cursore ====
...
%kc:setting
==== Apre la finestra di dialogo Seleziona sintetizzatore ====
...
%kc:setting
==== Apri la finestra di dialogo Seleziona display Braille ====
...
%kc:setting
==== Apri le impostazioni presentazione oggetti ====
...
%kc:setting
==== Apri impostazioni modalità navigazione ====
...
==== Attiva modalità navigazione al caricamento della pagina ====
...
%kc:setting
==== Apri Impostazioni formattazione documento ====
...
Si legga la documentazione della propria barra Braille per ottenere informazioni su dove sono situati i vari tasti.
+++ Assegnazione tasti per tutti i modelli +++
%kc:beginInclude
|| Nome | Tasto |
...
+++ Assegnazione tasti per Brailliant BI 32, BI 40 e B 80 +++ 
%kc:beginInclude
|| Nome | Tasto |
...
+++ Assegnazione tasti per Brailliant BI 14 +++ 
%kc:beginInclude
|| Nome | Tasto |
...
+ Assicurarsi di aver installato [il driver più recente https://www.papenmeier-rehatechnik.de/en/service/downloadcenter/software/articles/software-braille-devices.html].
+ Aprire gestione dispositivi di Windows.
+ Scorrere l'elenco fino a "Controller USB" o "Dispositivi USB".
+ Selezionare "Dispositivo USB Papenmeier Braillex".
+ Aprire le proprietà e passare alla scheda "Avanzate".
...
+ Disabilitare l'opzione "Carica VCP".
+
...
Se si vogliono trovare maggiori informazioni o richiedere assistenza riguardo NVDA, si prega di consultare il [Sito internazionale di NVDA in lingua inglese NVDA_URL], oppure quello relativo alla comunità italiana all'indirizzo www.nvda.it.
