...
| Nome |Tasto Desktop |Tasto Laptop |Descrizione|
|---|---|---|---|
|Legge i caratteri digitati |`NVDA+2` |`NVDA+2` |Stabilisce come NVDA debba leggere i caratteri che vengono scritti |
|Legge le parole digitate |`NVDA+3` |`NVDA+3` |Stabilisce come NVDA debba leggere le parole che vengono scritte. |
...
Premere Invio o la barra spaziatrice per attivare (cliccare) il testo alla posizione del cursore, se possibile.
Premere il tasto Esc per uscire dal documento riconosciuto.
Premere `NVDA+f5` per Aggiornare il risultato del riconoscimento.
...
NVDA fornisce funzionalità aggiuntive per alcune applicazioni, al fine di rendere più semplici determinati compiti o offrire l’accesso a funzioni che altrimenti non sarebbero accessibili agli utenti di screen reader.
...
| Nome |Tasto |Descrizione|
|---|---|---|
|Imposta intestazioni di colonna | `NVDA+Shift+C` | Premendo questo comando una volta, si indica a NVDA che la cella corrente è la prima della riga con le intestazioni di colonna. Le intestazioni verranno così annunciate automaticamente mentre ci si sposta tra le colonne sottostanti. Premendolo due volte, l’impostazione verrà annullata.|
|Imposta intestazioni di riga | `NVDA+Shift+R` | Premendo questo comando una volta, si indica a NVDA che la cella corrente è la prima della colonna con le intestazioni di riga. Le intestazioni verranno così annunciate automaticamente mentre ci si sposta tra le righe sottostanti. Premendolo due volte, l’impostazione verrà annullata.|
...
Ciò significa che gli utenti di altri screen reader che apriranno questo documento in un secondo momento avranno automaticamente le intestazioni di riga e colonne già impostate.
...
<!-- KC:beginInclude -->
Per leggere eventuali commenti alla posizione del cursore, premere "NVDA+alt+c".
La doppia pressione presenta le informazioni in una finestra in modalità navigazione.
<!-- KC:endInclude -->
Tutti i commenti presenti nel documento, insieme alle altre modifiche tracciate, possono essere elencati anche nell'elenco elementi di NVDA, selezionando come tipo “Annotazioni”
...
<!-- KC:endInclude -->
Queste impostazioni saranno memorizzate nel documento come segnalibri, in un formato compatibile con altri screen reader, ad esempio Jaws.
Ciò significa che gli utenti di altri screen reader che aprono questa cartella di lavoro in un secondo momento avranno automaticamente le intestazioni di riga e colonne già impostate.
...
<!-- KC:beginInclude -->
Per leggere eventuali note inerenti la cella focalizzata, premere "NVDA+alt+c".
La doppia pressione presenterà il contenuto in una finestra in modalità navigazione.
...
NVDA offre un supporto avanzato per Poedit 3.5 o versioni successive.
...
<!-- KC:settingsSection: || Nome | tasto Desktop | tasto Laptop| Descrizione | -->
...
Una casella di controllo che permette di stabilire se, alla chiusura di NVDA, debbano comparire opzioni aggiuntive per decidere come chiudere il programma.
Se attivata, comparirà una finestra all'uscita di NVDA, con le seguenti scelte:

* esci
* riavvia
* riavvia con gli add-on disattivati e il debug nei log attivo
* riavvia con il debug nei log attivo
* Installa aggiornamenti in sospeso (se presenti).
...
* Un ID univoco per l'utente corrente NVDA, che cambia una volta al mese
...
si può anche installare manualmente l’aggiornamento in sospeso dalla finestra di uscita di NVDA (se abilitata), dal menu di NVDA oppure eseguendo un nuovo controllo dal menu “Aiuto”.

##### Mirror di aggiornamento {#UpdateMirror}

Questi controlli consentono di specificare un URL alternativo da utilizzare per verificare gli aggiornamenti a NVDA.
Ciò può risultare utile in quelle situazioni in cui l'accesso al server NV Access che si occupa degli aggiornamenti di NVDA risulti lento o non disponibile.

La casella di testo di sola lettura mostra l'URL corrente del Mirror.
Se non viene usato alcun mirror (ad esempio si utilizza il server di aggiornamento di Nv Access), verrà visualizzato "Nessun mirror".

Se si desidera modificare il server mirror di aggiornamento, premere il pulsante "Cambia..." per aprire la [finestra Imposta Mirror di Aggiornamento](#SetURLDialog).

Si noti che quando si utilizza un mirror di aggiornamento, l'amministratore di quel server ha accesso a tutte le [informazioni inviate con i controlli di aggiornamento](#GeneralSettingsCheckForUpdates).
Contattare il gestore del mirror di aggiornamento per conoscere i dettagli sulle politiche di gestione dei dati, così da assicurarsi di essere a proprio agio con il modo in cui le informazioni verranno trattate prima di impostare un mirror di aggiornamento.

##### Impedisci allo schermo di disattivarsi durante la lettura continua o con il Braille. {#PreventDisplayTurningOff}

Questa casella di controllo, se selezionata, garantisce che lo schermo rimanga attivo durante la lettura continua o con il Braille (ad esempio, quando si premono i pulsanti di scorrimento).
Ciò evita la situazione in cui lo schermo si blocca inaspettatamente durante la lettura continua.
L'opzione è attivata di default.
Considerare la disattivazione di questa impostazione nel caso in cui si riscontrasse una durata della batteria più breve.
...
La prima opzione nella categoria Voce è il pulsante Cambia...
Questo bottone attiva la finestra [Seleziona Sintetizzatore](#SelectSynthesizer), che consente di selezionare il sintentizzatore vocale attivo.
...
L'opzione Voce consiste in una casella combinata che elenca tutte le voci installate del sintetizzatore corrente.
...
##### Normalizzazione Unicode {#SpeechUnicodeNormalization}

Quando questa opzione è abilitata, viene eseguita la normalizzazione Unicode sul testo pronunciato da NVDA.
Ciò è utile quando la sintesi vocale incontra caratteri che possono essere rappresentati in diverse forme.
NVDA utilizza l'algoritmo NFKC (Normalization Form Compatibility Composition), che offre, tra gli altri, i seguenti vantaggi:

1. Le versioni in grassetto e corsivo dei caratteri che fanno parte dello standard Unicode e sono comunemente utilizzate sui social media vengono normalizzate al loro equivalente compatibile più comune.
Ad esempio, la lettera latina "h" può anche essere presentata come"𝐡" (grassetto), "ℎ" (corsivo), etc. ma sarà sempre pronunciata come "h" quando la normalizzazione è abilitata.
Questo aspetto della normalizzazione aiuta anche a leggere le equazioni nell'editor di equazioni di Microsoft Word.

1. Normalizzazione per i caratteri composti.
Ad esempio, il carattere "ü" (u con umlaut/dieresi), un carattere comune in lingue come il tedesco e il turco può essere rappresentato in due forme.
  1. Un carattere Unicode autonomo (ü)
  1. Una scomposizione in due caratteri (ü), ossia la lettera latina U e una parte aggiuntva, dieresi
  La normalizzazione Unicode garantisce che venga utilizzata una sola forma per la pronuncia di questi caratteri.

1. Scomposizione di alcune legature, inclusa "ĳ" (legatura ij) nella loro forma di due lettere ("ij").

1. Ordine stabile dei modificatori nei caratteri compositi, ad esempio nell'ebraico antico.

Per attivare/disattivare la normalizzazione Unicode da qualsiasi posto, assegnare un gesto personalizzato utilizzando la [finestra gesti e tasti di immissione](#InputGestures).
...
##### Annuncia "Normalizzato durante la navigazione per caratteri {#SpeechReportNormalizedForCharacterNavigation}

Questa impostazione è una casella di controllo che, se selezionata, indica a NVDA di segnalare esplicitamente che un carattere viene normalizzato quando pronunciato come carattere individuale, ad esempio durante un controllo ortografico.
Ad esempio, quando questa opzione è abilitata, l'ortografia del carattere "ĳ" sarà resa come "i j normalizzato".

Si tenga presente che questa impostazione è disponibile solo quando l'opzione "[Normalizzazione unicode](#SpeechUnicodeNormalization)" risulta attiva.
...
Anche i sintetizzatori di Code Factory, sia il componente aggiuntivo che l'applicazione SAPI, non lo implementano correttamente provocando lo spelling indesiderato del testo (ad esempio nel menu o nelle finestre di dialogo di NVDA).
...
Se si seleziona "automatico", la tabella verrà impostata in base alla lingua corrente di NVDA.
...
In questo caso, il Braille non seguirà il focus e il cursore di sistema.
...
##### Indicatore di inizio paragrafo {#BrailleParagraphStartMarkers}

Se l'opzione "Lettura per paragrafi" è attiva, verrà mostrato un indicatore di inizio paragrafo in base alla propria selezione ad indicare l'inizio dello stesso.
Ciò può essere particolarmente utile nelle applicazioni utilizzate per leggere grandi porzioni di testo, come documenti strutturati o libri.
In tali documenti, sapere dove iniziano i paragrafi può essere utile per comprendere la struttura del contenuto o per impostare segnalibri o annotazioni in base alla posizione del paragrafo.

Le opzioni comprendono l'uso di due spazi come interruzione di paragrafo meno invasiva e il simbolo di paragrafo, pilcrow (¶), come interruzione più evidente.

| . {.hideHeaderRow} |.|
|---|---|
|Opzioni | Nessun indicatore di inizio paragrafo, Doppio spazio (  ), Pilcrow (¶)|
|Predefinito |Nessun indicatore di inizio paragrafo|
...
| . {.hideHeaderRow} |.|
|---|---|
|Opzioni |Predefinito (abilitato), abilitato, disabilitato|
|Predefinito |Abilitato|

##### Visualizzazione della formattazione {#BrailleFormattingDisplay}

Questa impostazione determina il modo in cui NVDA mostrerà la formattazione del testo in braille.
L'opzione ha effetto soltanto se NVDA è impostato a [Mostrare gli attributi carattere in braille](#DocumentFormattingFontAttributes).

| . {.hideHeaderRow} |.|
|---|---|
| Opzioni | Predefinito (Liblouis), Liblouis, Tags |
| Predefinito | Liblouis |

| Opzione | Comportamento |
|---|---|
| Liblouis | Utilizza la formattazione Braille nativa. Si tenga presente che l'opzione fornirà indicazioni solo sul testo in grassetto, corsivo e sottolineato e solo se la tabella braille selezionata supporta l'indicazione di questi attributi. |
| [Tags](#BrailleFormattingDisplayTags) | Utilizza tag che descrivono come e dove cambia la formattazione del testo. |

###### Tags {#BrailleFormattingDisplayTags}

Quando la "Visualizzazione della formattazione" è impostata su "Tag", verrà visualizzato un tag di formattazione nel caso in cui essa subisce delle modifiche.
Questi tag iniziano con ⣋ e terminano con ⣙.
Un tag di formattazione conterrà uno o più simboli che descrivono la formattazione del testo.
Sono definiti i seguenti simboli:

| Simbolo | Significato |
|---|---|
| ⠃ ("b") | Inizio grassetto |
| ⡃ ("b" con punto 7) | Fine grassetto |
| ⠊ ("i") | Inizio corsivo |
| ⡊ ("i" con punto 7) | Fine corsivo |
| ⠥ ("u") | Inizio sottolineato |
| ⡥ ("u" con punto 7) | Fine sottolineato |
| ⠎ ("s")| Inizio barrato |
| ⡎ ("s" con punto 7) | Fine barrato |

##### Leggi il carattere alla pressione di un cursor routing {#BrailleSpeakOnRouting}

Se questa opzione è abilitata, NVDA pronuncerà automaticamente il carattere in corrispondenza del cursore, alla pressione di un cursor routing.

Per attivare/disattivare questa opzione da qualsiasi luogo, assegnare un gesto personalizzato utilizzando la [Finestra tasti e gesti di immissione](#InputGestures).

##### Parla durante la navigazione per riga o paragrafo {#BrailleSpeakOnNavigating}

Abilitando questa opzione, NVDA leggerà le righe o i paragrafi raggiunti utilizzando i controlli di navigazione del display Braille.

Per attivare/disattivare questa opzione da qualsiasi luogo, si prega di assegnare un gesto personalizzato a "speakOnNavigatingByUnit" nella sezione "Braille" della [Finestra Tasti e Gesti](#InputGestures).

##### Evita di spezzare le parole quando possibile {#BrailleSettingsWordWrap}

Se questa opzione è abilitata, quando si incontra una parola troppo grande per essere contenuta nel display braille, essa non verrà spezzata.
Invece, ci sarà uno spazio vuoto alla fine del display.
Sarà sufficiente scorrere con la barra braille per leggere la parola nella riga nuova.
Questo a volte viene chiamato "a capo automatico".
Si tenga presente che se la parola è troppo grande per essere visualizzata anche da sola sul display, sarà necessario comunque dividerla.

Se questa opzione è disabilitata, verrà visualizzato il più possibile della parola, ma il resto verrà per forza tagliato.
Quando si scorre il display, sarà possibile leggere il resto della parola.

Abilitare questa opzione può consentire una lettura più fluida, ma generalmente richiede di scorrere maggiormente il display.

##### Normalizzazione Unicode {#BrailleUnicodeNormalization}

Quando questa opzione è abilitata, viene eseguita la normalizzazione Unicode sul testo in braille.
Ciò è utile quando ci si imbatte in caratteri braille non presenti in una particolare tabella braille e che hanno un’alternativa compatibile, come i caratteri in grassetto e corsivo comunemente usati sui social media.
Altri vantaggi della normalizzazione Unicode sono spiegati in maniera più dettagliata nella [sezione equivalente per le impostazioni della voce](#SpeechUnicodeNormalization).

Per attivare/disattivare la normalizzazione Unicode da qualsiasi luogo, assegnare un gesto personalizzato utilizzando la[Finestra tasti e gesti di immissione](#InputGestures).

| . {.hideHeaderRow} |.|
|---|---|
|Opzioni |Predefinito (Disabilitata), Abilitata, Disabilitata|
|Predefinito |Disabilitata|

##### Interrompi la voce durante lo scorrimento {#BrailleSettingsInterruptSpeech}

Questa impostazione determina se la voce debba essere interrotta quando si scorre indietro/avanti il display Braille.
I comandi di riga precedente/successiva interrompono sempre la voce.

La voce che parla di continuo potrebbe fungere da distrazione durante la lettura del Braille.
Per questo motivo è abilitata di default l'opzione che interrompe la voce durante lo scorrimento del display braille.

Disabilitando questa opzione è possibile ascoltare la sintesi vocale mentre si legge contemporaneamente in Braille.
...
* NVDA a sinistra e applicazioni in entrambi i canali: NVDA parlerà nel canale sinistro, mentre le applicazioni riprodurranno i suoni nei canali sinistro e destro.
...
* Disattivato: NVDA non annuncerà i caratteri digitati.
* Solo nei controlli editazione: NVDA annuncerà solo caratteri digitati nei controlli di modifica e in altre aree in cui è possibile digitare il testo.
* Sempre: NVDA annuncerà tutti i caratteri digitati.
...
* Disattivato: NVDA non leggerà le parole digitate.
* Solo nei controlli editazione: NVDA annuncerà solo parole digitate nei controlli di modifica e in altre aree in cui è possibile digitare il testo.
* Sempre: NVDA annuncerà tutte le parole digitate.
...
La funzione risulta attivata di default, tuttavia alcuni utenti potrebbero valutare di disattivarla, per esempio chi è solito scrivere in vietnamita con il software di scrittura UniKey, poiché provocherebbe errori nell'immissione dei caratteri.

##### Timeout per la pressione di più tasti {#MultiPressTimeout}

Alcuni comandi da tastiera di NVDA eseguono azioni diverse in base a quante volte lo stesso tasto viene premuto in rapida successione.
Un esempio di questo comportamento è il comando "Leggi il carattere sul quale è posizionato il cursore di controllo".
Questo comando legge il carattere se premuto una volta, una descrizione fonetica del carattere se premuto due volte, ed il valore numerico del carattere se premuto tre volte.
Questa opzione configura il timeout dopo il quale un'ulteriore pressione dello stesso tasto avvierà un nuovo gesto, anziché essere considerata come una successiva pressione del primo.
Tornando al comando di esempio, un timeout troppo breve farà sì che due pressioni facciano leggere a NVDA due volte il carattere corrente, anziché la descrizione fonetica.
Il timeout predefinito è 500 ms, ovvero mezzo secondo.
Aumentare questo timeout può essere particolarmente utile per le persone che utilizzano la funzione di tasti permanenti o che devono convivere con una disabilità fisica.
...
"Questa opzione, attiva per impostazione predefinita, consente di scegliere se tutti i caratteri suggeriti visibili debbano essere annunciati automaticamente quando viene visualizzato un elenco di caratteri suggeriti o quando si cambia pagina."
...
Tuttavia, per i metodi di input fonetici come il Chinese New Phonetic, potrebbe essere più utile disattivare questa opzione, poiché tutti i simboli suoneranno allo stesso modo e sarà necessario usare i tasti freccia per scorrere singolarmente gli elementi dell'elenco e ottenere maggiori informazioni dalle descrizioni dei caratteri per ciascun suggerimento.
...
"Per i metodi di input che permettono di cambiare la selezione tramite i tasti freccia (come il Chinese New Phonetic), questa opzione è necessaria. Tuttavia, con altri metodi di input, la digitazione potrebbe risultare più efficiente se l’opzione è disattivata."
...
Questa opzione consente di scegliere se NVDA debba segnalare o meno nuovi simboli man mano che appaiono nella stringa di composizione.
...
Abilitata per impostazione predefinita, questa opzione consente di scegliere se i gesti (come la pressione di tasti) che non producono un comando NVDA e che in generale non sono considerati tasti di comando debbano essere inibiti dal raggiungere il documento su cui è attualmente posizionato il focus.
...
* Carattere
  ...
  * Attributi carattere [(Disattivo, Voce, Braille, Voce e braille)](#DocumentFormattingFontAttributes)
  * Apici e pedici
  * Testo enfatizzato
...
* Elementi
  * Intestazioni
  * Collegamenti
  * Tipo di link (destinazione alla stessa pagina)
  * Grafici
  * elenchi
...
##### Attributi caratteri {#DocumentFormattingFontAttributes}

Questa opzione consente di impostare il modo in cui verranno gestiti determinati attributi del carattere, come grassetto, corsivo, sottolineato e barrato.
La casella combinata degli attributi del carattere ha quattro opzioni:

* Disattivo: NVDA non leggerà gli attributi dei caratteri.
* Voce: NVDA leggerà i cambiamenti degli attributi carattere.
* Braille: NVDA mostrerà gli attributi in braille.
È possibile configurare esattamente il modo in cui vengono visualizzati nelle [Impostazioni Braille di NVDA](#BrailleFormattingDisplay).
* Voce e braille: NVDA riporterà gli attributi dei caratteri utilizzando entrambi i metodi sopra indicati.
...
* Voce: Se è selezionata l’opzione “Voce”, quando cambia il livello di indentazione, NVDA dirà qualcosa come “dodici spazi” oppure “quattro tabulazioni”.
* Beep: Se è selezionata l’opzione “Beep", quando cambia il livello di indentazione, dei segnali acustici indicheranno l’entità della variazione.**
...
#### Impostazioni Add-on Store {#AddonStoreSettings}

Questa categoria consente di regolare il comportamento dello Store dei componenti aggiuntivi.

##### aggiornamenti automatici {#AutomaticAddonUpdates}

Quando questa opzione è impostata su "Notifica", si verrà avvisati dopo l'avvio di NVDA se sono disponibili aggiornamenti dei componenti aggiuntivi.
Questo controllo viene eseguito ogni 24 ore.
Per impostazione predefinita, le notifiche saranno valide solo per i componenti aggiuntivi con aggiornamenti disponibili all'interno dello stesso [canale](#AddonStoreFilterChannel) (ad esempio stabile, beta or dev).
È possibile configurare i canali di aggiornamento dei componenti aggiuntivi [individualmente per ciascun add-on](#AddonStoreUpdateChannel) o per [tutti gli add-on](#DefaultAddonUpdateChannel).

Quando impostato su "Aggiorna automaticamente",gli add-on si aggiorneranno automaticamente in background.
Verrà richiesto di riavviare NVDA al termine degli aggiornamenti.

| . {.hideHeaderRow} |.|
|---|---|
|Opzioni |Notifica (Default), Aggiorna Automaticamente, Disattivati |
|predefinito |notifica |

|Opzione |comportamento |
|---|---|
|Notifica |Notifica quando ci sono aggiornamenti disponibili per i componenti aggiuntivi |
|Aggiorna Automaticamente |Aggiorna automaticamente i componenti aggiuntivi in background |
|Disattivata |Non verificare automaticamente la presenza di aggiornamenti per i componenti aggiuntivi |

##### Canale di aggiornamento predefinito {#DefaultAddonUpdateChannel}

Quando [Gli aggiornamenti automatici degli add-on](#AutomaticAddonUpdates) sono abilitati, per impostazione predefinita, i componenti aggiuntivi si aggiornano solo per lo stesso [canale](#AddonStoreFilterChannel).
Ad esempio, una versione beta installata si aggiornerà solo ad una versione beta più recente.
Questa opzione imposta il canale di aggiornamento predefinito per tutti i componenti aggiuntivi.
è anche possibile modificare il canale di aggiornamento per uno [specifico componente aggiuntivo individualmente dall'Add-on Store](#AddonStoreUpdateChannel).

| . {.hideHeaderRow} |.|
|---|---|
| Opzioni | Lo stesso (Predefinito), Qualsiasi, Non aggiornare, Stabile, Beta o dev, Beta, Dev |
| Predefinito | Lo stesso |

| Opzione | Comportamento |
|---|---|
| Lo stesso | Gli add-on rimarranno nel loro canale |
| Qualsiasi | I componenti aggiuntivi si aggiorneranno automaticamente all'ultima versione, indipendentemente dal canale |
| Non aggiornare | I componenti aggiuntivi non si aggiorneranno automaticamente per impostazione predefinita, è necessario abilitarli individualmente |
| Stabile | I componenti aggiuntivi si aggiorneranno automaticamente alle versioni stabili |
| Beta o dev | I componenti aggiuntivi si aggiorneranno automaticamente alle versioni beta o dev |
| Beta | I componenti aggiuntivi si aggiorneranno automaticamente alle versioni beta |
| Dev | I componenti aggiuntivi si aggiorneranno automaticamente alle versioni Dev |

##### Consenti agli aggiornamenti automatici di installare add-on incompatibili {#AllowIncompatibleAddonUpdates}

Questa impostazione consente aggiornamenti automatici a componenti aggiuntivi che potrebbero non essere completamente compatibili con la versione corrente di NVDA.
Per impostazione predefinita, la funzione è disabilitata, il che significa che gli aggiornamenti automatici opereranno solo sulle versioni contrassegnate come compatibili con la versione corrente di NVDA.
Gli aggiornamenti automatici aggiorneranno comunque una versione incompatibile di un componente aggiuntivo a una versione compatibile non appena sarà disponibile.
Abilitare questa opzione può essere utile per passare alle versioni che interrompono la compatibilità con i componenti aggiuntivi (la prima versione dell’anno).
Questo è particolarmente utile per i tester delle versioni alpha e beta, che verificano la compatibilità dei componenti aggiuntivi nelle fasi iniziali di una versione che interrompe la compatibilità.

##### Server mirror {#AddonStoreMetadataMirror}

Questi controlli consentono di specificare un URL alternativo per scaricare i dati dei componenti aggiuntivi
Questo può essere utile in luoghi in cui l’accesso al server dello Store dei componenti aggiuntivi di NV Access è lento o non disponibile.

La casella di testo di sola lettura mostra l'URL del mirror corrente.
Se non risulta in uso alcun mirrror (ad esempio si sta utilizzando il server di NVAccess per gli add-on, verrà mostrata la dicitura "Nessun mirror".

Se si desidera modificare il mirrrror per l'add-on store, premere il pulsante "Cambia..." per aprire la [Finestra Imposta il server mirror per gli add-on](#SetURLDialog).

#### Impostazioni accesso remoto {#RemoteSettings}

Questa categoria consente di configurare il comportamento dell'[Accesso Remoto](#RemoteAccess).

##### Abilita l'Accesso Remoto {#RemoteEnable}

Utilizzare questa casella di controllo per attivare o disattivare la funzionalità di Accesso Remoto di NVDA.

Se la casella è non selezionata, la funzionalità non sarà disponibile.

Nota: per poter continuare una sessione di Accesso Remoto nelle schermate protette, come i prompt del Controllo dell’Account Utente, è necessario abilitare questa opzione, salvare le modifiche e poi [copiare le impostazioni nella directory di configurazione di sistema di NVDA](#GeneralSettingsCopySettings).
L’Accesso Remoto continuerà solo le sessioni avviate dalla copia installata di NVDA nelle schermate protette.
Questo significa, ad esempio, che non è possibile avviare una sessione di Accesso Remoto con una copia portable di NVDA e farla proseguire nelle schermate protette.

Le seguenti opzioni sono disponibili solo se l'accesso remoto è abilitato.

##### Chiedi conferma prima di disconnetterti se il pc è controllato {#RemoteConfirmDisconnect}

Questa opzione determina se è necessaria una conferma prima di disconnettersi da una sessione di Accesso Remoto quando si è collegati come computer controllato.

Se controllato, NVDA chiederà conferma prima di disconnettersi.
Altrimenti, NVDA si disconnetterà immediatamente senza richiedere alcuna conferma.

##### Connetti automaticamente dopo l'avvio di NVDA {#RemoteAutoconnect}

Questa opzione consente di stabilire automaticamente una sessione di accesso remoto all'avvio di NVDA.
Ciò potrebbe essere utile, ad esempio, per controllare il computer di casa quando ci si trova lontano.

Usare cautela quando si abilita questa opzione, in quanto può aumentare il rischio di accesso non autorizzato al proprio computer.

Per ulteriori dettagli sulle seguenti opzioni, consultare la [sezione impostare una connessione di accesso remoto](#RemoteAccessSetup)

##### Modalità {#RemoteAutoconnectMode}

Selezionare la modalità di connessione per connessioni automatiche.

Questa opzione è disponibile soltanto quando [Connetti automaticamente dopo l'avvio di NVDA](#RemoteAutoconnect) è attivata.

|Opzione |Comportamento |
|---|---|
|Consenti il controllo di questo computer |Utilizza questo computer come “controllato”. In questo modo, l’operatore del computer “controllante” potrà utilizzarlo a distanza come se fosse fisicamente davanti allo schermo. |
|Controlla un altro computer |Usa questo pc come "computer controllato". Ciò consente di eseguire comandi sul pc remoto. |

##### Server {#RemoteAutoconnectServer}

Selezionare il tipo di server per connessioni automatiche.

Questa opzione è disponibile soltanto quando [Connetti automaticamente dopo l'avvio di NVDA](#RemoteAutoconnect) è attivata.

|Opzione |Comportamento |
|---|---|
|Usa esistente |Utilizza un server di Accesso Remoto esistente per gestire la connessione (consigliato). |
|Ospita in locale |Usare questa istanza di NVDA come server di Accesso Remoto (avanzato). Questa opzione potrebbe richiedere una configurazione di rete non trattata in questo manuale. |

##### Host {#RemoteAutoconnectHost}

Usare questo campo per impostare l’URL del server di Accesso Remoto che si desidera utilizzare per le connessioni automatiche.Use this field to set the URL of the Remote Access server you would like to use for automatic connections.
Se il server utilizza una porta diversa da 6837, inserirla dopo i due punti (esempio `example.com:1234`).

Questa opzione è disponibile soltanto quando [Connetti automaticamente dopo l'avvio di NVDA](#RemoteAutoconnect) è attivata, e l'opzione [Server](#RemoteAutoconnectServer) è impostata su "Usa esistente".

##### Porta {#RemoteAutoconnectPort}

Usare questo campo per impostare la porta TCP che si desidera utilizzare per connessioni automatiche.

Questa opzione è disponibile soltanto quando [Connetti automaticamente dopo l'avvio di NVDA](#RemoteAutoconnect) è attivata, e l'opzione [Server](#RemoteAutoconnectServer) è impostata su "Usa esistente".

##### Chiave {#RemoteAutoconnectKey}

Usare questo campo per impostare la chiave (password) che si desidera utilizzare per connessioni automatiche.

Questa opzione è disponibile soltanto quando [Connetti automaticamente dopo l'avvio di NVDA](#RemoteAutoconnect) è attivata, e l'opzione [Server](#RemoteAutoconnectServer) è impostata su "Usa esistente".

##### Elimina tutte le impronte digitali attendibili. {#RemoteDeleteFingerprints}

Questo pulsante consente di dimenticare le impronte digitali di tutti i server di Accesso Remoto.
Questo significa che verrà nuovamente chiesto se si desidera connettersi a tutti i server di Accesso Remoto non riconosciuti, anche a quelli a cui ci si è già connesso in passato.
Verrà richiesta conferma prima che tutte le impronte digitali attendibili vengano eliminate.

Questa opzione è disponibile solo se nella configurazione sono memorizzate impronte digitali attendibili.
...
#### Finestra impostazione mirror {#SetURLDialog}

Questa finestra consente di specificare l'URL del mirror da utilizzare quando [si aggiorna NVDA](#GeneralSettingsCheckForUpdates) o [si utilizza l'Add-on Store](#AddonsManager).
Ciò può essere utile in luoghi in cui l'accesso ai server di NVAccess per queste funzioni risulta lento o non disponibile.

* Quando si imposta il [Mirror di aggiornamento per NVDA](#UpdateMirror), il titolo di questa finestra diventerà "Imposta mirror per gli aggiornamenti di NVDA".
* Se invece si imposta il [mirror per l'Add-on Store](#AddonStoreMetadataMirror), il titolo della finestra sarà "Imposta il server mirrror per l'Add-on Store".

##### URL {#SetURLTextbox}

Inserire l'URL (indirizzo web) del mirror che si desidera utilizzare.
Sono supportati solo indirizzi di tipo HTTP e HTTPS.
Per la tua privacy, NV Access consiglia di utilizzare gli URL HTTPS quando possibile.

Lasciare il campo vuoto per utilizzare il server NV Access predefinito.

##### Test... {#SetURLTest}

Premere questo pulsante per testare l'indirizzo del mirror inserito.
Bisogna essere connessi a Internet affinché il test abbia successo.
Si consiglia di testare sempre l'URL prima di salvarlo.
...
* Solo quando necessario: laddove il modello a oggetti Microsoft Word non risulta disponibile in alcun modo
...
##### Rimuovi silenzi iniziali della sintesi {#TrimLeadingSilenceSpeech}

Se attivato, NVDA rimuoverà eventuali silenzi da quando la sintesi vocale inizia a parlare, il che può portare ad un aumento delle prestazioni per alcuni sintetizzatori.
Questa opzione è abilitata per impostazione predefinita, dovrebbe influenzare il silenzio presente solo all'inizio del parlato.
Se si nota che mancano anche alcune pause necessarie (ad esempio tra due frasi) quando si usa un sintetizzatore vocale proveniente da un add-on, si può disattivare completamente questa funzione per risolvere il problema.

##### Utilizza WASAPI per l'uscita audio Sapi4 {#UseWASAPIForSAPI4}

Questa opzione consente alle voci Microsoft Speech API versione 4 (SAPI 4) di riprodurre l’audio tramite l’API Windows Audio Session (WASAPI).
Questo consente alle voci SAPI 4 di sfruttare funzionalità aggiuntive come la riduzione automatica del volume degli altri suoni (audio ducking), l’eliminazione del silenzio iniziale e il mantenimento attivo del dispositivo audio.
Tuttavia, alcune voci SAPI 4 potrebbero non funzionare con l'attuale implementazione WASAPI.
Se ci si rende conto che la voce SAPI 4 in utilizzo smette di funzionare, è possibile disabilitare questa opzione.

| . {.hideHeaderRow} |.|
|---|---|
|Ozioni |Predefinito (Attivato), Disattivato, Attivato|
|Predefinito |Attivato|
...
Nel campo di sostituzione, i normali backslash devono quindi essere raddoppiati; ad esempio, per ottenere la sostituzione "a\\b", è necessario digitare "a\\\\b".
...
| Nome |Tasto Desktop |Tasto Laptop |Descrizione|
|---|---|---|---|
|Salva configurazione |`NVDA+control+c` |`NVDA+control+c` |Salva la configurazione attuale in modo che non vada persa all'uscita di NVDA|
|Ripristina configurazione |`NVDA+control+r` |`NVDA+control+r` |Con una pressione, la configurazione verrà ripristinata all’ultimo salvataggio. Con la tripla pressione, verranno ripristinate le impostazioni predefinite di fabbrica.|
...
## Accesso Remoto {#RemoteAccess}

Con la funzione integrata di Accesso Remoto di NVDA, è possibile controllare un altro computer su cui è in esecuzione lo screen reader oppure permettere a qualcuno di controllare il proprio.
Questo rende semplice offrire o ricevere assistenza, collaborare o accedere da remoto al proprio computer.

L'accesso remoto è disabilitato da impostazioni predefinite per la propria sicurezza.
È possibile attivare l'Accesso Remoto [nelle impostazioni di Accesso Remoto](#RemoteSettings).

### Per cominciare {#RemoteAccessGettingStarted}

Prima di iniziare, assicurarsi che NVDA sia installato ed in esecuzione su entrambi i computer.
Inoltre accertarsi che la casella di controllo Attiva Accesso Remoto [nelle impostazioni di Accesso Remoto](#RemoteSettings) sia selezionata in entrambi i computer.
La funzione di accesso remoto sarà quindi disponibile dal menu Strumenti in NVDA; Non sono necessari download o installazioni aggiuntive.

Se NVDA è in esecuzione in [Modalità Protetta](#SecureMode), Non è possibile avviare manualmente una nuova sessione di accesso remoto.
Tuttavia, NVDA avvierà comunque automaticamente una nuova sessione di Accesso Remoto se è configurato per farlo tramite [l’impostazione Connetti automaticamente all’avvio di NVDA.](#RemoteAutoconnect)
È comunque possibile disconnettersi manualmente in modalità protetta.

### Impostazione di una sessione di accesso remoto {#RemoteAccessSetup}

Le sessioni di accesso remoto sono configurate tramite la finestra di dialogo Connessione di accesso remoto.
Aprire il menu NVDA e selezionare strumenti, quindi "Accesso remoto", poi "Connetti ...".
O da qualsiasi punto Premere `nvda+alt+r`.

#### Selezione della modalità (controllare o essere controllati) {#RemoteAccessMode}

È necessario stabilire quale computer sarà controllato (cioè quello accessibile da remoto) e quale fungerà da postazione di controllo (cioè quello che gestirà la sessione).

La casella combinata "Modalità" consente di scegliere se questo computer sarà controllato (un’altra persona interagirà con il tuo computer) oppure funga da postazione di controllo (interagirai tu con il computer di un’altra persona).

* Se si necessita che un'altra persona possa aiutarci con il nostro computer, scegliere "Consenti il controllo di questo computer".
* Se invece si ha intenzione di aiutare qualcuno con il proprio computer, scegliere "Controlla un altro computer".

#### Connessione ad un server esistente (Host e chiave forniti) {#RemoteAccessJoin}

In questo caso, l’altra persona ha avviato la connessione tramite un server di intermediazione oppure la sta ospitando localmente.

1. Per la voce Server, selezionare "Usa esistente".
1. Nel campo Host, Inserire l'indirizzo (e la porta se è stata fornita).
  * È possibile includere la porta su cui connettersi aggiungendo il simbolo dei duepunti (":") e il numero di porta per l'indirizzo host.
Ad esempio, `esempio.com:1234`.
1. Nel campo chiave, immettere la chiave fornita.
Fare attenzione a digitare la chiave esattamente come indicato.
1. Premere OK.

#### Avviare una Connessione tramite un Server di Intermediazione {#RemoteAccessConnectRelay}

Se si desidera consentire a un’altra persona di connettersi tramite Internet, l’opzione più semplice è l’uso di un server di intermediazione. Per procedere:

1. Per la voce Server, Selezionare "Usa esistente".
1. Per il campo host, fornire il nome di un server di intermediazione, come `nvdaremote.com`.
Si dovrà fornire questo indirizzo all'altra persona.
  * È possibile includere la porta su cui connettersi aggiungendo il simbolo dei duepunti (":") e il numero di porta per l'host.
Ad esempio, `esempio.com:1234`.
1. Per la chiave: inventarne una che sia lunga e difficile da indovinare, o premere il pulsante "Genera Chiave" in modo da crearne una automaticamente.
Si dovrà fornire la password all'altra persona.
  * Si noti che la chiave funge sia da identificatore della sessione sia da password, quindi è molto importante che sia difficile da indovinare.
1. Premere OK.
1. Fornire all’altra persona i dettagli della connessione (come host e chiave) affinché possa connettersi.

#### Utilizzare NVDA come server {#RemoteAccessConnectSelfHosted}

Se non si desidera utilizzare un server di intermediazione oppure ci si sta connettendo ad una rete locale:

1. Per la voce Server, selezionare "Ospita in locale".
1. Se ci si connette tramite Internet, è possibile premere il pulsante “Ottieni IP esterno” per visualizzare il proprio indirizzo IP visibile su Internet.
Si dovrà fornire l'indirizzo IP esterno all'altra persona.
1. Se ci si connette tramite una rete locale, sarà necessario fornire all’altra persona il proprio indirizzo IP locale.
L'individuazione di questo parametro non è trattato in questa documentazione.
1. Per la porta, immettere la porta che si desidera utilizzare per le connessioni remote in arrivo.
Anche l'altra persona ne avrà bisogno.
  * Questa dovrebbe essere una porta che non viene utilizzata da altri servizi.
  * Per impostazione predefinita, l'accesso remoto utilizza la porta 6837.
  * Se ci si connette tramite Internet, potrebbe essere necessario inoltrare la porta.
  L'inoltro/forwarding delle porte va oltre lo scopo di questa documentazione.
  Potrebbe essere necessario contattare l'amministratore di rete per farlo.
1. Per la chiave: inventarne una che sia lunga e difficile da indovinare, o premere il pulsante "Genera Chiave" in modo da crearne una automaticamente.
Si dovrà fornire la password all'altra persona.
...
### Utilizzare l'Accesso Remoto {#RemoteAccessUsage}

Una volta attivata una sessione di Accesso Remoto, è possibile passare dal controllo del computer remoto al proprio, condividere gli appunti e disattivare l’audio della sessione remota.

* Premere NVDA+Alt+Tab per passare dal controllo del computer remoto al proprio e viceversa.
* Invia il testo dagli appunti all'altro computer aprendo il menu NVDA, quindi selezionando strumenti, poi "accesso remoto", quindi "invia gli appunti".
* Se si è connessi come postazione di controllo, è possibile disattivare l’audio del sintetizzatore vocale del computer remoto sul proprio computer aprendo il menu di NVDA, selezionando Strumenti, poi “Accesso Remoto” e infine “Silenzia dispositivo remoto”.
* Se si è connessi come postazione di controllo, è possibile inviare Control+Alt+Canc al computer controllato aprendo il menu di NVDA, selezionando Strumenti, poi “Accesso Remoto” e infine “Invia Control+Alt+Canc”.
  Affinché ciò funzioni, il computer controllato deve eseguire una copia installata di NVDA.
  Inoltre, a seconda delle impostazioni del computer controllato, questa funzione potrebbe non funzionare sempre.

### Riepilogo dei comandi da tastiera per l’Accesso Remoto {#RemoteAccessGestures}

<!-- KC:beginInclude -->
| Nome |Tasto |Descrizione|
|---|---|---|
| Attiva/disattiva connessione remota | `NVDA+alt+r` | Inizia una nuova Sessione di Accesso Remoto o, Se una sessione è già in corso, si disconnette. |
| Passa tra remoto e locale | NVDA+Alt+Tab | Alterna il controllo tra il computer remoto e quello locale. |
| Connetti | Nessuno | Avvia una nuova sessione di accesso remoto. Non disponibile in [modalità protetta](#SecureMode). |
| Copia link | Nessuno | Copia un collegamento alla sessione remota negli appunti. |
| Disconnetti | Nessuno | Termina la sessione di Accesso Remoto esistente. |
| Silenzia Dispositivo Remoto | Nessuno | Attiva o disattiva l’audio del sintetizzatore vocale del computer remoto. |
| Invia appunti | Nessuno | Invia il contenuto degli appunti al computer remoto. |
<!-- KC:endInclude -->
...
#### Ordinare l'elenco degli add-on per colonna {#AddonStoreSortByColumn}

Per impostazione predefinita, l'elenco dei componenti aggiuntivi viene ordinato per nome.
La casella combinata “Ordina per colonna” consente di ordinare l’elenco in base alle colonne disponibili per ciascuna scheda.
Ad esempio, si potrebbe voler ordinare i componenti aggiuntivi per editore, versione disponibile, ecc.
Gli add-on possono essere elencati in ordine crescente o discendente.
...
Se il download o l’installazione falliscono, è possibile riprovare l’installazione.
È anche possibile annullare l'installazione prima di uscire dall'add-on store.
...
Per impostazione predefinita, dopo l'avvio di NVDA, si riceverà una notifica nel caso della presenza di aggiornamenti sugli add-on installati.
Per saperne di più e configurare questo comportamento, consultare ["Notifiche sugli aggiornamenti"](#AutomaticAddonUpdates).
...
È possibile disattivare un add-on se risulta “attivo”, oppure attivarlo se risulta “disattivato”.
...
#### Modifica del canale di aggiornamento automatico {#AddonStoreUpdateChannel}

È possibile gestire i canali di aggiornamento automatici per i componenti aggiuntivi dalle [schede add-on installabili e aggiornabili](#AddonStoreFilterStatus).
Quando [gli aggiornamenti automatici per gli add-on](#AutomaticAddonUpdates) sono attivi, i componenti aggiuntivi si aggiorneranno allo stesso [canale](#AddonStoreFilterChannel) dal quale erano stati installati per [impostazioni predefinite](#DefaultAddonUpdateChannel).
Dal menu Azioni di un componente aggiuntivo, tramite il sottomenu “Canale di aggiornamento”, è possibile modificare i canali dai cui il componente aggiuntivo si aggiornerà automaticamente.

| Opzione | Comportamento |
|---|---|
| Predefinito | L'add-on seguirà le impostazioni del [canale di aggiornamento di default](#DefaultAddonUpdateChannel) |
| Lo Stesso | Il componente aggiuntivo rimarrà sullo stesso canale |
| Qualsiasi | Il componente aggiuntivo si aggiornerà automaticamente all'ultima versione, indipendentemente dal canale |
| Non aggiornare | Il componente aggiuntivo non si aggiornerà automaticamente |
| Stabile | Il componente aggiuntivo si aggiornerà automaticamente alle versioni stabili |
| Beta o dev | Il componente aggiuntivo si aggiornerà automaticamente alle versioni beta o dev |
| Beta | Il componente aggiuntivo si aggiornerà automaticamente alle versioni beta |
| Dev | Il componente aggiuntivo si aggiornerà automaticamente alle versioni Dev |
...
Verrà visualizzata una finestra di dialogo che consente di creare una copia portable di NVDA a partire dalla versione in esecuzione.

Seguire le istruzioni in [Creazione di una copia portable](#CreatingAPortableCopy) per ulteriori informazioni.

### Esegui Utility risoluzione registrazioni Com di sistema... {#RunCOMRegistrationFixingTool}

A volte possono verificarsi problemi con il Registro di sistema di Windows, che comportano un comportamento anomalo di NVDA.
Ciò può essere causato, ad esempio, dall’installazione o disinstallazione di alcuni programmi (come Adobe Reader o MathPlayer), da aggiornamenti di Windows o da altri eventi.
Lo strumento di risoluzione registrazioni COM tenta di risolvere questi problemi riparando le voci relative all’accessibilità nel Registro di sistema.

I tipi di problema che questo strumento può risolvere includono:

* NVDA inizia ad annunciare “sconosciuto” o “pannello” durante la navigazione in browser come Firefox o Edge, programmi di posta come Thunderbird, Esplora file di Windows, la barra delle applicazioni e altri programmi.
* NVDA non riesce più a passare tra le modalità focus e navigazione.
* Pulsanti che in precedenza venivano identificati correttamente con il loro nome, improvvisamente sono letti solo come "pulsante".
* Una lentezza innaturale di NVDA quando si utilizza la modalità navigazione nei browser.

Poiché questo strumento corregge le voci nel registro di Windows, richiede i diritti amministrativi per funzionare, proprio come quando si installa un programma.
Se il Controllo dell’Account Utente (UAC) è attivato, come avviene nella maggior parte dei casi, sarà necessario seguire i prompt visualizzati da UAC per eseguire correttamente lo strumento.
...
Per un elenco ancora più ampio di sintesi vocali gratuite e commerciali che è possibile acquistare e scaricare per l’uso con NVDA, consultare la [pagina delle voci aggiuntive](https://github.com/nvaccess/nvda/wiki/ExtraVoices).
...
* [Microsoft Speech Platform - Runtime (Versione 11), x86](https://www.microsoft.com/download/en/details.aspx?id=27225)
* [Microsoft Speech Platform - Runtime Languages (Versione 11)](https://www.microsoft.com/download/en/details.aspx?id=27224)
...
Le serie Brailliant BI e B della [HumanWare](https://www.humanware.com/), compresi BI 14, BI 32, BI 20X, BI 40, BI 40X e B 80, sono supportate se connessi tramite USB o Bluetooth.
...
* NLS eReader
  * Si noti che Zoomax non è supportato senza driver esterni

Seguono i tasti che sono stati assegnati con NVDA per i display Brailliant BI/B e BrailleNote touch.
Si legga la documentazione della propria barra Braille per ottenere informazioni su dove sono situati i vari tasti.
...
Per ciò che concerne il BrailleNote Touch, fare riferimento alla sezione [Serie Brailliant BI / BrailleNote Touch](#HumanWareBrailliant).
...
|Tasto FrecciaSinistra |spazio+punto3, joystick sinistro o dpad left|
...
Il Dot Pad di Dot Inc è un dispositivo in grado di mostrare sia braille standard che immagini.
NVDA può mostrare il braille in due modalità: su una singola linea, utilizzando la riga braille dedicata del dispositivo, oppure su più righe, sfruttando l’area della grafica tattile.
Il modello A300 dispone di un’area grafica tattile composta da 120 per 80 punti, in grado di visualizzare fino a 8 righe da 20 celle ciascuna.

È possibile stabilire se NVDA debba visualizzare il braille sulla riga braille dedicata o sull’area grafica tattile tramite l’opzione “Destinazione del braille” nelle impostazioni Braille di NVDA per questo driver.

I tasti di scorrimento sono supportati, ma, a causa del numero limitato di pulsanti sul dispositivo, al momento non sono disponibili altri comandi né la funzione di curosr routing.

Quando si seleziona il driver del dot pad in NVDA, è necessario configurare manualmente la porta seriale virtuale USB / Bluetooth a cui è collegato il dispositivo.
Questo driver non supporta il rilevamento automatico.

Si segnala che, a causa di limitazioni hardware, il Dot Pad potrebbe non aggiornare correttamente tutti i punti mentre la mano è posata sul dispositivo.
Assicurati di sollevare la mano completamente dal dispositivo durante la navigazione con NVDA e ricominciare a leggere solo quando l'aggiornamento è stato completato.

<!-- KC:beginInclude -->

| Nome |Tasto|
|---|---|
|Scorre braille indietro | `pan_left` |
|Scorre braille avanti | `pan_right` |
...
* L'apertura di documenti esterni dal menu NVDA, come la Guida dell'utente.
...
|`-n LANGUAGE` |`--lang=LANGUAGE` |Sovrascrive la lingua configurata in NVDA. Impostare su "Windows" Per il valore predefinito dell'utente corrente, "en" for Inglese, etc.|
...
|`-d` |`--disable-addons` |I componenti aggiuntivi non avranno effetto|
|Nessuno |`--debug-logging` |Abilita il debug nei log solo per la presente istanza. Questa impostazione ha la precedenza su qualsiasi altro livello di log ( `--loglevel`, `-l`), compresa la funzione no logging|
|Nessuno |`--no-logging` |Disabilita completamente le funzioni di log durante l'uso di NVDA. Questa impostazione può essere sovrascritta se viene specificato un livello di log ( `--loglevel`, `-l`) dalla riga di comando o se la funzione debug logging è attiva.|
|Nessuno |`--no-sr-flag` |Non modifica il flag Screen Reader per il sistema|
|Nessuno |`--install` |Installa NVDA (avviando la copia appena installata)|
|Nessuno |`--install-silent` | Installa NVDA (non avviando la copia appena installata)|
|Nessuno |`--enable-start-on-logon=True|False` |Durante l'installazione, abilita [l'avvio di NVDA alla schermata di logon](#StartAtWindowsLogon)|
|Nessuno |`--copy-portable-config` |Durante l'installazione, copia la configurazione portable dal percorso specificato (`--config-path`, `-c`) all'account utente corrente|
|Nessuno |`--create-portable` |Crea una copia portable di NVDA (avviando la copia appena creata). Richiede `--portable-path` da specificare|
|Nessuno |`--create-portable-silent` |Crea una copia portable di NVDA (senza avviare la nuova copia). Richiede che venga specificato `--portable-path`. Questa opzione sopprime gli avvisi durante la scrittura in directory non vuote e può sovrascrivere file senza preavviso.|
|None |`--portable-path=PORTABLEPATH` |Il percorso in cui verrà creata la copia portable di NVDA|

Allo stesso modo in cui è possibile installare silenziosamente NVDA passando il parametro `--install-silent`, può essere disinstallato senza ricevere output tramite l'opzione a riga di comando `/S` al disinstallatore.

Il disinstallatore di NVDA si chiama `uninstall.exe` e risiede nella directory di installazione di NVDA, `%ProgramFiles(x86)%\nvda` in Windows a 64-bit, o `%ProgramFiles%\nvda` nei sistemi a 32 bit.

Seguono le opzioni a riga di comando valide per il software di disinstallazione:

| Breve |Lungo |Descrizione|
|---|---|---|
|`/S` |Nessuno |Disinstalla silenziosamente NVDA. |
