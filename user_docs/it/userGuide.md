...
* Sistemi operativi: tutte le versioni a 32 e 64 bit dei Sistemi Windows 7, Windows8, Windows8.1, Windows10, Windows11 e tutte le edizioni Server a partire da Windows Server 2008 r2.
 * Per Windows 7, NVDA richiede Service Pack 1 o successivo.
 * Per Windows Server 2008 R2, NVDA richiede Service Pack 1 o successivo.
...
|Legge Selezione corrente |`NVDA+Shift+freccia su` |`NVDA+shift+s` |Legge il testo selezionato, se presente.|
|Legge testo negli appunti |`NVDA+c` |`NVDA+c` |Annuncia il testo contenuto negli appunti, se presente.|
...
NVDA dispone di una vivace comunità di utenti. In primo luogo, esiste il [gruppo di discussione in italiano](https://groups.io/g/nvda-it), [quello internazionale](https://nvda.groups.io/g/nvda) oltre ad una pagina con [le liste di discussione in varie lingue](https://github.com/nvaccess/nvda-community/wiki/Connect).
...
Se invece è già stata chiusa questa finestra di dialogo o si desidera eseguire l'installazione da una copia portable, scegiere la voce di menu Installa NVDA situata in Strumenti nel menu NVDA.
...
È possibile leggere ulteriori informazioni riguardo i dati raccolti da NV Access nella sezione impostazioni generali, [Consenti al progetto NVDA di raccogliere statistiche sull'utilizzo dello screen reader](#GeneralSettingsGatherUsageStats).
...
Se si sta eseguendo NVDA in un dispositivo dotato di touch screen e con un sistema operativo pari a Windos8 o superiore, è possibile controllare lo screen reader direttamente dallo schermo.
...
1. | Sposta il cursore di controllo sulla barra di stato | nessuno | nessuno | nessuno | Legge il contenuto della barra di stato, se individuata. Inoltre sposta il navigatore ad oggetti su quella posizione. |
<!-- KC:endInclude -->
...
#### Esplorazione schermo (modalità in linea) {#toc60}
...
Aprire la finestra di dialogo gesti e tasti di immissione dalla modalità navigazione.
Cercare l'elemento "Vai alla scheda successiva" nella sezione modalità navigazione.
Assegnare il tasto `y` al gesto.
Cercare l'elemento "Vai alla scheda precedente.
Assegnare `Maiusc+y` al gesto.
...
NVDA è in grado di leggere ed interagire con la matematica nei contenuti che fanno uso del software MathPlayer4, prodotto da Design Science.
Ciò richiede che MathPlayer4 sia installato sul computer.
MathPlayer può essere scaricato gratuitamente dal seguente sito web: https://www.dessci.com/en/products/mathplayer/.
Dopo aver installato MathPlayer, riavviare NVDA.
...
Può essere scaricato da https://www.dessci.com/en/products/mathtype/
* Adobe Reader:
...
A questo punto, è possibile utilizzare i comandi MathPlayer come i tasti freccia per esplorare l'espressione.
...
Si prega di consultare la [documentazione di MathPlayer sui comandi di navigazione](https://www.dessci.com/en/products/mathplayer/navigation_commands.htm) per ulteriori informazioni.
...
è possibile selezionare la tabella di traduzione per inserire il testo tramite la casella combinata [immissione braille](#BrailleSettingsInputTable) nella categoria braille della finestra [Impostazioni NVDA](#NVDASettings).

Quando viene utilizzato il braille non contratto (ossia quello classico presente in Italia), il testo viene inserito nel momento in cui si digitano i caratteri.
...
Tale emulazione può avvenire in due modi: assegnando una combinazione braille corrispondente alla pressione dei tasti desiderati servendosi anche dei tasti modificatori virtuali.
...
In secondo luogo, NVDA ha bisogno di conoscere quale riga o colonna contenga le intestazioni, per ciascuna tabella.
...
Una volta selezionato un elemento, appariranno opzioni inerenti soltanto alla categoria selezionata.
...
##### Apri impostazioni Generali {#toc102}
...
##### Consenti al progetto NVDA di raccogliere statistiche sull'utilizzo dello screen reader {#toc112}
...
##### Descrizione ritardata dei caratteri al movimento del cursore {#toc133}

| . {.hideHeaderRow} |.|
|---|---|
|Opzioni |Abilitata, disabilitata|
...
#### Seleziona sintetizzatore {#SelectSynthesizer}

<!-- KC:setting -->

##### Apri la finestra di dialogo seleziona sintetizzatore {#toc135}

Tasto: `NVDA+control+s`

La finestra Sintetizzatore, che si apre premendo il pulsante cambia... dalla categoria voce della finestra impostazioni di NVDA, consente di selezionare con quale sintesi vocale NVDA debba parlare.
Una volta scelto il sintetizzatore desiderato, premere OK affinché NVDA lo carichi ed inizi ad utilizzarlo.
Se durante il caricamento della nuova sintesi vocale apparisse un messaggio d'errore, NVDA provvederà a ripristinare il vecchio sintetizzatore e l'utente verrà avvisato con un messaggio.
...
| . {.hideHeaderRow} |.|
|---|---|
|Opzioni |Default (Abilitato), Abilitato, Disabilitato|
|Default |Abilitato|
...
##### Apri la finestra di dialogo Seleziona display Braille {#toc158}
...
##### Apri impostazioni audio {#toc163}

Tasto: `NVDA+control+u`

La categoria Audio nella finestra di dialogo Impostazioni NVDA contiene opzioni che consentono di modificare diversi aspetti della periferica audio di uscit.
...
Questo cursore di avanzamento consente di impostare il volume dei suoni e dei beep di NVDA.
L'impostazione ha effetto solo quando "Usa WASAPI per l'output audio" è abilitato e "Il volume dei suoni di NVDA segue il volume della voce" è disabilitato.
...
Per modificare lo stato della tenda schermo da qualsiasi punto ci si trovi, si prega di assegnare una combinazione di tasti o un gesto personalizzato servendosi della [Finestra Tasti e Gesti di immissione](#InputGestures).
...
##### Apri impostazioni tastiera {#toc173}
...
Se non viene selezionato alcun tasto che agisca come tasto NVDA, potrebbe risultare impossibile accedere ad alcuni comandi dello screen reader.
Pertanto, verrà visualizzato un messaggio d'errore nell'eventualità tutti i tasti siano deselezionati.
Una volta chiusa la finestra d'errore, è obbligatorio selezionare un tasto funzione di NVDA per proseguire.
...
##### Apri impostazioni mouse {#toc186}
...
##### Leggi il tipo dell'oggetto sotto il mouse {#MouseSettingsRole}
...
#### Impostazioni interazione al tocco {#TouchInteraction}

Questa categoria, disponibile soltanto  nei computer che montano Windows8 e successivi con un touch screen a bordo, consente di stabilire come NVDA debba interagire con tale touch screen.
Contiene le seguenti opzioni:
...
##### Apri le impostazioni presentazione oggetti {#toc203}
...
##### Apri impostazioni modalità navigazione {#toc221}
...
##### Attiva modalità navigazione al caricamento della pagina {#toc225}
...
##### Apri Impostazioni formattazione documento {#toc235}
...
##### Utilizza UI Automation per accedere alla console di Windows quando disponibile {#AdvancedSettingsConsoleUIA}

Quando questa opzione è abilitata, NVDA utilizzerà una nuova versione ancora in sviluppo del suo supporto per il prompt dei comandi e la console di Windows che sfrutta i [miglioramenti dell'accessibilità apportati da Microsoft](https://devblogs.microsoft.com/commandline/whats-new-in-windows-console-in-windows-10-fall-creators-update/). Questa funzione è altamente sperimentale ed è ancora incompleta, quindi il suo uso non è ancora raccomandato. Tuttavia, una volta completato, si prevede che questo nuovo supporto diventerà quello predefinito, migliorando le prestazioni e la stabilità di NVDA nel prompt dei comandi e nelle console di Windows.
...
Per un tutorial introduttivo fare riferimento al seguente indirizzo: [https://docs.python.org/3.7/howto/regex.html].
...
Esattamente come accade per l'installazioneo la rimozione i componenti aggiuntivi, è necessario riavviare NVDA affinché le modifiche abbiano effetto.
...
è possibile servirsi della combinazione di tasti NVDA+f1 per aprire una finestra dove verranno visualizzate  tutte le informazioni di log, comprese quelle per sviluppatori e sul navigatore ad oggetti.

Oltre a poter leggere il contenuto, è anche possibile salvare una copia del file di log, o aggiornare il visualizzatore di modo da poter controllare le ultime informazioni.
...
Comunque, se si clicca o si focalizza la finestra, NVDA fermerà temporaneamente l'aggiornamento di modo da permettere la selezione del testo e l'eventuale copia dello stesso.
...
In Windows7, windows 8 e 8.1, da impostazioni predefinite NVDA si avvia utilizzando Espeak NG, mentre su Windows10 e successive la sintesi vocale di default è [Windows OneCore](#OneCore).
...
Consultare questo articolo di Microsoft per un elenco delle voci disponibili: https://support.microsoft.com/en-us/windows/appendix-a-supported-languages-and-voices-4486e345-7730-53da-fcfe-55cc64300f01
...
Nel caso non si sia in possesso di tali driver, è possibile scaricarli dall'indirizzo https://support.freedomscientific.com/Downloads/Focus/FocusBlueBrailleDisplayDriver.
...
#### Assegnazione tasti per tutti i modelli {#toc317}
...
#### Assegnazione tasti per Brailliant BI 32, BI 40 e B 80 {#toc318}
...
#### Assegnazione tasti per Brailliant BI 14 {#toc319}
...
In caso di connessione tramite USB, è necessario prima installare i driver sviluppati dal produttore.
Si possono scaricare da qui all'indirizzo: http://www.himsintl.com/upload/HIMS_USB_Driver_v25.zip
...
|tasto alt+frecciaSinistra |punto2+punto7|
...
|tasto alt+frecciaDestra |punto5+punto7|
...
|tasto alt+frecciaSu |punto2+punto3+punto7|
...
|tasto alt+frecciaGiù |punto5+punto6+punto7|
...
Si possono reperire maggiori informazioni su https://en.seika-braille.com/down/index.html.
...
* Selezionare "Seika Braille Displays" per configurare manualmente
...
* Selezionare "Seika Notetaker" o "automatico" per configurare.
...
Per questo motivo, questi modelli non supportano la funzione di rilevazione automatica in background.
...
|Sposta il Display Braille alla riga successiva |lndn (va già di una riga)|
...
Sebbene il driver supporti una velocità di trasmissione di 9600, non ha modo di controllare la velocità di trasmissione utilizzata dal display.
...
|Passa tra le modalità parlare, bip e spento |`attribute2+attribute4`|
...
|Riporta informazioni sulla posizione del testo o dell'oggetto in corrispondenza del cursore di controllo |`f8`|
...
Ciò spesso include l'installazione e la configurazione di componenti aggiuntivi personalizzati, che andranno valutati indipendentemente dallo screen reader.
...
|-l LOGLEVEL |--log-level=LOGLEVEL |Il livello più basso di un messaggio loggato (debug 10, input/output 12, debug warning 15, info 20, disabled 100)|
...
Se si vogliono trovare maggiori informazioni o richiedere assistenza riguardo NVDA, si prega di visitare o il sito internazionale in lingua inglese su NVDA_URL, oppure quello relativo alla comunità italiana all'indirizzo [www.nvda.it](http://www.nvda.it).
