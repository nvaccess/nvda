...
Ovo izdanje uvodi NVDA daljinski pristup koji vam omogućuje upravljanje udaljenim računalom s NVDA s drugog uređaja s NVDA.

Unapređena je govorna podrška, a naročito odziv Sapi 4, Sapi 5 i OneCore glasova.
Ubrzavanje govora i automatsko otkrivanje jezika je sada dostupno pri korištenju SAPI 5 sintetizatora govora.
...
Unapređen je sustav automatskih ažuriranja Add-on storea, što vam omogućuje odabir kanala za automatske nadogradnje, te konfiguraciju pokretanja automatskih ažuriranja u pozadini.

Sada je lakše uključivati ili isključivati ručno osvježavanje sadržaja, kao i automatsko, a sve to uz pomoć novih prečaca.
...
* Ova inačica nije kompatibilna s prethodnim dodacima.
...
* Add-on Store:
  ...
  * Popis dodataka može biti poredan po stupcima, uključujući datum objave, ulaznim ili silaznim redoslijedom. (#15277, #16681, @nvdaes)
* Govor:
  * Od sada je podržano automatsko otkrivanje jezika prilikom korištenja Microsoft Speech API version 5 (SAPI5) i Microsoft Speech Platform glasova. (#17146, @gexgd0419)
...
* Internetski preglednici:
  ...
  * NVDA sada može izgovarati kada odredišna poveznica ukazuej na trenutnu stranicu. (#141, @LeonarddeR, @nvdaes)
* Microsoft Office:
...
* Izmjene u NVDA korisničkom sučelju:
  ...
  * Lratke inačice najčešće korištenih naredbi naredbenog redka su dodane: `-d` za `--disable-addons` i `-n` za `--lang`.
...
* Govor:
  ...
  * Unicode normalizacija je sada podrazumijevano uključena za govorni izlaz. (#17017, @LeonarddeR).
    * Još uvjkek možete isključiti ovu značajku u NVDA postavkama govora.
...
* Matematika:
  ...
  * Matematičke jednadžbe predstavljene samo slikom i zamjenskim tekstom bez MathML-a za obogaćenu navigaciju, sada se tretiraju kao normalne slike, a ne kao matematika bez sadržaja, omogućujući korisniku da skoči na njih s 'g' i da može streliti kroz zamjenski tekst po znaku. (#16007)
* IDE-ovi:
...
* Konfiguracijski profili:
  ...
  * Riješen je problem zbog kojeg su određene postavke izričito spremljene u aktivni konfiguracijski profil čak i kada je vrijednost te postavke bila jednaka vrijednosti u osnovnoj konfiguraciji. (#17157, @leonarddeR)
* Prilikom pravopisa, Unicode normalizacija sada radi prikladnije:
  * Nakon prijavljivanja normaliziranog znaka, NVDA više ne prijavljuje netočno sljedeće znakove kao normalizirane. (#17286, @LeonarddeR)
  * Složeni znakovi (kao što je é) sada se ispravno prijavljuju. (#17295, @LeonarddeR)
...
* Zbog povlačenja NVDA-ine winmm podrške (#17496, #17532, #17678):
  * Sljedeći simboli uklonjeni su iz 'nvwave' bez zamjene: 'CALLBACK_EVENT', 'CALLBACK_FUNCTION', 'CALLBACK_NULL', 'HWAVEOUT', 'LPHWAVEOUT', 'LPWAVEFORMATEX', 'LPWAVEHDR', 'MAXPNAMELEN', 'MMSYSERR_NOERROR', 'usingWasapiWavePlayer', 'WAVEHDR', 'WAVEOUTCAPS', 'waveOutProc', 'WAVE_MAPPER', 'WHDR_DONE', 'WinmmWavePlayer' i 'winmm'.
  ...
  * Izlaz iz 'nvwave.outputDeviceNameToID' i ulaz u 'nvwave.outputDeviceIDToName' sada su identifikatori nizova.
  ...
  * Parametar 'outputDevice' na 'WasapiWavePlayer.__init__' sada bi trebao biti proslijeđen samo argumentima niza.
  * Zastarjeli parametri 'closeWhenIdle' i 'buffered' u 'WasapiWavePlayer.__init__' uklonjeni su.
...
* Dodani prečaci za Tivomatic Caiku Albatross brajične redke. (#14844, #15002)
  ...
  * kretanje pomoću načina rada "Kursor sustava premještanja Brailleovog pisma prilikom usmjeravanja kursora pregleda". (#15122)
...
* U ažuriranju sustava Windows 10 za svibanj 2019. i novijima, NVDA može objaviti nazive virtualnih radnih površina prilikom otvaranja, mijenjanja i zatvaranja. (#5641)
...
* Ažuriran LibLouis prevoditelj brajice na [3.24.0](https://github.com/liblouis/liblouis/releases/tag/v3.24.0). (#14436)
...
eSpeak je ažuriran i LibLouis je ažuriran.
...
* Dodan je novi prečac za provjeru tipkovničkog prečaca trenutno fokusirane stavke. (#13960)
  ...
  * Prijenosno računalo: 'NVDA+ctrl+shift+.'.
...
* Nadograđen je LibLouis brajični prevoditelj na inačicu [3.23.0](https://github.com/liblouis/liblouis/releases/tag/v3.23.0). (#14112)
  * Dodane brajične tablice:
    ...
    * Luganda puno pismo
    * Švedska besugovorena Brailleova pisma
    * Švedski djelomično skraćeni Brailleovo pismo
    * Švedski ugovoreni Brailleovo pismo
...
* U hostu Windows naredbenog redka kojeg koristi naredbeni redak, PowerShell i windows podsustav za sustava Linux u Windowsima 11 inačica 22H2 (Sun Valley 2) i novijim:
  * Znatno poboljšane performanse i stabilnost. (#10964)
...
* NVDA sada najavljuje kad se NVDA radnja ne može izvesti. (#13500)
  * Ovo uključuje sljedeće:
    * Korištenje NVDA verzije za Windows Store.
    * U sigurnom kontekstu.
    * Čekanje odgovora na modalni dijalog.
...
eSpeak-NG i LibLouis su ažurirani, dodajući nove japanske, njemačke i katalonske tablice.
...
* Ispravljena sigurnosna preporuka `GHSA-mvc8-5rv9-w3hx`. (#13487)
  * Alat za inspekciju wx GUI sada je onemogućen u sigurnom načinu rada.
...
* Eksperimentalna podrška Aria zabilješki:
  * dodaje naredbu za čitanje sažetka detalja objekta s aria-details. (#12364)
  * dodaje opciju u naprednim postavkama za prijavu ima li objekt detalje u modu pretraživanja. (#12439)
...
* Poštuje se izgled prozora i njegov smjer baziran na jeziku NVDA, umjesto na jeziku sustava. (#638)
  * poznati problem za jezike koji se pišu zdesna nalijevo: desni rub isječaka grupiranja s oznakama/kontrolama. (#12181)
...
* U Korisničkom priručniku, kada se naredba <!-- KC:setting koristi za postavku koja ima zajednički ključ za sve izglede, ključ se sada može staviti iza dvotočke pune širine (:) kao i obične dvotočke (:). (#5739) -->
...
* capslock, umetak numeričke tipkovnice i prošireni umetak mogu se koristiti kao NVDA modifikatorska tipka. Također, ako se koristi jedna od ovih tipki, dvaput pritiskom na tipku bez pritiskanja bilo koje druge tipke poslat će tipku u operativni sustav, baš kao što ste pritisnuli tipku bez NVDA uključenog. Da bi jedna od tih tipki bila modifikatorska tipka NVDA, označite njezin potvrdni okvir u dijaloškom okviru Postavke tipkovnice (nekada se zvao dijaloški okvir odjeka tipkovnice).
...
* Dodani argumenti naredbenog retka u NVDA:
  ...
  * -d, --debug-file fileName: određuje gdje NVDA treba postaviti poruke za otklanjanje pogrešaka
  * -c, --config-file: navesti alternativnu konfiguracijsku datoteku
  * -h, -help: prikazuje poruku pomoći s popisom argumenata naredbenog retka
