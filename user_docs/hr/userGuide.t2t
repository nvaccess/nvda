...
+ Pojavit će se proces NVDA programa za pokretanje sa tekstom licencnog ugovora.
Pritišćite ``strelicu dolje`` kako biste pročitali licencni ugovor ako želite.
+ Pritišćite ``tab`` kako biste se pomaknuli na potvrdni okvir "Prihvaćam", a potom pritisnite ``razmaknicu`` kako biste ga odabrali.
+ Pritišćite ``tab`` kako biste se premještali između opcija, a potom pritisnite ``enter`` na željenoj opciji.
+
...
+ Uvjerite se da je potvrdni okvir za prihvaćanje licence odabran.
+ ``Pritišćite tab`` te aktivirajte gumb "Instaliraj NVDA na ovo računalo".
+ Slijede opcije za stvaranje prečaca na radnoj površini, te za automatsko pokretanje NVDA.
...
+ Dijaloški okvir kontrole korisničkog računa (UAC)" pojavit će se s pitanjem "Želite li dozvoliti ovoj aplikaciji vršenje izmjena na na ovom računalu?".
+ Pritisnite ``alt+d`` kako biste prihvatili upit dialoškog okvira UAC.
+ Traka napredka se popunjava tijekom NVDA instalacije.
U toku tog procesa, NVDA reproducira zvučni signal povišene frekvencije.
Ovaj proces je često brz i i neprimjetno brz.
+ Pojavljuje se dijaloški okvir koji obavještava korisnika o uspješnoj instalaciji.
...
+ Pojavljuje se dijaloški okvir "Dobrodošli u NVDA" te NVDA čita poruku dobrodošlice.
...
+ Pritisnite ``tab`` kako biste se premjestili na opciju "Koristi ``capsLock`` kao NVDA modifikacijsku tipku".
...
+ Koristite ``tab`` i ``razmaknicu`` kako biste prilagodili ostale opcije na tom zaslonu.
One određuju način automatskog pokretanja.
+ Pritisnite ``enter`` kako biste zatvorili dijaloški okvir sa pokrenutim programom NVDA.
+
...
+ Otvorit ćete dijaloški okvir ulaznih gesti iz modusa čitanja.
+ Pronači ćete "premješta se na sljedeću karticu svojstva" u odjeljku modus čitanja.
+ Pridjelit ćete slovo ``y`` za pronađeni prečac.
+ Pronaći ćete "premjesti se na prethodnu karticu svojstva".
+ Pridjelit ćete ``šift+y`` za pronađeni prečac.
+
...
+ Pomaknite pregledni kursor na matematički sadržaj.
...
+ Potom, pritisnite sljedeći prečac:
+
...
+ Premjestite se na tekst koji sadrži bilješku.
+ Pristupite opcijama označenog teksta kao što je gore opisano.
+ Odaberite opciju "Uredi bilješku". Napomena: budući da hrvatski prevoditelj NVDA čitača ekrana ne zna, je li je amazon lokaliziran na hrvatski, preveo je nazive opcija u postupcima za rad s tekstom.
+
...
+ Uvjerite se da imate instalirane [posljednje upravljačke programe https://www.papenmeier-rehatechnik.de/en/service/downloadcenter/software/articles/software-braille-devices.html].
