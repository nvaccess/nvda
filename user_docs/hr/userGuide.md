...
* Operacijski sustavi: sva 32-bitna i 64-bitna izdanja sustava Windows 7, Windows 8, Windows 8.1, Windows 10, Windows 11 i svih serverskih operacijskih sustava, počevši od Windows Server 2008 R2.
 * Za Windows 7, NVDA treba servisni paket 1 ili noviji.
 * Za Windows Server 2008 R2, NVDA treba servisni paket 1 ili noviji.
...
|Čitaj označeno |`NVDA+shift+strelica gore` |`NVDA+shift+s` |Čita bilo koji označeni tekst|
|Čitaj tekst u međuspremniku |`NVDA+c` |`NVDA+c` |Čita bilo koji tekst u međuspremniku|
...
|Izgovori fokus |`NVDA+tab` |`NVDA+tab` |Izgovara trenutnu kontrolu u fokusu.  Kada se pritisne dvaput, slovka informaciju|
...
|Uključi prećenje miša |`NVDA+m` |`NVDA+m` |Kaa je ova opcija uključena, NVDA će izgovarati tekst pod pokazivačem miša, u isto vrijeme kada ga pomićete. Ovo vam omogućuje nalaženje opcija na zaslonu fizički pomičući miša umjesto traženja opcija objektnom navigacijom.|
...
|Smanji vrijednost postavke govorne jedinice |`NVDA+control+strelica dolje` |`NVDA+shift+control+strelica dolje` |smanjuje vrijednost trenutne postavke govorne jedinice na kojoj se nalazite. Npr: smanjuje brzinu, bira prethodni glas, smanjuje glasnoću|
...
|Polje obrasca |`f` |Premješta an sljedeće polje obrasca (polje za uređivanje, gumb, itd)|
|veza |`k` |Premješta na sljedeću poveznicu|
|orjentir |`d` |premještava an sljedeći orjentir|
|popis |`l` |premješta na sljedeći popis|
|Tablica |`t` |Premješat na sljedeću tablicu|
...
Postoji također mpuno opširnihi materijal za obuku dostupan u [NV Access trgovini](https://www.nvaccess.org/shop).
...
* Nemogućnost čitanja zaslone kontrole korisničkih računa (UAC) prilikom pokušaja pokretanja aplikacije s administratorskim pravima.
* Windows 8 i noviji: nemogućnost korištenja dodirnika.
* Windows 8 i novije: nemogućnost podrške značajki poput modusa pregleda, čitanje upisanih znakova u Windows Store aplikacijama.
* Windows 8 i novije: utišavanje zvuka nije podržano.
...
U instaliranim kopijama, NVDA standardno posprema konfiguraciju u mapu aplikacijskih podataka Roaming trenutačnog korisnika (npr. "C:\Users\<user>\AppData\Roaming").
...
O tome koji se podaci prikupljaju, možete pročitati u poglavlju o općenitim postavkama, [Dozvoli NV accessu prikupljanje statistike o korištenju](#GeneralSettingsGatherUsageStats).
...
Ako pokrećete NVDA na uređaju koji ima ekran osjetljiv na dodir s pokrenutim sustavom Windows 8 ili noviji, možete upravljati NVDA čitačem izravno pomoću dodirnih naredbi, osim ako podrška ekrana osjetljivih na dodir nije isključena.
...
Osim prečaca za kretanej koji su popisani više, NVDA NVDA posjeduje prečace koji nisu definirani.
...
Imajte na umu a postoje dva tipa prečaca za kretanje po elementima za kretanje u naprijed i u nazad po dokumentu te morate pridjeliti oba prečaca kako biste se mogli kretati u oba smjera.
...
Koristeći MathPlayer4 tvrtke Design Science, NVDA može čitati i istovremeno biti u interakciji s podržanim matematičkim sadržajem.
Ovo zahtjeva instaliranu kopiju MathPlayera na vašem računalu.
MathPlayer je dostupan za besplatno preuzimanje s adrese: https://www.dessci.com/en/products/mathplayer/.
Poslije instalacije MathPlayer-a, ponovno pokrenite NVDA.
...
Može se preuzeti s adrese https://www.dessci.com/en/products/mathtype/
* Adobe Reader:
...
U tom trenutku, možete koristiti tipkovničke prečace MathPlayer-a da biste se kretali po sadržaju.
...
Za više informacija o prečacima za kretanje po matematičkom sadržaju, pogledajte dokumentaciju MathPlayera: https://www.dessci.com/en/products/mathplayer/navigation_commands.htm
...
Nakon toga, nvda treba znati koji stupci sadržavaju zaglavlja za tu tablicu.
Poslije pomicanja na čeliju u stupcu ili retku koja sadrži zaglavlja, koristite sljedeće naredbe:
<!-- KC:beginInclude -->
...
##### Dozvoli NVDA projektu prikupljanje statistike korištenja {#GeneralSettingsGatherUsageStats}

Ako je ova opcija aktivirana, NV Access će koristiti informacije dobivene od servera za nadogradnje kako bi pratila broj korisnika uključujući opće demografske statistike kao što su to operacijski sustav i zamlja porijekla.
...
##### Otvara postavke govorne jedinice {#toc137}
...
| . {.hideHeaderRow} |.|
|---|---|
|Options |Podrazumjevano (Omogućeno), omogućeno, onemogućeno|
|Podrazumjevano |Aktivirano|
...
| . {.hideHeaderRow} |.|
|---|---|
|Opcije |podrazumjevano (aktivirano), aktivirano, deaktivirano|
|podrazumjevano |aktivirano|
...
##### Otvara dijaloški okvir odabira brajičnog redka {#toc160}
...
Ova opcija se primjenjuje samo kada je opcija "Koristi WASAPI za zvučni izlaz" omogućena.
...
Kako biste ekransku zavjesu uključili i isključili s bilo kojeg mjesta, dodijelite prilagođeni prečac u dijaloškom okviru [Ulazne geste](#InputGestures).
...
##### Otvara postavke tipkovnice {#toc175}
...
Ako nijedna tipka nije odabrana kao NVDA modifikacijska tipka, nećete moći pristupiti većini NVDA funkcijama.
Stoga, ako ne odaberete niti jednu modifikacijsku tipku, a pritisnete U redu ili Primijeni, NVDA će prikazati poruku o grešci.
Poslije zatvaranja poruke o grešci, morat ćete odabrati jednu od ponuđenih opcija kao NVDA modifikacijsku tipku da biste mogli pravilno zatvoriti dijaloški okvir.
...
##### Izvijesti o vrsti objekta kad miš uđe u objekt {#MouseSettingsRole}
...
Ova kategorija postavki, koja je dostupna samo na računalima s instaliranim sustavom windows 8 ili novijim, omogućuje podešavanje načina interakcije NVDA čitača s ekranima osjetljivim na dodir.
Ova kategorija sadrži sljedeće opcije:
...
##### Otvara dijaloški okvir prezentacija objekata {#toc205}
...
##### Otvara postavke modusa čitanja {#toc223}
...
##### Otvara postavke oblikovanja dokumenta {#toc237}
...
|Podrazumjevani |neka rukuje aplikacija|
...
Odabirni okvir sadrđi sljedeće opcije:
* Podrazumijevano (samo kada ej to potrebno): podrazumjevana opcija koju koristi NVDA, trenutno se koristi "samo kada je to potrebno". Ova se podrazumjevana opcija može promijeniti u budućnosti kada tehnologija bude zrela za široku uporabu.
...
Omogućeno
 : Opcije
Podrazumevano (omogućeno), onemogućeno, omogućeno
...
|---|---|
 ...
 opcije
 Podrazumijevano (difovanje), difovanje, Obavijesti UIA
:
...
Za uvodni tutorijal, molimo konzultirajte [https://docs.python.org/3.7/howto/regex.html].
 ...
 * Geste sa modifikatorima možda neće moći biti pridjeljene emuliranoj gesti bez modifikatora
 Na primjer, postavljenj emuliranog unosa 'a' i podešavanje ulazne geste 'ctrl+m', prouzrokovat će
...
Preglednik zapisnika koji se nalazi u NVDA izborniku omogućuje pregled događaja od zadnjeg pokretanja NVDA čitača.
Koristeći kombinaciju tipki NVDA+F1 otvorit će se preglednik log zapisa te će se prikazati informacija o trenutačnom navigacijskom objektu.

Osim čitanja sadržaja, možete također spremiti kopiju datoteke log zapisa ili osvježiti preglednik tako da se prikazuju zadnji događaji.
Ove su opcije dostupne u izborniku preglednika pod "Log zapis".
...
Preglednik govora su uvijek pokušava otvoriti s istim dimenzijama i pozicijom kao pri zadnjoj upotrebi.

Dok je preglednik govora aktiviran, on se stalno osvježava kako bi prikazao tekst koji se trenutačno izgovara.
Međutim, ako kliknete ili fokusirate unutar preglednika, NVDA će privremeno zaustaviti osvježavanje teksta, tako da možete označiti ili kopirati postojeći sadržaj.
...
U Windowsima 7, 8 i 8.1, NVDA standardno koristi eSpeak NG ([Windows OneCore](#OneCore) se standardno koristi u Windowsima 10 i novijim).
...
Ako ih još nemate, možete ih preuzeti s adrese https://www2.freedomscientific.com/downloads/focus-40-blue/focus-40-14-blue-downloads.asp.
...
#### Dodijeljeni prečaci za sve modele {#toc311}
...
#### Dodijeljeni prečaci za Brailliant BI 32, BI 40 and B 80 {#toc312}
...
#### Dodijeljeni prečaci za Brailliant BI 14 {#toc313}
...
Ako redak spajate preko USB priključka, trebat ćete instalirati USB upravljačke programe tvrtke HIMS na vaš sustav.
Možete ih preuzeti ovdje: http://www.himsintl.com/upload/HIMS_USB_Driver_v25.zip
...
Više informacija o ovim brajičnim redcima možete pronaći na https://en.seika-braille.com/down/index.html.
...
Obja srednje tipke su mapirane na razmaknicu.
...
|Naizmenično uključuje režime govora isključeno, pištanje i priča |`atribut2+atribut4`|
...
|-l LOGLEVEL |--log-level=LOGLEVEL |Najniža razina zapisivanih poruak u zapisniku (debug 10, ulaz/izlaz 12, debug upozorenje 15, info 20, onemogućeno 100)|
...
Ako trebate više informacija ili pomoć za NVDA, posjetite NVDA web stranicu na adresi NVDA_URL.
