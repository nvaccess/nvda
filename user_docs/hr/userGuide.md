...
* Mogućnost [povezivanja i kontroliranja drugog računala koje pokreće NVDA](#RemoteAccess) za daljinsku pomoć ili suradnju
...
* Mogućnost pokretanja tijekom prijave u sustav Windows i [na drugim sigurnim radnim površinama](#SecureScreens)
...
#### Minimalni preduvjeti sustava {#MinimumSystemRequirements}
...
* Operacijski sustavi: sva 32-bitna i 64-bitna izdanja sustava Windows 8.1, Windows 10, Windows 11 i svih serverskih operacijskih sustava, počevši od Windows Server 2008 R2.
  ...
  * Imajte na umu da 32-bitni operacijski sustavi više nisu aktivno podržani.
...
#### Preporučeni sistemski preduvjeti {#RecommendedSystemRequirements}

* Operativni sustavi: 64-bitna izdanja sustava Windows 10, Windows 11 i Windows Server 2022.
  * obje inačice,  AMD64 kao i ARM64 operacijskog sustava Windows su podržane.
* najmanje 500 MB prostora za pohranu.
* najmanje 4 GB RAM-a.
...
Osim engleskog, NVDA je rpeveden na 55 jezika uključujući: afrikanerski, albanski, amharski, arapski, aragonski, bošnjački, bugarski, birmanski, katalonski, kineski (pojednostavljeni i tradicionalni), hrvatski, češki, danski, nizozemski, perzijski, finski, francuski, galicijski, gruzijski, njemački (Njemačka i Švicarska), grčki, hebrejski, hindski, mađarski, islandski, irski, talijanski, japanski, kannadaški, korejski, kirgiški, litavski, makedonski, mongolski, nepalski, norveški, poljski, portugalski (Brazilski i Europski), Pendžabski, rumunjski, ruski, srpski, slovački, slovenski, španjolski (Kolumbijski i Europski), Švedski, tamilski, tajski, turski, ukrajinski i vijetnamski.
...
Možda ćete trebati pričekati nekoliko sekundi dok se privremena kopija NVDA učita.
Jednom kada se pokrene, NVDA će govoriti tijekom cijelog procesa.
1. Prozor NVDA pokretačkog programa će se otvoriti s gumbom za otvaranje licencnog ugovora.
Pritišćite `StrelicuDolje` kako biste pročitali licenčni ugovor.
...
1. Prikazat će se dijaloški okvir sa potvrdom uspješne instalacije NVDA.
...
| Naziv |Prečac za stolna računala |Prečac za prijenosna računala |opis|
|---|---|---|---|
|Izgovor upisanih znakova |`NVDA+2` |`NVDA+2` |Regulira izgovor upisanih znakova.. |
|Izgovor upisanih riječi |`NVDA+3` |`NVDA+3` |Kontrolira izgovor upisanih riječi |
...
Preporučujemo vam započeti sa modulom "osnovna obuka za korištenje NVDA".
...
Ova opcija vam omogućuje da odaberete hoće li NVDA kopirati korisničku konfiguraciju s trenutno pokrenutog NVDA u konfiguraciju za trenutno prijavljenog korisnika, za instaliranu kopiju NVDA.
...
NVDA je podrazumijevano postavljen za korištenjem sa izgledom tipkovnice za stolna računala, ali ga možete prebaciti u kategoriji tipkovnica  u [dijaloškom okviru postavki NVDA,](#NVDASettings) kojeg možete pronaći u izborniku opcije u NVDA izborniku.
...
Jednostruki dodir s jednim prstom je jednostavno poznat kao dodir.
...
Stvarni se prečace neće izvršavati dok se nalazite u načinu "Pomoć pri unosu".
...
|Pomakni se na početak označenog u pregledu |`NVDA+alt+home` |`NVDA+alt+home` |nema |pomiće pregledni kursor na prvi znak označenog teksta|
|Pomakni se na kraj označenoga u pregledu |`NVDA+alt+end` |`NVDA+alt+end` |Nema |pomiće pregledni kursor na zadnji znak označenog teksta|
...
Dobar način za pamčenje osnovnih prečaca za pregled teksta pri korištanju izgleda tipkovnice za stolna računala je da ih pamislite kao mrežu u rasponu tri puta tri, gdje je od vrha do dna redak, riječ i znak a s lijeva na desno prethodna, trenutna i slijedeća stavka.
...
| Naziv |Tipka |Opis|
|---|---|---|
|Pretraži tekst |NVDA+control+f |Otvara dijaloški okvir za pretragu|
|Traži sljedeće |NVDA+f3 |Traži sljedeće pojavljivanje trenutačno traženog teksta|
|Pronađi prethodno |NVDA+shift+f3 |Traži prethodno poklapanje trenutnog upita pretraživanja|
...
Međutim, NVDA pruža mogućnost kopiranja sa čuvanjem oblikovanja što se može uključiti za određene dokumente u modusu čitanja u kojima je to podržano(Mozilla Firefox, te bilo koji preglednik baziran na Chromiumu 134 ili novijoj inačici) koja rezultira da kursor programa prati kursor unutar virtualnog spremnika NVDA.
...
Međutim, imajte na umu da se sve prethodno stvorene jednadžbe MathType najprije moraju pretvoriti u Office Math.
...
Pritisak prečaca `NVDA+f5` osvježit će rezultat prepoznavanja teksta.
...
NVDA pruža vlastite dodatne značajke za neke aplikacije kako bi određeni zadaci bili lakši ili bi se omogućio pristup funkcionalnostima koje inače nisu dostupne korisnicima čitača zaslona.
...
| Ime |Tipka |opis|
|---|---|---|
|Postavi zaglavlja stupaca |`NVDA+shift+c` |Kada se ovaj prečac pritisne jedamput, pokazujemo NVDA da je to prva ćelija u stupcu, koja treba biti izgovorena u stupcima ispod tog redka. Dvostruki brzi pritisak briše postavku.|
|Postavi zaglavlja redaka |`NVDA+shift+r` |Kada se pritisne jedamput, označava ovu ćeliju kao prvu ćeliju u redku koja sadrži zaglavlja redaka, koja prva treba biti pročitana prilikom čitanja od te ćelije. Dvostruki pritisak briše postavku.|

<!-- KC:endInclude -->
Ove će postavke biti spremljene kao knjižne oznake, kompatibilne s drugim čitačima ekrana poput Jawsa.
Ovo znači, da korisnici koji otvore dokument sa drugim čitačima zaslona kasnije, imat će označena zaglavlja redaka i stupaca.
...
<!-- KC:beginInclude -->
Za čitanje komentara na trenutnom položaju kursora, pritisnite `NVDA+alt+c`.
Kada se dvaput pritisne, informacija će se pokazati u modusu čitanja.
<!-- KC:endInclude -->
Svi komentari u dokumentu, sa svim praćenim izmjenama mogu biti prikazani u popisu elemenata kada se odaberu bilješke kao tip.
...
To znači da će korisnici drugih čitača zaslona koji kasnije otvore ovu radnu knjigu automatski imati već postavljena zaglavlja redaka i stupaca.
...
<!-- KC:beginInclude -->
Za čitanje bilješki za trenutno fokusiranu ćeliju pritisnite `NVDA+alt+c`.
Kada se pritisne dvaput, informacija će se pokazati u modusu čitanja.
...
NVDA nudi poboljšanu podršku za Poedit 3.5 ili noviji.
...
Kada je ovaj potvrdni okvir odabran, prikazat će se dijaloški okvir sa slijedećim mogućnostima:

* izađi
* ponovno pokreni
* ponovno pokreni sa isključenim dodacima i uključenim zapisnikom s otkrivanjem pogrešaka
* ponovno pokreni sa uključenim zapisnikom za otklanjanje pogrešaka
* Instaliraj ažuriranja na čekanju (ako postoje).
...
* jedinstveni identifikator korisnika NVDA, koji se mijenja svaki mjesec
* Jezik NVDA sučelja
...
Također možete instalirati ažuriranja na čekanju iz dijaloškog okvira za izlazak iz NVDA ako je to omogućeno u NVDA izborniku, ili ako pokrenete još jednu provjeru ažuriranja iz izbornika Pomoć.

##### Zrcalni server za nadogradnje {#UpdateMirror}

Ove opcije vam omogućuju određivanje alternativne adrese za provjeru ažuriranja NVDA.
To može biti korisno na lokacijama gdje je pristup do NV Access NVDA poslužitelja za nadogradnje spor ili nemoguć.

Polje za uređivanjj koje je samo za čitanje pokazuje trenutnu adresu zrcalnog poslužitelja.
Ako se ne koristi zrcalni poslužitelj, to jest, koristi se NV Accessov poslužitelj, prikazuje se "nema zrcalnog poslužitelja".

Ako želite promijeniti zrcalni poslužitelj za ažuriranja, pritisnite gumb "promijeni..." kako biste otvorili [dijaloški okvir za postavljanje zrcalnog poslužitelja](#SetURLDialog).

Imajte na umu, da kada koristite zrcalni poslužitelj za ažuriranjaoperator istog ima pristup [informacijama koje se šalju sa provjerama ažuriranja](#GeneralSettingsCheckForUpdates).
Za detalje o tome, kako se obrađuju vaši podaci, obratite se operatoru zrcalnog poslužitelja, kako biste bili sigurni da ste suglasni sa načinom njihove obrade prije postavljanja tog poslužitelja.

##### Izbjegni gašenje zaslona tijekom kontinuiranog čitanja ili tijekom čitanja uz pomoć brajičnog redka {#PreventDisplayTurningOff}

Ovaj potvrdni okvir, kada je omogućen, osigurava da zaslon ostane uključen pri korištenju naredbe čitaj sve ili uz pomoć brajičnog redka (npr. pirlikom pritiska tipke za pomicanje po redcima).
Time se izbjegava situacija u kojoj se zaslon neočekivano zaključava pri korištenju kontinuiranog čitanja.
Ova je opcija podrazumijevano uključena.
Razmislite o onemogućavanju ove opcije ako vam smeta kraći vijek trajanja baterije.
...
Prva opcija u kategoriji postavki govora je gumb Promijeni....
Pritiskom na ovaj gumb otvara se [dijaloški okvir odaberi govornu jedinicu,](#SelectSynthesizer) koji vam omogućuje promjenu aktivne govorne jedinice.
...
##### Normalizacija Unicode znakova {#SpeechUnicodeNormalization}

Kada je ova opcija omogućena, Normalizacija Unicode znakova biti će izvedena na tekstu koji će NVDA izgovoriti.
Ovo je ključno za znakove koji se mogu prikazivati u više oblika.
NVDA koristi  NFKC (engl. Normalization Form Compatibility Composition) algoritam, koji donosi sljedeća poboljšanja, među kojima su uključena:

1. Kose i podebljane verzije znakova koji su dio unicode standarda, te se često koriste na društvenim mrežama su normalizirane u njihove najčešće kompatibilne ekvivalente.
Na primjer, latinično slovo "h" može također biti prikazano kao "𝐡" (podebljano), "ℎ" (koso), itd., ali će biti izgovoreno kao "h" pri uključenoj normalizaciji.
Ovaj aspekt normalizacije također je od pomoći pri čitanju jednadžbi u Microsoft Word uređivaču jednadžbi.

1. Normalizacija u sastavljene znakove.
Na primjer, znak "ü" (u sa prijeglasom/dierezom), zajednički znak za jezike poput njemačkog i turskog može se prikazati u dva oblika.
  1. Jedan samostalan unicode znak (ü)
  1. rastavljanje na dva znaka (ü), točnije obično latinično u sa modifikatorom prijeglasa
  Unicode normalizacija osigurava to, da će se koristiti jedan oblik prilikom izgovaranja teksta, što je jedna varijanta znaka.

1. Rastavljanje nekih ligatura, uključujući "ĳ" (ligatura ij) u njihove dvoslovne oblike ("ij").

1. Stabilan poredak sastavljenih znakova, na primjer u starohebrejskom.

Kako biste mogli podesiti Unicode normalizaciju sa bilo kojeg mjesta, molimo dodijelite prilagođenu gestu koristeći [Dijaloški okvir ulazne geste](#InputGestures).

| . {.hideHeaderRow} |.|
|---|---|
|Opcije |podrazumijevano (omogućeno), omogućeno, onemogućeno|
|Podrazumijevano |Omogućeno|

##### Izgovaraj "normalizirano" pri kretanju po znakovima {#SpeechReportNormalizedForCharacterNavigation}

Ova postavka je potvrdni okvir koji služi za podešavanje izgovora stanja normaliziranosti Unicode znaka pri čitanju slovo po slovo.
Na primjer, kada je ova opcija uključena, slovkanje znaka "ĳ" rezultirat će izgovorom "i j normalizirano".

Imajte na umu da je ova postavka dostupna samo kad je "[Unicode normalizacija](#SpeechUnicodeNormalization)" uključena.
...
Ako odaberete "automatski", tablica će biti odabrana u skladu sa trenutno korištenim jezikom NVDA.
...
U tom slučaju, brajica neće pratiti fokus sustava i kursor sustava.
...
| . {.hideHeaderRow} |.|
|---|---|
|Opcije |Zadani (Nikad), Nikad, Samo kada je automatski povezano, Uvijek|
|Zadano |Nikada|
...
##### Oznaka početka odlomka {#BrailleParagraphStartMarkers}

Ako je označeno "Čitaj po odlomku", prikazat će se odabrana oznaka početka koja označava početak odlomka.
To može biti posebno korisno u aplikacijama koje se koriste za čitanje velikih dijelova teksta, poput strukturiranih dokumenata ili knjiga.
U takvim dokumentima, znati gdje odlomci počinju može biti korisno za razumijevanje strukture sadržaja ili za postavljanje oznaka ili bilješki na temelju položaja odlomka.

Opcije uključuju korištenje dva razmaka kao suptilnog prijeloma odlomka i simbola odlomka, pilcrow (¶), kao očitijeg.

| . {.hideHeaderRow} |.|
|---|---|
|Opcije |Nema oznake početka odlomka, Dvostruki razmak ( ), Pilcrow (¶)|
|Zadano |Nema oznake početka odlomka|
...
| . {.hideHeaderRow} |.|
|---|---|
|Opcije |Zadano (omogućeno), omogućeno, onemogućeno|
|Zadano |Omogućen|

##### Prikaz oblikovanja {#BrailleFormattingDisplay}

Ova postavka određuje kako će NVDA prikazati oblikovanje teksta na Brailleovom pismu.
Ova opcija ima učinak samo ako je NVDA postavljen na [prikaži atribute fonta na Brailleovom pismu](#DocumentFormattingFontAttributes).

| . {.hideHeaderRow} |.|
|---|---|
| Opcije | Zadano (Liblouis), Liblouis, Oznake |
| Zadano | Liblouis |

| Opcija | Ponašanje |
|---|---|
| Liblouis | Koristite izvorno oblikovanje Brailleovog pisma. Imajte na umu da će ova opcija označavati samo podebljani, kurziv i podcrtani tekst, i to samo ako odabrana tablica brajice podržava navođenje tih atributa. |
| [Oznake] (#BrailleFormattingDisplayTags) | Koristite oznake koje opisuju kako i gdje se mijenja oblikovanje teksta. |

###### Oznake {#BrailleFormattingDisplayTags}

Kada je "Prikaz formatiranja" postavljen na "Oznake", oznaka formatiranja prikazuje se na Brailleovom pismu kada se otkrije promjena formatiranja.
Ove oznake počinju s ⣋ i završavaju s ⣙.
Oznaka za oblikovanje sadržavat će jedan ili više simbola koji opisuju oblikovanje teksta.
Definirani su sljedeći simboli:

| Simbol | Značenje |
|---|---|
| ⠃ ("b") | Počnite podebljano |
| ⡃ ("b" s točkom 7) | Kraj podebljan |
| ⠊ ("i") | Započnite kurziv |
| ⡊ ("i" s točkom 7) | Završetak kurziva |
| ⠥ ("u") | Početak podcrtavanja |
| ⡥ ("u" s točkom 7) | Kraj podcrtan |
| ⠎ ("s")| Započnite precrtavanje |
| ⡎ ("s" s točkom 7) | Kraj precrtavanja |

##### Izgovaranje znakova prilikom usmjeravanja kursora u tekstu {#BrailleSpeakOnRouting}

Ako je to omogućeno, NVDA će automatski izgovoriti znak na kursoru prilikom usmjeravanja na njega pomoću tipki za usmjeravanje kursora na Brailleovom pismu.

Da biste ovu opciju prebacili s bilo kojeg mjesta, dodijelite prilagođenu gestu pomoću dijaloškog okvira [Input Gestures](#InputGestures).

##### Govori pri kretanju po redku ili odlomku {#BrailleSpeakOnNavigating}

Omogućavanje ove opcije prouzrokuje da NVDA izgovara redke ili odlomke koje čitamo na brajičnom redku.

Kako biset mogli uključiti iči isključiti ovu opciju sa bilo kojeg mjesta, molimo postavite prilagođeni prečac na opciju "izgovaraj pri kretanju po jedinicama" u odjeljku "brajica" u[dijaloškom okviru ulazne geste](#InputGestures).

##### Izbjegavajte dijeljenje riječi kada je to moguće {#BrailleSettingsWordWrap}

Ako je to omogućeno, riječ koja je prevelika da stane na kraj Brailleovog pisma neće se podijeliti.
Umjesto toga, na kraju zaslona bit će prazan prostor.
Kada se pomičete po zaslonu, moći ćete pročitati cijelu riječ.
To se ponekad naziva "omotavanje riječi".
Imajte na umu da ako je riječ prevelika da bi stala na zaslon čak i sama po sebi, riječ i dalje mora biti podijeljena.

Ako je to onemogućeno, prikazat će se što više riječi, ali ostatak će biti odsječen.
Kada se pomičete po zaslonu, moći ćete pročitati ostatak riječi.

Omogućavanje ovoga može omogućiti tečnije čitanje, ali općenito zahtijeva da se više pomičete po zaslonu.

##### Unicode normalizacija {#BrailleUnicodeNormalization}

Kada je ta mogućnost omogućena, Unicode normalizacija izvodi se na tekstu koji se ispisuje na Brailleovom pismu.
To je korisno kada naiđete na znakove na Brailleovom pismu koji su nepoznati u određenoj tablici na Brailleovom pismu i koji imaju kompatibilnu alternativu, poput podebljanih i kurzivnih znakova koji se obično koriste na društvenim mrežama.
Ostale prednosti unicode normalizacije detaljnije su objašnjene u [odjeljak za ekvivalentnu postavku govora](#SpeechUnicodeNormalization).

Da biste uključili Unicode normalizaciju s bilo kojeg mjesta, dodijelite prilagođenu gestu pomoću [Input Gestures dijaloški okvir](#InputGestures).

| . {.hideHeaderRow} |.|
|---|---|
|Opcije |Zadano (onemogućeno), omogućeno, onemogućeno|
|Zadano |Onemogućen|

##### Prekidanje govora tijekom pomicanja {#BrailleSettingsInterruptSpeech}

Ova postavka određuje treba li govor prekinuti kada se Brailleov zaslon pomiče unatrag/natrag.
Naredbe prethodnog/sljedećeg retka uvijek prekidaju govor.

Stalni govor mogao bi vam odvratiti pažnju dok čitate Brailleovo pismo.
Iz tog je razloga opcija omogućena prema zadanim postavkama, prekidajući govor prilikom pomicanja brajice.

Onemogućavanje ove opcije omogućuje da se čuje govor dok istovremeno čitate Brailleovo pismo.
...
* NVDA u lijevom kanalu i aplikacije na oba kanala: NVDA će govoriti u lijevom kanalu, dok će druge aplikacije reproducirati zvukove u oba kanala.
...
Ova opcija regulira izgovor pojedinačinh znakova pri pisanju s NVDA.
Dostupne su slijedeće opcije:

* isključeno: NVDA neće izgovarati upisane znakove.
* Samo u poljima za uređivanje: NVDA će izgovarati znakove samo u poljima za uređivanje, te na drugim mjestima nau kojim je moguće pisati tekst.
* Uvijek: NVDA će izgovarati sve upisane znakove.
...
Ova opcija regulira izgovor upisaivanih znakova na tipkovnici.
Slijedeće opcije su dostupne:

* Isključeno: NVDA neće izgovarati upisane riječi.
* Samo u poljima za uređivanje: NVDA će izgovarati riječi samo u poljima za uređivanje, i na mjestima gdje tekst može biti upisan.
* Uvijek: NVDA će izgovarati upisane riječi svugdje.
...
ova je opcija podrazumijevano uključena, ali će neki korisnici htjeti isključiti ovu opciju, kao na primjer oni, koji pišu na vijetnamskom uz pomoć UniKey programa za pisanje, jer će to prouzročiti nepravilan unos znakova.

##### Vremensko ograničenje pritiska na više tipki {#MultiPressTimeout}

Neke NVDA tipkovnice izvode različite radnje ovisno o tome koliko se puta pritisne ista tipka u brzom slijedu.
Primjer za to je naredba "Prijavi trenutni karakter objekta navigatora".
Ova naredba izvještava o znaku ako se pritisne jednom, fonetskom opisu znaka ako se pritisne dva puta i numeričkoj vrijednosti znaka ako se pritisne tri puta.
Ova opcija konfigurira vremensko ograničenje nakon kojeg će dodatni pritisak na istu tipku pokrenuti novu gestu, umjesto da se uzima kao sljedeći pritisak na prvu.
Za primjer naredbe, prekratko vremensko ograničenje uzrokovat će dva pritiska da dvaput prijave trenutni znak, umjesto fonetskog opisa.
Zadano vremensko ograničenje je 500 ms, odnosno pola sekunde.
Povećanje ovog vremenskog ograničenja može biti posebno korisno za ljude koji koriste ljepljive tipke ili koji imaju tjelesni invaliditet.
...
Ova opcija, koja je podrazumijevano uključena, omogućuje vam kontrolu nad tim, hoće li svi vidljivi kandidati biti izgovoreni automatski kada se popis kandidata pojavi, ili se stranica popisa promijeni.
...
Međutim, za fonetske metode unosa kao na primjer "Chinese New Phonetic", Korisnije je isključiti ovu opciju, jer će svi znakovi zvučati isto te će te trebati koristiti strelice za kretanje po popisu kako biste dobili informaciju za svaki znak skupa sa njegovim opisom znakova.
...
Za metode unosa, u kojima se odabir može mijenjati strelicama, (poput "Chinese New Phonetic"") to je nužno, ali kod nekih metoda unosa bolje je isključiti ovu opciju.
...
Ova vam opcija omogućuje kontrolu nad tim, hoće li NVDA ili neće izgovarati nove znakove, kako se pojavljuju u nizu znakova koje upisujete.
Ova je opcija standardno aktivirana.
...
Ova je opcija podrazumijevano uključena. Ova opcija vam omogućuje  kontrolu nad tim, hoće li prečaci (kao što su pritisci tipki) koji nisu prečaci programa NVDA i ne smatraju se općenito prečacem da prolaze kroz dokument u kojem se nalazite.
...
* font
  * ime fonta
  * veličina fonta
  * Atributi fonta [(Isključeno, Govor, Brailleovo pismo, Govor i Brailleovo pismo)](#DocumentFormattingFontAttributes)
...
* elementi
  * naslovi
  * poveznice
  * Tip poveznice (odredište na istoj stranici)
  * grafički elementi
  * popisi
  * citati
  * grupe
...
##### Atributi fonta {#DocumentFormattingFontAttributes}

Ova opcija omogućuje odabir načina prijavljivanja određenih atributa fonta, kao što su podebljano, kurziv, podcrtano i precrtano.
Kombinirani okvir atributa fonta ima četiri mogućnosti:

* Isključeno: NVDA neće prijaviti ove atribute fonta.
* Govor: NVDA će objaviti kada se ovi atributi fonta promijene.
* Brailleovo pismo: NVDA će prikazati ove atribute na Brailleovom pismu.
Točan način na koji se prikazuju može se konfigurirati u [NVDA-ovim postavkama brajice](#BrailleFormattingDisplay).
* Govor i Brailleovo pismo: NVDA će prijaviti atribute fonta koristeći obje gore navedene metode.
...
* Govor: ako je govor uključen, ako se količina uvlaka promijeni, NVDA će izgovoriti nešto poput "tvanaest razmak" ili "četiri tab."
* Zvučni signali: Ako su zvučni signali uključeni, kada se količina uvlaka promijeni, Tonovi će obavještavati o razini promijenjene uvlake.
...
##### Automatska ažuriranja {#AutomaticAddonUpdates}

Kada je ova opcija postavljena na "obavještavaj", Add-on store će vas obavještavati o dostupnosti ažuriranja dodataka poslije pokretanja NVDA.
Ova se provjera obavlja svakih dvadeset i četiri sata.
Podrazumijevano, bit ćete obaviješteni o ažuriranjima dodataka unutar [istog kanala,](#AddonStoreFilterChannel) stabilnnom, Beti ili dev.
Kanale za ažuriranje dodataka možete konfigurirati [pojedinačno za svaki dodatak](#AddonStoreUpdateChannel) ili za  [sve dodatke](#DefaultAddonUpdateChannel).

Kada je opcija postavljena na "ažuriraj automatski", dodaci će se ažurirati u pozadini.
Biti ćete zamoljeni da ponovno pokrenete NVDA.

| . {.hideHeaderRow} |.|
|---|---|
|Opcije |obavjesti (podrazumijevano), automatski nadogradi, onemogućeno |
|Podrazumjevano |obavijesti |

|Opcija |ponašanje |
|---|---|
|Obavjesti |obavještava o dostupnosti ažuriranja dodataka |
|Automatki ažuriraj |Automatski ažurira dodatke u pozadini |
|Onemogućeno |ne provjerava automatski za ažuriranja dodataka |

##### Podrazumijevani kanal za ažuriranja {#DefaultAddonUpdateChannel}

Kada su [automatska ažuriranja dodataka](#AutomaticAddonUpdates) uključena, podrazumijevano se dodaci ažuriraju na isti [kanal](#AddonStoreFilterChannel).
Na primjer, instalirana beta verzija dodatka će se ažurirati na noviju beta inačicu.
Ova opcoja postavlja kanal ažuriranja za sve dodatke.
Također možete promijeniti kanal ažuriranja za [određeni dodatak pojedinačno u upravitelju dodataka](#AddonStoreUpdateChannel).

| . {.hideHeaderRow} |.|
|---|---|
| Opcije | isti (podrazumijevano), bilo koji, ne ažuriraj, stabilni, beta ili dev, beta, Dev |
| Podrazumijevano | isti |

| Opcija | ponašanje |
|---|---|
| Isti | dodaci će ostati na svojim kanalima |
| Bilo koji | će se nadograditi na posljednju inačicu, bez obzira na kanal |
| Ne nadograđuj | podrazumijevano, dodaci se neće ažurirati automatski, Svaki od njih ćete morati uključiti posebno |
| stabilni | dodaci će se automatski ažurirati na stabilne inačice |
| Beta ili dev | dodaci će se automatski ažurirati na beta ili dev verzije |
| Beta | Dodaci će se automatski nadograđivati na beta verzije |
| Dev | dodaci će se automatski nadograđivati na dev verzije |

##### Dopusti instalaciju nekompatibilnih dodataka tijekom automatskog ažuriranja {#AllowIncompatibleAddonUpdates}

Ova postavka omogućuje ažuriranje dodataka na inačice koje mogu biti nekompatibilne sa trenutnom inačicom NVDA.
Podrazumijevano, ova opcija je isključena, što znači da će se dodaci ažurirati na verzije označene kao kompatibilne sa trenutnom inačicom NVDA.
Automatska ažuriranja i dalje će ažurirati nekompatibilnu verziju dodatka na kompatibilnu verziju kada bude objavljena.
Omogućavanje ove opcije može biti korisno za prelazak na korištenje inačice koja prekida podršku za starije inačice dodataka (prva inačica svake godine).
To je posebno korisno za alpha i beta testere, koji testiraju dodatke u ranoj fazi razvoja prve inačice u godini.

##### Zrcalni poslužitelj {#AddonStoreMetadataMirror}

Ove kontrole vam omogućuju da odredite izvor preuzimanja podataka za Add-on-store.
To može biti korisno na mjestima gedje je poslužitelj NV Access spor ili nedostupan.

Polje koje je samo za čitanje pokazuje trenutnu adresu zrcalnog poslužitelja.
Ako se ne prikazuje niti jedan zrcalni poslužitelj (to jest koristi se NV Access poslužitelj), prikazat će se "nema zrcalnog poslužitelja".

Ako želite promijeniti zrcalni poslužitelj za Add-on store, molimo pritisnite gumb "promijeni...", kako biste otvorili [dijaloški okvir za postavljanje zrcalnog poslužitelja Add-on store](#SetURLDialog).

#### Postavke udaljenog pristupa {#RemoteSettings}

Ova vam kategorija omogućuje postavljanje ponašanja [udaljene podrške](#RemoteAccess).

##### Uključi udaljenu podršku {#RemoteEnable}

Koristite ovaj potvrdni okvir kako biste uključili ili isključili značajku udaljene podrške NVDA.

Kada ovaj potvrdni okvir nije odabran, funkcija udaljene podrške je potpuno isključena.

Imajte na umu, da ako želite nastaviti koristiti daljinski pristup na sigurnim radnim površinama, kao što su dijaloški okviri kontrole korisničkih računa, trebat ćete uključiti ovu postavku, pospremiti postavke, a potom [ih kopirati u mapu postavki sustava NVDA](#GeneralSettingsCopySettings).
Udaljeni pristup na sigurnim radnim površinama bit će nastavljen samo kada se koristi instalirana kopija NVDA.
To znači da ne možete, na primjer, pokrenuti vezu udaljenog pristupa s prijenosnom kopijom NVDA i nastaviti vezu na sigurnim zaslonima.

Slijedeće su opcije dostupne kada je NVDA udaljena podrška uključena.

##### Pitaj prije prekida veze kada je računalo kontrolirano {#RemoteConfirmDisconnect}

Ovom se mogućnošću kontrolira je li potrebna potvrda prije prekida veze sa sesijom daljinskog pristupa kada je povezana kao kontrolirano računalo.

Kada je potvrdni okvir odabran, NVDA će zatražiti potvrdu prije prekida veze.
Kada nije odabran, NVDA će će odmah prekinuti udaljenu vezu bez potvrde.

##### Spoji se automatski poslije pokretanja NVDA {#RemoteAutoconnect}

Ova vam opcija omogućuje automatsko uspostavljanej veze udaljene podrške poslije pokretanja NVDA.
Ovo može biti korisno, na primjer, kada niste kod kuće, a želite kontrolirati svoje računalo.

Oprezno koristite ovu opciju, jer postoji povećana vjerojatnost neautoriziranog pristupa vašem računalu.

Za više pojedinosti o sljedećim opcijama pogledajte [Postavljanje sesije daljinskog pristupa] (#RemoteAccessSetup)

##### Način {#RemoteAutoconnectMode}

Omogućuje odabir načina povezivanja za automatska povezivanja.

Ova je opcija dostupna kada je odabran potvrdni okvir [Poveži se automatski poslije pokretanja NVDA](#RemoteAutoconnect).

|Opcija |Ponašanje |
|---|---|
|Dopusti da ovo računalo bude kontrolirano |Koristite ovo računalo kao "kontrolirano" računalo. To omogućuje osobi koja kontrolira udaljeno računalo da koristi to računalo kao da sjedi ispred njega. |
|Kontroliraj drugo računalo |Koristite ovo računalo kao računalo koje kontrolira drugo računalo. To vam omogućuje izvršavanje naredbi na udaljenom računalu. |

##### Poslužitelj {#RemoteAutoconnectServer}

Služi za odabir tipa poslužitelja za automatsko povezivanje.

Ova je opcija dostupna kada je odabran potvrdni okvir [Poveži se automatski poslije pokretanja NVDA](#RemoteAutoconnect).

|Opcija |Ponašanje |
|---|---|
|Koristi postojeći |Koristi postojeći poslužitelj udaljene podrške za uspostavljanje veze (preporučeno). |
|Postavi lokalno|Koristi ovu kopiju NVDA kao server udaljene podrške (napredno). Za ovu će opciju biti potrebno podešavanje mrežnih postavki, što nije tema ovih uputa. |

##### Poslužitelj {#RemoteAutoconnectHost}

Ovo polje služi za postavljanje adrese poslužitelja kojeg želite koristiti za automatsko povezivanje.
Ako poslužitelj koristi port drukčiji od 6837, odredite ga poslije dvotočke (npr. `primjer.com:1234`).

Ova je opcija dostupna kada je odabran potvrdni okvir [Poveži se automatski poslije pokretanja NVDA](#RemoteAutoconnect), a [opcija poslužitelj](#RemoteAutoconnectServer) je postavljena na "koristi postojeći".

##### Priključak {#RemoteAutoconnectPort}

Koristite ovo polje kako biste postavili TCP priključak za automatsko povezivanje.

Ova je opcija dostupna kada je odabran potvrdni okvir [Poveži se automatski poslije pokretanja NVDA](#RemoteAutoconnect), a [opcija poslužitelj](#RemoteAutoconnectServer) je postavljena na "postavi lokalni".

##### Ključ {#RemoteAutoconnectKey}

Koristite ovo polje za postavljanje ključa (lozinke) koje đelite koristiti za automatsko povezivanje.

Ova je opcija dostupna kada je odabran potvrdni okvir [Poveži se automatski poslije pokretanja NVDA](#RemoteAutoconnect).

##### Izbriši sve pouzdane otiske prstiju {#RemoteDeleteFingerprints}

Ovaaj vam gumb omogućuje brisanje svih prethodno spremljenih pouzdanih otisaka prstiju.
To znači da će vas se ponovno pitati želite li se povezati sa svim neprepoznatim poslužiteljima za daljinski pristup, čak i onima s kojima ste se prethodno povezali.
Trebat ćete potvrditi brisanje svih otisaka prstiju.

Ova je opcija dostupna samo ako u vašoj konfiguraciji postoje pouzdani otisci prstiju.
...
#### Dijaloški okvir za postavljanje zrcalnog poslužitelja {#SetURLDialog}

Ovaj dijaloški okvir omogućuje određivanje adrese zrcalnog poslužitelja koji će se koristiti prilikom [ažuriranja NVDA](#GeneralSettingsCheckForUpdates) ili [korištenja upravitelja dodataka](#AddonsManager).
Ovo je korisno za lokacije na kojima je pristup poslužiteljima NV Access spor ili nedostupan.

* Prilikom postavljanja [zrcalnog poslužitelja za ažuriranje NVDA](#UpdateMirror), naslov ovog dojaloškog okvira bit će "postavi zrcalni poslužitelj za ažuriranje NVDA".
* Prilikom postavljanja [zrcalnog poslužitelja za uravitelj dodataka](#AddonStoreMetadataMirror), naslov ovog dijaloškog okvira bit će "postavi zrcalni poslužitelj za upravitelj dodataka".

##### Adresa {#SetURLTextbox}

Unesite adresu zrcalnog poslužitelja kojeg želite koristiti.
samo HTTP i HTTPS adrese su podržane.
Kada je to god moguće,NV Access preporuča korištenje https adresa radi vaše privatnosti.

Ostavite ovo polje prazno ako želite koristiti podrazumijevani NV Access poslužitelj.

##### Provjeri... {#SetURLTest}

Pritisnite ovaj gumb za testiranje zrcalnog poslužitelja, čiju ste adresu unijeli.
Kako bi provjera uspjela, morate biti povezani s internetom.
Preporučuje se provjera adrese prije njenog spremanja.
...
* Samo tamo gdje je to potrebno: tamo gdje Microsoft Word objektni model nije dostupan
...
##### Odreži početnu tišinu unutar govornog izlaza ili zvuka {#TrimLeadingSilenceSpeech}

Kada je ova opcija uključena, NVDA će ukloniti početnu tišinu unutar govornog izlaza, što može unaprijediti odziv nekih govornih jedniica.
Ova je opcija podrazumijevano omogućena, i trebala bi djelovati samo na tišinu, koja se javlja na početku govornog izlaza.
Ako osjetite da neki intervali tišine nedostaju, na primjer, pauze između dvije rečenice pri korištenju govorne jedinice u NVDA dodatku, možda ćete htjeti isključiti ovu opciju u potpunosti, kako biste riješili problem.

##### Koristi WASAPI za sapi 4 govorni izlaz {#UseWASAPIForSAPI4}

Ova opcija omogućuje Microsoft Speech API version 4 (SAPI 4) glasovima reprodukciju zvuka koristeći Windows Audio Session API (WASAPI).
Ovo omogućuje podršku većeg broja značajki prilikom korištenja sapi 4 glasova, poput utišavanja zvukova, rezanje početne tišine i ostavljanje zvučne kartice budnom.
Međutim, neki glasovi mogu raditi neispravno sa trenutnom Wasapi implementacijom.
Ako otkrijete da vaš sapi 4 glas kojeg koristite prestane raditi, možete isključiti ovu opciju.

| . {.hideHeaderRow} |.|
|---|---|
|Opcije |podrazumijevano (omogućeno), onemogućeno, omogućeno|
|Podrazumijevano |omogućeno|
...
Dakle, obrnute kose crte u zamjeni trebaju biti napisane dva put, na primjer "a\\\\b" treba biti napisano, kako biste dobili "a\\b" kao zamjenu.
...
| Naziv |Tipka za stolna računala |Tipka za prijenosna računala |Opis|
|---|---|---|---|
|spremi konfiguraciju |`NVDA+control+c` |`NVDA+control+c` |Sprema konfiguraciju, tako da ne bude izgubljena poslije ponovnog pokretanja NVDA|
|Vrati konfiguraciju |`NVDA+control+r` |`NVDA+control+r` |Jednostruki pritisak vraća konfiguraciju na zadnju spremljenu konfiguraciju Kada se pritisne tri puta konfiguracija će se vratiti na zadane postavke.|
...
## Daljinska podrška {#RemoteAccess}

Sa ugrađenom značajkom NVDA udaljene podrške, možete kontrolirati udaljeno računalo koje koristi NVDA ili dopustiti nekome kontrolu vašeg računala.
Ovo olkašava pružanje ili primanje pomoći, surađivanje, ili pristup vlastitom udaljenom računalu.

Podrazumijevano, NVDA udaljena podrška je onemogućena radi vaše sigurnosti.
Možete omogućiti udaljeni pristup [u postavkama udaljenog pristupa](#RemoteSettings).

### Brzi početak {#RemoteAccessGettingStarted}

Prije nego što počnete, uvjerite se da je NVDA pokrenut i instaliran na oba računala.
Također provjerite je li potvrdni okvir Omogući daljinski pristup [u postavkama daljinskog pristupa](#RemoteSettings) označen na oba računala.
Značajka daljinskog pristupa tada će biti dostupna iz izbornika Alati u NVDA; Nema potrebe za dodatnim preuzimanjima ili instalacijama.

Ako NVDA radi u [Sigurnom načinu](#SecureMode), ne možete ručno pokrenuti novu sesiju daljinskog pristupa.
Međutim, NVDA će i dalje automatski pokrenuti novu sesiju daljinskog pristupa ako je konfiguriran za to s postavkom [Automatski poveži nakon pokretanja NVDA](#RemoteAutoconnect).
I dalje možete ručno prekinuti vezu u sigurnom načinu rada.

### Postavljanje veze udaljenog pristupa {#RemoteAccessSetup}

Sesije daljinskog pristupa konfiguriraju se putem dijaloškog okvira Veza s daljinskim pristupom.
Otvorite NVDA izbornik i odaberite Alati, zatim "udaljeni pristup", zatim "Poveži...".
Ili s bilo kojeg mjesta pritisnite 'NVDA+alt+r'.

#### Odabir načina rada (onaj koji kontrolira ili kontrolirani) {#RemoteAccessMode}

Morat ćete odlučiti kojim će se računalom upravljati (kontrolirano računalo), a koje (kontrolirajuće računalo).

Odabirni okvir modus omogućuje vam da odaberete hoće li se ovim računalom upravljati (druga osoba će komunicirati s vašim računalom) ili kontrolirati (vi ćete komunicirati s tuđim računalom).

* Ako vam je potrebna druga osoba da vam pomogne s računalom, odaberite "Dopusti upravljanje ovim računalom".
* Ako ćete nekome pomoći s njegovim računalom, odaberite "Kontroliraj drugo računalo".

#### Povezivanje s postojećim poslužiteljem (osigurani host i ključ) {#RemoteAccessJoin}

U tom je slučaju druga osoba ili pokrenula vezu putem relejnog poslužitelja ili je lokalno hostirana vezom.

1. Za Poslužitelj odaberite "Koristi postojeće".
1. U polje Glavno računalo unesite adresu glavnog računala (i priključak ako je također naveden).
  * Port za povezivanje možete uključiti dodavanjem dvotočke (":") i broja porta adresi hosta.
Na primjer, `primjer.com:1234`.
1. U polje Ključ unesite priloženi ključ.
Pazite da tipku unesete točno onako kako je navedeno.
1. Pritisnite U redu.

#### Pokretanje veze putem relejnog poslužitelja {#RemoteAccessConnectRelay}

Ako dopuštate drugoj osobi da se poveže s vama putem interneta, najlakša opcija je korištenje relejnog poslužitelja. Da biste to učinili:

1. Za Poslužitelj odaberite "Koristi postojeće".
1. Za glavno računalo navedite naziv glavnog računala poslužitelja za prijenos, kao što je 'nvdaremote.com'.
Morat ćete dati ovu adresu drugoj osobi.
  * Neobavezno možete dodati priključak za spajanje tako da dodate dvotočku (":") i broj priključka poslije adrese poslužitelja.
Na primjer, `primjer.com:1234`.
1. Za ključ: smislite dugački ključkoji je teško pogoditi ili pritisnite tipku "Generiraj ključ" da bi se automatski generirala.
To ćete morati dati drugoj osobi.
  * Imajte na umu da ključ djeluje i kao identifikator za sesiju i kao lozinka, pa je vrlo važno da je teško pogoditi.
1. Pritisnite U redu.
1. Navedite pojedinosti o vezi (kao što su glavno računalo i ključ) drugoj osobi kako bi se mogla povezati.

#### Korištenje NVDA kao poslužitelja {#RemoteAccessConnectSelfHosted}

Ako ne želite koristiti relejni poslužitelj ili se povezujete putem lokalne mreže:

1. Za Poslužitelj odaberite "Host lokalno".
1. Ako se povezujete putem interneta, možete pritisnuti gumb "Get External IP" da biste prikazali svoju vanjsku IP adresu (vidljivu na internetu).
Morat ćete dati ovu vanjsku IP drugu osobu.
1. Ako se povezujete putem lokalne mreže, morat ćete drugoj osobi dati svoj lokalni IP.
pronalaženjetoga nije obuhvaćeno ovom dokumentacijom.
1. U odjeljku Port unesite priključak koji želite koristiti za dolazne daljinske veze.
To će trebati i drugoj osobi.
  * Ovo bi trebao biti priključak koji ne koriste nikakve druge usluge.
  * Prema zadanim postavkama, daljinski pristup koristi port 6837.
  * Ako se povezujete putem interneta, port će možda trebati proslijediti.
  Ova dokumentacija ne pokriva kako prosljeđivati portove.
  Možda ćete se morati obratiti mrežnom administratoru da biste to učinili.
1. Za ključ: smislite dugački ključkoji je teško pogoditi ili pritisnite tipku "Generiraj ključ" da bi se automatski generirala.
To ćete morati dati drugoj osobi.
  * Imajte na umu da ključ djeluje i kao identifikator za sesiju i kao lozinka, pa je vrlo važno da je teško pogoditi.
1. Pritisnite U redu.
...
### Korištenje daljinskog pristupa {#RemoteAccessUsage}

Nakon što je sesija daljinskog pristupa aktivna, možete se prebacivati između upravljanja udaljenim računalom i vlastitim, zajednički koristiti međuspremnik i isključiti zvuk udaljene sesije:

* Pritisnite 'NVDA+alt+tab' za prebacivanje između upravljanja udaljenim računalom i povratka na vlastito računalo.
* Pošaljite tekst iz međuspremnika na drugo računalo tako da otvorite NVDA izbornik, zatim odaberete Alati, zatim "udaljeni pristup", a zatim "Pošalji međuspremnik".
* Ako ste spojeni kao upravljačko računalo, isključite zvuk govora udaljenog računala na lokalnom računalu tako da otvorite NVDA izbornik, zatim odaberete Alati, zatim "Daljinski pristup", a zatim "Isključi daljinski".
* Ako ste spojeni kao kontrolno računalo, pošaljite 'control+alt+delete' upravljanom računalu tako da otvorite NVDA izbornik, zatim odaberete Alati, zatim "Daljinski pristup", zatim "Pošalji kontrolu+alt+delete".
  Da bi to funkcioniralo, kontrolirano računalo mora imati instaliranu kopiju NVDA.
  Osim toga, ovisno o postavkama upravljanog računala, ova funkcija možda neće uvijek raditi.

### Kratak pregled prečaca NVDA udaljene podrške {#RemoteAccessGestures}

<!-- KC:beginInclude -->
| Naziv |Prečac |Opis|
|---|---|---|
| Uključi ili isključi udaljenu vezu | `NVDA+alt+r` | Pokreće udaljenu vezu, ili ako je u tije,u prekida ju. |
| Prebacuj kontrolu | `NVDA+alt+tab` | Prebacuje između lokalnog i udaljenog računala |
| Spoji se | nema | Stvara novu udaljenu vezu. Nedostupno [u sigurnom načinu rada](#SecureMode). |
| Kopiraj poveznicu | nema | Kopira poveznicu udaljene poveznice u međuspremnik. |
| Odspoji se | nema | Prekida vezu postojeće udaljene podrške.. |
| Utišaj udaljenu podršku | Nema | Utišava ili odtišava govor koji dolazi s udaljenog računala. |
| Pošalji međuspremnik | Nema | Šalje sadržaj međuspremnika na udaljeno računalo. |
<!-- KC:endInclude -->
...
#### Razvrstavanje popisa dodataka po stupcima {#AddonStoreSortByColumn}

Zadano, dodaci su poredani po svojim nazivima.
Odabirni okvir"Sortiraj po stupcu" može se koristiti za sortiranje popisa prema dostupnim stupcima za svaku karticu svojstva.
Na primjer, možda ćete htjeti sortirati dodatke prema izdavaču, dostupnoj verziji itd.
Dodaci mogu biti poredani po uzlaznom ili silaznom redoslijedu.
...
Ako preuzimanje ili instalacija ne uspije, možete ponovno pokušati s instalacijom.
Također je moguće otkazati instalaciju prije izlaska iz upravitelja dodataka.
...
Poslije pokretanja NVDA, podrazumjevano ćete biti obavješteni o ažuriranjima dodataka.
Kako biste saznali više i podesili ovu opciju, pogledajte ["obavijesti o ažuriranjima"](#AutomaticAddonUpdates).
...
Dodatak možete onemogućiti ako status dodatka pokazuje da je "omogućen" ili ga omogućiti ako je dodatak "onemogućen".
...
#### Promjena kanala automatskog ažuriranja {#AddonStoreUpdateChannel}

Kanalima automatskog ažuriranja dodataka možete upravljati iz [kartice instaliranih i ažuriranih dodataka](#AddonStoreFilterStatus).
Kada su omogućena[Automatska ažuriranja dodataka](#AutomaticAddonUpdates), dodaci će se ažurirati na isti [kanal](#AddonStoreFilterChannel) s kojeg su instalirani na [podrazumijevani](#DefaultAddonUpdateChannel).
Iz izbornika radnji dodatka, pomoću podizbornika "Ažuriraj kanal", možete izmijeniti kanale na koje će se dodatak automatski ažurirati.

| Opcija | ponašanje |
|---|---|
| Zadano | Dodatak će slijediti [zadani kanal za ažuriranje](#DefaultAddonUpdateChannel) |
| Isto | Dodatak će ostati na istom kanalu |
| Bilo koji | nadogradit će se na posljednju inačicu, bez obzira na kanal |
| Ne ažuriraj | Dodatak se neće automatski ažurirati |
| stabilni | dodaci će se automatski ažurirati na stabilne inačice |
| Beta ili dev | dodaci će se automatski ažurirati na beta ili dev verzije |
| Beta | Dodaci će se automatski nadograđivati na beta verzije |
| Dev | dodaci će se automatski nadograđivati na dev verzije |
...
Ponekad se mogu pojaviti problemi s registrom sustava Windows, što rezultira nenormalnim ponašanjem NVDA.
To može biti uzrokovano, na primjer, instaliranjem ili deinstaliranjem određenih programa (kao što su Adobe Reader ili Math Player), kao i ažuriranjima sustava Windows i drugim događajima.
COM alat za popravljanje registracije pokušava riješiti te probleme popravljanjem unosa pristupačnosti u registru.

Vrste problema koje ovaj alat može riješiti uključuju:

* NVDA čita "nepoznato" ili "okno" prilikom kretanja u preglednicima kao što su Firefox ili Edge, e-mail programima kao što su Thunderbird, te u Windows Exploreru, programskoj traci i drugim programima.
* NVDA se ne može prebaciti između modusa fokusa i modusa čitanja kada to očekujete.
* Gumbi koji su prethodno imali izgovorena imena, odjednom se čitaju samo kao "gumb".
* NVDA je vrlo spor pri navigaciji u preglednicima dok koristite način pregledavanja.

Budući da ovaj alat ispravlja unose u registru sustava Windows, za rad mu je potreban administrativni pristup, baš kao i kod instaliranja programa.
Ako imate omogućen UAC (kontrolu korisničkog računa), kao što to čini većina korisnika, morat ćete slijediti sve upute koje prikazuje UAC, kako biste uspješno pokrenuli alat.
...
Za još opsežniji popis besplatnih i komercijalnih govornih jedinica koje možete kupiti i preuzeti za korištenje s NVDA, pogledajte [stranicu s dodatnim glasovima](https://github.com/nvaccess/nvda/wiki/ExtraVoices).
...
* Tivomatic Caiku Albatross 46/80 brajični redcj
...
Serije brajičnih redaka Brailliant BI i B tvrtke [HumanWare](https://www.humanware.com/), uključujući BI 14, BI 32, BI 20X, BI 40, BI 40X i B 80, podržane su kada su povezane putem USB-a ili Bluetootha.
...
* NLS eReader
  * Imajte na umu da Zoomax trenutno nije podržan bez vanjskih upravljačkih programa

Slijede dodijeljeni tipkovnički prečaci za brajičn redkeBrailliant BI/B i BrailleNote s NVDA.
Smještaj ovih tipki potražite u dokumentaciji brajičnog retka.
...
Za BrailleNote Touch pogledajte odjeljak [Brailliant BI Series / BrailleNote Touch](#HumanWareBrailliant).
...
|Strelica lijevo |razmaknica+točkica3, joystick lijevo ili dpad lijevo|
...
Dot Pad tvrtke Dot Inc je uređaj koji može prikazati taktilnu grafiku i Brailleovo pismo.
NVDA može prikazati ili jednu liniju brajice na liniji za prikaz brajice namijenjenu uređaju ili više linija brajice na svom taktilnom grafičkom području.
Model A300 ima taktilno grafičko područje od 120 x 80 točaka, u koje može stati 8 redaka od po 20 ćelija.

Možete podesiti hoće li NVDA prikazivati Brailleovo pismo na namjenskoj liniji za prikaz brajice ili na taktilnom grafičkom području putem opcije Odredište brajice u NVDA postavkama Brajice za ovaj brajični redak.

Podržane su tipke za pomicanje, ali zbog ograničenih gumba na uređaju druge naredbe i mogućnosti usmjeravanja trenutno nisu dostupne.

Prilikom odabira upravljačkog programa Dot Pad u NVDA, morate ručno odabrati USB / Bluetooth virtualni serijski priključak na koji je Dot Pad spojen.
Ovaj upravljački program ne podržava automatsko otkrivanje.

Imajte na umu da zbog hardverskih ograničenja, Dot Pad neće ispravno osvježiti sve točke dok je vaša ruka na uređaju.
Obavezno podignite ruku s uređaja u potpunosti dok koristite NVDA čitač zaslona i počnite ponovno čitati tek nakon što se sadržaj na brajičnom redku potpuno ažurira.

<!-- KC:beginInclude -->

| Naziv |prečac|
|---|---|
|Pomakni brajični redak u nazad | `pan_left` |
|pomakni brajični redak u naprijed | `pan_right` |
...
* Otvaranje vanjskih dokumenata iz NVDA izbornika, kao što je datoteka korisničkog priručnika.
...
|`-n jezik` |`--lang=jezik` |Nadpisuje trenutni jezik NVDA. Postavite na "Windows" za trenutno korisnički zadan jezik, "en" za engleski itd.|
|`-m` |`--minimal` |bez zvuka, bez sučelja, bez poruke dobrodošlice itd|
|`-s` |`--secure` |Pokreće NVDA u [sigurnom načinu](#SecureMode)|
|`-d` |`--disable-addons` |Dodaci se neće izvršavati|
...
|Nema |`--create-portable` |Stvara prijenosnu kopiju NVDA i pokreće ju. Zahtijeva definiran `--portable-path`|
|Nema |`--create-portable-silent` |Stvara prijenosnu kopiju NVDA (bez pokretanja nove kopije). zahtijeva određen`--portable-path`. Ova opcija zaobilazi upozorenja prilikom pisannja u mape koje nisu prazne, te može nadpisivati datoteke.|
|nema |`--portable-path=PORTABLEPATH` |Putanja za stvorenu prijenosnu kopiju|

Baš kao što možete tiho instalirati NVDA proslijeđivanjem opcije naredbenog retka `--install-silent` NVDA-, tako se može tiho deinstalirati prosljeđivanjem naredbe `/S` deinstalacijskom programu.

Datoteka NVDA deinstalacijskog programa nosi naziv `uninstall.exe` i nalazi se u NVDA instalacijskoj mapi, `%ProgramFiles(x86)%\nvda` u 64-bitnim Windowsimas, ili `%ProgramFiles%\nvda` u 32-bitnim Windowsima.

Slijede opcije naredbenog retka za NVDA deinstalacijski program:

| Kratka |Duga |Opis|
|---|---|---|
|`/S` |Nema |Tiho uklanja NVDA. |
