...
Denne udgivelse introducerer NVDA Fjernsupport, så du kan styre en fjerncomputer, der kører NVDA fra en anden enhed, der kører NVDA.

Der er mange forbedringer af talen, særligt i forhold til reaktionstid for SAPI 4, SAPI 5 og OneCore-stemmer.
Øget talehastighed og automatisk sprogskift understøttes nu i SAPI 5.
Stemmer, der bruger SAPI 4, understøtter nu lyddæmpning, fjernelse af indledende stilhed og forhindring af, at lydenheden går i dvale.

Tilføjelsescenterets automatiske opdateringssystem er blevet forbedret, så du kan vælge kanaler til automatiske opdateringer, og køre automatiske opdateringer i baggrunden.

Nye kommandoer er blevet tilføjet for manuelt at opdatere resultatet for tekstgenkendelse, og for at slå automatisk regelmæssig opdatering til og fra.

Oprindelig markeringstilstand er nu også tilgængelig i Chrome og Edge.

Understøttelse for Microsoft Office og LibreOffice er forbedret. Flere genveje bliver nu oplæst.

Standard input og output punkttabeller kan nu bestemmes baseret på NVDAs aktuelle sprog.
Understøttelse tilføjet for Dot Pad tactile graphics fra Dot Inc, hvilket er et punktdisplay med flere linjer.
Der er andre rettelser for punktdisplays, der omfatter forbedringer af forbindelsen.

LibLouis punktoversætter, eSpeak-NG og Unicode CLDR er blevet opdateret.
Punkttabeller for koptisk, kompakte Kildesprog, og portugisiske 6 punkt Computer Braille er nu tilgængelige.
...
* Forhindrer visning af potentielt følsomme oplysninger på punktdisplays, når enheden er låst.
([GHSA-8f8q-2jc3-6rf4](https://github.com/nvaccess/nvda/security/advisories/GHSA-8f8q-2jc3-6rf4))
* Forhindrer installationsprogrammet i at indlæse uønskede DLL'er fra sin mappe.
([GHSA-qf5h-qw92-rx2f](https://github.com/nvaccess/nvda/security/advisories/GHSA-qf5h-qw92-rx2f))

### Nye Funktioner

* Tilføjelsescenteret:
  * Automatiske opdateringer (# 3208):
    * Automatiske opdateringskanaler til tilføjelser kan nu ændres.
      * Kanaler til automatiske opdateringer kan vælges via undermenuen "Kanal til automatiske opdateringer".
      * Kanalen, der bruges som standard, kan indstilles i NVDAs indstillinger.
    * Automatiske opdateringer kan nu foregå i baggrunden.
      * Dette kan aktiveres i NVDAs indstillinger ved at gå til Tilføjelsescenter og ændre indstillingen "Automatiske opdateringer" til "Opdater automatisk".
    * Automatiske opdateringer kan nu opdatere inkompatible tilføjelser til en anden nyere kompatibel version.
      * Dette kan aktiveres i Tilføjelsescenter i NVDAs indstillinger.
  * Tilføjet en handling, der kan annullere installationen af tilføjelser. (# 15578, @ hwf1324)
  * Tilføjet en handling, der vil forsøge at installere tilføjelsen igen, hvis download/installation af en tilføjelse mislykkes. (# 17090, @hwf1324)
  * Tilføjelseslisterne kan sorteres efter kolonner, herunder publiceringsdato, i stigende og faldende rækkefølge. (# 15277, # 16681, @ nvdaes)
* Tale:
  * Automatisk sprogskift understøttes nu, når du bruger Microsoft Speech API version 5 (SAPI5) og Microsoft Speech Platform stemmer. (# 17146, @gexgd0419)
  * Øget talehastighed understøttes nu, hvis du bruger Microsoft Speech API version 5 (SAPI5) og Microsoft Speech Platform, der understøtter helt op til 6X hastighed. (#17606, @gexgd0419)
* Webbrowsere:
  * NVDAs oprindelige markeringstilstand (`NVDA+shift + f10`) understøttes nu i Google Chrome, Edge og andre programmer baseret på Chromium 134 eller nyere. (#17838)
  * NVDA vil nu rapportere den fremhævede tekst, når en URL, der indeholder et tekstfragment, er besøgt i Mozilla Firefox. (# 16910, @ jcsteh)
  * NVDA kan nu rapportere, hvis et link peger til noget på den aktuelle side. (#141, @LeonarddeR, @nvdaes)
* Microsoft Office:
  * Når du redigerer i tekstbokse i PowerPoint, kan du nu bruge `alt+pil op`/`alt+pil ned` for at bevæge dig mellem sætninger. (#17015, @LeonarddeR)
  * I Word er opdatering af markeringen nu rapporteret, når du bruger Word-kommandoer til at udvide eller formindske markeringen (`f8` eller `shift+f8`). (# 3293, @CyrilleB79)
  * I Word 16.0.18226 og nyere, eller når du bruger Word-objektmodellen, vil NVDA nu rapportere, hvis en overskrift er sammenfoldet i både tale og braille. (# 17499)
  * Bevægelse af systemmarkøren rapporteres nu, når de oprindelige udvidede tabelnavigationskommandoer bruges (`alt+home`, `alt+end`, `alt+pageUp`, og `alt+pageDown`), og UIA er aktiv. (#17867, @CyrilleB79)
  * I Word bliver dokumentlayoutet nu rapporteret, når kommandoen "rapportér fokus" bruges – forudsat at denne information er tilgængelig, og at objektbeskrivelser er aktiveret. (#15088, @nvdaes)
  * Resultatet af flere genveje er nu rapporteret i Word og Outlook:
    * genveje for skriftyper (#10271, @CyrilleB79)
    * sammenfold eller udvid overskrift (#17545, @CyrilleB79)
* LibreOffice:
  * Når skriftstørrelsen øges eller mindskes i Writer ved hjælp af de tilhørende tastaturgenveje, annoncerer NVDA den nye skriftstørrelse. (#6915, @michaelweghorn)
  * Når typografien "Brødtekst" eller en overskriftstypografi anvendes i Writer 25.2 eller nyere ved hjælp af den tilhørende tastaturgenvej, vil NVDA annoncere den nye typografi. (#6915, @michaelweghorn)
  * Når dobbelt understregning slås til eller fra i Writer ved hjælp af den tilhørende tastaturgenvej, annoncerer NVDA den nye tilstand ("dobbelt understregning til"/"dobbelt understregning fra"). (#6915, @michaelweghorn)
  * NVDA kan nu rapportere første linjeindrykning på en linje i LibreOffice 25.8 og nyere. (#13052, @michaelweghorn)
* Tekstgenkendelse:
  * I et genkendelsesresultat, vil `NVDA+f5` manuelt opdatere det genkendte indhold. (# 17715, @CyrilleB79)
  * Tilføjet en utilknyttet kommando, der slår automatisk opdatering af genkendelsesresultatet til og fra. (#16897)
* Fjernstyringsfunktionalitet, baseret på NVDA Remote-tilføjelsen, er nu integreret i NVDA. (#4390, @ctoth, @tspivey, @daiverd, NVDA Remote-bidragsydere og crowdfundere)
* Det er nu muligt at benytte et link til en spejlserver for opdateringer for tilføjelser i Tilføjelsescenteret. (#14974, #17151, #17310, @christopherpross)
* Tilføjet understøttelse for matematik i PDF. (#9288, @NSoiffer)
  * Dette fungerer for formler med tilknyttet MathML, såsom nogle filer genereret af nyere versioner af TeX/LaTeX.
  * Dette understøttes i øjeblikket kun i Foxit Reader og Foxit Editor.
* NVDA kan nu konfigureres til at læse den aktuelle linje eller det aktuelle afsnit op, når du ruller displayet. (#17053, @nvdaes)
* NVDA oplyser nu systemmarkørens bevægelse, når der trykkes `alt+upArrow` eller `alt+downArrow` i eksempelvis Visual Studio Code. (#17652, @LeonarddeR)
* Tilføjede kommandoer til at flytte læsemarkøren til det første og sidste tegn i den valgte tekst, tildelt henholdsvis `NVDA+alt+home` og `NVDA+alt+end`. (# 17299, @nvdaes)
* Tilføjet en generel indstilling for at forhindre, at punktdisplayet går ud ved brug af sig alt eller læsning via punktskrift.
Denne indstilling er som standard aktiveret, men kan øge brug af batteriet. (#17649, @LeonarddeR)
* NVDA er nu oversat til bosnisk. (# 17953)
* I Adobe Acrobat kan NVDA nu læse og interagere med matematiske ligninger i PDF-dokumenter genereret af nyere versioner af Microsoft Word. (#18056)

### Ændringer

* Komponentopdateringer:
  * Opdateret LibLouis punktoversættelse til [3.33.0](https://github.com/liblouis/liblouis/releases/tag/v3.33.0). (#17469, #17768, @LeonarddeR, @codeofdusk)
    * Punkttabeller for koptisk, kompakte Kildesprog, og portugisiske 6 punkt Computer Braille er nu tilgængelige.
  * Opdateret CLDR til version 46.0. (#17484, @OzancanKaratas)
  * Opdateret eSpeak NG til 1.52.0. (#17056)
* Ændringer i NVDA-grænseflade:
  * Installationsprogrammet for NVDA afspiller ikke længere en lyd, når det starter. (#14068)
  * Dialogerne Rapportér linkdestination, Tegnformateringsoplysninger og Læs markering indeholder nu knapperne "Luk" og "Kopiér". (#17018, @XLTechie)
  * Dialogen til afslutning af NVDA lader dig nu genstarte NVDA med tilføjelser deaktiveret samt fejllogning aktiveret samtidigt. (#11538, @CyrilleB79)
  * Mulighed for ikke at benytte WASAPI til lydoutput er nu fjernet. (#16080)
  * Indstillingen "Indstil automatisk systemfokus til fokuserbare elementer" i gennemsynstilstand er blevet fjernet, dette er nu deaktiveret. (#17598)
  * NVDA advarer nu kun om inkompatible tilføjelser, når der opdateres til en ny version, som har en tilføjelses-API, der er inkompatibel med den nuværende version. (#17071, #17506)
  * Menupunktet "Bidragydere" er fjernet fra NVDA-menuen. (#16922)
  * NVDA-licensen er nu formateret i HTML for at forbedre læsbarheden. (#17600)
  * Korte versioner af de mest brugte kommandolinjeparametre er blevet tilføjet: `-d` for `--disable-addons` og `-n` for `--lang`.
  Præfiksmatching af kommandolinjeflag, f.eks. brug af `--di` i stedet for `--disable-addons`, understøttes ikke længere. (#11644, @CyrilleB79)
  * Ændringer til værktøjet for rettelser af COM-registreringer: (#12355, @XLTechie)
    * Værktøjet viser nu først en besked om dets anvendelse i stedet for en advarsel. (#12351)
    * Det første vindue, der kommer frem, kan nu lukkes ved brug af `escape` eller `alt+f4`. (#10799)
    * Værktøjet viser nu en besked til brugeren, inklusive fejlen, i det sjældne tilfælde af en Windows-fejl under forsøg på COM-genregistrering.
* Tale:
  * Microsoft Speech API version 5 og Microsoft Speech Platform voices bruger nu WASAPI for lydoutput, hvilket kan forbedre ydeevnen med disse stemmer. (#13284, @gexgd0419)
  * Indledende stilhed er nu fjernet, når du bruger stemmer fra OneCore eller SAPI5, samt nogle talesynteser, der er installeret via tilføjelsespakker i NVDA. Dette kan øge reaktionshastigheden. (#17614, @gexgd0419)
  * Stemmer, der bruger Microsoft Speech API Version 4, benytter nu WASAPI for lydoutput. Dette betyder, at de kan bruge funktioner som lyddæmpning, fjernelse af indledende stilhed, og kan forhindre, at lydenheder går i dvale.
  Hvis dette ikke virker med din SAPI 4 stemme, kan du deaktivere WASAPI til SAPI 4 i Avancerede indstillinger. (#17718, #17801, @gexgd0419)
  * Unicode Normalisering er nu aktiveret som standard for taleoutput. (#17017, @ LeonarddeR).
    * Du kan stadig deaktivere denne funktionalitet i talekategorien i NVDA indstillingsdialogen.
* Tastaturindstillingerne for "Oplæs indtastede tegn" og "Oplæs indtastede ord" har nu tre valgmuligheder: Fra, Kun i redigeringsfelter og Altid. (#17505, @Cary-rowen)
  * Som standard er "Oplæs indtastede tegn" indstillet til "Kun i redigeringsfelter".
* Punkttabeller, der bruges som standard, kan nu bestemmes ved brug af NVDAs aktuelle sprog. (#17306, #16390, #290, @nvdaes)

### Fejlrettelser

* Tale:
  * Rettet en fejl, der forårsagede, at nogle SAPI5-talesynteser ophørte med at læse ved slutningen på en linje under brug af "Sig alt". (#16691, @gexgd0419)
  * Stemmeparametre, såsom hastighed og lydstyrke, nulstilles ikke længere til standard, når man bruger ringen af indstillinger for talesyntese til at skifte mellem stemmer for SAPI5- og SAPI4. (#17693, #2320, @gexgd0419)
  * Rettet et problem, hvor nogle SAPI4-stemmer (f.eks. IBM TTS Chinese) ikke kunne indlæses. (#17726, @gexgd0419)
* Punkt:
  * Understøttelse tilføjet for Dot Pad tactile graphics fra Dot Inc, hvilket er et punktdisplay med flere linjer. (#17007)
  * Ved brug af Seika Notetaker vises kommandoer med mellemrum og mellemrum med punkter nu korrekt i dialogen Håndter Kommandoer. (#17047, @school510587)
  * NVDA burde ikke længere overfylde logfilen, når punktindstillingen "Vis taleoutput" benyttes uden et punktdisplay tilsluttet. (#17092, @Emil-18)
  * Når "Standard HID-punktdisplay" er valgt som punktdisplay-driver, viser NVDA nu korrekt denne driver som den valgte i listen over punktdisplays, i stedet for at indikere, at ingen driver er valgt. (#17537, @LeonarddeR)
  * Humanware Brailliant-driveren er nu mere pålidelig til at vælge det korrekte forbindelsespunkt, hvilket resulterer i mere stabil forbindelse og færre fejl. (#17537, @LeonarddeR)
  * Brugertilpassede punkttabeller i "Developer Scratchpad" ignoreres nu, når Nvda køres med tilføjelsesprogrammer deaktiveret. (#17565, @LeonarddeR)
  * Rettet et problem, hvor nogle USB-punktdisplays ikke blev detekteret af NVDA. (#18114, @christiancomaschi)
* Microsoft Office:
  * Kommandoen til at få oplyst den aktuelle webadresse for et link fungerer nu også, når den forældede objektmodel anvendes i Word, Outlook, Excel og PowerPoint. (#17292, #17362, #17435, @CyrilleB79)
  * I Excel viser elementlisten (`NVDA+f7`) nu korrekt kommentarer og formler på visse ikke-engelske systemer, hvor det tidligere mislykkedes. (#11366, @CyrilleB79)
  * Forbedringer i PowerPoint:
    * Markørrapportering fejler ikke længere, når teksten indeholder brede tegn som f. eks. emoji. (#17006, @LeonarddeR)
    * Oplysning af positionen for et tegn er nu nøjagtig (f.eks når der trykkes `NVDA+delete`. (#9941, @LeonarddeR)
    * NVDA vil ikke automatisk begynde oplæsning, når et diasshow starter og indstillingen "Automatisk Sig Alt efter indlæsning af side" er deaktiveret i Gennemsynstilstand. (#17488, @LeonarddeR)
* LibreOffice:
  * Når man bruger Microsoft Pinyin-indtastningsmetoden til kinesisk og aktiverer Pinyin-kompatibilitetsindstillingen for at bruge den tidligere version, forårsager indtastning i Writer (og muligvis andre programmer) ikke længere en fejl, mens en IME-popup vises. (#17198, @michaelweghorn)
  * Når en check box i en dialog vises som markeret eller ikke markeret, vil dette også blive vist på punkt (ikke kun oplæst). (#17218, @michaelweghorn)
* Matematik:
  * Læsning af matematik virker nu for nogle webelementer.
  Specifikt MathML inden i span og andre elementer, der har attributten `role="math"`. (#15058)
  * Matematiske formler, der kun vises som billeder med alternativ tekst og uden MathML til detaljeret navigation, behandles nu som almindelige billeder i stedet for som matematik uden indhold. Brugeren kan nu springe til dem med `g` og bevæge sig gennem den alternative tekst tegn for tegn med piletasterne. (#16007)
* IDE'er:
  * NVDA går ikke længere ned, når al tekst vælges i visse kildefiler i Android Studio eller IntelliJ Idea. (#17418, @thgcode)
  * NVDA overtager ikke længere `alt+pil op` og `alt+pil ned`-kommandoerne til sætningsnavigation i Visual Studio Code. (#17082, @LeonarddeR)
  * Visse section-elementer i Visual Studio Code genkendes nu korrekt som redigerbare kontrolelementer. (#17573, @Cary-rowen)
  * I Notepad, Notepad++ (på Windows 11) og andre UIA-baserede dokumenter flytter kommandoen "næste linje" på punktdisplayet nu markøren til den sidste linje, selv hvis den er tom.
  Markøren vil altid blive flyttet til slutningen af linjen, hvis den befinder sig på sidste linje og kommandoen bruges. (#17251, #17430, @nvdaes)
* Indstillingsprofiler:
  * Punktskrift ophører ikke længere med at fungere, hvis man aktivere "Sig alt" via en indstillingsprofil. (#17163, @LeonarddeR)
  * Rettet en fejl hvor visse indstillinger blev gemt direkte i den aktive indstillingsprofil, selv når værdien var identisk med værdien i den generelle indstillingsprofil. (#17157, @leonarddeR)
* Når stavefunktionalitet benyttes, virker unicode-normalisering mere hensigtsmæssig:
  * NVDA vil ikke længere fejlagtigt oplyse, at efterfølgende tegn er normaliserede, hvis det kun gælder det første tegn. (#17286, @LeonarddeR)
  * Sammensatte tegn (såsom é) rapporteres nu korrekt. (#17295, @LeonarddeR)
* NVDA kan nu læse undermenuerne, der vises som popups på søgesiden i Thunderbird. (#4708, @thgcode)
* Værktøjet til rettelse af COM-registreringer melder ikke længere succes, hvis værktøjet fejlede. (#12355, @XLTechie)
* NVDA annoncerer ikke længere indholdet af Udklipshistorik i Windows 11, når vinduet lukkes med elementer til stede. (#17308, @josephsl)
* Hvis plugins genindlæses, mens en meddelelse i gennemsynstilstand er åben, vil NVDA ikke længere undlade at oplyse, når fokus flyttes efterfølgende. (#17323, @CyrilleB79)
* Når programmer som Skype, Discord, Signal og Telefonlink bruges til lydkommunikation, dæmpes NVDA's tale og lyde ikke længere. (#17349, @jcsteh)
* Åbning af NVDA's Python-konsol mislykkes ikke længere, hvis der opstår en fejl under hentning af snapshot-variabler. (#17391, @CyrilleB79)
* I webbrowsere rapporteres ændringer i tekstmarkering nu altid korrekt i redigerbare tekstfelter. (#17501, @jcsteh)
* Når ankerlinks peger på det samme objekt, som den virtuelle markør står på, undlader NVDA ikke længere at rulle til linkets destination. (#17669, @nvdaes)
* Ikonet for NVDA Fremhævelsesvinduet forbliver ikke længere fastgjort til proceslinjen efter en genstart af Stifinder. (#17696, @hwf1324)

### Ændringer for udviklere

Se [udviklervejledningen](https://www.nvaccess.org/files/nvda/documentation/developerGuide.html#API) for information om NVDA's proces for udfasning og fjernelse af API'er.
...
* Komponentopdateringer:
  * Opdateret Ruff til 0.8.1. (#17102, #17260, #17473)
  * Opdateret Comtypes til 1.4.6. (#17061, @LeonarddeR)
  * Opdateret wxPython til 4.2.2. (#17181, @dpy013)
  * Opdateret SCons til 4.8.1. (#17254)
  * Opdateret sphinx til 8.1.2 og sphinx-rtd-theme til 3.0.1. (#17284, @josephsl)
  * Opdateret Robot Framework til 7.1.1. (#17329, @josephsl)
  * Opdateret configobj til 5.1.0 commit `8be5462`. (#17328)
  * Opdateret pre-commit til 4.0.1. (#17260)
  * Opdateret typing-extensions til 4.12.2. (#17438, @josephsl)
  * Opdateret licensecheck til 2024.3. (#17440, @josephsl)
  * Opdateret markdown til 3.7. (#17459, @josephsl)
  * Opdateret nh3 0.2.19. (#17465, @josephsl)
  * Opdateret nuitka til 2.5.4. (#17458, @josephsl)
  * Opdateret schedule til 1.2.2. (#17455, @josephsl)
  * Opdateret requests til 2.32.3. (#17456, @josephsl)
* `ui.browseableMessage` kan nu kaldes med muligheder for at præsentere en knap til kopiering til udklipsholderen, og / eller en knap til at lukke vinduet. (#17018, @XLTechie)
* Flere tilføjelser til at identificere linktyper (#16994, @LeonarddeR, @nvdaes)
  * Et nyt `utils.urlUtils` modul med forskellige funktioner til at identificerer linktyper
  * Et nyt `INTERNAL_LINK` state er blevet tilføjet til `controlTypes.states.State`
  * En ny `linkType` egenskab er tilføjet `NVDAObject`.
  Det spørger `treeInterceptor` som standard, hvis nogen.
  * `BrowseModeTreeInterceptor` objekt har en ny `documentUrl` egenskab
  * `BrowseModeTreeInterceptor`-objektet har fået en ny metode, `getLinkTypeInDocument`, som modtager en URL for at kontrollere objekts linktype
  * En `toggleBooleanValue` helper-funktion er blevet tilføjet `globalCommands`.
  Den kan bruges i scripts til at oplyse resultatet, når en boolesk værdi ændres i `config.conf`
* Kravet om at indrykke funktionsparameterlister med to tabulatorer er fjernet fra NVDA's kodestandarder for at være kompatibel med moderne automatisk lintning. (#17126, @XLTechie)
* [VS Code-workspace configuration for NVDA](https://nvaccess.org/nvaccess/vscode-nvda) er tilføjet som et git-submodul. (#17003)
* En ny funktion, `gui.guiHelper.wxCallOnMain`, er tilføjet. Den gør det muligt at kalde wx-funktioner sikkert og synkront fra tråde uden for GUI'en og få returværdien. (#17304)
* Et nyt API til beskeddialoger er tilføjet i `gui.message`. (#13007)
  * Tilføjet klasser: `ReturnCode`, `EscapeCode`, `DialogType`, `Button`, `DefaultButton`, `DefaultButtonSet`, `MessageDialog`.
* I modulet `brailleTables` er funktionen `getDefaultTableForCurrentLang` blevet tilføjet. (#17222, @nvdaes)
* Dataklassen `updateCheck.UpdateInfo` er tilføjet og indeholder metadata om NVDA-opdateringer. (#17310, @christopherpross)
* Indhentning af `labeledBy` egenskab virker nu for:
  * objekter i programmer, der implementerer IAccessible2-relationen `labelled-by`. (#17436, @michaelweghorn)
  * UIA-elementer, der understøtter den tilsvarende UIA-egenskab `LabeledBy`. (#17442, @michaelweghorn)
* Det er nu muligt at knytte en `wx.ComboBox` og en `wx.StaticText`-
label sammen ved hjælp af `gui.guiHelper.associateElements`. (#17476)
* Følgende extension points er tilføjet: (#17428, @ctoth):
  * `inputCore.decide_handleRawKey`: kaldt, hver gang en tast trykkes
  * `speech.extensions.post_speechPaused`: kaldes, når talen sættes på pause eller genoptages
* Ændringer i registreringen af automatisk detektion af punktdisplay i `bdDetect.DriverRegistrar`: (#17521, @LeonarddeR)
  * Tilføjet metoden `addUsbDevice` til at registrere én USB-enhed ad gangen.
  * Parameteren `matchFunc` er tilføjet til `addUsbDevices` og er nu også tilgængelig i `addUsbDevice`.
    * På denne måde kan enhedsdetektionen begrænses yderligere i tilfælde, hvor en kombination af VID/PID deles af flere enheder på tværs af drivere, eller når en HID-enhed f. eks. tilbyder flere endepunkter.
    * Se metodedokumentationen samt eksemplerne i driverne albatross og brailliantB for yderligere information.
* En ny funktion, `utils.mmdevice.getOutputDevices`, er tilføjet til at enumerere lydudgangsenheder. (#17678)
* Et nyt extension point, `pre_synthSpeak`, er tilføjet i `synthDriverHandler`. Det bliver kaldt, før Speech Manager kalder `speak` på den aktuelle syntese. (#17648)
* NVDA understøtter `text-indent` IAccessible2 object attribute. (#13052, @michaelweghorn)
* Når et script, der bruger `gesture.send` udløses med en kommando, der inkluderer `numLock` som en modifikator, deaktiveres NumLock ikke længere. (#10827, @CyrilleB79)
...
* Konstant `addonStore.network.BASE_URL` er blevet fjernet.
Da webadressen til tilføjelsescenteret nu kan konfigureres direkte i NVDA, er der ingen udskiftning planlagt. (#17099)
* `updateCheck.CHECK_URL` konstant er blevet fjernet.
Da webadressen til søgning af opdateringer nu kan konfigureres direkte i NVDA, er der ingen udskiftning planlagt. (#17151)
* `NVDAObjects.UIA.winConsoleUIA.WinTerminalUIA` er blevet fjernet uden planer om en eventuel erstatning. (#14047, #16820, @codeofdusk)
* `NVDAObjects.IAccessible.ia2TextMozilla.FakeEmbeddingTextInfo` er blevet fjernet. (#16768, @jcsteh)
* Følgende symboler i `appModules.soffice` er blevet omdøbt (#6915, @michaelweghorn):
  * `SymphonyDocument.announceToolbarButtonToggle` til `SymphonyDocument.announceFormattingGestureChange`
  * `SymphonyDocument.script_toggleTextAttribute` til `SymphonyDocument.script_changeTextFormatting`
* `space` nøgleordet for `brailleDisplayDrivers.seikantk.InputGesture` forventer nu `int` i stedet for `bool`. (#17047, @school510587)
* Afsnittet for konfiguration `[upgrade]` herunder `[upgrade][newLaptopKeyboardLayout]` er fjernet. (#17191)
* `updateCheck.checkForUpdate` returnerer nu et `UpdateInfo`-objekt i stedet for et dictionary. (#17310, @christopherpross)
* Konstruktørerne for `updateCheck.UpdateResultDialog` og `updateCheck.UpdateDownloader` er blevet opdateret til at modtage `UpdateInfo`-objekter i stedet for dictionaries med metadata. (#17310, @christopherpross)
* På grund af udfasningen af NVDA's understøttelse af winmm (#17496, #17532, #17678):
  * Følgende symboler er blevet fjernet fra `nvwave` uden erstatninger: `CALLBACK_EVENT`, `CALLBACK_FUNCTION`, `CALLBACK_NULL`, `HWAVEOUT`, `LPHWAVEOUT`, `LPWAVEFORMATEX`, `LPWAVEHDR`, `MAXPNAMELEN`, `MMSYSERR_NOERROR`, `usingWasapiWavePlayer`, `WAVEHDR`, `WAVEOUTCAPS`, `waveOutProc`, `WAVE_MAPPER`, `WHDR_DONE`, `WinmmWavePlayer`, og `winmm`.
  * Følgende symboler er blevet fjernet fra `nvwave`: `getOutputDeviceNames`, `outputDeviceIDToName`, `outputDeviceNameToID`.
  Brug `utils.mmdevice.getOutputDevices` i stedet.
  * `nvwave.WasapiWavePlayer` er blevet omdøbt til `WavePlayer`.
  Derudover er metodesignaturen for dens `__init__` ændret som følger:
    * Parameteren `outputDevice` skal nu kun modtage strengargumenter.
    * De udfasede parametre `closeWhenIdle` og `buffered` er blevet fjernet.
  * `gui.settingsDialogs.AdvancedPanelControls.wasapiComboBox` er blevet fjernet.
  * `WASAPI` er blevet fjernet fra `audio` afsnittet i config spec.
  * Konfigurationsnøglen `config.conf["speech"]["outputDevice"]` er blevet fjernet.
    Dette er blevet erstattet af `config.conf["audio"]["outputDevice"]`, som gemmer et Windows Core Audio endpoint-enheds-ID. (#17547)
* I `NVDAObjects.window.scintilla.ScintillaTextInfo`, hvis der ikke er markeret tekst, er `collapse`-metoden overskrevet, så den udvider til linjen, hvis parameteren `end` er sat til `True`. (#17431, @nvdaes)
* Følgende symboler er blevet fjernet uden erstatninger: `languageHandler.getLanguageCliArgs`, `__main__.quitGroup` og `__main__.installGroup` . (#17486, @CyrilleB79)
* Præfiksmatching af kommandolinjeflag, f.eks. brug af `--di` i stedet for `--disable-addons`, understøttes ikke længere. (#11644, @CyrilleB79)
* Keyword-argumentet `useAsFallBack` i `bdDetect.DriverRegistrar` er blevet omdøbt til `useAsFallback`. (#17521, @LeonarddeR)
* `[addonStore][showWarning]` indstillingen for konfiguration er fjernet.
Brug i stedet `addonStore.dataManager.addonDataManager.storeSettings.showWarning`. (#17597)
* `ui.browseableMessage` tager nu en parameter `sanitizeHtmlFunc`.
Denne har som standardværdi `nh3.clean` med standardargumenter.
Dette betyder, at al HTML, der sendes til `ui.browseableMessage` med `isHtml=True`, nu som standard bliver renset.
For at ændre reglerne for rensning, f.eks. ved at tillade bestemte tags eller attributter, kan du oprette en funktion, der kalder `nh3.clean` med de ønskede parametre. (#16985)
* `updateCheck.UpdateAskInstallDialog` udfører ikke længere automatisk en handling, når knapperne for opdatering eller udskydelse trykkes.
I stedet er der tilføjet en `callback`-egenskab, som returnerer en funktion, der udfører den relevante handling, når den kaldes med dialogens returværdi. (#17582)
* Dialoger, der åbnes med `gui.runScriptModalDialog`, er nu geknedt som modal af NVDA. (#17582)
* Følgende API-symboler relateret til indstillingen "Placer automatisk systemfokus på fokuserbare elementer" er blevet fjernet uden erstatning: (#17598)
...
* SAPI 5 stemmer bruger nu `nvwave.WavePlayer` til at afspille lyd: (#17592, @gexgd0419)
  * `synthDrivers.sapi5.SPAudioState` er blevet fjernet.
  * `synthDrivers.sapi5.SynthDriver.ttsAudioStream` er blevet fjernet.
* Instanser af `autoSettingsUtils.driverSetting.DriverSetting` med et `id`, der starter med en understregning (_), vises ikke længere i NVDA's indstillinger. (#17599)
* Ændret konfigurationen af tastaturekko fra boolean til integer-værdier. (#17505, @Cary-rowen)
  * `config.conf["keyboard"]["speakTypedCharacters"]` og `config.conf["keyboard"]["speakTypedWords"]` bruger nu integer-værdier.
  * Tilføjet `TypingEcho` enum i `config.configFlags` til at repræsentere disse tilstande: 0=Fra, 1=Kun i redigeringsfelter, 2=Altid.
  * `gui.settingsDialogs.KeyboardSettingsPanel.wordsCheckBox` og `gui.settingsDialogs.KeyboardSettingsPanel.charsCheckBox` er blevet fjernet.
* I `winUser.paint` er navnet blevet rettet fra `painStruct` til `paintStruct`, hvilket løser en fejl, hvor indsendelse af en `PAINTSTRUCT` ville udløse en exception. (#17744)
* `documentationUtils.getDocFilePath` og `installer.getDocFilePath` søger ikke længere efter `.txt`-filer i lokale dokumentationsmapper. (#17911, @CyrilleB79)
* `config.conf["documentFormatting"]["reportFontAttributes"]` er blevet fjernet, brug i stedet `config.conf["documentFormatting"]["fontAttributeReporting"]`. (#18066)
* `config.conf["speech"]["includeCLDR"]` er blevet fjernet, kontroller eller tilpas i stedet, om `config.conf["speech"]["symbolDictionaries"]` indeholder `"cldr"`. (#18066)

#### Udfasninger

* `braille.filter_displaySize` extension point er udfaset.
Brug `braille.filter_displayDimensions` i stedet. (#17011)
* Funktionerne `gui.message.messageBox` og `gui.runScriptModalDialog`, og `gui.nvdaControls.MessageDialog` class er udfaset.
Brug `gui.message.MessageDialog` i stedet. (#17582)
* Følgende symboler er udfaset (#17486, @CyrilleB79):
  * `NoConsoleOptionParser`, `stringToBool`, `stringToLang` i `__main__`; brug samme symboler i `argsParsing` i stedet.
  * `__main__.parser`; brug `argsParsing.getParser()` i stedet.
* `bdDetect.DeviceType` er udfaset til fordel for `bdDetect.ProtocolType` og `bdDetect.CommunicationType`, for at tage højde for, at både HID- og Seriel kommunikation kan foregå over USB og Bluetooth. (#17537, @LeonarddeR)

## 2024.4.2

Dette er en patch release til at rette fejl med punktdisplays og læse matematik i Chromium.

### Fejlrettelser

* Rettet fejl ved læsning af matematik i Chromium-baserede browsere (Chrome, Edge). (#17421, @NSoiffer)
* Humanware Brailliant BI 40X enheder, der kører firmware version 2.4, virker nu som forventet. (#17518, @bramd)

## 2024.4.1

Dette er en patch udgivelse til at rette en fejl, når du gemmer ordbøger til brug ved udtale af symboler.

### Fejlrettelser

* Rettet en fejl, der gjorde, at udtaleordbøger for symboler ikke kunne gemmes og dialogen ikke kunne lukkes. (#17344)
...
* Rettet problemer med matematiske attributter, der blev læst i Adobe Reader, hvilket resulterede i dårlig eller forkert tale og output på punkt. (#17980)
* Windows 11-rettelser:
...
* Denne udgivelse bryder kompatibiliteten med eksisterende tilføjelsesprogrammer.
...
* NVDA-specifikke objekter, der bliver fundet af Pythons cykliske garbage collector, logges nu, når de bliver slettet af samleren for at hjælpe med at fjerne referenceløkker fra NVDA. (#11499)
  * Størstedelen af NVDAs klasser spores, herunder NVDAObjects, appModules, GlobalPlugins, SynthDrivers og TreeInterceptors.
...
* Udgået funktionalitet er nu fjernet. (#9548)
  ...
  * Fjernet nogle synthDriverHandler.SynthSetting classes.
    * Tidligere bevaret for bagudkompatibilitet (#8214), men anses nu for forældet.
    * Drivere, der brugte SynthSetting-klasserne, bør opdateres til at bruge DriverSetting-klasserne.
  * Noget forældet kode er blevet fjernet, herunder:
    * Understøttelse af Outlook-meddelelseslisten fra før 2003.
    * En overlay-klasse for den klassiske startmenu, som kun fandtes i Windows Vista og tidligere.
    * Understøttelse af Skype 7 er fjernet, da det definitivt ikke fungerer længere.
* Tilføjet en ramme til at oprette vision-enhancement-udbydere; moduler, der kan ændre skærmindhold, valgfrit baseret på input fra NVDA om objektplaceringer. (#9064)
  * Tilføjelsesprogrammer kan inkludere deres egne providere i en visionEnhancementProviders-mappe.
  * Se modulerne vision og visionEnhancementProviders for henholdsvis implementeringen af frameworket og eksempler.
  * Vision enhancement providers aktiveres og konfigures via kategorien 'Syn' i NVDAs indstillinger.
...
* Introducerede displayModel.UNIT_DISPLAYCHUNK som en textInfos-enhedskonstant specifik for DisplayModelTextInfo. (#10165)
  * Denne nye konstant gør det muligt at gennemgå teksten i en `DisplayModelTextInfo` på en måde, der i højere grad svarer til, hvordan tekststykkerne gemmes i den underliggende model.
...
* Eksterne moduler (appModules og globalPlugins) er nu mindre tilbøjelige til at bryde oprettelsen af NVDAObjects.
  * Exceptions forårsaget af metoderne `chooseNVDAObjectOverlayClasses` og `event_NVDAObject_init` bliver nu korrekt opfanget og logført.
...
* Du kan nu slå automatisk gennemsynstilstand fra ved brug af boksen "Aktivér gennemsynstilstand ved sideindlæsning" under NVDAs indstillinger for gennemsynstilstand. (#8716)
  * Bemærk, at når denne indstilling er deaktiveret, kan du stadig aktivere gennemsynstilstand ved brug af NVDA+mellemrum.
...
* Tastaturkommandoer kan nu tildeles indstillingsprofiler. (#4209)
  * Kommandoen skal konfigures via dialogen "Håndter kommandoer".
* Tilføjet understøttelse for autoforslag i koderedigering i Eclipse. (#5667)
  * Derudover kan Javadoc-information læses fra editoren, når den er tilgængelig, ved at bruge NVDA+d.
...
* Der er nu et enhedstestningsrammeværk for NVDA. (#7026)
  * Unit tests og infrastruktur er placeret i tests/unit-mappen. Se docstringen i tests\unit\init.py-filen for detaljer.
  * Du kan køre tests ved hjælp af "scons tests". Se afsnittet "Køre Tests" i readme.md for detaljer.
  * Hvis du indsender en pull request for NVDA, bør du først køre testsene og sikre dig, at de lykkedes.
...
* Forbedret I/O for punktdisplays: (#5609)
  * Thread-safe punktdisplay-drivere kan erklære sig som sådan ved hjælp af attributten BrailleDisplayDriver.isThreadSafe. En driver skal være thread-safe for at drage fordel af følgende funktioner.
  * Data skrives til thread-safe punktdisplay-drivere i baggrunden, hvilket forbedrer ydeevnen.
...
* Der er flere nye egenskaber tilgængelige fra touchscreen-bevægelser: (#5652)
  * MultiTouchTracker-objekter indeholder nu en childTrackers-egenskab, der indeholder MultiTouchTrackers, som trackeren er sammensat af. For eksempel har dobbelt tap med to fingre childTrackers for to 2-finger tap. 2-finger taps har selv childTrackers for to tap.
...
* I Windows 8 og senere starter NVDA nu meget tidligere, når det er konfigureret til at starte efter login på Windows. (#308)
  * Hvis du aktiverede dette med en tidligere version af NVDA, skal du deaktivere det og aktivere det igen for at ændringen træder i kraft. Følg denne procedure:
    ...
    1. Fjern markeringen i check boxen Start NVDA, når jeg er logget ind.
...
* I stedet for en enkelt i18nName-attribut har synthDriverHandler.SynthSetting nu separate displayNameWithAccelerator- og displayName-attributter for at undgå rapportering af acceleratoren i ringen af talesynteseindstillinger på nogle sprog.
  ...
  * I18nName-attributten er udfaset og og kan blive fjernet i en fremtidig udgivelse.
...
* Tale-kommandoer er implementeret til at indsætte en pause mellem ord og ændre tonehøjde, lydstyrke og hastighed. Se BreakCommand, PitchCommand, VolumeCommand og RateCommand i tale-modulet. (#4674)
  * Der er også speech.PhonemeCommand til at indsætte specifik udtale, men de nuværende implementeringer understøtter kun et meget begrænset antal fonemer.
...
* Det er nu muligt at genstarte NVDA eller genstarte NVDA med tilføjelser deaktiveret fra NVDA's afslutningsdialog. (#4057)
  * NVDA kan også startes med tilføjelser deaktiveret ved brug af kommandolinjeparametre --disable-addons.
...
* Kommandoen for rapportering af placering (NVDA+delete) er mere kontekstspecifik i visse situationer. (#4219)
  ...
  * Hvis denne kommando trykkes to gange, vil placeringsinformation for hele kontrollen blive rapporteret, som var tilfældet tidligere.
* Nyt sprog: Katalansk.
...
* Forbedret support for ARIA-liveområder i Firefox og andre Mozilla Gecko-applikationer:
  ...
  * Liveområder opdateres ikke længere i stilhed, hvis de opdateres samtidig med, at fokus flytter. (#3777)
...
* NVDA har nu en indbygget talesyntese kaldet eSpeak, udviklet af Jonathan Duddington. Den er meget responsiv og letvægtig, og har understøttelse af mange forskellige sprog. SAPI-talesynteser kan stadig bruges, men eSpeak vil være standardtalesyntesen.
  ...
  * For mere information om eSpeak, eller for at finde andre versioner, gå til http://espeak.sourceforge.net/.
