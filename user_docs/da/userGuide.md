...
* Mulighed for at [oprette forbindelse til og styre en anden computer, der kører NVDA](#RemoteAccess) til fjernhjælp eller samarbejde
...
Udover engelsk er NVDA blevet oversat til 55 sprog, herunder: afrikaans, albansk, amharisk, arabisk, aragonisk, bosnisk, bulgarsk, burmesisk, catalansk, kinesisk (traditionelt og forenklet), kroatisk, tjekkisk, dansk, hollandsk, farsi, finsk, fransk, galicisk, georgisk, græsk, hebraisk, hindi, ungarsk, islandsk, irsk, italiensk, japansk, kannada, koreansk, kirgisisk, litauisk, makedonsk, mongolsk, nepalesisk, norsk, polsk, portugisisk (brasiliansk og Portugal), punjabi, rumænsk, russisk, serbisk, slovakisk, slovensk, spansk (colombiansk og spansk), svensk, tamilsk, thai, tyrkisk, ukrainsk, tysk (Tyskland og Schweiz) og vietnamesisk.
...
Det kan være nødvendigt at vente et par sekunder, mens en midlertidig kopi af NVDA indlæses.
Når den er indlæst, taler NVDA gennem resten af processen.
1. NVDA-installationsprogrammet viser licensaftalen.
Tryk på `pil ned` for at læse licensaftalen.
...
|Sig indtastede ord|`NVDA+3` |`NVDA+3` |Når denne funktion er slået til, oplæser NVDA alle de ord, du skriver på tastaturet. |
...
De faktiske kommandoer vil ikke udføres, når tastaturhjælpen er slået til.
...
|Flyt til starten af læsemarkørens valgte tekst |`NVDA+alt+home` |`NVDA+alt+home` |None |Flytter læsemarkøren til det første tegn i den valgte tekst|
|Flyt til slutningen af læsemarkørens valgte tekst |`NVDA+alt+end` |`NVDA+alt+end` |None |Flytter læsemarkøren til det sidste tegn i den valgte tekst|
...
NVDA har en indstilling, der kan aktiveres, der vil bevare denne formatering. Dette hedder "Oprindelig markeringstilstand". Dette virker i Mozilla Firefox og browsere, der er baseret på Chromium 134 eller nyere.
...
* [Dokumentationen for MathCAT](https://nsoiffer.github.io/MathCAT/users.html)
* [Dokumentation for Access8Math](https://github.com/tsengwoody/Access8Math)
* [Dokumentation for MathPlayer](https://docs.wiris.com/mathplayer/en/mathplayer-user-manual.html)
...
Et tryk på `NVDA+F5` opdaterer genkendelsesresultatet.
...
<!-- KC:beginInclude -->
For at få oplyst en kommentar ved markørens position skal du trykke `NVDA+alt+c`.
Ved to tryk bliver informationen vist i gennemsynstilstand.
<!-- KC:endInclude -->
...
<!-- KC:beginInclude -->
For at få oplyst en note for den celle, som er i fokus, skal du trykke `NVDA+alt+c`.
Ved to tryk bliver informationen vist i gennemsynstilstand.
...
NVDA har følgende kommandoer til brug af Poedit 3.5 eller nyere.
...
Hvis markeret, vises en dialog når du forsøger at afslutte NVDA, hvilket giver følgende muligheder:

* afslut
* genstart
* genstart med tilføjelser deaktiveret og fejlfindingslogning aktiveret
* genstart med logning for fejlrettelse aktiveret
* installér afventende opdateringer (hvis nogen).
...
Du kan også manuelt installere de ventende opdateringer fra dialogen "Afslut NVDA" (hvis aktiveret), eller ved at søge efter opdateringer fra menupunktet "Hjælp" i NVDA-menuen.

##### Opdateringsspejl {#UpdateMirror}

Disse indstillinger giver dig mulighed for at angive en alternativ webadresse, når du søger efter opdateringer til NVDA.
Dette kan være nyttigt, hvis adgang til NV Access NVDA opdateringsserveren er langsom eller utilgængelig.

Det skrivebeskyttede tekstfeltet viser den aktuelle webadresse, der bruges som spejlserver.
Hvis der ikke er noget spejl i brug (dvs. at NV Access-opdateringsserveren anvendes), vises "Intet spejl".

Hvis du ønsker at ændre opdateringsspejlet, skal du trykke på knappen "Skift..." for at åbne [Indstil opdateringsspejldialog](#SetURLDialog).

Bemærk venligst, at når der anvendes et opdateringsspejl, har ejeren af serveren adgang til [oplysninger sendt under søgningen efter opdateringer](#GeneralSettingsCheckForUpdates).
Kontakt ejeren af opdateringsspejlet for detaljer om deres datahåndteringspolitikker for at sikre, at du er komfortabel med den måde, dine oplysninger vil blive håndteret på, før du indstiller et opdateringsspejl.

##### Undgå, at skærmen slukker under sig alt eller læsning med punkt {#PreventDisplayTurningOff}

Denne check box, hvis markeret, sikrer at din skærm forbliver tændt, når du læser ved brug af sig alt eller med punkt (f.eks. ved panorering).
Dette forhindrer, at skærmen utilsigtet låser under brugen af sig alt.
Denne indstilling er slået til som standard.
Overvej at deaktivere denne indstilling, hvis du oplever forkortet batterilevetid.
...
Den første indstilling i kategorien Taleindstillinger er knappen Skift.
Denne knap aktiverer dialogen [Vælg talesyntese](#SelectSynthesizer) som lader dig vælge den aktive talesyntese.
...
Hvis du vælger "Automatisk", vælges tabellen ud fra NVDAs aktuelle sprog.
...
| . {.hideHeaderRow} |.|
|---|---|
|Muligheder |Standard (aldrig), Aldrig, Kun når punkt følger er indstillet til automatisk, Altid|
|Standard |Aldrig|
...
##### Startmarkør for afsnit {#BrailleParagraphStartMarkers}

Hvis "Læs efter afsnit" er markeret, vil den valgte startmarkør blive vist for at angive begyndelsen af et afsnit.
Dette kan især være nyttigt i programmer, der bruges til at læse store stykker tekst, såsom strukturerede dokumenter eller bøger.
...
Mulighederne omfatter brug af to mellemrum som en diskret afsnitspause og afsnitssymbolet (¶), som en mere tydelig markering.

| . {.hideHeaderRow} |.|
|---|---|
|Muligheder |Ingen startmarkør for afsnit, Dobbelt mellemrum (  ), Afsnitstegn (¶)|
|Standard |Ingen startmarkør for afsnit|
...
##### Visning af formatering {#BrailleFormattingDisplay}

Denne indstilling bestemmer, hvordan NVDA vil vise tekstformatering på punktskrift.
Denne indstilling kræver, at NVDA er indstillet til at [vise skrifttypeattributter på punktskrift](#DocumentFormattingFontAttributes).

| . {.hideHeaderRow} |.|
|---|---|
| Muligheder| Standard (Liblouis), Liblouis, Tags |
| Standard | Liblouis |

| Mulighed | Adfærd |
|---|---|
| Liblouis | Brug oprindelig formatering på punkt. Bemærk, at denne indstilling kun vil angive fed, kursiv og understreget tekst, og kun hvis den valgte punkttabel understøtter visning af disse attributter. |
| [Tags](#BrailleFormattingDisplayTags) | Brug tags til at beskrive, hvor og hvordan formatering ændrer sig. |

###### Tags {#BrailleFormattingDisplayTags}

Når "Vis formatering på punktskrift" er indstillet til "Tags", vises et formateringstag på punktskrift, når en ændring i formatering registreres.
Disse tags starter med ⣋ og slutter med ⣙.
Et formateringstag vil indeholde ét eller flere symboler, der beskriver tekstformateringen.
Følgende symboler er defineret:

| Symbol | Betydning |
|---|---|
| ⠋ ("f") | Start fed |
| ⡋ ("f" med punkt 7) | Slut fed |
| ⠅ ("k") | Start kursiv |
| ⡅ ("k" med punkt 7) | Slut kursiv |
| ⠥ ("u") | Start understreget |
| ⡥ ("u" med punkt 7) | Slut underline |
| ⠛ ("g")| Start gennemstreget |
| ⡛ ("g" med punkt 7) | Slut gennemstreget |

##### Oplæs tegn, når markørknapper flytter markøren i tekst {#BrailleSpeakOnRouting}

Hvis dette er aktiveret, vil NVDA automatisk sige tegnet ved markøren, når du flytter markøren ved brug af markørknapperne.

For at ændre denne indstilling fra et hvilket som helst sted, skal du tildele et brugerdefineret tastetryk ved hjælp af [dialogen Håndter kommandoer](#InputGestures).

##### Læs op, når du navigerer efter &linje eller afsnit {#BrailleSpeakOnNavigating}

Hvis du aktiverer denne indstilling, vil NVDA læse op, når du navigerer mellem linjer eller afsnit ved brug af dit punktdisplay.

For at aktivere og deaktivere denne indstilling fra hvor som helst, tildel venligst en brugerdefineret kommando til "Tale ved linje- eller afsnitsnavigation" i afsnittet "Punkt" i [dialogen Håndter Kommandoer](#InputGestures).

##### Undgå at opdele ord hvis muligt {#BrailleSettingsWordWrap}

Hvis denne indstilling er slået til, vil et ord, som er for langt til at være for enden af linjen på dit punktdisplay, ikke blive opdelt.
I stedet vil der blive vist nogle blanke tegn for enden af displayet.
Når du ruller displayet videre, kan du læse hele ordet.
Dette kaldes nogle gange for "ombrydning af ord".
Bemærk, at hvis ordet er for langt til at være på displayet, selv hvis det står for sig selv, bliver det stadig opdelt.

Hvis indstillingen er slået fra, vil så meget af ordet som muligt blive vist, men resten vil blive vist på næste linje.
Når du ruller displayet videre, kan du læse resten af ordet.
...
Denne indstilling bestemmer, om talen skal afbrydes, når punktdisplayet rulles tilbage/fremad.
Forrige/næste linje kommandoer afbryder altid tale.

Igangværende tale kan være distraherende, mens du læser punktskrift.
...
Denne indstilling bestemmer, om NVDA annoncerer de tegn, du skriver på tastaturet.
De tilgængelige indstillinger er:

* Fra: NVDA vil ikke annoncere indtastede tegn.
* Kun i redigeringsfelter: NVDA vil kun annoncere tegn indtastet i redigeringsfelter og andre områder, hvor tekst kan indtastes.
* Altid: NVDA vil altid annoncere indtastede tegn.
...
Denne indstilling bestemmer, om NVDA annoncerer de ord, du skriver på tastaturet.
De tilgængelige indstillinger er:

* Fra: NVDA vil ikke annoncere indtastede ord.
* Kun i redigeringsfelter: NVDA vil kun annoncere tegn indtastet i ord og andre områder, hvor tekst kan indtastes.
* Altid: NVDA vil altid annoncere indtastede ord.
...
Denne indstilling er slået til som standard, men nogle brugere vil måske foretrække at slå den fra, f.eks. hvis man skriver vietnamesisk med UniKey-software til indtastning. I dette tilfælde kan det forårsage indtastning af forkerte bogstaver.

##### Timeout for flere tryk af et tastetryk, der udfører mere end én handling {#MultiPressTimeout}

Nogle NVDA-tastaturkommandoer udfører forskellige handlinger baseret på, hvor mange gange den samme tast trykkes hurtigt efter hinanden.
Et eksempel på dette er kommandoen "Oplys det aktuelle tegn for navigatorobjekt".
Denne kommando oplyser tegnet hvis der trykkes på én gang, en fonetisk beskrivelse af tegnet hvis der trykkes to gange, og den numeriske værdi af tegnet hvis trykket tre gange.
Denne indstilling konfigurerer timeout, hvorefter et yderligere tryk på samme tast vil udføre en anden handling, i stedet for at gentage den samme.
For kommandoen, der blev brugt som eksempel, ville et for kort timeout oplyse tegnet to gange, i stedet for tegnbeskrivelsen.
Standard timeout er 500 ms, dvs. et halvt sekund.
Forøgelse af denne timeout kan være særligt nyttigt for personer, der bruger "Træge taster", eller som har et fysisk handicap.
...
Denne indstilling, der er slået til som standard, giver dig mulighed for automatisk at få alle synlige kandidater rapporteret, når de vises, eller når siden ændres.
...
Denne indstilling bestemmer, om NVDA automatisk skal annoncere nye symboler, efterhånden som de vises i kompositionsstrengen.
Denne indstilling er slået til som standard.
...
Med denne indstilling, der som udgangspunkt er slået til, kan du vælge, om kommandoer, f.eks. tastetryk, der hverken er NVDA-kommandoer eller generelle kommandoer, skal forhindres i at blive sendt videre til det dokument, du har fokus på.
...
* Skrifttype
  ...
  * Skrifttypeattributter [(Fra, Tale, Punkt, Tale og punkt)](#DocumentFormattingFontAttributes)
...
* Elementer
  * Overskrifter
  * Links
  * Linktype (destination på samme side)
  * Grafik
  * Lister
  * Bloksitater
  * Grupper
...
##### Skrifttypeattributter {#DocumentFormattingFontAttributes}

Denne indstilling tillader dig at vælge hvordan visse skrifttype- attributter, såsom fed skrift, kursiv, understregning og gennemstregning, skal oplyses.
Combo boxen "Skrifttypeattributter" har fire valgmuligheder:

* Fra: NVDA vil ikke oplyse skrifttypeattributter.
* Tale: NVDA vil annoncere, når disse skrifttypeegenskaber ændres.
* Punkt: NVDA vil vise disse attributter på punkt.
Præcis hvordan de vises, kan konfigureres i [NVDA's punktindstillinger](#BrailleFormattingDisplay).
* Tale og punkt: NVDA vil oplyse skrifttypeattributter ved hjælp af begge ovennævnte metoder.
...
#### Dokumentnavigation {#DocumentNavigation}
...
##### Automatiske oppdateringer {#AutomaticAddonUpdates}
...
Som standard vil notifikationer kun forekomme for tilføjelser med tilgængelige opdateringer inden for samme [kanal](#AddonStoreFilterChannel) (f.eks. stabil, beta eller dev).
Du kan konfigurere tilføjelsesopdateringskanaler [individuelt for hver tilføjelse](#AddonStoreUpdateChannel) eller for [alle tilføjelser](#DefaultAddonUpdateChannel).

Når dette er indstillet til "Opdater automatisk", vil tilføjelser automatisk opdatere i baggrunden.
Du vil blive bedt om at genstarte NVDA, når opdateringerne er færdige.

| . {.hideHeaderRow} |.|
|---|---|
|Muligheder |Underret (Standard), Opdater Automatisk, Deaktiveret |
|Standard |Underret |

|Indstilling |Adfærd |
|---|---|
|Underret |Giv besked, når opdateringer for tilføjelser er tilgængelige |
|Opdatér automatisk |Opdatér automatisk tilføjelser i baggrunden |
|Deaktiveret |Kontrollér ikke automatisk for opdateringer til tilføjelser |

##### Standard opdateringskanal {#DefaultAddonUpdateChannel}

Når [Automatisk tilføjelsesopdateringer](#AutomaticAddonUpdates) er aktiveret (standard), opdateres tilføjelser kun til den samme [kanal](#AddonStoreFilterChannel).
For eksempel vil en installeret betaversion kun blive opdateret til en nyere betaversion.
Denne indstilling bestemmer den opdateringskanal, der bruges for alle tilføjelser som standard.
Du kan også ændre opdateringskanalen for en [specifik tilføjelse fra tilføjelsescenteret](#AddonStoreUpdateChannel).

| . {.hideHeaderRow} |.|
|---|---|
| Mulighed | Samme (Standard), Enhver, Opdatér ikke, Stabil, Beta eller dev, Beta, Dev |
| Standard | Samme |

| Mulighed | Adfærd |
|---|---|
| Samme | Tilføjelser forbliver på deres kanal |
| Enhver | Tilføjelse vil altid opdaterer til den nyeste version, uanset kanal |
| Opdatér ikke | Tilføjelser vil ikke automatisk opdatere, og du skal aktivere dem individuelt |
| Stabil | Tilføjelser opdateres automatisk til stabile versioner |
| Beta eller dev | Tilføjelser opdateres automatisk til beta eller dev-versioner |
| Beta | Tilføjelser opdateres automatisk til betaversioner |
| Dev | Tilføjelser vil automatisk opdatere til udviklingsversioner (dev) |

##### Tillad automatiske opdateringer at installere inkompatible tilføjelser {#AllowIncompatibleAddonUpdates}

Denne indstilling aktiverer automatiske opdateringer til tilføjelser, der muligvis ikke er fuldt kompatible med den aktuelle version af NVDA.
Som standard er dette deaktiveret, hvilket betyder at automatiske opdateringer kun opgraderer til versioner af tilføjelser, som er kompatible med den aktuelle version af NVDA.
Automatiske opdateringer vil stadig opdatere en inkompatibel version af tilføjelsen til en kompatibel version, når den udgives.
Det kan være nyttigt at aktivere dette, når NVDA opdateres og tilføjelsesudgivelser, der bryder kompatibiliteten med NVDA (årets første udgivelse) anvendes.
Dette er især nyttigt for alfa og beta-testere, der tester tilføjelser under en tidlig fase for en version, der bryder kompatibiliteten med NVDA.

##### Spejlserver {#AddonStoreMetadataMirror}

Disse indstillinger giver dig mulighed for at angive en alternativ webadresse, når du henter data til Tilføjelsescenteret.
Dette kan være nyttigt, hvis adgang til serveren for tilføjelsescenteret hos NV Access er langsom eller utilgængelig.

Det skrivebeskyttede tekstfelt viser den aktuelle webadresse, der bruges som spejlserver.
Hvis der ikke er noget spejl i brug (dvs. at serveren for tilføjelsescenteret hos NV Access anvendes), vises "Intet spejl".

Hvis du ønsker at ændre spejlet for tilføjelsescenteret, skal du trykke på knappen "Ændre ..." for at åbne [Indstil spejl for tilføjelsescenter](#SetURLDialog).

#### Indstillinger For Fjernstyring {#RemoteSettings}

Denne kategori giver dig mulighed for at konfigurere indstillingerne for [Fjernstyring](#RemoteAccess).

##### Aktivér Fjernstyring {#RemoteEnable}

Brug denne check box til at aktivere eller deaktivere NVDAs Fjernstyring.

Når dette ikke er markeret, er Fjernstyring helt utilgængelig.

Bemærk, at for at kunne fortsætte en fjernstyringssession på sikre skærme som for eksempel Kontrol af Brugerkonti, skal du aktivere denne indstilling, gemme ændringerne og derefter [kopiere dine indstillinger til NVDAs systemkonfigurationsmappe](#GeneralSettingsCopySettings).
Fjernstyring vil kun fortsætte sessioner startet af den installerede kopi af NVDA på sikre skriveborde.
Det betyder, at du for eksempel ikke kan starte en fjernstyringssession med en flytbar kopi af NVDA og fortsætte sessionen på sikre skærme.

Følgende indstillinger er kun tilgængelige, hvis Fjernstyring er aktiveret.

##### Bekræft afbrydelse af forbindelsen, når denne computer fjernstyres {#RemoteConfirmDisconnect}

Denne indstilling styrer, om der skal kræves bekræftelse, før forbindelsen afbrydes fra en Fjernstyringssession, når man er forbundet som den fjernstyrede computer.

Når dette er markeret, vil NVDA bede om bekræftelse før du afbryder.
Når dette ikke er markeret, vil NVDA straks afbryde uden bekræftelse.

##### Forbind automatisk efter NVDA starter {#RemoteAutoconnect}

Denne indstilling giver dig mulighed for automatisk at oprette en fjernstyringssession, når NVDA starter.
Dette kunne f.eks. være nyttigt til at styre din hjemmecomputer, når du er væk hjemmefra.

Vær forsigtig, når du aktiverer denne mulighed, da det kan øge risikoen for uautoriseret adgang til din computer.

For yderligere detaljer om følgende muligheder, henvises til afsnittet [Opsætning af en fjernstyringssession](#RemoteAccessSetup)

##### Tilstand {#RemoteAutoconnectMode}

Vælg forbindelsestilstand for automatiske forbindelser.

Denne indstilling er kun tilgængelig, når [Forbind automatisk efter NVDA starter](#RemoteAutoconnect) er markeret.

|Indstilling |Adfærd |
|---|---|
|Tillad, at denne computer fjernstyres |Tillader, at denne computer fjernstyres. Det gør det muligt for personen, der fjernstyrer, at betjene computeren, som om de sad foran den. |
|Fjernstyr en anden computer |Brug denne computer til at fjernstyre en anden. Dette lader dig fjernstyre en anden computer. |

##### Server {#RemoteAutoconnectServer}

Vælg servertypen for automatiske forbindelser.

Denne indstilling er kun tilgængelig, når [Forbind automatisk efter NVDA starter](#RemoteAutoconnect) er markeret.

|Indstilling |Adfærd |
|---|---|
|Brug eksisterende - Brug en eksisterende fjernstyringsserver til at oprette forbindelsen (anbefales). |
|Lokal vært |Brug denne instans af NVDA som fjernstyringsserver(avanceret). Denne indstilling kan kræve netværkskonfiguration, der ikke er beskrevet i NVDAs brugervejledning. |

##### Vært {#RemoteAutoconnectHost}

Brug dette felt til at indstille webadressen på den fjernstyringsserver, du ønsker at bruge til automatiske forbindelser.
Hvis serveren bruger en anden port end 6837, skal den inkluderes efter et kolon (f.eks. `example.com:1234`).

Denne indstilling er kun tilgængelig, når [Forbind automatisk efter NVDA starter](#RemoteAutoconnect) er markeret, og [Server](#RemoteAutoconnectServer) er indstillet til "Brug eksisterende".

##### Port {#RemoteAutoconnectPort}

Brug dette felt til at indstille TCP-porten på den fjernstyringsserver, du ønsker at bruge til automatiske forbindelser.

Denne indstilling er kun tilgængelig, når [Forbind automatisk efter NVDA starter](#RemoteAutoconnect) er markeret, og [Server](#RemoteAutoconnectServer) er indstillet til "Lokal vært".

##### Nøgle {#RemoteAutoconnectKey}

Brug dette felt til at indstille nøglen (adgangskoden) på den fjernstyringsserver, du ønsker at bruge til automatiske forbindelser.

Denne indstilling er kun tilgængelig, når [Forbind automatisk efter NVDA starter](#RemoteAutoconnect) er markeret.

##### Slet alle gemte fingeraftryk {#RemoteDeleteFingerprints}

Denne knap sletter fingeraftryk fra alle tidligere servere brugt til fjernstyring.
Det betyder, at du igen vil blive spurgt, om du vil oprette forbindelse til alle ikke-genkendte fjernadgangsservere, selv dem, du tidligere har tilsluttet til.
Du vil blive bedt om at bekræfte, før alle godkendte fingeraftryk slettes.

Denne indstilling er kun tilgængelig, hvis der er godkendte fingeraftryk gemt i din konfiguration.
...
#### Dialogen Indstil Spejl {#SetURLDialog}

Denne dialog lader dig angive webadressen på et spejl, der skal bruges ved [opdatering af NVDA](#GeneralSettingsCheckForUpdates) eller [brug af tilføjelsescenteret](#AddonsManager).
Dette kan være nyttigt, hvis adgang til serveren for disse funktioner hos NV Access er langsom eller utilgængelig.

* Ved indstilling af [spejlserver til opdatering af NVDA](#UpdateMirror), vil titlen på denne dialog være "Indstil spejlserver for NVDA-opdateringer".
* Ved indstilling af [spejlserver til opdatering af tilføjelser](#AddonStoreMetadataMirror), vil titlen på denne dialog være "Indstil spejlserver for tilføjelsescenter".

##### Webadresse {#SetURLTextbox}

Angiv webadressen for det spejl, du ønsker at bruge her.
Kun HTTP og HTTPS-adresser understøttes.
For at beskytte dit privatliv, anbefaler NV Access, at du benytter HTTPS, når det er muligt.

Lad dette felt være tomt for at bruge standard NV Access server.

##### Test... {#SetURLTest}

Tryk på denne knap for at teste den webadresse, du har indtastet.
Du skal være forbundet til internettet, for at testen kan lykkes.
Det anbefales, at du altid tester webadressen, før du gemmer den.
...
##### Fjern indledende stilhed i talens lydoutput {#TrimLeadingSilenceSpeech}

Når aktiveret, vil NVDA fjerne indledende stilhed, der forekommer med nogle talesynteser. Dette kan øge reaktionshastigheden.
Denne indstilling er aktiveret som standard, og bør kun påvirke stilheden i begyndelsen af talen.
Hvis du oplever, at nogle nødvendige stilhedsperioder også mangler (f.eks. pause mellem to sætninger), når du bruger en talesyntese via en tilføjelse, kan du deaktivere denne funktion helt for at løse problemet.

##### Brug WASAPI for SAPI 4 lydoutput {#UseWASAPIForSAPI4}

Denne indstilling aktiverer Microsoft Speech API version 4 (SAPI 4) stemmer til output lyd via Windows Audio Session API (WASAPI).
Dette kan gøre det muligt for SAPI 4-stemmer at fungere med flere funktioner, såsom automatisk dæmpning af lyd, fjernelse af indledende stilhed og at holde lydenheden aktiv.
Men nogle SAPI 4 stemmer fungerer måske ikke med den aktuelle WASAPI implementering.
Hvis du oplever, at SAPI 4-stemmen, du bruger stopper med at fungere, kan du deaktivere denne indstilling.
...
Almindelige omvendte skråstreger i erstatningsfeltet skal derfor angives dobbelt, for eksempel skal "a\\\\b" skrives for at få erstatningen "a\\b".
...
## Fjernstyring {#RemoteAccess}

Med NVDAs indbyggede fjernstyringsfunktion kan du styre en anden computer, der kører NVDA, eller tillade nogen at styre din computer.
Dette gør det nemt at yde eller modtage assistance, samarbejde eller få adgang til din egen computer eksternt.

Fjernstyring er som standard deaktiveret for din sikkerhed.
Du kan aktivere Fjernstyring i [indstillingerne for Fjernstyring](#RemoteSettings).

### Kom godt i gang {#RemoteAccessGettingStarted}

Før du begynder, skal du sørge for, at NVDA er installeret og kører på begge computere.
Sørg også for, at check boxen Aktivér Fjernstyring [i Fjernstyring](#RemoteSettings) er markeret på begge computere.
Funktionen Fjernstyring bliver herefter tilgængelig i menuen Funktioner i NVDA. Der er ikke behov for yderligere downloads eller installationer.

Hvis NVDA kører i [sikker tilstand](#SecureMode), kan du ikke manuelt starte en ny fjernstyringssession.
NVDA vil dog stadig automatisk starte en ny Fjernstyringssession, hvis den er konfigureret til det med indstillingen [Automatisk forbindelse efter NVDA starter](#RemoteAutoconnect).
Du kan stadig manuelt afbryde forbindelsen i sikker tilstand.

### Opsætning af en fjernstyringssession {#RemoteAccessSetup}

Fjernstyringssessioner er konfigureret via forbindelsesdialogen Fjernstyring.
Åbn NVDA-menuen og vælg Værktøjer, derefter "Fjernstyring", derefter "Forbind ...".
Eller fra hvor som helst tryk `NVDA+alt+r`.

#### Valg af tilstand (Fjernstyrende eller fjernstyret) {#RemoteAccessMode}

Her skal du bestemme, hvilken computer der skal fungere som den fjernstyrende computer, og hvilken der skal fungere som den fjernstyrede computer.

Combo boxen lader dig vælge, om du vil lade denne maskine blive fjernstyret af en anden person, eller om denne maskine skal fjernstyre en anden computer.

* Hvis du har brug for en anden person til at hjælpe dig med din computer, skal du vælge "Tillad, at denne computer fjernstyres".
* Hvis du vil hjælpe nogen med deres computer, skal du vælge "Fjernstyr en anden computer".

#### Forbind til en eksisterende server (vært og nøgle angivet) {#RemoteAccessJoin}

I dette tilfælde har den anden person enten startet forbindelsen via en relæserver, eller er vært for forbindelsen lokalt.

1. For server, vælg "Brug eksisterende".
1. I feltet Vært, Indtast værtsadressen (og port hvis det også er angivet).
  * Du kan inkludere porten, der skal forbindes til, ved at tilføje et kolon (":") og portnummeret til værtsadressen.
F.eks. `example.com:1234`.
1. I feltet "nøgle" skal du indtaste nøglen.
Sørg for, at du skriver nøglen nøjagtigt.
1. Tryk OK.

#### Start af en forbindelse via en relæserver {#RemoteAccessConnectRelay}

Hvis en anden person skal oprette forbindelse til dig via internettet, er den nemmeste mulighed at bruge en relæserver. For at gøre dette:

1. For server, vælg "Brug eksisterende".
1. For vært, angive værtsnavnet på en relæserver, såsom `nvdaremote.com`.
Du skal dele denne adresse med den anden person.
  * Du kan inkludere porten, der skal forbindes til, ved at tilføje et kolon (":") og portnummeret til værtsadressen.
F.eks. `example.com:1234`.
1. For nøglen: Find på en adgangskode,der er lang og svær at gætte, eller tryk på knappen "Generer Nøgle" for at få en automatisk genereret.
Du skal dele denne nøgle med den anden person.
  * Bemærk, at nøglen fungerer som både identifikator for sessionen og adgangskoden, så det er meget vigtigt, at den er svær at gætte.
1. Tryk OK.
1. Giv forbindelsesoplysningerne (vært og nøgle) til den anden person, så de kan forbinde.

#### Brug af NVDA som server {#RemoteAccessConnectSelfHosted}

Hvis du ikke ønsker at bruge en relæserver eller du forbinder over et lokalt netværk:

1. For server, vælg "Lokal vært".
1. Hvis du forbinder over internettet, kan du trykke på knappen "Hent ekstern IP" for at vise din eksterne IP (synlig på nettet) adresse.
Du skal dele denne adresse med den anden person.
1. Hvis du forbinder over et lokalt netværk, skal du give den anden person din lokale IP.
Hvordan du finder denne er ikke en del af denne brugervejledning.
1. For port skal du indtast den port, du vil bruge til indgående fjernforbindelser.
Den anden person har også brug for denne oplysning.
  * Dette skal være en port, der ikke bruges af andre tjenester.
  * Som standard bruger Fjernstyring port 6837.
  * Hvis der forbindes over internettet, kan "port forwarding" være nødvendig.
  Dette er ikke en del af denne brugervejledning.
  Du skal måske kontakte din netværksadministrator for at gøre dette.
1. For nøglen: Find på en adgangskode,der er lang og svær at gætte, eller tryk på knappen "Generer Nøgle" for at få en automatisk genereret.
Du skal dele denne nøgle med den anden person.
  * Bemærk, at nøglen fungerer som både identifikator for sessionen og adgangskoden, så det er meget vigtigt, at den er svær at gætte.
1. Tryk OK.
...
### Brug af NVDA Fjernstyring {#RemoteAccessUsage}

Når en fjernstyringssession er aktiv, kan du skifte mellem at styre fjerncomputeren og din egen, del din udklipsholder, og slå lyden for fjernsessionen til og fra:

* Tryk på `NVDA+alt+tab` for at skifte mellem at styre og vende tilbage til din egen computer.
* Send tekst fra din udklipsholder til den anden computer ved at åbne NVDA-menuen, vælge Værktøjer, derefter "Fjernstyring" og til sidst "Send udklipsholder".
* Hvis du er forbundet som den computer, der fjernstyrer en anden maskine, kan du slå den fjernstyrede computers taleoutput fra på din lokale computer ved at åbne NVDA-menuen, vælge Værktøjer, derefter "Fjernstyring" og til sidst "Gør fjerncomputeren lydløs".
* Hvis du er forbundet som den computer, der styrer en anden maskine, kan du sende `control+alt+delete` til den styrede computer ved at åbne NVDA-menuen, vælge Værktøjer, derefter "Fjernstyring" og til sidst "Send control+alt+delete".
  For at dette kan fungere, skal den fjernstyrede computer køre en installeret kopi af NVDA.
  Det kan være muligt, at denne funktion ikke altid vil virke, afhængigt af indstillingerne på den maskine, der fjernstyres.

### Oversigt over genvejstaster til Fjernstyring {#RemoteAccessGestures}

<!-- KC:beginInclude -->
| Navn |Tast |Beskrivelse|
|---|---|---|
| Forbinder til eller afbryder fjernstyringssession | `NVDA+alt+r` | Starter en ny Fjernstyringssession eller, hvis en session allerede er i gang, afbryder forbindelsen til den. |
| Skifter mellem tastaturinput for lokal og fjernstyret computer | `NVDA+alt+tab` | Skifter tastaturinput mellem den lokale computer og fjerncomputeren. |
| Forbind | Ingen | Starter en ny fjernstyringssession. Ikke tilgængelig i [sikker tilstand](#SecureMode). |
| Kopier link | Ingen | Kopierer et link for den aktuelle fjernstyringssession til udklipsholderen. |
| Afbryd | Ingen | Afslutter en fjernstyringssession. |
| Gør fjerncomputeren lydløs | Ingen | Slå talen, der kommer fra fjerncomputeren, til eller fra. |
| Send udklipsholder | Ingen | Sender indholdet fra din udklipsholder til fjerncomputeren. |
<!-- KC:endInclude -->
...
#### Sortering af tilføjelseslisten efter kolonne {#AddonStoreSortByColumn}

Som standard er tilføjelseslisten sorteret efter tilføjelsernes visningsnavn.
Combo boxen "Sorter efter kolonne" kan bruges til at sortere listen efter de tilgængelige kolonner for hvert faneblad.
F.eks. vil du måske gerne sorterer tilføjelser efter udgiver, tilgængelig version, osv.
Tilføjelser kan sorteres i stigende eller faldende rækkefølge.
...
Hvis download eller installation fejler, kan du forsøge installationen igen.
Det er også muligt at annullere installationen, før du lukker tilføjelsescenteret.
...
#### Ændring af den automatiske opdateringskanal {#AddonStoreUpdateChannel}

Du kan administrere de automatiske opdateringskanaler for tilføjelser fra fanerne [installeret og opdaterbare tilføjelser](#AddonStoreFilterStatus).
Når [Automatiske tilføjelsesopdateringer](#AutomaticAddonUpdates) er aktiveret, opdateres tilføjelser som udgangspunkt til den samme [kanal](#AddonStoreFilterChannel), som de blev installeret fra som standard ifølge [standardindstillingen](#DefaultAddonUpdateChannel).
Du kan ændre, hvilken kanal en bestemt tilføjelse benytter, når den opdateres. Dette gøres under menupunktet "Opdatér kanal" i handlingsmenuen.

| Mulighed | Adfærd |
|---|---|
| Standard | Tilføjelsen vil benytte [opdateringskanalen, der er indstillet som standard](#DefaultAddonUpdateChannel) |
| Samme | Tilføjelser benytter den samme kanal |
| Enhver | Tilføjelse vil altid opdaterer til den nyeste version, uanset kanal |
| Opdater ikke | Tilføjelsen vil ikke automatisk blive opdateret |
| Stabil | Tilføjelser opdateres automatisk til stabile versioner |
| Beta eller dev | Tilføjelser opdateres automatisk til beta eller dev-versioner |
| Beta | Tilføjelser opdateres automatisk til betaversioner |
| Dev | Tilføjelser vil automatisk opdatere til udviklingsversioner (dev) |
...
Nogle gange kan problemer forekomme med Windows-registreringsdatabasen, der forårsager, at NVDA fungerer uhensigtsmæssig.
Dette kan f.eks. skyldes installation eller afinstallation af visse programmer (som Adobe Reader eller Math Player), samt Windows-opdateringer og andre hændelser.
Værktøjet til rettelse af COM-registreringer forsøger at løse disse problemer ved at reparere tilgængelighedsoplysninger i registreringsdatabasen.

De typer af problemer dette værktøj kan løse omfatter:

* NVDA siger "ukendt" eller "rude", når du navigerer i browsere som Firefox eller Edge, mail-programmer som Thunderbird, Windows Stifinder, proceslinjen og andre programmer.
* NVDA vil ikke korrekt skifte mellem fokus- og gennemsynstilstand.
* Knapper, som tidligere havde navne, vises nu blot som "knap" for NVDA.
* NVDA forekommer meget langsom, når du navigere i gennemsynstilstand.

Fordi dette værktøj retter oplysninger i Windows registreringsdatabasen, kræver det administrativ adgang, ligesom når du installerer et program.
Hvis du har Kontrol af brugerkonti aktiveret, hvilket mange brugere har, skal du følge vejledningen i disse dialoger, før du kan køre værktøjet.
...
Dot Pad fra Dot Inc er en enhed, der kan vise opdaterbar taktil grafik og punktskrift.
NVDA kan enten vise en enkelt linje med punktskrift på enhedens linje til læsning af punkt, eller flere linjer af punktskrift på området beregnet til taktil grafik.
A300-modellen har et taktilt grafikområde på 120 gange 80 punktceller, hvilket kan rumme 8 linjer med 20 celler i hver.

Du kan indstille, om NVDA skal vise braille på den dedikerede punktdisplaylinje eller på det taktile grafikområde via indstillingen "Destination for output via punkt" i NVDAs brailleindstillinger for denne driver.

Knapper til panorering understøttes, men grundet et begrænset antal knapper på enheden, understøttes andre kommandoer og flytning af markøren ikke.

Når du vælger Dot Pad driveren i NVDA, skal du manuelt vælge den virtuelle USB / Bluetooth serielle port Dot Pad er tilsluttet til.
Denne driver understøtter ikke automatisk detektion.

Bemærk venligst, at på grund af hardwarebegrænsninger vil Dot Pad ikke opdatere alle punkter korrekt, mens din hånd er på enheden.
Sørg for at løfte din hånd helt fra enheden, når du navigerer med NVDA, og begynd først at læse igen, når den er fuldt opdateret.

<!-- KC:beginInclude -->

| Navn |Tast|
|---|---|
|Rul punktdisplay tilbage | `pan_left` |
|Rul punktdisplay fremad | `pan_right` |
...
* Åbning af eksterne filer fra NVDAs menu, såsom brugervejledningen.
...
|`-n LANGUAGE` |`--lang=sprog` | Brug et specifikt sprog uanset NVDAs konfiguration. Skriv "Windows" for standardsprog, "en" for engelsk, osv.|
...
|`-d` |`--disable-addons` |ingen tilføjelsesprogrammer|
...
|Ingen |`--create-portable-silent` |Opretter en flytbar kopi af NVDA (uden at starte den nye kopi). Kræver, at `--portable-path` er angivet. Denne mulighed undertrykker advarsler ved skrivning til ikke-tomme mapper og kan overskrive filer uden advarsel.|
|Ingen |`--portable-path=sti` |Stien, hvor den flytbare kopi oprettes|

Ligesom du kan installere NVDA uden brugerinteraktion ved at tilføje kommandolinjeoptionen `--install-silent` til NVDA, kan den også afinstalleres stille ved at tilføje kommandoen `/S` til afinstallationsprogrammet.

NVDAs afinstallationsprogram hedder `uninstall.exe` og findes i NVDA-installationsmappen, `%ProgramFiles(x86)%\nvda` på 64-bit Windows eller `%ProgramFiles%\nvda` på 32-bit Windows.

De mulige kommandolinjeparametre til NVDAs afinstallationsprogram er følgende:

| Kort |Lang |Beskrivelse|
|---|---|---|
|`/S` |Ingen |Affinstallér NVDA uden brugerinteraktion. |
