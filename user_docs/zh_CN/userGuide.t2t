...
- 操作系统：所有 32 位或者 64 位版本的 Windows 8.1、Windows 10、Windows 11 系统，以及从 2012 R2 起的所有服务器系统。
...
+ 出现 NVDA 安装程序窗口，并显示许可协议。
如果愿意，可以按``下光标``键来阅读许可协议。
+ 按 ``tab`` 键转到“我同意”复选框并按``空格``键将其选中。
+ 按 ``tab`` 键在各选项间切换，切换到想要选中的选项时按下 ``enter`` 键。
+
...
+ 确保应用程序窗口的同意许可协议的复选框被选中。
+ 用 ``tab`` 切换到“在此计算机上安装 NVDA”按钮并激活它。
+ 接下来是 Windows 登录时启动 NVDA，以及创建桌面快捷方式的选项。
这些选项默认是选中状态。
如果愿意，可以用 ``tab`` 及``空格``键更改任何选项，或保持默认。
+ 按下 ``enter`` 继续。
+ 系统会弹出 Windows 用户账户控制（UAC）对话框并询问“您是否允许此应用对您的计算机做出更改？”。
+ 按下 ``alt+y`` 接受此 UAC 提示。
+ 随着安装的进行，会有一个不断被填充的进度栏提示 NVDA 的安装进度。
在这个过程中，NVDA 会发出音调不断升高的嘟嘟声。
安装过程一般很快，而且可能不会被注意到。
+ 弹出一个对话框，证实 NVDA 已经成功安装。
其中的消息提示“点击确定以启动安装好的副本。”
按下 ``enter`` 启动安装好的副本。
+ 随着 NVDA 朗读出表示欢迎的消息，“欢迎使用 NVDA”的对话框出现了。
此时的焦点位于“键盘配置”组合框。
默认采用“台式机”键盘配置，执行一些功能要用到数字键盘（小键盘）。
如果愿意，可以按``下光标``选择“笔记本”键盘配置，将数字键盘的功能分配到其它按键上。
+ 按 ``tab`` 切换到“用 ``大小写锁定``（``capsLock``）作为 NVDA 键之一”。
``插入``键（``Insert``）默认被设置为 NVDA 键。
按``空格``选择``大小写锁定``键作为一个 NVDA 键。
注意：键盘配置和 NVDA 键的设置不会相互影响。
此后，NVDA 键和键盘配置还可以在键盘设置中被修改。
+ 用 ``tab`` 和``空格``对屏幕上的其它选项进行调整。
这些选项用来设置 NVDA 是否自动启动。
+ 按 ``enter`` 关闭对话框，NVDA 现在就运行了。
+
...
++ Poedit ++[Poedit]
NVDA 为 Poedit 3.4 及更高版本提供了增强支持。

%kc:beginInclude
|| 名称 | 按键 | 描述 |
| 朗读翻译者注释 | ``control+shift+a`` | 读出翻译者注释。连按两次，在浏览模式下显示该信息 |
| 朗读批注 | ``control+shift+c`` | 读出窗口内的批注。连按两次，在浏览模式下显示该信息 |
| 朗读旧的源文本 | ``control+shift+o`` | 读出旧的源文本（如果有）。连按两次，在浏览模式下显示该信息 |
