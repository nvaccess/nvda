...
本版新增了远程访问功能，您可以通过一台运行 NVDA 的计算机远程操作另一台运行 NVDA 的计算机。

语音模块迎来多项改进，显著提升了 SAPI 4、SAPI 5 和 OneCore 语音的响应速度。
SAPI 5 语音现支持语速加倍和自动语言切换。
SAPI 4 语音现也支持了音频闪避、去除语音开头的静音和保持音频设备唤醒。

改进了插件商店的自动更新系统，您可以选择插件的自动更新通道并在后台执行自动更新。

添加了两个新命令：手动刷新 OCR 结果和切换定期刷新 OCR 结果的开关。

现在也支持了 Chrome 和 Edge 浏览器的应用原生选择。

一如既往的对 Microsoft Office 和 LibreOffice 进行了改进，本版支持读出更多快捷键的操作结果。

现在会基于 NVDA 的语言确定默认的输入和输出盲文表。
原生支持了 Dot Inc 的 Dot Pad 触觉图形设备，将其视为多行盲文点显器。
还有一些关于盲文点显器的小修复，尤其是在连接稳定性方面。

对 LibLouis 盲文翻译器、eSpeak-NG 和 Unicode CLDR 等依赖进行了更新。
新增科普特语、紧凑型楔形文字和葡萄牙语 6 点计算机盲文表。
...
* 插件商店：
  * 自动更新：(#3208)
    * 可修改插件的自动更新通道。
      * 可通过“自动更新通道”子菜单为已安装的插件选择自动更新通道。
      * 默认自动更新通道可在 NVDA 设置的插件商店面板中设置。
    * 可在后台进行自动更新。
      * 在 NVDA 设置的插件商店面板中将“自动更新”更改为“自动更新”即可启用此功能。
    * 可自动将不兼容的插件更新到另一个较新的不兼容版本。
      * 可在 NVDA 设置的插件商店面板中启用此功能。
  * 新增取消安装插件的操作。(#15578, @hwf1324)
  * 新增当插件下载/安装失败时重试的操作。(#17090, @hwf1324)
  * 插件列表可按列排序（包括发布日期），支持升序和降序排列。(#15277, #16681, @nvdaes)
* 语音：
  * 使用 Microsoft Speech API version 5（SAPI 5）和 Microsoft Speech Platform 语音时支持自动语言切换。(#17146, @gexgd0419)
  * 使用 Microsoft Speech API version 5（SAPI 5）和 Microsoft Speech Platform 语音时支持语速加倍，最高可达 6 倍速。(#17606, @gexgd0419)
* Web 浏览器：
  * Google Chrome、Edge 和其他基于 Chromium 134 及更高版本的应用程序已支持 NVDA 浏览模式的应用原生选择（`NVDA+shift+f10`）。(#17838)
  * 在 Mozilla Firefox 中，访问包含文本片段的网址时，将读出高亮显示的文本。(#16910, @jcsteh)
  * 现可读出链接目标是否指向当前页面。(#141, @LeonarddeR, @nvdaes)
* Microsoft Office：
  * 在 PowerPoint 文本框中编辑时，可以使用 `alt+上光标`/`alt+下光标`逐句移动。(#17015, @LeonarddeR)
  * 在 Word 中使用扩展或缩小选区命令（`f8` 或 `shift+f8`）时，会读出选区更新。(#3293, @CyrilleB79)
  * 在 Word 16.0.18226 及更高版本或使用 Word 对象模型时，会通过语音和盲文提示标题是否折叠。(#17499)
  * 在使用 UIA 的 Word 中，使用原生扩展表格导航命令（`alt+home`、`alt+end`、`alt+pageUp`、`alt+pageDown`）时，会读出输入焦点移动。(#17867, @CyrilleB79)
  * 在 Word 中使用“读出焦点”命令时，如果文档布局信息可用且启用了读出对象描述，会读出文档布局。(#15088, @nvdaes)
  * 在 Word 和 Outlook 中会读出更多快捷键的操作结果：
    * 字体格式快捷键 (#10271, @CyrilleB79)
    * 折叠/展开标题 (#17545, @CyrilleB79)
* LibreOffice：
  * 在 Writer 中使用键盘快捷键增大/减小字号时，会读出新字号。(#6915, @michaelweghorn)
  * 在 Writer 25.2 或更高版本中使用对应快捷键应用“正文文本”或标题段落样式时，会读出新段落样式。(#6915, @michaelweghorn)
  * 在 Writer 中使用快捷键切换双下划线时，会读出新状态（“双下划线开启”/“双下划线关闭”）。(#6915, @michaelweghorn)
  * 可在 LibreOffice 25.8 或更高版本中读出首行缩进。(#13052, @michaelweghorn)
* OCR：
  * 在识别结果中，可按 `NVDA+f5` 手动刷新识别内容。(#17715, @CyrilleB79)
  * 添加了一个未分配的手势，用于切换定期刷新识别结果。(#16897)
* 基于 NVDA 远程插件的远程访问功能已集成到 NVDA 中。(#4390, @ctoth, @tspivey, @daiverd, NVDA Remote贡献者和资助者)
* 可为 NVDA 更新和插件商店指定镜像网址。(#14974, #17151, #17310, @christopherpross)
* 新增对 PDF 数学公式的支持。(#9288, @NSoiffer)
  * 这适用于包含 MathML 的公式（如新版 TeX/LaTeX 生成的部分文件）。
  * 目前仅支持 Foxit Reader 和 Foxit Editor。
* 支持设置 NVDA 在使用盲文导航键（上一行/下一行）导航时读出当前行或段落。(#17053, @nvdaes)
* 支持在按 `alt+上光标`或 ` alt+下光标`时读出输入焦点的改变（例如在 Visual Studio Code 中）。(#17652, @LeonarddeR)
* 新增将查看光标移动到选中文本的第一个和最后一个字符的命令，分别分配给 `NVDA+alt+home` 和 `NVDA+alt+end`。(#17299, @nvdaes)
* 新增了一个常规设置——在全文朗读或阅读盲文时禁止关闭显示器。
此选项默认启用，但可能会增加电池消耗。(#17649, @LeonarddeR)
* NVDA 现已被翻译为波斯尼亚语。(#17953)
* 在 Adobe Acrobat 中，可读出由新版 Microsoft Word 生成的 PDF 文档中的数学公式并与之交互。(#18056)

### 改进

* 组件更新：
  * 将 LibLouis 盲文翻译器更新到 [3.33.0](https://github.com/liblouis/liblouis/releases/tag/v3.33.0)。(#17469, #17768, @LeonarddeR, @codeofdusk)
    * 新增了科普特语、紧凑型楔形文字和葡萄牙语 6 点计算机盲文表。
  * 将 CLDR 更新到版本 46.0。(#17484, @OzancanKaratas)
  * 将 eSpeak NG 更新到1.52.0。(#17056)
* NVDA 界面更改：
  * 取消了 NVDA 安装程序启动时的提示音。(#14068)
  * “读出链接网址”、“读出字符格式信息”和“读出选中文本”的浏览模式对话框新增了“关闭”和“复制”按钮以提升用户体验。(#17018, @XLTechie)
  * 在退出对话框中支持同时禁用插件并启用调试日志记录来重启 NVDA。(#11538, @CyrilleB79)
  * 移除了禁用 WASAPI 音频输出的选项。(#16080)
  * 移除了浏览模式设置中的自动将焦点跳转到可聚焦元素，该功能现已禁用。(#17598)
  * 仅当更新到与当前版本存在插件 API 不兼容的新版本时，NVDA 才会发出插件兼容性警告。(#17071, #17506)
  * 从 NVDA 菜单中移除了贡献者名单文件。(#16922)
  * NVDA 许可协议现以 HTML 格式呈现以提升可读性。(#17600)
  * 新增常用命令行选项的短版本：`-d` 对应 `--disable-addons`，`-n` 对应 `--lang`。
  命令行参数的前缀匹配功能（例如用 `--di` 代替 `--disable-addons`）已废弃。(#11644, @CyrilleB79)
  * COM 注册修复工具的改进：(#12355, @XLTechie)
    * 启动时改为显示功能说明而非警告提示。(#12351)
    * 初始窗口现可通过 `Esc` 或 `Alt+F4` 关闭。(#10799)
    * 在极少数 COM 重新注册失败时，会向用户显示包含错误信息的提示。
* 语音：
  * Microsoft Speech API version 5 和 Microsoft Speech Platform 语音现使用 WASAPI 输出音频，这可能会提升这些语音的响应速度。(#13284, @gexgd0419)
  * 使用 OneCore 语音、SAPI5 语音及部分第三方语音插件时，将自动去除语音开头的静音以提升响应速度。(#17614, @gexgd0419)
  * Microsoft Speech API version 4 语音现默认使用 WASAPI 输出音频，以支持音频闪避、去除语音开头的静音和保持音频设备唤醒功能。
  若您的 SAPI4 语音出现异常，可通过高级设置禁用 SAPI 4 使用 WASAPI 输出音频。(#17718, #17801, @gexgd0419)
  * 默认启用了语音输出的 Unicode 规范化。(#17017, @LeonarddeR).
    * 仍可在 NVDA 设置的“语音”类别中关闭此功能。
* “读出输入的字符”和“读出输入的单词”的键盘设置现包含三个选项：关闭、仅在编辑控件中、始终。(#17505, @Cary-rowen)
  * “读出输入的字符”选项的默认值已更改为“仅在编辑控件中”。
* 默认盲文输入/输出表现可根据 NVDA 界面语言自动匹配。(#17306, #16390, #290, @nvdaes)

### 错误修复

* 语音：
  * 修复了使用部分 SAPI5 语音合成器时，连续阅读（全文朗读）会在首句末尾中断的问题。(#16691, @gexgd0419)
  * 修复了使用循环切换合成器快捷键切换语音时 SAPI5 和 SAPI4 的语速、音量等语音参数会被重置为默认值的错误。(#17693, #2320, @gexgd0419)
  * 修复了部分 SAPI4 语音（如 IBM TTS Chinese 即 CHSTTS 语音）无法加载的问题。(#17726, @gexgd0419)
* 盲文：
  * 新增对 Dot Inc 公司的 Dot Pad 触觉图形设备作为多行盲文点显器的原生支持。(#17007)
  * 修复了 Seika Notetaker 的 space 以及 space + 点的手势未能在按键与手势对话框中正确显示的问题。(#17047, @school510587)
  * 修复了未连接点显器时，使用显示语音输出盲文模式会触发错误提示音并产生大量日志记录的错误。(#17092, @Emil-18)
  * 当已明确选择“标准 HID 盲文点显器驱动”后，在打开盲文点显器列表时，NVDA  会正确显示 该 HID 驱动为已选择。(#17537, @LeonarddeR)
  * Humanware Brailliant 驱动在选择连接端点时更可靠，提升了连接稳定性并减少错误。(#17537, @LeonarddeR)
  * 禁用插件运行时，开发者实验目录中的自定义盲文表会被正确忽略。(#17565, @LeonarddeR)
  * 修复了NVDA未正确检测到某些 USB 盲文点显器的问题。(#18114, @christiancomaschi)
* Microsoft Office：
  * 修复了在 Word、Outlook、Excel、PowerPoint 使用旧版对象模型时，读出链接网址命令不生效的错误。(#17292, #17362, #17435, @CyrilleB79)
  * 修复了 Excel 中的元素列表对话框（`NVDA+f7`）在非英文系统中未正确列出批注和公式的错误。(#11366, @CyrilleB79)
  * PowerPoint 的改进：
    * 修复了包含宽字符（例如 emoji 字符）时，光标位置不准确的错误。(#17006 , @LeonarddeR)
    * 读出字符位置更精准（如按 `NVDA+delete` 时）。(#9941, @LeonarddeR)
    * 修复了禁用“页面加载完成后朗读所有内容”选项后，启动幻灯片放映时仍然触发全文朗读的错误。(#17488, @LeonarddeR)
* LibreOffice：
  * 修复了使用微软拼音输入法并开启旧版兼容模式时，在 Writer 等应用中 IME 窗口显示期间出现错误的问题。(#17198, @michaelweghorn)
  * 可以用盲文和语音正确呈现对话框中复选框的选中状态。(#17218, @michaelweghorn)
* 数学：
  * 修复了部分网页数学元素的阅读问题。
  特别是带 `role="math"` 属性的 span 等元素内的 MathML。(#15058)
  * 仅有图片替代文本而无 MathML 的公式现视为普通图形，支持用 `g` 跳转并逐字浏览替代文本。(#16007)
* 集成开发环境：
  * 修复了 Android Studio/IntelliJ Idea 中特定文件全选会导致 NVDA 崩溃的错误。(#17418, @thgcode)
  * 修复了在 Visual Studio Code中，NVDA 强制使用 `Alt+下光标`和 `Alt+上光标`用于句子导航的问题。(#17082, @LeonarddeR)
  * Visual Studio Code 中的部分区域能被正确识别为可编辑控件。(#17573, @Cary-rowen)
  * 在记事本、其他 UIA 文档及 Windows 11 上的 Notepad++ 文档中，若最后一行是空行，“移动点显器到下一行”会将光标移至最后一行。
  在任何文档中，若光标已经位于最后一行，此命令会将其移至行尾。(#17251, #17430, @nvdaes)
* 配置文件：
  * 修复了使用关联的配置激活全文朗读，盲文功能会失效的错误。(#17163, @LeonarddeR)
  * 修复了即使用户未更改某些设置（其值与基础配置中的值保持一致），在保存操作后仍会被保存到当前激活的配置的问题。(#17157, @leonarddeR)
* 改进了拼读时的 Unicode 规范化处理：
  * 修复了在朗读一个规范化的字符后，NVDA 会错误地将后续字符也识别为规范化字符的问题。(#17286, @LeonarddeR)
  * 修复了组合字符（例如 é）未能被正确读出的问题。(#17295, @LeonarddeR)
* 修复了在 Thunderbird 搜索结果页面中，NVDA 无法读取其弹出子菜单项的问题。(#4708, @thgcode)
* 修复了 COM 注册修复工具在执行失败时却提示成功的问题。(#12355, @XLTechie)
* 修复了当 Windows 11 剪贴板历史记录中存在条目时关闭该窗口，NVDA 仍会读出这些条目的问题。(#17308, @josephsl)
* 修复了在浏览模式下显示期间重新加载插件导致 NVDA 无法朗读后续焦点移动的问题。(#17323, @CyrilleB79)
* 修复了在使用 Skype、Discord、Signal 和 Phone Link 等应用在语音通信时，NVDA 语音和音效音量会自动降低的问题。(#17349, @jcsteh)
* 修复了因检索快照变量时发生错误，导致打开 NVDA Python 控制台失败的问题。(#17391, @CyrilleB79)
* 修复了 web 浏览器中可编辑文本控件的文本选择更改有时无法被读出的错误。(#17501, @jcsteh)
* 修复了当锚点链接指向虚拟光标当前所在的对象时，NVDA 无法滚动到链接目标的问题。(#17669, @nvdaes)
* 修复了重启文件资源管理器（Explorer）后，NVDA 高亮窗口图标仍固定在任务栏的问题。(#17696, @hwf1324)
...
需要重新测试插件并更新其清单文件。
* 组件更新：
  * 将 Ruff 更新到 0.8.1。(#17102, #17260, #17473)
  * 将 Comtypes 更新到 1.4.6。(#17061, @LeonarddeR)
  * 将 wxPython 更新到 4.2.2。(#17181, @dpy013)
  * 将 SCons 更新到 4.8.1。(#17254)
  * 将 sphinx 更新到 8.1.2，sphinx-rtd-theme 更新到 3.0.1。(#17284, @josephsl)
  * 将 Robot Framework 更新到 7.1.1。(#17329, @josephsl)
  * 将 configobj 更新到 5.1.0 commit `8be5462`。(#17328)
  * 将 pre-commit 更新到 4.0.1。(#17260)
  * 将 typing-extensions 更新到 4.12.2。(#17438, @josephsl)
  * 将 licensecheck 更新到 2024.3。(#17440, @josephsl)
  * 将 markdown 更新到 3.7。(#17459, @josephsl)
  * 将 nh3 更新到 0.2.19。(#17465, @josephsl)
  * 将 nuitka 更新到 2.5.4。(#17458, @josephsl)
  * 将 schedule 更新到 1.2.2。(#17455, @josephsl)
  * 将 requests 更新到 2.32.3。(#17456, @josephsl)
* 支持调用 `ui.browseableMessage`，并传入选项以显示一个用于复制到剪贴板的按钮，和/或一个用于关闭窗口的按钮。(#17018, @XLTechie)
* 新增了若干用于识别链接类型的功能。(#16994, @LeonarddeR, @nvdaes)
  * 新增了一个 `utils.urlUtils` 模块，其中包含用于确定链接类型的不同函数
  * `controlTypes.states.State` 中新增了 `INTERNAL_LINK` 状态
  * `NVDAObject` 上新增了 `linkType` 属性。默认情况下，如果存在 `treeInterceptor`，它会查询该拦截器。
  `BrowseModeTreeInterceptor` 对象新增了 `documentUrl` 属性。
  * `BrowseModeTreeInterceptor` 对象新增了 `getLinkTypeInDocument` 方法，该方法接受一个 URL 来检查对象的链接类型
  * `BrowseModeTreeInterceptor` 对象新增了 `getLinkTypeInDocument` 方法，该方法接受一个 URL 来检查对象的链接类型
  * `globalCommands` 中新增了 `toggleBooleanValue` 辅助函数。
  当切换 `config.conf` 中的布尔值时，可以调用它来读出结果
* 已从 NVDA 的编码标准中移除函数参数列表必须缩进两个制表符的要求，以兼容现代自动代码风格检查工具。(#17126, @XLTechie)
* 将 [NVDA 的 VS Code 工作区配置](https://nvaccess.org/nvaccess/vscode-nvda)添加为 git 子模块。(#17003)
* 新增了函数 `gui.guiHelper.wxCallOnMain`，允许从非 GUI 线程安全且同步地调用 `wx` 函数，并获取其返回值。(#17304)
* `gui.message` 中新增了消息对话框 API。(#13007)
  * 新增了类：`ReturnCode`、`EscapeCode`、`DialogType`、`Button`、`DefaultButton`、`DefaultButtonSet`、`MessageDialog`。
* 在 `brailleTables` 模块中，新增了 `getDefaultTableForCurrentLang` 函数。(#17222, @nvdaes)
* 新增了 `updateCheck.UpdateInfo` 数据类，用于封装 NVDA 更新相关的元数据。(#17310, @christopherpross)
* 现在，检索 `labeledBy` 属性适用于：
  * 实现了 `labelled-by` IAccessible2 关系的应用中的对象。(#17436, @michaelweghorn)
  * 支持相应 `LabeledBy` UIA 属性的 UIA 元素。(#17442, @michaelweghorn)
* 新增了使用 `gui.guiHelper.associateElements` 关联 `wx.ComboBox` 和标签 `wx.StaticText` 的功能。(#17476)
* 新增了以下扩展点 (#17428, @ctoth)：
  * `inputCore.decide_handleRawKey`：每次按下按键时调用
  * `speech.extensions.post_speechPaused`：当语音暂停或取消暂停时调用
* `bdDetect.DriverRegistrar` 中盲文显示器自动检测注册的变更：(#17521, @LeonarddeR)
  * 新增了 `addUsbDevice` 方法，用于一次注册一个 USB 设备。
  * `addUsbDevices` 中新增了 `matchFunc` 参数，该参数也可用于 `addUsbDevice`。
    * 通过这种方式，在 VID/PID 组合被多个驱动程序中的多个设备共享，或者 HID 设备提供多个端点等情况下，可以进一步约束设备检测。
    * 更多信息请参阅该方法的文档以及 `albatross` 和 `brailliantB` 驱动程序中的示例。
* 新增了函数 `utils.mmdevice.getOutputDevices`，用于枚举音频输出设备。(#17678)
* 在 `synthDriverHandler` 中新增了扩展点 `pre_synthSpeak`，将在语音管理器调用当前语音合成器的 `speak` 方法之前被调用。(#17648)
* NVDA 支持 `text-indent` IAccessible2 对象属性。(#13052, @michaelweghorn)
* 修复了使用包含 `numLock` 作为修饰键的手势触发调用 `gesture.send` 的脚本时，Num Lock 键会被关闭的问题。(#10827, @CyrilleB79)
...
* 常量 `addonStore.network.BASE_URL` 已移除。
由于现在可以直接在 NVDA 中设置插件商店的基础 URL，因此未计划提供替代方案。(#17099)
* 常量 `updateCheck.CHECK_URL` 已移除。
由于现在可以直接在 NVDA 中配置 NVDA 更新检查 URL，因此未计划提供替代方案。(#17151)
* `NVDAObjects.UIA.winConsoleUIA.WinTerminalUIA` 已移除，且无公开替代方案。(#14047, #16820, @codeofdusk)
* `NVDAObjects.IAccessible.ia2TextMozilla.FakeEmbeddingTextInfo` 已移除。(#16768, @jcsteh)
* `appModules.soffice` 中的以下符号已重命名 (#6915, @michaelweghorn)：
  * `SymphonyDocument.announceToolbarButtonToggle` 重命名为 `SymphonyDocument.announceFormattingGestureChange`
  * `SymphonyDocument.script_toggleTextAttribute` 重命名为 `SymphonyDocument.script_changeTextFormatting`
* `brailleDisplayDrivers.seikantk.InputGesture` 的 `space` 关键字参数现在需要 `int` 类型的值，而非 `bool` 类型。(#17047, @school510587)
* 配置节 `[upgrade]` 及其下的 `[upgrade][newLaptopKeyboardLayout]` 已移除。(#17191)
* `updateCheck.checkForUpdate` 现在返回 `UpdateInfo` 对象，而非字典。(#17310, @christopherpross)
* `updateCheck.UpdateResultDialog` 和 `updateCheck.UpdateDownloader` 的构造函数已更新，现在接受 `UpdateInfo` 对象，而非元数据字典。(#17310, @christopherpross)
* 由于 NVDA 终止了对 `winmm` 的支持 (#17496, #17532, #17678)：
  * 已从 `nvwave` 中移除以下符号，且无替代方案：`CALLBACK_EVENT`、`CALLBACK_FUNCTION`、`CALLBACK_NULL`、`HWAVEOUT`、`LPHWAVEOUT`、`LPWAVEFORMATEX`、`LPWAVEHDR`、`MAXPNAMELEN`、`MMSYSERR_NOERROR`、`usingWasapiWavePlayer`、`WAVEHDR`、`WAVEOUTCAPS`、`waveOutProc`、`WAVE_MAPPER`、`WHDR_DONE`、`WinmmWavePlayer` 和 `winmm`。
  * 已从 `nvwave` 中移除以下符号：`getOutputDeviceNames`、`outputDeviceIDToName`、`outputDeviceNameToID`。
  请改用 `utils.mmdevice.getOutputDevices`。
  * `nvwave.WasapiWavePlayer` 已重命名为 `WavePlayer`。
  此外，其 `__init__` 方法签名变更如下：
    * `outputDevice` 参数现在只应传递字符串类型的参数。
    * 已弃用的 `closeWhenIdle` 和 `buffered` 参数已移除。
  * `gui.settingsDialogs.AdvancedPanelControls.wasapiComboBox` 已移除。
  * 配置中 `audio` 部分的 `WASAPI` 键已移除。
  * 配置键 `config.conf["speech"]["outputDevice"]` 已移除。
    它已被 `config.conf["audio"]["outputDevice"]` 替代，该配置键存储 Windows 核心音频端点设备 ID。(#17547)
* 在 `NVDAObjects.window.scintilla.ScintillaTextInfo` 中，如果未选择文本，且 `end` 参数设置为 `True`，则 `collapse` 方法将被重写以扩展到当前行。(#17431, @nvdaes)
* 以下符号已移除，且无替代方案：`languageHandler.getLanguageCliArgs`、`__main__.quitGroup` 和 `__main__.installGroup`。(#17486, @CyrilleB79)
* 命令行标志的前缀匹配（例如使用 `--di` 代替 `--disable-addons`）不再受支持。(#11644, @CyrilleB79)
* `bdDetect.DriverRegistrar` 的 `useAsFallBack` 关键字参数已重命名为 `useAsFallback`。(#17521, @LeonarddeR)
* 配置设置 `[addonStore][showWarning]` 已移除。
请改用 `addonStore.dataManager.addonDataManager.storeSettings.showWarning`。(#17597)
* `ui.browseableMessage` 现在接受 `sanitizeHtmlFunc` 参数。
该参数默认为使用默认参数的 `nh3.clean`。
这意味着现在默认情况下，任何使用 `isHtml=True` 传递给 `ui.browseableMessage` 的 HTML 都会被清理。
要更改清理规则（例如，将标签或属性列入白名单），请创建一个使用所需参数调用 `nh3.clean` 的函数。(#16985)
* 当按下更新或推迟按钮时，`updateCheck.UpdateAskInstallDialog` 不再自动执行操作。
取而代之的是，新增了 `callback` 属性，该属性返回一个函数，当使用对话框的返回值调用此函数时，它会执行相应的操作。(#17582)
* 通过 `gui.runScriptModalDialog` 打开的对话框现在被 NVDA 识别为模态对话框。(#17582)
* 以下与“自动将焦点跳转到可聚焦元素”设置相关的 API 符号已移除，且无替代方案：(#17598)
...
* 由于 SAPI5 语音现在使用 `nvwave.WavePlayer` 输出音频：(#17592, @gexgd0419)
  * `synthDrivers.sapi5.SPAudioState` 已移除。
  * `synthDrivers.sapi5.SynthDriver.ttsAudioStream` 已移除。
* ID 以小写下划线 (`_`) 开头的 `autoSettingsUtils.driverSetting.DriverSetting` 实例将不再显示在 NVDA 的设置中。(#17599)
* 将键盘键入朗读的配置从布尔值更改为整数值。(#17505, @Cary-rowen)
  * `config.conf["keyboard"]["speakTypedCharacters"]` 和 `config.conf["keyboard"]["speakTypedWords"]` 现在使用整数值。
  * `config.configFlags` 中新增了 `TypingEcho` 枚举来表示这些模式：0=关闭，1=仅在编辑控件中，2=总是。
  * `gui.settingsDialogs.KeyboardSettingsPanel.wordsCheckBox` 和 `gui.settingsDialogs.KeyboardSettingsPanel.charsCheckBox` 已移除。
* `winUser.paint` 已从 `painStruct` 重命名为 `paintStruct`，从而修复了传递 `PAINTSTRUCT` 时会引发异常的错误。(#17744)
* `documentationUtils.getDocFilePath` 和 `installer.getDocFilePath` 不再在区域设置文档文件夹中查找 `.txt` 文件。(#17911, @CyrilleB79)
* `config.conf["documentFormatting"]["reportFontAttributes"]` 已移除，请改用 `config.conf["documentFormatting"]["fontAttributeReporting"]`。(#18066)
* `config.conf["speech"]["includeCLDR"]` 已移除，请检查/修改 `config.conf["speech"]["symbolDictionaries"]` 是否包含 `"cldr"` 作为替代。(#18066)

#### 弃用

* 扩展点 `braille.filter_displaySize` 已弃用。
请改用 `braille.filter_displayDimensions`。(#17011)
* 函数 `gui.message.messageBox`、`gui.runScriptModalDialog` 以及类 `gui.nvdaControls.MessageDialog` 已弃用。
请改用 `gui.message.MessageDialog`。(#17582)
* 以下符号已弃用 (#17486, @CyrilleB79)：
  * `__main__` 中的 `NoConsoleOptionParser`、`stringToBool`、`stringToLang` 已弃用；请改用 `argsParsing` 中的同名符号。
  * `__main__.parser` 已弃用；请改用 `argsParsing.getParser()`。
* `bdDetect.DeviceType` 已弃用，推荐使用 `bdDetect.ProtocolType` 和 `bdDetect.CommunicationType`，以便更清晰地体现 HID 和串行通信均可通过 USB 和蓝牙进行。(#17537 , @LeonarddeR)

## 2024.4.2

这是一个补丁版本，用于修复盲文设备和 Chromium 中数学阅读的错误。

### 错误修复

* 修复了 Chromium 内核浏览器（如 Chrome、Edge）中阅读数学的错误。(#17421, @NSoiffer)
* 运行固件版本 2.4 的 Humanware Brailliant BI 40X 设备现在可以按预期工作。(#17518, @bramd)

## 2024.4.1

这是一个补丁版本，主要修复了保存语音符号字典时的错误。

### 错误修复

* 修复了未保存语音符号字典且对话框无法关闭的错误。(#17344)
...
* 语音和盲文输出新增 Unicode 规范化支持。(#11570, #16466 @LeonarddeR)
  * 该功能对于阅读某些语音合成器或盲文表无法识别的字符会非常有用，这些字符可能有兼容的替代字符，如社交媒体上常用的粗体和斜体字符。
...
* 修复了 Adobe Reader 中数学属性被读出的问题，此问题曾导致语音和盲文输出效果不佳或错误。(#17980)
* Windows 11 修复：
...
* 当打开已存在的设置对话框时，NVDA 会将焦点设置到现有对话框，而非引发错误。(#5383)
...
NVDA 中的 COM 注册修复工具现在能够解决更多的系统问题。
...
* 更方便的 NVDA Python 控制台的输出浏览支持。(#9784)
  * Alt+上光标/下光标跳转到上一个/下一个输出结果（同时按下 shift 键可进行选择）。
  * Control+l 清除输出窗格。
...
* 注意：这是一个插件 API 兼容性破坏版本。插件需要重新测试并更新其清单文件。
...
增加了在 NVDA 设置对话框内按 F1 打开对应选项上下文帮助的功能。
...
* 在 Microsoft Word 中按住 control+shift+下方向键时，NVDA 不再退出或无响应。(#9463)
...
尽管这些更改确实破坏了与旧版 NVDA 插件的兼容性，但是为了安全起见，必须升级到 Python 3，并且语音框架允许在不久的将来进行一些激动人心的创新。
 此发行版中的其他新特性包括对 Java VM 的 64 位支持，新增黑屏和焦点高亮功能，对更多盲文点显器和新增的盲文查看器的支持以及许多其他错误修复。
...
* 在 Microsoft Excel 中，使用 shift+回车或 shift+小键盘回车时，NVDA 现在能准确读出单元格移动后的位置。(#9499)
...
相反，这些应该放在 NVDA 插件里安装。如果在 NVDA 新的高级设置面板中打开了开发人员测试目录选项，那么开发者可以把测试代码放在 NVDA 用户配置目录中新的开发人员测试目录中。
...
* 在 Mozilla Firefox 和 Google Chrome 中，浏览模式现在可在列表框和树是图中朗读所选项目。
  ...
  * 这不适用于 Chrome 中的某些列表框例如（HTML 选择控件）。This does not work for certain list boxes (HTML select controls) in Chrome.
...
* NVDA 现在不再自动从 NVDA 用户配置目录加载自定义 appModules，globalPlugins 以及盲文和合成器驱动程序。应将需要测试的代码打包为具有正确版本信息的插件，以确保根当前版本的 NVDA 兼容。(#9238)
  * 对于需要在开发过程中测试代码的开发者，请在 NVDA 设置的高级分类中启用允许从开发者实验目录加载自定义代码，并在启用此设置时将代码放在 NVDA 用户配置目录中的“试验”目录中。
...
* 您现在可以从盲文键盘切换控制，移位，替代，窗口和 NVDA，并将这些修饰符与盲文输入（例如按下 control+s）进行组合。(#7306)
  * 您可以使用“输入手势”对话框中的“模拟系统按键”下的命令指定这些新的修饰键切换。
...
此版本的亮点包括对Microsoft Edge的改进支持、浏览模式在Windows 10邮件应用程序的增强支持和NVDA的对话框布局的重大改进。

### 新特性

* NVDA现在可以使用音调指示行缩进。这可以使用NVDA的“文档格式设置”对话框中的“行缩进提示方式”组合框进行配置。(#5906)
* 支持 Orbit Reader20盲文点显器。(#6007)
* 添加了一个在启动时打开语音查看器窗口的选项。这可以通过语音查看器窗口中的复选框来启用。(#5050)
* 重新打开语音查看器窗口时，位置和尺寸将恢复原样。(#5050)
* Microsoft Word 中的交叉引用字段现在被视为超链接对待，它们被报告为链接，并且可以被激活。(#6102)
* 支持 Baum SuperVario2、Baum Vario 340 和 HumanWare Brailliant2 点显器。(#6116)
* 初步支持Microsoft Edge的周年更新。(#6271)
* 在Windows 10邮件应用程序中阅读电子邮件时，现在可使用浏览模式。(#6271)
* 新语言：立陶宛语。

### 改进

* 更新liblouis盲文翻译为3.0.0。这包括对英语盲文的显着增强。(#6109,#4194,#6220,#6140)
* 在插件管理器中，禁用插件和启用插件按钮现在具有键盘快捷键(分别为alt + d和alt + e)。(#6388)
* NVDA对话框中的各种填充和对齐问题已解决。(#6317,#5548,#6342,#6343,#6349)
* 已调整文档格式对话框，以便滚动内容。(#6348)
* 调整符号发音对话框的布局，以使对话框的全部宽度用于显示符号列表。(#6101)
* 在浏览器的浏览模式下，编辑框(e和shift + e)和表单(f和shift + f)单键导航命令现在可以用于移动到只读编辑框。(#4164)
* 在NVDA的文档格式设置中，“朗读光标后面的格式化更改”已重命名为“指示光标后的格式更改”，因为它会影响盲文和语音的输出信息。(#6336)
* 调整NVDA“欢迎”对话框的外观。(#6350)
* NVDA对话框的“确定”和“取消”按钮现在对话框右对齐。(#6333)
* 旋转控件现在用于数字输入字段，如“语音设置”对话框中的“大写时音高变化的百分比”设置。您可以输入所需的值，或使用向上和向下箭头键来调整值。(#6099)
* Firefox 的IFrame 提示方式已经和其他浏览器统一，都朗读为“框架”。(#6047)

### 错误修复

* 修正了在语音查看器打开时退出NVDA时出现的罕见错误。(#5050)
* Mozilla Firefox打开的图像地图现在可在浏览模式下按预期的呈现。(#6051)
* 在字典对话框中，按“回车”键现在可保存您所做的任何更改并关闭对话框。以前，按“回车”键什么也没做。(#6205)
* 更改输入法的(本地输入/字母数字输入，全角/半角等)的输入模式时，消息现在可用盲文显示。(#5892，#5893)
* 禁用后立即重新启用插件，或者反过来，插件状态现在可正确地恢复到之前的状态。(#6299)
* 使用Microsoft Word时，现在可以读取标题中的页码字段。(#6004)
* 现在可以使用鼠标在符号列表和符号发音对话框中的编辑框之间移动焦点。(#6312)
* 在Microsoft Word中的浏览模式下，修复了当文档包含无效超链接时停止显示元素列表的问题。(#5886)
* 通过任务栏或alt + F4快捷键关闭后，NVDA菜单中的语音查看器复选框现在将反映窗口的实际可见性。(#6340)
* “重载插件”命令不再对触发的配置文件、Web浏览器中的新文档和屏幕浏览造成问题。(#2892，#5380)
* 在NVDA的“常规设置”对话框的语言列表中，Windows 10上可正确的显示Aragonese等语言。(#6259)
* 现在，在输入帮助和输入手势对话框中，用配置的NVDA语言呈现模拟的系统键盘按键(例如点显器上模拟按 Tab 键的按钮)。以前，他们总是用英语显示。(#6212)
* 在重新启动NVDA之前，更改NVDA语言(从“常规设置”对话框)不会生效。(#4561)
* 对于新的语音字典条目，不允许留空“源文本”编辑框。(#6412)
* 修复了在某些系统上扫描串行端口时导致某些点显器驱动程序无法使用的罕见问题。(#6462)
* 在Microsoft Word中，按单元格移动时，表格单元中的项目编号符号现在可被读取。(#6446)
* 现在可以在NVDA输入手势对话框中为Handy Tech点显器驱动程序的按键分配手势。(#6461)
* 在Microsoft Excel中，导航电子表格时按Enter键或numpadEnter现在可以正确地提示导航到下一行。(#6500)
* iTunes不再间歇性地冻结，可正常的使用浏览模式查看iTunes商店、Apple Music等(#6502)
* 修复了64位Mozilla和基于Chrome开发的应用程序的崩溃问题。(#6497)
* 在启用了多进程的Firefox中，浏览模式和可编辑文本框现在可以正常工作。(#6380)

### 开发者需要了解的变化

* 现在可以为名称中包含点 (.) 的可执行文件提供应用模块。点替换为下划线 (_)。(#5323)
* 新的 gui.guiHelper 模块包含简化 wxPython GUI 创建的实用程序，包括自动管理间距。这有利于更好的视觉外观和一致性，并简化盲人开发人员创建新 GUI 的过程。(#6287)

## 2016.3

此版本的亮点包括禁用单个插件的能力；支持 Microsoft Excel 中的表单字段；读出颜色的重大改进；与几个点显器有关的修复和改进；以及Microsoft Word 支持的修复与改进。

### 新特性

* 在 Windows 10 周年更新中，Microsoft Edge 现可使用浏览模式阅读 PDF 文档。(#5740)
* 现在，Microsoft Word 会在适用的情况下读出删除线和双删除线。(#5800)
* 在 Microsoft Word 中，如果表格提供了标题，现在会被读出。若存在描述内容，可通过浏览模式下的打开长描述命令（NVDA+D）进行访问。(#5943)
* 在 Microsoft Word 中，使用 alt+shift+下光标 和 alt+shift+上光标 移动段落时，NVDA 现在会读出位置信息。(#5945)
* 在 Microsoft Word 中，行间距现在可通过 NVDA 的朗读格式命令读出。当使用各类 Word 快捷键修改行间距，或在 NVDA 文档格式设置中启用"读出行间距"功能后，移动至不同行间距的文本时，均会触发提示。(#2961)
* 在 Internet Explorer 中，现已支持识别 HTML5 结构元素。(#5591)
* 现可通过文档格式设置中的"批注"复选框禁用批注朗读（如 Word 中的批注）。(#5108)
* 现可在插件管理器中禁用单个插件。(#3090)
* 新增 ALVA BC640/680 系列点显器的快捷键分配。(#5206)
* 新增命令将盲文显示移至当前焦点。目前仅 ALVA BC640/680 系列点显器分配了按键，其他显示器可在输入手势对话框中手动分配。(#5250)
* 现可与Microsoft Excel 中的表单字段交互。通过元素列表或浏览模式单键导航移动至表单字段。(#4953)
* 现可通过输入手势对话框分配手势以切换简易查看模式。(#6173)

### 改进

* NVDA 现使用 9 种基础色调和 3 种深浅描述颜色，而非主观色彩名称。(#6029)
* NVDA+F9 后按 NVDA+F10 的行为修改为首次按 F10 选择文本，快速连按两次 F10 复制到剪贴板。(#4636)
* 将 eSpeak NG 更新到版本 Master 11b1a7b (22 June 2016)。(#6037)

### 错误修复

* Microsoft Word 浏览模式复制到剪贴板现保留格式。(#5956)
* 在 Microsoft Word 中，NVDA 现在能够在使用 Word 自带的表格导航命令（Alt+Home、Alt+End、Alt+PageUp 和 Alt+PageDown）以及表格选择命令（在导航命令前加 Shift 键）时正确朗读。（#5961）
* 大幅改进 Microsoft Word 对话框中的对象导航。(#6036)
* Visual Studio 2015 等应用中，快捷键（如 Ctrl+C）现正常读出。(#6021)
* 修复某些系统扫描串行端口时导致盲文驱动不可用的问题。(#6015)
* Microsoft Word 颜色报告更准确，考虑 Office 主题变化。(#5997)
* 2016年4月后的 Windows 10 版本再次提供对 Microsoft Edge 浏览模式和开始菜单搜索建议的支持。(#5955)
* 处理合并单元格时，Word 自动表头读取更可靠。(#5926)
* 在 Windows 10 邮件应用程序中，NVDA 不再无法获取邮件内容(#5635)
* 启用"读出命令键"时，大写锁定等键不再重复读出。(#5490)
* 在 Windows 10 周年更新中正确读取用户账户控制对话框。(#5942)
* 网络会议插件（如 out-of-sight.net）不再因麦克风输入相关进度条更新发出提示音。(#5888)
* 浏览模式中"查找下一个/上一个"现正确区分大小写。(#5522)
* 编辑词典条目时，现对无效正则表达式给出反馈。字典文件含无效正则表达式时不再崩溃。(#4834)
* 无法与点显器通信时（如断开连接），自动禁用该点显器。(#1555)
* 部分情况下浏览模式元素列表过滤时的性能小幅提升。(#6126)
* Excel 中背景图案名称现与 Excel 使用的名称一致。(#6092)
* 改进了对 Windows 10 登录屏幕的支持，包括通知警报和通过触摸激活密码字段。(#6010)
* 正确检测 ALVA BC640/680 的辅助路由按钮。(#5206)
* 恢复了对 Windows 10 最新版 Toast 通知的朗读。(#6096)
* 修复了 Baum 兼容和 HumanWare Brailliant B 点显器偶发停止识别按键的问题。(#6035)
* 启用朗读行号时，点显器现显示行号。(#5941)
* 当语音模式关闭时，读出对象（例如按下NVDA+Tab朗读焦点）现会正确显示在语音查看器中。（#6049）
* Outlook 2016 消息列表中不再报告草稿信息。(#6219)
* 非英文版 Chrome 及基于 Chrome 的浏览器中，浏览模式在多数文档中不再失效。(#6249)

### 开发者需要了解的变化

* 直接从属性获取日志信息不再导致递归调用。(#6122)

## 2016.2.1

此版本修复了使 Microsoft Word 崩溃的错误：

* 在 Windows XP 系统中，NVDA 不再导致 Microsoft Word 启动后立即崩溃。(#6033)
* 删除了读出语法错误，因为会导致 Microsoft Word 崩溃。(#5954, #5877)
...
此版本的亮点包括对 Windows 10 的性能改进；包含在 Windows 8 及更高版本的轻松访问中心中； Microsoft Excel 的增强功能，包括列出和重命名工作表以及访问受保护工作表中的锁定单元格；并支持在 Mozilla Firefox、Google Chrome 和 Mozilla Thunderbird 中编辑富文本。

### 新特性

* NVDA 如今会出现在 Windows8 或更高版本的系统的轻松访问中心。(#308)
...
* Outlook 2010/2013 消息列表浏览速度的重大提升。(#5268)
* 在 Microsoft Excel 的单元格中，某些浏览键（例如切换表格 control+上翻页 和 control+下翻页） 现在可以正常工作了。(#5336)
...
* 在 internet explorer 或其他 MSHTML 控件的可编辑输入框的拼写错误朗读支持 (#4174)
* 现在可以读出文本中更多的 unicode 数学符号。(#3805)
* 自动读出 Windows 10 开始屏幕中的搜索建议。(#5049)
* 对 EcoBraille 20、 EcoBraille 40、 EcoBraille 80 和 EcoBraille + 盲文显示器的支持。(#4078)
* 在浏览模式中，你现在可以按 NVDA + shift + 空格键打开和关闭单键导航。关闭时，单个字母键会直接传递给应用程序，这对于 Gmail、 Twitter 和 Facebook 等一些 web 应用程序很有用。(#3203)
* 新的盲文翻译表: 芬兰 6 点点字、爱尔兰1级点字、爱尔兰2级点字、韩语1级点字(2006)、韩语2级点字(2006)。(#5137, #5074, #5097)
* 支持Papenmeier BRAILLEX Live Plus 点显器的QWERTY全键盘。(#5181)
* 对 Windows 10 edge浏览器及edge浏览引擎的实验性支持。(#5212)
* 新语言：埃纳德语。

### 改进

* 将 liblouis 盲文翻译器更新到 2.6.3。(#5137)
* 当你试图安装比当前安装的 NVDA 更早的版本，你现在会收到警告不建议这样做，应该完全卸载之前安装的 NVDA 。(#5037)

### 错误修复

* 在 Internet Explorer 和其他 MSHTML 控件 的浏览模式中，使用表单导航时不再错误的识别仅用作装饰的列表项目。(#4204)
* 在 Firefox 中，当焦点在 ARIA 选项卡面板内移动时，NVDA 不再不恰当地读出 ARIA 选项卡面板的内容。(#4638)
* 在 Internet Explorer 和其他 MSHTML 控件中，切换到区域、控件或对话框不会再错误的读出整个对话框的内容。(#5021, #5025)
* 修复 Baum/HumanWare/APH 所有按键松开之后还输出a0的问题 (#3541)
* 移除使用 alt+tab键进行窗口切换时读出的冗余信息。(#5116)
* 修复在outlook启用盲文输出后键入会有乱码的bug (#2953)
* Internet Explorer 或 其他MSHTML 控件，新增或修改了元素后焦点会被及时刷新，相应内容也会被读出 (#5040)
* 修复 word下进行单键导航时点显器不刷新 (#4968)
* 控件或格式指示器之间或前后的多余空格不再用盲文显示。(#5043)
* 提高 NVDA 在低响应程序的反应速度。(#3831)
* Windows 10 Toast 通知可以正常朗读 (#5136)
* 某些 UIA 组合框值发生改变之后无朗读的修复。
* 在 web 浏览器的浏览模式中， 切换到 frame文档后的tab键动作可以如预期的工作 (#5227)
* windows10 锁定屏幕可以使用触摸屏跳出。(#5220)
* 修复 windows7 或其后版本系统中某些文本编辑软件下盲文输出乱码。(#4291)
* 无法在 windows 10 的锁屏界面读取剪贴板、使用浏览光标访问程序或更改 NVDA 配置。(#5269)

### 开发者需要了解的变化

* 现在，您可以使用新的 KeyboardHandler.injectRawKeyboardInput 函数从 Windows 本身不处理的系统键盘（例如盲文显示器上的 QWERTY 键盘）注入原始输入。(#4576)
* 添加了 eventHandler.requestEvents 以请求默认情况下阻止的特定事件；例如即使在后台也显示来自特定控件的事件或某些事件。(#3831)
* 现在，synthDriverHandler.SynthSetting 不再是单个 i18nName 属性，而是具有单独的 displayNameWithAccelerator 和 displayName 属性，以避免在某些语言循环切换合成器设置时读出加速键。
  * 为了向后兼容，在构造函数中，displayName 是可选的，如果未提供，则将从 displayNameWithAccelerator 派生。但是，如果您打算为某个设置提供加速键，则应同时提供两者。
  * i18nName 属性已弃用，可能会在未来版本中删除。
...
此次发布的重要更新包括 Microsoft Word 和 outlook 的浏览模式支持，Skype 桌面版支持的少量提升及 Microsoft Explorer 支持的显著提升。

### 新特性

* 您可以使用“标点符号读音”对话框新增标点。(#4354)
* 在“输入手势”对话框，您可以使用“过滤关键字”筛选您需要查看的触摸手势。(#4458)
* NVDA 现在可以自动的朗读 mintty 的新文本。(#4588)
* 浏览模式的查找对话框中增加了区分大小写的选项。(#4584)
* 在word 中，现在可以使用“NVDA+空格”切换到浏览模式后使用h等进行元素跳转，也可以使用“NVDA+F7”进行元素筛选。(#2975)
* 提升对 Outlook 2007 及其后版本中的 HTML 消息浏览模式自动切换的支持，如果没有自动切换则可以使用“NVDA+空格”强行切换。(#2975)
* 由作者使用 Microsoft Word 的表格属性指定的列标题行现在可以正常的识别为标题。(#4510)
  * 如果列表被合并，列标题不能正常识别，您还可以手动使用“NVDA+Shift+C”设置列标题行。
* 支持 Skype 桌面版的通知自动朗读。(#4741)
* 在 Skype 桌面版中，您可以使用“NVDA+control+1到0”朗读最近的第x条消息。(#3210)
* 在 Skype 桌面版的绘画框中，输入状态可以自动读出。(#3506)
* 现在您可以使用命令行对 NVDA 进行静默安装，这种方式安装之后不会启动新安装的 NVDA 副本，参数是 --install-silent。(#4206)
* 支持 Papenmeier BRAILLEX Live 20, BRAILLEX Live and BRAILLEX Live Plus 点显器 (#4614)

### 改进

* 在文档格式设置的“读出拼写错误”选项增加了快速键“r”。(#793)
* NVDA 现在使用语音语言来对文字和标点的朗读进行处理，此选项与自动语言切换的开关没有关联，如果您想取消这个特性可以通过取消选中“处理文字和标点时信任语音的语言”复选框。(#4210)
* 对 Newfon 合成器的支持被移除，因为它已经变成了一个插件。(#3184)
* NVDA 仅支持 Skype 7桌面版以上的版本，低版本不再支持。(#4218)
* NVDA 升级的下载变得更加安全，文件信息由 HTTPS 协议传输，且在文件下载完成后会进行 hash 校验。(#4716)
* ESpeak已经被升级到了 1.48.04 (#4325)
...
* 新的语言：哥伦比亚西班牙语，旁遮普与。
* 现在，可以使用 NVDA 退出对话框设置“重新启动”、“重新启动并停用插件”。(#4057)
  * NVDA 也可以使用命令行选项 --disable-addons 来禁用插件。
* 在语音字典中，可以为来源字符定义只匹配整个单词，也就是说，不再匹配长单词的一部分。(#1704)

### 改进

* 如果您使用浏览光标浏览到一个包含在浏览模式文档的对象时，而您上次移动到的对象却并不包含在浏览模式文档中，此时，浏览模式将被设置为文档。以前，仅当浏览光标导致焦点移动后，模式才会被重置。(#4369)
* 点显器、合成器各自的设置对话框中，除了“无点显器”和“无合成器”选项之外的其他元素都按照字母顺序排列，而这两个选项将被放在列表底部。(#2724)
* 升级 liblouis 盲文翻译器为2.6.0。(#4434, #3835)
* 在浏览模式中，按“shift+e或e”键浏览编辑框时包含可编辑组合框，这会包含最新版本的 google 搜索框。(#4436)
* 现在，左键单机通知区域的 NVDA 图标将弹出 NVDA 菜单，而不是什么都不做。(#4459)

### 错误修复

* 当焦点返回浏览模式文档（如使用alt+tab切换到一个已经打开的网页），焦点将被定位在上次浏览的字符上，而不是焦点临近的控件（如最近的链接）。(#4369)
* 在 Powerpoint 幻灯片展示时，浏览光标可正确的跟随虚拟输入焦点。(#4370)
* 在 Mozilla Firefox 和其他 基于 Gecko 的浏览器中， 活动区域的新内容将被读出，即使新的活动区域拥有可用的、与上级活动的区域不同的 ARIA 活动类型， 如为标记为 polite 的活动区域添加标记为 assertive  的内容。(#4169)
* 在 Internet Explorer 和其他 MSHTML 控件， 在一个文档包含于另外一个文档时的某些情况下，不会妨碍用户访问被嵌套文档的内容（特别是被嵌套在 frameset 的 frameset）。(#4418)
* NVDA 不会在当您视图使用 Handy Tech 点显器时崩溃。(#3709)
* 在 Windows Vista 中， 当使用 NVDA 桌面图标或使用其快捷键启动 NVDA 的时候，虚假的 “输入点未找到”的对话框将不再显示。(#4235)
* 最新版本的 Eclipse 中的某些对话框包含的编辑控件存在的一些问题被修复。(#3872)
* 在 Outlook 2010中，在“约会地点”和“会议要求”移动输入焦点时可以正常的工作。(#4126)
* 在一个活动区域中，被标记为 不活动（如aria-live="off"）的内容现在可被正确的忽略。(#4405)
* 当朗读包含名称的状态栏文本时，名称可以被状态栏文本的第一个单词正确拆分。(#4430)
* 在密码输入框启用“朗读输入单词”时，密码将不会被以单词的形式读出。(#4402)
* 在 Microsoft Outlook 消息列表中，消息项目将不会无意义的读出“数据项目”。(#4439)
* 当在 Eclipse IDE 的代码编辑控件中选择文本时， 将不会在每次选择发生变化时读出全部选择的内容。(#2314)
* 变种版本的 Eclipse（ 如Spring 工具套装和被包含在Android 开发工具的捆绑版本）现在可正确的识别为Eclipse 并可正确的控制。(#4360, #4454)
* 在 Internet Explorer 和其他 MSHTML 控件（还包括某些 windows 8 应用程序） 的高DPI 显示器下或文档大小被更改时，鼠标导航和触摸浏览将变得更加准确。(#3494)
* 在 Internet Explorer 和其他 MSHTML 控件  使用鼠标导航或触摸浏览时，按钮的更多标签将被读出。(#4173)
* 当使用带有 BrxCom 的 Papenmeier BRAILLEX 点显器时，   点显器上的按键现在可正常的工作。(#4614)

### 开发者需要了解的变化

* 对于托管许多不同应用程序的可执行文件（例如 javaw.exe），现在可以提供代码来为每个应用程序加载特定的应用模块，而不是为所有托管应用程序加载相同的应用模块。(#4360)
  * 有关详细信息，请参阅 appModuleHandler.AppModule 的代码文档。
  * 实现了对 javaw.exe 的支持。
...
* 启动或退出 NVDA 时播放的声音，可以通过“常规设置”对话框的新选项进行停用。(#834)
* 插件带有的帮助可以在“插件管理器”中进行访问 (#2694)
* 支持 Microsoft Outlook2007以上版本的日历 (#2943) 包括:
  * 使用光标键在日历中移动时朗读当前时间。
  * 如果选择的时间以包含在日程中，做出提示。
  * 使用 tab 键 移动时朗读选中的约会。
  * 智能日期过滤，仅在选择的约会或时间与上次选择的不同时读出日期。
* 增强对 Microsoft Outlook 2010和以上版本的收件箱及其他消息列表的支持 (#3834) 包括：
  * 通过关闭“文档格式设置”对话框的“朗读行列标题”选项，可忽略对行列标题的朗读(如发件人、主题等)。
  * 允许使用表格导航命令(control + alt + 光标键)在单独的列之间移动。
* Microsoft word: 如果内嵌图形没有设置内部文本， NVDA 会通过朗读作者设置的图形标题来代替。(#4193)
* Microsoft Word: NVDA 可以使用朗读格式命令“NVDA+f”组合键读出段落缩进，您也可以通过启用“文档格式设置”对话框的新选项“朗读段落缩进”使其自动读出。(#4165)
* 当您在可编辑文档或文本编辑框按下“回车”键时，朗读自动插入的文本（如新的项目符号、数字和表格标记）。(#4185)
* Microsoft word: 按下“NVDA+alt+c”将朗读光标所在的注释文本。(#3528)
* 提高 Microsoft Excel 的行列标题自动朗读之匙 (#3568) 包括：
  * 支持识别标题单元的 Excel 自定义名称范围(与 JAWS 屏幕阅读器兼容)
  * 使用命令“设置列标题”(NVDA+shift+c)和“设置行标题”(NVDA+shift+r)进行的设置现在存储在工作表内，因而，下次打开此表格时仍旧可用，且与其他支持自定义名称范围方案的屏幕阅读器兼容。
  * 在每个表格中还可多次使用此命令，为不同的区域设置不同的标题。
* 支持 Microsoft Word 的自动行列标题朗读 (#3110) 包括:
 * 支持识别标题单元的 Microsoft Word 标记(与 JAWS 屏幕阅读器兼容)
 - 在表格中的第一个标题单元格上设置列标题 (NVDA+Shift+C) 和设置行标题 (NVDA+Shift+R) 命令允许您告诉 NVDA 应自动读出这些标题。设置存储在文档中，以便下次打开文档时可用，并且可供其他支持书签方案的屏幕阅读器使用。
* Microsoft Word: 按下“tab”键时读出与页面左边界的距离。(#1353)
* Microsoft Word: 为大多数可用的格式快捷键提供语音和盲文反馈(如粗体、斜体、下划线、对齐、大纲级别、超级脚本、子脚本和字体大小)。(#1353)
* Microsoft Excel: 如果选择的单元包含注释，可使用“NVDA+alt+c”读出。(#2920)
* Microsoft Excel: 在使用 Excel 快捷键“shift+f2”进入评论编辑模式时，提供一个 NVDA 定制的对话框来编辑选择单元的评论。
* Microsoft Excel: 为更多的移动选择命令提供语音和点字反馈 (#4211) 包括:
  * 垂直页面移动 (上翻页和下翻页);
  * 水平页面移动 (alt+上翻页和 alt+下翻页);
  * 扩展选择(上面的按键加上shift)
  * 选择当前区域 (control+shift+8)。
* Microsoft Excel: 单元格的垂直对齐和水平对齐方式现在可以使用格式朗读命令“NVDA+f”进行朗读，或通过启用“文档格式设置”的“朗读对齐方式”选项自动的读出。(#4212)
* Microsoft Excel: 单元格的文本样式现在可以使用格式朗读命令“NVDA+f”进行朗读，或通过启用“文档格式设置”的“朗读文本样式”选项自动的读出。(#4213)
* Microsoft PowerPoint: 现在，使用光标键在幻灯片的形状对象中移动时，其当前位置可朗读 (#4214) 包括:
  * 形状对象与幻灯片边界的距离现在可被朗读。
  * 如果形状对象覆盖着其他的形状或者当前形状对象被其他形状覆盖，覆盖距离和覆盖形状都会被读出。
  * 在不希望移动形状的任何时候，可按下位置信息朗读命令“NVDA+数字键盘删除键”读出上述信息。
  * 当选定一个形状对象，如果它被其他形状覆盖，NVDA 会提示“被覆盖”。
* 朗读位置的命令(NVDA+删除)在特定的一些场景下拥有着更加丰富的内容。(#4219)
  * 使用浏览模式浏览标准的编辑区域时，光标的位置（如位置所在内容的百分比和屏幕坐标）都将被读出。
  * 在 Powerpoint 演示的形状对象中，形状对象与幻灯片或其他形状的位置信息将被读出。
  * 连按两次此命令将针对整个控件再度执行先前的动作，读出位置信息。
* 新的语言，加泰罗尼亚语。

### 改进

* 更新 liblouis 盲文翻译器到2.5.4. (#4103)

### 错误修复

* Google Chrome 和基于其开发的浏览器，，警告或对话框的当前文本块(如突出文本)不会再被重复朗读。(#4066)
* 在 Mozilla 应用程序使用浏览模式，在按钮等控件上按下回车不会再发生某些时候，点击控件失败（或点击了错误的控件）的情况。(#4106)
* 在浏览 iTunes 时不在朗读无用信息。(#4128)
* 在 iTunes 的当前列表（如音乐播放列表），使用对象导航浏览下一个项目的命令可以正常工作。(#4129)
* HTML 元素现在被当成标题，因为 WAI ARIA 标记已经被包含在浏览模式元素列表和 Internet Explorer 的快捷键导航。(#4140)
* 在最近版本的 Internet Explorer 的浏览文档中，跟随到同页面链接现在将可正确的移动和朗读目标位置。(#4134)
* Microsoft Outlook 2010 和以上版本: 所有安全对话框(如“新建配置文件”对话框、“邮件设置”对话框)的可访问性都得到了提高。(#4090, #4091, #4095)
* Microsoft Outlook: 在当前对话框浏览时，命令工具栏的无用提示信息减少。(#4096, #3407)
* Microsoft word: 切换到表格中的空白单元时，不会再错误的提示为“离开表格”。(#4151)
* Microsoft Word: 表格外的第一个字符（包含着新的空白航）不会被错误地认为属于此表格。(#4152)
* Microsoft Word 2010 拼写检查对话框: 实际的拼写错误单词将被朗读，而不仅仅是不恰当的朗读第一个黑体单词。(#3431)
* 在 Internet Explorer ，切换或使用单键浏览找到表单时朗读出其标签（特别是 HTML 元素已经定义好标签的情况）。(#4170)
* Microsoft Word: 注释的存在和位置将读的更加准确。(#3528)
* 浏览某些对话框时，通过不在读出某些对用户无用的特别的控件容器工具栏的方式，提高对 Microsoft Office 产品(word、OutLook、Excel等)的支持。(#4198)
* 任务窗格（如文件恢复器、剪贴板管理器）不会再发生偶然丢失焦点的情况，这个情况会发生在打开某些程序时（如Word、Excel等），用户再也无需切换到其他窗口后返回才能使用文档或电子表格。(#4199)
* 在系统语言被设置成塞尔维亚语（拉丁）时，NVDA 不会发生无法运行在最新的 Windows 操作系统的情况。(#4203)
* 现在，在打开“帮助模式”的情况下，按下数字键盘切换可以正常的切换状态，而不是强制键盘和系统保持在一种状态。(#4226)
* 在 Chrome 切换标签时，文档标题将再次可以正常的朗读，这种情况不会发生在 NVDA2014.2。(#4222)
* 在使用 Chrome 或基于 Chrome 所开发的浏览器，在朗读文档时文档的 URL 将不会被朗读。(#4223)
* 当在选择“无合成器”时进行全文朗读，内容将可完全读完，而不会再发生在读完某些行之后自动停止的情况。(#4225)
* Microsoft Outlook的签名对话框: 签名编辑区域将具有完全的可访问性，允许完全的鼠标导航和格式检测。(#3833)
* Microsoft Word: 当朗读表格单元的最后一行时，不会发生读出整个表格的情况。(#3421)
* Microsoft Word: 当朗读表格单元的第一行或最后一行时，不会发生读出整个列表的情况。(#3421)
* 在朗读输入字词打开时，词语不会再被某些印第安语的符号打断。(#4254)
* GoldWave 的数字可编辑文本区域现在可以正确获取。(#670)
* Microsoft Word: 当使用 control+下光标或control加上光标进行段落跳转时，无需在通过按下两次该键来略过项目列表和数字列表。(#3290)

### 开发者需要了解的变化

* NVDA 现在拥有统一的插件文档支持，详情请查阅开发文档的插件帮助章节。(#2694)
* 当为 scriptableObject 和 gesture 提供 gesture绑定时，允许为脚本设定关键字为空，取消了许多基本类的绑定。(#4240)
* 现在，您可以为某些启动快捷键会造成问题的地区修改启动 NVDA 的快捷键。(#2209)
  * 此功能通过 gettext 实现。
  * 请注意，在安装 NVDA 对话框，设置启动快捷方式的快捷键如果被更改，也应当在帮助文档中体现。
...
* Gettext message contexts are now supported. This allows multiple translations to be defined for a single English message depending on the context. (#1524)
  ...
  * 有关详细信息，请参阅 http://www.gnu.org/software/gettext/manual/html_node/Contexts.html#Contexts for more information。
...
* 为了方便第三方开发者，现在可以重新加载应用模块和全局插件，而无需重启 NVDA。使用 NVDA 菜单中的工具 -> 重新加载插件或 NVDA+Ctrl+F3。(#544)
...
* NVDA 不再错误地自动为可编辑文本字段启用焦点模式，这些文本字段会在焦点更改时更新其值；例如http://tigerdirect.com/。(#220)
...
* 添加查看 NVDA 当前日志文件的功能。可在 NVDA 菜单 -> 工具中找到
...
* NVDA now has a built-in synthesizer called eSpeak, developed by Jonathan Duddington.It is very responsive and lite-weight, and has support for many different languages. Sapi synthesizers can still be used, but eSpeak will be used by default.
  ...
  * 有关 eSpeak 的更多信息或查找其他版本，请访问 http://espeak.sourceforge.net/。
