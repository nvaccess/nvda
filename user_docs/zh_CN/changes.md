...
此版本引入了 NVDA 远程访问功能，支持您通过一台运行 NVDA 的设备远程操控另一台运行 NVDA 的计算机。

语音模块迎来多项改进，显著提升了 SAPI 4、SAPI 5 和 OneCore 语音的响应速度。
SAPI 5 现在支持语速加倍和自动语言切换。
SAPI 4 语音现支持音频闪避、去除语音开头的静音及保持音频设备唤醒。

插件商店的自动更新系统已改进，允许您选择自动更新通道并在后台运行自动更新。

现在通过新命令可以更轻松地手动刷新 OCR 识别结果、切换自动刷新的开关。

Chrome 和 Edge 浏览器现在支持应用原生选择。

Microsoft Office 和 LibreOffice 支持已改进，特别是可读出更多快捷键的操作结果。

现在可以根据 NVDA 语言确定默认的输入和输出盲文表。
新增对 Dot Inc 公司的 Dot Pad 触觉图形设备作为多行盲文点显器的原生支持。
盲文点显器还有其他小修复，特别是在连接性方面。

LibLouis 盲文翻译器、eSpeak-NG 和 Unicode CLDR 已更新。
...
* 插件商店：
  * 自动更新 (#3208)：
    ...
    * 现在可以自动将不兼容的插件更新到另一个较新的不兼容版本。
      * 可在 NVDA 设置的插件商店面板中启用此功能。
  * 新增取消安装插件的操作。 (#15578, @hwf1324)
  * 新增当插件下载/安装失败时重试的操作。 (#17090, @hwf1324)
  * 插件列表现在可按列排序（包括发布日期），支持升序和降序排列。 (#15277, #16681, @nvdaes)
* 语音：
  * 现在使用 Microsoft Speech API version 5（SAPI 5）和 Microsoft Speech Platform 语音时支持自动语言切换。 (#17146, @gexgd0419)
  * 现在使用 Microsoft Speech API version 5（SAPI 5）和 Microsoft Speech Platform 语音时支持语速加倍，最高可达 6 倍速。 (#17606, @gexgd0419)
* Web 浏览器：
  * 现在 Google Chrome、Edge 和其他基于 Chromium 134 及更高版本的 应用程序支持 NVDA 浏览模式的应用原生选择（`NVDA+shift+f10`）。 (#17838)
  * 在 Mozilla Firefox 中，访问包含文本片段的网址时，NVDA 将读出高亮显示的文本。 (#16910, @jcsteh)
  * NVDA 现在可以读出链接目标是否指向当前页面。 (#141, @LeonarddeR, @nvdaes)
* Microsoft Office：
  * 在 PowerPoint 文本框中编辑时，现在可以使用`alt+上光标`/`alt+下光标`逐句移动。 (#17015, @LeonarddeR)
  * 在 Word 中使用扩展或缩小选区命令（`f8` 或 `shift+f8`）时，现在会读出选区更新。 (#3293, @CyrilleB79)
  * 在 Word 16.0.18226 及更高版本或使用 Word 对象模型时，NVDA 将通过语音和盲文提示标题是否折叠。 (#17499)
  * 在使用 UIA 的 Word 中，使用原生扩展表格导航命令（`alt+home`、`alt+end`、`alt+pageUp`、`alt+pageDown`）时，现在会读出输入焦点移动。 (#17867, @CyrilleB79)
  * 在 Word 中使用“读出焦点”命令时，如果文档布局信息可用且启用了读出对象描述，将会读出文档布局。 (#15088, @nvdaes)
  * 现在 Word 和 Outlook 中会读出更多快捷键的操作结果：
    * 字体格式快捷键(#10271, @CyrilleB79)
    * 折叠/展开标题(#17545, @CyrilleB79)
* LibreOffice：
  * 在 Writer 中使用键盘快捷键增大/减小字号时，NVDA 会读出新字号。 (#6915, @michaelweghorn)
  * 在 Writer 25.2 或更高版本中使用对应快捷键应用“正文文本”或标题段落样式时，NVDA 会读出新段落样式。 (#6915, @michaelweghorn)
  * 在 Writer 中使用快捷键切换双下划线时，NVDA 会读出新状态（“双下划线开启”/“双下划线关闭”）。 (#6915, @michaelweghorn)
  * NVDA 现可在 LibreOffice 25.8 或更高版本中读出首行缩进。(#13052, @michaelweghorn)
* OCR：
...
* 现可为 NVDA 更新和插件商店指定镜像网址。 (#14974, #17151, #17310, @christopherpross)
* 新增对 PDF 数学公式的支持。 (#9288, @NSoiffer)
  * 这适用于包含 MathML 的公式（如新版 TeX/LaTeX 生成的部分文件）。
  * 目前仅支持 Foxit Reader 和 Foxit Editor。
* 现在可以配置 NVDA 在使用盲文导航键导航时读出当前行或段落。 (#17053, @nvdaes)
* NVDA 现可在按 `alt+上光标`或 `alt+下光标` 时读出输入焦点变化（例如在 Visual Studio Code 中）。 (#17652, @LeonarddeR)
...
* NVDA 界面更改：
  * NVDA 安装程序启动时不再播放提示音。 (#14068)
  * “读出链接网址”、“读出字符格式信息”和“读出选中文本”的浏览模式对话框新增了“关闭”和“复制”按钮以提升用户体验。 (#17018, @XLTechie)
  * 退出对话框现在允许您同时禁用插件并启用调试日志记录来重启 NVDA。 (#11538, @CyrilleB79)
  * 移除了禁用 WASAPI 音频输出的选项。 (#16080)
  * 移除了浏览模式设置中的自动将焦点跳转到可聚焦元素，该功能现已默认关闭。 (#17598)
...
* 语音：
  ...
  * 语音输出的 Unicode 规范化现默认启用。 (#17017, @LeonarddeR).
    * 仍可在 NVDA 设置的“语音”类别中关闭此功能。
* “读出输入字符”和“读出输入单词”的键盘设置现包含三个选项：关闭、仅在编辑控件中、始终。 (#17505, @Cary-rowen)
  * “读出输入字符”的默认模式已更改为“仅在编辑控件中”。
...
* 语音：
  ...
  * 使用循环切换合成器设置切换 SAPI5/SAPI4 语音时，语速、音量等语音参数不再重置为默认值。 (#17693, #2320, @gexgd0419)
  * 修复了部分SAPI4语音（如IBM TTS Chinese 即 CHSTTS 语音）无法加载的问题。 (#17726, @gexgd0419)
* 盲文：
  * 新增对 Dot Inc 公司的 Dot Pad 触觉图形设备作为多行盲文点显器的原生支持。 (#17007)
  * 使用 Seika Notetaker 时，space 以及 space + 点的手势现能在按键与手势对话框中正确显示。 (#17047, @school510587)
  * 未连接点显器时，使用显示语音输出盲文模式不再触发错误提示音或日志文件刷屏。 (#17092, @Emil-18)
  ...
  * 禁用插件运行时，开发者实验目录中的自定义盲文表现会被正确忽略。 (#17565, @LeonarddeR)
* Microsoft Office：
  * 在 Word、Outlook、Excel、PowerPoint 使用旧版对象模型时，读出链接网址命令现可正常生效。 (#17292, #17362, #17435, @CyrilleB79)
  * Excel 中的元素列表对话框（`NVDA+f7`）现可在非英文系统中正常列出批注和公式。 (#11366, @CyrilleB79)
  * PowerPoint 的改进：
    ...
    * 禁用“页面加载完成后朗读所有内容”选项后，启动幻灯片放映时 NVDA 不再自动开启全文朗读。 (#17488, @LeonarddeR)
* LibreOffice：
  * 使用微软拼音输入法并开启旧版兼容模式时，Writer 等应用中 IME 弹窗显示期间输入不再出现错误。 (#17198, @michaelweghorn)
  * 对话框中复选框的选中状态现同时通过盲文与语音反馈。 (#17218, @michaelweghorn)
* 数学：
  * 修复部分网页数学元素的阅读问题。
  特别是带 `role="math"` 属性的 span 等元素内的 MathML。 (#15058)
  * 仅有图片替代文本而无 MathML 的公式现视为普通图像，支持用`g`跳转并逐字浏览替代文本。 (#16007)
* 开发工具：
  * 在 Android Studio/IntelliJ Idea 中选择特定源码文件全文时，NVDA 不再崩溃。 (#17418, @thgcode)
  ...
  * Visual Studio Code 中的部分区域现能被正确识别为可编辑控件。 (#17573, @Cary-rowen)
...
* 配置文件：
  * 使用关联的配置激活全文朗读时，盲文功能不再失效。 (#17163, @LeonarddeR)
...
* 拼读时，Unicode 规范化现在更合理：
  ...
  * 组合字符现能正确读出。 (#17295, @LeonarddeR)
* NVDA is able to read the popup submenu items on Thunderbird search results page. (#4708, @thgcode)
* COM 注册修复工具现不会在失败时误报成功。 (#12355, @XLTechie)
* 当存在项目时关闭窗口，NVDA 将不再读出 Windows 11 剪贴板历史条目。 (#17308, @josephsl)
* 在浏览模式消息窗口打开时重新加载插件，NVDA 不再无法读出后续的焦点移动。 (#17323, @CyrilleB79)
* 使用Skype、Discord、Signal、Phone Link 等应用进行语音通信时，NVDA 的语音和音效音量不再降低。 (#17349, @jcsteh)
* 获取快照变量出错时，打开 NVDA Python 控制台不再失败。 (#17391, @CyrilleB79)
* 修复了网络浏览器中可编辑文本控件的文本选择更改有时无法被读出的错误。 (#17501, @jcsteh)
...
* 重启 Explorer 后，NVDA 高亮窗口图标不再固定在任务栏。 (#17696, @hwf1324)
...
* 组件更新：
  ...
  * 将markdown 更新到 3.7。 (#17459, @josephsl)
