...
Ova verzija donosi udaljeni pristup, omogućujući vam da kontrolišete udaljeni računar s pokrenutim NVDA.

Tu je i mnogo poboljšanja govora, posebno odziva Sapi 4, Sapi 5 i OneCore glasova.
Dodatna brzina i automatsko mijenjanje jezika je sada podržano i u SAPI 5 glasovima.
SAPI 4 glasovi sada podržavaju stišavanje zvuka, uklanjanje tišine iz zvuka govora, kao i održavanje zvučnog uređaja budnim.

Sistem automatskog ažuriranja trgovine dodataka je poboljšan, pa vam sada omogućava odabir kanala za automatsko ažuriranje i pokretanje ažuriranja u pozadini.

Dodane su nove komande za ručno osvježavanje OCR rezultata i uključivanje automatskog osvježavanja OCR rezultata.

Način izvornog odabira je sada dostupan u Chromeu i Edgeu.

Podrška za Microsoft Office i LibreOffice je poboljšana, s obzirom na veći broj prečica koje NVDA sada najavljuje.

Zadane ulazne i izlazne brajeve tabele sada mogu biti određene na osnovu NVDA jezika.
Dodana je izvorna podrška za Dot Pad taktilni grafički uređaj od Dot Inc.
Izvršene su i druge manje ispravke za brajeve zaslone, posebno u vezi s povezivanjem.

Liblouis prevoditelj brajice, eSpeak NG i Unicode CLDR su ažurirani.
Nove brajeve table su dostupne: koptska, kompaktno klinasto pismo i portugalska šestotačkasta računarska brajica.

### Važne napomene

* Ova verzija prekida kompatibilnost s postojećim dodacima.

### Sigurnosne popravke

Molimo vas da odgovorno prijavite sigurnosne probleme u skladu s [NVDA sigurnosnom politikom](https://github.com/nvaccess/nvda/blob/master/security.md).

* Sprječava prikazivanje potencijalno osjetljivih podataka na Brajevim zaslonima kada je uređaj zaključan.
([GHSA-8f8q-2jc3-6rf4](https://github.com/nvaccess/nvda/security/advisories/GHSA-8f8q-2jc3-6rf4))
* Sprječava instalacijski program da učita neželjene DLL-ove iz svog direktorija.
...
* Trgovina dodataka:
  * Automatska ažuriranja (#3208):
    * Kanali za automatsko ažuriranje dodataka sada mogu biti promijenjeni.
      * Kanal za automatsko ažuriranje instaliranih dodataka može biti odabran iz podizbornika "Kanal ažuriranja".
      * Zadani kanal za automatska ažuriranja može biti postavljen u postavkama trgovine dodataka.
    * Automatska ažuriranja sada se mogu odvijati u pozadini.
      * Ova opcija se može uključiti u postavkama trgovine dodataka, promjenom vrijednosti kombinovanog okvira "Automatska ažuriranja" na "Automatski ažuriraj".
    * Automatska ažuriranja sada mogu ažurirati nekompatibilne dodatke na novu, kompatibilnu verziju.
      * Ovo se može uključiti u postavkama trgovine dodataka.
  * Dodana je radnja za otkazivanje instaliranja dodataka. (#15578, @hwf1324)
  * Dodana je radnja za ponovni pokušaj ako preuzimanje ili instaliranje dodatka nije uspjelo. (#17090, @hwf1324)
  * Popis dodataka može se sortirati po kolonama, uključujući datum objavljivanja, uzlaznim i silaznim redoslijedom. (#15277, #16681, @nvdaes)
* Govor:
  * Automatsko mijenjanje jezika je sada podržano i u Microsoft SAPI 5 i Microsoft Speech Platform glasovima. (#17146, @gexgd0419)
  * Dodatna brzina je sada podržana i u Microsoft SAPI 5 i Microsoft Speech  Platform glasovima, koji podržavaju ubrzanje do šest puta. (#17606, @gexgd0419)
* Web pretraživači:
  * NVDA izvorni način odabira (`NVDA+Shift+F10`) je sada podržan u Google Chromeu, Edgeu i drugim aplikacijama temeljenim na Chromiumu 134 ili novijem. (#17838)
  * U Mozilla Firefoxu, NVDA će prijaviti istaknuti tekst kada se posjeti poveznica koja sadrži tekstualni fragment. (#16910, @jcsteh)
  * NVDA sada može prijaviti kada odredište poveznice vodi na istu, trenutnu stranicu. (#141, @LeonarddeR, @nvdaes)
* Microsoft Office:
  * Pri uređivanju u tekstualnim okvirima u PowerPointu, sada se možete kretati po rečenicama s prečicom `Alt+Strelica gore` ili `Alt+Strelica dolje`. (#17015, @LeonarddeR)
  * U Wordu se sada prijavljuje ažuriranje odabira kada se koriste Wordove komande za proširivanje ili smanjivanje odabira (`F8` ili `Shift+F8`). (#3293, @CyrilleB79)
  * U Wordu verzije 16.0.18226 i novijim, ili kada se koristi Wordov objektni model, NVDA će sada govorom i brajicom prijaviti ako je naslov sažet. (#17499)
  * U Wordu s UIA, pri korištenju izvornih proširenih komandi za kretanje po tabelama (`Alt+Home`, `Alt+End`, `Alt+PageUp` i `Alt+PageDown`), sada će se prijavljivati pomjeranje kursora. (#17867, @CyrilleB79)
  * U Wordu, pri korištenju komande "Prijavi fokus", izgovorit će se izgled dokumenta ako je ta informacija dostupna i ako je uključeno prijavljivanje opisa objekata. (#15088, @nvdaes)
  * Rezultati većeg broja prečica sada se prijavljuju u Wordu i Outlooku:
    * prečice za oblikovanje fonta (#10271, @CyrilleB79)
    * sažimanje ili proširivanje naslova (#17545, @CyrilleB79)
* LibreOffice:
  * Pri povećavanju ili smanjivanju veličine fonta u Writeru pomoću odgovarajućih prečica, NVDA izgovara novu veličinu fonta. (#6915, @michaelweghorn
  * Pri primjeni stila odlomka ili naslova pomoću odgovarajuće prečice u Writeru 25.2 ili novijem, NVDA izgovara novi stil odlomka. (#6915, @michaelweghorn)
  * Pri uključivanju ili isključivanju dvostrukog podvlačenja u Writeru pomoću odgovarajuće prečice, NVDA izgovara novo stanje ("dvostruko podvlačenje je uključeno"/"dvostruko podvlačenje je isključeno"). (#6915, @michaelweghorn)
  * NVDA sada može prijaviti uvlačenje prvog reda u LibreOfficeu verzije 25.8 i novijim. (#13052, @michaelweghorn) (#13052, @michaelweghorn)
* OCR:
  * U prozoru rezultata prepoznavanja, prečica `NVDA+F5` ručno osvježava prepoznati rezultat. (#17715, @CyrilleB79)
  * Dodana je nedodijeljena gesta za uključivanje i isključivanje automatskog osvježavanja Windows OCR rezultata. (#16897)
* Funkcija udaljenog pristupa, bazirana na dodatku NVDA Remote, je integrisana u NVDA. (#4390, @ctoth, @tspivey, @daiverd, saradnici i donatori NVDA Remote dodatka)
* Sada je moguće odrediti adrese za ažuriranje za NVDA i trgovinu dodataka. (#14974, #17151, #17310, @christopherpross)
* Dodana je podrška za matematiku u PDF dokumentima. (#9288, @NSoiffer)
  * Ovo funkcioniše za formule sa povezanim MathML-om, kao što su neke datoteke stvorene novijim verzijama TeX/LaTeX-a.
  * Trenutno je ovo podržano samo u Foxit Readeru i Foxit Editoru.
* NVDA sada može biti podešen da izgovara trenutni red ili odlomak pri kretanju brajevim navigacijskim tipkama. (#17053, @nvdaes)
* NVDA sada može prijaviti promjene kursora prečicom `Alt+Strelica gore` ili `Alt+Strelica dolje`, na primjer u Visual Studio Codeu. (#17652, @LeonarddeR)
* Dodane su komande za pomjeranje preglednog kursora na prvi i posljednji znak odabranog teksta, dodijeljene prečici `NVDA+Alt+Home` i `NVDA+Alt+End`. (#17299, @nvdaes)
* Dodana je opcija u općim postavkama koja sprječava zaslon da se isključi dok je aktivna funkcija "Pročitaj sve" ili pri čitanju brajice.
Ova opcija je po zadanim postavkama uključena, ali može uzrokovati povećanje potrošnje baterije. (#17649, @LeonarddeR)
* NVDA je sada preveden na bosanski. (#17953)
* U Adobe Acrobatu, NVDA sada može čitati i upravljati matematičkim jednačinama u PDF dokumentima stvorenim u novijim verzijama Worda. (#18056)

### Promjene

* Ažuriranja komponenti:
  * LibLouis brajev prevoditelj je ažuriran na verziju [3.33.0](https://github.com/liblouis/liblouis/releases/tag/v3.33.0). (#17469, #17768, @LeonarddeR, @codeofdusk)
    * Dodane su nove brajeve table: koptska, kompaktno klinasto pismo i portugalska šestotačkasta računarska brajica.
  * CLDR je ažuriran na verziju 46.0. (#17484, @OzancanKaratas)
  * Govorna sinteza eSpeak je ažurirana na verziju 1.52.0. (#17056)
* Promjene NVDA sučelja:
  * NVDA instalacijski program više ne reprodukuje zvuk pri pokretanju. (#14068)
  * Radi lakšeg korištenja, dijalozi odredišne adrese poveznice, oblikovanja i odabranog teksta, sada sadrže dugmad "Zatvori" i "Kopiraj". (#17018, @XLTechie)
  * Dijalog za izlaz sada vam omogućava ponovno pokretanje s onemogućenim dodacima i uključenim zapisnikom grešaka istovremeno. (#11538, @CyrilleB79)
  * Uklonjena je opcija za onemogućavanje korištenja WASAPI-ja kao zvučnog izlaza. (#16080)
  * Postavka načina pretraživanja pod nazivom "Automatski postavi sistemski fokus na elemente koji se mogu fokusirati" je uklonjena, a ponašanje je sada onemogućeno. (#17598)
  * NVDA će ubuduće upozoravati na nekompatibilne dodatke samo ako nova verzija koristi API koji nije kompatibilan s onim u trenutnoj verziji. (#17071, #17506)
  * Datoteka doprinositelja ("Contributors") je uklonjena iz NVDA izbornika. (#16922)
  * NVDA licenca je sada oblikovana u HTML-u za poboljšanje čitljivosti. (#17600)
  * Dodane su kratke verzije najkorištenijih opcija komandne linije: `-d` za `--disable-addons` i `-n` za `--lang`.
  Više nije podržano korištenje prefiksa kod opcija komandne linije (na primjer, `--di` za `--disable-addons`. (#11644, @CyrilleB79)
  * Promjene u alatu za popravljanje COM registracija: (#12355, @XLTechie)
    * Sada se pokreće s korisniku jasnijim objašnjenjem svoje svrhe, umjesto s upozorenjem. (#12351)
    * Početni prozor se sada može zatvoriti pomoću tipke Escape ili prečice Alt+F4. (#10799)
    * U rijetkom slučaju Windows greške pri pokušaju popravljanja COM registracija, korisniku će sada biti prikazana poruka s opisom greške.
* Govor:
  * Microsoft SAPI5 i Microsoft Speech Platform glasovi sada koriste WASAPI za zvučni izlaz, što bi moglo poboljšati odziv ovih glasova. (#13284, @gexgd0419)
  * Tišina na početku govora će biti uklonjena pri korištenju OneCore, SAPI5, i nekih glasova treće strane, u cilju poboljšanja njihovog odziva. (#17614, @gexgd0419)
  * Microsoft SAPI4 glasovi sada koriste WASAPI kao zvučni izlaz, tako da mogu raditi s funkcijama kao što su stišavanje zvuka, uklanjanje tišine iz zvuka, kao i održavanje zvučnog uređaja budnim.
  Ako ovo ne radi s vašim SAPI4 glasom, možete onemogućiti WASAPI za SAPI4 u naprednim postavkama. (#17718, #17801, @gexgd0419)
  * Unicode normalizacija govora je sada prema zadanim postavkama omogućena. (#17017, @LeonarddeR).
    * I dalje možete onemogućiti ovu funkciju u kategoriji govora NVDA postavki.
* Funkcija "Izgovaranje unesenih slova" i "Izgovaranje unesenih riječi" sada ima tri opcije: isključeno, samo u poljima za uređivanje i uvijek. (#17505, @Cary-rowen)
  * Po zadanim postavkama, "Izgovaranje unesenih znakova" je sada podešeno na "Samo u poljima za uređivanje".
* Zadane ulazne i izlazne brajeve tabele sada mogu biti određene na osnovu NVDA jezika. (#17306, #16390, #290, @nvdaes)

### Ispravke grešaka

* Govor:
  * Ispravljen je problem kada se kontinuirano čitanje (pročitaj sve) prekidalo na kraju prve rečenice pri korištenju nekih SAPI5 glasova. (#16691, @gexgd0419)
  * Parametri glasa, kao što su brzina i glasnoća, više neće biti vraćeni na zadane postavke pri korištenju prstena postavki govorne sinteze za promjenu glasova u SAPI4 ili SAPI5 sintezama. (#17693, #2320, @gexgd0419)
  * Ispravljen je problem u kojem se neki SAPI4 glasovi (na primjer, kineski IBM TTS) nisu mogli učitati. (#17726, @gexgd0419)
* Brajica:
  * Dodana je izvorna podrška za Dot Pad taktilni grafički uređaj od Dot Inc. (#17007)
  * Pri korištenju Seika Notetaker brajevog zaslona, komande "razmak" i "razmak s tačkicama" su sada ispravno prikazane u dijalogu ulaznih komandi. (#17047, @school510587)
  * NVDA više neće reprodukovati zvukove greški ili pretrpavati datoteku zapisnika pri korištenju brajevog načina "Prikaži govor" ako brajev zaslon nije povezan. (#17092, @Emil-18)
  * Kada je upravljački program za Standard HID brajev zaslon eksplicitno odabran kao upravljački program za brajev zaslon, i kada se otvori popis brajevih zaslona, NVDA sada ispravno prikazuje HID upravljački program kao odabrani, umjesto da prikazuje da nijedan nije odabran. (#17537, @LeonarddeR)
  * Upravljački program za Humanware Brailliant sada pouzdanije bira ispravan komunikacijski kanal, što rezultuje stabilnijom povezanošću i manjim brojem grešaka. (#17537, @LeonarddeR)
  * Prilagođene brajeve tabele u direktoriju Developer Scratchpad su sada ispravno ignorisane dok su dodaci onemogućeni. (#17565, @LeonarddeR)
  * Ispravljen je problem kada NVDA nije prepoznavao neke brajeve zaslone. (#18114, @christiancomaschi)
* Microsoft Office:
  * Komanda za prijavljivanje odredišne adrese poveznice sada radi pri korištenju zastarjelog objekt modela u Wordu, Outlooku, Excelu i PowerPointu. (#17292, #17362, #17435, @CyrilleB79)
  * U Excelu, dijalog popisa elemenata (`NVDA+F7`), sada uspješno prikazuje komentare i formule i na sistemima koji nisu na engleskom jeziku. (#11366, @CyrilleB79)
  * Poboljšanja u PowerPointu:
    * Prijavljivanje pozicije kursora više se ne prekida kada tekst sadrži široke znakove, kao što su emotikoni. (#17006 , @LeonarddeR)
    * Prijavljivanje lokacije preglednog kursora sada je tačno (na primjer, pritiskanjem prečice `NVDA+Delete`. (#9941, @LeonarddeR)
    * NVDA više ne pokreće funkciju "Pročitaj sve" pri pokretanju prezentacije ako je opcija "Automatski pročitaj sve pri učitavanju stranice" isključena." (#17488, @LeonarddeR)
* LibreOffice:
  * Kada se koristi Microsoft Pinyin način unosa za kineski i uključi opcija kompatibilnosti za stariju verziju, tipkanje u Writeru (i eventualno drugim), dok je prikazan IME skočni izbornik, više ne izaziva grešku." (#17198, @michaelweghorn
  * Trenutno stanje potvrdnog okvira (potvrđeno/nije potvrđeno) u dijalozima se sada prijavljuje i brajicom, a ne samo govorom. (#17218, @michaelweghorn)
* Matematika:
  * Ispravljeno je čitanje matematičkih sadržaja u nekim web elementima.
  Konkretno, MathML unutar span elemenata i drugih elemenata koji imaju atribut `role="math"`. (#15058)
  * Matematičke jednačine koje su bile prikazane samo kao slika s alternativnim tekstom, bez MathML-a za bogatu navigaciju, sada se tretiraju kao obične slike, a ne kao matematika bez sadržaja. Korisnik sada može prijeći na njih pomoću tipke `G` i kretati se kroz alternativni tekst po znakovima koristeći strelice. (#16007)
* Integrisana razvojna okruženja (IDEs):
  * NVDA se više neće rušiti pri odabiru cijelog teksta u nekim datotekama koda u programima Android Studio ili IntelliJ Idea. (#17418, @thgcode)
  * U Visual Studio Codeu, NVDA neće više blokirati prečice `Alt+Strelica gore` i `Alt+Strelica dolje` za kretanje po rečenicama. (#17082, @LeonarddeR)
  * Određeni elementi odjeljaka sada su ispravno prepoznati kao uredive kontrole u Visual Studio Codeu. (#17573, @Cary-rowen)
  * U Notepad i drugim UIA dokumentima, kao i Notepad++ dokumentima na Windowsu 11, ako je posljednji red prazan, komanda brajevog zaslona za pomjeranje na sljedeći red će pomjeriti kursor na posljednji red.
  U bilo kojem dokumentu, ako je kursor na posljednjem redu, bit će pomjeren na kraj pri korištenju ove komande. (#17251, #17430, @nvdaes)
* Konfiguracijski profili:
  * Brajica više nije nefunkcionalna pri korištenju funkcije "Pročitaj sve" u kombinaciji sa dodijeljenim konfiguracijskim profilom. (#17163, @LeonarddeR)
  * Ispravljen je problem kada su određene postavke bile spremane isključivo u profil aktivne konfiguracije, iako je vrijednost te postavke jednaka onoj iz standardne. (#17157, @leonarddeR)
* Pri izgovaranju slovo po slovo, Unicode normalizacija sada radi ispravnije:
  * Nakon prijavljivanja normalizovanog znaka, NVDA više neće neispravno prijavljivati sljedeće znakove kao normalizovane. (#17286, @LeonarddeR)
  * Složeni znakovi (kao što su "é") sada se ispravno prijavljuju.
* NVDA sada može čitati skočne  podizbornike na stranici rezultata Thunderbird pretrage. (#4708, @thgcode)
* Alat za popravljanje COM registracija više ne prikazuje poruku da je operacija uspjela ako nije. (#12355, @XLTechie)
* NVDA više neće najavljivati historiju Windows 11 međuspremnika pri zatvaranju prozora. (#17308, @josephsl)
* Ako su dodaci ponovno učitani dok je otvorena neka poruka u načinu pretraživanja, NVDA će sada prijavljivati naredne promjene fokusa. (#17323, @CyrilleB79)
* Pri korištenju aplikacija kao što su Skype, Discord, Signal i Povezivanje s telefonom, NVDA govor i zvukovi se više ne smanjuju. (#17349, @jcsteh)
* NVDA Python konzola se sada uspješno otvara čak i ako dođe do greške tokom dohvaćanja snapshot varijabli. (#17391, @CyrilleB79)
* U web pretraživačima se sada pouzdano prijavljuju promjene odabira teksta unutar polja za uređivanje. (#17501, @jcsteh)
* Kada poveznice vode do istog objekta na kom se virtuelni kursor već nalazi, NVDA će se ispravno pomjeriti na odredište poveznice. (#17669, @nvdaes)
* Ikonica NVDA Highlighter prozora više ne ostaje "zaglavljena" u traci zadataka nakon ponovnog pokretanja Explorera. (#17696, @hwf1324)
...
Ova verzija uključuje brojna poboljšanja u Microsoft Officeu, brajici i oblikovanju dokumenata.

U Wordu ili Excelu, sada je moguće dvaput pritisnuti gestu za komentar kako biste pročitali komentar ili bilješku u dijaloškom okviru za pregledavanje.
...
* Component updates:
  ...
  * eSpeak NG has been updated to 1.52-dev commit `961454ff`. (#16775)
    * Dodani su novi jezici: faerski i Xextan.
...
Tu je nova funkcija pod nazivom podjela zvuka.
...
* Trgovina dodataka:
  * Minimalna i posljednja testirana NVDA verzija sada su prikazane u području "Više informacija". (#15776, @Nael-Sayegh)
  * The community reviews action will be available in all tabs of the store. (#16179, @nvdaes)
* Ažuriranja komponenti:
  * Liblouis prevoditelj brajice je ažuriran na verziju [3.29.0](https://github.com/liblouis/liblouis/releases/tag/v3.29.0). (#16259, @codeofdusk)
  ...
  * eSpeak govorna sinteza je ažurirana na verziju 1.52-dev commit `cb62d93fd7`. (#15913)
...
* Dodana je nedodijeljena gesta za kretanje po karticama u načinu pretraživanja. (#15046)
...
* Popravljen je potencijalni uzrok rušenja NVDA pri pokretanju. (#15517)
...
* Dodana je trgovina dodataka. (#13985)
  * Pregledajte, pretražujte, instalirajte i ažurirajte dodatke zajednice.
...
* New braille features:
  ...
  * Baum braille driver: added several braille chord gestures for performing common keyboard commands such as `windows+d` and `alt+tab`.
  Pogledajte korisnički priručnik za cijeli popis. (#14714)
...
* Braille changes:
  * When using a braille display via the Standard HID braille driver, the dpad can be used to emulate the arrow keys and enter.
  Također, `Razmak+tačka 1` i `Razmak+tačka 4` sada odgovaraju strelici gore i strelici dolje. (#14713)
  * Updates to dynamic web content (ARIA live regions) are now displayed in braille.
  Ovo se može onemogućiti u naprednim postavkama. (#7756)
...
* NVDA ima brži odziv pri pomicanju kursora u kontrolama za uređivanje. (#14708)
...
* Stvaranje prijenosne kopije više ne zahtijeva da slovo diska bude uneseno kao dio apsolutne putanje. (#14680)
* Ako je Windows konfigurisan da prikazuje i sekunde u sistemskom satu, prečica `NVDA+F12` za izgovaranje trenutnog vremena, poštovat će tu postavku. (#14742)
...
* Novi jezici: mongolski, njemački (Švicarska).
...
* Novi jezici: španski (Kolumbija), Punjabi.
...
* Novi jezici: amharski, korejski, nepalski, slovenački.
...
* Trake napretka sada se najavljuju u načinu pretraživanja.
...
* Kada se fokusira polje za uređivanje koje je samo za čitanje, NVDA će to izgovoriti. (#1436)
...
* Dodan je prijevod na tradicionalni kineski. Zahvaljujemo se Coscell Kao.
...
* NVDA se više ne smrzava duže od minute pri otvaranju vrlo velikih dokumenata u Wordu.
