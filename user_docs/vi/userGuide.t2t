...
+ Cửa sổ cài đặt NVDA xuất hiện với thỏa thuận về giấy phép sử dụng.
Bấm ``mũi tên xuống`` để đọc giấy phép sử dụng, nếu muốn.
+ Bấm ``tab`` để chuyển đến hộp kiểm "Tôi đồng ý", rồi bấm khoảng trắng để đánh dấu chọn nó.
+ Bấm ``tab`` để di chuyển qua các tùy chọn, rồi bấm ``enter`` với các tùy chọn mong muốn.
+
...
+ Từ tập tin cài đặt, hãy chắc rằng hộp kiểm đồng ý với giấy phép sử dụng đã được chọn.
+ Bấm ``Tab`` đến, và kích hoạt nút "Cài NVDA vào máy tính".
+ Tiếp theo là các tùy chọn để dùng NVDA trong khi đăng nhập Windows và tạo biểu tượng trên desktop.
Mặc định, chúng được chọn.
Nếu muốn, bấm ``tab`` và ``khoảng trắng`` để thay đổi tùy chọn, hoặc để như mặc định.
+ Bấm ``enter`` để tiếp tục.
+ Hộp thoại "User Account Control (UAC)" hiện ra, hỏi "Do you want to allow this app to make changes to your PC?" (Bạn có muốn ứng dụng này thực hiện các thay đổi trên máy không?).
+ Bấm ``alt+y`` (tương đương trả lời có) để xác nhận đồng ý câu hỏi của UAC.
+ Một thanh tiến độ sẽ xuất hiện khi cài đặt NVDA.
Trong quá trình này, âm thanh beep của NVDA  sẽ tăng dần.
Quá trình này thường nhanh và có thể không gây chú ý.
+ Một hộp thoại xuất hiện xác nhận rằng NVDA đã được cài đặt thành công.
Thông điệp hiện ra nói rằng "Bấm Đồng Ý để bắt đầu chạy bản cài đặt".
Bấm ``enter`` để chạy bản NVDA đã cài đặt.
+ Hộp thoại tên "Chào mừng bạn đến với NVDA" xuất hiện, và NVDA sẽ đọc lên thông tin chào mừng.
...
+ bấm ``tab`` để chuyển đến mục "Sử dụng ``phím Khóa Hoa`` làm phím bổ trợ NVDA".
...
+ Dùng ``tab`` và ``khoảng trắng`` để điều chỉnh các tùy chọn khác tại màn hình này.
Các tùy chọn này quyết định việc NVDA tự khởi động.
+ bấm ``enter`` để đóng hộp thoại và giờ đây NVDA đang chạy.
+
...
+ Mở hộp thoại quản lý các thao tác từ chế độ duyệt.
+ Tìm mục có tên "chuyển đến thẻ kế tiếp" trong phần chế độ duyệt.
+ Gán phím ``y`` cho thao tác.
+ Tìm mục "chuyển đến thẻ trước".
+ Gán ``shift+y`` cho thao tác.
+
...
+ Chuyển con trỏ duyệt đến phần nội dung toán học.
...
+ Chuyển con trỏ đến nội dung có phần ghi chú.
+ Mở các tùy chọn cho vùng chọn như hướng dẫn ở trên.
+ Chọn Edit Note (chỉnh sửa ghi chú).
+
...
%kc:setting
==== Mở hộp thoại chọn màn hình nổi ====
...
++ Microsoft Speech API phiên bản 4 (SAPI 4) ++
...
+ Hãy chắc chắn rằng bạn đã cài đặt [trình điều khiển mới nhất https://www.papenmeier-rehatechnik.de/en/service/downloadcenter/software/articles/software-braille-devices.html].
+ Mở Windows Device Manager.
+ Cuộn danh sách xuống đến "USB Controllers" hoặc "USB Devices".
+ Chọn  "Papenmeier Braillex USB Device".
+ Mở properties và chuyển đến thẻ "Advanced".
