...
* Hệ điều hành: tất cả các phiên bản 32bit và 64bit của Windows 7, Windows 8, Windows 8.1,  Windows 10, Windows 11 và các bản máy chủ từ bản Windows Server 2008 R2.
* Đối với Windows 7, NVDA yêu cầu bản Service Pack 1 hoặc cao hơn.
* Đối với Windows Server 2008 R2, NVDA yêu cầu bản Service Pack 1 hoặc cao hơn.
...
Hỗ trợ nhập liệu chữ nổi đầy đủ và viết tắt với bàn phím của màn hình nổi.
...
|Đọc vùng chọn |`NVDA+shift+mũi tên lên` |`NVDA+shift+s` |đọc những văn bản được chọn|
|Đọc văn bản trong bộ nhớ tạm |`NVDA+c` |`NVDA+c` |Đọc nội dung văn bản trong bộ nhớ tạm|
...
|Thông báo vị trí con trỏ |`NVDA+tab` |`NVDA+tab` |Thông báo điều khiển tại vị trí con trỏ.  Bấm hai lần để đánh vần thông tin|
...
Để mở trực tiếp hộp thoại cấu hình chung của NVDA, bấm `NVDA+control+g`.
...
NVDA có một cộng đồng người dùng sôi động.  [Diễn đàn trao đổi qua thư điện tử chính thức  bằng tiếng Anh](https://nvda.groups.io/g/nvda) và một trang với đầy đủ các [nhóm ngôn ngữ bản địa](https://github.com/nvaccess/nvda-community/wiki/Connect).
...
* Windows 8 trở lên: Không có khả năng tương tác trên màn hình cảm ứng.
* Windows 8 trở lên: Không có khả năng tương tác trong chế độ duyệt (browse mode) hay đọc ký tự khi gõ trong các ứng dụng của Windows Store.
* Windows 8 trở lên: Không hỗ trợ chế độ giảm âm.
...
Nếu bạn đang sử dụng các thiết bị cảm ứng chạy trên hệ điều hành từ Windows 8 trở lên thì bạn có thể điều khiển trực tiếp NVDA thông qua các thao tác cảm ứng.
...
Sử dụng MathPlayer 4 từ Design Science, NVDA có thể đọc và tương tác với những nội dung liên quan đến toán học.
Yêu cầu chương trình MathPlayer 4 phải được cài đặt trên máy tính.
MathPlayer được tải về miễn phí tại trang web: https://www.dessci.com/en/products/mathplayer/.
Sau khi cài đặt MathPlayer, khởi động lại NVDA.
...
Có thể tải tại https://www.dessci.com/en/products/mathtype/
* Adobe Reader:
...
Lúc này, bạn có thể dùng các lệnh của MathPlayer như các phím mũi tên để đọc biểu thức đó.
...
Vui lòng đọc [tài liệu hướng dẫn của MathPlayer phần các lệnh di chuyển](https://www.dessci.com/en/products/mathplayer/navigation_commands.htm) để biết thêm thông tin.
...
Để làm được điều này, trước tiên, phải bật tùy chọn thông báo tiêu đề dòng / cột trong phân loại Định dạng tài liệu trong [hộp thoại cấu hình NVDA](#NVDASettings).
Tiếp theo, NVDA cần biết dòng nào cột nào chứa tiêu đề trong bảng bất kỳ.
...
|Thông báo chú thích cho người phiên dịch |control+shift+a |Thông báo chú thích cho người phiên dịch.|
...
Một số thiết lập có thể thay đổi thông qua các phím tắt sẽ được giới thiệu chi tiết dưới đây.
...
#### Thiết lập chung (NVDA+control+g) {#GeneralSettings}
...
##### Sử dụng cấu hình hiện tại trong khi đăng nhập và các màn hình bảo vệ (yêu cầu quyền quản trị) {#GeneralSettingsCopySettings}
...
#### Thiết lập bộ đọc (NVDA+control+v) {#SpeechSettings}
...
#### Chọn bộ đọc (NVDA+control+s) {#SelectSynthesizer}
...
Tùy chọn này cho phép bạn chọn thiết bị audio cho bộ đọc mà bạn đã chọn để NVDA đọc.
...
* Không giảm (no ducking): NVDA không giảm âm lượng âm thanh từ các ứng dụng khác.
...
* Luôn luôn giảm: NVDA giảm âm lượng âm thanh của ứng dụng khác trong suốt thời gian NVDA chạy.
...
#### Chọn màn hình nổi (NVDA+control+a) {#SelectBrailleDisplay}
...
Nếu có  lỗi khi gọi các driver của màn hình, NVDA sẽ thông báo và tiếp tục dùng màn hình trước đó nếu có.
...
Để bật / tắt tính năng Che Màn Hình ở bất cứ đâu, hãy gán thao tác / phím tắt thông qua [hộp thoại Quản lý thao tác](#InputGestures).
...
#### Bàn phím (NVDA+control+k) {#KeyboardSettings}
...
Nếu không chọn phím nào làm phím bổ trợ thì NVDA không thể sử dụng được một số   lệnh nhất định.
Vậy nên, NVDA sẽ hiển thị thông báo lỗi nếu không chọn phím nào mà lại bấm nút Đồng ý hay Áp dụng.
Sau khi bỏ qua bảng báo lỗi, bạn phải chọn ít nhất một phím làm phím bổ trợ.
...
#### Thiết Lập Chuột(NVDA+control+m) {#MouseSettings}
...
##### Thông báo vai trò đối tượng tại vị trí chuột {#MouseSettingsRole}

Nếu được chọn, NVDA sẽ thông báo vai trò (loại) của đối tượng khi chuột đi qua nó.
...
Phân loại này  chỉ xuất hiện trên các máy tính có màn hình cảm ứng, chạy Windows 8 trở lên, cho phép bạn thiết lập cách NVDA tương tác với màn hình cảm ứng.
...
#### Trình Bày Đối Tượng (NVDA+control+o) {#ObjectPresentationSettings}
...
#### Chế Độ Duyệt (NVDA+control+b) {#BrowseModeSettings}
...
#### Định Dạng Tài Liệu (NVDA+control+d) {#DocumentFormattingSettings}
...
Tùy chọn này chỉ phát huy tác dụng khi  bật "Dùng WASAPI cho đầu ra âm thanh".

##### Âm lượng âm thanh NVDA {#SoundVolume}

Thanh trượt này cho phép bạn điều chỉnh âm lượng cho âm thanh và  tiếng beep của NVDA.
Thiết lập này chỉ có tác dụng khi bật "Dùng WASAPI cho đầu ra âm thanh" và tắt "Âm lượng âm thanh NVDA đi theo âm lượng giọng đọc".
...
Để xem hướng dẫn về nội dung này, vui lòng truy cập [https://docs.python.org/3.7/howto/regex.html].
...
Chọn một add-on từ thẻ "Add-on hiện hành" hoặc "Add-on có bản cập nhật" tab.
Rồi bấm vào các nút cập nhật, cài đặt, hành động thay thế để bắt đầu cài đặt.

Để cài một add-on ngoài Cửa Hàng Add-On, bấm nút "Cài đặt từ các nguồn bên ngoài" button.
Điều này sẽ cho phép bạn tìm kiếm một gói add-on (tập tin `.nvda-addon`) ở đâu đó trên máy tính hoặc từ hệ thống mạng.
...
Trình hiển thị log, được mở từ trình đơn Công cụ của NVDA, cho phép bạn xem các thông tin đầu vào, đầu ra ở lần chạy cuối cùng của NVDA từ lúc nó được khởi động đến hiện tại.
Bấm NVDA+F1 sẽ mở trình hiển thị log và hiển thị thông tin của nhà phát triển về đối tượng điều hướng hiện tại.

Ngoài việc xem log, bạn có thể sao chép hay cập nhật log để xem các log mới nhất.
Các mục này có trong trình đơn trình bày của trình hiển thị log.
...
Tuy nhiên, nếu bạn kích hoạt hay đưa focus đến trình hiển thị thì nó sẽ tạm dừng cập nhật nội dung, do vậy bạn có thể dễ dàng sao chép các nội dung đã có để kiểm tra.
...
Trên Windows 7, 8 và 8.1, NVDA mặc định dùng bộ đọc  eSpeak NG ([bộ đọc Windows OneCore](#OneCore) mặc định được dùng cho Windows 10) trở lên.
...
### Microsoft Speech API phiên bản 4 (SAPI 4) {#toc291}
...
Vui lòng xem bài viết sau của Microsoft để biết danh sách các giọng đọc hiện có: https://support.microsoft.com/en-us/windows/appendix-a-supported-languages-and-voices-4486e345-7730-53da-fcfe-55cc64300f01
...
Bạn có thể tải chúng về từ trang https://support.freedomscientific.com/Downloads/Focus/FocusBlueBrailleDisplayDriver.
...
#### Các phím được gán cho tất cả sản phẩm {#toc305}
...
#### Các phím gán cho màn hình Brailliant BI 32, BI 40 và B 80 {#toc306}
...
#### Phím gán cho Brailliant BI 14 {#toc307}
...
Bạn cần phải cài trình điều khiển nếu kết nối qua cổng USB.
Bạn có thể tải về tại đây: http://www.himsintl.com/upload/HIMS_USB_Driver_v25.zip
...
Bạn có thể tìm thêm thông tin về các màn hình này tại https://en.seika-braille.com/down/index.html.
...
|Chuyển giữa các chế độ nói: tắt, beep và nói |`attribute2+attribute4`|
...
|-l LOGLEVEL |--log-level=LOGLEVEL |Cấp độ thấp nhất của thông điệp log (dò lỗi 10, đầu vào / đầu ra 12, cảnh báo gỡ lỗi 15, thông tin 20, vô hiệu 100)|
...
Nếu bạn cần thêm thông tin hoặc trợ giúp liên quan đến NVDA, vui lòng xem trang web của NVDA tại NVDA_URL.
