...
- 강화된 사운드 관리:
  - 새로운 오디오 설정 패널:
    ...
    - 사용 중인 음성의 볼륨 설정에 따라 NVDA 소리와 비프음의 음량을 설정할 수 있는 오디오 설정 옵션 추가 (#1409)
    - NVDA 사운드의 볼륨을 별도로 구성하는 오디오 설정 옵션 추가 (#1409, #15038)
    - 오디오 출력 장치를 변경하고 오디오 더킹을 전환하기 위한 설정이 '음성 엔진 선택' 대화상자에서 새 오디오 설정 패널로 이동되었습니다.
...
== 버그 수정 ==
- Microsoft 오피스:
  - 마이크로소프트 워드에서 "문서 서식 알림"의 "헤딩"과 "주석 및 코멘트"을 비활성화했을 때 충돌이 나는 것을 수정했습니다. (#15019)
  - 워드 및 엑셀에서, 더 많은 상황에서 텍스트 정렬을 적절하게 읽습니다. (#15206, #15220)
  - 특정 셀에 글자 서식 단축키 사용 시 안내를 수정합니다. (#15527)
  -
- Microsoft Edge:
  - NVDA는 더이상 Microsoft Edge에서 컨텍스트 메뉴를 열었을 때 이전 브라우즈 모드 위치로 돌아가지 않습니다. (#15309)
...
더이상 업데이트가 지원되지 않고 호환되지 않는 문제가 있는 추가 기능을 불안정성을 감수하고 수동으로 활성화 할 수 있습니다.
...
== 새로운 기능 ==
- NVDA에 애드온 스토어가 추가되었습니다. (#13985)
  - 커뮤니티 추가기능을 둘러보고, 검색하고, 설치 및 업데이트할 수 있습니다.
  - 오래된 추가 기능의 호환되지 않는 문제를 수동으로 재정의합니다.
  - 추가기능 관리가 제거되고 애드온 스토어로 대체됩니다.
...
- 새로운 점자 기능:
  ...
  - Baum 점자 드라이버: Windows 키+ D, Alt + Tab 등과 같은 일반적인 키보드 명령을 수행하기 위한 몇 가지 점자 코드 제스처를 추가합니다. 전체 목록은 NVDA 사용자 가이드를 참조하십시오. (#14714)
  -
- 유니코드 기호 추가:
...
- 웹 브라우저:
  ...
  - 이전에는 불가능했던 크롬 내장 컨트롤에서의 브라우즈 모드를 사용할 수 있습니다. (#13493, #8553)
...
- 텍스트 검토 "위로 쓸기" 터치 제스처를 발생시키는 버그를 수정하여 이전 줄로 이동하는 대신 페이지를 이동합니다. (#15127)
-


== 개발자를 위한 변경사항 ==
- 개발자 변경사항은 영문 변경사항 문서를 참고해 주시기 바랍니다.
-
...
- 새로운 "문서 탐색" 설정에 "문단 탐색 스타일" 옵션이 추가되었습니다. 한 줄 개행(기본)과 여러줄 개행(블럭) 문단 탐색 스타일을 지원합니다.
...
- 몇몇 설정을 여러 프로파일과 함께 사용할 경우 더 이상 예기치 않게 다르게 설정되지 않습니다. (#14170)
  - 다음 설정이 해결되었습니다:
    ...
    - 점자 설정의 점자 연계설정
    -
  - 아주 드믈게 이 버전의 NVDA를 설치할 때 프로필에 사용된 이러한 설정이 예기치 않게 수정될 수 있습니다. 버전 업데이트 시 위 옵션을 확인하시기 바랍니다..
  -
- 이모지가 이제 더 많은 언어로 보고되어야 합니다. (#14433)
- 일부 요소에서는 더 이상 점자에 주석이 없습니다. (#13815)
-
...
특히, 업데이트된 Adobe Acrobat 또는 Reader에서 PDF 문서를 읽을 때, 더이상 충돌이 일어나지 않습니다.
...
== 새로운 기능 ==
- 명령프롬프트, PowerShell, Windows 11 22H2 버전(Sun Valley 2) 이상 리눅스용 Windows 하위시스템으로 사용되는 Windows Console host에서:
...
새로운 표 탐색 기능이 추가되으며, 유니코드 CLDR이 업데이트되었습니다.
...
- 이제 NVDA가 동작을 수행하지 못할 때 이에 대해 알립니다. (#13500)
  - 다음 상황이 포함됩니다:
    ...
    -
  -
- (한국어 번역 변경사항) 테이블 요소가 음역인 "테이블"에서 "표"로 번경되었습니다.
-
...
- 점자 버그 수정:
  ...
  -
- 이제 NVDA 설치관리자를 특수한 문자로 된 디렉토리에서 실행 가능합니다. (#13270)
...
- Windows 10과 11의 계산기 10.1908버전 이상에서, NVDA는 공학 모드의 명령 등, 더 많은 명령에 대해 결과를 알립니다. (#13383)
- Windows 11에서 작업표시줄이나 작업보기같은 유저 인터페이스 요소와 마우스 또는 터치 상호작용을 통해 탐색 및 상호작용이 다시 가능해 집니다. (#13506)
- 워드패드와 다른 ``richText`` 컨트롤에서 숨겨진 텍스트를 더이상 알리지 않습니다. (#13618)
...
== 개발자를 위한 변경사항 ==
개발자를 위한 내용에 대해서는 영문판 changes.html 문서를 참조하세요.

= 2022.1 =
이번 릴리즈는 마이크로소프트 Office의 UIA 지원을 포함합니다.
...
Seika 노트테이커, Panenmeier, HID 점자 디스플레이의 점자 디스플레이 드라이버가 향상되었습니다.
...
- 
- 이제 Seika 노트테이커를 USB나 Bluetooth로 입력시 자동으로 인식됩니다. (#13191, #13142)
...
- NVDA 기본 언어를 지원하는 설치된 OneCore 음성이 없을 경우 NVDA는 기본적으로 eSpeak으로 설정됩니다. (#10451)
- OneCore 음성이 발화에 실패하면 음성엔진을 eSpeak로 설정합니다. (#11544)
...
- UIA를 통해 Microsoft Word 문서를 탐색할 때, 문장으로 탐색(alt+downArrow / alt+upArrow)이 다시 동작합니다. (#9254)
...
NVDA 2021.2에서 스크린커튼을 활성화하려면 Windows 10 21H2 또는 그 이상이 필요합니다. 이는 Windows 10 Insiders와 Windows 11을 포함합니다.
...
- 이제 NVDA가 Microsoft Outlook(마이크로소프트 아웃룩)에서 약속에 할당된 범주를 보고합니다. (#11598)
...
- 마이크로소프트 SQL Server Management Studio와 Visual Studio 2017 이후 버전에서의 자동완성 기능(IntelliSense)을 지원합니다. (#7504)
...
== 변경된 기능 ==
- liblouis braille translator가 버전 3.16.1으로 업데이트되었습니다.:
 - 여러 충돌 문제 수정
...
= 2020.3 =
이 버전에서는 특히 마이크로소프트 오피스 어플리케이션에서의 안정성과 성능이 몇가지 점에서 크게 향상되었습니다. 터치스크린 사용을 지원하고 그래픽 내용을 알려주는 기능을 켜고 끌 수 있는 새로운 설정 항목이 생겼습니다.
...
- 마이크로소프트 엑셀에서 요소목록 대화상자의 수식을 선택된 지역의 형식에 맞춰 출력합니다. (#9144)
...
- 고급 설정에 새로 소개된 실험적인 선택항목 "UIA 이벤트와 속성 변화를 일부 적용"을 활성화하면, 마이크로소프트 Visual Studio 및 UIAutomation을 사용하는 다른 어플리케이션에 주요한 성능향상이 있을 수 있습니다. (#11077, #11209)
...
- 마이크로소프트 워드 365의 최근 릴리즈에서 문서를 편집하던 중에 Control + Backspace를 누르면 NVDA가 "삭제 뒤로가기"라고 말하는 버그가 수정되었습니다. (#10851)
...
- 모질라 파이어폭스와 구글 크롬에서 브라우즈 모드일 때, 웹 콘텐트의 CSS에 display: inline-flex가 적용되면 다른 줄에 문자가 잘못 나타나는 문제가 수정되었습니다. (#11075)
...
- 모두 읽기 시 건너뛰어 일기가 활성화된 상태에서 방향키로 목록 및 테이블을 벗어났을 때 더 이상 목록 또는 테이블 종료 메시지를 반복적으로 알리지 않습니다. (#10706)
...
- 윈도우즈 8 혹은 그 이후의 버전에서, NVDA 는 이제 마이크로소프트 스토어에서 다운로드 받은 앱들처럼 온라인 서버를 통해서 운용되는 제품의 이름과 버전 정보를 알려줍니다. 이 정보는 앱에서 제공하는 정보를 바탕으로 합니다. (#4259, #10108)
...
- 모질라 파이어폭스와 구글 크롬 브라우저에서 ARIA 1.1 콤보박스를 지원합니다. (#9616)
...
- 모질라 파이어폭스 브라우저에서, 포커스된 요소가 현재 위치(aria-current)로 바뀔 경우 이를 여러 번 읽는 버그가 수정됐습니다. (#8960)
...
- 모질라 파이어폭스의 브라우즈 모드에서 포커스의 움직임이 빨라졌으며, 브라우즈 모드에서 많은 경우 커서의 반응속도가 빨라졌습니다. (#10584)
...
- 파이어폭스 및 크롬 브라우저를 이용해서 Gmail에서 필터 생성이나 이메일 설정 등 특정 팝업 메뉴를 사용할 때 동작이 멈추는 현상을 해결했습니다. (#10175, #9402, #8924)
- 윈도우즈 7의 시작 메뉴에서 마우스를 이용할 때 탐색기가 동작을 멈추는 문제를 해결했습니다. (#9435)
- 윈도우즈 7의 탐색기에서 메타데이터 편집 창에 접근할 때 동작을 멈추는 현상을 해결했습니다. (#5337)
- 파이어포스나 크롬 브라우저에서 base64로 인코딩된 위치에 있는 이미지에 접근할 때 NVDA가 멈추는 문제를 해결했습니다. (#10227)
...
- NVDA의 표 탐색 명령을 다시 Google Docs 에서 쓸 수 있게 됐습니다. 파이어폭스와 크롬에서 동작합니다. (#9494)
...
- 모질라 파이어폭스에서 실시간 갱신 영역이 백그라운드 탭에 있는 경우에도 그 내용을 업데이트하는 문제가 해결됐습니다. (#1318)
...
== 변경된 기능 ==
- 리브루이 점자  변환기 version 3.8.0 사용. (#9013)
...
= 2018.4 =
이번 버전의 주 기능들: 최신 파이어폭스 버전에서의 탐색 속도 향상; 모든 음성 엔진에서의 이모지 출력 기능; Outlook 사용 시 답장/전달 여부 확인; Microsoft Word에서의 페이지 좌측 상단으로부터의 커서 위치 확인 및 수많은 버그 수정 등입니다.
...
- 파이어폭스, 크롬 및 Acrobat DC 사용 중 마우스를 탐색 객체로 옮길 시 현재 브라우즈 모드 위치로 마우스가 제대로 옮겨지도록 함. (#6460)
- 파이어폭스, 크롬 및 Internet Explorer에서의 콤보박스 조작 정확도가 향상됨. (#8664)
- 일본어판 윈도우 XP 또는 비스타 사용 시 운영 체제 요구사항 관련 메시지가 제대로 출력되도록 함. (#8771)
- 파이어폭스를 통한 대형 페이지(특히 자동 컨텐츠 변경이 많은 페이지) 탐색 속도가 향상됨. (#8678)
...
- NVDA 업데이트 후 파이어폭스나 크롬이 다운되거나 현재 탐색중인 페이지의 컨텐츠 변경 내용이 제대로 출력되지 않던 문제 수정. (#7641) 
- 브라우즈 모드 사용 시 클릭 가능한 요소를 만났을 때 클릭 가능 상태가 여러번 출력되던 문제 수정. (#7430)
- baum Vario 40 점자 디스플레이 사용 시 점자 입력 명령이 실행되지 않던 문제 수정. (#8894)
- 파이어폭스에서 Google 프레젠테이션 사용 시 포커스를 옮길 때마다 선택된 내용이 불필요하게 출력되던 문제 수정. (#8964)
...
= 2018.3.2 =
본 버전에서는 크롬을 사용하여 www.twitter.com 탐색시 발생하던 오류 문제를 수정했습니다. (#8777)


= 2018.3.1 =
본 버전에서는 파이어폭스 32비트 사용중 파이어폭스가 다운되던 문제를 수정하였습니다. (#8759)
...
== 새로 추가된 기능 ==
- 파이어폭스와 크롬 사용 시 페이지 컨텐츠 내 문법 오류(컨텐츠가 오류 출력 지원 시)가 인식되도록 함. (#8280)
- 크롬 사용 시 웹페이지 컨텐츠 삽입 또는 삭제 내용이 출력되도록 함. (#8558)
...
- 파이어폭스 63 사용 시 ARIA로 처리된 인용 문구(blockquote 역할) 탐색 지원 추가. (#8577)
...
- 리브루이 점자 변환기 3.6.0 버전 사용. (#8365)
- 러시아어 8점 점자표 경로 변경. (#8446)
- eSpeak NG 1.49.3dev commit 910f4c2 사용. (#8561)


== 버그 수정 ==
- 크롬 사용 시 컨텐츠로 인식되지 못한 컨트롤 레이블이 브라우즈 모드에서 인식되도록 함. (#4773)
...
= 2018.2 =
이번 버전의 주 기능들: PC Kindle 앱에서의 표 탐색 지원; 휴먼웨어 브레일노트 터치 및 Brailliant BI14 지원 추가; OneCore 및 Sapi5  음성 엔진 지원 향상; 마이크로소프트 Outlook 지원 향상 등입니다.
...
- 크롬과 파이어폭스에서 브라우즈 모드 사용 중 탭 또는 한 글자 명령을 사용하여 체크박스나 라디오 버튼으로 이동 시 컨트롤 레이블이 두번 출력되던 문제 수정. (#7960)
...
- 마이크로소프트 워드 사용 시 더 많은 bullet 글자를 NVDA가 인식하도록 함. (#6778)
...
- 최신 마이크로소프트 SAPI5 음성 엔진의 반응 속도(특히 음성 출력 끝부분)가 향상됨(이로 인해 본 음성 엔진을 사용하여 텍스트 읽을 시 NVDA의 반응 속도가 향상됨). (#8174)
...
= 2018.1 =
이번 버전의 주 기능들: 마이크로소프트 워드 및 파워포인트내 차트 접근 지원; Eurobraille과 구형 Optelec/프로토컬 컨버터 디스플레이 지원 추가; Optelec 및 힘스 디스플레이 지원 향상; 파이어폭스 58 이상에서의 반응 속도 향상 등입니다.
...
- 힘스 스마트 비틀 스크롤 키 사용 시 명령 인식 정확도가 향상됨. (#6086)
- 모질라 파이어폭스 58 이상 사용 시 대용량 콘텐츠를 탐색 시 NVDA의 반응 속도가 개선됨. (#7719)
- 마이크로소프트 Outlook 사용 시 표가 있는 이메일 읽을 시 NVDA가 다운되던 문제 수정. (#6827)
- 점자 입력 사용 시 점자 키보드에 할당된 시스템 기능키와 다른 기능키를 조합한 명령을 사용할 수 있음(모델별로 다름). (#7783)
- 모질라 파이어폭스 사용 시 LastPass와 bitwarden과 같은 추가 기능 팝업에서 브라우즈 모드가 작동하도록 함. (#7809)
- 파이어폭스나 크롬 사용 시 프로그램이 다운되었을 때 나타나던 포커스 변경 시 NVDA 반응 속도 저하 문제 수정. (#7818)
...
- 파이어폭스, 크롬 및 인터넷 익스플로러 사용 시 편집창 및 폼 필드 첫 글자 명령을 사용하여 rich edit 편집창(예: contentEditable)으로 이동할 수 있음. (#5534)
...
- 파이어폭스 및 크롬에서 웹 대화상자가 열렸을 때 웹 앱에 위치하지 않는한 브라우즈 모드가 작동하도록 함. (#4493)
- 웹 브라우저 사용 시 효율성을 높이기 위해 탭 또는 첫 글자 명령을 사용하여 사이트 탐색 시 컨테이너 요소(목록, 표 등)의 끝 메시지가 출력되지 않도록 함. (#2591)
- 파이어폭스 및 크롬에서 브라우즈 모드 사용 시 탭 또는 첫 글자 명령을 사용하여 폼 필드로 옮길시 필드 그룹 이름이 출력되도록 함. (#3321)
...
== 버그 수정 ==
- 마이크로소프트 워드를 포함한 여러 프로그램 사용 시 링크가 점자로 표기되도록 함. (#6780)
- 파이어폭스 및 크롬 사용 시 수많은 탭이 열렸을 때 NVDA의 반응 속도가 느려지던 문제 수정. (#3138)
...
- 파이어폭스에서 브라우즈 모드 사용 시 컨트롤 레이블 컨텐츠가 없을때 접근성 레이블 내용이 출력되도록 함. (#4773)
- Windows 10 Creators Update 사용 시 파이어폭스 사용 도중 NVDA를 재시작할 때 브라우즈 모드가 작동하지 않던 문제 수정. (#7269)
- 파이어폭스가 포커스된 후 NVDA를 재시작한 후 브라우즈 모드가 작동하지 않던 문제 수정(alt+tab을 눌러 포커스를 파이어폭스로 옮겨야 함). (#5758)
- 파이어폭스를 설치하지 않고도 크롬에서 수학 컨텐츠를 읽을 수 있음. (#7308)
...
- 마이크로소프트 Outlook 메시지 목록 탐색 시 메시지 플래그가 출력되도록 함. (#6374)
- 마이크로소프트 파워포인트에서 슬라이드 편집 시 도형 모양(예: 삼각형, 동그라미, 비디오, 화살표 등)이 출력되도록 함. (#7111)
- 구글 크롬 사용 시 수학 컨텐츠(특히 MathML로 표기된 내용) 탐색 가능. (#7184)
...
- 마이크로소프트 워드에서 브라우즈 모드 사용 시 w/shift+w를 사용하여 철자 오류 탐색 가능. (#6942)
- 마이크로소프트 Outlook 내 일정 대화상자 내 날짜 선택 컨트롤 지원 추가. (#7217)
...
- 마이크로소프트 Edge 사용 시 파일 다운로드 시작과 같은 알림 내용이 자동으로 출력되도록 함. (#7281)
...
- 파이어폭스와 크롬 사용 시 컨텐츠 일부만 보여지는 spreadsheet(특히 ARIA 1.1에 추가된 aria-rowcount, aria-colcount, aria-rowindex 및 aria-colindex 속성이 적용된 complex dynamic grid) 탐색 지원 추가.  (#7410)
...
== 버그 수정 ==
- 크롬에서 브라우즈 모드 사용 시 편집 가능 div 요소명이 요소의 값으로 출력되지 않도록 함. (#7153)
- 마이크로소프트 워드에서 빈 문서가 열린 상태에서 브라우즈 모드 사용 시 end 키를 누를 때 RuntimeError가 나지 않도록 함. (#7009)
- 마이크로소프트 Edge 사용 시 특정 요소가 ARIA 문서 롤로 표기되었을 때 브라우즈 모드가 제대로 작동하지 않던 문제 수정. (#6998)
...
- 마이크로소프트 Outlook 사용 시 새 미팅 또는 일정 대화상자 내 필요 없는 정보가 출력되던 문제 수정. (#7216)
- 값이 없는 징행률 막대(예: 업데이트 확인 대화상자)의 값을 출력 시 만약 진행률 출력이 비프음이 아닌 설정이 적용된 경우 비프음이 출력되지 않도록 함. (#6759)
- 마이크로소프트 엑셀 2003 및 2007 사용 시 방향키로 worksheet 셀을 탐색할 수 없던 문제 수정. (#8243)
...
- 편집창에서 캐럿을 옮길 때(예: 방향키 사용 또는 백스페이스를 누를 때) 거의 대부분의 편집창에서(예: 크롬 및 터미널 창) NVDA의 음성 반응이 빨라지도록 함. (#6424)
- 마이크로소프트 Outlook 2016 내 signature 대화상자 내용을 탐색할 수 있도록 함. (#7253)
...
- 마이크로소프트 Edge 사용 시 첫 글자를 이용하여 헤딩 탐색 시 명령 반응 속도가 개선됨.  (#7343)
- 마이크로소프트 Edge 브라우즈 모드 사용 시 특정 문서 내용(예: Wordpress 2015 테마)을 탐색할 수 없던 문제 수정. (#7143)
- 마이크로소프트 Edge 사용 시 영어 외 타국어 사용 시 랜드마크 이름이 제대로 출력되지 않던 문제 수정. (#7328)
- 점자 디스플레이의 셀 길이보다 긴 텍스트를 선택할 때 선택 내용이 점자로 제대로 표시되도록 함(예: shift+아래 화살표를 눌러 여러줄 선택 시 마지막 줄의 내용이 점자로 표시되도록 함). (#5770)
- 파이어폭스 사용 시 twitter.com에서 트위트를 볼때 "섹션"이 반복되어 출력되던 문제 수정. (#5741)
- 레이아웃 표에서 문서 서식 대화상자 내 레이아웃 표 알림 옵션이 적용되지 않는한 표 셀 탐색 명령을 사용할 수 없도록 변경됨. (#7382)
- 파이어폭스와 크롬에서 브라우즈 모드를 사용하여 표를 탐색 시 셀 탐색 명령이 표에 표시되지 않은 셀을 건너뛰도록 함. (#6652, #5655)


== 한국어 번역 관련 ==
- 마이크로소프트 오피스 관련 문자열 번역이 업데이트됨.
...
== 새로 추가된 기능 ==
- 마이크로소프트 엑셀 사용 시 NVDA+f 명령을 사용하여 셀 테두리 정보를 확인할 수 있음. (#3044)
- 웹 브라우저 사용 시 aria-current 요소 인식 기능 추가. (#6358)
- 마이크로소프트 Edge 사용 시 자동 언어 변경 기능 지원 추가. (#6852)
...
- - 모질라 파이어폭스와 같은 Gecko 기반 프로그램에서 멀티프로세스 기능 사용 시 NVDA가 다운되던 문제 수정. (#6885)
...
- 윈도우 10 Creators Update/마이크로소프트 Edge 사용 시 줄 단위 이동 속도가 최대 3배 빨라짐. (#6994)
- 윈도우 10 Creators Update/마이크로소프트 Edge 문서에 포커스될 때 "Web Runtime grouping"이 출력되던 문제 수정. (#6948)
...
- 마이크로소프트 Edge내 브라우즈 모드 사용 시 t/shift+t를 사용하여 표를 탐색 시 접근 가능한 표(ARIA grids)가 인식되지 않던 문제 수정. (#6977)
...
= 2017.1 =
이번 버전의 주 기능들: 마이크로소프트 워드 내 섹션 및 텍스트 열 수 알림; Kindle PC 앱을 통한 책 독서; Edge 브라우저 지원 향상 등입니다.

== 새로 추가된 기능 ==
- 마이크로소프트 워드 사용 시 문서 서식 대화상자 내 쪽 수 알림 설정이 선택된 경우 섹션 끝 종류 및 순위가 출력되도록 함. (#5946)
- 마이크로소프트 워드 사용 시 문서 서식 대화상자내 쪽 수 알림 설정이 선택된 경우 텍스트 열이 출력되도록 함. (#5946)
- WordPad 사용 시 자동 언어 변경 기능 지원 추가. (#6555)
- 마이크로소프트 Edge 브라우즈 모드 사용 시 NVDA의 문자열 찾기 명령(NVDA+control+f) 사용 가능. (#6580)
- 마이크로소프트 Edge 브라우즈 모드 사용 시 버튼 탐색 명령(b/shift+b) 사용 가능. (#6577)
- 마이크로소프트 엑셀에서 sheet를 복사 시 행 및 열 머릿말 정보도 복사되도록 함. (#6628)
- Kindle 앱 1.19 버전 지원(책 독서중 링크, 각주, 그래픽, 텍스트 하이라이트 및 사용자 노트 탐색 가능). 자세한 정보는 NVDA 사용설명서 내 "Kindle PC 앱" 섹션을 참고바람. (#6247, #6638)
- 마이크로소프트 Edge 브라우즈 모드에 표 탐색 명령 추가. (#6594)
- 마이크로소프트 엑셀에서 리뷰 커서 위치 명령(NVDA+numpadDelete (desktop)/NVDA+delete (laptop))을 실행 시 worksheet와 현재 셀 위치가 출력되도록 함. (#6613)
...
== 버그 수정 ==
- 마이크로소프트 워드 내 브라우즈 모드를 사용하여 긴 문서를 문단 단위로 이동 시 NVDA가 다운되던 문제 수정. (#6368)
- 마이크로소프트 엑셀에서 워드로 표를 복사 시 복사된 표가 구조표(layout table)로 인식되던 문제 수정. (#5927)
- 마이크로소프트 엑셀에서 보호 모드가 설정된 경우 글자 입력 시 경고음이 출력되도록 함(이전에는 실제로 입력되지 않은 글자가 출력되었음). (#6570)
- 마이크로소프트 엑셀 사용 중 escape를 눌렀을 때 이전에 브라우즈 모드(NVDA+space)를 사용하다가 폼 필드에서 엔터를 눌러 포커스 모드로 전환한 이상 브라우즈 모드가 활성화되지 않도록 함. (#6569) 
- 마이크로소프트 엑셀 사용 시 행 또는 열 전체가 병합될 때 NVDA가 다운되던 문제 수정. (#6216)
- 마이크로소프트 엑셀 셀 텍스트의 잘려진/삐져나온 정보 출력이 향상됨. (#6472)
- 읽기 전용 체크박스가 인식되도록 함. (#6563)
- 오디오 장치가 없는 컴퓨터에 NVDA 설치 파일을 실행 시 오디오 출력 불가로 인한 로고 재생 관련 경고 대화상자가 나타나지 않도록 함. (#6289)
- 마이크로소프트 엑셀 리본 탐색 시 사용할 수 없는 컨트롤의 사용 여부가 제대로 출력되도록 함. (#6430)
- 모든 창을 최소화할 때 "창"이 출력되던 문제 수정. (#6671)
- 윈도우 10 Creators Update에서 마이크로소프트 Edge와 같은 유니버셜(Universal Windows Platform) 앱 사용 시 입력된 글자가 출력되도록 함. (#6017)
...
- 마이크로소프트 워드 사용 시 Cross Reference 필드가 링크로 인식되도록 함(다른 링크처럼 동작할 수 있음). (#6102)
...
== 변경된 기능 ==
- 리브루이 점자 변환기 3.0.0 버전 사용. 특히 통합 영어 점자(UEB) 출력이 향상됨. (#6109, #4194, #6220, #6140)
...
- 문서 서식 정보가 음성 및 점자로 출력되는 까닭에 문서 서식 설정 대화상자내 "Announce formatting changes after the cursor"을 "Report formatting changes after the cursor"로 고침(한국어는 이전 레이블 그대로 사용). (#6336)
...
- 웹브라우저 사용 시 IFrame(문서내 문서) 컨트롤 정보 출력이 통일됨(특히 파이어폭스에서 IFrame을 "프레임"으로 출력하도록 함). (#6047)
...
- 마이크로소프트 워드 사용 시 머리말 내 쪽 수 정보 탐색 가능. (#6004)
- 기호 발음 설정 대화상자에서 마우스를 사용하여 기호 목록과 편집 필드를 오갈 수 있음. (#6312)
- 마이크로소프트 워드에서 브라우즈 모드 사용 중 특정 문서에 링크가 잘못 삽입되었을때 요소 목록 대화상자가 나타나지 않던 문제 수정. (#5886)
...
- 마이크로소프트 워드에서  표 셀 이동 명령 사용 시  셀에 위치한 숫자 bullet 목록이 제대로 출력되도록 함. (#6446)
- 단춧키 설정을 통해 Handy Tech 점자 디스플레이에 할당된 명령을 변경할 수 있음. (#6461)
- 마이크로소프트 엑셀 사용 시 엔터(일반 엔터 또는 numpad 엔터)를 눌렀을때 spreadsheet의 다음 행으로 이동했다는 메시지가 출력되도록 함. (#6500)
- iTunes 사용 시 iTunes Store, Apple Music 등에서 브라우즈 모드 사용 시 프로그램이 다운되던 문제 수정. (#6502)
- 64비트 모질라 및 크롬 기반 앱 사용 도중 특정 상황에서 사용하던 앱이 다운되던 문제 수정. (#6497)
- 파이어폭스 multi-process 사용 시 브라우즈 모드 및 편집창 탐색 명령 사용 가능. (#6380)
...
= 2016.3 =
이번 버전의 주 기능들: 특정 추가 기능 일시 정지 기능 추가; 엑셀 내 폼 필드 지원; 색상 출력 향상; 여러 점자 디스플레이 관련 버그 수정 및 지원 향상; 마이크로소프트 워드 지원 향상 및 여러 버그 수정 등입니다.

== 새로 추가된 기능 ==
- 윈도우 10 Anniversary Update 사용 시 마이크로소프트 Edge 브라우저내에서 브라우즈 모드를 사용하여 PDF 문서를 읽을 수 있음. (#5740)
- 마이크로소프트 워드 사용 시 취소선/이중 취소선이 적절하게 출력되도록 함. (#5800)
- 마이크로소프트 워드 내 표에 이름이 있는 경우 표 이름이 출력되고 표 설명이 있을 경우 브라우즈 모드의 긴 설명 핫키(NVDA+d)를 사용하여 확인할 수 있음. (#5943)
- 마이크로소프트 워드 사용 시 문단 배치 변경(alt+shift+아래쪽 화살표/alt+shift+위쪽 화살표)시 새 위치 정보가 출력되도록 함. (#5945)
- 마이크로소프트 워드 사용 시 줄 간격이 인식되도록 함(서식 정보 명령을 통해 확인 가능, 워드 명령을 통해 간격 변경 시 설정이 출력되도록 함, 문서 서식 설정에서 줄 간격 출력이 활성화된 후 각 줄로 이동 시 간격 변경이 출력되도록 함). (#2961)
- 인터넷 익스플로러 사용 시 HTML5 구조 요소가 인식되도록 함. (#5591)
- 문서 서식 대화상자에 추가된 주석 알림 체크박스를 통해 주석(예: 마이크로소프트 ㅜ어드내 주석) 출력을 설정할 수 있음. (#5108)
...
- 마이크로소프트 엑셀 폼 필드 탐색 가능(요소 목록 또는 브라우즈 모드를 사용하여 폼 필드로 이동할 수 있음). (#4953)
...
== 버그 수정 ==
- 마이크로소프트 워드에서 브라우즈 모드 사용 중 클립보드에 복사시 텍스트 서식이 적용되도록 함. (#5956)
- 마이크로소프트 워드 사용 시 워드 자체의 표 탐색 명령(alt+home, alt+end, alt+pageUp, alt+pageDown) 및 설정 명령(shift 추가)을 NVDA가 제대로 인식하도록 함. (#5961)
- 마이크로소프트 워드 대화상자에서의 객체 탐색 향상. (#6036)
...
- 마이크로소프트 ㅜ어드 사용 시 오피스 테마가 색상 출력에 반영되도록 함(이를 통해 색상 출력 정확도가 향상됨). (#5997)
- 2016년 4월 이후 출시된 윈도우 10 빌드에서의 Microsoft Edge 브라우즈 모드 및 시작 메뉴 검색 결과 기능이 동작하도록 함. (#5955)
- 마이크로소프트 워드 사용 시 자동으로 표 머릿말 읽기가 설정되어 있고 표에 병합된 cell이 있는 경우 표 머릿말이 제대로 출력되도록 함. (#5926)
...
- 마이크로소프트 엑셀에서 배경 패턴 레이블 출력 시 NVDA가 엑셀 자체에서 사용하는 레이블을 사용하도록 함. (#6092)
...
- Google 크롬 및 크롬 기반 브라우저 사용 시 인터페이스 언어가 영어 외 다른 언어로 설정되어 있을 때 브라우즈 모드가 작동하지 않던 문제 수정. (#6249)
...
= 2016.2.1 =
이번 버전에서는 마이크로소프트 워드 사용 시 발생하던 여러 문제를 수정하였습니다:

- 윈도우 XP에서 마이크로소프트 워드 실행 시 다운되던 문제 수정. (#6033)
- 마이크로소프트 ㅜ어드 사용 시 발생하던 여러 문제의 원인이었던 맞춤법 오류 알림 기능 제거. (#5954, #5877)


= 2016.2 =
이번 버전의 주 기능들: 입력 시 철자 오류 알림; 마이크로소프트 워드 내 문법 오류 인식; 마이크로소프트 오피스 지원 향상 및 버그 수정 등입니다.

== 새로 추가된 기능 ==
- 인터넷 익스플로러와 같은 MSHTML 컨트롤에서 브라우즈 모드 사용 시 다음 및 이전 주석(a/shift+a) 명령을 사용하여 삽입 또는 삭제된 텍스트로 이동할 수 있음. (#5691)
- 마이크로소프트 엑셀 사용 시 셀 그룹 레벨 및 그룹 확장 또는 축소 여부가 출력되도록 함. (#5690)
- 텍스트 서식 알림 핫키를 두번 누를 시 서식 정보가 브라우즈 모드로 출력되도록 함. (#4908)
- 마이크로소프트 엑셀 2010 이상 사용 시 셀 음영 및 채우기 그라데이션이 출력되도록 함(본 기능은 문서 서식 대화상자내 색상 알림 설정을 통해 조절할 수 있음). (#3683)
...
- 마이크로소프트 워드 사용 시 맞춤법 오류가 인식되도록 함(본 기능은 문서 서식 대화상자에 추가된 맞춤법 알림 설정을 통해 조절할 수 있음). (#5877)


== 변경된 기능 ==
- 브라우즈 모드와 편집창에서 키패드 enter 키가 제 기능을 하지 못하던 문제 수정. (#5385)
- NVDA 기본 음성 엔진을 eSpeak NG로 교체함. (#5651)
- 마이크로소프트 엑셀 사용 시 열 머릿말과 셀 사이에 빈 열이 있을 경우 특정 행 머릿말이 출력되지 않던 문제 수정. (#5396)
- 마이크로소프트 엑셀 사용 시 셀의 위치가 셀 머릿말에 앞서 출력되도록 하여 머릿말과 콘텐츠간 혼돈을 최소화하도록 함. (#5396)


== 버그 수정 ==
- 브라우즈 모드 사용 시 첫 글자 핫키로 문서가 지원하지 않는 요소로 옮기고자 할 때 "다음/이전 요소 없음" 대신 "요소 지원 안함"이라고 출력되도록 함. (#5691)
- 마이크로소프트 엑셀에서 요소 목록 호출 시 차트만 있는 시트도 시트 목록에 포함되도록 함. (#5698)
...
- 마이크로소프트 워드에서 문단 들여쓰기는 항상 사용자가 지정한 길이의 단위 (센티미터와 인치 등)로 알려지도록 함. (#5804)
- 음성으로 만 출력되던 많은 NVDA 메시지가 점자 디스플레이로 출력되도록 함. (#5557)
- 접근 가능한 Java 응용 프로그램에서 트리 뷰의 항목 수준이 알려지도록 함. (#5766)
- 모질라 파이어폭스 사용 도중 가끔 Adobe Flash가 다운되던 문제 수정. (#5367)
- 구글 크롬 및 크롬 기반 브라우저에서 대화상자나 응용 프로그램에 포함된 문서를 브라우즈 모드에서 읽을 수 있음. (#5818)
- 구글 크롬 및 크롬 기반 브라우저에서 웹의 대화상자나 응용 프로그램에서 NVDA를 강제로 브라우즈 모드로 전환할 수 있음. (#5818)
...
- 마이크로소프트 워드에서 표 행/렬의 머리말 알림이 활성화된 경우 큰 표에서 NVDA가 정지되던 문제 수정. (#5878)
- 마이크로소프트 워드 사용 시 개요 수준 텍스트(개요 텍스트가 헤딩이 아닐 경우)를 헤딩으로 인식하던 문제 수정. (#5186)
- 마이크로소프트 워드 내 브라우즈 모드 사용 시 컨테이너 요소 시작 및 끝 명령(comma/shift+comma)을 사용하여 표 시작 및 끝으로 이동할 수 있음. (#5883)


== 한국어 번역 관련 ==
- 문서 서식 대화상자내 "맞춤법 오류"를 "철자 오류"로 변경. (#5877)
- 철자 오류 서식을 지원하는 컨트롤에서 "맞춤법 오류/철자 오류 끝" 메시지가 "철자 오류"로 통합됨. (#5877)
...
= 2016.1 =
이번 버전의 주 기능들: 배경음 볼륨 조절; 점자 디스플레이 지원 향상; 마이크로소프트 오피스 지원 향상; iTunes 브라우즈 모드 관련 여러 문제 수정 등입니다.
...
- 마이크로소프트 엑셀내 요소 목록 대화상자내 공식(formula) 라디오 버튼 핫키를 Alt+R로 변경하여 검색 핫키와 혼돈되지 않도록 함. (#5527)
- 리브루이 점자 변환기 2.6.5 버전 사용. (#5574)
...
- 마이크로소프트 아웃룩에서 이메일 작성시 "삽입됨"과 같은 불필요한 메시지가 출력되던 문제 수정. (#5486)
...
- 마이크로소프트 워드 사용 시 track 마커가 존재하지 않는 경우 "삽입됨"과 같은 track 정보가 출력되지 않도록 함. (#5566)
...
- 마이크로소프트 오피스 2013 이상 사용 시 ribbon 탭만 표시된 경우 탭 선택 시 ribbon 아이템이 출력되지 않던 문제 수정. (#5504)
- 터치스크린 명령 실행 및 할당 관련 여러 문제 수정 및 기능 향상. (#5652)
- 입력 도움말 사용 시 터치스크린 hover 동작이 출력되지 않도록 함. (#5652)
- 마이크로소프트 엑셀내 merged 셀내 주석이 있는 경우 주석 목록이 요소 목록에 뜨지 않던 문제 수정. (#5704)
- 표 행과 열 머릿말 알림 설정이 켜져 있을때 특정 상황에서 마이크로소프트 엑셀내 sheet 내용이 출력되지 않던 문제 수정. (#5705)
- 구글 크롬 사용 시 동아시아 입력 구성창 사용 시 오류가 발생하던 문제 수정. (#4080)
- iTunes내 Apple Music 검색시 검색 결과 문서내 브라우즈 모드가 업데이트되지 않던 문제 수정. (#5659)
- 마이크로소프트 엑셀에서 shift+f11을 눌러 새 sheet 생성시 새 위치가 출력되도록 함(이전에는 아무런 내용도 출력되지 않았음). (#5689)
...
= 2015.4 =
이번 버전의 주 기능들: 윈도우 10 지원 향상; 윈도우 8 이상 접근성 센터내 NVDA 추가; 엑셀 지원 향상(sheet 목록 및 이름 변경과 잠겨진 셀로 이동 가능); 모질라 파이어폭스, 구글 크롬 및 모질라 썬더버드에서의 rich text 편집 지원 향상 등입니다.

== 새로 추가된 기능 ==
- 윈도우 8이상 사용 시 NVDA가 접근성 센터에 표시되도록 함. (#308)
- 마이크로소프트 엑셀에서 셀을 옮길때 문서 서식 설정에서 활성화된 서식 정보가 출력되도록 함. (#4878)
...
- 마이크로소프트 워드내 요소 목록에 track 변경 나열시 더 많은 정보(예: 서식 변경)가 포함되도록 함. (#4920)
- 마이크로소프트 엑셀내 요소 목록(NVDA+f7)를 통해 sheet 목록을 보거나 이름 변경을 할 수 있음. (#4630, #4414)
- 기호 발음 설정을 통해 음성 엔진이 기호를 처리할지(예: 띄어 읽기 또는 억양 조절 등)를 조절할 수 있음. (#5234)
- 마이크로소프트 엑셀 사용 시 sheet 저자가 셀에 입력 메시지를 첨부한 경우 메시지 내용이 출력되도록 함. (#5051)
- Baum Pronto! V4 및 VarioUltra 지원 추가(블루투스로 연결 가능). (#3717)
- 모질라 기반 프로그램에서의 rich text 편집 지원 향상(예: 점자 디스플레이 연결 후 파이어폭스에서 Google Docs 문서 편집 또는 썬더버드내 HTML 이메일 작성중). (#1668)
- 구글 크롬 또는 크롬 기반 브라우저에서의 rich text 편집 지원 향상(예: 점자 디스플레이 연결 후 Google Docs 문서 편집). (#2634)
 - 크롬 47 이상 사용 바람.
- 마이크로소프트 엑셀에서 브라우즈 모드 사용 시 방향키를 사용하여 보호된 sheet안에 있는 잠겨진 셀로 이동 가능. (#4952)


== 변경된 기능 ==
- 문서 서식 대화상자내 문서 변경 추적 내용 설정이 기본적으로 선택되도록 변경됨. (#4920)
- 문서 변경 추적 내용 알림이 선택된 상태에서 마이크로소프트 워드에서 글자 단위로 옮길시 효율성을 높이기 위해 track 변경 내용이 덜 출력되도록 함(자세한 track 변경 정보는 요소 목록에서 확인할 수 있음). (#4920)
- 리브루이 점자 변환기 2.6.4 버전 사용. (#5341)
...
- 마이크로소프트 엑셀 차트를 읽다가 특정 키(예: control+pageUp/control+pageDown을 사용하여 sheet 변경)를 누를시 키가 제대로 동작하지 않던 문제 수정. (#5336)
...
- 윈도우 8 이상 사용 시 로그인후 NVDA가 실행되도록 설정했을때 NVDA가 일찍 실행되도록 함. (#308)
 - 만약 이전 버전에서 본 설정을 설정했다면 일반 설정에서 본 설정을 해제 후 다시 설정하기 바람. 설정 방법은 다음과 같습니다:
  + 일반 설정을 호출합니다.
  + 윈도우 로그인 후 NvDA 자동 시작 설정을 해제합니다.
  + 확인 버튼을 선택합니다.
  + 일반 설정을 다시 호출합니다.
  + 윈도우 로그인 후 NvDA 자동 시작 설정을 선택합니다.
  + 확인 버튼을 선택합니다.
- UI Automation 지원 속도 향상됨(예: 파일 탐색기, 작업 전환). (#5293)
- 모질라 파이어폭스와 같은 Gecko 기반 컨트롤에서 읽기 전용 ARIA grid를 만났을때 포커스 모드로 전환되도록 함. (#5118)
...
- 마이크로소프트 파워포인트에서 문서 편집시 엔터를 눌렀을때 bullet  또는 숫자가 출력되지 않던 문제 수정. (#5360)
...
- 마이크로소프트 워드에서 브라우즈 모드 사용중 첫 글자로 이동시 점자와 리뷰 커서 위치가 업데이트되지 않던 문제 수정. (#4968)
...
= 2015.2 =
이번 버전의 주 기능들: 마이크로소프트 엑셀에 삽입된 차트 읽기 및 수학 컨텐츠 탐색 기능 추가 등입니다.

== 새로 추가된 기능 ==
- 마이크로소프트 워드와 아웃룩 사용 시 alt+아래 방향키와 alt+위 방향키를 사용하여 다음 및 이전 문장으로 이동 가능. (#3288)
- 여러 인도 언어 점자 표기법 추가. (#4778)
- 마이크로소프트 엑셀에서 여러 셀에 걸쳐있는 정보가 인식되도록 함. (#3040)
- 마이크로소프트 엑셀 사용 시 요소 목록(NVDA+F7)을 사용하여 차트와 공식 또는 주석이 있는 셀 목록을 ㅗ학인할 수 있음. (#1987)
- 마이크로소프트 엑셀 차트 지원 추가(NvDA+F7을 눌러 요소 목록을 호출하고 타츠를 선택한 후 방향키를 이용하여 데이타 확인 가능). (#1987)
- Design Science MathPlayer 4를 이용하여 웹브라우저, 마이크로소프트 워드 및 파워포인트 문서에 삽입된 수학 컨텐츠를 읽거나 집중 탐색할 수 있음(NVDA 사용설명서에 있는 수학 컨텐츠 섹션 참고 바람). (#4673)
...
- 마이크로소프트 오피스 사용중 색상을 선택 시 선택된 색상이 출력되지 않던 문제 수정. (#3045)
...
= 2015.1 =
이번 버전의 주 기능들: 마이크로소프트 워드와 아웃룩 문서 및 메시지에서의 브라우즈 모드 기능, Skype 지원 향상 및 인터넷 익스플로러 사용 시 발생하던 여러 중대 버그 수정 등입니다.
...
- 마이크로소프트 ㅜ어드 사용중 NvDA+SPACE를 눌러 브라우즈 모드로 전환후 첫 글자 명령(예: 헤딩 (H)) 및 요소 목록 (NVDA+F7) 호출 등을 사용할 수 있음. (#2975)
- 마이크로소프트 아웃룩 2007 이상에서 브라우즈 모드를 사용하여 HTML 이메일을 읽을 수 있음(만약 브라우즈 모드가 자동으로 적용되지 않았을 경우 NVDA+SPACE를 눌러 수동으로 브라우즈 모드를 선택바람). (#2975) 
- 마이크로소프트 워드 문서 저자가 표 속성 중 열 머릿말 행을 표시한 경우 표의 열 머릿말이 자동으로 출력되도록 함. (#4510) 
...
== 버그 수정 ==
- 마이크로소프트 엑셀에서 통합된 머릿말 셀(예: A1과 B1이 통합되었을때)의 머릿말이 출력되도록 함(이전엔 아무것도 출력되지 않았음). (#4617)
...
- 마이크로소프트 워드의 철자 및 문법 오류 대화상자에서 현재 오류를 변경 또는 무시 후 다음 오류가 알리지 않던 문제 수정. (#1938)
...
- 모질라 파이어폭스에서 실시간 구역내 위치한 atomic 요소 변경시 aria-atomic 속성이 반영되도록 함(이전에는 본 요소내 하위 요소에만 적용되었음). (#4794)
...
- 마이크로소프트 아웃룩 2013에서 철자 오류 검사시 철자 오류가 있는 단어가 출력되지 않던 문제 수정. (#4848)
...
- 리브루이 점자 변환기 2.6.0 버전 사용. (#4434, #3835)
...
- 모질라 파이어폭스와 같은 Gecko 기반 프로그램에서 실시간 구역(live region)을 제공하는 객체의 상위 객체의 ARIA 구역 정보가 다르더라도(예: "polite" 컨텐츠 구역에 "assertive" 컨텐츠가 출력될 경우) 새 내용이 출력되도록 함. (#4169).
...
- 마이크로소프트 워드에서 문서내 이미지에 alt text가 존재하지 않는 경우 문서 저자가 제공한 이미지의 제목을 출력하도록 함. (#4193)
- 마이크로소프트 워드 사용 시 서식 정보(NVDA+F) 호출시 문단 들여쓰기 값도 출력하도록 함. 또한 문서 서식 알림 설정에 있는 문단 들여쓰기 알림을 선택한 경우 이 정보를 자동으로 출력하도록 함. (#4165)
- 편집참과 문서에서 엔터를 눌렀을때 자동으로 삽입된 텍스트(예: 새 글 머리 기호 항목(bullet), 숫자, 탭 등)이 출력되도록 함. (#4185)
- 마이크로소프트 워드에서 현재 문서 위치에 주석이 존재하는 경우 NvDA+Alt+C를 눌러 주석을 확인할 수 ㅣㅇㅆ음. (#3528)
- 마이크로소프트 엑셀에서의 행과 열 머릿말 알림 기능 향상 (#3568):
...
- 마이크로소프트 워드에서 행과 열 머릿말 알림 기능 추가 (#3110):
...
- 마이크로소프트 워드에서 탭을 누를때 페이지 좌측 가장자리로부터의 간격을 알리도록 함. (#1353)
- 마이크로소프트 워드 사용 시 여러 서식(굵게 쓰기, 이태릭, 밑줄, 표시, outline 레벨 등) 변환 명령이 음성과 점자로 출력되도록 함. (#1353)
- 마이크로소프트 엑셀에서 현재 셀에 주석이 존재하는 경우 NvDA+Alt+C를 눌러 주석을 확인할 수 ㅣㅇㅆ음. (#2920)
- 마이크로소프트 엑셀에서 Shift+F2를 눌러 주석 편��� 모드를 선택한 후 NVDA가 제공하는 주석 편집 대화상자를 통해 포커스된 셀에 주석을 삽입할 수 있음. (#2920)
- 마이크로소프트 엑셀 사용 시 더 많은 셀 선택 및 이동 명령에 대한 피드백을 음성 및 점자로 확인 가능 (#4211):
 - 수직으로  페이지 이동 (pageUp/pageDown)
 - 수평으로 페이지 이동 (alt+pageUp/alt+pageDown)
 - 선택 확장 (위 키에 shift를 더함)
 - 현재 영역 선택 (control+shift+8)
- 마이크로소프트 엑셀에서 서식 정보(NVDA+F) 호출시 셀의 정렬(수직 및 수평 배치)도 출력하도록 함. 또한 문서 서식 알림 설정에 있는 배치 알림을 선택한 경우 이 정보를 자동으로 출력하도록 함. (#4212)
- 마이크로소프트 엑셀에서 서식 정보(NVDA+F) 호출시 셀의 스타일이 출력되도록 함. 또한 문서 서식 알림 설정에 스타일 알림을 선택한 경우 이 정보를 자동으로 출력하도록 함. (#4213)
- 마이크로소프트 파워포인트에서 방향키를 이용하여 셰이프(shape)를 옮길시 셰이프의 위치가 출력되도록 함 (#4214):
...
== 변경된 기능 ==
- 리브루이 점자 변환기 2.5.4 버전 사용. (#4103)


== 버그 수정 ==
- 구글 크롬이나 크롬 기반 브라우저 사용 시 경고나 대화상자 텍스트중 특정 텍스트(특히 특정 서식이 적용된 텍스트)가 반복으로 출력되던 문제 수정. (#4066)
...
- 마이크로소프트 아웃룩 2010 이상에서 새 계정 및 이메일 계정 설정과 같은 보안용 대화상자의 접근성이 향상됨. (#4090, #4091, #4095)
- 여러 아웃룩 대화상자에서 불필요한 command toolbar 내용이 출력되지 않도록 함. (#4096, #3407)
- 마이크로소프트 워드에서 탭을 이용하여 표의 빈 셀로 옮길때 표 끝이라고 출력되지 않도록 함. (#4151)
- 마이크로소프트 워드에서 표 이후 글자(표 밖에 있는 빈칸 등)가 표 내 글자로 인식되던 문제 수정. (#4152)
- 마이크로소프트 워드 2010 철자 오류 대화상자에서 철자 오류가 있는 단어 대신 첫 굵게 표시된 단어가 출력되던 문제 수정. (#3431) 
- 인터넷 익스플로러와 같은 MSHTML 컨트롤에서 브라우즈 모드 사용 시 탭을 누르거나 글자키로 폼 필드로 이동시 필드 제목(HTML 레이블 요소가 사용된 경우)이 출력되지 않던 문제 수정. (#4170)
- 마이크로소프트 워드에서 주석 존재 여부 및 위치 알림 정확도가 향상됨. (#3528)
- 마이크로소프트 오피스 프로그램(예: 워드, 엑셀, 아웃룩) 사용 시 여러 대화상자에서 불필요한 command toolbar 컨테이너 내용이 출력되지 않도록 함. (#4198)
- 마이크로소프트 워드, 엑셀 또는 다른 프로그램 실행 시 클립보드 메니저와 파일 복구와 같은 테스크 페인이 포커스되는 것을 방지하여 사용자가 다른 프로그램으로 전환하다가 돌아오는 것을 최소화하도록 함. (#4199)
...
- 구글 크롬에서 탭 전환시 페이지 제목이 출력되지 않던 문제 수정(NVDA 2014.2 사용 시 특정 상황에서 이 문제가 발생했었음). (#4222)
- 구글 크롬이나 크롬 기반 브라우저 사용 시 문서 URL(주소)가 문서와 함께 출력되던 문제 수정. (#4223)
- "음성 출력 끔" 음성 엔진을 사용(자동 테스트에 유용함)하여 모두 읽기 실행 시 문서 전체를 읽도록 함(이전에는 몇 줄만 읽었음). (#4225)
- 마이크로소프트 아웃룩 서명 편집창 접근 향상 (커서 이동 및 서식 정보 확인 가능). (#3833) 
- 마이크로소프트 워드에서 표 셀의 마지막 줄을 읽을때 표 셀의 내용이 출력되던 문제 수정. (#3421)
- 마이크로소프트 워드에서 목차의 첫줄 또는 마지막 줄을 읽을때 목차의 내용이 출력되던 문제 수정. (#3421)
- 입력된 단어 출력 및 여러 상황에서 인도어 모음 또는 virama 글자 입력 시 입력된 단어가 깨지는 문제 수정. (#4254)
- 골드웨이브에서 숫자 입력창을 제대로 처리하지 못하던 문제 수정. (#670)
- 마이크로소프트 워드에서 control+아래 방향키 또는 control+위 방향키를 사용하여 다음 및 이전 문단을 읽을때 bullet 또는 숫자 목록에서 본 명령을 두번 눌러야 문단을 이동하던 문제 수정. (#3290)
...
- 마이크로소프트 파워포인트 사용 시 입력된 단어 읽기를 선택한 후 벡스페이스를 누를때 삭제된 글자가 이후 입력된 단어에 이어 출력되지 않도록 함. (#3231)
- 마이크로소프트 오피스 2010 설정 대화상자에 ㅣㅇㅆ는 콤보박스의 이름이 제대로 출력되도록 함. (#4056)
...
== 새로 추가된 기능 ==
- 마이크로소프트 파워포인트 2013 지원 (단 보안 모드는 지원않함). (#3578)
- 마이크로소프트 워드ㅗ아 엑셀의 기호 삽입 대화상자에서 삽입하고자 하는 기호가 출력되도록 함. (#3538)
...
- 마이크로소프트 VC Runtime 11 사용으로 인해 윈도우 XP 서비스팩 2 또는 윈도우 서버 2003 서비스팩 1 이전 시스템에서 NVDA 실행 불가능.
- 별표(*)와 드하기(+) 문자가 기호 발음 출력을 기호 출력으로 설정했을때에도 출력되도록 함. (#3614)
- 이스피크 음성 엔진 1.48.04 사용 및 이에 따른 여러 버그 및 언어 수정. (#3842, #3739)


== 버그 수정 ==
- 마이크로소프트 엑셀에서 셀로 이동 또는 선택할때 엑셀이 다운될때 새 셀이 아닌 이전 셀이 출력되던 문제 수정. (#3558)
- 마이크로소프트 엑셀에서 셀의 팝업 메뉴를 눌렀을때 나타나는 드랍다운 목록을 제대로 처리하지 않던 문제 수정. (#3586)
...
- 구글 크롬에서 브라우즈 모드 사용 시 체크박스와 라디오 버튼의 레이블이 잘못 출력되던 문제 수정.  (#1562)
...
- 마이크로소프트 엑셀에서 모든 셀의 서식 정보가 밑줄이 적용된 것처럼 출력되는 문제 수정. (#3669)
...
- 마이크로소프트 워드 사용 시 표 셀의 행 및 열 정보가 반복으로 출력되던 문제 수정. (#3702)
- 프랑스어나 독일어와 같이 공백으로 숫자 그루핑을 나눌때 각 숫자 그루핑이 한 숫자로 출력되던 문제 수정(특히 표 셀에 숫자가 있을 경우 문제가 발생했음). (#3698)
- 마이크로소프트 워드 2013에서 시스템 캐럿 이동시 점자 커서가 제대로 움직이지 않던 문제 수정. (#3784)
- 마이크로소프트 워드에서 점자 디스플레이 커서가 헤딩 문자열의 첫 글자에 위치한 경우 헤딩 레벨 등과 같은 헤딩 정보가 점자로 출력되지 않던 문제 수정. (#3701)
...
- 모질라 파이어폭스와 같은 Gecko 기반 프로그램에서 ARIA live regions 지원 향상됨:
...
- 모질라 파이어폭스와 같은 Gecko 기반 프로그램에서 브라우즈 모드 사용 시 특정 presentation 컨트롤(특히 ARIA 포커스 가능)이 더 이상 출력되지 않도록 함. (#3781)
- 마이크로소프트 ㅜ어드 사용 시 철자 오류가 ㅣㅇㅆ는 문서를 읽을때 NVDA의 반응 속도가 개선됨. (#3785)
...
== 새로 추가된 기능 ==
- 마이크로소프트 워드 문서 폼 필드 인식 가능. (#2295)
- 마이크로소프트 워드에서 트랙 변경을 설정하고 NVDA 문서 서식 설정에서 문서 리비전 알림을 선택한 경우 리비전이 출력되도록 함 (본 NvDA 설정은 해제되어 있음). (#1670)
- 마이크로소프트 엑셀 2003-2010 사용 시 드랍다운 목록을 열고 탐색 시 항목이 출력되도록 함. (#3382)
...
- 마이크로소프트 엑셀에서 셀이 링크인 경우 링크로 인식하도록 함. (#3042)
- 마이크로소프트 엑셀에서 셀에 코멘트가 있는 경우 코멘트 정보도 확인 가능. (#2921)
 

== 버그 수정 ==
- Zend Studio 개발 도구 사용 시 Eclipse 사용할때와 똑같이 NVDA가 작동하도록 함. (#3420)
- 마이크로소프트 아웃룩 2010 사용 시 메시지 규칙 대화상자에 있는 여러 체크박스의 선택 및 해제 상태가 알려지지 않던 문제 수정. (#3063)
- 모질라 파이어폭스와 같은 프로그램에서 특정 컨트롤이 핀(pinned)된 경우 이를 인식할 수 있음. (#3372)
...
- 보안 처리가 된 마이크로소프트 워드 2010 문서를 읽을시 워드가 종료되던 문제 수정. (#1686)
- NVDA 페키지 실행 시 부정확한 명령 스위치를 입력하였을때 연속으로 오류 메시지가 뜨던 문제 수정. (#3463)
- 마이크로소프트 워드 사용 시 그래픽이나 객체의 alt tag 텍스트에 따옴표와 같은 특정 문자열이 있는 경우 alt tag 텍스트가 출력되지 않던 문제 수정. (#3579)
- 브라우즈 모드에서 가로로 표시된 목록의 항목수가 어떤때 두배로 알려지던 문제 수정. (#2151)
- 마이크로소프트 엑셀에서 CTRL+A를 눌렀을때 선택 변경 내용이 제대로 알려지지 않던 문제 수정. (#3043)
...
- 마이크로소프트 엑셀에서 다중 선택된 셀과 통합된 셀이 다르게 출력되도록 함. (#3567)
...
- 마이크로소프트 아웃룩 2010에서 자동 끝내기 알림 추가. (#2816)
...
- 오피스 2013의 확인 대화상자의 내용이 자동으로 읽지않던 문제 수정.
- 마이크로소프트 워드에서 특정 표를 읽을시 다운되던 문제 수정. (#3326)
- 마이크로소프트 워드에서 표의 셀이 여러 행에 걸쳐 있는 경우 NVDA의 표 탐색 명령(Control+Alt+방향키) 실행 시 다운되던 문제 수정.
- 추가 기능 관리자는 이미 열려 있고 사용자가 관리자를 재실행할때 (예: NVDA 도구 메뉴에서 선택하거나 추가 기능 파일을 연 경우) 추가 기능 관리자를 닫을 수 없던 문제 수정. (#3351)
- 오피스 2010 중국어 또는 일본어 IME 사용 시 특정 대화상자에서 NVDA가 다운되던 문제 수정. (#3064)
...
- 마이크로소프트 파워포인트에서 특정 객체를 선택 또는 편집할때 객체값이 두번 출력되던 문제 수정. (#3394)
...
== 새로 추가된 기능 ==
- 마이크로소프트 파워포인트 문서 읽기 및 편집 기능 지원 (기초단계임) (#501)
- Lotus Notes 8.5에서의 메시지 작성 및 읽기 기능 지원 (기초 단계). (#543)
- 자동 언어 변경 기능을 마이크로소프트 워드 문서 읽을때에도 적용. (#2047) 
...
- 여러 구글 크롬을 기반으로 한 웹 브라우저 지원 추가: Rockmelt, BlackHawk, Comodo Dragon, SRWare Iron. (#2236, #2813, #2814, #2815)


== 변경된 기능 ==
- 리브루이 점자 변환기 2.5.2 버전 사용. (#2737)
...
- 마이크로소프트 워드 2003과 인터넷 익스플로러 편집창과 같은 컨트롤에서 선택된 텍스트가 점자로 제대로 표시되도록 함.
- 점자 디스플레이 사용 시 마이크로소프트 워드에서 반대 방향으로 글자 선택이 되지 않던 문제 해결.
...
- IBM Lotus Symphony와 오픈오피스에서 캐럿이 움직일때 리뷰 커서도 움직이도록 함.
...
- 인터넷 익스플로러 10 기준 모드(예: www.gmail.com의 로그인 페이지)에서 브라우즈 모드를 사용할 수 없었던 문제 수정. (#3151)
...
- 마이크로소프트 엑셀에서 행과 열 머릿말 알림 기능 추가. NVDA+Shift+C를 눌러 현재 행을 열 머릿말 행으로, NVDA+Shift+R을 눌러 현재 열을 행 머릿말 열로 지정; 이 키들을 두번 눌러 머릿말 설정을 해제할 수 있음. (#1519)
...
- 마이크로소프트 아웃룩 2007에서 이메일 주소 입력 시 자동 끝내기 알림 추가 (#689)
...
- 마이크로소프트 워드 2013에서의 문서 컨트롤 지원 (기본 지원 단계). (#2543)
...
- 리브루이 점자 변환기 2.5.1 버전 사용. (#2319, #2480, #2662, #2672)
...
- 마이크로소프트 아웃룩이나 다른 비밀번호 편집창과 같은 비밀번호 입력창에서 비밀번호가 출력되던 문제 수정. (#2021)
- 어도비 리더에서 폼 필드 내 변경된 정보가 브라우즈 모드에서 읽어지도록 수정. (#2529)
- 윈도우 비스타 이상 컴퓨터에 NVDA가 설치된 경우 마이크로소프트 워드의 문법 확인 기능 지원 개선 (예: 문법 오류 표시 향상).
...
- 마이크로소프트 엑셀에서 셀 좌표가 점자로 표시되지 않던 문제 수정.
- 마이크로소프트 워드에서 숫자나 리스트로된 문단에서 왼쪽 화살표나 CTRL+왼쪽 화살표를 누를때 이전 문단의 내용이 목록 서식으로 된 내용으로 인식되던 문제 수정. (#2402)
...
- 구글 크롬에서 브라우즈 모드 사용 시 탭 내용이 잘못 출력되던 문제 수정.
