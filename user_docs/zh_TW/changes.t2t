...
Mozilla Firefox 的 NVDA 瀏覽模式現已提供新的原生選取模式 (透過 NVDA+Shift+F10 切換)，開啟後，在瀏覽模式下選擇文字也會操作 Firefox 自己的原生選取。當使用 Ctrl+C 複製文字將直接使用 Firefox 的複製，能夠複製豐富的內容，而不是 NVDA 的純文字表示形式。

附加元件商店現在支援透過選擇多個附加元件進行批次操作 (例如安裝、啟用附加元件)，且加入一個新動作可以開啟所選附加元件的評論網頁。

音訊輸出裝置和自動音量調整模式選項已從「選擇合成器」對話框中刪除，它們被移到音訊設定類別，且可以使用 NVDA+Ctrl+U 開啟該設定。
...
== 新功能 ==
- 附加元件商店
  - 附加元件商店現在支援透過選擇多個附加元件進行批次操作 (例如安裝、啟用附加元件)。(#15350, #15623, @CyrilleB79)
  - 新增了一個新動作，用於開啟網頁以查看或提供有關所選附加元件的回饋。(#15576, @nvdaes)
  -
- 增加了對藍牙低功耗 HID 點字顯示器的支援 (#15470)
- Mozilla Firefox 的 NVDA 瀏覽模式提供新的原生選取模式 (透過「NVDA+shift+f10」切換)。
開啟後，在瀏覽模式下選擇文字也會操作 Firefox 自己的原生選取。
當使用 Ctrl+C 複製文字將直接使用 Firefox 的複製，能夠複製豐富的內容，而不是 NVDA 的純文字表示形式。
但請注意，由於是由 Firefox 處理複製內容，因此 NVDA 在此模式下不會讀出「複製到剪貼簿」的訊息。 (#15830)
...
- 元件更新：
  - LibLouis 點字轉譯表更新為 [3.28.0 https://github.com/liblouis/liblouis/releases/tag/v3.28.0]。(#15435, #15876, @codeofdusk)
    - 新推出泰文、菲律賓文和羅馬尼亞文點字表。
    -
  - eSpeak NG 已更新至 1.52-dev commit ``530bf0abf``。(#15036)
  - CLDR 表情符號和符號說明已更新至版本 44.0。(#15712, @OzancanKaratas)
  - Java Access Bridge 更新至 17.0.9+8Zulu(17.46.19). (#15744)
  -
- 按鍵指令：
  - 以下指令現在支援按兩下拼讀及按三下以字元說明拼讀：讀出選取內容、讀出剪貼簿內的文字和讀出焦點物件。(#15449, @CyrilleB79)
  - 切換螢幕簾幕的指令現在有一個預設快速鍵：NVDA+Ctrl+Esc。(#10560, @CyrilleB79)
  - 按四下時，讀出選取內容的指令現在會在可瀏覽的視窗中顯示選取內容。(#15858, @Emil-18)
  - 
- Microsoft Office：
...
= 2023.3 更新中文摘譯 =
此版本包括對音訊輸出的效能、反應速度和穩定性的改進，新增了選項來控制 NVDA 聲音和嗶嗶聲的音量，或讓它們跟隨您正在使用的語音音量。

NVDA 現在可以定時更新 OCR 的結果，並說出新出現的文字，這可以在 NVDA 設定對話框中的 Windows OCR 類別中進行設定。

多項點字修正，改進了設備偵測和游標移動，現在可以在自動偵測中排除不需要的驅動程式，以提高自動偵測的效能，同時也加入了新的 BRLTTY 指令。
...
- 增強的聲音管理：
  - 新增音訊設定類別：
    - 可用 ``NVDA+control+u`` 來開啟。 (#15497)
    - 可讓 NVDA 音效和嗶嗶聲的音量跟隨您正在使用的語音的音量設定。 (#1409)
    - 可獨立設定 NVDA 音效的音量。 (#1409, #15038)
    - 更改音訊輸出裝置和切換自動音量調整的設定已從「選擇合成器」對話框移至新的音訊設定類別。
    這些選項將從 2024.1 中的「選擇合成器」對話方塊中刪除。 (#15486, #8711)
    -
  - NVDA 現在將透過 Windows Audio Session API (WASAPI) 輸出音訊，這可能會提高 NVDA 語音和音效的反應、效能和穩定性。 
  - 注意：WASAPI 與某些附加元件不相容。
  這些附加元件有相容的更新版本，請在更新 NVDA 之前更新它們。
  更新 NVDA 時將停用這些附加元件的不相容版本：
...
== 更新 ==
- 點字:
  - 當終端程式中的文字發生變更而沒有移動插入點位置時，  點顯器上的文字將能夠正確顯示。
  這包括點字跟隨檢閱游標的情況。 (#15115)
  - 更多 BRLTTY 按鍵組合對應到 NVDA 指令 (#6483):
    - ``learn``: 切換 NVDA 輸入說明
    - ``prefmenu``: 開啟 NVDA 功能表
    - ``prefload``/``prefsave``: 載入/儲存 NVDA 組態
    - ``time``: 顯示時間
    - ``say_line``: 讀出檢閱遊標所在行
    - ``say_below``: 使用檢閱游標讀出全部
    -
  - 僅當啟用了 BrlAPI 的 BRLTTY 個體正在執行時，BRLTTY 驅動程式才可用。 (#15335)
  - 進皆設定中的「啟用 HID 點字支援」已被刪除，取而代之的是新選項，
  現在您可以在選擇點顯器對話框中停用特定的點顯器驅動程式。 (#15196)
  -
...
- 點字：
  ...
  - 修正了儘管已連接另一個點字設備，但 Albatross 點字顯示器仍嘗試初始化的錯誤。 (#15226)
  -
- 附加元件商店：
  ...
  - 修正瞭如果先前的下載失敗或被取消則無法安裝附加元件的錯誤。 (#15469)
...
= 2023.2 更新中文摘譯 =
此版本引入附加元件商店來取代原有的附加元件管理員，在附加元件商店中，您可以瀏覽、搜尋、安裝及更新社群附加元件，您現在也可在自行承擔風險的情況下手動忽略附加元件不相容的問題。
加入新的點字功能、指令及點顯器支爰，也加入了 OCR 及平面物件導覽的輸入手勢。
Microsoft Office 的導覽與格式報讀有所改進，並且有許多錯誤修正，特別是針對點字、Microsoft Office、網頁瀏覽器及 Windows 11。同時也更新了 eSpeak-NG、LibLouis 點字轉譯表、 及 Unicode CLDR 等。

== 新功能 ==
- 在 NVDA 中加入附加元件商店。 (#13985)
  - 瀏覽、搜尋、安裝及更新社群附加元件。
  - 針對過時的附加元件可以手動忽略不相容。
  - 移除附加元件管理員，並以附加元件商店取代。
  - 更多資訊請參考更新的用戶指南。
  -
- 新的輸入手勢：
...
- 新的點字功能：
  ...
  - Baum 點字驅動程式：增加幾個點字手勢來執行常見的鍵盤指令，例如 Windows+D 和 Alt+Tab。
  請參考 NVDA 用戶指南以取得完整列表。 (#14714)
  -
- 增加 Unicode 符號的發音：
  - 點字符號，例如 ⠐⠣⠃⠗⠇⠐⠜ (#13778)
  - Mac Option 鍵 ⌥ (#14682)
...
- 實驗性加強聲音管理：
  ...
  - 可到進階設定中啟用使用 WASAPI。
  此外，如果啟用了 WASAPI，還可以調整以下進階設定。
    - 讓 NVDA 音效及嗶嗶聲跟隨您所使用的語音音量的選項。 (#1409)
    - 單獨調整 NVDA 音效音量的選項。 (#1409, #15038)
    -
  - 啟用 WASAPI 時存在已知的間歇性崩潰問題。 (#15150)
  -
- 在 Mozilla Firefox 和 Google Chrome 中，如果作者使用 aria-haspopup 來指定控制項，當開啟對話框、網格、列表或樹狀檢視時，NVDA 現在能夠報讀。 (#8235)
- 現在可以在建立 NVDA 可攜式版時在指定路徑中使用系統變數 (例如 %temp% 或 %homepath%)。 (#14680)
- 在 Windows 10 May 2019 更新及以後版本中，NVDA 可以在開啟、更改和關閉虛擬桌面時讀出虛擬桌面名稱。 (#5641)
...
== 更新 ==
- 更新組件：
  - eSpeak NG 更新至 1.52-dev commit ed9a7bcf。 (#15036)
  - 更新 LibLouis 點字轉譯表至 [3.26.0 https://github.com/liblouis/liblouis/releases/tag/v3.26.0]。 (#14970)
  - CLDR 更新至版本 43.0。 (#14918)
  -
- LibreOffice 更新：
  - 在 LibreOffice Writer 7.6及以後版本，當讀出檢閱游標位置時，能夠正確讀出游標/插入點在該頁面的正確位置，如同在 Microsoft Word 中一樣。 (#11696)
  - 讀出狀態列 (例如按下 NVDA+End 觸發) 適用於 LibreOffice。 (#11698)
...
- 點字更新：
  ...
  同時 Space+第一點及 Space+第四點也對應到向上及向下方向鍵。 (#14713)
...
- Windows 11 修正：
  - NVDA 再次能夠讀出記事本狀態列內容。 (#14573)
  - 在記事本及檔案總管中的分頁之間切換將可報讀分頁名稱及位置。 (#14587, #14388)
  - 輸入中文和日文等語言的文字時，NVDA 再次能夠讀出候選字。 (#14509)
  - 能夠再次開啟 NVDA 說明功能表中的供獻者與授權頁。 (#14725)
  -
-
...
= 2023.1 更新中文摘譯 =
- 在設定中加入「文件導覽」類別，其中包含一個新選項「段落樣式」，這可用於本身不支援段落導覽的文件編輯器，例如記事本或 Notepad++。
...
這可用於本身不支援段落導覽的文件編輯器，例如記事本或 Notepad++。 (#13797)
- 現在能夠讀出多個註釋。
按 NVDA+D 循環讀出具有多個註釋來源中每個註釋的摘要。
例如，當文字具有相關的註釋及註腳時。 (#14507, #14480)
- 支援 Tivomatic Caiku Albatross 46/80 點字顯示器。 (#13045)
- 新的全域指令用來讀出連結目標 (NVDA+K)。
按一下會以語音/點字讀出導覽物件所在的連結目標 URL。
按兩下會將它顯示在一個視窗中，便於詳細瀏覽。 (#14583)
- 新增未指派的指令 (工具類別)：在視窗中讀出連結目標
與按兩下 NVDA+K 相同，但可能對點字使用者更方便。 (#14583)
-


== 變更 ==
- 更新 LibLouis 點字轉譯表至 [3.24.0 https://github.com/liblouis/liblouis/releases/tag/v3.24.0]。 (#14436)
  - 主要更新包括匈牙利文、UEB 及中文注音點字。
  - 支援丹麥點字標準 2022。
  - 新的點字表，包括喬治亞文書寫點字、史瓦西里文 (肯亞) 及齊切瓦文 (馬拉威)。
  -
- 更新Sonic rate boost library 至 commit 1d70513。 (#14180)
- 更新 CLDR 至版本 42.0。 (#14273)
- 更新 eSpeak NG 至 1.52-dev commit f520fecb。 (#14281, #14675)
  - 修正對於大數的報讀。 (#14241)
  -
...
== 錯誤修正 ==
- Windows 11 修正：
  - 在啟動「開始」功能表時，NVDA 會讀出搜尋。 (#13841)  
  - 在 ARM 系統架構上，x64 應用程式不再被視為 ARM64 應用程式。 (#14403)
  - 可以操作剪貼簿歷程紀錄，例如「釘選項目」。 (#14508)
...
- 當按下 HTML 表格內的按鈕來更改針對欄標題的排序，NVDA 會自動讀出排序狀態。 (#10890)
...
- 透過通知區域啟用 NVDA 功能表，當沒有更新可用時，NVDA 將不會再提示有擱置的更新。 (#14523)
- 在 foobar2000 中能夠正確讀出長度超過一天音檔的剩餘時間、經過時間及總時間。 (#14127)
- 在 Chrome 與 Firefox 等網頁瀏覽器中，檔案下載等通知訊息除了以語音提示外，也能夠以點字呈現。 (#14562)
- 修正在 Firefox 中導覽到表格的第一列或最後一列時的錯誤。 (#14554)- When NVDA is launched with ``--lang=Windows`` parameter, it is again possible to open NVDA's General settings dialog. (#14407)
-
...
有新的中文、瑞典語、盧干達文及金揚萬答文點字表。

== 新功能 ==
- 在用戶指南中加入「快速入門指南」。(#13934)
- 加入一個新指令來檢查目前焦點的鍵盤快捷鍵。(#13960)
  - 桌上型電腦：Shift+數字鍵盤2
  - 筆記型電腦：NVDA+Ctrl+Shift+.
  -
- 加入新的指令，可在應用程式支援情況下，以頁移動檢閱游標。(#14021)
  - 移到上一頁：
    - 桌上型電腦：NVDA+PageUp
    - 筆記型電腦：NVDA+Shift+pageUp
    -
  - 移到下一頁：
    - 桌面： NVDA+PageDown
    - 筆記型電腦： NVDA+Shift+pageDown
    -
  -
- 加入以下的表格指令。(#14070)
  - 向下讀出整欄：NVDA+Ctrl+Alt+向下鍵
  - 向右讀出整列：NVDA+Ctrl+Alt+向右鍵
  - 讀出整欄：NVDA+Ctrl+Alt+向上鍵
  - 讀出整列：NVDA+Ctrl+Alt+向左鍵
  -
...
== 更新 ==
- eSpeak NG 更新至 1.52-dev commit 735ecdb8。 (#14060, #14079, #14118, #14203)
  - 修復使用中文時拉丁字元報讀的錯誤。(#12952, #13572, #14197)
  -
- 更新 LibLouis 點字轉譯表至 [3.23.0 https://github.com/liblouis/liblouis/releases/tag/v3.23.0]。 (#14112)
  - 增加的點字表：
    - 中國通用盲文 (簡體中文字)
    - 金揚萬答文書寫點字
    - 盧干達文書寫點字
    ...
    - 瑞典文部分收縮點字
    - 瑞典文收縮點字
    - 中文 (中國) 現行盲文 (無聲調)(#14138)
    -
- NVDA 使用者資料蒐集現在包含作業系統的架構。 (#14019)
-
...
- 在 Bookworm、WordPad 或 NVDA 事件紀錄檢視器等應用程式中使用「讀出全部」期間，游標正確地跟隨唸出的文字。(#13420, #9179)
- 在使用 UI Automation 的程式中，部分勾選的核取方塊可以正確讀出。(#13975)
- 改進 Microsoft Visual Studio、Windows 終端程式和其他基於 UI Automation 應用程式的效能及穩定性。(#11077, #11209)
...
請注意，Chrome 和 Edge 仍然存在已知問題。(#13254)
  - 視覺醒目提示外框在大多數應用程式中，應可正確呈現。(#13370, #3875, #12070)  - 觸控螢幕互動對於大多數應用程式，現在應該是準確的。(#7083)
  - 鼠標跟隨現在應適用於大多數應用程式。(#6722)
  -
...
此更新也包含一些錯誤修正。
值得注意的是，使用最新版本的 Adobe Acrobat/Reader 閱讀 pdf 文件時將不再崩潰。
...
- 在使用 Windows 主控台的應用程式，包括命令提示字元、PowerShell、及在 Windows 11 版本 22H2 (Sun Valley 2) 及更高版本上適用於 Linux 的 Windows 子系統等中：
  - 大幅提高了效能和穩定性。 (#10964)
  - 當按 ``Ctrl+F`` 搜尋文字時，檢閱游標會跟隨移動至找到的字串上。 (#11172)
  - 預設情況下停用讀出未出現在螢幕上的輸入文字 (例如密碼)。
它可以在 NVDA 的進皆設定中重新啟用。 (#11554)
  - 可以在不捲動主控台視窗的情況下查看已捲動到螢幕之外的文字。 (#12669)
  - 提供更詳細的文字格式資訊。 ([microsoft/terminal PR 10336 https://github.com/microsoft/terminal/pull/10336])
  -
...
== 更新 ==
- 更新 eSpeak NG 至版本 1.52-dev commit ``9de65fcb``。(#13295)
  - 新增語言：
    - 白俄羅斯語
    - 盧森堡語
    - Totontepec Mixe
...
== 錯誤修正 ==
- 當閱讀 pdf 文件時，Adobe Acrobat / Reader 64 位元不會再崩潰。(#12920)
  - 注意：要避免崩潰也需要安裝最新版本的 Adobe Acrobat / Reader 以避免崩潰。
  -
...
= 2022.2 更新中文摘譯 =
此版本更新包含了許多錯誤修正，尤其是基於 Java 的應用程式、點字顯示器及 Windows 功能。
加入新的表格導覽指令、更新 Unicode CLDR、更新 LibLouis，包含了新的德文點字表。。

== 新功能 ==
- 支援在 Microsoft Office  產品中與 Microsoft Loop 元件互動。(#13617)
- 加入新的表格導覽指令。(#957)
 - ``Ctrl+Alt+Home/End`` 移至第一/最後一欄。
...
- 更新 LibLouis 點字轉譯表至 [3.22.0 https://github.com/liblouis/liblouis/releases/tag/v3.22.0]。(#13775)
  - 新增點字表： 德文二級 (詳細)
  -
- 新增「忙碌指示器」 (busy indicator) 控件之角色。(#10644)
...
- 修正基於 Java 的應用程式內錯誤：
  ...
  - NVDA 會讀出索引標籤控件的位置資訊。(#13744)
  -
- 點字修正：
  - 修正當在 Mozilla rich edit 控製項中導覽文字時，點字輸出的問題。(#12542)
  - 當點字跟隨設為自動，且跟隨鼠標朗讀啟用並移動滑鼠時，點字會跟著所朗讀的文字呈現。(#11519)
...
- 當在 Adobe 中閱讀無法互動的 pdf 時，表單欄位的類型與狀態 (如核取方塊、選擇鈕等) 可被讀出來。(#13285)
...
- 在 Windows 10 及 11 小算盤版本 10.1908 及之後版本，NVDA 會讀出當按下更多指令時的結果，例如在科學模式下。(#13386)
- 在 Windows 11 中，使用滑鼠及觸控互動時，能夠導覽並與使用者介面元素互動，如工作列及工作檢視。(#13508)
- 在 Wordpad 及其他 ``richEdit`` 控件中，隱藏文字不會再被讀出來。(#13618)
- 在 Windows 11 記事本，NVDA 會讀出狀態列內容。(#13386)
...
= 2022.1 中文摘譯 =
此更新包含了大幅改善 UIA 對於 MS Office 的支援，對於 Windows 11 上的 Microsoft Office 16.0.15000 及更高的版本，NVDA 會預設使用 UI Automation 來處理 Microsoft Word 文件，這相較於舊的物件模型處理，有顯著的效能提升。
在點顯器的驅動程式支援也有改善，包括 Seika Notetaker, Papenmeier 及 HID Braille。
...
- 使用新的 ``--lang`` 命令列參數可覆蓋 NVDA 設定的語言。(#10044)
- 對於未知或沒有任何附加元件使用的命令列參數，NVDA 會提出警告。(#12795)
- 在 Microsoft Word 中使用 UI Automation，NVDA 可以讀取並導覽 Office Math 方程式。(#12946)
...
- 支援 Windows 11 計算機。(#13212)
- 在 Windows 11 中的 Microsoft Word 並啟用 UI Automation，行號、章節號與版面配置列號現在能夠被讀出。 (#13283)
- 對於 Windows 11 上的 Microsoft Office 16.0.15000 及更高的版本，NVDA 會預設使用 UI Automation 來處理 Microsoft Word 文件，這相較於舊的物件模型處理有顯著的效能提升。(#13437)
 - 這包括 Microsoft Word 文件，以及 Microsoft Outlook 中讀取及撰寫訊息。
 -
-

== 變更 ==
- 更新 Espeak-ng 1.51-dev commit ``7e5457f91e10``。 (#12950)
- 更新 liblouis 點字轉譯表至 [3.21.0 https://github.com/liblouis/liblouis/releases/tag/v3.21.0]。(#13141, #13438)
...
- 使用讀出文字格式指令 (NVDA+F) 不會再讀出「基準線」。(#11815)
- 「顯示長說明」不再有預設指派的手勢。(#13380)
- 「讀出詳細資料的摘要」目前有預設快速鍵 NVDA+D。 (#13380)
