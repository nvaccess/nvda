...
* 元件更新：
  * 將 LibLouis 點字轉譯表更新為 [3.33.0](https://github.com/liblouis/liblouis/releases/tag/v3.33.0)。 (#17469, @LeonarddeR，@codeofdusk)
    * 新增了科普特文、精簡楔形文字，以及葡萄牙文六點電腦點字的點字表。
...
* 數學：
  * 已修正部分網頁元素上數學內容的閱讀問題。
  具體來說，是指位於 span 元素以及其他具有屬性 `role="math"` 的元素內部的 MathML。(#15058)
...
* 拼讀時，Unicode 正規化現在運作得更為恰當：
  ...
  * 組合字元 (例如 é) 能被正確讀出。(#17295, @LeonarddeR)
...
請參閱 [開發者指南](https://www.nvaccess.org/files/nvda/documentation/developerGuide.html#API) 以了解 NVDA 的 API 廢除與移除過程。
...
* 已將 [NVDA 的 VS Code 工作區組態](https://nvaccess.org/nvaccess/vscode-nvda) 作為 Git 子模組加入。(#17003)
...
* 由於 NVDA 已停止支援 winmm (#17496, #17532, #17678)：
  ...
  * 組態金鑰 `config.conf["speech"]["outputDevice"]` 已被移除。
...
* 以下符號已被移除，且無替代方案：`languageHandler.getLanguageCliArgs`, `__main__.quitGroup` and `__main__.installGroup` 。 (#17486, @CyrilleB79)
...
* 按下更新或延後按鈕時，`updateCheck.UpdateAskInstallDialog` 不再自動執行動作。
...
* 函數 `gui.message.messageBox`、`gui.runScriptModalDialog` 以及類 `gui.nvdaControls.MessageDialog` 已棄用。
...
* 元件更新：
  * 將 LibLouis 點字轉譯表更新為 [3.31.0](https://github.com/liblouis/liblouis/releases/tag/v3.31.0)。 (#17080, @LeonarddeR，@codeofdusk)
    * 修復了西班牙語點字數字的翻譯。
    * 新增點字表：
      * 泰文一級
      * 希臘國際點字 (一方重音字母)
...
* 修正了 NVDA 偶發無法打開可瀏覽訊息（例如按 `NVDA+f` 兩次）的問題。(#16806, @LeonarddeR)
...
* 組件更新：
  * 將 py2exe 更新到 0.13.0.2。(#16907, @dpy013)
  * 將 setuptools 更新到 72.0。(#16907, @dpy013)
  * 將 Ruff 更新到 0.5.6。(#16868, @LeonarddeR)
  * 將 nh3 更新到 0.2.18。(#17020, @dpy013)
* 在 NVDA 的儲存庫中新增了一個 `.editorconfig` 檔，以便多個 IDE 預設情況下採用基本的 NVDA 代碼樣式規則。(#16795, @LeonarddeR)
* 新增了對自訂語音符號字典的支援。(#16739, #16823, @LeonarddeR)
  * 字典可以在附加元件包中的特定語言區域資料夾中提供，例如 locale\en。
  * 字典中繼資料可以在附加元件清單（manifest）的可選部分聲明為 `symbolDictionaries` 欄位。
  * 詳情請參考[開發人員指南中的 Custom speech symbol dictionaries 章節](https://www.nvaccess.org/files/nvda/documentation/developerGuide.html#AddonSymbolDictionaries)。
* 支援使用 `NVDAObject.objectFromPointRedirect` 方法重新導向從螢幕座標檢索的物件。(#16788, @Emil-18)
* 執行 SCons 時使用參數 `--all-cores` 會自動選擇 CPU 的最大核心數。(#16943, #16868, @LeonarddeR)
* 目前導航物件的開發者資訊現在包括處理器的體系結構（例如 AMD64）。(#16488, @josephsl)

#### 棄用

* 不推薦使用在 2025.1 中即將被移除的 `bool` 配置鍵 `[documentFormatting][reportFontAttributes]`，請考慮使用 `[fontAttributeReporting]`。(#16748)
  * 新的配置鍵具有與 `OutputMode` `enum` 匹配的 `int` 值，其中包含語音、點字、語音和點字以及關閉選項。
  * API 調用者可以像以前一樣使用 `bool` 值，或者如果專門處理語音或點字，則檢查 `OutputMode`。
  * 這些配置鍵目前會同步至 2025.1。
...
* 新的按鍵指令：
  * 新增了未指派的快速鍵，用於垂直和水平滾動滑鼠滾輪，從而增強在具有動態內容的網頁和應用程式中的導航，例如 Dism++。(#16462, @Cary-Rowen)
...
* 元件更新：
  ...
  * Fast_diff_match_patch(用於偵測終端和其他動態內容的差異)已更新至 2.1.0 版本。(#16508, @codeofdusk)
  * LibLouis 點字翻譯器已更新至 [3.30.0](https://github.com/liblouis/liblouis/releases/tag/v3.30.0)。(#16652, @codeofdusk)
    * 新增點字表：
      * 塞爾維亞語西里爾字母。
      * 意第緒文。
      * 幾種古代語言：聖經希伯來文、阿卡德文、敘利亞文、烏加里特文，以及轉寫的楔形文字。
      * 土耳其文二級點字。(#16735)
      * 國際音標。(#16773)
  * NSIS 已更新至 3.10 版本。(#16674, @dpy013)
  * Markdown 已更新至 3.6 版本。(#16725, @dpy013)
  * Nh3 已更新至 0.2.17 版本。(#16725, @dpy013)
...
* Windows 11 修正：
  * 修正了關閉剪貼簿歷史記錄和表情符號面板時，NVDA 卡住的問題。(#16346, #16347, @josephsl)當關閉剪貼簿歷史記錄和表情符號面板時，NVDA 不再會顯示卡住的情況。
  * 修正了 NVDA 在輸入候選介面無法讀出可見候選字的錯誤。(#14023, @josephsl)
  * 修正了在流覽表情符號面板功能表項目時，NVDA 會重複朗讀“剪貼簿歷史記錄”的問題。(#16532, @josephsl)
  * 修正了在表情符號面板中檢視顏文字和符號時，NVDA 的語音和點字輸出被打斷的問題。(#16533, @josephsl)
* 網頁瀏覽器修正：
  * 修正了在 Google Chrome 和 Mozilla Firefox 中，未讀出使用 `aria-errormessage` 引用的錯誤訊息的問題。(#8318)
  * 現在 NVDA 會在 Mozilla Firefox 中使用 `aria-labelledby` 來作為表格的可存取名稱(如果存在)。(#5183)
...
* 新增了自訂點字表的功能。(#3304, #16208, @JulienCochuyt, @LeonarddeR)
  ...
  * 請參閱 [開發人員指南中的點字翻譯表部分](https://www.nvaccess.org/files/nvda/documentation/developerGuide.html#BrailleTables) 以取得更多詳細資訊。
* 當 `gainFocus` 事件排隊處理帶有有效 `focusRedirect` 屬性的物件時，現在由 `eventHandler.lastQueuedFocusObject` 持有 `focusRedirect` 屬性指向的物件，而不是最初排隊的物件。(#15843)
...
* 新增了一個用於排程任務的模組 utils.schedule，該模組使用了 pip 模組 schedule。(#16636)
  * 您可以使用 scheduleThread.scheduleDailyJobAtStartUp 來自動排程一個任務，該任務會在 NVDA 啟動後執行，並在之後每 24 小時執行一次。
  工作排程會有延遲，以避免衝突。
  * 可以使用 `scheduleThread.scheduleDailyJob` 和 `scheduleJob` 計畫自訂時間任務，如果任務計畫發生衝突將拋出 `JobClashError`。
...
* 新的按鍵指令：
  ...
  * 新增了在語音選項設定到第一個、最後一個、向前和向後的指令。(#13768, #16095, @rmcpantoja)
    ...
    * 以較大幅度增加或減小目前的語音選項設定(#13768)：
      * 桌上型電腦：`NVDA+control+pageUp` 及 `NVDA+control+pageDown`。
      * 筆記型電腦：`NVDA+control+shift+pageUp` 及 `NVDA+control+shift+pageDown`。
  * 新增了一個未指派的輸入手勢來切換讀出圖表及說明的開關。(#10826, #14349)
* 點字：
  * 新增了對 BrailleEdgeS2 和 BrailleEdgeS3 顯示器的支援。(#16033, #16279, @EdKweon)
...
* 附加元件商店：
  * 在“其他詳細資料”區域支援顯示附加元件的最低 NVDA 版本要求和最新通過測試的 NVDA 版本。(#15776, @Nael-Sayegh)
...
* 元件更新：
  * 將 LibLouis 點字轉譯表更新為 [3.29.0](https://github.com/liblouis/liblouis/releases/tag/v3.29.0)。(#16259, @codeofdusk)
    ...
    * 用於閱讀希臘文字的新西班牙文表。
    * 新寮國一級點字錶。(#16470)
  * eSpeak NG 已更新至 1.52-dev commit `cb62d93fd7`。 (#15913)
    * 新增了新語言提格利尼亞語。
* 更改了 BrailleSense 裝置的多個手勢，以避免和法文點字表的字元發生衝突。(#15306)
  * `alt+向左鍵`對應到 27 點+Space
...
* 附加元件商店：
  * 當按下 `ctrl+tab` 時，焦點會正確移動到新的索引標籤標題。(#14986, @ABuffEr)
  * 如果快取檔案不正確，NVDA 將不再重新啟動。(#16362, @nvdaes)
* 基於 Chromium 的瀏覽器搭配 UIA 使用的修正：
  * 修正了會導致 NVDA 沒有回應的錯誤。(#16393, #16394)
...
* 修正了命令列參數 `-h`/`--help` 無法使用的問題。(#16522, @XLTechie)
...
* 為不明 Windows 版本(高於 10.0.22000，例如 10.0.25398)產生實體 `winVersion.WinVersion` 對象時，返回“Windows 11 不明”而不是“Windows 10 不明”。(#15992, @josephsl)
...
* 組件更新：
  * LibLouis 點字翻譯器已更新至 [3.28.0](https://github.com/liblouis/liblouis/releases/tag/v3.28.0)。(#15435, @codeofdusk)。(#15435, #15876, @codeofdusk)
    * 新推出泰文、菲律賓文和羅馬尼亞文點字表。
...
* 附加元件商店：
  * 當附加元件的狀態在其被聚焦時發生變化，例如從「正在下載」變更為「已下載」，現在會正確地讀出更新後的項目狀態。(#15859, @LeonarddeR)
  * 安裝附加元件時，安裝提示不再被重新啟動對話框覆蓋。(#15613, @lukaszgo1)
  * 重新安裝不相容的附加元件時，不會再被強制停用。(#15584, @lukaszgo1)
  * 已停用或不相容的附加元件現在可以更新。(#15568, #15029)
  * 當附加元件下載失敗時，NVDA 現在會復原並顯示錯誤訊息。(#15796)
  * NVDA 在開啟和關閉附加元件商店後，不再間歇性地無法重新啟動。(#16019, @lukaszgo1)
* 音訊：
  * 當多個聲音快速連續播放時，NVDA 不再會短暫卡頓。(#15311, #15757, @jcsteh)
  * 如果音訊輸出裝置設定為非預設值，且該裝置在不可用後重新變為可用，NVDA 現在會切回已設定的裝置，而非繼續使用預設裝置。(#15759, @jcsteh)
  * 如果輸出裝置的設定變更或其他應用程式釋放了對裝置的獨占控制，NVDA 現在會恢復音訊播放。(#15758, #15775, @jcsteh)
* 點字：
  * 多行點字顯示器不再導致 BRLTTY 驅動程式崩潰，並會被視為一個連續的顯示器。(#15386)
  * 更多包含有用文字的物件會被檢測到，且其文字內容將顯示在點字上。(#15605)
  * 縮寫點字輸入現已恢復正常運作。(#15773, @aaclause)
  * 在更多情況下，當將導覽物件移動到表格儲存格之間時，點字會即時更新。(#15755, @Emil-18)
  * 讀出目前焦點、目前導航物件和目前選取內容的結果，現在會顯示在點字上。(#15844, @Emil-18)
  * 修正了 Albatross 點字驅動程式將 Esp32 微控制器誤識別為 Albatross 點顯器的錯誤。(#15671)
* LibreOffice：
  * 使用 Control+Backspace 鍵刪除單字時，若被刪除的單字後方有空白字元(如空格或 Tab)，現在也會正確朗讀。(#15436, @michaelweghorn)
  * 使用 NVDA+End 鍵報讀狀態列的功能，現在也適用於 LibreOffice 24.2 及更新版本的對話框。(#15591, @michaelweghorn)
  * 在 LibreOffice 24.2 及更新版本中，現已支援所有預期的文字屬性。
  這使得在 Writer 中報讀一行時，拼寫錯誤的報讀可以正常運作。 (#15648, @michaelweghorn)
  * 在 LibreOffice 24.2 及更新版本中，現在也能正確報讀標題層級。(#15881, @michaelweghorn)
* Microsoft Office：
  * 修正了在停用 UIA 的 Excel 中，當按下 `control+y`、`control+z` 或 `alt+backspace` 時，未朗讀操作結果且點字顯示未同時更新的錯誤。(#15547)
  * 修正了在停用 UIA 的 Word 中，當按下 `control+v`、`control+x`、`control+y`、`control+z`、`alt+backspace`、亦或按住 `backspace`、`control+backspace` 時，點字未更新的錯誤。
  當啟用了 UIA 並輸入文字時，且點字顯示與檢視同步，且檢視跟隨插入點時，也會進行更新。 (#3276)
  * 修正了在 Word 中，使用原生 Word 表格導航快速鍵 `alt+home`、`alt+end`、`alt+pageUp`、`alt+pageDown` 時，不讀儲存格座標的錯誤。(#15805, @CyrilleB79)
...
* 修正了對 Windows Forms 應用程式中系統清單檢視（`SysListView32`）控制項的支援。(#15283, @LeonarddeR)
...
* 修正了在 Firefox 或 Chrome 中使用 `escape` 或 `alt+upArrow` 關閉下拉式方塊後，NVDA 未切換回流覽模式的錯誤。(#15653)
...
關於 Visual Studio 元件的具體清單，請參閱 [NVDA 文件](https://github.com/nvaccess/nvda/blob/release-2024.1/projectDocs/dev/createDevEnvironment.md)
* Added the following extension points:
  * `treeInterceptorHandler.post_browseModeStateChange`. (#14969, @nvdaes)
  * `speech.speechCanceled`. (#15700, @LeonarddeR)
  * `_onErrorSoundRequested`(應該透過調用 `logHandler.getOnErrorSoundRequested()` 取得) (#15691, @CyrilleB79)
* 在附加元件的翻譯中現在可以使用複數形式。(#15661, @beqabeqa473)
* 在二進位包中包含了 python3.dll，供附加元件使用外部庫（利用[穩定 API](https://docs.python.org/3.11/c-api/stable.html)。(#15674, @mzanm)
...
* 新增了 `speech.speakSsml` 函數，您可以使用 [SSML](https://www.w3.org/TR/speech-synthesis11/) 編寫 NVDA 語音序列。(#15699, @LeonarddeR)
  * 目前支援將以下標籤轉換為相應的 NVDA 語音命令：
    * `Prosody` (`pitch`，`rate` 和 `volume`)。僅支援倍數的形式調節(例如 `200%`)。
    * 將 `say-as` 的 `interpret` 屬性設定為 `characters`
    * 將 `voice` 的 `xml:lang` 設定為 XML 的語言
    * 將 `break` 的 `time` 屬性設定為毫秒值，例如 `200ms`
    * 將 `mark` 的 `name` 屬性設定為標記的名稱，例如 `mark1`，需要提供回呼函數
  * 示例： `speech.speakSsml('<speak><prosody pitch="200%">hello</prosody><break time="500ms" /><prosody rate="50%">John</prosody></speak>')`
  * SSML 解析功能由 `speechXml` 模組中的 `SsmlParser` 類提供支援。
* 對 NVDA Controller Client 庫的改進：
  * 庫的檔案名稱不再包含表示架構的尾碼，即 `nvdaControllerClient32/64.dll` 現在稱為 `nvdaControllerClient.dll`。(#15718, #15717, @LeonarddeR)
