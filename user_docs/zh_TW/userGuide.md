...
* 遠端存取：連線並控制另一台執行 NVDA 的電腦，用於遠端協助或協作。
...
1. 執行下載的檔案，
您可能需要稍候幾秒鐘，等待 NVDA 的臨時副本載入。
載入後 NVDA 將會在接下來的過程中朗讀。
1. NVDA 啟動器視窗隨即出現，內含一個開啟授權合約的按鈕。
按方向鍵向下閱讀授權合約。
...
| 名稱 |桌電快速鍵 |筆電快速鍵 |說明|
|---|---|---|---|
|報讀書入字元 | NVDA+2 | NVDA+2 | 控制 NVDA 是否報讀您輸入的字元。 |
|報讀書入的字詞 | NVDA+3 | NVDA+3 | 控制 NVDA 是否報讀您輸入的字詞。 |
...
| 選項 | 行為 |
|---|---|
| Liblouis | 使用原生點字格式。請注意，此選項只會指示粗體、斜體和有底線的文字，而且僅在所選的點字表支援指示這些屬性時才會指示。 |
...
|`-n LANGUAGE` |`--lang=LANGUAGE` |覆蓋已配置的 NVDA 語言。使用 "Windows" 表示目前使用者預設值、"en" 表示英文等。|
