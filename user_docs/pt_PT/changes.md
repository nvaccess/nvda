...
Houve também uma série de correcções, incluindo o acompanhamento do rato no Firefox e o modo de voz a pedido.
...
Feitas várias correcções de erros, particularmente para o Painel de Emojis e o histórico da Área de Transferência no Windows 11.
Para navegadores web, há correcções para o anúncio de mensagens de erro, figuras, legendas, etiquetas de tabelas e itens de menu de caixas de verificação/botões de opção.
...
Esta é uma versão de correcção para resolver uma questão de segurança e outra questão na instalação.
...
Esta é uma versão de correcção para resolver uma questão de segurança.
A correcção de segurança em 2023.3.1 não foi resolvida corretamente.
Por favor, divulgue questões de segurança de forma responsável seguindo a [política de segurança do NVDA](https://github.com/nvaccess/nvda/blob/master/security.md).
...
Há muitas correcções de erros, particularmente para Braille, Microsoft Office, navegadores web e Windows 11.
...
* Braille:
  * Várias correcções de estabilidade na entrada/saída para linhas Braille, resultando em erros e travamentos menos frequentes do NVDA. (#14627)
...
* Navegadores Web:
  ...
  * No modo de navegação, o NVDA já não ignora, incorretamente, o movimento do foco para um controlo de nível superior ou inferior, por exemplo, movendo-se de um elemento para a lista ou grelha que o contém. (#14611)
    * No entanto, observe que esta correcção se aplica apenas quando a opção "Definir automaticamente o foco em elementos com foco" nas configurações do modo de navegação está desactivada (que é a configuração padrão).
* Correcções para Windows 11:
...
Há também várias correcções de erros.

O eSpeak e LibLouis foram actualizados.
...
* Desempenho e estabilidade melhorados no Microsoft Visual Studio, no Windows Terminal e noutras aplicações baseadas na UIA. (#11077, #11209)
  * Estas correcções aplicam-se ao Windows 11 Sun Valley 2 (versão 22H2) e posterior.
...
Existem várias correcções para a utilização de uma definição de DPI superior a 100% ou vários monitores.
Ainda podem existir problemas com versões do Windows anteriores ao Windows 10 1809.
Para que estas correcções funcionem, as aplicações com as quais o NVDA interage também têm de ser sensíveis ao DPI.
...
Esta versão também inclui várias correcções de erros.
 ...
 * `control+alt+home/end` para ir para a primeira/última coluna.
 * `control+alt+pageUp/pageDown` para saltar para a primeira/última linha.
...
Existem também várias correcções de erros do Windows 11, para aplicações como a Calculadora, a Consola, o Terminal, o Mail e o Painel de Emoji.
...
Uma correcção do UIA é o facto do Outlook ignorar mais tipos de tabelas de apresentação nas mensagens.
...
* a detecção de teclas modificadoras (como Control ou Insert) é mais robusto quando o watchdog está a recuperar. (#12609)
...
A ferramenta de correcção de COM Registration pode agora resolver mais problemas na utilização do NVDA.
Há actualizações para o sintetizador eSpeak e para o conversor Braille LibLouis.
Há também várias correcções de erros e melhorias, nomeadamente no suporte Braille e nos terminais Windows, calculadora, painel de emojis e histórico da área de transferência.
...
* A Ferramenta de correcção de Registo COM resolve agora mais problemas, especialmente no Windows de 64 bits. (#12560)
...
Além de muitas correcções de erros e melhorias, incluindo no Office, Visual Studio e em vários idiomas.
...
Além de muitas outras correcções de erros e melhorias importantes.
...
Os destaques desta versão incluem o suporte para vários novos dispositivos Braille da HumanWare e da APH, além de muitas outras correcções de erros importantes, como a capacidade de ler novamente matemática no Microsoft Word utilizando o MathPlayer / MathType.
...
Although these changes do break compatibility with older NVDA add-ons, the upgrade to Python 3 is necessary for security, and the changes to speech allow for  some exciting innovations in the near future.
 Outros destaques desta versão incluem o suporte de 64 bits para Java VMs, a funcionalidade Screen Curtain e Focus Highlight, o suporte para mais dispositivos Braille e um novo visualizador Braille, e muitas outras correcções de erros.
...
* Todas as Preferências do NVDA podem agora ser encontradas numa janela de Configurações, em Menu NVDA -> Preferências -> Configurações, em vez de estarem espalhadas por várias janelas. (#577)
...
* Adicionar a capacidade de ver o ficheiro de registo actual do NVDA. Pode ser encontrado no menu NVDA -> Ferramentas
...
* Alterar a voz na caixa de diálogo de definições de voz agora define os controlos deslizantes de velocidade, altura e volume para os novos valores de acordo com o sintetizador, em vez de forçar o sintetizador a ser definido para os valores antigos. Isto corrige problemas em que um sintetizador como o eloquence ou o viavoice parece falar a um ritmo muito mais rápido do que todos os outros sintetizadores.
