...
Esta versão introduz o Acesso Remoto NVDA, permitindo-lhe controlar um computador remoto com NVDA a partir de outro dispositivo com NVDA.

Existem muitas melhorias na voz, particularmente na capacidade de resposta com vozes SAPI 4, SAPI 5 e OneCore.
O aumento da taxa e a mudança automática de língua são agora suportados no SAPI 5.
As vozes SAPI 4 suportam agora a redução de áudio, o corte do silêncio principal e a manutenção do dispositivo de áudio acordado.

O sistema de atualização automática da Add-on Store foi melhorado, permitindo-lhe selecionar canais para actualizações automáticas e executar actualizações automáticas em segundo plano.

Foram adicionados novos comandos para atualizar manualmente um resultado de OCR e para alternar a atualização periódica dos resultados de OCR.

A seleção nativa está agora disponível no Chrome e no Edge.

O suporte ao Microsoft Office e LibreOffice foi melhorado, nomeadamente com o anúncio de mais atalhos.

As tabelas braille de entrada e saída predefinidas podem agora ser determinadas com base no idioma NVDA.
Foi adicionado o suporte nativo para o dispositivo Braille gráfico tátil Dot Pad da Dot Inc como um dispositivo Braille de várias linhas.
Houve outras pequenas correções para os dispositivos Braille, nomeadamente no que se refere à conetividade.

O conversor Braille LibLouis, o eSpeak-NG e o Unicode CLDR foram atualizados.
Estão disponíveis novas tabelas Braille, Copta, Cuneiformes compacto e Português Braille de computador de 6 pontos.
...
Divulgue de forma responsável os problemas de segurança de acordo com a [política de segurança] da NVDA (https://github.com/nvaccess/nvda/blob/master/security.md).

* Evita a apresentação de informações potencialmente sensíveis em dispositivos Braille quando o dispositivo está bloqueado.
([GHSA-8f8q-2jc3-6rf4](https://github.com/nvaccess/nvda/security/advisories/GHSA-8f8q-2jc3-6rf4))
* Impede que o instalador carregue DLLs indesejadas a partir do seu diretório.
([GHSA-qf5h-qw92-rx2f](https://github.com/nvaccess/nvda/security/advisories/GHSA-qf5h-qw92-rx2f))

### Novas funcionalidades

* Loja de Extras:
  * Atualizações automáticas (#3208):
    * Os canais de atualização automática para extras podem agora ser modificados.
      * Os canais de atualização automática podem ser selecionados para os extras instalados através de um submenu "Canal de atualização".
      * O canal de atualização automática predefinido pode ser configurado nas Configurações do NVDA, Loja de extras.
    * As atualizações automáticas podem agora ser feitas em segundo plano.
      * Isto pode ser ativado nas Configurações do NVDA, Loja de extras, alterando "Atualizações automáticas" para "Atualizar automaticamente".
    * As atualizações automáticas podem agora atualizar extras incompatíveis para outra versão mais recente, mesmo que também incompatível.
      * Isto pode ser ativado nas Configurações do NVDA, Loja de extras.
  * Adicionada uma ação para cancelar a instalação de extras. (#15578, @hwf1324)
  * Adicionada uma ação para tentar novamente a instalação, se a transferência/instalação de um extra falhar. (#17090, @hwf1324)
  * As listas de extras podem ser ordenadas por colunas, incluindo a data de publicação, por ordem ascendente e descendente. (#15277, #16681, @nvdaes)
* Voz:
  * A alteração automática de idioma é agora suportada quando se utiliza a API de voz da Microsoft versão 5 (SAPI5) e Vozes Windows OneCore. (#17146, @gexgd0419)
  * O aumento da velocidade é agora suportado quando se utiliza a API de Discurso da Microsoft versão 5 (SAPI5) e as Vozes Windows OneCore, que suporta uma velocidade até 6X. (#17606, @gexgd0419)
* Navegadores Web:
  * O modo de seleção nativo do modo de navegação do NVDA (`NVDA+shift+f10`) é agora suportado no Google Chrome, Edge e outras aplicações baseadas no Chromium 134 ou mais recente. (#17838)
  * No Mozilla Firefox, o NVDA irá reportar o texto destacado quando um URL contendo um fragmento de texto for visitado. (#16910, @jcsteh)
  * O NVDA agora pode informar quando um destino de link aponta para a página atual. (#141, @LeonarddeR, @nvdaes)
* Microsoft Office:
  * Ao editar em caixas de texto do PowerPoint, agora é possível mover por frase com `alt+seta para cima`/`alt+seta para baixo`. (#17015, @LeonarddeR)
  * No Word, a atualização da seleção agora é relatada ao usar comandos do Word para estender ou reduzir a seleção (`f8` ou `shift+f8`). (#3293, @CyrilleB79)
  * No Word 16.0.18226 e superiores, ou ao usar o modelo de objeto do Word, o NVDA agora anunciará, em voz e Braille, se um cabeçalho está recolhido. (#17499)
  * No Word com UIA, ao usar os comandos nativos de navegação de tabela estendida (`alt+home`, `alt+end`, `alt+pageUp` e `alt+pageDown`), o movimento do cursor agora será anunciado. (#17867, @CyrilleB79)
  * No Word, ao utilizar o comando "Anunciar objeto em foco", o esquema do documento será anunciado se essa informação estiver disponível e "Anunciar descrições de objecto" estiver ativada. (#15088, @nvdaes)
  * O resultado de mais atalhos é agora comunicado no Word e no Outlook:
    * atalhos para formatação de fontes (#10271, @CyrilleB79)
    * recolher ou expandir o cabeçalho (#17545, @CyrilleB79)
* LibreOffice:
  * Ao aumentar ou diminuir o tamanho da fonte no Writer usando os atalhos de teclado correspondentes, o NVDA anuncia o novo tamanho da fonte. (#6915, @michaelweghorn)
  * Ao aplicar o estilo de parágrafo "Corpo do texto" ou um título usando o atalho de teclado correspondente no Writer 25.2 ou mais recente, o NVDA anuncia o novo estilo de parágrafo. (#6915, @michaelweghorn)
  * Ao alternar o sublinhado duplo no Writer usando o atalho de teclado correspondente, o NVDA anuncia o novo estado ("sublinhado duplo ligado"/"sublinhado duplo desligado"). (#6915, @michaelweghorn)
  * O NVDA agora pode informar o recuo da primeira linha no LibreOffice 25.8 e posterior. (#13052, @michaelweghorn)
* OCR:
  * Num resultado de reconhecimento, `NVDA+f5` atualiza manualmente o conteúdo reconhecido. (#17715, @CyrilleB79)
  * Adicionado um comando não atribuído para ativar a atualização periódica do resultado do OCR do Windows. (#16897)
* A funcionalidade de acesso remoto, baseada no complemento NVDA Remote, foi integrada no NVDA. (#4390, @ctoth, @tspivey, @daiverd, colaboradores e financiadores do NVDA Remote)
* Agora é possível especificar URLs espelho para usar nas atualizações do NVDA e na Loja de extras. (#14974, #17151, #17310, @christopherpross)
* Foi adicionado o suporte para matemática em PDFs. (#9288, @NSoiffer)
  * Isto funciona para fórmulas com MathML associado, como alguns ficheiros gerados por versões mais recentes do TeX/LaTeX.
  * Atualmente, isto só é suportado no Foxit Reader e no Foxit Editor.
* O NVDA pode agora ser configurado para falar a linha ou parágrafo atual ao navegar com teclas de navegação em Braille. (#17053, @nvdaes)
* O NVDA agora é capaz de anunciar alterações no cursor ao pressionar os comandos `alt+seta para cima` ou `alt+seta para baixo`, por exemplo, no Visual Studio Code. (#17652, @LeonarddeR)
* Adicionados comandos para mover o cursor de revisão para o primeiro e último carácter do texto selecionado, atribuídos a `NVDA+alt+home` e `NVDA+alt+end`, respetivamente. (#17299, @nvdaes)
* Foi adicionada uma opção geral para evitar que o ecrã se desligue durante a leitura de "Ler tudo" ou a leitura em Braille.
Esta opção está ativada por predefinição, mas pode resultar num aumento do consumo da bateria. (#17649, @LeonarddeR)
* O NVDA está agora traduzido para bósnio. (#17953)
* No Adobe Acrobat, o NVDA pode agora ler e interagir com equações matemáticas em documentos PDF gerados por versões recentes do Microsoft Word. (#18056)

### Alterações

* Actualização de Componentes:
  * Atualizado o conversor Braille da LibLouis para [3.33.0](https://github.com/liblouis/liblouis/releases/tag/v3.33.0). (#17469, #17768, @LeonarddeR, @codeofdusk)
    * Adicionadas novas tabelas Copta, Cuneiformes compacta e Português Braille de computador de 6 pontos.
  * Atualização do CLDR para a versão 46.0 (#17484, @OzancanKaratas)
  * Atualizado eSpeak NG para 1.52.0. (#17056)
* Alterações na interface do NVDA:
  * O instalador do NVDA já não reproduz um som quando é iniciado. (#14068)
  * As caixas de diálogo "Destino da ligação do relatório", "Informações de formatação de caracteres" e "Seleção de orador" incluem agora os botões "Fechar" e "Copiar" para conveniência do utilizador. (#17018, @XLTechie)
  * O diálogo de saída permite agora reiniciar o NVDA com os extras desativados e o registo de depuração ativado simultaneamente. (#11538, @CyrilleB79)
  * A opção de optar por não usar WASAPI para saída de áudio foi removida. (#16080)
  * A opção do Modo de Navegação "Definir automaticamente o foco do sistema para elementos focáveis" foi removida, o comportamento está agora desativado. (#17598)898
  * O NVDA agora só avisará sobre a incompatibilidade de complementos quando atualizar para uma nova versão que tenha uma API de complemento incompatível com a versão atual. (#17071, #17506)
  * O ficheiro "Contribuidores" foi removido do menu NVDA. (#16922)
  * A licença NVDA está agora formatada em HTML para melhorar a legibilidade. (#17600)
  * Versões curtas das opções de linha de comando mais usadas foram adicionadas: `-d` para `--disable-addons` e `-n` para `--lang`.
  Combinação de prefixos em sinalizadores de linha de comando, por exemplo, usando `--di` para `--disable-addons` não é mais suportado. (#11644, @CyrilleB79)
  * Alterações à ferramenta de correção do registo COM: (#12355, @XLTechie)
    * Agora começa com uma explicação mais amigável do seu objetivo, em vez de um aviso. (#12351)
    * A janela inicial pode agora ser encerrada com `escape` ou `alt+f4`. (#10799)
    * Agora, mostrará uma mensagem ao utilizador, incluindo o erro, no caso raro de um erro do Windows ao tentar efetuar novos registos de COM.
* Voz:
  * A API de voz da Microsoft versão 5 e as vozes da Plataforma de voz da Microsoft utilizam agora a WASAPI para a saída de áudio, o que pode melhorar a capacidade de resposta dessas vozes. (#13284, @gexgd0419)
  * O silêncio no início do discurso será agora cortado quando se utilizam vozes OneCore, vozes SAPI5 e alguns add-ons de voz de terceiros para melhorar a sua capacidade de resposta. (#17614, @gexgd0419)
  * As vozes da API de voz da Microsoft versão 4 utilizam agora a WASAPI para a saída de áudio, de modo a poderem trabalhar com funcionalidades como a redução de áudio, o corte do silêncio inicial e a manutenção do dispositivo de áudio acordado.
  Se isto não funcionar com a sua voz SAPI 4, pode desativar o WASAPI para SAPI 4 nas definições avançadas. (#17718, #17801, @gexgd0419)
  * A Normalização Unicode está agora ativada por predefinição para a saída de voz. (#17017, @LeonarddeR).
    * Pode ainda desativar esta funcionalidade na categoria Voz das Configurações do NVDA.
* As opções do teclado para "Anunciar caracteres digitados" e "Anunciar palavras digitadas" têm agora três opções: Desligado, Apenas em campos de edição e Sempre. (#17505, @Cary-rowen)
  * Por predefinição, "Anunciar caracteres digitados" está agora definido para "Apenas em campos de edição".
* As tabelas Braille de leitura e escrita predefinidas podem agora ser determinadas com base no idioma do NVDA. (#17306, #16390, #290, @nvdaes)

### Correção de erros

* Voz:
  * Foi corrigido um problema em que a leitura contínua (Ler tudo) parava no fim da primeira frase quando se usavam alguns sintetizadores SAPI5. (#16691, @gexgd0419)
  * Os parâmetros de voz, tais como a velocidade e o volume, já não serão repostos para o valor predefinido quando se utiliza o anel de definições do sintetizador para mudar entre vozes no sintetizador SAPI5 e SAPI4. (#17693, #2320, @gexgd0419)
  * Foi corrigido um problema em que algumas vozes SAPI4 (por exemplo, IBM TTS Chinese) não podiam ser carregadas. (#17726, @gexgd0419)
* Braille:
  * Suporte nativo para o dispositivo gráfico tátil Dot Pad da Dot Inc como um dispositivo Braille de várias linhas. (#17007)
  * Ao usar o Seika Notetaker, os gestos de espaço e espaço com pontos são agora apresentados corretamente na caixa de diálogo Definir comandos. (#17047, @school510587)
  * O NVDA já não deve reproduzir sons de erro ou enviar lixo para o ficheiro de log ao usar o modo Braille "Apresentar saída de voz" sem nenhum dispositivo Braille ligado. (#17092, @Emil-18)
  * Quando o "Dispositivos Braille com protocolo HID" é explicitamente selecionado como o dispositivo Braille e a lista de dispositivos Braille é aberta, o NVDA identifica corretamente o dispositivo HID como o dispositivo selecionado em vez de mostrar nenhum dispositivo selecionado. (#17537, @LeonarddeR)
  * O driver para o dispositivo Humanware Brailliant é agora mais fiável na seleção do ponto de ligação correto, resultando em maior estabilidade da ligação e menos erros. (#17537, @LeonarddeR)
  * As tabelas Braille personalizadas no ScratchPad do programador são agora corretamente ignoradas quando o NVDA é executado com os extras desativados. (#17565, @LeonarddeR)
  * Foi corrigido um problema em que alguns dispositivos Braille USB não eram detetados corretamente pelo NVDA. (#18114, @christiancomaschi)
* Microsoft Office:
  * O comando para "Anunciar o URL de destino de um link" funciona agora como esperado quando se utiliza o modelo de objeto herdado no Word, Outlook, Excel e PowerPoint. (#17292, #17362, #17435, @CyrilleB79)
  * No Excel, a caixa de diálogo de lista de elementos (`NVDA+f7`) não falha mais em listar comentários ou fórmulas em alguns sistemas não ingleses. (#11366, @CyrilleB79)
  * Melhorias no PowerPoint:
    * O anúncio do cursor já não é interrompido quando o texto contém caracteres largos, tais como emoji. (#17006 , @LeonarddeR)
    * O anúncio de localização de caracteres já é preciso (por exemplo, ao pressionar `NVDA+delete`. (#9941, @LeonarddeR)
    * O NVDA já não começa a ler tudo ao iniciar uma apresentação de diapositivos e a definição do modo de navegação "Ler tudo automaticamente ao carregar a página" está desativada. (#17488, @LeonarddeR)
* LibreOffice:
  * Ao usar o método de entrada Pinyin da Microsoft para chinês e ativar a opção de compatibilidade Pinyin para usar a versão anterior, digitar no Writer (e potencialmente em outros aplicativos) enquanto um pop-up IME estiver sendo exibido não aciona mais um erro. (#17198, @michaelweghorn)
  * O estado atual das caixas de verificação (marcado/desmarcado) nas caixas de diálogo é agora também comunicado em braille e não apenas em voz. (#17218, @michaelweghorn)
* Matemática:
  * A leitura matemática foi corrigida para alguns elementos da Web.
  Especificamente, MathML dentro de span e outros elementos que têm o atributo `role="math"`. (#15058)
  * As equações matemáticas representadas apenas por uma imagem e texto alternativo sem MathML para navegação rica, são agora tratadas como imagens normais, em vez de matemática sem conteúdo, permitindo ao utilizador saltar para elas com `g` e ser capaz de percorrer o texto alternativo por carácter. (#16007)
* IDEs:
  * O NVDA já não irá falhar ao selecionar todo o texto em determinados ficheiros de origem no Android Studio ou no IntelliJ Idea. (#17418, @thgcode)
  * No Visual Studio Code, o NVDA não sequestra mais os gestos `alt+seta para cima` e `alt+seta para baixo` para navegação de frases. (#17082, @LeonarddeR)
  * Alguns elementos de seção são agora corretamente reconhecidos como controlos editáveis no Visual Studio Code. (#17573, @Cary-rowen)
  * No Notepad e noutros documentos UIA, bem como nos documentos Notepad++ no Windows 11, se a última linha estiver vazia, o "comando braille de linha seguinte" deslocará o cursor para a última linha.
  Em qualquer documento, se o cursor estiver na última linha, será movido para o fim quando utilizar este comando. (#17251, #17430, @nvdaes)
* Perfis de configuração:
  * O Braille já não é disfuncional quando se ativa "Ler tudo" com um perfil de configuração associado. (#17163, @LeonarddeR)
  * Foi corrigido um problema em que determinadas definições eram explicitamente guardadas no perfil de configuração ativo, mesmo quando o valor dessa definição era igual ao valor na configuração de base. (#17157, @leonarddeR)
* Quando se escreve, a normalização unicode funciona agora de forma mais adequada:
  * Depois de reportar um carácter normalizado, o NVDA já não reporta incorretamente os caracteres subsequentes como normalizados. (#17286, @LeonarddeR)
  * Os caracteres compostos (como o é) são agora reportados corretamente. (#17295, @LeonarddeR)
* O NVDA é capaz de ler os itens do submenu popup na página de resultados de pesquisa do Thunderbird. (#4708, @thgcode)
* A Ferramenta de Correção do Registo COM já não reporta sucesso em caso de falha. (#12355, @XLTechie)
* O NVDA não anunciará mais as entradas do histórico da área de transferência do Windows 11 ao fechar a janela enquanto os itens estiverem presentes. (#17308, @josephsl)
* Se os extras forem recarregados enquanto uma mensagem navegável estiver aberta, o NVDA não falhará mais ao relatar movimentos de foco subsequentes. (#17323, @CyrilleB79)
* Ao utilizar aplicações como o Skype, Discord, Signal e Phone Link para comunicação áudio, a voz e os sons do NVDA já não diminuem de volume. (#17349, @jcsteh)
* Abrir a Consola Python do NVDA não falhará mais no caso de ocorrer um erro ao recuperar variáveis de instantâneo. (#17391, @CyrilleB79)
* Nos navegadores Web, o anúncio das alterações à seleção de texto já não falham ocasionalmente nos controlos de texto editáveis. (#17501, @jcsteh)
* Quando os links de âncora apontam para o mesmo objeto em que o cursor virtual é colocado, o NVDA não falha mais ao rolar para o destino do link. (#17669, @nvdaes)
* O ícone da Janela do Realce visual do NVDA não é mais fixado na barra de tarefas após reiniciar o Explorer. (#17696, @hwf1324)

### Alterações para Desenvolvedores

Consulte [o guia do programador](https://www.nvaccess.org/files/nvda/documentation/developerGuide.html#API) para obter informações sobre o processo de descontinuação e remoção da API do NVDA.

* Nota: esta é uma versão que quebra a compatibilidade da API dos extras.
Os extras terão de ser novamente testados e o seu manifesto atualizado.
* Atualização de Componentes:
  * Atualizado o Ruff para 0.8.1. (#17102, #17260, #17473)
  * Atualizado o Comtypes para 1.4.6. (#17061, @LeonarddeR)
  * Atualizado o wxPython para 4.2.2 (#17181, @dpy013)
  * Atualização do SCons para 4.8.1. (#17254)
  * Atualizado o sphinx para 8.1.2 e o sphinx-rtd-theme para 3.0.1. (#17284, @josephsl)
  * Atualizado o Robot Framework para 7.1.1. (#17329, @josephsl)
  * Atualizado configobj para 5.1.0 commit `8be5462`. (#17328)
  * Pré-commit atualizado para 4.0.1. (#17260)
  * Atualização de typing-extensions para 4.12.2 (#17438, @josephsl)
  * Atualizado o licensecheck para 2024.3. (#17440, @josephsl)
  * Atualização do markdown para 3.7. (#17459, @josephsl)
  * Atualizado nh3 0.2.19. (#17465, @josephsl)
  * Atualização do nuitka para a versão 2.5.4 (#17458, @josephsl)
  * Calendário atualizado para 1.2.2. (#17455, @josephsl)
  * Requests actualizado para 2.32.3. (#17456, @josephsl)
* `ui.browseableMessage` pode agora ser chamado com opções para apresentar um botão para copiar para a área de transferência, e/ou um botão para fechar a janela. (#17018, @XLTechie)
* Várias adições para identificar tipos de links (#16994, @LeonarddeR, @nvdaes)
  * Um novo módulo `utils.urlUtils` com diferentes funções para determinar tipos de links
  * Um novo estado `INTERNAL_LINK` foi adicionado a `controlTypes.states.State`
  * Foi adicionada uma nova propriedade `linkType` em `NVDAObject`.
  Ele consulta o `treeInterceptor` por padrão, se houver.
  * o objeto `BrowseModeTreeInterceptor` tem uma nova propriedade `documentUrl`
  * o objeto `BrowseModeTreeInterceptor` tem um novo método `getLinkTypeInDocument` que aceita um URL para verificar o tipo de ligação do objeto
  * Uma função auxiliar `toggleBooleanValue` foi adicionada ao `globalCommands`.
  Pode ser usada em scripts para relatar o resultado quando um booleano é alternado em `config.conf`
* Removido o requisito de indentação de listas de parâmetros de funções por duas tabulações dos Padrões de Codificação do NVDA, para ser compatível com o linting automático moderno. (#17126, @XLTechie)
* Adicionada a [configuração do espaço de trabalho do VS Code para NVDA](https://nvaccess.org/nvaccess/vscode-nvda) como um submódulo git. (#17003)
* Uma nova função, `gui.guiHelper.wxCallOnMain`, foi adicionada, o que permite de forma segura e síncrona chamar funções wx de threads não-GUI, e obter o seu valor de retorno. (#17304)
* Uma nova API de diálogo de mensagem foi adicionada ao `gui.message`. (#13007)
  * Classes adicionadas: `ReturnCode`, `EscapeCode`, `DialogType`, `Button`, `DefaultButton`, `DefaultButtonSet`, `MessageDialog`.
* No módulo `brailleTables`, uma função `getDefaultTableForCurrentLang` foi adicionada (#17222, @nvdaes)
* Adicionada uma classe de dados `updateCheck.UpdateInfo`, que encapsula metadados sobre as atualizações do NVDA. (#17310, @christopherpross)
* A recuperação da propriedade `labeledBy` agora funciona para:
  * em aplicações que implementam a relação `labelled-by` IAccessible2. (#17436, @michaelweghorn)
  * Elementos UIA que suportam a propriedade UIA `LabeledBy` correspondente. (#17442, @michaelweghorn)
* Adicionada a capacidade de associar `wx.ComboBox` e um rótulo `wx.StaticText` usando `gui.guiHelper.associateElements`. (#17476)
* Adicionados os seguintes pontos de extensão (#17428, @ctoth):
  * `inputCore.decide_handleRawKey`: chamado a cada pressionamento de tecla
  * `speech.extensions.post_speechPaused`: chamado quando o discurso é pausado ou não pausado
* Alterações ao registo da deteção automática do ecrã braille em `bdDetect.DriverRegistrar`: (#17521, @LeonarddeR)
  * Adicionado o método `addUsbDevice` para registar um dispositivo USB de cada vez.
  * Adicionado o parâmetro `matchFunc` para `addUsbDevices` que também está disponível em `addUsbDevice`.
    * Desta forma, a deteção de dispositivos pode ser ainda mais limitada nos casos em que uma combinação VID/PID é partilhada por vários dispositivos em vários controladores, ou quando um dispositivo HID oferece vários end points, por exemplo.
    * Consulte a documentação do método, bem como exemplos nos drivers albatross e brailliantB para obter mais informações.
* Adicionada uma nova função, `utils.mmdevice.getOutputDevices`, para enumerar os dispositivos de saída de áudio. (#17678)
* Adicionado um novo ponto de extensão `pre_synthSpeak` em `synthDriverHandler`, que será chamado antes do gerenciador de fala chamar `speak` do sintetizador atual. (#17648)
* O NVDA suporta o atributo de objeto IAccessible2 `text-indent`. (#13052, @michaelweghorn)
* Quando um script que utiliza `gesture.send` é ativado com um gesto que inclui `numLock` como modificador, o Num Lock já não é desativado. (#10827, @CyrilleB79)

#### Alterações de quebra da API

Estas são alterações de quebra da API.
Abra um problema no GitHub se o seu add-on tiver um problema com a atualização para a nova API.

* A constante `addonStore.network.BASE_URL` foi removida.
Como o URL base da Loja de Complementos é agora configurável diretamente no NVDA, não está planeada qualquer substituição. (#17099)
* A constante `updateCheck.CHECK_URL` foi removida.
Como o URL de verificação de atualização do NVDA é agora configurável diretamente no NVDA, não está planeada qualquer substituição. (#17151)
* o `NVDAObjects.UIA.winConsoleUIA.WinTerminalUIA` foi removido sem nenhum substituto público. (#14047, #16820, @codeofdusk)
* o `NVDAObjects.IAccessible.ia2TextMozilla.FakeEmbeddingTextInfo` foi removido. (#16768, @jcsteh)
* Os seguintes símbolos em `appModules.soffice` foram renomeados (#6915, @michaelweghorn):
  * `SymphonyDocument.announceToolbarButtonToggle` para `SymphonyDocument.announceFormattingGestureChange`
  * `SymphonyDocument.script_toggleTextAttribute` para `SymphonyDocument.script_changeTextFormatting`
* O argumento `space` da palavra-chave `brailleDisplayDrivers.seikantk.InputGesture` agora espera um `int` ao invés de um `bool`. (#17047, @school510587)
* A seção de configuração `[upgrade]` incluindo `[upgrade][newLaptopKeyboardLayout]` foi removida. (#17191)
* `updateCheck.checkForUpdate` agora retorna um objeto `UpdateInfo` ao invés de um dicionário. (#17310, @christopherpross)
* Os construtores de `updateCheck.UpdateResultDialog` e `updateCheck.UpdateDownloader` foram atualizados para receber objetos `UpdateInfo` ao invés de dicionários de metadados. (#17310, @christopherpross)
* Devido à retirada do suporte winmm da NVDA (#17496, #17532, #17678):
  * Os seguintes símbolos foram removidos do `nvwave` sem substituições: `CALLBACK_EVENT`, `CALLBACK_FUNCTION`, `CALLBACK_NULL`, `HWAVEOUT`, `LPHWAVEOUT`, `LPWAVEFORMATEX`, `LPWAVEHDR`, `MAXPNAMELEN`, `MMSYSERR_NOERROR`, `usingWasapiWavePlayer`, `WAVEHDR`, `WAVEOUTCAPS`, `waveOutProc`, `WAVE_MAPPER`, `WHDR_DONE`, `WinmmWavePlayer` e `winmm`.
  * Os seguintes símbolos foram removidos do `nvwave`: `getOutputDeviceNames`, `outputDeviceIDToName`, `outputDeviceNameToID`.
  Utilize `utils.mmdevice.getOutputDevices` em seu lugar.
  * `nvwave.WasapiWavePlayer` foi renomeado para `WavePlayer`.
  Além disso, a assinatura do método de seu `__init__` foi alterada da seguinte forma:
    * O parâmetro `outputDevice` agora só deve receber argumentos do tipo string.
    * Os parâmetros obsoletos `closeWhenIdle` e `buffered` foram removidos.
  * o `gui.settingsDialogs.AdvancedPanelControls.wasapiComboBox` foi removido.
  * A chave `WASAPI` foi removida da secção `audio` da especificação de configuração.
  * A chave de configuração `config.conf["speech"]["outputDevice"]` foi removida.
    Ele foi substituído por `config.conf["audio"]["outputDevice"]`, que armazena um ID de dispositivo de endpoint de áudio do núcleo do Windows. (#17547)
* Em `NVDAObjects.window.scintilla.ScintillaTextInfo`, se nenhum texto for selecionado, o método `collapse` é substituído para expandir a linha se o parâmetro `end` for definido como `True` (#17431, @nvdaes)
* Os seguintes símbolos foram removidos sem substituição: `languageHandler.getLanguageCliArgs`, `__main__.quitGroup` e `__main__.installGroup` . (#17486, @CyrilleB79)
* Combinação de prefixos em sinalizadores de linha de comando, por exemplo, usando `--di` para `--disable-addons` não é mais suportado. (#11644, @CyrilleB79)
* O argumento `useAsFallBack` da palavra-chave `bdDetect.DriverRegistrar` foi renomeado para `useAsFallback`. (#17521, @LeonarddeR)
* A definição de configuração `[addonStore][showWarning]` foi removida.
Em vez disso, use `addonStore.dataManager.addonDataManager.storeSettings.showWarning`. (#17597)
* `ui.browseableMessage` agora recebe um parâmetro `sanitizeHtmlFunc`.
O padrão é `nh3.clean` com argumentos padrão.
Isso significa que qualquer HTML passado para `ui.browseableMessage` usando `isHtml=True` é agora higienizado por padrão.
Para alterar as regras de sanitização, tais como a lista branca de tags ou atributos, crie uma função que chame `nh3.clean` com os parâmetros desejados. (#16985)
* o `updateCheck.UpdateAskInstallDialog` já não executa automaticamente uma ação quando os botões de atualização ou adiamento são premidos.
Em vez disso, foi adicionada uma propriedade `callback`, que devolve uma função que executa a ação apropriada quando chamada com o valor de retorno da caixa de diálogo. (#17582)
* Os diálogos abertos com `gui.runScriptModalDialog` são agora reconhecidos como modais pelo NVDA. (#17582)
* Os seguintes símbolos API relacionados com a definição "Definir automaticamente a focagem do sistema para elementos focáveis" foram removidos sem qualquer substituição: (#17598)
...
* Porque as vozes SAPI5 agora usam `nvwave.WavePlayer` para emitir áudio: (#17592, @gexgd0419)
  * o ficheiro `synthDrivers.sapi5.SPAudioState` foi removido.
  * o `synthDrivers.sapi5.SynthDriver.ttsAudioStream` foi removido.
* As instâncias de `autoSettingsUtils.driverSetting.DriverSetting` com um `id` que começa com um sublinhado (_) já não são mostradas nas definições do NVDA. (#17599)
* Alterada a configuração do eco de digitação do teclado de valores booleanos para inteiros. (#17505, @Cary-rowen)
  * `config.conf["keyboard"]["speakTypedCharacters"]` e `config.conf["keyboard"]["speakTypedWords"]` agora usam valores inteiros.
  * Adicionado o enum `TypingEcho` em `config.configFlags` para representar estes modos, 0=Off, 1=Somente em controlos de edição, 2=Sempre.
  * `gui.settingsDialogs.KeyboardSettingsPanel.wordsCheckBox` e `gui.settingsDialogs.KeyboardSettingsPanel.charsCheckBox` foram removidos.
* O `winUser.paint` foi renomeado de `painStruct` para `paintStruct`, corrigindo um erro onde a passagem de um `PAINTSTRUCT` levantava uma exceção. (#17744)
* `documentationUtils.getDocFilePath` e `installer.getDocFilePath` não procuram mais por arquivos `.txt` em pastas de documentação de localidades. (#17911, @CyrilleB79)
* o `config.conf["documentFormatting"]["reportFontAttributes"]` foi removido; em vez disso, use o `config.conf["documentFormatting"]["fontAttributeReporting"]`. (#18066)
* o `config.conf["speech"]["includeCLDR"]` foi removido, verifique/modifique se o `config.conf["speech"]["symbolDictionaries"]` contém `"cldr"` em seu lugar. (#18066)

#### Obsoletos

* O ponto de extensão `braille.filter_displaySize` está obsoleto.
Por favor, utilize `braille.filter_displayDimensions` em vez disso. (#17011)
* As funções `gui.message.messageBox` e `gui.runScriptModalDialog`, e a classe `gui.nvdaControls.MessageDialog` estão obsoletas.
Utilize `gui.message.MessageDialog` em vez disso. (#17582)
* Os seguintes símbolos foram descontinuados (#17486, @CyrilleB79):
  * `NoConsoleOptionParser`, `stringToBool`, `stringToLang` em `__main__`; utilize os mesmos símbolos em `argsParsing`.
  * `__main__.parser`; use `argsParsing.getParser()` em seu lugar.
* o `bdDetect.DeviceType` foi descontinuado a favor do `bdDetect.ProtocolType` e do `bdDetect.CommunicationType` para ter em conta o facto de que tanto a comunicação HID como a de série podem ter lugar através de USB e Bluetooth. (#17537 , @LeonarddeR)

## 2024.4.2

Esta é uma versão de correção para corrigir erros com dispositivos braille e leitura matemática no Chromium.

### Correcções de erros

* Corrigido o erro com a leitura de matemática nos navegadores Chromium (Chrome, Edge). (#17421, @NSoiffer)
* Os dispositivos Humanware Brailliant BI 40X com firmware versão 2.4 agora funcionam como esperado. (#17518, @bramd)

## 2024.4.1

Esta é uma versão de correção para corrigir um erro ao guardar dicionários de símbolos de voz.

### Correcções de erros

* Corrigido o erro em que os dicionários de símbolos de voz não eram guardados e a caixa de diálogo não era fechada. (#17344)

## 2024.4

Esta versão inclui uma série de melhorias no Microsoft Office, Braille e formatação de documentos.

No Word ou no Excel, é agora possível pressionar duas vezes o comando de comentário para ler o comentário ou a nota numa mensagem navegável.
Já pode utilizar os comandos de selecção do cursor de revisão para seleccionar texto no PowerPoint.
O NVDA também já não apresenta caracteres Braille estranhos ao mostrar texto de cabeçalho de linha ou coluna em tabelas no Word quando utiliza o modelo de objeto.

O NVDA já pode ser configurado para anunciar, separadamente, os atributos do tipo de letra em voz e Braille.

Foi adicionada uma nova opção para configurar o tempo de espera para efectuar um comando de múltiplos pressionamentos, como o comando de anúncio da hora/data.

Agora pode configurar a forma como o NVDA mostra a formatação de texto em Braille e configurar o NVDA para mostrar o início dos parágrafos em Braille.
O NVDA já pode anunciar o carácter no cursor quando executa uma acção de encaminhamento do cursor Braille.
A fiabilidade do encaminhamento do cursor foi melhorada e foi adicionado o suporte para teclas de encaminhamento no PowerPoint.
Todas as linhas de células passam a ser utilizadas quando se utiliza um dispositivo Braille de várias linhas através de HID Braille.
O NVDA já não fica instável após reiniciar o NVDA durante uma pesquisa automática de dispositivos Braille via Bluetooth.

A versão mínima necessária do Poedit para funcionar corretamente com o NVDA é agora a versão 3.5.

eSpeak NG actualizado, adicionando suporte para as línguas faroense e xextana.

LibLouis actualizado, acrescentando novas tabelas Braille para o Braille internacional tailandês e grego com letras acentuadas de célula única.

Houve também uma série de correções, incluindo o acompanhamento do rato no Firefox e o modo de voz a pedido.

### Novas funcionalidades

* Novas funcionalidades Braille:
  * Já é possível alterar a forma como o NVDA apresenta determinados atributos de formatação de texto em Braille.
    As opções disponíveis são:
    * Liblouis (predefinição): Utiliza os marcadores de formatação definidos na tabela Braille seleccionada.
    * Etiquetas: Utiliza etiquetas de início e fim para indicar onde começam e terminam certos atributos do tipo de letra. (#16864)
  * Quando a opção "Ler por parágrafo" está activada, o NVDA pode agora ser configurado para indicar o início dos parágrafos em Braille. (#16895, @nvdaes)
  * Ao executar uma acção de encaminhamento do cursor Braille, o NVDA pode agora falar automaticamente o carácter no cursor. (#8072, @LeonarddeR)
    * Esta opção está desactivada por padrão.
      Pode activar a opção "Falar o caracter quando o cursor é encaminhado para o texto" nas configurações Braille do NVDA.
* O comando de comentário no Microsoft Word e o de notas no Microsoft Excel podem agora ser pressionados duas vezes para mostrar o comentário ou a nota numa mensagem navegável. (#16800, #16878, @Cary-Rowen)
* O NVDA agora pode ser configurado para relatar atributos de fonte em fala e Braile separadamente. (#16755)
* O tempo limite para pressionar várias vezes uma tecla é agora configurável. Isto pode ser especialmente útil para pessoas com problemas de destreza motora. (#11929, @CyrilleB79)

### Alterações

* As opções de linha de comando `-c`/`--config-path` e `--disable-addons` são agora respeitadas quando se inicia uma actualização a partir do NVDA. (#16937)
* Actualizações de componentes:
  * Conversor Braille LibLouis actualizado para [3.31.0](https://github.com/liblouis/liblouis/releases/tag/v3.31.0). (#17080, @LeonarddeR, @codeofdusk)
    * Corrigida a conversão de números em Braille espanhol.
    * Novas tabelas Braille:
      * Tailandês grau 1
      * Grego Braille internacional (letras acentuadas numa só célula)
    * Tabelas renomeadas:
      * por razões de coerência, a designação "Thai 6 pontos" foi alterada para "Thai grau 0".
      * A tabela "Grego Braille internacional" existente foi renomeada para "Grego Braille internacional (letras acentuadas em 2 células)" para clarificar a distinção entre os dois sistemas gregos.
  * eSpeak NG actualizado para a versão 1.52-dev commit `961454ff`. (#16775)
    * Adicionados novos idiomas: faroense e xextan.
* Quando se utiliza um dispositivo Braille multi-linha através do driver padrão HID Braille, todas as linhas de células serão utilizadas. (#16993, @alexmoon)
* A estabilidade do suporte ao Poedit do NVDA foi melhorada com o efeito colateral de que a versão mínima requerida do Poedit é agora a versão 3.5. (#16889, @LeonarddeR)

### Correcção de erros

* Correcções ao Braille:
  * Já é possível utilizar as teclas de encaminhamento do dispositivo Braille para mover o cursor do texto no Microsoft PowerPoint. (#9101)
  * Ao aceder ao Microsoft Word sem utilizar UIA, o NVDA já não produz caracteres estranhos nos cabeçalhos de tabela definidos com os comandos de definição de cabeçalho de linha e coluna. (#7212)
  * O driver Seika Notetaker já gera correctamente a entrada em Braille para espaço, retrocesso e comandos de pontos mais espaço/retrocesso. (#16642, @school510587)
  * O encaminhamento do cursor é agora muito mais fiável quando uma linha contém um ou mais selectores de variação Unicode ou caracteres decompostos. (#10960, @mltony, @LeonarddeR)
  * O NVDA já não apresenta um erro ao deslocar a linha de Braille para a frente em alguns controlos de edição vazios. (#12885)
  * O NVDA já não fica instável depois de reiniciar o NVDA durante uma verificação automática de dispositivos Braille via Bluetooth. (#16933)
* Já é possível utilizar os comandos de selecção do cursor de revisão para seleccionar texto no Microsoft PowerPoint. (#17004)
* No modo de voz a pedido, o NVDA já não fala quando é aberta uma mensagem no Outlook, carregada uma nova página num browser ou apresentado um novo diapositivo num diaporama do PowerPoint. (#16825, @CyrilleB79)
* No Mozilla Firefox, mover o rato sobre o texto antes ou depois de um link anuncia o texto de forma fiável. (#15990, @jcsteh)
* O NVDA já não falha ocasionalmente ao abrir mensagens navegáveis (como pressionar `NVDA+f` duas vezes). (#16806, @LeonarddeR)
* A actualização do NVDA enquanto as actualizações de extras estão pendentes já não resulta na remoção do extra. (#16837)
* Já é possível interagir com listas de validação de dados suspensas no Microsoft Excel 365. (#15138)
* O NVDA já não é tão lento quando se desloca para cima e para baixo através de ficheiros grandes no VS Code. (#17039)
* O NVDA já não deixa de responder depois de manter pressionada uma tecla de seta durante muito tempo no modo de navegação, especialmente no Microsoft Word e no Microsoft Outlook. (#16812)
* O NVDA já não lê a última linha quando o cursor está na penúltima linha de um controlo de edição de várias linhas em aplicações Java. (#17027)

### Alterações para programadores

Consulte [o guia do desenvolvedor](https://www.nvaccess.org/files/nvda/documentation/developerGuide.html#API) para obter informações sobre o processo de descontinuação e remoção da API do NVDA.

* Actualização de Componentes:
  * Atualizado py2exe para 0.13.0.2 (#16907, @dpy013)
  * Atualização do setuptools para 72.0 (#16907, @dpy013)
  * Ruff atualizado para 0.5.6. (#16868, @LeonarddeR)
  * Atualizado nh3 para 0.2.18 (#17020, @dpy013)
* Adicionado um ficheiro `.editorconfig` ao repositório do NVDA para que várias IDEs possam pegar as regras básicas de estilo de código do NVDA por padrão. (#16795, @LeonarddeR)
* Adicionado suporte para dicionários de símbolos de voz personalizados. (#16739, #16823, @LeonarddeR)
  * Os dicionários podem ser fornecidos em pastas específicas de internacionalização num pacote add-on, por exemplo, `locale\en`.
  * Os metadados do dicionário podem ser adicionados a uma secção opcional `symbolDictionaries` no manifesto do add-on.
  * Consulte a secção [Dicionários de símbolos de voz personalizados no guia do programador] (https://www.nvaccess.org/files/nvda/documentation/developerGuide.html#AddonSymbolDictionaries) para obter mais informações.
* Agora é possível redirecionar objectos recuperados de coordenadas no ecrã, usando o método `NVDAObject.objectFromPointRedirect`. (#16788, @Emil-18)
* Executando SCons com o parâmetro `--all-cores` irá automaticamente escolher o número máximo de núcleos de CPU disponíveis. (#16943, #16868, @LeonarddeR)
* A informação do programador inclui agora informação sobre a arquitetura da aplicação (tal como AMD64) para o objecto do navegador. (#16488, @josephsl)

#### Obsoletos

* A chave de configuração `bool` `[documentFormatting][reportFontAttributes]` está obsoleta para ser removida na versão 2025.1, em vez disso use `[fontAttributeReporting]`. (#16748)
  * A nova chave tem um valor `int` que corresponde a um `enum` `OutputMode` com opções para voz, braille, voze braille e desligado.
  * Os utilizadores da API podem utilizar o valor `bool` como anteriormente, ou verificar o `OutputMode` se estiverem a lidar especificamente com fala ou braille.
  * Estas chaves estão atualmente sincronizadas até 2025.1.
* o `NVDAObjects.UIA.InaccurateTextChangeEventEmittingEditableText` está obsoleto e não tem substituto. (#16817, @LeonarddeR)

## 2024.3.1

Esta é uma versão para corrigir um erro na notificação automática de actualização de extras.

### Correcção de erros

* Ao verificar automaticamente se há actualizações de extras, o NVDA já não congela em más ligações à net. (#17036)

## 2024.3

A Loja de Extras agora notificará, no arranque do NVDA, se existem actualizações de extras disponíveis.
...
Feitas várias correções de erros, particularmente para o Painel de Emojis e o histórico da Área de Transferência no Windows 11.
Para navegadores web, há correções para o anúncio de mensagens de erro, figuras, legendas, etiquetas de tabelas e itens de menu de caixas de verificação/botões de opção.

LibLouis actualizado, adicionando novas tabelas de Braille para Sérvio Cirílico, Iídiche, várias línguas antigas, Turco e o Alfabeto Fonético Internacional.
eSpeak actualizado, adicionando suporte para a língua Caracalpaque.
Unicode CLDR também foi actualizado.
...
* Adicionado suporte para Normalização Unicode na saída de voz e Braille. (#11570, #16466 @LeonarddeR).
  ...
  * Pode activar esta funcionalidade, tanto para a voz como para o Braille, nas respectivas secções nas Configurações do NVDA.
* Por padrão, após o arranque do NVDA, será notificado se houver actualizações de extras disponíveis. (#15035)
  * Isto pode ser desactivado na secção "Loja de Extras" das configurações do NVDA.
  * O NVDA verifica diariamente por actualizações de extras.
  * Apenas as actualizações dentro do mesmo canal serão verificadas (por exemplo, extras beta instalados terão apenas notificações para actualizações no canal beta).
* Adicionado suporte para os dispositivos Braille Help Tech Activator Pro. (#16668)

### Alterações

* Actualização de Componentes:
  * eSpeak NG actualizado para 1.52-dev commit `54ee11a79`. (#16495)
    * Adicionado suporte para a língua Caracalpaque.
  * Unicode CLDR actualizado para a versão 45.0. (#16507, @OzancanKaratas)
  * Actualizado fast_diff_match_patch (usado para detectar mudanças em terminais e outro conteúdo dinâmico) para a versão 2.1.0. (#16508, @codeofdusk)
  * Conversor Braille LibLouis actualizado para [3.30.0](https://github.com/liblouis/liblouis/releases/tag/v3.30.0). (#16652, @codeofdusk)
    * Novas tabelas Braille:
      * Sérvio Cirílico.
      * Iídiche.
      * Várias línguas antigas: Hebraico Bíblico, Acádio, Siríaco, Ugarítico e texto Cuneiforme transliterado.
      * Turco grau 2. (#16735)
      * Alfabeto Fonético Internacional. (#16773)
  * NSIS actualizado para 3.10 (#16674, @dpy013)
  * Markdown actualizado para 3.6 (#16725, @dpy013)
  * Nh3 actualizado para 0.2.17 (#16725, @dpy013)
* A tabela predefinida de entrada de Braille é agora igual à tabela padrão de saída, que é o Código Braille Unificado Inglês grau 1. (#9863, @JulienCochuyt, @LeonarddeR)
* O NVDA passa a anunciar figuras sem filhos acessíveis, mas com uma etiqueta ou descrição. (#14514)
* Ao ler linha por linha no modo de navegação, "legenda" já não é anunciado em cada linha de uma figura ou legenda de tabela longa. (#14874)
* Na consola Python, o último comando não executado já não é perdido ao navegar no histórico de entradas. (#16653, @CyrilleB79)
...
* Correcções para Windows 11:
  * O NVDA já não parecerá ficar preso ao fechar o histórico da área de transferência e o painel de emojis. (#16346, #16347, @josephsl)
  * O NVDA anunciará novamente os candidatos visíveis ao abrir a interface IME. (#14023, @josephsl)
  * O NVDA não anunciará "histórico da área de transferência" duas vezes ao navegar pelos itens de menu do painel de emojis. (#16532, @josephsl)
  * O NVDA não cortará mais a fala e o Braille ao rever kaomojis e símbolos no painel de emojis. (#16533, @josephsl)
* Correcções para navegadores web:
  * Mensagens de erro referenciadas com `aria-errormessage` são agora anunciadas no Google Chrome e Mozilla Firefox. (#8318)
  * Se presentes, o NVDA usará `aria-labelledby` para fornecer nomes acessíveis para tabelas no Mozilla Firefox. (#5183)
  * O NVDA anunciará correctamente itens de menu de opção e caixas de selecção ao entrar pela primeira vez em sub-menus no Google Chrome e Mozilla Firefox. (#14550)
  * A funcionalidade de pesquisa no modo de navegação do NVDA é agora mais precisa quando a página contém emojis. (#16317, @LeonarddeR)
  * No Mozilla Firefox, o NVDA já anuncia correctamente o carácter, palavra e linha actuais quando o cursor está no ponto de inserção no final de uma linha. (#3156, @jcsteh)
  * Já não causa falhas no Google Chrome ao fechar um documento ou sair do Chrome. (#16893)
* O NVDA anunciará correctamente as sugestões de preenchimento automático no Eclipse e outros ambientes baseados no Eclipse no Windows 11. (#16416, @thgcode)
* Melhorada a fiabilidade da leitura automática de texto, particularmente em aplicações de terminal. (#15850, #16027, @Danstiv)
* É novamente possível redefinir a configuração para os padrões de fábrica de forma confiável. (#16755, @Emil-18)
* O NVDA anunciará correctamente as mudanças de selecção ao editar o texto de uma célula no Microsoft Excel. (#15843)
* Em aplicações que usam Java Access Bridge, o NVDA lerá agora correctamente a última linha em branco de um texto em vez de repetir a linha anterior. (#9376, @dmitrii-drobotov)
* No LibreOffice Writer (versão 24.8 e mais recentes), ao alternar a formatação de texto (negrito, itálico, sublinhado, subscrito/sobrescrito, alinhamento) usando o atalho de teclado correspondente, o NVDA anuncia o novo atributo de formatação (por exemplo, "Negrito ligado", "Negrito desligado"). (#4248, @michaelweghorn)
* Ao navegar com as teclas de cursor em caixas de texto em aplicações que usam UI Automation, o NVDA já não anunciará, às vezes, o carácter, palavra, etc., errados. (#16711, @jcsteh)
* Ao colar na Calculadora do Windows 10/11, o NVDA já anunciará correctamente o número completo colado. (#16573, @TristanBurchett)
* A fala não ficará mais silenciosa após desligar e religar a uma sessão de Ambiente de Trabalho Remoto. (#16722, @jcsteh)
* Adicionado suporte para comandos de revisão de texto para o nome de um objeto no Visual Studio Code. (#16248, @Cary-Rowen)
* Os sons do NVDA já não falham em dispositivos de áudio mono. (#16770, @jcsteh)
* O NVDA já anuncia os endereços ao percorrer os campos Para/CC/BCC no outlook.com / Modern Outlook. (#16856)
* O NVDA lida agora de forma mais graciosa com falhas na instalação de extras. (#16704)
...
* Uma fonte de largura fixa é agora utilizada no Visualizador de Registos e na Consola Python do NVDA para que o cursor permaneça na mesma coluna durante a navegação vertical.
É especialmente útil para ler os marcadores de localização de erros em tracebacks. (#16321, @CyrilleB79)
* Foi adicionado o suporte para tabelas braille personalizadas. (#3304, #16208, @JulienCochuyt, @LeonarddeR)
  * As tabelas podem ser fornecidas na pasta `brailleTables` num pacote add-on.
  * Os metadados da tabela podem ser adicionados a uma secção opcional `brailleTables` no manifesto do add-on ou a um ficheiro `.ini` com o mesmo formato encontrado no subdiretório brailleTables do diretório scratchpad.
  * Consulte a secção [tabelas de tradução braille no guia do programador] (https://www.nvaccess.org/files/nvda/documentation/developerGuide.html#BrailleTables) para obter mais informações.
* Quando um evento `gainFocus` é enfileirado com um objeto que tem uma propriedade `focusRedirect` válida, o objeto apontado pela propriedade `focusRedirect` é agora mantido por `eventHandler.lastQueuedFocusObject`, ao invés do objeto originalmente enfileirado. (#15843)
* O NVDA irá registar a sua arquitetura executável (x86) no arranque. (#16432, @josephsl)
* o `wx.CallAfter`, que é envolvido no `monkeyPatches/wxMonkeyPatches.py`, agora inclui a indicação apropriada do `functools.wraps`. (#16520, @XLTechie)
* Existe um novo módulo para agendamento de tarefas `utils.schedule`, utilizando o módulo `schedule` do pip. (#16636)
  * Você pode usar `scheduleThread.scheduleDailyJobAtStartUp` para agendar automaticamente um trabalho que acontece depois que o NVDA é iniciado, e a cada 24 horas depois disso.
  Os trabalhos são programados com um atraso para evitar conflitos.
  * o `scheduleThread.scheduleDailyJob` e o `scheduleJob` podem ser utilizados para agendar trabalhos em horários personalizados, onde um `JobClashError` será levantado em um conflito de agendamento de trabalho conhecido.
* Agora é possível criar módulos de aplicação para aplicações que alojam controlos Edge WebView2 (msedgewebview2.exe). (#16705, @josephsl)
...
Há novos comandos para modificar o anel de definições do sintetizador, permitindo aos utilizadores saltar para a primeira ou última definição e aumentar ou diminuir a definição actual num passo maior.
Existem também novos comandos de navegação rápida, que permitem aos utilizadores associar comandos para saltar rapidamente entre parágrafo, parágrafo alinhado verticalmente, texto do mesmo estilo, texto de estilo diferente, item de menu, botão de alternância, barra de progresso, figura ou fórmula matemática.

Adicionadas muitas funcionalidades e corrigidos muitos erros no suporte Braille.
Adicionado um novo modo Braille denominado "Apresentar saída de voz".
Quando activo, o dispositivo Braille mostra exactamente o que o NVDA fala.
Foi também adicionado suporte para os dispositivos BrailleEdgeS2 e BrailleEdgeS3.
LibLouis actualizado, adicionando novas tabelas Braille bielorrussas e ucranianas detalhadas (com indicação de letras maiúsculas), tabela para Laos e uma tabela espanhola para leitura de textos gregos.

eSpeak actualizado, adicionando o novo idioma Tigrinya.

Existem muitas correções de erros menores para aplicações, tais como o Thunderbird, Adobe Reader, navegadores Web, Nudi e Geekbench.
...
* Novos comandos de teclas:
  ...
  * Adicionados comandos para saltar para primeiro, último, a frente e para trás no anel de definições do sintetizador. (#13768, #16095, @rmcpantoja)
    * Definir a primeira e a última definição no anel de definições de sintetizador não tem nenhum comando atribuído. (#13768)
    * Diminuir e aumentar a definição actual do anel de definições de sintetizador num passo maior (#13768):
      * Desktop: `NVDA+control+pageUp` ou `NVDA+control+pageDown`.
      * Laptop: `NVDA+controlo+shift+pageAcima` ou `NVDA+controlo+shift+pageAbaixo`.
  * Adicionado um novo comando não atribuído para alternar o anúncio de figuras e legendas. (#10826, #14349)
* Braille:
  * Adicionado suporte para os dispositivos BrailleEdgeS2 e BrailleEdgeS3. (#16033, #16279, @EdKweon)
  * Adicionado um novo modo Braille chamado "Apresentar saída de voz". (#15898, @Emil-18)
    * Quando activo, o dispositivo Braille mostra exactamente o que o NVDA fala.
    * Pode ser alternado pressionando `NVDA+alt+t`, ou a partir do diálogo de configurações Braille.
* Divisão de som: (#12985, @mltony)
  * Permite dividir os sons do NVDA num canal (por exemplo, esquerdo) enquanto os sons de todos os outros aplicativos são colocados no outro canal (por exemplo, direito).
...
* Loja de extras:
  ...
  * A acção de revisões da comunidade estará disponível em todos os separadores da loja. (#16179, @nvdaes)
* Actualizações de componentes:
  * Conversor Braille LibLouis actualizado para [3.29.0](https://github.com/liblouis/liblouis/releases/tag/v3.29.0). (#16259, @codeofdusk)
    * Adicionadas novas tabelas detalhadas (com indicação de letras maiúsculas) de Braille bielorrusso e ucraniano.
    * Nova tabela espanhola para ler textos gregos.
    * Nova tabela para Lao Grau 1. (#16470)
  * eSpeak NG actualizado para a versão 1.52-dev commit `cb62d93fd7`. (#15913)
...
* Foi corrigida a leitura de atributos matemáticos no Adobe Reader, o que resultava num discurso e braille pobres ou errados. (#17980)
* Correcções para Windows 11:
  * O NVDA volta a anunciar as sugestões de entrada de teclado de hardware. (#16283, @josephsl)
...
* Loja de extras:
  * Ao pressionar `ctrl+tab`, o foco move-se correctamente para o novo título do separador actual. (#14986, @ABuffEr)
...
* Correcções para navegadores baseados em Chromium quando usados com UIA:
  * Corrigidos erros que faziam o NVDA travar. (#16393, #16394)
  * A tecla Retrocesso está agora a funcionar corretamente nos campos de início de sessão do Gmail. (#16395)
* A tecla Retrocesso já funciona correctamente quando se utiliza o Nudi 6.1 com a definição "Tratar teclas enviadas por outras aplicações" do NVDA activada. (#15822, @jcsteh)
...
* Nos navegadores baseados no Firefox e no Chromium, o NVDA entrará correctamente no modo de foco ao pressionar enter quando posicionado numa lista de apresentação (ul /ol) dentro de conteúdo editável. (#16325)
* A alteração do estado da coluna é automaticamente anunciada ao seleccionar colunas para apresentar na lista de mensagens do Thunderbird.(#16323)
...
* A instanciação de objectos `winVersion.WinVersion` com versões desconhecidas do Windows acima de 10.0.22000, como 10.0.25398, devolve "Windows 11 desconhecido" em vez de "Windows 10 desconhecido" para o nome da versão. (#15992, @josephsl)
* Tornar o processo de construção do AppVeyor mais fácil para os forks do NVDA, adicionando variáveis configuráveis no appveyor.yml para desativar ou modificar partes específicas do NV Access dos scripts de construção. (#16216, @XLTechie)
* Adicionado um documento de instruções, explicando o processo de criação de forks do NVDA no AppVeyor. (#16293, @XLTechie)

## 2024.1

Foi adicionado um novo modo de voz "a pedido".
Quando o modo de voz está configurado para "a pedido", o NVDA não fala automaticamente (por exemplo, ao mover o cursor) mas fala ao activar comandos cujo objectivo é explicitamente anunciar algo (por exemplo, anunciar o título da janela).
Na secção Voz das configurações do NVDA, passa a ser possível excluir modos de voz não desejados do comando Alternar entre modos de voz (`NVDA+s`).
...
Quando activado, seleccionar texto no modo de navegação também manipulará a selecção nativa do Firefox.
Copiar texto com `control+c` passará directamente para o Firefox, copiando o conteúdo HTML, em vez da representação em texto simples do NVDA.
...
Muitas correções de erros, particularmente para a Loja de extras, Braille, Libre Office, Microsoft Office e áudio.
...
* Adicionado suporte para dispositivos Braille Bluetooth HID Low Energy. (#15470)
* Um novo modo de Selecção Nativa (activado por `NVDA+shift+f10`) está agora disponível no modo de navegação do NVDA para o Mozilla Firefox.
...
* Foi adicionado um novo modo de voz, "a pedido",.
Quando a voz está configurada para "a pedido" o NVDA não fala automaticamente (por exemplo, ao mover o cursor) mas fala ao activar comandos cujo objectivo é explicitamente anunciar algo (por exemplo, anunciar o título da janela). (#481, @CyrilleB79)
* Na secção Voz das configurações do NVDA, agora é possível excluir modos de voz não desejados do comando "Alternar entre modos de voz" (`NVDA+s`). (#15806, @lukaszgo1)
...
* Actualização de Componentes:
  * Conversor Braille LibLouis actualizado para [3.28.0](https://github.com/liblouis/liblouis/releases/tag/v3.28.0). (#15435, #15876, @codeofdusk)
    * Adicionadas tabelas Braille para Tailandês, Romeno e Filipino.
  * eSpeak NG actualizado para 1.52-dev commit `530bf0abf`. (#15036)
  * Anotações de emoji e símbolos CLDR actualizadas para a versão 44.0. (#15712, @OzancanKaratas)
  * Java Access Bridge actualizado para 17.0.9+8Zulu (17.46.19). (#15744)
* Comandos de Teclado:
...
* Microsoft Office:
  * Ao solicitar informações de formatação em células do Excel, margens e fundo só serão anunciados se houver tal formatação. (#15560, @CyrilleB79)
...
* A opção "Anunciar o tipo de controlo quando o rato entrar no objecto" na secção de configurações do rato do NVDA foi renomeada para "Anunciar objecto quando o rato entra nele".
...
Os utilizadores do Poedit 1 são incentivados a actualizar para o Poedit 3 se quiserem contar com acessibilidade melhorada no Poedit, como atalhos para ler notas e comentários do tradutor. (#15313, #7303, @LeonarddeR)
* O visualizador de Braille e o visualizador de discurso agora estão desactivados no modo seguro. (#15680)
* Durante a navegação por objectos, objectos desactivados (indisponíveis) já não são ignorados. (#15477, @CyrilleB79)
* Adicionado índice ao documento Referência rápida de comandos. (#16106)

### Correcção de erros

* Loja de extras:
  * Quando o estado de um extra é alterado enquanto ele está em foco, por exemplo, uma mudança de "a descarregar" para "descarregado", o item actualizado agora é anunciado correctamente. (#15859, @LeonarddeR)
  ...
  * Os extras desactivados e incompatíveis já podem ser actualizados. (#15568, #15029)
  * O NVDA agora recupera e mostra um erro no caso de um extra não ter sido descarregado correctamente. (#15796)
  * O NVDA já não falha intermitentemente ao reiniciar após abrir e fechar a Loja de extras. (#16019, @lukaszgo1)
* Áudio:
  * O NVDA já não congela brevemente quando múltiplos sons são reproduzidos em rápida sucessão. (#15311, #15757, @jcsteh)
...
* Braille:
  ...
  * Mais objectos que contêm texto útil são detectados, e o conteúdo de texto é mostrado em Braille. (#15605)
  * A introdução de texto em Braille contraído volta a funcionar correctamente. (#15773, @aaclause)
  * O Braille agora é actualizado ao mover o objecto de navegação entre células de tabela em mais situações (#15755, @Emil-18)
  * O resultado dos comandos de anunciar foco actual, objecto de navegação actual e comandos de selecção actual, é agora mostrado em Braille. (#15844, @Emil-18)
  * O driver do dispositivo Braille Albatross não trata mais um microcontrolador Esp32 como um dispositivo Albatross. (#15671)
* LibreOffice:
  * Palavras apagadas usando o comando `control+Retrocesso` agora também são anunciadas correctamente quando a palavra apagada é seguida por espaço em branco (como espaços e tabs). (#15436, @michaelweghorn)
  * O anúncio da barra de estado usando o comando `NVDA+fim` já funciona para diálogos na versão 24.2 e mais recente do LibreOffice. (#15591, @michaelweghorn)
...
* Microsoft Office:
  * No Excel com UIA desactivado, o Braille é actualizado, e o conteúdo da célula activa é falado, quando `control+y`, `control+z` ou `alt+Retrocesso` são pressionados. (#15547)
  * No Word com UIA desactivado o Braille é actualizado quando `control+v`, `control+x`, `control+y`, `control+z`, `alt+Retrocesso`, `Retrocesso` ou `control+Retrocesso` são pressionados.
  Também é actualizado com UIA activado, ao digitar texto e o Braille está ligado à revisão e a revisão segue o cursor. (#3276)
  * No Word, a célula de destino agora será correctamente anunciada ao usar os comandos nativos do Word para navegação em tabelas `alt+início`, `alt+fim`, `alt+páginaAcima` e `alt+páginaAbaixo`. (#15805, @CyrilleB79)
...
* O NVDA anunciará o conteúdo para mais diálogos do Windows 10 e 11. (#15729, @josephsl)
...
* Ao aceder ao Microsoft Edge usando UI Automation, o NVDA é capaz de activar mais controlos no modo de navegação. (#14612)
* O NVDA já não falhará ao iniciar quando o ficheiro de configuração estiver corrompido, mas restaurará a configuração para o padrão como fazia no passado. (#15690, @CyrilleB79)
* Corrigido o suporte para controles System List view (`SysListView32`) em aplicações Windows Forms. (#15283, @LeonarddeR)
...
* O NVDA deve permanecer responsivo ao ser inundado com muitos eventos de UI Automation, por exemplo, quando grandes pedaços de texto são mostrados  em um terminal ou ao ouvir mensagens de voz no WhatsApp. (#14888, #15169)
  * Este novo comportamento pode ser desactivado usando a nova configuração "Utilizar o processamento de eventos melhorado" nas configurações avançadas do NVDA.
* O NVDA é novamente capaz de seguir o foco em aplicações executadas dentro do Windows Defender Application Guard (WDAG). (#15164)
* O texto da fala já não é actualizado quando o rato se move no Visualizador de discurso. (#15952, @hwf1324)
* O NVDA voltará novamente ao modo de navegação ao fechar caixas de combinação com `escape` ou `alt+Seta acima` no Firefox ou Chrome. (#15653)
...
* Nota: esta é uma versão que quebra a compatibilidade da API de alguns extras.
Os extras terão de ser novamente testados e o seu manifesto atualizado.
* A compilação do NVDA agora requer o Visual Studio 2022.
Consulte os [documentos do NVDA] (https://github.com/nvaccess/nvda/blob/release-2024.1/projectDocs/dev/createDevEnvironment.md) para obter a lista específica de componentes do Visual Studio. (#14313)
...
* Agora é possível usar formas plurais nas traduções de um extra. (#15661, @beqabeqa473)
* Incluído python3.dll na distribuição binária para uso por extras com bibliotecas externas utilizando a [ABI estável](https://docs.python.org/3.11/c-api/stable.html). (#15674, @mzanm)
* A classe base `BrailleDisplayDriver` agora tem as propriedades `numRows` e `numCols` para fornecer informações sobre dispositivos braille de várias linhas.
A definição de `numCells` continua a ser suportada para dispositivos braille de uma linha e `numCells` devolverá o número total de células para dispositivos braille de várias linhas. (#15386)
* Atualizado BrlAPI para BRLTTY para a versão 0.8.5, e seu módulo python correspondente para uma compilação compatível com Python 3.11. (#15652, @LeonarddeR)
* Adicionada a função `speech.speakSsml`, que permite escrever sequências de voz NVDA usando [SSML](https://www.w3.org/TR/speech-synthesis11/). (#15699, @LeonarddeR)
  * As seguintes etiquetas são actualmente suportadas e traduzidas para comandos de voz NVDA adequados:
    * `Prosódia` (`pitch`, `rate` e `volume`). Apenas a multiplicação (por exemplo, `200%`) é suportada.
    ...
    * `mark` com o atributo `name` definido como um nome de marca, por exemplo, `mark1`, requer o fornecimento de uma chamada de retorno
  * Exemplo: `speech.speakSsml('<speak><prosody pitch="200%">olá</prosody><break time="500ms" /><prosody rate="50%">João</prosody></speak>')`
  * As capacidades de análise SSML são suportadas pela classe `SsmlParser` no módulo `speechXml`.
* Alterações na biblioteca do controlador do NVDA:
  * Os nomes dos ficheiros da biblioteca já não contêm um sufixo que denota a arquitetura, ou seja, `nvdaControllerClient32/64.dll` são agora chamados `nvdaControllerClient.dll`. (#15718, #15717, @LeonarddeR)
  * Adicionado um exemplo para demonstrar o uso do nvdaControllerClient.dll a partir do Rust. (#15771, @LeonarddeR)
  * Adicionadas as seguintes funções ao controlador: (#15734, #11028, #5638, @LeonarddeR)
    * `nvdaController_getProcessId`: Para obter o ID do processo (PID) da instância atual do NVDA que o controlador está a utilizar.
    * `nvdaController_speakSsml`: Para instruir o NVDA a falar de acordo com o SSML fornecido. Esta função também suporta:
      ...
      * Fornecer a prioridade do discurso a ser proferido.
      * Falando tanto de forma síncrona (bloqueante) como assíncrona (retorno instantâneo).
    * `nvdaController_setOnSsmlMarkReachedCallback`: Para registar uma chamada de retorno do tipo `onSsmlMarkReachedFuncType` que é chamada em modo síncrono para cada etiqueta `<mark />` encontrada na sequência SSML fornecida ao `nvdaController_speakSsml`.
  * Nota: as novas funções no cliente do controlador apenas suportam o NVDA 2024.1 e superior.
* Atualizadas as dependências do `include`:
  * desvios para `4b8c659f549b0ab21cf649377c7a84eb708f5e68`. (#15695)
  ...
  * para `5e9be7b2d2fe3834a7107f430f7d4c0631f69833`. (#15695)
* As informações do dispositivo obtidas pelo `hwPortUtils.listUsbDevices` agora contêm a descrição relatada pelo barramento do dispositivo USB (chave `busReportedDeviceDescription`). (#15764, @LeonarddeR)
* Para dispositivos seriais USB, `bdDetect.getConnectedUsbDevicesForDriver` e `bdDetect.getDriversForConnectedUsbDevices` agora produzem correspondências de dispositivos contendo um dicionário `deviceInfo` enriquecido com dados sobre o dispositivo USB, como `busReportedDeviceDescription`. (#15764, @LeonarddeR)
* Quando o ficheiro de configuração `nvda.ini` está corrompido, é guardada uma cópia de segurança antes de ser reinicializado. (#15779, @CyrilleB79)
* Ao definir um script com o decorador de script, o argumento booleano `speakOnDemand` pode ser especificado para controlar se um script deve falar enquanto estiver no modo de fala "on-demand". (#481, @CyrilleB79)
  * Os scripts que fornecem informações (por exemplo, título da janela, hora/data do relatório) devem falar no modo "on-demand".
  * Os scripts que executam uma ação (por exemplo, mover o cursor, alterar um parâmetro) não devem falar no modo "on-demand".
* Corrigido o bug onde a exclusão de arquivos rastreados pelo git durante o `scons -c` resultava em interfaces UIA COM ausentes na reconstrução. (#7070, #10833, @hwf1324)
* Corrigido um bug onde algumas mudanças de código não eram detectadas ao construir o `dist`, o que impedia que uma nova construção fosse acionada.
Agora o `dist` sempre reconstrói. (#13372, @hwf1324)
* Um `gui.nvdaControls.MessageDialog` com tipo padrão de standard, não lança mais uma exceção de conversão None porque nenhum som é atribuído. (#16223, @XLTechie)

#### Alterações de rutura da API

Estas são alterações de rutura da API.
Abra um problema no GitHub se o seu extra tiver um problema com a atualização para a nova API.

* NVDA agora é construído com Python 3.11. (#12064)
* Dependências do pip actualizadas:
...
* Removidas as dependências do pip:
  * typing_extensions, estas devem ser suportadas nativamente no Python 3.11 (#15544)
  * em vez disso, é utilizado o unittest-xml-reporting para gerar relatórios XML. (#15544)
* o `IAccessibleHandler.SecureDesktopNVDAObject` foi removido.
Em vez disso, quando o NVDA estiver a ser executado no perfil do utilizador, monitorize a existência do ambiente de trabalho seguro com o ponto de extensão: `winAPI.secureDesktop.post_secureDesktopStateChange`. (#14488)
* o `braille.BrailleHandler.handlePendingCaretUpdate` foi removido sem nenhum substituto público. (#15163, @LeonarddeR)
* `bdDetect.addUsbDevices e bdDetect.addBluetoothDevices` foram removidos.
Os controladores de dispositivos Braille devem, em vez disso, implementar o método da classe `registerAutomaticDetection`.
Este método recebe um objeto `DriverRegistrar` no qual os métodos `addUsbDevices` e `addBluetoothDevices` podem ser utilizados. (#15200, @LeonarddeR)
* A implementação padrão do método de verificação no `BrailleDisplayDriver` agora requer que ambos os atributos `threadSafe` e `supportsAutomaticDetection` sejam definidos como `True`. (#15200, @LeonarddeR)
* Passar funções lambda para `hwIo.ioThread.IoThread.queueAsApc` não é mais possível, pois as funções devem ser fracamente referenciáveis. (#14627, @LeonarddeR)
* `IoThread.autoDeleteApcReference` foi removido. (#14924, @LeonarddeR)
* Para suportar mudanças de pitch de capital, os sintetizadores devem agora declarar explicitamente o seu suporte para o `PitchCommand` no atributo `supportedCommands` no driver. (#15433, @LeonarddeR)
* o `speechDictHandler.speechDictVars` foi removido. Use `NVDAState.WritePaths.speechDictsDir` em vez de `speechDictHandler.speechDictVars.speechDictsPath`. (#15614, @lukaszgo1)
* `languageHandler.makeNpgettext` e `languageHandler.makePgettext` foram removidos.
`npgettext` e `pgettext` são suportados nativamente agora. (#15546)
* O módulo app para [Poedit](https://poedit.net) foi alterado significativamente. A função `fetchObject` foi removida. (#15313, #7303, @LeonarddeR)
* Os seguintes tipos e constantes redundantes foram removidos do `hwPortUtils`: (#15764, @LeonarddeR)
  * `PCWSTR`
...
* o `gui.addonGui.AddonsDialog` foi removido. (#15834)
* o `touchHandler.TouchInputGesture.multiFingerActionLabel` foi removido sem qualquer substituição. (#15864, @CyrilleB79)
* o `NVDAObjects.IAccessible.winword.WordDocument.script_reportCurrentHeaders` foi removido sem qualquer substituição. (#15904, @CyrilleB79)
* Os seguintes módulos de aplicação foram removidos.
O código que importa de um deles deve, em vez disso, importar do módulo de substituição. (#15618, @lukaszgo1)

| Nome do módulo removido |Módulo de substituição|
|---|---|
|`azardi-2.0` |`azardi20`|
|`azuredatastudio` |`code`|
|`azuredatastudio-insiders` |`code`|
|`calculatorapp` |`calculator`|
|código - insiders` | código`|
|`commsapps` |`hxmail`|
|`dbeaver` |`eclipse`|
...
* O uso de `watchdog.getFormattedStacksForAllThreads` está obsoleto - por favor, use `logHandler.getFormattedStacksForAllThreads` em seu lugar. (#15616, @lukaszgo1)
* `easeOfAccess.canConfigTerminateOnDesktopSwitch` foi descontinuado, pois tornou-se obsoleto desde que o Windows 7 não é mais suportado. (#15644, @LeonarddeR)
* `winVersion.isFullScreenMagnificationAvailable` foi descontinuado - utilize `visionEnhancementProviders.screenCurtain.ScreenCurtainProvider.canStart` em vez disso. (#15664, @josephsl)
...
* O uso de `gui.ExecAndPump` está obsoleto - por favor use `systemUtils.ExecAndPump` em seu lugar. (#15852, @lukaszgo1)
...
* Corrigido o erro que fazia com que o processo do NVDA não terminasse correctamente. (#16123)
...
Isto pode ser configurado na secção OCR do Windows nas configurações do NVDA.

Houve várias correções no Braille, melhorando a detecção de dispositivos e o movimento do cursor.
...
* Gestão de som melhorada:
  * Uma nova secção, Áudio, nas configurações  do NVDA:
    ...
    * As opções para alterar o dispositivo de saída de áudio e alternar a redução de áudio foram movidas para a nova secção Configurações de áudio a partir da caixa de diálogo Seleccionar sintetizador.
    Estas opções serão removidas da caixa de diálogo "seleccionar sintetizador" em 2024.1. (#15486, #8711)
  ...
  * Nota: WASAPI é incompatível com alguns extras.
  Actualizações compatíveis estão disponíveis para estes extras, actualize-os antes de actualizar o NVDA.
...
* Microsoft Office:
  * Correcção de falha no Microsoft Word quando as opções de Formatação de Documento "Anunciar cabeçalhos" e "Anunciar comentários e notas" estavam desativadas. (#15019)
...
* Loja de extras:
  ...
  * Corrigidos problemas com o tratamento de extras incompatíveis ao actualizar o NVDA. (#15414, #15412, #15437)
...
* os ficheiros `braille.handler.handleUpdate` e `braille.handler.handleReviewMove` foram alterados para não serem actualizados instantaneamente.
Antes desta alteração, quando qualquer um destes métodos era chamado com muita frequência, esgotava muitos recursos.
Em vez disso, estes métodos colocam agora uma atualização em fila de espera no final de cada ciclo do núcleo.
Eles também devem ser seguros para threads, tornando possível chamá-los a partir de threads em segundo plano. (#15163)
* Adicionado suporte oficial para registar controladores de ecrãs braille personalizados no processo de deteção automática de ecrãs braille.
Para mais informações, consulte a documentação da classe `braille.BrailleDisplayDriver`.
Mais notavelmente, o atributo `supportsAutomaticDetection` deve ser definido como `True` e o `classmethod` `registerAutomaticDetection` deve ser implementado.  (#15196)

#### ObsoletosDeprecations

* `braille.BrailleHandler.handlePendingCaretUpdate` está agora obsoleto e não tem substituto público.
Será removido em 2024.1. (#15163)
...
Em vez disso, utilize as enumerações `XlHAlign` ou `XlVAlign`. (#15205)
* O mapeamento `NVDAObjects.window.excel.alignmentLabels` está obsoleto.
Em vez disso, utilize os métodos `displayString` das enumerações `XlHAlign` ou `XlVAlign`. (#15205)
* `bdDetect.addUsbDevices` e `bdDetect.addBluetoothDevices` foram descontinuados.
Os controladores de ecrãs Braille devem implementar o método de classe `registerAutomaticDetection`.
Esse método recebe um objeto `DriverRegistrar` no qual os métodos `addUsbDevices` e `addBluetoothDevices` podem ser utilizados. (#15200)
* A implementação padrão do método de verificação em `BrailleDisplayDriver` utiliza `bdDetect.driverHasPossibleDevices` para dispositivos que são marcados como thread safe.
A partir do NVDA 2024.1, para que o método base use `bdDetect.driverHasPossibleDevices`, o atributo `supportsAutomaticDetection` deve ser definido como `True` também. (#15200)

## 2023.2

Esta versão introduz a Loja de Extras para substituir o Gestor de Extras.
Na Loja de Extras, pode navegar, pesquisar, instalar e actualizar extras da comunidade.
...
eSpeak-NG, conversor de Braille LibLouis e Unicode CLDR foram actualizados.

### Novas funcionalidades

* Adicionada a Loja de extras ao NVDA. (#13985)
  * Navegue, pesquise, instale e actualize extras da comunidade.
  * Ignore manualmente problemas de incompatibilidade de extras antigos.
  * Substituído o Gestor de extras pela Loja de extras.
  * Para obter mais informações, leia o guia do utilizador actualizado.
* Novos comandos:
  ...
  * Adicionados comandos padrão para mover para o pobjecto anterior ou seguinte em revisão plana da hierarquia de objectos (#15053)
...
* Novas funcionalidades Braille:
  ...
  * Nova opção Braille para alternar a visualização do indicador de selecção (pontos 7 e 8). (#14948)
  ...
  * Adicionado suporte para o atributo ARIA 1.3 `aria-brailleroledescription`, permitindo que autores da web substituam o tipo de um elemento mostrado no Braille. (#14748)
...
* Funcionalidades no Microsoft Office:
  * Quando o texto realçado está ativado na Formatação de Documentos, as cores de realce são agora comunicadas no Microsoft Word. (#7396, #12101, #5866)
  * Quando as cores estão activadas na Formatação de Documentos, as cores de fundo são agora comunicadas no Microsoft Word. (#5866)
...
* No Mozilla Firefox e no Google Chrome, o NVDA agora informa quando um controlo abre um diálogo, grelha, lista ou árvore se o autor especificou isso usando `aria-haspopup`. (#14709)
...
* Actualização de componentes:
  * eSpeak NG foi actualizado para a versão 1.52-dev commit `ed9a7bcf`. (#15036)
  * Conversor Braille LibLouis foi actualizado para [3.26.0](https://github.com/liblouis/liblouis/releases/tag/v3.26.0). (#14970)
  * CLDR foi actualizado para a versão 43.0. (#14918)
* Alterações no LibreOffice:
  * Ao anunciar a localização do cursor de revisão, a localização actual do cursor passa a ser anunciada em relação à página actual no LibreOffice Writer para as versões do LibreOffice >= 7.6, similar ao que é feito para o Microsoft Word. (#11696)
  * O anúncio da barra de estado (`NVDA+Fim`) já funciona no LibreOffice. (#11698)
...
* Alterações no Braille:
  * Ao usar um Dispositivo Braille via driver Braille HID padrão, o D-pad pode ser usado para emular as teclas de seta e Enter.
  Além ddisso, `espaço+ponto1` e `espaço+ponto4` agora correspondem, respectivamente, às teclas de seta acima e para baixo. (#14713)
  * As actualizações no conteúdo dinâmico da web (ARIA live regions) agora são mostradas em Braille.
  Isto pode ser desactivado no painel Configurações avançadas. (#7756)
* Os símbolos de hífen e hífen emparelhado sempre serão enviados para o sintetizador. (#13830)
* A distância anunciada no Microsoft Word passa a respeitar a unidade definida nas opções avançadas do Word, mesmo ao usar o UIA para aceder a documentos do Word. (#14542)
* O NVDA responde mais rapidamente ao mover o cursor em controlos de edição. (#14708)
...
* Se o Windows estiver configurado para mostrar segundos no relógio da barra do sistema, `NVDA+f12` para anunciar a hora passa a respeitar essa configuração. (#14742)
...
* Navegadores Web:
  ...
  * No Mozilla Firefox e no Google Chrome, os caracteres digitados já não são relatados em algumas caixas de texto, mesmo quando a opção de falar caracteres digitados está desativada. (#14666)
  * Já é possível usar o modo de navegação em Controlos Embutidos do Chromium onde antes não era possível. (#13493, #8553)
  * No Mozilla Firefox, ao passar o rato sobre o texto, após um link, o NVDA já anuncia correctamente o texto. (#9235)
  * O destino de links gráficos agora é relatado corretamente no Chrome e no Edge. (#14779)
  ...
  * No modo de navegação, o NVDA já não ignora, incorretamente, o movimento do foco para um controlo de nível superior ou inferior, por exemplo, movendo-se de um elemento para a lista ou grelha que o contém. (#14611)
    * No entanto, observe que esta correção se aplica apenas quando a opção "Definir automaticamente o foco em elementos com foco" nas configurações do modo de navegação está desactivada (que é a configuração padrão).
* Correcções para Windows 11:
  * O NVDA volta a poder anunciar o conteúdo da barra de estado do Bloco de Notas. (#14573)
  * Ao mudar de separadore é anunciado o novo nome, e a sua posição, no Bloco de Notas e Explorador de Ficheiros. (#14587, #14388)
  * O NVDA volta a anunciar itens candidatos ao inserir texto em idiomas como chinês e japonês. (#14509)
  * É mais uma vez possível abrir os itens Contribuidores e Licença no menu Ajuda do NVDA. (#14725)
* Correcções para o Microsoft Office:
  * Ao mover rapidamente entre células no Excel, o NVDA agora tem menos probabilidade de anunciar a célula ou selecção errada. (#14983, #12200, #12108)
...
* Para símbolos que não têm uma descrição no idioma actual, será usada a descrição de símbolo padrão em inglês. (#14558, #14417)
...
* Na Calculadora do Windows 10 e 11, uma cópia portátil do NVDA não fará mais nada ou emitirá tons de erro ao inserir expressões no modo de  calculadora padrão em modo compacto. (#14679)
* O NVDA recupera-se novamente de muitas situações, como aplicativos que param de responder, que anteriormente causavam paragens completas. (#14759)
* Ao forçar o suporte UIA com determinado terminal e consoles, foi corrigido um bug, que causava congelamento e spam no arquivo de log. (#14689)
* O NVDA não recusará mais guardar a configuração após uma reinicialização da configuração. (#13187)
* Ao executar uma versão temporária a partir do instalador, o NVDA não induzirá os utilizadores a pensar que podem guardar a configuração. (#14914)
...
* Corrigido o erro que fazia com que o gesto "varrer para cima" em revisão de texto movesse páginas em vez de mover para a linha anterior. (#15127)

### Alterações para Desenvolvedores

Consulte [o guia do desenvolvedor](https://www.nvaccess.org/files/nvda/documentation/developerGuide.html#API) para obter informações sobre o processo de descontinuação e remoção da API do NVDA.

* Foram adicionadas convenções sugeridas à especificação do manifesto do extra.
Estes são opcionais para a compatibilidade do NVDA, mas encorajados, para serem admitidos na loja .  (#14754)
  * Use `lowerCamelCase` para os nomes de campo.
...
* Adicionado um novo tipo de ponto de extensão chamado `Chain`, que pode ser usado para iterar sobre iteráveis retornados por manipuladores registrados. (#14531)
* Adicionado o ponto de extensão `bdDetect.scanForDevices`.
Podem ser registados manipuladores que produzem pares `BrailleDisplayDriver/DeviceMatch` que não se encaixam nas categorias existentes, como USB ou Bluetooth. (#14531)
* Adicionado ponto de extensão: `synthDriverHandler.synthChanged`. (#14618)
* O anel de configurações do sintetizador do NVDA agora armazena em cache os valores de configuração disponíveis na primeira vez em que são necessários, em vez de ao carregar o sintetizador. (#14704)
* Agora é possível chamar o método de exportação num mapa de comandos para o exportar para um dicionário.
Este dicionário pode ser importado com outro comando, passando-o para o construtor do `GlobalGestureMap` ou para o método de actualização de um mapa existente. (#14582)
* `hwIo.base.IoBase` e seus derivados agora têm um novo parâmetro de construção para receber um `hwIo.ioThread.IoThread`.
Se não for fornecida, é utilizada a thread predefinida. (#14627)
...
Da mesma forma, o novo método `getCompletionRoutine` permite-lhe converter um método python numa rotina de conclusão de forma segura. (#14627)
...
* `highlight-color` é agora um atributo de campo de formatação. (#14610)
* O NVDA deve determinar com mais precisão se uma mensagem registada está a vir do núcleo do NVDA. (#14812)
* O NVDA já não registará avisos ou erros imprecisos sobre appModules obsoletos. (#14806)
* Todos os pontos de extensão NVDA são agora brevemente descritos num novo capítulo dedicado no Guia do Programador. (#14648)
* o `scons checkpot` não irá mais verificar a subpasta `userConfig`. (#14820)
* As expressões traduzíveis podem agora ser definidas com uma forma singular e uma forma plural usando `ngettext` e `npgettext`. (#12445)

#### Obsoletos

* A passagem de funções lambda para `hwIo.ioThread.IoThread.queueAsApc` está obsoleta.
Em vez disso, as funções devem ser fracamente referenciáveis. (#14627)
* A importação de `LPOVERLAPPED_COMPLETION_ROUTINE` de `hwIo.base` está obsoleta.
Em vez disso, importar de `hwIo.ioThread`. (#14627)
* `IoThread.autoDeleteApcReference` está obsoleto.
Foi introduzida no NVDA 2023.1 e nunca foi concebida para fazer parte da API pública.
Até à remoção, comporta-se como um no-op, ou seja, um gestor de contexto que não produz nada. (#14924)
* `gui.MainFrame.onAddonsManagerCommand` está obsoleto, use `gui.MainFrame.onAddonStoreCommand` em vez disso. (#13985)
* `speechDictHandler.speechDictVars.speechDictsPath` está obsoleto, use `NVDAState.WritePaths.speechDictsDir` em vez disso. (#15021)
* A importação de `voiceDictsPath` e `voiceDictsBackupPath` de `speechDictHandler.dictFormatUpgrade` está obsoleta.
Em vez disso, utilize `WritePaths.voiceDictsDir` e `WritePaths.voiceDictsBackupDir` do `NVDAState`. (#15048)
* `config.CONFIG_IN_LOCAL_APPDATA_SUBKEY` está obsoleto.
Em vez disso, utilize `config.RegistryKey.CONFIG_IN_LOCAL_APPDATA_SUBKEY`. (#15049)
...
Corrigidos muitos erros, nomeadamente no Windows 11.

eSpeak, o LibLouis, o Sonic rate boost e o Unicode CLDR foram actualizados.
...
* Foi adicionada uma nova opção, "Estilo de parágrafo" em "Navegação do documento".
...
* Novo comando global, sem atribuição de teclas, (secção Ferramentas): Anunciar o destino do link numa janela.
...
* o eSpeak NG foi actualizado para a versão 1,52-dev commit `f520fecb`. (#14281, #14675)
  * Corrigido o anúncio de números grandes. (#14241)
...
* Correcções para  Windows 11:
  ...
  * Os itens do menu do histórico da Área de transferência, tais como "item pin", podem ser acedidos. (#14508)
...
* Algumas configurações deixarão de ser inesperadamente diferentes quando usadas com perfis múltiplos. (#14170)
  * As seguintes configurações foram contempladas:
    * Indentação das linhas na secção Formatação de documentos.
    * "Linhas de grelha da célula:" na secção "Formatação de documentos"
    * Mostrar mensagens na secção Braille das configurações
...
* O NVDA já não falha ao continuar a leitura no Kindle para PC depois de virar a página. (#14390)

### Alterações para Desenvolvedores

Nota: esta é uma versão que quebra a compatibilidade da API dos extras.
Os extras terão de ser novamente testados e o seu manifesto actualizado.
Consulte [o guia do programador](https://www.nvaccess.org/files/nvda/documentation/developerGuide.html#API) para obter informações sobre o processo de descontinuação e remoção da API do NVDA.

* Os testes de sistema agora devem passar quando executados localmente em sistemas não ingleses. (#13362)
* No Windows 11 em ARM, as aplicações x64 já não são identificadas como aplicações ARM64. (#14403)
* Já não é necessário utilizar `SearchField` e `SuggestionListItem` `UIA` `NVDAObjects` em novos cenários de automatização da interface do utilizador, em que o relatório automático de sugestões de pesquisa e a digitação foram expostos através da automatização da interface do utilizador com o padrão `controllerFor`.
Esta funcionalidade está agora disponível genericamente através de `behaviours.EditableText` e da base `NVDAObject` respectivamente. (#14222)
* A categoria de registo de depuração do UIA, quando activada, produz agora significativamente mais registos para os manipuladores de eventos e utilitários do UIA. (#14256)
* Padrões de construção do NVDAHelper actualizados. (#13072)1
  * Actualmente utiliza a norma C++20, em vez da C++17.
  * Agora utiliza a flag de compilação `/permissive-` que desabilita comportamentos permissivos, e define as opções do compilador `/Zc` para conformidade estrita.
* Alguns objectos de plugin (por exemplo, drivers e extras) têm agora uma descrição mais informativa na consola python do NVDA. (#14463)
* O NVDA agora pode ser totalmente compilado com o Visual Studio 2022, não exigindo mais as ferramentas de compilação do Visual Studio 2019. (#14326)
* Registo mais detalhado dos congelamentos do NVDA para ajudar na depuração. (#14309)
* A classe singleton `braille._BgThread` foi substituída por `hwIo.ioThread.IoThread`. (#14130)
  * Uma única instância `hwIo.bgThread` (no núcleo do NVDA) desta classe fornece i/o em segundo plano para drivers de dispositivos Braille seguros para threads.
  * Esta nova classe não é um singleton por design, os autores de extras são encorajados a usar a sua própria instância quando fazem i/o de hardware.
* A arquitetura do processador do computador pode ser consultada a partir do atributo `winVersion.WinVersion.processorArchitecture.` (#14439)
* Foram adicionados novos pontos de extensão. (#14503)
...
* É possível definir useConfig como False em configurações suportadas para um driver de sintetizador. (#14601)

#### Alterações de quebra da API

Estas são alterações de quebra da API.
Abra um problema no GitHub se o seu Complemento tiver um problema com a atualização para a nova API.

* A especificação da configuração foi alterada, as chaves foram removidas ou modificadas:
  * Na secção `[documentFormatting]` (#14233):
    * `reportLineIndentation` armazena um valor int (0 a 3) em vez de um booleano
    * a opção `reportLineIndentationWithTones` foi removida.
    * `reportBorderStyle` e `reportBorderColor` foram removidos e substituídos por `reportCellBorders`.
  * Na secção `[braille]` (#14233):
    * `noMessageTimeout` foi removido, substituído por um valor para `showMessages`.
    * `messageTimeout` não pode mais receber o valor 0, substituído por um valor para `showMessages`.
    * o `autoTether` foi removido; o `tetherTo` pode agora assumir o valor "auto".
  * Na secção `[teclado]` (#14528):
    * `useCapsLockAsNVDAModifierKey`, `useNumpadInsertAsNVDAModifierKey`, `useExtendedInsertAsNVDAModifierKey` foram removidos.
    São substituídos por `NVDAModifierKeys`.
* A classe `NVDAHelper.RemoteLoader64` foi removida sem substituição. (#14449)
* As seguintes funções em `winAPI.sessionTracking` são removidas sem substituição. (#14416, #14490)
  * `isWindowsLocked`
  * `handleSessionChange`
  * "cancelar o registo
  * `registo`
  * `isLockStateSuccessfullyTracked`
* Já não é possível ativar/desativar o manipulador de braille definindo `braille.handler.enabled`.
Para desativar o manipulador de braille programaticamente, registe um manipulador para `braille.handler.decide_enabled`. (#14503)
* Já não é possível atualizar o tamanho do ecrã do manipulador através da definição de `braille.handler.displaySize`.
Para atualizar o displaySize programaticamente, registe um handler para `braille.handler.filter_displaySize`.
Consulte o `brailleViewer` para um exemplo de como fazer isso. (#14503)
* Houve mudanças no uso do `addonHandler.Addon.loadModule`. (#14481)
  * `loadModule` agora espera ponto como separador, ao invés de barra invertida.
  Por exemplo, "lib.example" em vez de "lib\example".
  * o `loadModule` agora levanta uma exceção quando um módulo não pode ser carregado ou tem erros, ao invés de retornar silenciosamente `None` sem dar informações sobre a causa.
* Os seguintes símbolos foram removidos de `appModules.foobar2000` sem substituição direta. (#14570)
  * `statusBarTimes`
  * `parseIntervalToTimestamp`
  * `getOutputFormat`
  * `getParsingFormat`
* Os seguintes já não são singletons - o seu método get foi removido.
...
* `NVDAObjects.UIA.winConsoleUIA.WinTerminalUIA` está obsoleto e a sua utilização é desencorajada. (#14047)
* `config.addConfigDirsToPythonPackagePath` foi movido.
Use `addonHandler.packaging.addDirsToPythonPackagePath` em vez disso. (#14350)
* `braille.BrailleHandler.TETHER_*` estão obsoletos.
Use `configFlags.TetherTo.*.value` em vez disso. (#14233)
* `utils.security.postSessionLockStateChanged` está obsoleto.
Use `utils.security.post_sessionLockStateChanged` em vez disso. (#14486)
* `NVDAObject.hasDetails`, `NVDAObject.detailsSummary`, `NVDAObject.detailsRole` foram descontinuados.
Utilize `NVDAObject.annotations` em vez disso. (#14507)
* `keyboardHandler.SUPPORTED_NVDA_MODIFIER_KEYS` está obsoleto e não tem substituto direto.
Considere usar a classe `config.configFlags.NVDAKey` em vez disso. (#14528)
* o comando `gui.MainFrame.evaluateUpdatePendingUpdateMenuItemCommand` foi descontinuado.
Use `gui.MainFrame.SysTrayIcon.evaluateUpdatePendingUpdateMenuItemCommand` em vez disso. (#14523)

## 2022.4

Esta versão inclui vários novos comandos principais, inclusive os comandos de leitura contínua em tabelas.
Uma secção "Guia de Início Rápido" foi adicionada ao Manual do utilizador.
Há também várias correções de erros.

eSpeak e LibLouis foram actualizados.
Existem novas tabelas Braille em chinês, sueco, luganda e kinyarwanda.

### Novas funcionalidades

* Adicionada uma secção "Guia de Início Rápido" ao Manual do Utilizador. (#13934)
* Introduzido um novo comando para verificar o atalho de teclado do foco actual. (#13960)
  * Computador de secretária: `shift+numpad2`.
  * Computador portátil: `NVDA+ctrl+shift+.`.
* Introduzidos novos comandos para mover o cursor de revisão por página, quando suportado pela aplicação. (#14021)
  * Mover para a página anterior:
    * Computador de secretária: `NVDA+páginaAcima`.
    * Computador portátil: `NVDA+shift+páginaAcima`.
  * Mover para a página seguinte:
    * Computador portátil: `NVDA+páginaAbaixo`.
    * Computador portátil: `NVDA+shift+páginaAbaixo`.
* Adicionados os seguintes comandos de tabela. (#14070)
  * Lê a coluna da célula actual até ao fim: `NVDA+control+alt+setaAbaixo`
  * Lê a linha da célula actual até ao fim: `NVDA+control+alt+setaDireita`
  * Lê a coluna toda: `NVDA+control+alt+setaAcima`
  * Lê a linha toda: `NVDA+control+alt+setaEsquerda`
* Microsoft Excel usando a UIA: O NVDA agora anuncia quando sai de uma tabela dentro de uma folha de cálculo. (#14165)
* O anúncio de cabeçalhos das tabelas pode agora ser configurado separadamente para linhas e colunas. (#14075)

### Alterações

* eSpeak NG foi actualizado para a versão 1.52-dev commit `735ecdb8`. (#14060, #14079, #14118, #14203)
  * Corrigido o anúncio de caracteres latinos quando se utiliza o mandarim. (#12952, #13572, #14197)
* Actualizado o conversor Braille LibLouis para [3.23.0](https://github.com/liblouis/liblouis/releases/tag/v3.23.0). (#14112)
  * Adicionadas tabelas Braille:
    * Chinês Braille comum (caracteres de chinês simplificado)
    * Ruanda Braille literário
    * Luganda Braille literário
    * Sueco Braille não contraído
    * Sueco Braille parcialmente contraído
    * Sueco Braille contraído
    * Chinês (China, Mandarim) Sistema Braille actual (sem tons) (#14138)
* O NVDA inclui agora a arquitetura do sistema operativo como parte das informações estatísticas do utilizador. (#14019)

### Correcção de erros

* Ao actualizar o NVDA usando o Windows Package Manager CLI (também conhecido como winget), uma versão definitiva do NVDA já não é sempre tratada como mais recente do que qualquer versão alfa instalada. (#12469)
* Agora, o NVDA anuncia corretamente as caixas de grupo em aplicações Java. (#13962)
* O cursor segue correctamente o texto falado durante a "leitura contínua" em aplicações como o Bookworm, WordPad ou o visualizador de registos do NVDA. (#13420, #9179)
* Nos programas que utilizam a UIA, as caixas de verificação parcialmente marcadas serão indicadas correctamente. (#13975)
* Desempenho e estabilidade melhorados no Microsoft Visual Studio, no Windows Terminal e noutras aplicações baseadas na UIA. (#11077, #11209)
  * Estas correções aplicam-se ao Windows 11 Sun Valley 2 (versão 22H2) e posterior.
  * Registo selectivo para eventos de UIA e alterações de propriedades agora activados por predefinição.
* O anúncio de texto, a saída Braille e a supressão de palavra-passe funcionam agora como esperado no controlo incorporado do Terminal do Windows no Visual Studio 2022. (#14194)
* O NVDA agora reconhece o DPI ao usar vários monitores.
Existem várias correções para a utilização de uma definição de DPI superior a 100% ou vários monitores.
Ainda podem existir problemas com versões do Windows anteriores ao Windows 10 1809.
Para que estas correções funcionem, as aplicações com as quais o NVDA interage também têm de ser sensíveis ao DPI.
Note que ainda existem problemas conhecidos com o Chrome e o Edge. (#13254)
  * As molduras de realce visual devem agora ser colocadas correctamente na maioria das aplicações. (#13370, #3875, #12070)
  * A interação com o ecrã tátil deve agora ser precisa para a maioria das aplicações. (#7083)
  * O acompanhamento do rato deve agora funcionar na maioria das aplicações. (#6722)
* As alterações do estado de orientação (paisagem/retrato) são agora correctamente ignoradas quando não há alterações (por exemplo, alterações do monitor). (#14035)
* O NVDA irá anunciar o arrastamento de itens no ecrã em locais como a reorganização dos mosaicos do menu Iniciar do Windows 10 e dos ambientes de trabalho virtuais no Windows 11. (#12271, #14081)
* Nas configurações avançadas, a opção "Reproduzir um som para erros registados: Apenas em versões de teste do NVDA" é agora correctamente restaurada para o seu valor predefinido ao pressionar o botão "Restaurar padrões". (#14149)
* O NVDA já pode seleccionar texto utilizando o atalho de teclado `NVDA+f10` em aplicações Java. (#14163)
* O NVDA já não fica preso num menu quando se desloca para cima e para baixo em conversas encadeadas no Microsoft Teams. (#14355)

### Alterações para Desenvolvedores

Consulte [o guia do desenvolvedor](https://www.nvaccess.org/files/nvda/documentation/developerGuide.html#API) para obter informações sobre o processo de descontinuação e remoção da API do NVDA.

* A lista de discussão [NVDA API Announcement mailing list](https://groups.google.com/a/nvaccess.org/g/nvda-api/about) foi criada. (#13999)
* O NVDA não processa mais eventos `textChange` para a maioria dos aplicativos de automação de interface do utilizador devido ao seu impacto negativo extremo no desempenho. (#11002, #14067)

#### Depreciações

* `core.post_windowMessageReceipt` está obsoleto, utilize `winAPI.messageWindow.pre_handleWindowMessage` em seu lugar.
* `winKernel.SYSTEM_POWER_STATUS` está obsoleto e a sua utilização é desencorajada, tendo sido transferido para `winAPI._powerTracking.SystemPowerStatus`.
* as constantes `winUser.SM_*` estão obsoletas, utilize `winAPI.winUser.constants.SystemMetrics` em vez disso.

## 2022.3.3

Esta é uma versão menor para corrigir problemas com 2022.3.2, 2022.3.1 e 2022.3.
Isto também resolve um problema de segurança.

### Correcções de Segurança

* Impede o possível acesso ao sistema (por exemplo, consola NVDA Python) por parte de utilizadores não autenticados.
([GHSA-fpwc-2gxx-j9v7](https://github.com/nvaccess/nvda/security/advisories/GHSA-fpwc-2gxx-j9v7))

### Correcção de erros

* Corrigido o erro que fazia com que, se o NVDA congelasse ao bloquear, o NVDA permitisse o acesso ao ambiente de trabalho do utilizador enquanto estivesse no ecrã de bloqueio do Windows. (#14416)
* Corrigido o erro que fazia com que, se o NVDA congelasse durante o bloqueio, não se comportasse corretamente, como se o dispositivo ainda estivesse bloqueado (#14416)
* Correção de problemas de acessibilidade no processo "esqueci-me do PIN" do Windows e na experiência de atualização/instalação do Windows. (#14368)
* Corrigido o erro ao tentar instalar o NVDA em alguns ambientes Windows, por exemplo, Windows Server. (#14379)
...
* As seguintes funções em `winAPI.sessionTracking` estão obsoletas para serem removidas na versão 2023.1. (#14416)
  * `isWindowsLocked`
  * `handleSessionChange`
  * "cancelar o registo
  * `registo`
...
Esta é uma versão menor para corrigir regressões com a versão 2022.3.1 e resolver um problema de segurança.

### Correcções de Segurança

* Evita o possível acesso ao nível do sistema por parte de utilizadores não autenticados.
([GHSA-3jj9-295f-h69w](https://github.com/nvaccess/nvda/security/advisories/GHSA-3jj9-295f-h69w))

### Correcção de erros

* Corrige uma regressão da versão 2022.3.1 em que determinadas funcionalidades eram desactivadas em ecrãs seguros. (#14286)
* Corrige uma regressão da versão 2022.3.1 em que determinadas funcionalidades eram desactivadas após o início de sessão, se o NVDA fosse iniciado no ecrã de bloqueio. (#14301)

## 2022.3.1

Esta é uma versão menor para corrigir vários problemas de segurança.
Por favor, divulgue de forma responsável os problemas de segurança para <info@nvaccess.org>.

### Correcções de Segurança

* Corrigida a exploração em que era possível passar de privilégios de utilizador para privilégios de sistema.
([GHSA-q7c2-pgqm-vvw5](https://github.com/nvaccess/nvda/security/advisories/GHSA-q7c2-pgqm-vvw5))
* Foi corrigido um problema de segurança que permitia o acesso à consola python no ecrã de bloqueio através de uma condição de corrida para o arranque do NVDA.
([GHSA-72mj-mqhj-qh4w](https://github.com/nvaccess/nvda/security/advisories/GHSA-72mj-mqhj-qh4w))
* Foi corrigido o problema em que o texto do visualizador de voz era colocado em cache ao bloquear o Windows.
([GHSA-grvr-j2h8-3qm4](https://github.com/nvaccess/nvda/security/advisories/GHSA-grvr-j2h8-3qm4))

### Correcção de erros

* Impedir que um utilizador não autenticado actualize as definições do visualizador de voz e de Braille no ecrã de bloqueio. ([GHSA-grvr-j2h8-3qm4](https://github.com/nvaccess/nvda/security/advisories/GHSA-grvr-j2h8-3qm4))

## 2022.3

Uma parte significativa desta versão foi contribuída pela comunidade de desenvolvimento NVDA.
Isto inclui descrições de caracteres atrasadas e suporte melhorado para a Consola do Windows.

Esta versão também inclui várias correções de erros.
Nomeadamente, as versões actualizadas do Adobe Acrobat/Reader já não irão falhar ao ler um documento PDF.

o eSpeak foi atualizado, com a introdução de 3 novas línguas: Bielorrusso, Luxemburguês e Totontepec Mixe.

### Novas funcionalidades

* No Anfitrião da Consola do Windows utilizado pelo Prompt de Comando, PowerShell e Subsistema do Windows para Linux no Windows 11 versão 22H2 (Sun Valley 2) e posterior:
  * Desempenho e estabilidade muito melhorados. (#10964)
  * Ao pressionar `control+f` para encontrar texto, a posição do cursor de revisão é actualizada para seguir o termo encontrado. (#11172)
  * A leitura de texto digitado, que não aparece no ecrã (como palavras-passe) está desativada por predefinição.
Pode ser reativada no painel de definições avançadas do NVDA. (#11554)
  * O texto que foi deslocado para fora do ecrã pode ser revisto sem deslocar a janela da consola. (#12669)
  * Estão disponíveis informações mais pormenorizadas sobre a formatação do texto. ([microsoft/terminal PR 10336](https://github.com/microsoft/terminal/pull/10336))
* Foi adicionada uma nova opção de Discurso para ler descrições de caracteres após um atraso. (#13509)
* Foi adicionada uma nova opção Braille para determinar se a deslocação do ecrã para a frente/para trás deve interromper a voz. (#2124)

### Alterações

* o eSpeak NG foi atualizado para a versão 1.52-dev commit `9de65fcb`. (#13295)
  * Idiomas adicionados:
    * Belarusian
    * Luxemburguês
    * Totontepec Mixe
* Ao utilizar a automatização da interface do utilizador para aceder aos controlos da folha de cálculo do Microsoft Excel, o NVDA pode agora informar quando uma célula é combinada. (#12843)
* Em vez de indicar "tem detalhes", o objetivo dos detalhes é incluído sempre que possível, por exemplo "tem comentário". (#13649)
* O tamanho da instalação do NVDA é agora mostrado na secção Programas e Funcionalidades do Windows. (#13909)

### Correcção de erros

* O Adobe Acrobat / Reader de 64 bits não irá mais falhar ao ler um documento PDF. (#12920)
  * Note-se que a versão mais actualizada do Adobe Acrobat / Reader também é necessária para evitar a falha.
* As medidas de tamanho de letra são agora traduzíveis no NVDA. (#13573)
* Ignora os eventos do Java Access Bridge quando não é possível encontrar um identificador de janela para aplicações Java.
Isto irá melhorar o desempenho de algumas aplicações Java, incluindo o IntelliJ IDEA. (#13039)
* O anúncio de células selecionadas para o LibreOffice Calc é mais eficiente e já não resulta num congelamento do Calc quando estão selecionadas muitas células. (#13232)
* Quando executado com um utilizador diferente, o Microsoft Edge já não está inacessível. (#13032)
* Quando o aumento de taxa está desativado, a taxa do eSpeak já não desce entre as taxas 99% ae 100%. (#13876)
* Correção do erro que permitia a abertura de 2 diálogos de definir comandos. (#13854)

### Alterações para Desenvolvedores

* Atualização do Comtypes para a versão 1.1.11. (#12953)
* Em compilações do Console do Windows (`conhost.exe`) com um nível de API NVDA de 2 (`FORMATTED`) ou superior, como as incluídas no Windows 11 versão 22H2 (Sun Valley 2), a automação da interface do utilizador agora é usada por padrão. (#10964)
  * Isto pode ser anulado alterando a definição "Suporte da consola do Windows" no painel de definições avançadas do NVDA.
  * Para encontrar o nível de API NVDA da Consola do Windows, defina "Suporte da Consola do Windows" para "UIA quando disponível" e, em seguida, verifique o registo NVDA+F1 aberto a partir de uma instância da Consola do Windows em execução.
* O buffer virtual do Chromium é agora carregado mesmo quando o objeto documento tem o MSAA `STATE_SYSTEM_BUSY` exposto via IA2. (#13306)
* Uma especificação de configuração do tipo `featureFlag` foi criada para uso com recursos experimentais no NVDA. Veja `devDocs/featureFlag.md` para mais informações. (#13859)

#### Depreciações

Não são propostas depreciações em 2022.3.

## */*2022.2.4

Esta é uma versão de patch para corrigir um problema de segurança.

### Correcção de Erros

* Correção de uma exploração em que era possível abrir o console python do NVDA por meio do visualizador de logs na tela de bloqueio.
([GHSA-585m-rpvv-93qg](https://github.com/nvaccess/nvda/security/advisories/GHSA-585m-rpvv-93qg))

## 2022.2.3

Esta é uma versão de correção para corrigir uma quebra acidental de API introduzida na versão 2022.2.1.

### Correcção de Erros

* Foi corrigido um erro em que o NVDA não anunciava "Área de trabalho segura" ao entrar em uma área de trabalho segura.
Isso fazia com que o NVDA remote não reconhecesse áreas de trabalho seguras. (#14094)

## 2022.2.2

Esta é uma versão de correção para corrigir um erro introduzido na versão 2022.2.1 com gestos de entrada.

### Correcção de Erros

* Corrigido um erro em que os comandos nem sempre funcionavam. (#14065)

## 2022.2.1

Esta é uma versão secundária para corrigir um problema de segurança.
Por favor, divulgue de forma responsável os problemas de segurança para <info@nvaccess.org>.

### Correções de segurança

* Correção da exploração em que era possível executar um console python a partir da tela de bloqueio. (GHSA-rmq3-vvhq-gp32)
* Correção do exploit em que era possível escapar da tela de bloqueio usando a navegação por objetos. (GHSA-rmq3-vvhq-gp32)

### Alterações para desenvolvedores

#### Depreciações

Essas depreciações não estão programadas para remoção no momento.
Os aliases obsoletos permanecerão até segunda ordem.
Teste a nova API e forneça feedback.
Para autores de extras, abra um problema no GitHub se essas alterações impedirem que a API atenda às suas necessidades.

* `appModules.lockapp.LockAppObject` deve ser substituído por `NVDAObjects.lockscreen.LockScreenObject`. (GHSA-rmq3-vvhq-gp32)
* o `appModules.lockapp.AppModule.SAFE_SCRIPTS` deve ser substituído por `utils.security.getSafeScripts()`. (GHSA-rmq3-vvhq-gp32)

## 2022.2

Esta versão inclui várias correções de erros.
Notavelmente, há melhorias significativas para aplicativos baseados em Java, dispositivos braile e recursos do Windows.

Foram introduzidos novos comandos de navegação na tabela.
O O Unicode CLDR foi atualizado.
A LibLouis foi atualizada e inclui uma nova tabela em braile alemão.

### Novas funcionalidades

* Suporte para interação com componentes do Microsoft Loop em produtos do Microsoft Office. (#13617)
* Novos comandos de navegação de tabela foram adicionados. (#957)
  * `control+alt+home/end` to jump to first/last column.
  * `control+alt+pageUp/pageDown` to jump to first/last row.
* Foi adicionado um script não atribuído para percorrer os modos de alternância de idioma e dialeto. (#10253)

### Alterações

* O NSIS foi atualizado para a versão 3.08. (#9134)
* O CLDR foi atualizado para a versão 41.0. (#13582)
* Atualização do tradutor de braile LibLouis para [3.22.0](https://github.com/liblouis/liblouis/releases/tag/v3.22.0). (#13775)
  * Nova tabela braille: Alemão grau 2 (detalhado)
* Adicionada nova função para os controles do "indicador de ocupado". (#10644)
* O NVDA agora anuncia quando uma ação do NVDA não pode ser executada. (#13500)
  * Isso inclui quando:
    * A usar a versão do NVDA para Windows Store.
    * Num contexto seguro.
    * A aguardar uma resposta a uma caixa de diálogo modal.

### Correcção de Erros

* Correções para aplicativos baseados em Java:
  * O NVDA agora anunciará o estado somente leitura. (#13692)
  * O NVDA agora anunciará corretamente o estado desativado/ativado. (#10993)
  * O NVDA agora anunciará os atalhos das teclas de função. (#13643)
  * O NVDA agora pode emitir um bipe ou falar nas barras de progresso. (#13594)
  * O NVDA não removerá mais incorretamente o texto dos widgets ao apresentá-los ao usuário. (#13102)
  * O NVDA agora anunciará o estado dos botões de alternância. (#9728)
  * O NVDA agora identificará a janela num aplicativo Java com várias janelas. (#9184)
  * O NVDA agora anunciará as informações de posição dos controlos de separadores. (#13744)
* */*Correções em braile:
...
* Agora há suporte para a compilação de dependências do NVDA com o Visual Studio 2022 (17.0).
Para compilações de desenvolvimento e lançamento, o Visual Studio 2019 ainda é usado. (#13033)
* Ao recuperar a contagem de crianças selecionadas através de accSelection,
...
Podem ser utilizados para utilizar um único módulo de aplicação com vários executáveis. (#13366)

#### Obsoletos

Estas são propostas de alterações de rutura da API.
A parte obsoleta da API continuará a estar disponível até à versão especificada.
Se não for especificada qualquer libertação, o plano de remoção não foi determinado.
Note-se que o roteiro para as remoções é o "melhor esforço" e pode estar sujeito a alterações.
Teste a nova API e dê o seu feedback.
...
O código que importa de um deles deve, em vez disso, importar do módulo de substituição.  (#13366)
...
Existem também várias correções de erros do Windows 11, para aplicações como a Calculadora, a Consola, o Terminal, o Mail e o Painel de Emoji.
...
* Resolvido o aviso de segurança `GHSA-xc5m-v23f-pgr7`.
  * O diálogo de pronúncia de símbolos está agora desactivado no modo de segurança.
...
* Resolvido o aviso de segurança `GHSA-354r-wr4v-cx28`. (#13488)
  * Removida a capacidade de iniciar o NVDA com o registo de depuração ativado quando o NVDA é executado em modo seguro.
  * Removida a capacidade de actualizar o NVDA quando o NVDA é executado em modo seguro.
* Resolvido o aviso de segurança `GHSA-wg65-7r23-h6p9`. (#13489)
  * Removida a capacidade de abrir a a janela Definir comandos no modo de segurança.
  * Removida a capacidade de abrir os dicionários, qualquer que seja, em modo de segurança.
* Resolvido o aviso de segurança `GHSA-mvc8-5rv9-w3hx`. (#13487)
  * A ferramenta de inspeção de GUI wx está agora desactivada no modo seguro.

## 2021.3.3

Esta versão é idêntica à 2021.3.2.
Existia um erro no NVDA 2021.3.2 em que este se identificava incorrectamente como 2021.3.1.
Esta versão identifica-se correctamente como 2021.3.3.
...
* Correcção de segurança: Impedir a navegação de objectos para fora do ecrã de bloqueio no Windows 10 e no Windows 11. (#13328)
* Correcção de segurança: A janela do gestor de extras está agora desactivada em ecrãs seguros. (#13059)
* Correcção de segurança: A ajuda de contexto do NVDA já não está disponível em ecrãs seguros. (#13353)

## 2021.3.1

Esta é uma versão menor para corrigir vários problemas na versão 2021.3.

### Alterações

* O novo protocolo HID Braille já não é preferido quando pode ser utilizado outro driver para o dispositivo Braille. (#13153)
* O novo protocolo HID Braille pode ser desactivado através de uma opção na secção Avançadas das configurações do NVDA. (#13180)

### Correcção de erros

* A região é novamente abreviada em Braille. #13158
* Correcção da instabilidade na detecção automática dos dispositivos Braille Humanware Brailliant e APH Mantis Q40, quando é utilizada a ligação Bluetooth (#13153)

## 2021.3

Esta versão introduz o suporte para a nova especificação HID Braille.
Esta especificação tem por objetivo normalizar o suporte dos dispositivos Braille sem necessidade de controladores individuais.
Há actualizações do eSpeak-NG e do LibLouis, incluindo novas tabelas em russo e tshivenda.
Os sons de erro podem ser activados em versões estáveis do NVDA através de uma nova opção nas configurações avançadas do NVDA.
A leitura contínua no Word agora desloca o texto para manter visível a posição actual.
Existem muitas melhorias na utilização do Office com o UIA.
Uma correção do UIA é o facto do Outlook ignorar mais tipos de tabelas de apresentação nas mensagens.

Notas importantes:

Devido a uma actualização do nosso certificado de segurança, um pequeno número de utilizadores pode receber um erro quando o NVDA 2021.2 procura actualizações.
O NVDA solicita ao Windows para actualizar os certificados de segurança, o que evitará este erro no futuro.
Os utilizadores afectados terão de descarregar esta actualização manualmente.

### Novas funcionalidades

* Adiciona um comando para alternar as definições para anunciar o estilo dos limites das células. (#10408)
* Suporte para a nova especificação HID Braille que tem como objectivo normalizar o suporte para dispositivos Braille. (#12523)
  * Os dispositivos que suportam esta especificação serão detectados automaticamente pelo NVDA.
...
* Adicionar suporte para o dispositivo Braille VisioBraille Vario 4. (#12607)
* As notificações de erros podem ser activadas (configurações avançadas) quando se utiliza qualquer versão do NVDA. (#12672)
* No Windows 10 e posterior, o NVDA anunciará a contagem de sugestões ao introduzir termos de pesquisa em aplicações como as Definições e a Microsoft Store. (#7330, #12758, #12790)
* A navegação em tabelas é agora suportada em controlos de grelha criados utilizando o Out-GridView cmdlet no PowerShell. (#12928)

### Alterações

* Espeak-ng actualizado para 1.51-dev commit `74068b91bcd578bd7030a7a6cde2085114b79b44`. (#12665)
* O NVDA irá usar o eSpeak por defeito se nenhuma voz OneCore instalada suportar o idioma preferido do NVDA. (#10451)
* Se as vozes OneCore falharem constantemente, volte a utilizar o eSpeak como sintetizador. (#11544)
* Ao ler a barra de estado com `NVDA+end`, o cursor de revisão já não é movido para a sua localização.
Se precisar desta funcionalidade, atribua um comando ao script apropriado na categoria Navegação por objectos na janela Definir comandos. (#8600)
* Ao tentar abrir uma janela de configurações que já está aberta, o NVDA passa a colocar o foco na janela já existente em vez de apresentar um erro. (#5383)
* Conversor de Braille liblouis actualizado para [3.19.0](https://github.com/liblouis/liblouis/releases/tag/v3.19.0). (#12810)
  * Novas tabelas braille: Russo grau 1, Tshivenda grau 1, Tshivenda grau 2
* Em vez de "conteúdo marcado" ou "mrkd", será anunciado "Realce" ou "realc" para a fala e o Braille, respectivamente. (#12892)
* O NVDA já não tentará sair quando as caixas de diálogo estiverem a aguardar uma ação necessária (por exemplo, Confirmar/Cancelar). (#12984)

### Correcção de erros

* Reconhecer teclas modificadoras (como Control, ou Insert) é mais robusto quando o watchdog está a recuperar. (#12609)
* É novamente possível verificar por actualizações do NVDA em certos sistemas; por exemplo, instalações limpas do Windows. (#12729)
* O NVDA anuncia corretamente células de tabela em branco no Microsoft Word ao utilizar a automatização da IU. (#11043)
* Nas células da grelha de dados ARIA na Web, a tecla Escape será agora transmitida para a grelha e deixará de desligar incondicionalmente o modo de foco. (#12413)
* Ao ler uma célula de cabeçalho de uma tabela no Chrome, corrigida a repetição do nome da coluna. (#10840)
* O NVDA já não reporta um valor numérico para os controlos deslizantes UIA que têm uma representação textual do seu valor definido. (O UIA ValuePattern é agora preferível ao RangeValuePattern). (#12724)
* O NVDA já não trata o valor dos controlos deslizantes UIA como sendo sempre baseado em percentagem.
* O anúncio da localização de uma célula no Microsoft Excel (quando acedido através da Automatização da IU) volta a funcionar corretamente no Windows 11. (#12782)
* O NVDA já não define locais inválidos para o Python. (#12753)
* Se um extra desactivado for removido e depois reinstalado, é novamente activado. (#12792)
* Corrigidos os erros relativos à actualização e remoção de extras onde a pasta do extra foi renomeada ou tem ficheiros abertos. (#12792, #12629)
* Ao utilizar a automatização da interface do utilizador para aceder aos controlos da folha de cálculo do Microsoft Excel, o NVDA já não anuncia de forma redundante quando uma única célula é selecionada. (#12530)
* Mais texto de janelas é lido automaticamente no LibreOffice Writer, como as de confirmação. (#11687)
* Ler / navegar com o modo de navegação no Microsoft Word através da automatização da interface do utilizador garante agora que o documento é sempre deslocado para que a posição atual do modo de navegação seja visível e que a posição do cursor no modo de foco reflecte correctamente a posição do modo de navegação. (#9611)
* Ao executar o Ler tudo no Microsoft Word através da automatização da IU, o documento é agora automaticamente deslocado e a posição do cursor é correctamente actualizada. (#9611)
* Ao ler e-mails no Outlook, e o NVDA estiver a aceder à mensagem com a Automatização da IU, determinadas tabelas passam a ser marcadas como tabelas de layout, o que significa que já não serão comunicadas por predefinição. (#11430)
* Corrigido um erro raro ao mudar de dispositivo de áudio. (#12620)
* A introdução usando tabelas de Braille literário deve comportar-se de forma mais fiável nos campos de edição. (#12667)
* Ao navegar no calendário da bandeja do sistema Windows, o NVDA agora informa o dia da semana por completo. (#12757)
* Quando se utiliza um método de introdução chinês, como o Taiwan - Microsoft Quick no Microsoft Word, o deslocamento do ecrã braille para a frente e para trás deixa de saltar incorretamente para a posição original do cursor. (#12855)
* Ao aceder a documentos do Microsoft Word através do UIA, a navegação por frase (alt+seta para baixo / alt+seta para cima) voltou a ser possível. (#9254)
* Ao aceder ao MS Word com o UIA, a indentação de parágrafos é agora comunicada. (#12899)
* Ao aceder ao MS Word com o UIA, o comando de seguimento de alterações e alguns outros comandos localizados são agora anunciados no Word. (#12904)
* Corrigida a duplicação do Braille e da voz quando a "descrição" corresponde ao "conteúdo" ou ao "nome". (#12888)
* No MS Word com o UIA ativado, a reprodução de sons de erros ortográficos é mais precisa à medida que escreve. (#12161)
* No Windows 11, o NVDA já não anuncia "painel" ao premir Alt+Tab para alternar entre programas. (#12648)
* O novo painel lateral Comentários modernos é agora suportado no MS Word quando não se acede ao documento através do UIA. Prima alt+f12 para se deslocar entre o painel lateral e o documento. (#12982)
...
Esta versão introduz o suporte preliminar ao Windows 11.
Embora o Windows 11 ainda não tenha sido lançado, esta versão foi testada em versões de pré-visualização do Windows 11.
Isto inclui uma correção importante para a Cortina de ecrã (ver notas importantes).
A ferramenta de correção de COM Registration pode agora resolver mais problemas na utilização do NVDA.
Há actualizações para o sintetizador eSpeak e para o conversor Braille LibLouis.
Há também várias correções de erros e melhorias, nomeadamente no suporte Braille e nos terminais Windows, calculadora, painel de emojis e histórico da área de transferência.
...
Utilize o NVDA 2021.2 para activar a "Cortina de ecrã" com o Windows 10 21H2 (10.0.19044) ou posterior.
Isto inclui o Windows 10 Insiders e o Windows 11.
Por motivos de segurança, quando utilizar uma nova versão do Windows, obtenha uma confirmação visual de que a "Cortina de ecrã" torna o ecrã totalmente preto.

### Novas funcionalidades

* Suporte experimental para anotações ARIA:
  * adiciona um comando para ler um resumo dos detalhes de um objeto com aria-details. (#12364)
  * adiciona uma opção nas configurações avançadas para informar se um objecto tem detalhes no modo de navegação. (#12439)
* No Windows 10 Versão 1909 e posterior (incluindo o Windows 11), o NVDA anunciará a contagem de sugestões ao efetuar pesquisas no Explorador de Ficheiros. (#10341, #12628)
* No Microsoft Word, o NVDA agora anuncia o resultado dos atalhos de recuo e recuo suspenso quando executados. (#6269)

### Alterações

* Espeak-ng actualizado para o commit 1.51-dev `ab11439b18238b7a08b965d1d5a6ef31cbb05cbb`. (#12449, #12202, #12280, #12568)
* Se o artigo estiver activado nas configurações do utilizador para a formatação de documentos, o NVDA anuncia "artigo" após o conteúdo. (#11103)
* Actualizado o conversor de braille liblouis para [3.18.0](https://github.com/liblouis/liblouis/releases/tag/v3.18.0). (#12526)
  * Novas tabelas braille: Búlgaro grau 1, Birmanês grau 1, Birmanês grau 2, Cazaque grau 1, Khmer grau 1, Curdo do Norte grau 0, Sepedi grau 1, Sepedi grau 2, Sesotho grau 1, Sesotho grau 2, Setswana grau 1, Setswana grau 2, Tatar grau 1, Vietnamita grau 0, Vietnamita grau 2, Vietnamita do Sul grau 1, Xhosa grau 1, Xhosa grau 2, Yakut grau 1, Zulu grau 1, Zulu grau 2
* O Windows 10 OCR foi renomeado para Windows OCR. (#12690)

### Correcção de erros

* Na Calculadora do Windows 10, o NVDA anunciará as expressões da calculadora num dispositivo Braille. (#12268)
* Em programas de terminal no Windows 10 versão 1607 e posterior, ao inserir ou excluir caracteres no meio de uma linha, os caracteres à direita do cursor não são mais lidos. (#3200)
  * O Diff Match Patch está agora activado por padrão. (#12485)
* A escrita em Braille funciona corretamente com as seguintes tabelas contraídas: Árabe grau 2, Espanhol grau 2, Urdu grau 2, Chinês (China, Mandarim) grau 2. (#12541)
* A Ferramenta de correção de Registo COM resolve agora mais problemas, especialmente no Windows de 64 bits. (#12560)
* Melhorias no manuseamento de botões para o dispositivo Braille Seika Notetaker da Nippon Telesoft (#12598)
* Melhorias para o anúncio do painel de emoji do Windows e o histórico da área de transferência. (#11485)
* Actualizadas as descrições dos caracteres do alfabeto Bengali. (#12502)
* O NVDA sai com segurança quando um novo processo é iniciado. (#12605)
* Voltar a selecionar o driver de dispositivo Braille Handy Tech na caixa de diálogo Seleccionar dispositivo Braille já não provoca erros. (#12618)
* O Windows versão 10.0.22000 ou posterior é reconhecido como Windows 11, não como Windows 10. (#12626)
* O suporte para "cortinas de ecrã" foi corrigido e testado para versões do Windows até 10.0.22000. (#12684)
* Se não forem apresentados resultados ao filtrar os comandos, a janela de definição dos comandos continua a funcionar como esperado. (#12673)
* Foi corrigido um erro em que o primeiro item de menu de um submenu não era anunciado em alguns contextos. (#12624)
...
Além de muitas correções de erros e melhorias, incluindo no Office, Visual Studio e em vários idiomas.
...
* Easier navigation of output in NVDA Python Console. (#9784)
  * alt+Seta acima/abaixo salta para o resultado de saída anterior/seguinte (adicione shift para selecionar).
...
* Actualizado o Unicode Common Locale Data Repository (CLDR) para 39.0. (#11943, #12314)
...
* Agora é anunciado "Não suportado" quando se tenta alternar o layout do ecrã em aplicações que não o suportam, como o Microsoft Word. (#7297)
* a opção "Tentar cancelar a voz para eventos de foco expirados", na secção de configurações avançadas, está agora activada por predefinição. (#10885)
  * Este comportamento pode ser desactivado definindo esta opção como "Não".
...
* O suporte para conteúdo Adobe Flash foi removido do NVDA devido ao facto de a utilização do Flash ser activamente desencorajada pela Adobe. (#11131)
...
* Na janela Lista de elementos a tecla de aceleração no botão "Activar" foi removida em alguns idiomas, para evitar conflitos com alguuma etiqueta de botão de opção de tipo de elemento. Quando disponível, o botão continua a ser a predefinição da caixa de diálogo e, como tal, ainda pode ser invocado premindo simplesmente enter a partir da própria lista de elementos. (#6167)
...
* In terminal programs on Windows 10 version 1607 and later, when inserting or deleting characters in the middle of a line, the characters to the right of the caret are no longer read out. (#3200)
  * Esta correcção experimental tem de ser activada manualmente no painel de configurações avançadas do NVDA, alterando o algoritmo "diff" para Diff Match Patch.
* No MS Outlook, o anúncio inadequado de distância ao pressionar Tab para se deslocar do corpo da mensagem para o campo de assunto já não deve ocorrer. (#10254)
* Na Consola Python, inserir um Tab, para indentação, no início de uma linha, não vazia, de entrada e executar o preenchimento de tab no meio de uma linha de entrada são agora suportados. (#11532)
...
* Já é possível ler comentários no MS Word com o UIA activado. (#9285)
* Performance when interacting with Visual Studio has been improved. (#12171)
* Corrigidos erros gráficos, tais como elementos em falta quando se utiliza o NVDA com uma disposição da direita para a esquerda. (#8859)
* Respeitar a direcção da disposição da GUI com base no idioma do NVDA e não no idioma do sistema. (#638)
  * known issue for right-to-left languages: the right border of groupings clips with labels/controls. (#12181)
* O idioma do python está definido para corresponder ao idioma seleccionado nas configurações de forma consistente, e irá ocorrer quando utilizar o idioma predefinido. (#12214)
* TextInfo.getTextInChunks no longer freezes when called on Rich Edit controls such as the NVDA log viewer. (#11613)
* É mais uma vez possível usar o NVDA em idiomas que contêm sublinhado no nome do idioma, como de_CH, no Windows 10 1803 e 1809. (#12250)
* No WordPad, a configuração de anúncio de sobrescrito/subscrito funciona como esperado. (#12262)
...
* Os atributos de formatação riscado, sobrescrito e subscrito para células inteiras do Excel são agora anunciados se a opção correspondente estiver activada. (#12264)
* Corrigida a cópia das configurações durante a instalação, a partir de uma cópia portátil, quando a pasta padrão de destino das configurações está vazia. (#12071, #12205)
...
* No MS Word ou Outlook, a tecla de navegação rápida para tabelas pode agora saltar para a tabela de apresentação se a opção "Incluir tabelas de apresentação" estiver activada nas definições do modo de navegação. (#11899)
* NVDA will no longer announce "↑↑↑" for emojis in particular languages. (#11963)
* O eSpeak suporta novamente o Cantonês e o Mandarim. (#10418)
* In the new Chromium based Microsoft Edge, text fields such as the address bar are now announced when empty. (#12474)
* Corregido o driver Seika Braille. (#10787)

### Alterações para Desenvolvedores

* Nota: esta é uma versão que quebra a compatibilidade da API de extras. Os extras terão de ser testados novamente e ter o manifesto actualizado.
* NVDA's build system now fetches all Python dependencies with pip and stores them in a Python virtual environment. This is all done transparently.
  ...
  * Para realizar testes unitários, execute `rununittests.bat [<extra unittest discover options>]`
  * Para efetuar testes de sistema: executar `runsystemtests.bat [<extra robot options>]`
  * Para realizar o linting, execute `runlint.bat <base branch>`
...
Correcções de erros e melhorias no Mail, Outlook, Teams, Visual Studio, Azure Data Studio, Foobar2000.
On the web, there are improvements to Google Docs, and greater support for ARIA.
Além de muitas outras correções de erros e melhorias importantes.
...
Os destaques desta versão incluem suporte para um novo dispositivo Braille da Nattiq, melhor suporte para a GUI do antivírus ESET e o Terminal do Windows, melhorias de desempenho no 1Password e no sintetizador do Windows OneCore. Além de muitas outras correções de erros e melhorias importantes.
...
Os destaques desta versão incluem o suporte para vários novos dispositivos Braille da HumanWare e da APH, além de muitas outras correções de erros importantes, como a capacidade de ler novamente matemática no Microsoft Word utilizando o MathPlayer / MathType.
...
O NVDA 2019.3 é uma versão muito significativa que contém muitas alterações de fundo, incluindo a actualização do Python 2 para o Python 3 e uma grande reformulação do subsistema de voz do NVDA.
Embora estas alterações quebrem a compatibilidade com extras do NVDA mais antigos, a actualização para Python 3 é necessária para a segurança, e as alterações ao subsistema de voz permitem algumas inovações interessantes num futuro próximo.
 Outros destaques desta versão incluem o suporte de 64 bits para Java VMs, a funcionalidade Cortina de ecrã e Realce visual, o suporte para mais dispositivos Braille e um novo visualizador Braille, e muitas outras correções de erros.

### Novas funcionalidades

* The accuracy of the move mouse to navigator object command has been improved in text fields in Java applications. (#10157)
* Adicionado suporte para os seguintes dispositivos Braille Handy Tech (#8955):
  * Basic Braille Plus 40
  * Basic Braille Plus 32
  * Connect Braille
* Todos os comandos definidos pelo utilizador podem agora ser removidos através de um novo botão "Restaurar para o padrão original" na janela Definir comandos. (#10293)
* O anúncio de fontes no Microsoft Word agora inclui se o texto está marcado como oculto. (#8713)
...
* No Internet Explorer, Microsoft Edge e versões recentes do Firefox e Chrome, as marcas já são anunciadas no modo de foco e de navegação por objectos. (#10101)
* No Internet Explorer, Google Chrome e Mozilla Firefox, é agora possível navegar por artigos e agrupamentos utilizando teclas de navegação rápida. Estes comandos não estão atribuídos por defeito e podem ser atribuídos na janela Definir comandos quando a janela é aberta a partir de um documento em modo de navegação. (#9485, #9227)
  * As figuras também são anunciadas. São consideradas objectos e, como tal, navegáveis com a tecla de navegação rápida "o".
* No Internet Explorer, no Google Chrome e no Mozilla Firefox, os artigos já são anunciados com a navegação por objectos e, opcionalmente, no modo de navegação, se estiverem marcados nas definições de formatação de documentos. (#10424)
* Adicionada a cortina de ecrã que, quando activada, torna todo o ecrã preto no Windows 8 e posterior. (#7857)
  * Adicionado um comando para activar a cortina de ecrã (até ao próximo reinício com um pressionamento, ou sempre enquanto o NVDA estiver a funcionar com dois pressionamentos), sem teclas atribuídas por predefinição.
  * Pode ser activada e configurada através da categoria "visão" nas configurações do NVDA.
* Adicionada a funcionalidade de realce visual ao NVDA. (#971, #9064)
  * O realce do foco, do objecto de navegação e da posição do cursor no modo de navegação podem ser activados e configurados através da categoria "visão" nas configurações do NVDA.
  * Nota: Esta funcionalidade é incompatível com o extra de realce de foco, no entanto, o extra pode ser utilizado enquanto a funcionalidade incorporada estiver desactivada.
* Adicionada a ferramenta Visualizador Braille, que permite visualizar a saída em Braille através de uma janela no ecrã. (#7788)

### Alterações

* O manual do utilizador descreve agora como utilizar o NVDA na Consola do Windows. (#9957)
...
* No Windows 8 e posterior, o NVDA irá agora anunciar o nome do produto e as informações da versão para aplicações alojadas, tais como aplicações transferidas da Microsoft Store, utilizando as informações fornecidas pela aplicação. (#4259, #10108)
* Ao activar e desactivar as alterações de faixa com o teclado no Microsoft Word, o NVDA anunciará o estado da definição. (#942)
...
* A janela de Configurações já não permite alterar o nível de registo configurado se este tiver sido substituído a partir da linha de comandos. (#10209)
...
* Actualizado o conversor de Braille Liblouis para o commit 58d67e63. (#10094)
* Quando o anúncio de caracteres CLDR (incluindo emojis) está activada, estes são anunciados em todos os níveis de pontuação. (#8826)
...
* Se o Java Access Bridge não estiver activado para o utilizador, o NVDA activa-o automaticamente no arranque do NVDA. (#7952)
* Actualizado eSpeak-NG para 1.51-dev, commit ca65812ac6019926f2fbd7f12c92d7edd3701e0c. (#10581)
...
* No Windows 10, o NVDA anunciará as dicas de ferramentas das aplicações universais se o NVDA estiver configurado para comunicar as dicas de ferramentas na secção Aapresentação de objectos das Configurações. (#8118)
* Na Actualização de Aniversário do Windows 10 e posterior, o texto digitado já é anunciado no Mintty. (#1348)
* Na Actualização de Aniversário do Windows 10 e posterior, a saída na Consola do Windows que aparece perto do acento circunflexo já não é escrita. (#513)
...
* O NVDA impedirá que o sistema entre no modo de suspensão ao percorrer o texto com comandos do dispositivo Braille. (#9175)
* No Windows 10, o Braille segue agora a edição do conteúdo das células no Microsoft Excel e noutros controlos de texto UIA em que estava a ficar para trás. (#9749)
* NVDA will once again report suggestions in the Microsoft Edge address bar. (#7554)
* O NVDA já não fica silencioso quando se foca um cabeçalho de controlo de separador HTML no Internet Explorer. (#8898)
...
* O NVDA já não anuncia o conteúdo de colunas visualmente ocultas para itens de lista em controlos SysListView32. (#8268)
* A janela de configurações já não mostra "info" como o nível de registo actual quando em modo seguro. (#10209)
* In Start menu for Windows 10 Anniversary Update and later, NVDA will announce details of search results. (#10340)
* No modo de navegação, se ao mover o cursor ou utilizar a navegação rápida fizer com que o documento mude, o NVDA já não fala conteúdo incorreto em alguns casos. (#8831, #10343)
* Some bullet names in Microsoft Word have been corrected. (#10399)
* Na actualização do Windows 10 de maio de 2019 e posterior, o NVDA anunciará novamente o primeiro emoji ou item da área de transferência selecionado quando o painel de emoji e o histórico da área de transferência forem abertos, respetivamente. (#9204)
...
* Alguns problemas raros de conversão em Braille e erros com o liblouis foram resolvidos. (#9982)
...
* No Windows 10 versão 1607 e posterior, os caracteres digitados por teclados Braille são falados em mais situações. (#10569)
* Ao alterar o dispositivo de saída de áudio, os tons reproduzidos pelo NVDA passam a ser reproduzidos através do novo dispositivo seleccionado. (#2167)
* No Mozilla Firefox, mover o foco no modo de navegação é mais rápido. Isto torna o movimento do cursor no modo de navegação mais reactivo em muitos casos. (#10584)

### Alterações para Desenvolvedores

* Actualização do Python para 3.7. (#7105)
* Actualização do pySerial para a versão 3.4. (#8815)
* Actualizado wxPython para 4.0.3 para suportar Python 3.5 e posterior. (#9630)
* Actualizado seis para a versão 1.12.0. (#9630)
* Actualizado py2exe para a versão 0.9.3.2 (em desenvolvimento, commit b372a8e de albertosottile/py2exe#13). (#9856)
* Actualização do módulo UIAutomationCore.dll comtypes para a versão 10.0.18362. (#9829)
* A completação do tab na consola Python apenas sugere atributos que começam com um sublinhado se o sublinhado for digitado pela primeira vez. (#9918)
* Flake8 linting tool has been integrated with SCons reflecting code requirements for Pull Requests. (#5918)
* Como o NVDA já não depende do pyWin32, módulos como win32api e win32con já não estão disponíveis para extras. (#9639)
...
* A função Braille getBrailleTextForProperties foi renomeada para getPropertiesBraille. (#10469)
* Several speech functions have been changed to return speech sequences. (#10098)
  * getControlFieldSpeech
  * getFormatFieldSpeech
  * getSpeechTextForProperties que agora se chama getPropertiesSpeech
...
* Funcionalidade obsoleta removida. (#9548)
  * AppModules removed:
  ...
  * synthDriverHandler.SynthDriver.speakText/speakCharacter foram removidos.
  * Removed several synthDriverHandler.SynthSetting classes.
    ...
    * Os drivers que utilizavam as classes SynthSetting devem ser actualizados para utilizar as classes DriverSetting.
  * Some legacy code has been removed, particularly:
    ...
    * Fim do suporte ao Skype 7, uma vez que já não funciona de todo.
* Added a framework to create vision enhancement providers; modules that can change screen contents, optionally based on input from NVDA about object locations. (#9064)
  * Os extras podem agrupar os seus próprios fornecedores numa pasta visionEnhancementProviders.
...
* Introduzido displayModel.UNIT_DISPLAYCHUNK como uma constante de unidade de textInfos específica para DisplayModelTextInfo. (#10165)
...
* External modules (appModules and globalPlugins) are now less likely to be able to break the creation of NVDAObjects.
  * As excepções causadas pelos métodos "chooseNVDAObjectOverlayClasses" e "event_NVDAObject_init" são agora correctamente detectadas e registadas.
...
* O NVDA já não congela ao interagir com imagens com um URI base64 no Mozilla Firefox ou no Google Chrome. (#10227)

## 2019.2

Os destaques desta versão incluem a detecção automática de dispositivos Braille da Freedom Scientific, uma opção experimental no painel Avançado para impedir que o modo de navegação mova automaticamente o foco (o que pode proporcionar melhorias de desempenho), uma opção de aumento de taxa para o sintetizador do Windows OneCore para obter taxas muito rápidas e muitas outras correcções de erros.
...
* Todas as opções do NVDA podem agora ser encontradas numa janela de Configurações, em Menu NVDA -> Preferências -> Configurações, em vez de estarem espalhadas por várias janelas. (#577)
...
* No Manual do Utilizador, quando um comando <!-- KC:setting é utilizado para uma definição que tem uma tecla comum a todas as disposições, a tecla pode agora ser colocada depois de dois pontos a toda a largura (：), bem como dos dois pontos normais (:). (#5739) -->
...
* Os nomes dos gestos tácteis listados na caixa de diálogo definir comandos do NVDA são agora amigáveis e localizados. (#3624)
* O NVDA não faz mais com que certos programas travem ao mover o rato sobre seus controles de edição avançada (TRichEdit). Os programas incluem o Jarte 5.1 e o BRfácil. (#3693, #3603, #3581)
...
* Um dispositivo braille Papenmeier pode agora ser utilizado mesmo que nunca tenha sido ligado um ecrã Papenmeier via USB. (#3712)
...
* Quando a função Teclas presas do Windows está activada, a tecla modificadora NVDA passa a comportar-se como as outras teclas modificadoras. Isto permite-lhe utilizar a tecla modificadora NVDA sem ter de a manter premida enquanto prime outras teclas. (#230)
...
* Suporte para dispositivos braille HIMS Braille Sense, Braille EDGE e SyncBraille. (#1266, #1267)
...
* Experimental support for Touch screens on Windows 8, including:
  ...
  * Muitos comandos para efetuar a navegação de objectos, revisão de texto e outros comandos NVDA.
...
* Suporte para os dispositivos braille Seika Versão 3, 4 e 5 e Seika80 da Nippon Telesoft (#2452)
* O primeiro e o último botão de encaminhamento superior nos dispositivos Braille PAC Mate e Focus da Freedom Scientific podem agora ser utilizados para se deslocarem para trás e para a frente. (#2556)
* São suportadas muitas outras funcionalidades nos dispositivos Braille Freedom Scientific Focus, tais como barras de avanço, barras basculantes e determinadas combinações de pontos para acções comuns. (#2516)
...
* Suporte para modelos mais recentes de dispositivos braille Papenmeier BRAILLEX. (#1265)
...
* A opção Guardar configurações ao sair está agora activada por predefinição para novas configurações.
...
* Os extras que incluem ficheiros com caracteres não ingleses podem agora ser instalados corretamente na maioria dos casos. (#2505)
* In Adobe Reader, the language of text is no longer lost when it is updated or scrolled to. (#2544)
* Ao instalar um extra, a caixa de diálogo de confirmação mostra agora corretamente o nome localizado do extra, se disponível. (#2422)
...
* Os comandos NVDA já não podem ser executados a partir de um dispositivo braille enquanto um ecrã seguro do Windows (como o ecrã de bloqueio) estiver ativo. (#2449)
...
* Para conveniência dos programadores externos, os módulos de aplicações, bem como os plugins globais, podem agora ser recarregados sem reiniciar o NVDA. Utilize ferramentas -> Recarregar plugins no menu do NVDA ou NVDA+control+f3. (#544)
...
* Adicionada a capacidade de ver o ficheiro de registo actual do NVDA. Pode ser encontrado no menu NVDA -> Ferramentas
...
* Adicionado ficheiro em português de Rui Batista.
* Alterar a voz na caixa de diálogo de configurações de voz agora define os controles deslizantes de velocidade, tom e volume para os novos valores de acordo com o sintetizador, em vez de forçar o sintetizador a ser definido com os valores antigos. Isso corrige problemas em que um sintetizador como o eloquence ou o viavoice parece falar em uma taxa muito mais rápida do que todos os outros sintetizadores.
...
* As novas mensagens são agora faladas automaticamente quando se está numa conversa utilizando o Windows Live Messenger. (até à data, só funciona nas versões em inglês)
* A janela do histórico numa conversa do Windows Live Messenger pode agora ser lida utilizando as teclas de setas. (Até à data, só funciona nas versões em inglês)
...
* Se houver um erro com um idioma ao escolhê-lo na caixa de diálogo Configurações da interface do utilizador, uma caixa de mensagem alertará o utilizador para esse facto.
* O NVDA pergunta agora se deve guardar a configuração e reiniciar se o utilizador tiver acabado de alterar o idioma na caixa de diálogo Configurações da interface do utilizador. O NVDA deve ser reiniciado para que a alteração de idioma tenha efeito total.
