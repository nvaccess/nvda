...
* Eliminouse a asignación `controlTypes.isCurrentLabels`. (#11782)
  * No seu lugar, usa a propiedade `displayString` nun valor do enum `controlTypes.IsCurrent`. EG `controlTypes.IsCurrent.YES.displayString`
* A propiedade `NVDAObject` (e derivados) agora devolve estrictamente `controlTypes.IsCurrent`. (#11782)
  * `isCurrent` xa non é  Opcional, e polo tanto non devolverá None, cando un obxecto non é actual `controlTypes.IsCurrent.NO` se devolve.
* Eliminouse `controlTypes.isCurrentLabels`, no seu lugar úsase a propiedade `displayString` nun valor do enum `controlTypes.IsCurrent`. (#11782)
  * Ex.: `controlTypes.IsCurrent.YES.displayString`
...
* As pantallas braille Braillino, Bookworm e Modular (co firmware vello) de Handy Tech xa non se soportan ao sacalas da caixa. Instala o controlador universal de Handy Tech e o complemento do NVDA para usar as pantallas. (#7590)
...
* Agora tradúcese o anunciado de nomes de follas en Excel. (#6848)
...
* Agora sopórtanse todas as versións existentes de SecureCRT. (#6302)
* Adobe Acrobat Reader xa non se rompe en certos documentos PDF (especcialmente, en aqueles que conteñan  atributos empty ActualText). (#7021, #7034)
...
* Realizando un deslizamento cara a esquerda ou cara a dereita cun so dedo cando se estea no modo obxecto agora móvese cara anterior ou seguinte nunha vista cha da navegación de obxectos. Utiliza un deslizamento con 2 dedos cara a esquerda ou cara a dereita para levar a cabo a orde orixinal de obxecto anterior/seguinte limitada ó nivel  actual.
...
* NVDA xa non causa que o Adobe Reader se bloquee ou se colgue para certos documentos PDF mal formados que conteñan filas fora das táboas. (#3399)
...
* Reinstaurado o diálogo editor de celdas de Excell do NVDA que se desactivara accidentalmente no NVDA 2011.1.
