...
+ Prozor NVDA pokretača se pojavljuje sa licencnim ugovorom.
Pritisnite ``strelicuDole`` da pročitate licencni ugovor ako želite.
+ Pritisnite ``tab`` da se prebacite na izborno polje "Slažem se", a zatim pritisnite ``razmak`` da ga označite.
+ Pritisnite ``tab`` da se krećete kroz opcije, zatim pritisnite ``enter`` na željenoj opciji.
+
...
+ Iz pokretača, uverite se da je izborno polje slaganja sa licencnim ugovorom označeno.
+ Krećite se ``Tabom`` do, i aktivirajte dugme "Instaliraj NVDA na ovaj računar".
+ Slede opcije korišćenja programa NVDA na Windows ekranu za prijavljivanje i da napravite prečicu na radnoj površini.
...
+ Windows dijalog "Kontrole korisničkog naloga (KKN)" će se pojaviti i upitaće vas "Da li želite da dozvolite ovoj aplikaciji da izvrši promene na vašem računaru?".
+ Pritisnite ``alt+D`` na Srpskoj ili ``Alt+Y`` na Engleskoj verziji Windowsa da dozvolite promene.
+ Traka napredovanja će se popunjavati dok se NVDA instalira.
...
+ Dijalog će se pojaviti i potvrditi da je NVDA instalacija uspešna.
...
+ Dijalog "Dobrodošli u NVDA" će se pojaviti, i NVDA čita poruku dobrodošlice.
...
+ Pritisnite ``tab`` da se prebacite na izborno polje "Koristite ``capsLock`` kao NVDA taster".
...
+ Koristite ``tab`` i ``razmak`` da podesite druge opcije na ovom ekranu.
One određuju da li se NVDA automatski pokreće.
+ Pritisnite ``enter`` da zatvorite ovaj dijalog uz pokrenut NVDA.
+
...
+ Otvorite dijalog ulaznih komandi iz režima pretraživanja.
+ Pronađite stavku "premešta se na narednu karticu" u sekciji režim pretraživanja.
+ Dodelite taster ``y`` pronađenoj komandi.
+ Pronađite stavku "premešta se na prethodnu karticu".
+ Dodelite ``šift+y`` pronađenoj komandi.
+
...
+ Prebacite kursor na matematički sadržaj.
...
+ Zatim, aktivirajte sledeću komandu:
+
...
+ Prebacite kursor na tekst koji sadrži napomenu.
+ Pristupite opcijama izbora koje su opisane iznad.
+ Izaberite uredi napomenu.
+
...
%kc:setting
==== Otvori dijalog za izbor sintetizatora ====
...
%kc:setting
==== Otvori dijalog za izbor brajevog reda ====
...
%kc:setting
==== Otvori podešavanja predstavljanja objekata ====
...
%kc:setting
==== Otvori podešavanja režima pretraživanja ====
...
%kc:setting
==== Otvori  podešavanja formatiranja dokumenta ====
...
+ Molimo uverite se da ste instalirali [najnoviji drajver https://www.papenmeier-rehatechnik.de/en/service/downloadcenter/software/articles/software-braille-devices.html].
+ Otvorite Windows upravljač uređajima.
+ U listi pronađite stavku "USB kontroleri" ili "USB uređaji".
+ Izaberite "Papenmeier Braillex USB Device".
