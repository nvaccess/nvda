...
* Sistemas Operacionais: todas as edições de 32-bit e 64-bit do Windows 7, Windows 8, Windows 8.1, Windows 10, Windows 11, e todos os sistemas operacionais de servidor a partir do Windows Server 2008 R2;
  * Para o Windows 7, o NVDA requer Service Pack 1 ou superior;
  * Para o Windows Server 2008 R2, é requerido o Service Pack 1 ou superior.
...
O NVDA está disponível sob a GNU General Public License [GNU GPL — Licença Pública Geral GNU] versão 2, com duas exceções especiais.
...
Para mais detalhes, pode [ver a licença completa [em inglês]](https://www.gnu.org/licenses/old-licenses/gpl-2.0.html) [[e GNU GPL 2 tradução não oficial em português brasileiro].](http://licencas.softwarelivre.org/gpl-2.0.pt-br.html)
...
|Ler seleção |`NVDA+shift+seta para cima` |`NVDA+shift+s` |Lê qualquer texto selecionado|
|Ler texto da área de transferência |`NVDA+c` |`NVDA+c` |Lê qualquer texto na área de transferência|
...
|Relatar foco |`NVDA+tab` |`NVDA+tab` |Relata o controle atual que está focalizado. Pressionar duas vezes soletrará a informação|
...
* Windows 8 e posteriores: impossibilidade de suportar entradas oriundas duma tela sensível ao toque;
* Windows 8 e posteriores: impossibilidade de oferecer recursos tais como o modo de navegação e o anúncio de caracteres digitados na Windows Store [Loja Windows];
* Windows 8 e posteriores: prioridade de áudio não é suportada.
...
Você pode ler mais informações sobre os dados coletados pela NV Access na seção de configurações gerais, [Permitir que o projeto NVDA colete estatísticas de uso do NVDA](#GeneralSettingsGatherUsageStats).
...
Caso esteja executando o NVDA em um dispositivo com tela sensível ao toque e executando Windows 8 ou superior, você também pode controlar o NVDA diretamente via comandos de toque.
...
|Alternar Modo de Fala |NVDA+s |NVDA+s |nenhum |Alterna o modo de fala entre fala, bipe e desativado.|
...
## Lendo Conteúdo Matemático {#ReadingMath}

Usando o MathPlayer 4 da Design Science, o NVDA pode ler e navegar interativamente pelo conteúdo matemático suportado.
Isso requer que o MathPlayer 4 esteja instalado no computador.
O mesmo encontra-se disponível para download gratuito em: https://www.dessci.com/en/products/mathplayer/.
...
Pode ser baixado em https://www.dessci.com/en/products/mathtype/
* Adobe Reader.
...
A partir desse ponto, você pode usar os comandos do MathPlayer tais como as setas para explorar a expressão.
Por exemplo, você pode mover-se pela expressão com as setas à direita e à esquerda e ampliar uma porção da expressão, tal como uma fração usando a seta para baixo.
Por favor consulte a [documentação do MathPlayer sobre comandos de navegação](https://www.dessci.com/en/products/mathplayer/navigation_commands.htm) para informações adicionais.
...
Em segundo lugar, o NVDA precisa ser informado sobre qual linha ou coluna contém os cabeçalhos em qualquer tabela mostrada.
...
#### Lendo Células Protegidas {#ExcelReadingProtectedCells}
...
##### Abrir configurações Gerais {#toc105}
...
##### Permitir que o Projeto NVDA colete estatísticas de uso do NVDA {#GeneralSettingsGatherUsageStats}

Se isso estiver habilitado, a NV Access usará as informações das procuras de atualização para rastrear o número de usuários do NVDA, incluindo informações demográficas específicas, como Sistema operacional e país de origem.
...
##### Abrir configurações de Fala {#toc118}
...
##### Abrir caixa de diálogo Selecionar Sintetizador {#toc138}
...
##### Abrir caixa de diálogo Selecionar Linha Braille {#toc161}
...
##### Abrir configurações de Áudio {#toc166}
...
No Windows 8 e superiores, essa opção permite-lhe escolher se o NVDA deve reduzir o volume de outros aplicativos enquanto ele estiver falando, ou durante todo o tempo em que o NVDA estiver em execução.
...
Para alternar a Cortina de Tela de qualquer lugar, atribua um comando personalizado usando o [diálogo Definir Comandos](#InputGestures).
...
##### Abrir configurações de Teclado {#toc176}
...
Caso nenhuma tecla seja escolhida como modificadora do NVDA, poderá ser impossível executar certos comandos.
Por isso, o diálogo de Configurações do NVDA exibirá uma mensagem de erro se todas as teclas estiverem desselecionadas quando você pressionar Ok ou Aplicar.
Após fechar a mensagem de erro, será necessário selecionar pelo menos uma antes de poder pressionar Ok para fechar o diálogo corretamente.
...
##### Abrir configurações de Mouse {#toc189}
...
##### Anunciar função [tipo] do objeto quando o mouse entra nele {#MouseSettingsRole}

Se essa caixa de seleção estiver ativada, o NVDA anunciará a função (tipo) do objeto quando o mouse se mover dentro dele.
...
Essa categoria de configurações, disponível apenas em computadores que executam o Windows 8 e posteriores com recursos de toque, permite que você configure como o NVDA interage com as telas sensíveis ao toque.
Essa categoria contém as seguintes opções:
...
##### Abrir configurações da Apresentação de Objetos {#toc206}
...
##### Abrir configurações do Modo de Navegação {#toc224}
...
##### Abrir configurações de Formatação de Documento {#toc238}
...
Em situações em que a UI Automation [Automação de Interface do Usuário] não está disponível ou resulta em uma experiência inferior ao usuário, o suporte de console legado do NVDA está disponível como um retrocesso.
...
O retrocesso legado sempre será usado mesmo em situações em que a UI Automation proporcionaria uma experiência superior ao usuário.
...
Para obter um tutorial introdutório, consulte [https://docs.python.org/3.7/howto/regex.html].

#### Pronúncia de pontuação/símbolos {#SymbolPronunciation}

Este diálogo permite-lhe mudar o modo como a pontuação e outros símbolos [sinais] serão anunciados, bem como em que grau [nível] de símbolos eles serão falados.
 ...
 * Um comando [gesto] de entrada com teclas modificadoras pode não ser mapeado para um comando emulado sem
 teclas modificadoras. Por exemplo, definir a entrada emulada 'a' e configurar um comando de entrada de 'ctrl+m' pode fazer
...
O visualizador de log [eventos registrados] encontra-se no submenu Ferramentas do Menu do NVDA e permite-lhe ver todas as saídas de eventos que têm ocorrido até agora desde que iniciou o NVDA pela última vez.
Usando NVDA+F1 abrirá o visualizador de logs e exibirá informações de desenvolvedor sobre o objeto atual do navegador.

Além de ler o conteúdo, este também pode salvar uma cópia do arquivo de log, ou atualizar o visualizador de forma a apresentar a saída mais recente desde que o Visualizador de Log foi aberto.
Estas ações encontram-se disponíveis no menu Log do visualizador.
...
Contudo, se clicar ou se colocar o foco no visualizador, o NVDA parará temporariamente a atualização do texto, de modo que possa selecionar facilmente ou copiar o conteúdo existente.
...
Isso abrirá uma caixa de diálogo que permite criar uma cópia portátil do NVDA a partir da versão instalada.
De qualquer forma, ao executar uma cópia portátil do NVDA, no submenu extra de ferramentas, o item de menu será chamado "instalar NVDA neste PC" em vez de "criar cópia portátil".

O diálogo para criar uma cópia portátil do NVDA ou instalar o NVDA neste PC solicitará que você escolha um caminho de pasta no qual o NVDA deve criar a cópia portátil ou no qual o NVDA deve ser instalado.

Neste diálogo, você pode habilitar ou desabilitar o seguinte:

* Copiar a configuração atual do usuário (isso inclui os arquivos em %appdata%\roaming\NVDA ou na configuração do usuário da sua cópia portátil e também inclui complementos e outros módulos)
* Iniciar a nova cópia portátil após a criação ou iniciar o NVDA após a instalação (inicia o NVDA automaticamente após a criação ou instalação da cópia portátil)
...
No Windows 7, 8 e 8.1, o NVDA usa o eSpeak NG por padrão ([Windows OneCore](#OneCore) é usado no Windows 10 e posterior por padrão).
...
Consulte este artigo da Microsoft para obter uma lista de vozes disponíveis: https://support.microsoft.com/pt-br/windows/ap%C3%AAndice-a-idiomas-e-voz-com-suporte-4486e345-7730-53da-fcfe-55cc64300f01
...
Se ainda não tem os mesmos, pode obtê-los em https://support.freedomscientific.com/Downloads/Focus/FocusBlueBrailleDisplayDriver.
...
#### Atribuições de teclas para todos os modelos {#toc319}
...
#### Atribuições de teclas para Brailliant BI 32, BI 40 e B 80 {#toc320}
...
#### Atribuições de teclas para Brailliant BI 14 {#toc321}
...
Caso esteja conectando via USB, você precisará instalar os drivers USB da HIMS em seu sistema.
Você pode baixá-los daqui: http://www.himsintl.com/upload/HIMS_USB_Driver_v25.zip
...
|tecla alt+setaParaEsquerda |ponto2+ponto7|
...
|tecla alt+setaParaDireita |ponto5+ponto7|
...
|tecla alt+setaParaCima |ponto2+ponto3+ponto7|
...
|tecla alt+setaParaBaixo |ponto5+ponto6+ponto7|
...
Você pode encontrar mais informações sobre as linhas em https://en.seika-braille.com/down/index.html.
...
|Mover a linha braille para a linha anterior |espaço + ponto1|
|Mover a linha braille para a próxima linha |espaço + ponto4|
|Encaminhar para a cela braille |apontar encaminhamento|
|Alternar braille vinculado a |cima+baixo|
|tecla Seta para cima |joystick para cima|
|tecla Seta para baixo |joystick para baixo|
|tecla Seta para esquerda |espaço+ponto3 ou joystick para esquerda|
|tecla Seta para a direita |espaço+ponto6 ou joystick para direita|
|tecla shift+tab |espaço+ponto1+ponto3|
|tecla tab |espaço+ponto4+ponto6|
|tecla alt |espaço+ponto1+ponto3+ponto4 (espaço+m)|
|tecla Esc |espaço+ponto1+ponto5 (espaço+e)|
|tecla enter |ponto8 ou centro do joystick|
|tecla windows |espaço+ponto3+ponto4|
|tecla alt+tab |espaço+ponto2+ponto3+ponto4+ponto5 (espaço+t)|
|Menu NVDA |espaço+ponto1+ponto3+ponto4+ponto5 (espaço+n)|
|tecla windows+d (minimizar todos os aplicativos) |espaço+ponto1+ponto4+ponto5 (espaço+d)|
...
O NVDA pode ser iniciado em modo seguro com a [opção de linha de comando](#CommandLineOptions) `-s`.
...
* O [console Python](#PythonConsole)
...
|-f NOMEDOARQUIVODELOG |--log-file=NOMEDOARQUIVODELOG |O arquivo no qual as mensagens de log devem ser gravadas|
|-l NÍVELDELOG |--log-level=NÍVELDELOG |O nível mais baixo de mensagem registrada (depuração 10, entrada/saída 12, alerta de depuração 15, informação 20, alerta 30, erro 40, crítico 50, desabilitado 100), o padrão é alerta|
|-c CAMINHODASCONFIGURAÇÕES |--config-path=CAMINHODASCONFIGURAÇÕES |O caminho onde todas as configurações do NVDA são armazenadas|
...
|Nenhuma |--create-portable |Cria uma cópia portátil do NVDA (iniciando a cópia recém criada). Requer --portable-path a ser especificado|
|Nenhuma |--create-portable-silent |Cria uma cópia portátil do NVDA (não inicia a cópia recém-instalada). Requer --portable-path a ser especificado|
...
Se você precisar de mais informações ou assistência referente ao NVDA, visite o site do NVDA em NVDA_URL.
...
[Para conhecer todas as novidades, mudanças e correções introduzidas, consulte o arquivo das Novidades que pode ser encontrado no submenu ajuda do menu NVDA.]
[Você pode igualmente obter informações e tirar dúvidas sobre o NVDA na lista de discussão do nvda em português. Envie um e-mail em branco e sem assunto para o endereço nvdaemportugues+subscribe@googlegroups.com para se inscrever nessa lista.]
