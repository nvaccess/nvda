...
* Capacidade de [se conectar e controlar outro computador executando o NVDA](#RemoteAccess) para assistência ou colaboração remota
...
Além do inglês, o NVDA está traduzido para 55 idiomas, estando inclusos: africâner, albanês, alemão (Alemanha e Suíça), amárico, árabe, aragonês, birmanês, bósnio, búlgaro, canará, catalão, chinês (simplificado e tradicional), coreano, croata, dinamarquês, eslovaco, esloveno, espanhol (Colômbia e Espanha), finlandês, francês, galego, georgiano, grego, hebraico, hindi, holandês, húngaro, irlandês, islandês, italiano, japonês, lituano, macedônio, mongol, nepali, norueguês, persa, polonês, português (Brasil e Portugal), punjabi, quirguiz, romeno, russo, sérvio, sueco, tailandês, tâmil, tcheco, turco, ucraniano e vietnamita.
...
Pode ser necessário aguardar alguns segundos enquanto uma cópia temporária do NVDA é carregada.
Depois de carregado, o NVDA falará durante o restante do processo.
1. A janela NVDA Launcher — Lançador do NVDA — aparece com um botão para abrir o contrato de licença.
...
1. Um diálogo aparece para confirmar que a instalação do NVDA foi bem-sucedida.
...
| Nome |Tecla de desktop |Tecla de laptop |Descrição|
|---|---|---|---|
|Falar caracteres digitados |`NVDA+2` |`NVDA+2` |Controla quando o NVDA anuncia os caracteres que você digita. |
|Falar palavras digitadas |`NVDA+3` |`NVDA+3` |Controla quando o NVDA anuncia as palavras que você digita. |
...
Recomendamos começar com o módulo "Treinamento Básico para NVDA" — em inglês —.
...
|Mover para o início da seleção em exploração |`NVDA+alt+home` |`NVDA+alt+home` |Nenhum |Move o cursor de exploração para o primeiro caractere do texto selecionado|
|Mover para o final da seleção em exploração |`NVDA+alt+end` |`NVDA+alt+end` |Nenhum |Move o cursor de exploração para o último caractere do texto selecionado|
...
No entanto, o NVDA possui um Modo de Seleção Nativa que pode ser ativado em documentos específicos no Modo de Navegação que o suportem (Mozilla Firefox e qualquer navegador baseado no Chromium 134 ou mais recente), o que faz com que a seleção nativa do documento siga a seleção do Modo de Navegação do NVDA.
...
O NVDA fornece seus próprios recursos adicionais para alguns aplicativos, com vistas a executar mais facilmente determinadas tarefas, ou para proporcionar acesso a funcionalidades que de outra forma não são acessíveis aos usuários de leitores de telas.
...
Pressionar duas vezes mostra as informações numa mensagem navegável.
<!-- KC:endInclude -->
...
<!-- KC:beginInclude -->
Para anunciar quaisquer notas na célula atualmente sob o foco, pressione `NVDA+alt+c`.
Pressionar duas vezes mostra as informações numa mensagem navegável.
...
O NVDA oferece suporte aprimorado para Poedit 3.5 ou mais recente.
...
Se marcada, um diálogo aparecerá quando você tentar encerrar o NVDA, oferecendo as seguintes possibilidades:

* sair
* reiniciar
* reiniciar com complementos desabilitados e log de depuração habilitado
* reiniciar com o log de depuração habilitado
* instalar atualizações pendentes (se houver).
...
##### Espelho de Atualizações {#UpdateMirror}

Esses controles permitem que você especifique uma URL alternativa a ser usada para verificar se há atualizações para o NVDA.
Isso pode ser útil em locais onde o acesso ao servidor de atualização do NVDA da NV Access estiver lento ou indisponível.

A caixa de texto somente leitura mostra a URL de espelho atual.
Se nenhum espelho estiver em uso (ou seja, o servidor de atualização da NV Access está sendo usado), "Sem espelho" será exibido.

Se você deseja alterar o espelho de atualização, pressione o botão "Alterar..." para abrir o [diálogo Definir Espelho de Atualização](#SetURLDialog).

Observe que, ao usar um espelho de atualização, o operador do espelho tem acesso a todas as [informações enviadas com as verificações de atualização](#GeneralSettingsCheckForUpdates).
Entre em contato com o operador do espelho de atualização para obter detalhes de suas políticas de tratamento de dados para garantir que você esteja confortável com a maneira como suas informações serão tratadas antes de configurar um espelho de atualização.

##### Impedir que a tela seja desligada durante a leitura contínua ou leitura em braille {#PreventDisplayTurningOff}

Essa caixa de seleção, quando habilitada, garante que a tela permaneça ligada durante a leitura contínua ou leitura com braille (por exemplo, ao pressionar botões de rolagem).
Isso evita a situação em que a tela é bloqueada inesperadamente durante a leitura contínua.
Essa opção está habilitada por padrão.
Considere desabilitá-la se você estiver sofrendo com uma vida útil mais curta da bateria.
...
A primeira opção na categoria Configurações de fala é o botão Alterar....
Esse botão ativa a caixa de diálogo [Selecionar Sintetizador](#SelectSynthesizer), que permite selecionar o sintetizador de fala ativo.
...
| . {.hideHeaderRow} |.|
|---|---|
|Opções |Padrão (Habilitado), Habilitado, Desabilitado|
|Padrão |Habilitado|
...
Se você selecionar "Automático", a tabela será selecionada de acordo com o idioma atual do NVDA.
...
| . {.hideHeaderRow} |.|
|---|---|
|Opções |Padrão (Nunca), Nunca, Apenas quando vinculado automaticamente, Sempre|
|Padrão |Nunca|
...
Se "Ler por parágrafo" estiver marcado, o marcador de início selecionado será exibido para indicar o início de um parágrafo.
Isso pode ser especialmente útil em aplicativos usados para ler grandes partes de texto, como documentos estruturados ou livros.
Em tais documentos, saber onde os parágrafos começam pode ser útil para entender a estrutura do conteúdo ou para definir marcadores ou anotações com base na posição do parágrafo.

As opções incluem uso de dois espaços como uma quebra de parágrafo sutil, e o símbolo indicador de parágrafo, pé de mosca — caldeirão — (¶), como uma quebra mais óbvia.

| . {.hideHeaderRow} |.|
|---|---|
|Opções |Sem marcador de início de parágrafo, Espaço duplo (  ), Pé de mosca — Caldeirão — (¶)|
...
##### Exibição de formatação {#BrailleFormattingDisplay}

Esta configuração determina como o NVDA exibirá a formatação de texto em braille.
Esta opção só tem efeito se o NVDA estiver configurado para [exibir atributos de fonte em braille](#DocumentFormattingFontAttributes).
...
| Opção | Comportamento |
|---|---|
| Liblouis | Use formatação nativa Braille. Note que esta opção indicará somente texto em negrito, itálico e sublinhado, e somente se a tabela braille selecionada suportar a indicação desses atributos. |
| [Etiquetas](#BrailleFormattingDisplayTags) | Use rótulos que descrevam como e onde a formatação do texto muda. |

###### Etiquetas {#BrailleFormattingDisplayTags}

Quando "Exibição de formatação" estiver definida como "Etiquetas", uma etiqueta — marcação — de formatação é exibida em braille quando uma alteração na formatação é detectada.
Essas etiquetas — rótulos — iniciam com ⣋ e finalizam com ⣙.
Uma etiqueta — indicador — de formatação conterá um ou mais símbolos que descrevem a formatação do texto.
Os seguintes símbolos são definidos:
...
| ⠎ ("s") | Início de tachado — riscado |
| ⡎ ("s" com ponto 7) | Fim de tachado — riscado |

##### Falar caractere ao direcionar o cursor no texto {#BrailleSpeakOnRouting}

Se isso estiver habilitado, o NVDA falará automaticamente o caractere no cursor ao direcionar para ele com as teclas de direcionamento do cursor braille.

Para alternar esta opção de qualquer lugar, atribua um comando — gesto — personalizado usando o [diálogo Definir comandos](#InputGestures).

##### Falar ao navegar por linha ou parágrafo {#BrailleSpeakOnNavigating}

Habilitar essa opção fará com que o NVDA fale as linhas ou parágrafos alcançados usando os controles de navegação da linha braille.

Para alternar esta opção de qualquer lugar, atribua um comando — gesto — personalizado para "Liga e desliga a fala ao navegar por linha ou parágrafo em braille" — speakOnNavigatingByUnit — na seção "Braille" do [diálogo Definir Comandos](#InputGestures).

##### Evitar partir palavras quando possível {#BrailleSettingsWordWrap}

Se isso estiver habilitado, uma palavra que seja muito grande para caber no final da linha braille não será dividida.
Em vez disso, haverá algum espaço em branco no final da linha.
Quando você rolar a linha, você poderá ler a palavra inteira.
Às vezes, isso é chamado de "quebra automática de palavras".
Observe que, se a palavra for muito grande para caber na linha, a palavra ainda deve ser partida.

Se isso estiver desabilitado, o máximo possível da palavra será exibido, mas o restante será cortado.
Ao rolar a linha, você poderá ler o restante da palavra.

Habilitar isso pode permitir uma leitura mais fluente, mas geralmente requer que você role mais a linha.

##### Normalização unicode {#BrailleUnicodeNormalization}

Quando esta opção está habilitada, a normalização unicode é executada no texto que está em braille na linha braille.
Isso é benéfico ao encontrar caracteres em braille que são desconhecidos em uma tabela braille específica e que têm uma alternativa compatível, como os caracteres em negrito e itálico comumente usados em mídias sociais.
Outros benefícios da normalização unicode são explicados em maiores detalhes na [seção da configuração de fala equivalente](#SpeechUnicodeNormalization).
...
##### Interromper a fala enquanto rola {#BrailleSettingsInterruptSpeech}

Esta configuração determina se a fala deve ser interrompida quando a linha Braille é rolada para trás/frente.
Os comandos de linha anterior/seguinte sempre interrompem a fala.

A fala contínua pode ser uma distração durante a leitura Braille.
Por esse motivo, a opção está habilitada por padrão, interrompendo a fala ao rolar em braille.

A desativação desta opção permite que a fala seja ouvida ao ler Braille simultaneamente.
...
Essa opção controla quando o NVDA anuncia os caracteres que você introduzir através do teclado.
As opções disponíveis são:

* Desligado: O NVDA não anunciará caracteres digitados.
* Somente em controles de edição: O NVDA anunciará apenas caracteres digitados em controles de edição e outras áreas onde o texto pode ser digitado.
* Sempre: O NVDA anunciará todos os caracteres digitados.
...
Essa opção controla quando o NVDA anuncia as palavras que você introduzir através do teclado.
As opções disponíveis são:

* Desligado: O NVDA não anunciará palavras digitadas.
* Somente em controles de edição: O NVDA anunciará apenas palavras digitadas em controles de edição e outras áreas onde o texto pode ser digitado.
...
##### Tempo limite de pressionamento de várias teclas {#MultiPressTimeout}

Alguns comandos — gestos — de teclado do NVDA executam ações diferentes com base em quantas vezes a mesma tecla é pressionada em rápida sucessão.
Um exemplo disso é o comando "Relatar o caractere atual do objeto de navegador".
Este comando informa o caractere se pressionado uma vez, uma descrição fonética do caractere se pressionado duas vezes e o valor numérico do caractere se pressionado três vezes.
Esta opção configura o tempo limite após o qual um pressionamento adicional da mesma tecla iniciará um novo comando — gesto —, em vez de ser considerado um pressionamento subsequente do primeiro.
Para o comando de exemplo, um tempo limite muito curto fará com que dois pressionamentos relatem o caractere atual duas vezes, em vez da descrição fonética.
O tempo limite padrão é 500 ms, ou seja, meio segundo.
Aumentar esse tempo limite pode ser especialmente útil para pessoas que usam teclas de aderência ou que têm uma deficiência física.
...
* Fonte
  * Nome da Fonte
  * Tamanho da Fonte
  * Atributos da Fonte [(Desativado, Fala, Braille, Fala e braille)](#DocumentFormattingFontAttributes)
...
##### Atributos da fonte {#DocumentFormattingFontAttributes}

Esta opção permite que você selecione como certos atributos de fonte, como negrito, itálico, sublinhado e tachado — riscado — são relatados.
A caixa de combinação de atributos da fonte tem quatro opções:

* Desativado: o NVDA não relatará esses atributos de fonte.
* Fala: O NVDA anunciará quando esses atributos de fonte forem alterados.
* Braille: O NVDA exibirá esses atributos em braille.
Exatamente como eles são exibidos pode ser configurado nas [configurações de braille do NVDA](#BrailleFormattingDisplay).
* Fala e braille: o NVDA relatará atributos de fonte usando ambos os métodos acima.
...
Por padrão, as notificações ocorrerão apenas para complementos com atualizações disponíveis no mesmo [canal](#AddonStoreFilterChannel) (por exemplo, estável, beta ou em desenvolvimento).
Você pode configurar canal de atualização de complementos [individualmente para cada complemento](#AddonStoreUpdateChannel) ou para [todos os complementos](#DefaultAddonUpdateChannel).

Quando definido como "Atualizar automaticamente", os complementos serão atualizados automaticamente em segundo plano.
Você será solicitado a reiniciar o NVDA quando as atualizações forem concluídas.

| . {.hideHeaderRow} |.|
|---|---|
|Opções |Notificar (Padrão), Atualizar Automaticamente, Desabilitado |
|Padrão |Notificar |

|Opção |Comportamento |
|---|---|
|Notificar |Notificar quando houver atualizações disponíveis para complementos |
|Atualizar Automaticamente |Atualizar complementos automaticamente em segundo plano |
|Desabilitado |Não verificar automaticamente se há atualizações de complementos |

##### Canal de Atualização Padrão {#DefaultAddonUpdateChannel}

Quando as [Atualizações automáticas de complementos](#AutomaticAddonUpdates) estão habilitadas, por padrão, os complementos são atualizados apenas para o mesmo [canal](#AddonStoreFilterChannel).
Por exemplo, uma versão beta instalada só será atualizada para uma versão beta mais recente.
Essa opção define o canal de atualização padrão para todos os complementos.
Você também pode alterar o canal de atualização de um [complemento específico individualmente na Loja de Complementos](#AddonStoreUpdateChannel).

| . {.hideHeaderRow} |.|
|---|---|
| Opções | O mesmo (Padrão), Qualquer, Não atualizar, Estável, Beta ou em desenvolvimento, Beta, Em desenvolvimento |
| Padrão | O mesmo |

| Opção | Comportamento |
|---|---|
| O mesmo | Os complementos permanecerão em seu canal |
| Qualquer | Os complementos serão atualizados automaticamente para a versão mais recente, independentemente do canal |
| Não atualizar | Os complementos não serão atualizados automaticamente por padrão, você deve habilitá-los individualmente |
| Estável | Os complementos serão atualizados automaticamente para versões estáveis |
| Beta ou em desenvolvimento | Os complementos serão atualizados automaticamente para versões beta ou em desenvolvimento |
| Beta | Os complementos serão atualizados automaticamente para versões beta |
| Em desenvolvimento | Os complementos serão atualizados automaticamente para as versões em desenvolvimento |

##### Permitir que atualizações automáticas instalem complementos incompatíveis {#AllowIncompatibleAddonUpdates}

Essa configuração habilita atualizações automáticas para complementos que podem não ser totalmente compatíveis com a versão atual do NVDA.
Por padrão, isso está desabilitado, o que significa que as atualizações automáticas atualizarão apenas para versões de complementos marcadas como compatíveis com a versão atual do NVDA.
As atualizações automáticas ainda atualizarão uma versão de complemento incompatível para uma versão compatível quando ela for lançada.
Habilitar isso pode ser útil para alternar para o uso de versões de quebra de complementos (a primeira versão do ano).
Isso é particularmente útil para testadores alfa e beta, que testam a compatibilidade de complementos durante os estágios iniciais de uma versão de quebra de complemento.

##### Servidor espelho {#AddonStoreMetadataMirror}

Esses controles permitem que você especifique uma URL alternativa para baixar os dados da Loja de Complementos.
Isso pode ser útil em locais onde o acesso ao servidor da Loja de Complementos da NV Access estiver lento ou indisponível.

A caixa de texto somente leitura mostra o URL do espelho atual.
Se nenhum espelho estiver em uso (ou seja, o servidor da Loja de Complementos da NV Access estiver sendo usado), "Sem espelho" será exibido.

Se você deseja alterar o espelho da Loja de Complementos, pressione o botão "Alterar..." para abrir o [diálogo Definir Espelho da Loja de Complementos](#SetURLDialog).

#### Configurações de Acesso Remoto {#RemoteSettings}

Essa categoria permite-lhe configurar o comportamento do [Acesso Remoto](#RemoteAccess).

##### Habilitar Acesso Remoto {#RemoteEnable}

Use essa caixa de seleção para habilitar ou desabilitar o recurso de acesso remoto do NVDA.

Quando essa opção está desmarcada, o recurso Acesso Remoto fica totalmente indisponível.

Por favor observe que, para continuar uma sessão de Acesso Remoto em telas seguras, como prompts do Controle de Conta de Usuário, você deve habilitar esta configuração, salvar as alterações e então [copiar suas configurações para o diretório de configuração do sistema do NVDA](#GeneralSettingsCopySettings).
O Acesso Remoto só continuará as sessões em áreas de trabalho seguras iniciadas pela cópia instalada do NVDA.
Isso significa que você não pode, por exemplo, iniciar uma sessão de Acesso Remoto com uma cópia portátil do NVDA e fazer com que a sessão continue em telas seguras.

As opções a seguir só estarão disponíveis se o Acesso Remoto estiver habilitado.

##### Confirme antes de desconectar quando controlado {#RemoteConfirmDisconnect}

Essa opção controla se a confirmação é necessária antes de se desconectar de uma sessão de Acesso Remoto quando conectado como o computador controlado.

Quando marcada, o NVDA solicitará confirmação antes de desconectar.
Quando desmarcada, o NVDA desconectará imediatamente sem confirmação.

##### Conectar automaticamente após a inicialização do NVDA {#RemoteAutoconnect}

Essa opção permite que você estabeleça automaticamente uma sessão de Acesso Remoto quando o NVDA for iniciado.
Isso pode ser útil, por exemplo, para controlar seu computador doméstico quando você estiver fora de casa.

Tenha cuidado ao habilitar essa opção, pois ela pode aumentar o risco de acesso não autorizado ao seu computador.

Para mais detalhes sobre as seguintes opções, consulte a [seção Configurando uma Sessão de Acesso Remoto](#RemoteAccessSetup)．

##### Modo {#RemoteAutoconnectMode}

Selecione o modo de conexão para conexões automáticas.

Essa opção só está disponível quando a opção [Conectar automaticamente após a inicialização do NVDA](#RemoteAutoconnect) estiver marcada.

|Opção |Comportamento |
|---|---|
|Permitir que este computador seja controlado |Use este computador como o computador "controlado". Isso permite que o operador do computador "controlador" o utilize como se estivesse sentado na frente dele. |
|Controlar outro computador |Use este computador como o computador "controlador". Isso permite que você execute comandos no computador remoto. |

##### Servidor {#RemoteAutoconnectServer}

Selecione o tipo de servidor para conexões automáticas.

Essa opção só está disponível quando a opção [Conectar automaticamente após a inicialização do NVDA](#RemoteAutoconnect) estiver marcada.

|Opção |Comportamento |
|---|---|
|Use existente |Use um servidor de acesso remoto existente para mediar a conexão (recomendado). |
|Servir localmente |Use esta instância do NVDA como servidor de acesso remoto (avançado). Esta opção pode exigir configuração de rede que está fora do escopo deste manual. |

##### Anfitrião {#RemoteAutoconnectHost}

Use este campo para definir o URL do servidor de Acesso Remoto que você gostaria de usar para conexões automáticas.
Se o servidor usar uma porta diferente de 6837, inclua-a após dois pontos (ex., `exemplo.com:1234`).

Essa opção só está disponível quando a opção [Conectar automaticamente após a inicialização do NVDA](#RemoteAutoconnect) estiver marcada e o [Servidor](#RemoteAutoconnectServer) estiver definido como "Usar existente".

##### Porta {#RemoteAutoconnectPort}

Use este campo para definir a porta TCP que você gostaria de usar para conexões automáticas.

Essa opção só está disponível quando a opção [Conectar automaticamente após a inicialização do NVDA](#RemoteAutoconnect) estiver marcada e o [Servidor](#RemoteAutoconnectServer) estiver definido como "Servir localmente".

##### Chave {#RemoteAutoconnectKey}

Use esse campo para definir a chave (senha) que você gostaria de usar para conexões automáticas.

Essa opção só está disponível quando a opção [Conectar automaticamente após a inicialização do NVDA](#RemoteAutoconnect) estiver marcada.

##### Excluir todas as impressões digitais confiáveis {#RemoteDeleteFingerprints}

Esse botão permite que você esqueça as impressões digitais de todos os servidores de Acesso Remoto anteriormente confiáveis.
Isso significa que você será questionado novamente se deseja se conectar a todos os servidores de acesso remoto não reconhecidos, mesmo aqueles aos quais você já se conectou.
Você será solicitado a confirmar antes que todas as impressões digitais confiáveis sejam excluídas.

Essa opção só está disponível se houver impressões digitais confiáveis armazenadas na sua configuração.
...
#### Diálogo Definir Espelho {#SetURLDialog}

Essa caixa de diálogo permite que você especifique o URL de um espelho a ser usado ao [atualizar o NVDA](#GeneralSettingsCheckForUpdates) ou [usar a Loja de Complementos](#AddonsManager).
Isso pode ser útil em locais onde o acesso aos servidores da NV Access para essas funções estiver lento ou indisponível.

* Ao definir o [espelho de atualização do NVDA](#UpdateMirror), o título dessa caixa de diálogo será "Configurar Espelho de Atualizações do NVDA".
* Ao definir o [espelho da Loja de Complementos](#AddonStoreMetadataMirror), o título desse diálogo será "Definir Servidor Espelho da Loja de Complementos".

##### URL {#SetURLTextbox}

Digite o URL (endereço da web) do espelho que deseja usar aqui.
Somente URLs HTTP e HTTPS são suportados.
Para sua privacidade, a NV Access recomenda o uso de URLs HTTPS sempre que possível.

Deixe em branco para usar o servidor padrão da NV Access.

##### Teste... {#SetURLTest}

Pressione esse botão para testar o URL do espelho que você inseriu.
Você deve estar conectado à Internet para que o teste seja bem-sucedido.
É recomendável que você sempre teste o URL antes de salvá-lo.
...
##### Cortar o silêncio inicial no áudio da fala {#TrimLeadingSilenceSpeech}

Quando habilitado, o NVDA removerá o silêncio do início do áudio da fala, o que pode melhorar a capacidade de resposta de alguns sintetizadores de fala.
Essa opção está habilitada por padrão e deve afetar apenas o silêncio no início da fala.
Se você perceber que alguns períodos de silêncio necessários também estão faltando (por exemplo, pausa entre duas frases) ao usar um complemento de sintetizador de fala, desative esse recurso completamente para resolver o problema.

##### Usar WASAPI para saída de áudio SAPI 4 {#UseWASAPIForSAPI4}

Essa opção permite que as vozes Microsoft Speech API versão 4 (SAPI 4) produzam áudio por meio da API de Sessão de Áudio do Windows (WASAPI).
Isso pode permitir que as vozes SAPI 4 funcionem com mais recursos, como áudio prioritário — redução de áudio —, corte de silêncio inicial e manter o dispositivo de áudio acordado.
No entanto, algumas vozes SAPI 4 podem não funcionar com a implementação WASAPI atual.
Se você perceber a voz SAPI 4 que você está usando parar de funcionar, você pode desativar esta opção.
...
Com o recurso de acesso remoto integrado do NVDA, você pode controlar outro computador executando o NVDA ou permitir que alguém controle seu computador.
Isso facilita o fornecimento ou recebimento de assistência, a colaboração ou o acesso remoto ao seu próprio computador.

O acesso remoto está desabilitado por padrão para sua segurança.
Você pode habilitar o Acesso Remoto [nas configurações de Acesso Remoto](#RemoteSettings).

### Começando {#RemoteAccessGettingStarted}

Antes de começar, certifique-se de que o NVDA esteja instalado e em execução em ambos os computadores.
Verifique também se a caixa de seleção Habilitar Acesso Remoto [nas configurações de Acesso Remoto](#RemoteSettings) está marcada em ambos os computadores.
O recurso de acesso remoto estará disponível no menu Ferramentas do NVDA; Não há necessidade de downloads ou instalações adicionais.

Se o NVDA estiver sendo executado no [modo seguro](#SecureMode), você não poderá iniciar manualmente uma nova sessão de Acesso Remoto.
No entanto, o NVDA ainda iniciará automaticamente uma nova sessão de Acesso Remoto se configurado para fazê-lo com a configuração [Conectar automaticamente após a inicialização do NVDA](#RemoteAutoconnect).
Você ainda pode desconectar manualmente no modo seguro.

### Configurando uma Sessão de Acesso Remoto {#RemoteAccessSetup}

As sessões de acesso remoto são configuradas por meio do diálogo de conexão de Acesso Remoto.
Abra o menu do NVDA e selecione Ferramentas, depois "Acesso Remoto" e depois "Conectar...".
Ou, de qualquer lugar, pressione `NVDA+alt+r`.

#### Seleção de Modo (Controlador ou Controlado) {#RemoteAccessMode}

Você precisará decidir qual computador será controlado (o computador controlado) e qual será o controlador (o computador controlador).

a caixa de combinação de modo permite que você escolha se este computador será controlado (outra pessoa interagirá com o seu computador) ou controlador (você interagirá com o computador de outra pessoa).

* Se você precisar que outra pessoa o ajude com seu computador, escolha "Permitir que este computador seja controlado".
* Se você for ajudar alguém com o computador, escolha "Controlar outro computador".

#### Conectando a um servidor existente (Anfitrião e Chave Fornecidos) {#RemoteAccessJoin}

Nesse caso, a outra pessoa iniciou a conexão por meio de um servidor de retransmissão ou está hospedando a conexão localmente.

1. Para Servidor, escolha "Usar existente".
1. No campo Anfitrião, insira o endereço do hospedeiro (e a porta, se também fornecida).
  * Você pode incluir a porta para se conectar anexando dois pontos (":") e o número da porta ao endereço do anfitrião.
Por exemplo, `exemplo.com:1234`.
1. No campo Chave, insira a senha fornecida.
Tenha cuidado para digitar a chave exatamente como fornecida.
1. Pressione OK.

#### Iniciando uma Conexão Via um Servidor de Retransmissão {#RemoteAccessConnectRelay}

Se permitir que outra pessoa se conecte a você pela Internet, a opção mais fácil é usar um servidor de retransmissão. Para fazer isso:

1. Para Servidor, escolha "Usar existente".
1. Para anfitrião, informe o nome do hospedeiro de um servidor de retransmissão, como `nvdaremote.com`.
Você precisará fornecer esse endereço à outra pessoa.
  * Você pode anexar a porta de conexão acrescentando dois pontos (":") e o número da porta ao anfitrião.
Por exemplo, `exemplo.com:1234`.
1. Para chave: crie uma senha longa e difícil de adivinhar ou pressione o botão "Gerar Chave" para gerar uma automaticamente.
Você precisará fornecê-la à outra pessoa.
  * Observe que a chave atua como o identificador da sessão e da senha, por isso é muito importante que seja difícil de adivinhar.
1. Pressione OK.
1. Forneça os detalhes da conexão (como anfitrião e chave) à outra pessoa para que ela possa se conectar.

#### Usando o NVDA como servidor {#RemoteAccessConnectSelfHosted}

Se você não deseja usar um servidor de retransmissão ou está se conectando por meio de uma rede local:

1. Para Servidor, escolha "Servir localmente".
1. Se estiver se conectando pela Internet, você pode pressionar o botão "Obter IP externo" para mostrar seu endereço IP externo (visível na Internet).
Você precisará fornecer esse IP externo para a outra pessoa.
1. Se estiver se conectando por uma rede local, você precisará fornecer à outra pessoa seu IP local.
A localização de IP local não é abrangida nesta documentação.
1. Em Porta, insira a porta que deseja usar para conexões remotas de entrada.
A outra pessoa também precisará disso.
  * Esta deve ser uma porta que não é usada por nenhum outro serviço.
  * Por padrão, o Acesso Remoto usa a porta 6837.
  * Se estiver se conectando pela internet, pode ser necessário redirecionar a porta.
  Esta documentação não abrange como redirecionar portas.
  Pode ser necessário entrar em contato com o administrador da rede para fazer isso.
1. Para chave: crie uma senha longa e difícil de adivinhar ou pressione o botão "Gerar Chave" para gerar uma automaticamente.
Você precisará fornecê-la à outra pessoa.
  * Observe que a chave atua como o identificador da sessão e da senha, por isso é muito importante que seja difícil de adivinhar.
1. Pressione OK.
...
### Usando o Acesso Remoto {#RemoteAccessUsage}

Depois que uma sessão de acesso remoto estiver ativa, você poderá alternar entre controlar o computador remoto e o seu próprio, compartilhar sua área de transferência e silenciar a sessão remota:

* Pressione `NVDA+alt+tab` para alternar entre controlar e retornar ao seu próprio computador.
* Envie texto da sua área de transferência para o outro computador abrindo o menu do NVDA, selecionando Ferramentas, depois "Acesso Remoto" e então "Enviar área de transferência".
* Se estiver conectado como o computador controlador, silencie a saída de fala do computador remoto em seu computador local abrindo o menu NVDA, selecionando Ferramentas, depois "Acesso remoto" e por fim "Silenciar controle remoto".
* Se estiver conectado como o computador controlador, envie `control+alt+delete` para o computador controlado abrindo o menu do NVDA, selecionando Ferramentas, "Acesso Remoto" e "Enviar control+alt+delete".
  Para que isso funcione, o computador controlado deve estar executando uma cópia instalada do NVDA.
  Além disso, dependendo das configurações do computador controlado, essa função pode nem sempre funcionar.

### Resumo das Teclas de Comando do Acesso Remoto {#RemoteAccessGestures}

<!-- KC:beginInclude -->
| Nome |Tecla |Descrição|
|---|---|---|
| Alternar Conexão remota | `NVDA+alt+r` | Inicia uma nova sessão de Acesso Remoto ou, se uma sessão já estiver em andamento, desconecta-se dela. |
| Alternar Controle | `NVDA+alt+tab` | Alterna entre o controle do computador remoto e local. |
| Conectar | Nenhum | Inicia uma nova sessão de Acesso Remoto. Indisponível no [modo seguro](#SecureMode). |
| Copiar link | Nenhum | Copia um link para a área de transferência da sessão remota. |
| Desconectar | Nenhum | Encerra uma sessão de Acesso Remoto existente. |
| Silenciar remoto | Nenhum | Silencia ou ativa o som da fala proveniente do computador remoto. |
...
#### Ordenando a lista de complementos por coluna {#AddonStoreSortByColumn}

Por padrão, a lista de complementos é classificada pelo nome de exibição dos complementos.
A caixa de combinação "Ordenar por colunas" pode ser usada para classificar a lista pelas colunas disponíveis para cada aba.
Por exemplo, você pode ordenar os complementos por editor, versão disponível, etc.
Os complementos podem ser ordenados em ordem crescente ou decrescente.
...
Se o download ou a instalação falhar, você pode tentar instalar novamente.
Também é possível cancelar a instalação antes de sair da Loja de Complementos.
...
#### Alterando o canal de atualização automática {#AddonStoreUpdateChannel}

Você pode gerenciar os canais de atualização automática para complementos nas [abas de complementos instalados e a atualizar](#AddonStoreFilterStatus).
Quando as [Atualizações automáticas de complementos](#AutomaticAddonUpdates) estão habilitadas, os complementos serão atualizados para o mesmo [canal](#AddonStoreFilterChannel) de onde foram instalados por [padrão](#DefaultAddonUpdateChannel).
A partir do menu de ações de um complemento, usando o submenu "Canal de atualizações", você pode modificar os canais para os quais um complemento será atualizado automaticamente.

| Opção | Comportamento |
|---|---|
| Padrão | O complemento seguirá o [canal de atualização padrão](#DefaultAddonUpdateChannel) |
| O mesmo | O complemento permanecerá no mesmo canal |
| Qualquer | O complemento será atualizado automaticamente para a versão mais recente, independentemente do canal |
| Não atualizar | O complemento não será atualizado automaticamente |
| Estável | O complemento será atualizado automaticamente para versões estáveis |
| Beta ou em desenvolvimento | O complemento será atualizado automaticamente para as versões beta ou em desenvolvimento |
| Beta | O complemento será atualizado automaticamente para versões beta |
| Em desenvolvimento | O complemento será atualizado automaticamente para versões em desenvolvimento |
...
Às vezes, podem surgir problemas com o Registro — Registry — do Windows, que resultam no comportamento anormal do NVDA.
Isso pode ser causado, por exemplo, pela instalação ou desinstalação de certos programas (como Adobe Reader ou Math Player), bem como atualizações do Windows e outros eventos.
A Ferramenta de Correção de Registro COM tenta corrigir esses problemas reparando as entradas de acessibilidade no Registro.

Os tipos de problemas que esta ferramenta pode corrigir incluem:

* O NVDA relata "desconhecido" ou "painel", ao navegar em navegadores como Firefox ou Edge, programas de e-mail como Thunderbird, Explorador do Windows, barra de tarefas e outros programas.
* O NVDA falha ao alternar entre o modo de foco e o modo de navegação quando você espera isso.
* Botões que antes tinham seus nomes falados, de repente passaram a ser relatados apenas como "botão".
* O NVDA fica muito lento ao navegar em navegadores enquanto usa o modo de navegação.

Como essa ferramenta corrige entradas no registro do Windows, ela requer acesso administrativo para funcionar, assim como na instalação de um programa.
Se você tiver o UAC (Controle de Acesso do Usuário) habilitado, como a maioria dos usuários, precisará seguir as instruções apresentadas pelo UAC para executar a ferramenta com êxito.
...
|tecla shift+tab |barraDeEspaçoBraille+ponto1+ponto2|
|tecla tab |barraDeEspaçoBraille+ponto4+ponto5|
|tecla setaParaCima |barraDeEspaçoBraille+ponto1|
|tecla setaParaBaixo |barraDeEspaçoBraille+ponto4|
|tecla control+setaParaEsquerda |barraDeEspaçoBraille+ponto2|
|tecla control+setaParaDireita |barraDeEspaçoBraille+ponto5|
|setaParaEsquerda |barraDeEspaçoBraille+ponto3|
|tecla setaParaDireita |barraDeEspaçoBraille+ponto6|
|tecla home |barraDeEspaçoBraille+ponto1+ponto3|
|tecla end |barraDeEspaçoBraille+ponto4+ponto6|
|tecla control+home |barraDeEspaçoBraille+ponto1+ponto2+ponto3|
|tecla control+end |barraDeEspaçoBraille+ponto4+ponto5+ponto6|
|tecla alt |barraDeEspaçoBraille+ponto1+ponto3+ponto4|
|tecla alt+tab |barraDeEspaçoBraille+ponto2+ponto3+ponto4+ponto5|
...
|tecla escape |barraDeEspaçoBraille+ponto1+ponto5|
|tecla windows |barraDeEspaçoBraille+ponto2+ponto4+ponto5+ponto6|
...
|tecla windows+d (minimizar todos os aplicativos) |barraDeEspaçoBraille+ponto1+ponto2+ponto3+ponto4+ponto5+ponto6|
|Anunciar a linha atual |barraDeEspaçoBraille+ponto1+ponto4|
|Menu do NVDA |barraDeEspaçoBraille+ponto1+ponto3+ponto4+ponto5|
...
|Guiar para a cela braille |sensor|
|tecla shift+tab |esc, tecla de ação tripla esquerda para cima+baixo|
|tecla alt |b2+b4+b5|
|tecla escape |b4+b6|
...
|tecla setaParaCima |espaço+ponto1|
|tecla setaParaBaixo |espaço+ponto4|
|tecla setaParaEsquerda |espaço+ponto3|
|tecla setaParaDireita |espaço+ponto6|
|tecla shift+tab |espaço+ponto1+ponto3|
|tecla tab |espaço+ponto4+ponto6|
|tecla alt |espaço+ponto1+ponto3+ponto4 (espaço+m)|
|tecla escape |espaço+ponto1+ponto5 (espaço+e)|
...
O Dot Pad — Bloco de Pontos — da Dot Inc é um dispositivo que pode exibir gráficos táteis atualizáveis e braille.
O NVDA pode exibir uma única linha de braille na linha de exibição de braille dedicada do dispositivo ou várias linhas de braille em sua área de gráficos táteis.
O modelo A300 possui uma área de gráficos táteis de 120 por 80 pontos, que comporta 8 linhas de 20 células cada.

Você pode configurar se o NVDA exibe braille na linha de exibição de braille dedicada ou na área de gráficos táteis por meio da opção Destino do Braille nas configurações de Braille do NVDA para este driver.

As teclas de movimento panorâmico são suportadas, mas devido a botões limitados no dispositivo, outros comandos e recursos de encaminhamento não estão disponíveis no momento.

Ao selecionar o driver do Dot Pad no NVDA, você deve selecionar manualmente a porta serial virtual USB / Bluetooth à qual o Dot Pad está conectado.
Esse driver não suporta detecção automática.

Por favor observe que, devido a limitações de hardware, o Dot Pad não atualizará todos os pontos corretamente enquanto sua mão estiver no dispositivo.
Certifique-se de levantar totalmente a mão do dispositivo ao navegar com o NVDA e só comece a ler novamente quando ele estiver totalmente atualizado.

<!-- KC:beginInclude -->

| Nome |Tecla|
|---|---|
|Rolar a linha braille para trás | `pan_left` |
|Rolar a linha braille para frente | `pan_right` |
...
* Abertura de documentos externos a partir do menu do NVDA, como o arquivo do guia do usuário.
...
|`-n IDIOMA` |`--lang=IDIOMA` |Suplanta o idioma configurado do NVDA. Defina como "Windows" para o padrão do usuário atual, "en" para inglês, etc.|
|`-m` |`--minimal` |Sem sons, sem interface, sem mensagem de início, etc.|
|`-s` |`--secure` |Inicia o NVDA em [Modo Seguro](#SecureMode)|
|`-d` |`--disable-addons` |Complementos não funcionarão|
...
Assim como você pode instalar o NVDA silenciosamente passando a opção de linha de comando `--install-silent` para o NVDA, ele pode ser desinstalado silenciosamente passando o comando `/S` para o desinstalador.

O desinstalador do NVDA é chamado `uninstall.exe` e reside no diretório de instalação do NVDA, `%ProgramFiles(x86)%\nvda` no Windows de 64-bit ou `%ProgramFiles%\nvda` no Windows de 32-bit.

A seguir estão as opções de linha de comando para o desinstalador do NVDA:

| Curta |Longa |Descrição|
|---|---|---|
|`/S` |Nenhuma |Desinstale silenciosamente o NVDA. |
