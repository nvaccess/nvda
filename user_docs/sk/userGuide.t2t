...
+ Zobrazí sa okno inštalácie programu NVDA s licenčnými podmienkami.
Ak si to želáte, licenciu si môžete čítať stláčaním klávesu ``Šípka dolu``.
+ Stlačením klávesu ``Tab`` prejdite na začiarkavacie políčko "Súhlasím" a začiarknite ho stlačením ``Medzerníka``.
+ Stláčajte ``Tab`` na prechádzanie po tlačidlách a požadovanú voľbu aktivujte stlačením ``Enter``. 
+
...
+ Uistite sa, že začiarkavacie políčko Súhlasím s licenčnou dohodou je začiarknuté.
+ Stláčaním klávesu ``Tab`` nájdite a stlačením ``enter`` aktivujte tlačidlo "Nainštalovať NVDA do počítača".
+ Nasledujú možnosti súvisiace s používaním NVDA pri prihlasovaní do systému Windows a vytvorením odkazu na pracovnej ploche.
...
+ Zobrazí sa dialógové okno kontroly používateľských účtov Windows s otázkou, či chcete tejto aplikácii povoliť vykonať zmeny v systéme.
+ Stlačte ``alt+a`` aby ste akceptovali výzvu.
+ Počas inštalácie sa zobrazí  indikátor priebehu.
Počas tohto procesu NVDA prehráva postupne sa zvyšujúci tón.
Môže to prebehnúť veľmi rýchlo a nemusíte si to vôbec všimnúť.
+ Na konci sa zobrazí dialógové okno potvrdzujúce úspešnosť inštalácie.
Správa napovedá "Stlačte OK, aby ste spustili nainštalovanú verziu.".
Stlačte ``enter`` a spustí sa nainštalovaná verzia NVDA.
+ NVDA prečíta uvítaciu správu a zobrazí sa dialógové okno "Vitajte v NVDA".
...
+ Stlačte kláves ``tab`` a prejdite na "Používať Capslock ako kláves NVDA".
...
+ Použite klávesy ``tab`` a ``medzerník`` na úpravu ostatných nastavení na tejto obrazovke.
Určujú, či sa NVDA bude spúšťať automaticky.
+ Stlačte kláves ``enter`` na zatvorenie dialógového okna a NVDA ostane spustené.
+
...
+ Otvoríte si nejaký dokument v režime prehliadania a následne otvoríte dialóg s nastavením klávesových skratiek.
+ V časti režim prehliadania nájdete položku "prejde na nasledujúcu záložku".
+ Priradíte skratku ``y``.
+ Nájdete položku "prejde na predchádzajúcu záložku".
+ Priradíte skratku ``shift+y``.
+
...
+ Presuňte kurzor na miesto, kde sa nachádza poznámka.
+ Otvorte kontextové menu.
+ Vyberte položku upraviť poznámku.
+
...
+ Uistite sa, že máte nainštalovaný [Najnovší ovládač https://www.papenmeier-rehatechnik.de/en/service/downloadcenter/software/articles/software-braille-devices.html].
+ Otvorte správcu zariadení Windows.
+ Nájdite vetvu USB zariadenia.
+ Vyberte "Papenmeier Braillex USB Device".
+ Otvorte vlastnosti a prejdite na záložku pokročilé.
...
+ Vypnite možnosť "Load VCP" (načítať vcp).
+
