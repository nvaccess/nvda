...
* Možnosť [pripojiť sa a ovládať iný počítač so spusteným NVDA na diaľku](#RemoteAccess), čo umožňuje pomoc na diaľku alebo spoluprácu
...
* Podpora pre prihlasovaciu obrazovku windows a [iné zabezpečené obrazovky](#SecureScreens)
...
Okrem Angličtiny je NVDA preložený do 55 jazykov, vrátane: afrikánčiny, albánčiny, amharčiny, arabčiny, aragónčiny, bosniačiny, bulharčiny, barmčiny, katalánčiny, čínštiny (zjednodušenej aj tradičnej), chorvátčiny, češtiny, dánčiny, holandčiny, perzštiny (fársí), fínčiny, francúzštiny, galícijčiny, gruzínčiny, nemčiny (Nemecko a Švajčiarsko), gréčtiny, hebrejčiny, hindčiny, maďarčiny, islandčiny, írčiny, taliančiny, japončiny, kannadčiny, kórejčiny, kirgizštiny, litovčiny, macedónčiny, mongolčiny, nepálčiny, nórčiny, poľštiny, portugalčiny (Brazília a Portugalsko), pandžábčiny, rumunčiny, ruštiny, srbčiny, slovenčiny, slovinčiny, španielčiny (Kolumbia a Španielsko), švédčiny, tamilčiny, thajčiny, turečtiny, ukrajinčiny a vietnamčiny.
...
Spustenie môže chvíľu trvať.
Po úspešnom spustení už NVDA ozvučí celý proces inštalácie.
1. Objaví sa okno s tlačidlom, ktorým je možné otvoriť licenčné podmienky.
Licenčné podmienky môžete čítať stláčaním `šípky dole`.
...
1. Zobrazí sa dialóg s potvrdením úspešnej inštalácie.
...
1. Stlačte kláves `enter` na zatvorenie dialógového okna a NVDA ostane spustený.
...
| Názov |Klávesová skratka pre Desktop |Klávesová skratka pre laptop |Popis|
|---|---|---|---|
|Čítať napísané znaky |`NVDA+2` |`NVDA+2` |určuje, kedy NVDA oznamuje znaky, ktoré ste napísali. |
|Čítať napísané slová |`NVDA+3` |`NVDA+3` |Určuje, kedy NVDA oznamuje slová, ktoré ste napísali. |
...
Odporúčame vám začať s modulom Základy NVDA ("Basic Training for NVDA").
...
Umožňuje určiť, či budú nastavenia z aktuálne spustenej kópie NVDA skopírované do používateľského profilu aktuálne prihláseného používateľa. Tieto nastavenia môžu byť následne použité s nainštalovanou verziou NVDA.
...
Predvolene sa používa rozloženie desktop. Na rozloženie Laptop môžete kedykoľvek prepnúť v menu NVDA, Možnosti, [nastavenia](#NVDASettings), v kategórii klávesnica.
...
Klepnutie raz jedným prstom je jednoduché klepnutie.
...
Pri spustenej nápovede vstupu sa akcie nevykonávajú.
...
|Prezerací kurzor na začiatok výberu |`NVDA+alt+home` |`NVDA+alt+home` |nie je |Presunie prezerací kurzor na prvý znak vybratého textu|
|Prezerací kurzor na koniec výberu |`NVDA+alt+end` |`NVDA+alt+end` |nie je |Presunie prezerací kurzor na posledný znak vybratého textu|
...
Dobrý spôsob, ako si zapamätať základné príkazy na prehliadanie textu pri použití rozloženia Desktop, je predstaviť si ich ako mriežku tri krát tri, kde zhora nadol sú riadky, slová a znaky a zľava doprava predchádzajúci, aktuálny a nasledujúci.
...
| Názov |Klávesová skratka |Popis|
|---|---|---|
|Nájdi text |NVDA+ctrl+f |otvorí dialóg hľadania|
|Vyhľadať nasledujúci výskyt |NVDA+f3 |Vyhľadá nasledujúci výskyt hľadaného reťazca od aktuálnej pozície kurzora|
|Vyhľadať predchádzajúci výskyt |NVDA+shift+f3 |Vyhľadá predchádzajúci výskyt hľadaného reťazca od aktuálnej pozície kurzora|
...
Je však možné aktivovať aj vstavaný režim formátovaného výberu, ktorý je dostupný v niektorých dokumentoch v režime prehliadania (v prehliadači Mozilla Firefox, a prehliadačoch postavených na Chromium od verzie 134), čo spôsobí, že text vybratý v NVDA sa označí aj v prehliadači.
...
Stlačením `NVDA+f5` obnovíte rozpoznaný text.
...
To znamená, že ak neskôr dokument otvorí používateľ, ktorý používa iný čítač obrazovky, bude mať už správne nastavené hlavičky v tabuľkách.
...
<!-- KC:beginInclude -->
Ak si chcete prečítať komentár na pozícii systémového kurzora, stlačte NVDA+alt+c`.
Stlačené dva krát rýchlo za sebou zobrazí informáciu v režime prehliadania.
<!-- KC:endInclude -->
...
<!-- KC:endInclude -->
Tieto nastavenia sa natrvalo uložia do dokumentu tak, aby im porozumeli aj iné čítače obrazovky (napríklad JAWS).
To znamená, že ak neskôr zošit otvorí používateľ, ktorý používa iný čítač obrazovky, bude mať už správne nastavené hlavičky v tabuľkách.
...
|Oznámiť dĺžku skladby |ctrl+shift+t |Oznámi dĺžku skladby, ak je spustené prehrávanie.|
...
NVDA ponúka vylepšenú podporu pre Poedit 3.5 alebo novší.
...
Ak je začiarknuté, pred ukončením NVDA sa zobrazí dialóg s nasledujúcimi možnosťami:

* ukončiť
* reštartovať
* reštartovať s vypnutými doplnkami a najvyššou úrovňou záznamu
* reštartovať a nastaviť najvyššiu úroveň záznamu
* nainštalovať čakajúcu aktualizáciu (ak je dostupná).
...
* Aktuálny jazyk NVDA
* Či používate prenosnú alebo nainštalovanú verziu NVDA
...
##### Alternatívny zdroj aktualizácie {#UpdateMirror}

Tu môžete nastaviť alternatívny zdroj, z ktorého NVDA sťahuje aktualizácie.
Toto je užitočné, ak je prístup k hlavnému aktualizačnému serveru NV Access pomalý alebo úplne nemožný.

V neprepisovateľnom editačnom poli je uvedená aktuálna URL adresa zdroja.
Ak sa nepoužíva alternatívny zdroj (teda aktualizácie sa sťahujú z hlavného zdroja NV Access), v neprepisovateľnom editačnom poli je napísané "žiadny alternatívny zdroj".

Ak chcete nastaviť alternatívny zdroj aktualizácie, aktivujte tlačidlo "zmeniť", čím sa otvorí [dialóg s nastaveniami alternatívneho zdroja](#SetURLDialog).

Upozorňujeme, že administrátor alternatívneho zdroja má prístup k [informáciám odosielaným počas kontroly aktualizácií](#GeneralSettingsCheckForUpdates).
Pred použitím alternatívneho zdroja kontaktujte autora a uistite sa, že s vašimi údajmi zaobchádza podľa vašich predstáv.

##### Nevypínať obrazovku počas plynulého čítania a čítania na brailovom riadku {#PreventDisplayTurningOff}

Ak je táto možnosť zapnutá, Nvda zabráni vypnutiu obrazovky počas plynulého čítania, alebo keď čítate text s použitím brailového riadka (pri posúvaní tlačidlami na riadku).
Takto môžete zabrániť tomu, že sa počas plynulého čítania zamkne obrazovka.
Predvolene je toto nastavenie začiarknuté.
Zvážte vypnutie tejto možnosti, ak potrebujete zvýšiť výdrž batérie.
...
Prvou možnosťou v kategórii reč, je tlačidlo zmeniť.
Toto tlačidlo aktivuje [Nastavenia hlasového výstupu](#SelectSynthesizer), kde je možné nastaviť hlasový výstup.
...
Ak zvolíte možnosť "automaticky", tabuľka bude nastavená podľa jazyka NVDA.
...
##### Privolávacie tlačidlá posúvajú systémový aj prezerací kurzor {#BrailleSettingsReviewRoutingMovesSystemCaret}

Toto nastavenie určuje, či sa pri používaní privolávacích tlačidiel na brailovom riadku posúva aj systémový kurzor.
Predvolene je toto nastavenie nastavené na nikdy, takže pri stláčaní smerových tlačidiel sa systémový kurzor neposúva.

Ak je táto možnosť nastavená na vždy, a súčasne je [Brailový kurzor zviazaný s](#BrailleTether) nastavený na automaticky, alebo na prezerací kurzor, stlačenie privolávacích tlačidiel presunie aj systémový kurzor, alebo zameria položku pod kurzorom, ak je to možné.
Ak je aktuálny režim prezerania [prezeranie obrazovky](#ScreenReview), fyzicky systémový kurzor neexistuje.
V tomto prípade sa NVDA pokúsi premiestniť fokus na objekt, na ktorého texte ste použili privolávacie tlačidlá.
...
V tomto prípade budú privolávacie tlačidlá posúvať systémový kurzor len vtedy, ak došlo automaticky k zviazaniu systémového a prezeracieho kurzora.
...
##### Značka začiatku odseku {#BrailleParagraphStartMarkers}

Ak je začiarknuté nastavenie "Čítať po odsekoch", vybratá značka začiatku odseku bude zobrazená na začiatku každého odseku.
Toto môže byť užitočné predovšetkým v aplikáciách bežne používaných na čítanie väčšieho množstva textu, akými sú štrukturované dokumenty alebo knihy.
V takýchto dokumentoch môže poznanie pozície začiatku odseku pomôcť lepšie porozumieť štruktúre obsahu, alebo môže byť užitočné pri ukladaní záložiek alebo pri písaní vlastných poznámok.

Medzi možnosti patrí použitie dvoch medzier ako nenápadného zalomenia odseku a symbolu odseku, označenie odseku (¶), ako zjavnejšieho zalomenia.

| . {.hideHeaderRow} |.|
|---|---|
|Možnosti |Neoznačovať začiatok odseku, dvojitá medzera (  ), Označenie odseku (¶)|
|Predvolené |Neoznačovať začiatok odseku|
...
##### Zobrazovanie formátovania {#BrailleFormattingDisplay}

Toto nastavenie určuje, ako bude NVDA zobrazovať formátovanie textu v brailovom písme.
Prejaví sa to len vtedy, ak je aktívne [zobrazovanie vlastností písma v brailovom písme](#DocumentFormattingFontAttributes).

| . {.hideHeaderRow} |.|
|---|---|
| Možnosti | Predvolené (Liblouis), Liblouis, Značky |
| Predvolené | Liblouis |

| Možnosť |Správanie |
|---|---|
| Liblouis | Použije sa pôvodné formátovanie v brailovom písme. Pozor, táto možnosť zobrazuje len tučný alebo podčiarknutý text, alebo text písaný kurzívou a to len vtedy, ak aj vybratá tabuľka brailovho písma podporuje znázornenie týchto vlastností. |
| [Značky](#BrailleFormattingDisplayTags) | Použijú sa značky, ktoré popisujú ako a kde sa mení formátovanie textu. |

###### Značky {#BrailleFormattingDisplayTags}

Ak je zobrazovanie formátovania nastavené na "Značky", v brailovom písme sa zobrazí príslušná podporovaná značka vždy, keď sa formátovanie zmení.
Každá takáto značka zmeny formátovania sa začína symbolom ⣋ (bodmi 12478) a končia symbolom ⣙ (bodmi 14578).
Formátovacia značka okrem toho obsahuje jeden alebo viacej symbolov označujúcich vlastnosť formátovania textu.
K dispozícii sú tieto symboly:

| Symbol | Význam |
|---|---|
| ⠆ (body 23) | Začiatok tučného textu |
| ⠆⠜ (body 23 a body 345) | Koniec tučného textu |
| ⠔ (body 35) | Začiatok textu písaného kurzívou |
| ⠔⠜ (body 35 a body 345) | Koniec textu písaného kurzívou |
| ⠤ (body 36) | Začiatok podčiarknutého textu |
| ⠤⠜ (body 36 a body 345) | Koniec podčiarknutého textu |
| ⠄ (bod 3)| Začiatok prečiarknutého textu |
| ⠄⠜ (bod 3 a body 345) | Koniec prečiarknutého textu |

##### Vyslovovať znaky pri posúvaní kurzora privolávacími tlačidlami brailového riadku {#BrailleSpeakOnRouting}

Ak je to povolené, NVDA automaticky vysloví znak v texte, na ktorý presuniete kurzor použitím privolávacích tlačidiel brailového riadku.

Ak si túto voľbu želáte prepínať odkiaľkoľvek zo systému, prosím priradte si vlastnú skratku v dialógu [Klávesové skratky](#InputGestures).

##### Hovoriť pri pohybe po riadkoch a odsekoch {#BrailleSpeakOnNavigating}

Zapnutie tejto možnosti spôsobí, že NVDA bude oznamovať riadky a odseky aj v prípade, že na pohyb po texte použijete tlačidlá na brailovom riadku.

Ak chcete túto možnosť prepínať odkiaľkoľvek, nastavte skratku na funkciu "Oznamuje text pri pohybe po riadkoch a odsekoch" v časti braille v dialógu [Klávesové skratky](#InputGestures).

##### Zabrániť deleniu slov, keď je to možné {#BrailleSettingsWordWrap}

Ak je to povolené, priveľmi dlhé slovo, ktorého zobrazenie sa nezmestí na koniec brailového riadku, nebude rozdelené.
Namiesto toho bude na konci riadku nejaké prázdne miesto.
Celé nerozdelené slovo si budete môcť prečítať, len čo posuniete zobrazenie vpred.
Niekedy sa toto nazýva aj "zalamovanie slov".
Všimnite si, že ak sa samotné slovo nezmestí na riadok, musí byť aj tak rozdelené.

Ak je to zakázané, čo najviac zo slova sa zobrazí na konci riadku a zvyšok bude odstrihnutí.
Odstrihnutý zvyšok slova si budete môcť prečítať, len čo posuniete zobrazenie vpred.

Povolenie tejto voľby predpokladá priamočiarejšie čítanie na úkor toho, že budete musieť zobrazenie v brailovom písme viac posúvať.

##### Normalizovať znaky unicode {#BrailleUnicodeNormalization}

Keď je táto možnosť zapnutá, vykoná sa normalizácia unicode na texte, ktorý je zobrazovaný na brailovom riadku.
...
Ak si želáte normalizáciu znakov unicode prepínať odkiaľkoľvek zo systému, prosím priraďte si vlastnú skratku [v Dialógu klávesové skratky](#InputGestures).
...
Súčasné čítanie textu v brailovom písme a počúvanie iného textu môže byť rušivé.
Z tohto dôvodu je predvolene zapnuté prerušenie reči počas posúvania textu zobrazeného na brailovom riadku.
...
Táto možnosť je dostupná, len ak je NVDA nainštalovaný.
...
Toto nastavenie určuje, kedy NVDA oznamuje znaky, ktoré píšete na klávesnici.
Dostupné sú tieto možnosti:

* Vypnuté: NVDA vôbec neoznamuje napísané znaky.
* Len v editačných poliach: NVDA bude oznamovať napísané znaky len v editačných poliach a na miestach, kde je možné vkladať text.
* Vždy: NVDA bude oznamovať napísané znaky vždy (napríklad aj pri použití navigácie pomocou prvých písmen na pracovnej ploche a v zozname súborov).
...
Toto nastavenie určuje, kedy NVDA oznamuje slová, ktoré píšete na klávesnici.
Dostupné sú tieto možnosti:

* Vypnuté: NVDA vôbec neoznamuje napísané slová.
* Len v editačných poliach: NVDA bude oznamovať napísané slová len v editačných poliach a na miestach, kde je možné vkladať text.
* Vždy: NVDA bude oznamovať napísané slová vždy.
...
##### Časový limit pre viacnásobné stlačenie {#MultiPressTimeout}

Niektorými klávesovými skratkami NVDA vykonávame rôzne akcie podľa toho, koľko krát klávesy stlačíme rýchlo za sebou.
Príkladom takéhoto použitia je príkaz "Prečítať znak aktuálneho navigačného objektu, na ktorý ukazuje prezerací kurzor".
Tento príkaz pri prvom stlačení prečíta znak, pri druhom stlačení ho vyhláskuje a pri treťom stlačení prečíta jeho numerickú hodnotu.
Toto nastavenie určuje časový limit, po ktorom bude ďalšie stlačenie tej istej kombinácie vyhodnotené ako nová skratka namiesto ďalšieho stlačenia pôvodnej klávesovej skratky.
Ak by ste si pre príkaz uvedený ako príklad nastavili veľmi krátky časový limit, dvojité stlačenie by dva krát oznámilo znak namiesto jeho vyhláskovania.
Predvolený časový limit je 500 ms, teda pol sekundy.
Zvýšenie časového limitu môže byť užitočné pre používateľov, ktorí používajú funkciu klávesnice jedným prstom alebo môže byť tiež výhodné pre telesne postihnutých.
...
Ak túto možnosť vypnete, dotyková obrazovka sa bude správať tak, akoby NVDA nebol spustený.
...
Táto možnosť je predvolene začiarknutá a spôsobuje, že skratky, ktoré sa nepoužívajú v rýchlej navigácii (napríklad písmená), nebudú posielané do aktívnej aplikácie.
...
* písmo
  * Názov písma
  * Veľkosť písma
  * Vlastnosti písma [(Vypnuté, Vyslovovať, Zobrazovať v brailovom písme, Vyslovovať aj zobrazovať v brailovom písme)](#DocumentFormattingFontAttributes)
  * indexy
  * Zvýraznenie
  * vyznačený text
  * štýl
...
* Prvky
  * nadpisy
  * odkazy
  * Typ odkazu (odkazy smerujúce na obsah na tej istej stránke)
  * grafiku
  * zoznamy
  * citácie
  * Zoskupenia
...
##### Vlastnosti písma {#DocumentFormattingFontAttributes}

Týmto je možné vybrať, akým spôsobom budú oznamované vlastnosti písma tučný text, text kurzívou, podčiarknutý a prečiarknutý text.
Zoznamový rámik obsahuje tieto štyri možnosti:

* Vypnuté: NVDA nebude vôbec oznamovať tieto vlastnosti písma.
* Vyslovovať: NVDA bude vyslovovať zmeny týchto vlastností písma.
* Zobrazovať v brailovom písme: NVDA bude tieto vlastnosti písma zobrazovať na brailovom riadku.
Presnejšie možnosti zobrazenia môžete nastaviť [v časti brailovo písmo](#BrailleFormattingDisplay).
* Vyslovovať aj zobrazovať v brailovom písme: NVDA bude oznamovať vlastnosti písma oboma práve popísanými spôsobmi.
...
* Reč: ak sa zmení odsadenie riadka, budete počuť napríklad "dvanásť medzera" alebo "štyry tab".
...
##### Automatické aktualizácie {#AutomaticAddonUpdates}
...
Táto kontrola dostupnosti aktualizácií doplnkov v katalógu s doplnkami je automaticky vykonávaná každých 24 hodín.
Predvolene sa upozornenia zobrazujú podľa určeného [zdroja](#AddonStoreFilterChannel) (napríklad stabilné, beta alebo vo vývoji).
Zdroje aktualizácií môžete nastaviť pre [každý doplnok samostatne](#AddonStoreUpdateChannel) alebo pre [všetky doplnky](#DefaultAddonUpdateChannel).

Ak nastavíte možnosť "automaticky aktualizovať", doplnky sa aktualizujú na pozadí.
Po skončení aktualizácie budete vyzvaní na reštartovanie NVDA.

| . {.hideHeaderRow} |.|
|---|---|
|Možnosti |upozorniť (predvolené), automaticky aktualizovať, zakázané |
|Predvolené |Upozorňovať |

|Možnosť |Správanie |
|---|---|
|Upozorniť |Upozorní, ak sú dostupné aktualizácie doplnkov |
|Automaticky aktualizovať |Automaticky aktualizuje doplnky na pozadí |
|Vypnuté |Vôbec nebude zisťovať dostupnosť aktualizácií doplnkov |

##### Predvolený zdroj aktualizácií {#DefaultAddonUpdateChannel}

Keď sú zapnuté [Automatické aktualizácie doplnkov](#AutomaticAddonUpdates), predvolene sa používa jeden konkrétny [zdroj](#AddonStoreFilterChannel).
Napríklad nainštalovaná beta verzia sa bude aktualizovať na novšiu beta verziu.
Táto možnosť nastavuje zdroj aktualizácie pre všetky nainštalované doplnky.
Môžete však nastaviť [zdroj aktualizácie pre každý doplnok zvlášť v katalógu s doplnkami](#AddonStoreUpdateChannel).

| . {.hideHeaderRow} |.|
|---|---|
| Možnosti | rovnaké (predvolené), ktorýkoľvek, neaktualizovať, stabilné, Beta alebo vo vývoji, Beta, vo vývoji |
| Predvolené | rovnaké |

| Možnosť |Správanie |
|---|---|
| Rovnaké | Doplnky sa aktualizujú podľa toho, z akého zdroja boli pôvodne nainštalované |
| Akýkoľvek | doplnky budú aktualizované na najnovšiu verziu z ktorého koľvek zdroja |
| Neaktualizovať | Doplnky sa nebudú aktualizovať, ich aktualizáciu je potrebné nastaviť pre každý doplnok zvlášť ručne |
| Stabilné | Aktualizovať sa bude len na najnovšiu stabilnú verziu doplnku |
| Beta alebo vo vývoji | doplnky sa budú aktualizovať na najnovšiu Beta verziu alebo verziu vo vývoji |
| Beta | doplnky sa budú aktualizovať na najnovšiu beta verziu |
| Vo vývoji | Doplnky sa budú aktualizovať na najnovšiu verziu vo vývoji |

##### Povoliť aktualizáciu na nekompatibilnú verziu doplnku {#AllowIncompatibleAddonUpdates}

Toto nastavenie umožňuje aktualizovať doplnok, ktorého novšia verzia nie je kompatibilná s verziou NVDA.
Predvolene je táto možnosť vypnutá, čo zaručuje, že doplnky sú aktualizované len v prípade, že sú kompatibilné s aktuálnou verziou NVDA.
Automatické aktualizácie vždy aktualizujú nekompatibilné doplnky na kompatibilné, ak sú dostupné.
Zapnutie tejto možnosti môže byť užitočné, ak prechádzate na novú verziu doplnku v prechodnom období (zvyčajne na začiatku roka).
Toto je užitočné pre alpha a beta testerov, ktorí testujú kompatibilitu doplnkov v ranom vývoji v prechodnom období, zvyčajne začiatkom roka.

##### Alternatívny zdroj {#AddonStoreMetadataMirror}

Tu môžete nastaviť alternatívny zdroj, z ktorého NVDA sťahuje aktualizácie doplnkov.
Toto je užitočné, ak je prístup k hlavnému aktualizačnému serveru NV Access pomalý alebo úplne nemožný.
...
#### Nastavenia vzdialeného ovládania {#RemoteSettings}

Tu môžete nastaviť správanie [vzdialeného prístupu](#RemoteAccess).

##### Povoliť vzdialený prístup {#RemoteEnable}

Toto začiarkávacie pole použite na zapnutie alebo vypnutie funkcie vzdialeného prístupu.

Ak je táto možnosť vypnutá, potom je vzdialený prístup nedostupný.

Upozorňujeme, že ak chcete používať vzdialený prístup na zabezpečených a prihlasovacích obrazovkách, ako napríklad kontrola používateľských kont, je najprv potrebné zapnúť túto možnosť, následne uložiť nastavenia a potom [skopírovať nastavenia do systémového priečinka NVDA](#GeneralSettingsCopySettings).
Vzdialený prístup dokáže v tomto prípade pokračovať na zabezpečených obrazovkách len ak bolo spojenie iniciované z nainštalovanej verzie NVDA.
Nie je možné obsluhovať zabezpečené obrazovky, ak bolo spojenie nadviazané z prenosnej verzie NVDA.

Nasledujúce možnosti sú dostupné, ak je začiarknuté začiarkávacie pole vzdialený prístup.

##### Potvrdiť odpojenie ak je tento počítač ovládaný na diaľku {#RemoteConfirmDisconnect}

Určuje, či bude vyžadované potvrdenie odpojenia od vzdialeného ovládania, ak ste zvolili možnosť, že tento počítač je ovládaný z iného počítača.

Ak začiarknete, NVDA bude vyžadovať potvrdenie odpojenia.
Ak toto pole odčiarknete, NVDA vás od vzdialeného ovládania odpojí okamžite po aktivovaní príslušnej položky alebo aktivovaní klávesovej skratky.

##### Automaticky pripojiť po spustení NVDA {#RemoteAutoconnect}

Umožňuje automaticky spustiť vzdialené ovládanie po štarte NVDA.
Toto môže byť užitočné, ak chcete napríklad ovládať váš domáci počítač na diaľku.

Túto možnosť však používajte opatrne, nakoľko môže dôjsť k neoprávnenému používaniu vášho počítača bez vášho súhlasu.

Viac informácií o nasledujúcich možnostiach nájdete v časti [Nastavenia vzdialeného ovládania](#RemoteAccessSetup)

##### Režim {#RemoteAutoconnectMode}

Zvoľte režim, ktorý sa použije pri automatickom spustení vzdialeného prístupu.

Táto možnosť je dostupná, len ak je začiarknuté [Automaticky pripojiť po spustení NVDA](#RemoteAutoconnect).

|Možnosť |Správanie |
|---|---|
|Povoliť ovládanie tohto počítača |Tento počítač bude ovládaný iným počítačom v sieti. Toto umožní druhej osobe ovládať počítač tak, ako keby mala k nemu fyzický prístup. |
|Ovládať iný počítač |Umožňuje použiť tento počítač na ovládanie iného počítača. Takto môžete z vášho počítača odosielať príkazy do iného počítača. |

##### Server {#RemoteAutoconnectServer}

Vyberte typ servera, ktorý sa použije pri automatickom pripojení k vzdialenému prístupu.

Táto možnosť je dostupná, len ak je začiarknuté [Automaticky pripojiť po spustení NVDA](#RemoteAutoconnect).

|Možnosť |Správanie |
|---|---|
|Použiť existujúci server |Použiť existujúci server na nadviazanie spojenia (odporúča sa). |
|Vytvoriť server |Použiť spustenú verziu NVDA ako server (pokročilé). Toto si vyžaduje venovať pozornosť konfigurácii vašej siete, čo je nad rámec tejto používateľskej príručky. |

##### Hostiteľ {#RemoteAutoconnectHost}

Sem vložte URL adresu servera, cez ktorý sa chcete pripájať.
Ak server používa iný ako predvolený port 6837, napíšte ho do adresy servera za dvojbodkou (napríklad `priklad.sk:1234`).

Táto možnosť je dostupná, len ak je začiarknuté [Automaticky pripojiť po spustení NVDA](#RemoteAutoconnect) a v časti [Server](#RemoteAutoconnectServer) je nastavené "použiť existujúci server".

##### Port {#RemoteAutoconnectPort}

Sem zadajte TCP port, ktorý chcete používať pri automatickom pripojení k vzdialenému ovládaniu.

Táto možnosť je dostupná, len ak je začiarknuté [Automaticky pripojiť po spustení NVDA](#RemoteAutoconnect) a v časti [Server](#RemoteAutoconnectServer) je nastavené "vytvoriť server".

##### Kľúč {#RemoteAutoconnectKey}

Sem zadajte kľúč (heslo), ktorý sa použije pri automatickom spojení.

Táto možnosť je dostupná, len ak je začiarknuté [Automaticky pripojiť po spustení NVDA](#RemoteAutoconnect).

##### Vymazať všetky dôverované odtlačky {#RemoteDeleteFingerprints}

Umožňuje odstrániť uložené certifikáty serverov, ku ktorým ste sa v minulosti pripojili.
To znamená, že bude potrebné nanovo schváliť pripojenie k neznámym serverom a to aj k tým, ku ktorým ste už v minulosti boli pripojení.
Zmazanie certifikátov je potrebné potvrdiť.

Táto možnosť je dostupná len v prípade, že sú uložené certifikáty.
...
#### Nastavenie zdroja {#SetURLDialog}

Tento dialóg umožňuje nastaviť alternatívny zdroj, ktorý sa použije pri [aktualizácii NVDA](#GeneralSettingsCheckForUpdates) alebo [doplnkov](#AddonsManager).
Toto je užitočné, ak je prístup k hlavnému aktualizačnému serveru NV Access pomalý alebo úplne nemožný.

* Ak nastavujete alternatívny zdroj pre [aktualizácie NVDA](#UpdateMirror), V názve okna bude uvedené "alternatívny zdroj aktualizácií NVDA".
* Ak nastavujete zdroj [pre aktualizácie doplnkov](#AddonStoreMetadataMirror), potom bude názov okna "alternatívny zdroj aktualizácií doplnkov".

##### URL {#SetURLTextbox}

Zadajte URL (webovú adresu) alternatívneho zdroja.
Podporované sú len protokoly HTTP a HTTPS.
Odporúčame vám používať protokol HTTPS vždy, keď je to možné.

Ponechajte pole prázdne, ak chcete používať predvolený server NV Access.

##### Testovať... {#SetURLTest}

Toto tlačidlo použite na otestovanie zadanej adresy alternatívneho zdroja.
Pre správnu funkčnosť testu sa vyžaduje internetové pripojenie.
Odporúčame vám pred uložením nastavení vždy otestovať zadanú adresu servera.
...
* Len ak je to nevyhnutné: ak objektový model Microsoft word nie je dostupný
...
##### Odstrániť nadbytočné počiatočné medzery v reči {#TrimLeadingSilenceSpeech}

Ak je zapnuté, NVDA automaticky odstráni nadbytočné pauzy na začiatku reči, čo môže zvýšiť odozvu niektorých hlasových výstupov.
Táto možnosť je predvolene zapnutá a mala by ovplyvniť len medzery na začiatku vyslovovaných fráz.
Ak sa ukazuje, že hlasový výstup ignoruje aj prirodzené očakávané pauzy (napríklad medzi vetami), vypnutie tejto možnosti môže vyriešiť váš problém.

##### Na výstup pre reč cez Sapi 4 používať rozhranie WASAPI {#UseWASAPIForSAPI4}

Táto možnosť umožňuje použiť systém Windows Audio Session API (WASAPI) ako výstupný zdroj aj pre hlasové výstupy používajúce Microsoft Speech API version 4 (SAPI 4).
Toto umožňuje používať funkcie ako automatické stišovanie, odstraňovanie nadbytočných páuz v reči a udržiavanie aktívneho zariadenia aj v prípade, že používate hlasový výstup cez rozhranie SAPI 4.
Avšak niektoré SAPI 4 hlasové výstupi nemusia spolupracovať s aktuálnou WASAPI implementáciou.
Ak sapi 4 hlasový výstup, ktorý používate, prestane fungovať, odporúčame vám vypnúť túto možnosť.
...
Štandardné opačné lomky je potrebné zadávať v poli nahradiť v dvojiciach, napríklad "a\\\\b" napíšte, ak očakávate "a\\b".
...
## Vzdialené ovládanie {#RemoteAccess}

Vďaka vstavanej funkcii vzdialeného ovládania, môžete prostredníctvom NVDA ovládať na diaľku iný počítač, prípadne umožniť ovládanie vášho počítača.
Takto môžete niekomu pomôcť na diaľku, prípadne pomoc od niekoho prijať, spolupracovať na spoločnom projekte, alebo pristupovať k domácemu počítaču na diaľku.

Z bezpečnostných dôvodov je táto funkcia predvolene vypnutá.
Môžete ju zapnúť v nastaveniach v časti [vzdialené ovládanie](#RemoteSettings).

### Začíname {#RemoteAccessGettingStarted}

Skôr než začnete, uistite sa, že NVDA je nainštalovaný a spustený na oboch počítačoch.
Tiež sa uistite, že na oboch počítačoch je v nastaveniach Nvda, [vo vetve vzdialené ovládanie](#RemoteSettings), začiarknutá možnosť povoliť vzdialené ovládanie.
Vzdialené ovládanie je dostupné z menu nástroje v ponuke NVDA; nie je potrebné nič viac sťahovať ani inštalovať.

Ak je NVDA spustený v [bezpečnom režime](#SecureMode), nie je možné sa k vzdialenému ovládaniu pripojiť ručne.
Avšak, dokáže sa pripojiť k vzdialenému ovládaniu, ak povolíte [Automatické pripojenie k vzdialenému serveru po spustení NVDA](#RemoteAutoconnect).
V bezpečnom režime je možné spojenie ručne ukončiť.

### Nastavenie vzdialeného prístupu {#RemoteAccessSetup}

Vzdialené ovládanie je možné nastaviť v samostatnom dialógu.
Z ponuky NVDA zvoľte nástroje, "vzdialené ovládanie" a následne "pripojiť...".
Alebo kedykoľvek stlačte `NVDA+alt+r`.

#### Voľba režimu (ovládať vzdialený počítač alebo povoliť ovládanie tohto počítača) {#RemoteAccessMode}

Najprv je potrebné dohodnúť sa, ktorý počítač budete ovládať (povoliť ovládanie tohto počítača) a ktorý počítač budete na ovládanie používať (ovládať vzdialený počítač).

v zoznamovom rámiku režim môžete určiť, či budete ovládať počítač niekoho iného, alebo chcete povoliť ovládanie svojho počítača.

* Ak chcete, aby vám niekto pomohol riešiť problém na vašom počítači, zvoľte možnosť "povoliť ovládanie tohto počítača".
* Ak vy pomáhate druhej osobe, zvoľte možnosť "ovládať iný počítač".

#### Pripojenie k existujúcemu serveru (pomocou hostiteľa a kľúča) {#RemoteAccessJoin}

Predpokladáme, že osoba na druhej strane vytvorila vzdialené spojenie na existujúcom serveri, alebo takýto server vytvorila.

1. V zoznamovom rámiku Server zvoľte možnosť použiť existujúci server.
1. Do editačného poľa hostiteľ zadajte adresu servera (prípadne aj port, ak je vyžadovaný).
  * Port vložíte tak, že za adresu vložíte dvojbodku (":") nasledovanú číslom portu.
Napríklad `priklad.sk:1234`.
1. Do poľa kľúč, zadajte heslo, ktoré ste k pripojeniu dostali.
Kľúč zadajte presne v takom tvare, ako ste ho dostali.
1. Stlačte tlačidlo OK.

#### Vytvorenie spojenia s použitím existujúceho servera {#RemoteAccessConnectRelay}

Ak chcete niekomu sprístupniť ovládanie vášho počítača, najjednoduchšie je použiť už existujúci server:

1. V zoznamovom rámiku Server zvoľte možnosť použiť existujúci server.
1. Do poľa hostiteľ zadajte adresu existujúceho servera, napríklad `nvdaremote.com`.
Túto adresu tiež poskytnite osobe na druhej strane.
  * Port vložíte tak, že za adresu vložíte dvojbodku (":") nasledovanú číslom portu.
Napríklad `priklad.sk:1234`.
1. Nasleduje zadanie kľúča: Vymyslite zložitý a ťažko uhádnuteľný kľúč, prípadne na vytvorenie kľúča použite tlačidlo Vygenerovať kľúč.
Aj kľúč poskytnite osobe na druhej strane.
  * Upozorňujeme, že kľúč slúži na identifikáciu spojenia, ale tiež ako heslo, preto je dôležité, aby bol dostatočne zložitý.
1. Stlačte tlačidlo OK.
1. Poskytnite informácie o pripojení (adresa, port, kľúč) druhej osobe, aby sa mohla pripojiť.

#### Vytvorenie vlastného servera {#RemoteAccessConnectSelfHosted}

Ak nechcete použiť existujúci server, alebo sa chcete pripojiť k počítačom v lokálnej sieti, postupujte nasledovne:

1. V zoznamovom rámiku server zvoľte možnosť "Vytvoriť server".
1. Ak chcete vytvoriť pripojenie dostupné cez internet, stlačte tlačidlo "zistiť verejnú adresu IP". Takto môžete zistiť vašu verejnú IP adresu.
Túto adresu poskytnite osobe na druhej strane.
1. Ak sa nachádzajú všetky zariadenia v lokálnej sieti, budete potrebovať IP adresu pridelenú v rámci lokálnej siete.
Zisťovanie týchto údajov je však nad rámec tejto používateľskej príručky.
1. Do poľa port zadajte číslo portu, na ktorom bude prebiehať komunikácia.
Aj tento údaj poskytnite osobe na druhej strane.
  * Tento port by nemala používať iná služba.
  * Predvolene sa používa port 6837.
  * Ak sa pripájate cez internet, port musí byť otvorený a presmerovaný.
  Presmerovanie a otvorenie portov nie je popísané v tejto používateľskej príručke.
  Možno bude potrebné na otvorenie a presmerovanie portov kontaktovať administrátora vašej siete.
1. Nasleduje zadanie kľúča: Vymyslite zložitý a ťažko uhádnuteľný kľúč, prípadne na vytvorenie kľúča použite tlačidlo "Vygenerovať kľúč".
Aj kľúč poskytnite osobe na druhej strane.
  * Upozorňujeme, že kľúč slúži na identifikáciu spojenia, ale tiež ako heslo, preto je dôležité, aby bol dostatočne zložitý.
1. Stlačte tlačidlo OK.
...
### Používanie vzdialeného ovládania {#RemoteAccessUsage}

Po pripojení k vzdialenému ovládaniu môžete ovládať svoj aj pripojený počítač, zdieľať schránku a tiež stlmiť reč zo vzdialeného počítača:

* Medzi ovládaním svojho a vzdialeného počítača sa prepínate skratkou `NVDA+alt+tab`.
* Ak chcete odoslať text z vašej schránky na vzdialený počítač, na vašom počítači otvorte ponuku NVDA, nástroje, Vzdialené ovládanie a aktivujte položku odoslať schránku.
* Ak chcete dočasne stlmiť reč zo vzdialeného počítača, aby ste ju nepočuli na vašom lokálnom počítači, prejdite do ponuky NVDA, nástroje, vzdialené ovládanie a aktivujte položku stlmiť reč vzdialeného počítača.
* Ak na vzdialený počítač potrebujete odoslať príkaz `ctrl+alt+delete`, otvorte ponuku NVDA, nástroje, Vzdialené ovládanie a tu aktivujte položku Odoslať príkaz Ctrl+Alt+Del.
  Toto však bude fungovať len v prípade, že je na druhom počítači spustená nainštalovaná kópia NVDA.
  V závislosti od nastavenia vzdialeného počítača toto nemusí vždy správne fungovať.

### Zoznam príkazov vzdialeného ovládania {#RemoteAccessGestures}

<!-- KC:beginInclude -->
| Názov |klávesová skratka |popis|
|---|---|---|
| Pripojiť a odpojiť vzdialené ovládanie | `NVDA+alt+r` | zobrazí okno s pripojením k vzdialenému ovládaniu, alebo, ak ste už pripojení, odpojí sa od vzdialeného ovládania. |
| Prepnúť ovládanie | `NVDA+alt+tab` | prepína medzi ovládaním lokálneho a vzdialeného počítača. |
| Pripojiť | nie je | Spustí nové vzdialené ovládanie. Táto možnosť nie je dostupná v [bezpečnom režime](#SecureMode). |
| Skopírovať odkaz na vzdialené pripojenie | nie je | skopíruje odkaz s údajmi vzdialeného pripojenia do schránky. |
| Odpojiť | nie je | ukončí aktuálne vzdialené pripojenie. |
| Stlmiť reč vzdialeného počítača | nie je | stlmí alebo obnoví reč zo vzdialeného počítača. |
| Odoslať schránku | Nie je | odošle obsah schránky na vzdialený počítač. |
<!-- KC:endInclude -->
...
#### Zoradiť zoznam doplnkov podľa {#AddonStoreSortByColumn}

Predvolene sú doplnky zoradené podľa názvu.
V zozname "zoradiť podľa" môžete určiť usporiadanie doplnkov pre každú záložku.
Môžete si napríklad zoradiť doplnky podľa vydavateľa, verzie a podobne.
Doplnky môžete zoradiť zostupne aj vzostupne.
...
Ak stiahnutie alebo inštalácia zlyhá, môžete postup zopakovať.
Je tiež možné prerušiť inštaláciu pred zatvorením katalógu s doplnkami.
...
Ak je NVDA nainštalovaný a spustený, môžete inštaláciu doplnku začať aj tým, že priamo v správcovi súborov otvoríte súbor s doplnkom.
...
#### Zmena zdroja aktualizácie {#AddonStoreUpdateChannel}

Môžete určiť zdroj, z ktorého sa budú doplnky aktualizovať, na [záložkách nainštalované doplnky a dostupné aktualizácie doplnkov](#AddonStoreFilterStatus).
Ak sú povolené [Automatické aktualizácie doplnkov](#AutomaticAddonUpdates), doplnky sa aktualizujú zo [zdroja](#AddonStoreFilterChannel), z ktorého boli pôvodne [inštalované](#DefaultAddonUpdateChannel).
Z menu akcií pre doplnok, môžete v podmenu "zdroj aktualizácií" určiť, z akého zdroja sa bude doplnok aktualizovať.

| Možnosť |Správanie |
|---|---|
| Predvolené | Doplnok sa bude aktualizovať zo zdroja, ktorý ste určili v [nastaveniach katalógu s doplnkami](#DefaultAddonUpdateChannel) |
| Rovnaké | Doplnky sa aktualizujú podľa toho, z akého zdroja boli pôvodne nainštalované |
| Akýkoľvek | doplnky budú aktualizované na najnovšiu verziu z ktorého koľvek zdroja |
| Neaktualizovať | doplnok sa nebude automaticky aktualizovať |
| Stabilné | Aktualizovať sa bude len na najnovšiu stabilnú verziu doplnku |
| Beta alebo vo vývoji | doplnky sa budú aktualizovať na najnovšiu Beta verziu alebo verziu vo vývoji |
| Beta | doplnky sa budú aktualizovať na najnovšiu beta verziu |
| Vo vývoji | Doplnky sa budú aktualizovať na najnovšiu verziu vo vývoji |
...
Niekedy môže nastať situácia, že údaje v registroch Windows spôsobujú neštandardné správanie NVDA.
Toto môže byť spôsobené napríklad inštaláciou alebo odinštaláciou niektorých programov (napríklad Adobe Reader alebo Math Player), ale tiež aktualizáciami Windowsu a inými udalosťami.
Opravný nástroj COM sa pokúsi opraviť tento problém opätovným zapísaním kľúčov ohľadom prístupnosti do registrov.

Toto môže opraviť nasledujúce problémy:

* Oznamovanie "neznáme" alebo "panel", pri navigácii v prehliadačoch ako Firefox alebo Edge, e-mailových programoch ako napríklad Thunderbird, tiež pri navigácii vo Windows Exploreri, na panely úloh a podobne.
* Nemožnosť prepínať medzi režimom fokusu a prehliadania.
* Nemožnosť čítať popisy tlačidiel, ktoré zjavne popis majú.
* Zaostávanie pri navigácii po webových stránkach.

Keďže nástroj upravuje údaje v registroch, jeho použitie vyžaduje administrátorské práva, rovnako ako keď inštalujete program.
Ak máte zapnutú kontrolu používateľských kont, ktorá je väčšinou zapnutá, pred spustením nástroja je potrebné potvrdiť všetky výzvy, ktoré sa objavia.
...
Zoznam ďalších hlasových výstupov, ktoré je možné používať s NVDA nájdete v časti [Hlasové výstupy (anglicky)](https://github.com/nvaccess/nvda/wiki/ExtraVoices).
...
|Prejsť na znak v brailly |privolávacie tlačidlá kurzora|
...
Tieto a aj niektoré ďalšie riadky majú dva rady privolávacích tlačidiel kurzora, pričom prvý rad sa používa na získanie informácii o formátovaní.
Stlačením niektorého z privolávacích tlačidiel v prvom rade v kombinácii s pohybom panela rýchleho prístupu emulujete prepnutie prepínača.
...
Druhý krok dosiahnete tak, že stlačíte príslušné privolávacie tlačidlo kurzora a panel rýchleho prístupu v požadovanom smere.
...
|Predchádzajúci riadok |up|
|Nasledujúci riadok |dn|
|Prejsť na znak v brailly |privolávacie tlačidlá|
...
|Oznámiť formátovanie textu pod brailovým znakom |prvý rad privolávacích tlačidiel|
...
|Aktivovať prvok zameraný objektovou navigáciou |l2|
|Prepnúť nastavenie brailový kurzor zviazaný s |r2|
|Oznámiť formátovanie textu pod aktuálnym brailovým znakom |horný rad privolávacích tlačidiel|
|Predchádzajúci riadok |up|
|Posunúť riadok späť |left|
|Posunúť riadok vpred |right|
|Nasledujúci riadok |dn|
...
|Prejsť na znak v braily |`privolávacie tlačidlá`|
...
|Prepína režimi pre nastavenie "Privolávacie tlačidlá posúvajú prezerací aj systémový kurzor" |`f1+f3`, `f9+f11`|
...
Hmatový displej Dot Pad od spoločnosti Dot Inc dokáže zobrazovať text v brailovom písme aj hmatovú grafiku.
NVDA dokáže zobraziť text na brailovom riadku ale tiež viacero riadkov textu na grafickej ploche.
Model A300 má grafickú plochu s rozmermi 120 na 80 bodov, takže dokáže zobraziť 8 riadkov s dvadsiatimi bunkami.

Zdroj zobrazovaného textu (brailový riadok alebo grafická plocha), nastavíte v nastaveniach Brailovho písma, v časti zdroj.

Nvda podporuje použitie tlačidiel na posun, avšak zariadenie má málo tlačidiel, preto nie sú dostupné napríklad funkcie na privolávanie kurzora.

Keď v nastaveniach brailovho písma zvolíte ako riadok dot pad, je potrebné ručne určiť, ako je pripojený. Dostupné sú možnosti USB, alebo bluetooth virtuálny sériový port.
Ovládač nepodporuje automatickú detekciu.

Upozorňujeme, že zariadenie nedokáže správne obnovovať bunky, keď sú ruky používateľa na zobrazovacej ploche.
Pred navigáciou v texte pomocou NVDA preto vždy zodvihnite ruky a vráďte ich na plochu až keď sa text obnoví.

<!-- KC:beginInclude -->

| Názov |klávesová skratka|
|---|---|
|Posunúť riadok späť |`tlačidlo na posun doľava` |
|Posunúť riadok vpred |`tlačidlo na posun doprava` |
...
* Otváranie externých dokumentov z ponuky NVDA, ako napríklad používateľská príručka.
...
NVDA sa spúšťa v [bezpečnom režime](#SecureMode) ak je spustený na zabezpečených obrazovkách. Ak chcete mať plný prístup, použite [systémový parameter](#SystemWideParameters) `serviceDebug`.
...
|`-n LANGUAGE`|`--lang=jazyk` |Uprednostniť iné nastavenie jazyka NVDA. Použite parameter Windows, ak chcete nastaviť jazyk NVDA rovnako, ako je nastavený jazyk systému. en pre angličtinu, sk pre slovenčinu, cs pre češtinu a pod.|
...
|`-d` |`--disable-addons` |Všetky doplnky budú vypnuté|
...
Rovnako, ako je možné nainštalovať NVDA bez dialógových okien s použitím argumentu `--install-silent`, môžete pre odinštalovanie bez dialógov pre odinštalátor použiť argument `/S`.

Odinštalátor spustíte cez súbor `uninstall.exe`, ktorý nájdete v priečinku s nainštalovanou verziou NVDA, zvyčajne `%ProgramFiles(x86)%\nvda` v 64 bitových verziách systému Windows, alebo %ProgramFiles%\nvda` v 32 bitových verziách.

Odinštalátor podporuje tieto parametre:

| Krátky zápis |dlhý zápis |popis|
|---|---|---|
|`/S` |nie je |Odinštalovanie NVDa bez dialógov. |
