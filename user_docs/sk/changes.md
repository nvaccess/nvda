...
V tejto verzii predstavujeme funkciu vzdialeného ovládania. Vďaka nej môžete ovládať na diaľku iný počítač, na ktorom je nainštalovaný NVDA.

Zrýchlili sme odozvu reči, konkrétne hlasových výstupov SAPI 4, SAPI 5 a hlasov OneCore.
Hlasové výstupy SAPI 5 odteraz podporujú dvojnásobnú rýchlosť aj prepínanie jazykov.
Hlasové výstupy SAPI 4 podporujú stišovanie ostatných zvukov, odstraňovanie počiatočných medzier a udržiavanie zvukového zariadenia v pohotovostnom režime.

Vylepšili sme možnosti automatického aktualizovania doplnkov tak, že je možné vybrať zdroj aktualizácií a tiež je možné doplnky aktualizovať na pozadí.

Pridali sme príkazy na obnovenie rozpoznaného textu a tiež na zapnutie a vypnutie automatického obnovovania rozpoznaného textu.

Režim formátovaného výberu je odteraz podporovaný aj v prehliadačoch Chrome a Edge.

Vylepšili sme podporu v editoroch Ms Office a LibreOffice a Nvda odteraz lepšie oznamuje napríklad zmeny formátovania pri použití príkazov z klávesnice.

Predvolená vstupná a výstupná brailová tabuľka sa odteraz dokáže prispôsobiť jazyku NVDA.
Pridali sme podporu pre hmatový displej Dot Pad od spoločnosti Dot Inc.
Opravili sme problémy s pripojením pre viaceré brailové riadky.

Aktualizovali sme hlasový výstup eSpeak-NG, brailovú prekladovú tabuľku LibLouis a databázu Unicode CLDR.
Pridané nové brailové tabuľky pre Klinové písmo s kompaktnou diakritikou, tiež koptčinu, portugalčinu a viaceré 6 bodové tabuľky.
...
* Ak je zariadenie uzamknuté, potom sa na brailovom riadku viac nezobrazujú potenciálne citlivé informácie.
([GHSA-8f8q-2jc3-6rf4](https://github.com/nvaccess/nvda/security/advisories/GHSA-8f8q-2jc3-6rf4))
* Zabraňuje inštalátoru načítať nežiadúce DLL knižnice z aktuálneho priečinka.
([GHSA-qf5h-qw92-rx2f](https://github.com/nvaccess/nvda/security/advisories/GHSA-qf5h-qw92-rx2f))

### Nové vlastnosti

* Katalóg s doplnkami:
  * Automatické aktualizácie (#3208):
    * Odteraz je možné nastaviť zdroj (aktualizačný kanál) pre doplnky.
      * Zdroj je možné pre nainštalované doplnky nastaviť v kontextovej ponuke v podmenu "Zdroj aktualizácií".
      * Predvolený zdroj je možné nastaviť v nastaveniach NVDA vo vetve Katalóg s doplnkami.
    * Automatické aktualizácie teraz môžu bežať na pozadí.
      * Toto je možné povoliť v rovnakej vetve nastavení, ak nastavíte možnosť "automatické aktualizácie" na automaticky.
    * Odteraz je možné automaticky aktualizovať aj nekompatibilné doplnky a to na inú, novú, alebo nekompatibilnú verziu.
      * Toto je tiež možné zapnúť v kategórii Katalog s doplnkami v nastaveniach NVDA.
  * Pridaná možnosť prerušiť inštaláciu doplnkov. (#15578, @hwf1324)
  * Pridaná možnosť zopakovať inštaláciu, ak došlo k zlyhaniu stiahnutia alebo inštalácie doplnku. (#17090, @hwf1324)
  * Zoznam doplnkov je možné zoradiť podľa rôznych kritérií, vrátane dátumu vydania, a to vzostupne aj zostupne. (#15277, #16681, @nvdaes)
* Reč:
  * Automatická detekcia jazyka odteraz funguje aj pri použití hlasových výstupov cez Microsoft Speech API version 5 (SAPI5) a hlasov cez Microsoft Speech Platform. (#17146, @gexgd0419)
  * Dvojnásobnú rýchlosť je možné aktivovať aj pri hlasových výstupoch využívajúcich Microsoft Speech API 5 (SAPI5) a tiež v systéme Microsoft Speech Platform, kde je možné zrýchliť reč až šesťnásobne. (#17606, @gexgd0419)
* Webové prehliadače:
  * Režim formátovaného výberu (`NVDA+shift+f10`) je podporovaný odteraz aj v prehliadačoch Google Chrome, Edge a ďalších aplikáciách postavených na Chromium od verzie 134. (#17838)
  * V prehliadači Mozilla Firefox, NVDA oznámi vyznačený text, ak navštívite URL, ktorá vyznačený text obsahuje. (#16910, @jcsteh)
  * NVDA odteraz dokáže oznámiť, ak odkaz smeruje na tú istú stránku. (#141, @LeonarddeR, @nvdaes)
* Microsoft Office:
  * Pri úprave textových polí v Ms Powerpoint, môžete na pohyb po vetách použiť skratky `alt+šípka hore` a `alt+šípka dole`. (#17015, @LeonarddeR)
  * Pri úprave výberu v Ms Worde, Nvda správne oznamuje zmeny ak použijete skratky Wordu (`f8` alebo `shift+f8`). (#3293, @CyrilleB79)
  * V MS Worde od verzie 16.0.18226 alebo pri použití Word object model, NVDA odteraz dokáže oznámiť a zobraziť na brailovom riadku ak je nadpis zbalený. (#17499)
  * V aplikácii MS word, pri aktivovanom rozhraní UIA, NVDa oznamuje pohyb kurzora, pri použití klávesových skratiek na navigáciu v tabuľke (`alt+home`, `alt+end`, `alt+PageDown`, a `alt+PageUp`). (#17867, @CyrilleB79)
  * Ak v aplikácii Ms word stlačíte skratku na zistenie fokusu, Nvda odteraz oznámi rozloženie dokumentu, ak je táto informácia dostupná a súčasne je zapnuté oznamovanie popisov objektov. (#15088, @nvdaes)
  * Nvda odteraz oznamuje stav funkcií po použití nasledujúcich skratiek v Ms Outlooku a worde:
    * skratky na zmenu formátu písma (#10271, @CyrilleB79)
    * skratky na rozbalenie alebo zbalenie nadpisov (#17545, @CyrilleB79)
* LibreOffice:
  * Pri zmene veľkosti písma v Libre office Writer s použitím predvolených klávesových skratiek Libre office Writer, NVDA správne oznamuje veľkosť písma. (#6915, @michaelweghorn)
  * Pri použití štýlu odseku „Základný text“ alebo nadpisu pomocou príslušnej klávesovej skratky v Libre office Writeri od verzie 25.2, NVDA oznámi nový štýl odseku. (#6915, @michaelweghorn)
  * Pri prepínaní dvojitého podčiarknutia v Libre Office Writer príslušnou klávesovou skratkou, NVDA oznamuje aktuálny stav ("zapnuté", "vypnuté"). (#6915, @michaelweghorn)
  * NVDA odteraz oznamuje odsadenie prvého riadka v LibreOffice od verzie 25.8. (#13052, @michaelweghorn)
* OCR:
  * V okne s rozpoznaným textom je možné vynútiť obnovenie rozpoznaného textu klávesovou skratkou `NVDA+f5`. (#17715, @CyrilleB79)
  * Pridaný príkaz na zapnutie a vypnutie automatického obnovenia rozpoznaného textu. Skratku je možné nastaviť v dialógu Klávesové skratky. (#16897)
* Vzdialené ovládanie je odteraz priamo integrované ako súčasť NVDA. (#4390, @ctoth, @tspivey, @daiverd, dobrovoľníci a podporovatelia)
* Je možné nastaviť samostatné URL zdroje (zrkadlá) pre aktualizácie NVDA aj doplnkov. (#14974, #17151, #17310, @christopherpross)
* Pridaná podpora pre matematiku v PDF dokumentoch. (#9288, @NSoiffer)
  * Toto funguje pre vzorce s asociovaným MathML, napríklad pri súboroch generovaných v nových verziách TeX/LaTeX.
  * V súčasnosti je čítanie matematiky v pdf podporované v Foxit Readeri a amp; Foxit Editor.
* Odteraz je možné NVDA nastaviť tak, aby pri navigácii pomocou tlačidiel na brailovom riadku automaticky čítal aktuálny riadok a odsek. (#17053, @nvdaes)
* NVDA odteraz dokáže oznamovať pozíciu kurzora pri použití skratiek `alt+šípka hore` alebo `alt+šípka dole`, napríklad v editore Visual Studio Code. (#17652, @LeonarddeR)
* Pridané skratky na pohyb textového kurzora na začiatok a koniec vybratého textu `NVDA+alt+home` a `NVDA+alt+end`. (#17299, @nvdaes)
* Pridaná možnosť zakázať vypínanie obrazovky počas plynulého čítania alebo pri čítaní na brailovom riadku.
Predvolene je táto možnosť zapnutá, ale môžete ju chcieť vypnúť z dôvodu šetrenia batérie. (#17649, @LeonarddeR)
* NVDA je odteraz preložený do Bosniančiny. (#17953)
* NVDA odteraz dokáže interagovať so vzorcami v PDF dokumentoch, ak ich otvoríte v aplikácii Adobe Acrobata boli vytvorené v aktuálnych verziách Microsoft Word. (#18056)

### Zmeny

* Aktualizácie komponentov:
  * Brailová Prekladová tabuľka LibLouis aktualizovaná na verziu [3.33.0](https://github.com/liblouis/liblouis/releases/tag/v3.33.0). (#17469, #17768, @LeonarddeR, @codeofdusk)
    * Pridané nové brailové tabuľky pre Klinové písmo s kompaktnou diakritikou, tiež koptčinu, portugalčinu a viaceré 6 bodové tabuľky.
  * Databáza znakov CLDR aktualizovaná na verziu 46.0. (#17484, @OzancanKaratas)
  * Hlasový výstup eSpeak NG aktualizovaný na verziu 1.52.0. (#17056)
* Zmeny v rozhraní NVDA:
  * Inštalátor NVDA po spustení viac neprehráva zvukové logo. (#14068)
  * Dialógy na zobrazenie cieľa odkazu, informácie o formátovaní a podobne, odteraz obsahujú tlačidlá "Kopírovať" a "Zavrieť". (#17018, @XLTechie)
  * Dialóg ukončiť NVDA odteraz obsahuje možnosť reštartovať NVDA s vypnutými doplnkami a najvyššou úrovňou záznamu. (#11538, @CyrilleB79)
  * Odstránená možnosť nepoužívať WASAPI ako zvukový systém pre hlasový výstup a zvuky. (#16080)
  * V režime prehliadania bola odstránená možnosť "automaticky nastaviť systémový fokus na zamerateľnú položku", toto správanie je odteraz vypnuté. (#17598)
  * NVDA odteraz upozorní pri aktualizácii nekompatibilného doplnku len v prípade, že aktualizujete na novú verziu, ktorá je nekompatibilná s aktuálnou verziou API. (#17071, #17506)
  * Z ponuky NVDA bola odstránená položka "tím NVDA". (#16922)
  * Licencia NVDA je odteraz formátovaná ako HTML, čím zvyšujeme jej prehľadnosť. (#17600)
  * Pridané krátke formy pre parametre príkazového riadka: `-d` pre `--disable-addons` a `-n` pre `--lang`.
  Odteraz nie sú viac podporované prefixy na príkazovom riadku, ako napríklad `--di` pre `--disable-addons`. (#11644, @CyrilleB79)
  * Úpravy pre opravný nástroj COM: (#12355, @XLTechie)
    * Nástroj zobrazuje používateľsky prívetivejšie informácie, nie iba strohé varovanie. (#12351)
    * Úvodné okno je možné zatvoriť skratkami `escape` alebo `alt+f4`. (#10799)
    * Ak pri registrácii dôjde k chybe, táto chyba bude zobrazená aj používateľovi.
* Reč:
  * Hlasové výstupy Microsoft Speech API 5 a Microsoft Speech Platform, používajú na výstup rozhranie WASAPI, čo zvyšuje odozvu. (#13284, @gexgd0419)
  * Pri použití hlasových výstupov OneCore, SAPI5, a niektorých ďalších hlasov, NVDA automaticky odstraňuje počiatočné medzery, čím sa zrýchľuje odozva. (#17614, @gexgd0419)
  * Aj hlasový výstup Microsoft Speech API 4 odteraz využíva rozhranie WASAPI, čo umožňuje používanie automatického stišovania, odstraňovania počiatočných medzier na začiatkoch fráz a tiež udržiavanie aktívneho zvukového zariadenia.
  Ak počas používania hlasov SAPI 4 cez rozhranie WASAPI narazíte na problémy, môžete ho zakázať v pokročilých nastaveniach NVDA. (#17718, #17801, @gexgd0419)
  * Normalizovanie znakov unicode je odteraz v nastaveniach reči predvolene zapnuté. (#17017, @LeonarddeR).
    * Stále je ho možné vypnúť v nastaveniach reči v ponuke Možnosti NVDA.
* Nastavenia odozvy klávesnice pre "oznamovanie napísaných znakov" a "oznamovanie napísaných slov" má odteraz tri možnosti: Vypnuté, zapnuté a oznamovanie len v editačných poliach. (#17505, @Cary-rowen)
  * Predvolene je "oznamovanie napísaných znakov" nastavené len pre editačné polia.
* Predvolená vstupná a výstupná brailová tabuľka sa odteraz môže prispôsobovať jazyku NVDA. (#17306, #16390, #290, @nvdaes)

### Opravy

* Reč:
  * Opravená chyba, ktorá spôsobovala, že sa plynulé čítanie zastavilo po prvej vete, ak sa používal hlasový výstup SAPI5. (#16691, @gexgd0419)
  * Parametre hlasu, ako rýchlosť reči a hlasitosť, sa neresetujú, ak v kruhu hlasového výstupu zmeníte hlasy a používate SAPI5 a SAPI4. (#17693, #2320, @gexgd0419)
  * Opravená chyba, ktorá spôsobovala nemožnosť načítať niektoré SAPI4 hlasy (napríklad Čínske IBM hlasy). (#17726, @gexgd0419)
* Brailovo písmo:
  * Vstavaná podpora pre brailový a hmatový riadok Dot Pad od spoločnosti Dot Inc. (#17007)
  * Príkazy s medzerou a medzerou a bodmi sú správne zobrazované v dialógu klávesové skratky pri používaní zápisníkov Seika. (#17047, @school510587)
  * NVDA viac neprehráva zvuk oznamujúci chybu a nezapisuje zbytočné chyby do záznamu, ak používate zobrazovač brailu a súčasne nie je pripojený žiadny brailový riadok. (#17092, @Emil-18)
  * Ak ručne zvolíte ovládač HID ako predvolený ovládač a zobrazíte zoznam brailových riadkov, NVDA správne zobrazuje zoznam zariadení a nezobrazuje hlásenie žiadny vybratý ovládač. (#17537, @LeonarddeR)
  * Ovládač Humanware Brailliant je stabilnejší pri výbere koncového spojenia a spojenie je menej chybové. (#17537, @LeonarddeR)
  * Vlastné brailové tabuľky vo vývojárskom priečinku scratchpad sú odteraz ignorované, ak spustíte Nvda so zakázanými doplnkami. (#17565, @LeonarddeR)
  * Opravená chyba, ktorá spôsobovala, že niektoré riadky pripojené cez USB neboli rozpoznané. (#18114, @christiancomaschi)
* Microsoft Office:
  * Príkaz na oznámenie "cieľa odkazu" funguje správne aj v prrípade, že sa používa starší, už nepodporovaný objektový model v MS Worde, Outlooku, Excely a PowerPointe. (#17292, #17362, #17435, @CyrilleB79)
  * V MS Excely dialóg so zoznamom prvkov (`NVDA+f7`) viac nezlyhá pri zobrazení vzorcov a komentárov ak je jazyk systému iný ako Angličtina. (#11366, @CyrilleB79)
  * Vylepšenia pre Powerpoint:
    * Oznamovanie pozície kurzora funguje správne aj v prípade, že text obsahuje špeciálne znaky, napríklad emoji. (#17006 , @LeonarddeR)
    * Oznamovanie pozície kurzora je odteraz presnejšie (napríklad pri použití skratky `NVDA+delete`. (#9941, @LeonarddeR)
    * NVDA viac po načítaní zobrazenia snímok nespustí plynulé čítanie, ak je vypnutá možnosť "automaticky spustiť plynulé čítanie po načítaní stránky". (#17488, @LeonarddeR)
* LibreOffice:
  * Pri použití metódy vstupu Microsoft Pinyin pre Čínštinu a povolení Pinyin kompatibility režimu na použitie predošlých verzií, písanie v Libre Office Writer (a pravdepodobne aj iných aplikáciách) pri súčasnom otvorení okna s návrhmi IME viac nespôsobuje chyby. (#17198, @michaelweghorn)
  * Aktuálny stav začiarkávacích polí v dialógoch je oznamovaný aj na brailovom riadku. (#17218, @michaelweghorn)
* Matematika:
  * Opravené čítanie matematiky pre niektoré webové prvky.
  Konkrétne, MathML vo vnútry span a iných prvkov, ktoré majú príznak `role="math"`. (#15058)
  * Matematické vzorce, ktoré sú reprezentované len obrázkom s alternatívnym textom bez MathML obsahu, sú reprezentované ako štandardné obrázky a nie ako prvok MathML bez obsahu. Toto umožňuje použiť na navigáciu medzi vzorcami skratky rýchlej navigácie `g` a umožňuje čítať popis po znakoch. (#16007)
* Vývojárske nástroje IDE:
  * NVDA viac nespadne pri vybratí celého textu v poli so zdrojovým kódom v Android Studio alebo IntelliJ Idea. (#17418, @thgcode)
  * V aplikácii Visual Studio Code NVDA viac nespracúva klávesové skratky `alt+šípka hore` a `alt+šípka dole` na čítanie po vetách. (#17082, @LeonarddeR)
  * Niektoré prvky sekcií vo Visual Studio Code sú správne rozpoznané ako editovateľné. (#17573, @Cary-rowen)
  * V poznámkovom bloku a iných dokumentoch zobrazených v UIA a tiež v aplikácii Notepad++ v systéme windows 11, ak je posledný riadok prázdny, príkaz na posunutie riadka na nasledujúci riadok posunie kurzor na posledný riadok.
  Ak v akomkoľvek dokumente prejdete na poslednom riadku na nasledujúci riadok, kurzor sa presunie na koniec dokumentu. (#17251, #17430, @nvdaes)
* Konfiguračné profily:
  * Zobrazenie na brailovom riadku je funkčné aj v prípade, že sa aktivuje profil asociovaný s plynulým čítaním. (#17163, @LeonarddeR)
  * Opravená chyba, pri ktorej sa niektoré nastavenia uložili do aktívneho profilu aj v prípade, že nastavenie bolo rovnaké v základnom profile. (#17157, @leonarddeR)
* Normalizácia znakov Unicode funguje presnejšie pri hláskovaní:
  * NVDA viac po oznámení normalizovaného znaku neoznamuje všetky nasledujúce znaky ako normalizované. (#17286, @LeonarddeR)
  * Zložené znaky (ako napríklad é) sú oznamované správne. (#17295, @LeonarddeR)
* NVDA dokáže oznamovať položky vyskakovacieho podmenu vo výsledkoch hľadania v aplikácii Mozila Thunderbird. (#4708, @thgcode)
* Opravný nástroj COM viac neoznamuje úspešné dokončenie operácie, ak došlo k chybe. (#12355, @XLTechie)
* NVDA viac neoznamuje históriu schránky windows 11, ak okno zatvoríte a sú v histórii nejaké položky. (#17308, @josephsl)
* Ak v otvorenom okne režimu prehliadania so správou NVDA znovu načítate Pluginy, NVDA odteraz správne dokáže oznamovať zmeny fokusu. (#17323, @CyrilleB79)
* Ak používate aplikácie, ako Discord, Signal a Phone Link, hlasový výstup a zvuky NVDA sa počas hovoru viac nestišujú. (#17349, @jcsteh)
* Otvorenie Python konzoly viac nezlyhá, ak nie je možné získať informácie o premenných. (#17391, @CyrilleB79)
* Opravená chyba, ktorá spôsobovala, že nefungovalo oznamovanie vybratého textu v editovateľných prvkoch v prehliadačoch. (#17501, @jcsteh)
* Keď kotvové odkazy smerujú na ten istý objekt, na ktorom je umiestnený virtuálny kurzor, NVDA už nezlyhá pri posúvaní na cieľ odkazu. (#17669, @nvdaes)
* Ikona zvýraznenia textu NVDA viac neostane na panely úloh po reštartovaní procesu Explorera. (#17696, @hwf1324)
...
Urobených bolo množstvo opráv, vrátane opravy sledovania kurzora myši v aplikácii Firefox a opráv pre režim reči na vyžiadanie.
...
* Opravy podpory brailovho písma:
  ...
  * NVDA viac nestratí stabilitu po reštarte počas automatického zisťovania brailových riadkov cez Bluetooth. (#16933)
...
Skratkou `NVDA+d` je možné postupne prečítať všetky dostupné anotácie pre zameraný prvok.
Napríklad ak text obsahuje komentár a poznámku pod čiarou. (#14507, #14480)
...
* Informácia o dostupných anotáciách je zobrazená vždy aj na braillovom riadku. (#13815)
...
* Ak otvoríte ponuku NVDA zo systémového panela, NVDA viac nehlási, že je dostupná aktualizácia v prípadoch, že skutočne nie je dostupná aktualizácia. (#14523)
...
* Ak je NVDA spustený s parametrom `--lang=Windows`, je opäť možné otvoriť aj v tomto prípade nastavenia NVDA. (#14407)
* Plynulé čítanie viac nezlyhá po prechode na ďalšiu stranu v programe Kindle for PC. (#14390)
...
Pozor, stále sú známe problémy s prehliadačmi Chrome a Edge. (#13254)
  * Rámčeky znázorňujúce vizuálne zvýraznenie  položiek by mali byť správne umiestnené vo väčšine aplikácií. (#13370, #3875, #12070)
...
Toto je drobná aktualizácia, ktorej cieľom je odstrániť bezpečnostný problém.

### Opravy

* Odstránená zraniteľnosť, ktorá umožňovala z okna zobrazovač logu spustiť python konzolu NVDA na zamknutej obrazovke.
...
* Opravená chyba, ktorá spôsobovala, že NVDA neoznamoval hlásenie "zabezpečená pracovná plocha".
Toto spôsobovalo, že doplnok NVDA Remote nemohol rozpoznať zabezpečené okná. (#14094)
...
* NSIS aktualizovaný na verziu 3.08. (#9134)
...
* Opravy pre brailovo písmo:
  ...
  * Ak je brailový kurzor zviazaný automaticky a súčasne je presunutý kurzor myšy pri aktívnom sledovaní myšy,
   skratky na prezeranie textu aktualizujú obsah na brailovom riadku. (#11519)
...
* Vo vstavanej kalkulačke V systéme Windows 10 a 11, od verzie 10.1908,
NVDA oznamuje výsledky aj v prípade, že je použitých viacero príkazov, napríklad príkazy vo vedeckom režime. (#13383)
* V systéme Windows 11 je opäť možné pracovať s panelom úloh a zobrazením úloh
pomocou myšy a dotyku. (#13506)
...
Na bezpečnostné problémy prosím upozorňujte anglicky emailom na adresu<info@nvaccess.org>.

### Opravy

* Opravená bezpečnostná chyba, ktorá umožňovala v objektovej navigácii prejsť mimo zamknutú obrazovku v systémoch Windows 10 a 11. (#13328)
...
* Opravené sledovanie kurzora myši pre niektoré prvky MSHTML v aplikácii Internet Explorer. (#10736)
...
Táto verzia opravuje chybu, ktorá spôsobovala pád aplikácie Chrome pri navigácii po stránke [www.twitter.com](http://www.twitter.com). (#8777)
...
* NVDA viac neblokuje akúkoľvek interakciu v aplikácii iTunes 12.9 a novších. (#8744)
...
Táto verzia prináša aktualizácie prekladov potrebné po odstránení problematickej vlastnosti na poslednú chvíľu.
...
* Pridané brailové tabuľky: Litovčina (8 bodová), Ukrajinčina, Mongolčina skratkopis. (#7839)
* Pridaná možnosť oznamovať formátovanie textu pod brailovým znakom. (#7106)
* Po úspešnom stiahnutí aktualizácie NVDA, je samotnú inštaláciu novej verzie možné odložiť na neskôr. (#4263)
* NVDA je odteraz lokalizovaný do mongolčiny a Švajčiarskej Nemčiny.
...
* V systéme Windows 10 RS5, NVDA viac pri prepínaní úloh klávesovou skratkou alt+tab nečíta nadbitočné informácie. (#8258)
...
* NVDA sa viac neodmieta spustiť v systéme Windows 7 zobrazujúc chybovú hlášku v internej knižnici  api-ms dll, keď iná aplikácia nainštalovala do systému konkrétnu verziu Visual Studio 2017 redistributables. (#7975)
...
* Profily a konfiguračné súbory sú automaticky aktualizované aby spĺňali požiadavky po zmenách schém. Ak sa počas tejto aktualizácie vyskytne chyba, bude zobrazené upozornenie, konfigurácia bude obnovená na predvolené hodnoty a informácie o pôvodnom nastavení budú uložené do logu NVDA na úrovni 'Info'. (#6470)
...
Táto verzia opravuje pády pri používaní aplikácii Microsoft Word:

* NVDA viac nespôsobuje pád aplikácie Microsoft Word hneď po spustení v systéme  Windows XP. (#6033)
* Odstránené oznamovanie gramatických chýb, pretože to spôsobuje pády v aplikácii Microsoft Word. (#5954, #5877)
...
* NVDA dokáže prejsť do prezerania obrazovky, ak navigačný objekt neobsahuje aktuálnu pozíciu. V takomto prípade sa prezerací kurzor presunie na začiatok obrazovky. (#3454)
* Opravená chyba, ktorá v niektorých prípadoch spôsobovala zlyhanie ovládača brailového riadku Freedom Scientific, keď bola komunikácia nastavená na USB port. (#3509, #3662)
* Opravený problém, keď klávesy na brailovom riadku Freedom Scientific neboli vždy správne detegované. (#3401, #3662)
...
* NVDA viac nepadá po štarte pri nastavenej Írskej lokalizácii.
...
* Odteraz je možné používať režim prehliadania v štandardnom režime prehliadača Internet Explorer 10, napríklad  na prihlasovacej stránke [www.gmail.com](http://www.gmail.com). (#3151)
...
* Opravený preklep v angličtine pri oznamovaní zoznamov s X počtom položiek. (#1515)
...
* V programe Microsoft word správne funguje plynulé čítanie aj v prípade, že dokument nie je ukončený koncom vety.
...
* Klávesová skratka v ponuke predvolby bola v angličtine zmenená z písmena r na písmeno c.
...
* Príkaz názov okna (NVDA+t) okna oznamuje len názov, nie obsah celého objektu. Ak objekt v popredí nie je pomenovaný, NVDA oznámy názov aplikácie.
* Namiesto termínu "režim prepúšťania" NVDA vo virtuálnom prehliadači teraz používa "prehliadací režim" a "režim fokusu".
* Do konfiguračného súboru sú uložené ID hlasov namiesto ich názvov. Uchovávanie nastavení hlasu je tak robustnejšie na rôznych systémoch s rôznymi konfiguráciami. Nastavenie hlasu zo staršej konfigurácie nebude zachované a pri prvom použití hlasového výstupu bude do logu zaznamenaná chyba. (#19)
...
* NVDA má zabudovaný hlasový výstup eSpeak, ktorý vyvíja Jonathan  Duddington. Je veľmi malý s rýchlou odozvou a podporuje množstvo rôznych jazykov. Sapi kompatibilné Hlasové výstupy môžete stále používať, eSpeak bude nastavený ako predvolený.
...
* pridané parametre na použitie z príkazového riadku:
...
* pridaný portugálsky preklad (vďaka Rui Batista).
...
* Ak je v nastavení uložený ovládač hlasového výstupu, ktorý viac neexistuje alebo má problém, NVDA automaticky použije SAPI5, ak nefunguje ani sapi5, použije sa hlasový výstup silence.
...
* Pri prvom použití hlasového výstupu sa použijú jeho predvolené parametre ako sú hlas, tempo a výška, nie natvrdo uložená konfigurácia NVDA.  Rieši to problém, keď hlasové výstupy Eloquence a Viavoice cez sapi4 hovoria rýchlym tempom.
