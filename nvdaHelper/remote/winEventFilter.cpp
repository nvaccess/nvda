...
void CALLBACK winEventFilterHook(HWINEVENTHOOK hookID, DWORD eventID, HWND window, long objectID, long childID, DWORD threadID, DWORD time) {
	...
	try {
		...
		if (childID<0) {
			HWND tempWindow=window;
			while (tempWindow && !(bool)(GetWindowLong(tempWindow,GWL_STYLE)&WS_POPUP)) {
				GetClassName(tempWindow,buf,255);
				if (wcscmp(buf,L"MozillaWindowClass")==0) 
					tempWindow=GetAncestor(tempWindow,GA_PARENT);
				else break;
			}
			if (tempWindow && wcscmp(buf,L"MozillaWindowClass")==0)
				window=tempWindow;
		}
		...
		if (equal(buf,buf+7,L"Mozilla") && childID<0 && (eventID==EVENT_OBJECT_SHOW || eventID==EVENT_OBJECT_HIDE || eventID==EVENT_OBJECT_REORDER)) {
			...
			GetClassName(window,buf,255); //FIXME: Strange, may be GetParent is forgoten here? just getting the ClassName second time
		...
		if (wcscmp(buf,L"MSNHiddenWindowClass")==0) {
...
bool winEventFilter_initialize() {
	...
	BOOST_FOREACH (DWORD eventID, allowedWinEventIDs) {
		HWINEVENTHOOK hookID=SetWinEventHook(eventID,eventID,dllHandle,winEventFilterHook,0,0,WINEVENT_INCONTEXT);
